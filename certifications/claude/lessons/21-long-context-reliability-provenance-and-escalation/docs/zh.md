...
> 大上下文窗口能装下更多证据。它不能告诉你哪些证据被注意到了、是否最新、是否权威、是否可以安全地据其行动。
...
**类型：** 参考
**语言：** Python
**前置条件：** 第 17 课（Agent SDK 会话、子代理与上下文）、第 18 课（工具契约、错误与渐进发现）、第 20 课（可靠抽取、批处理与独立评审者）
**预计用时：** 约 150 分钟

## 学习目标

- 摆放与检索关键事实，降低"中部丢失"失败。
- 裁剪工具输出而不丢失来源可溯信息、错误、冲突或决策相关细节。
- 在 Agent 工作流中传播 complete、partial、blocked 三种结果状态。
- 为不同的大代码库任务使用清单、草稿区、子代理与压缩。
- 从证据与后果出发校准置信度，并对人工评审分层。
- 在摄取与渲染全程保留来源身份、日期、冲突与内容类型。
...
1. 把任务、决定、硬约束与输出契约放在证据之前。
2. 按稳定身份（文件、论断、来源或子系统）给证据分组。
3. 把当前问题紧贴在模型必须回答它的位置之前。
4. 只在最终请求附近重复极少数关键约束。
5. 为当前决定检索窄证据，而不是搬运整个档案。
...
### complete、partial、blocked 是一等状态
...
- **complete（完成）：**每个必需部分都满足输出契约。
- **partial（部分完成）：**存在有效工作，但有具名的范围或证据缺失。
- **blocked（受阻）：**安全推进需要新的权限、政策、数据或外部状态。
...
|---|---|
| 证据缺失 | 指出需要的来源与受影响的结论 |
| 权威来源冲突 | 保住双方，应用文档化优先级，或路由给负责人 |
| 政策缺口 | 停下受治理的动作，向政策负责人要一条规则 |
| 权限缺口 | 申请范围化访问，或选择一条已批准的替代路径 |
| 反复的语义失败 | 停止有界重试，请求裁决 |
| 未知外部副作用 | 重试前先对账状态 |
...
模型生成的百分比不会因为带两位小数就自动校准。用可观察证据表达置信度：

- 支撑类别：直接、计算得出、间接、冲突或缺失。
- 来源的权威性与新鲜度。
- 覆盖率：已评审项除以必需项。
- 评测者一致性与已知分歧。
- 相对已测案例的新颖度。
...
- Markdown 用标题、列表、链接与围栏代码块承载结构。
- HTML 可能含有与可见文本不同的隐藏导航、脚本或无障碍标签。
- PDF 页面可能带表格、脚注、多栏、图示与扫描图像。
- CSV 与电子表格通过行、列、公式与工作表表达关系。
- 源代码依赖符号、导入、注释、生成文件与仓库路径。
- 图像与图示需要视觉解释，外加对原始资产的引用。
...
1. 把当前目标与关键约束放在清晰的边界上。
2. 选择相关证据并携带结构化的来源可溯信封。
3. 裁剪冗词，同时保住失败、计数、冲突与引用。
4. 显式传播 complete、partial、blocked 三态。
5. 把政策、权限与歧义缺口升级给具名负责人。
6. 从证据与覆盖率校准置信度。
7. 按后果、不确定性、新颖度与随机抽检对人工评审分层。
...
- **装得下即被注意到：**把容量误当可靠注意力。
- **摘要当证据：**源身份、日期与支撑片段消失。
- **把错误也裁掉：**唯一失败的断言随重复日志一起被删。
- **partial 当没有发现：**未评审范围被转成否定证据。
- **每个失败都重试：**授权与政策缺口烧掉预算却不改变状态。
- **草稿区当数据库：**上下文一变，持久决定就蒸发。
- **压缩当验证：**更小的摘要可能保留过期假设。
- **置信度当百分比：**把措辞精度误当校准。
- **所有格式都按纯文本摄取：**表格、脚注、代码结构与渲染含义全部丢失。

### 练习

1. 重排一个 50 页的上下文数据包，让任务与关键政策保持可见，又不重复每条规则。
2. 把一份 5000 行测试日志转成结构化 partial 结果，并带指向完整工件的指针。
3. 为一个 300 文件的仓库设计一份清单与三份子代理契约。
4. 为缺失证据、政策冲突与未知副作用各写一份升级数据包。
5. 为一万条抽取记录创建分层评审计划。
6. 对比从 Markdown 表格与其渲染视图的抽取，记录丢失的关系。

## 关键术语

- **中部丢失（lost in the middle）：**被埋在长上下文中部的相关信息，其可靠利用率下降。
- **来源可溯信封：**保存来源身份、版本、日期、权威性、位置与抽取方法的元数据。
- **partial 结果：**有效的已完成工作，附带显式缺失的范围或错误。
- **清单：**范围、状态、所有权、证据与缺口的持久结构化库存。
- **草稿区：**不是权威状态的临时工作笔记。
- **压缩：**把会话上下文压缩成更小的工作集。
- **置信度校准：**让表达的确定性或路由与测得的证据及错误行为对齐。
- **分层评审：**按风险类别加代表性抽检分配人工评审。
- **内容类型渲染：**保留原始格式的结构与视觉语义。

## 延伸阅读

- [Claude Certified Architect Foundations Exam Guide](https://everpath-course-content.s3-accelerate.amazonaws.com/instructor%2F6nizmqk8tpzpfjvt6qmmav7rh%2Fpublic%2F1783542750%2FClaude+Certified+Architect+%E2%80%93+Foundations+Exam+Guide.pdf) — Claude 认证架构师基础考试指南：官方蓝图原始出处
- [Anthropic: Long context prompting tips](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/long-context-tips) — Anthropic 长上下文提示技巧：摆放与引用的官方建议
- [Anthropic: Context windows](https://platform.claude.com/docs/en/build-with-claude/context-windows) — Anthropic 上下文窗口：容量与计价的官方说明
- [Anthropic: Citations](https://platform.claude.com/docs/en/build-with-claude/citations) — Anthropic 引用：让来源可溯落到 API 层
- [Anthropic: Agent SDK context management](https://platform.claude.com/docs/en/agent-sdk/context-management) — Anthropic Agent SDK 上下文管理：压缩与上下文编辑
- [AI Engineering from Scratch：上下文工程](../../../../../phases/11-llm-engineering/05-context-engineering/)
