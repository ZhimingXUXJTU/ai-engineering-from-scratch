...
**类型：** 动手构建
**语言：** Python
**前置条件：** 架构师专业级路线核心课程（第 01-06、08-16、18、22-28 课），建议最后完成
**预计用时：** 约 8-12 小时

## 学习目标

- 为生产级 Claude 系统交付一套从业务发现到运维的架构。
- 捍卫模式、模型、上下文、RAG、集成与控制等各类决策。
- 用显式门禁证明质量、延迟、成本、安全性与安全保障。
- 打包治理、上线、运维手册与生命周期归属。
- 把同一个决策讲给高管、工程、控制与运维四类听众。
...
方案包模板、填写完成的 [`outputs/reference-architecture-packet.md`](../outputs/reference-architecture-packet.md)、填好的 [`outputs/demo-readiness-report.json`](../outputs/demo-readiness-report.json) 和 [`outputs/scored-rubric.md`](../outputs/scored-rubric.md) 是实践产物。评分后的参考方案包在点名的线上硬门禁与交接证据通过之前，对生产保持阻塞。
...
1. 为什么这个模式比最强的被否决备选更简单？
2. 哪条需求为每一次模型与 Agent 调用提供了正当性？
3. 检索返回不足或相互冲突的证据时会怎样？
4. 哪个身份到达哪个工具，权威如何核查？
5. 什么证据会阻塞一次发布？
6. 你怎么知道更便宜的变体按每个成功结果计真的更便宜？
7. 评审者能看到什么、能决定什么、能上报什么？
8. 哪些产品细节在部署前需要重新核实？
9. 每个来源、控制、指标、告警和事件分别归谁负责？
10. 什么证据会让你反转这个架构决策？
...
| 领域 | 权重 | 掌握的证据 |
|------|-------:|---------------------|
| 方案设计 | 17 | 选项贴合需求；分解与反馈显式可见 |
| 模型、提示词、上下文 | 13 | 选型与复用遵循实测权衡 |
| 集成 | 19 | RAG、协议、身份与最小权限连贯一致 |
| 评测与优化 | 16 | 有代表性的测试与运维信号驱动发布 |
| 治理与风险 | 14 | 数据、控制、评审、公平性与审批都有责任人 |
| 干系人生命周期 | 14 | 决策能落地为交付、采纳、交接与变更 |
| 开发者运维 | 7 | 团队配置、调试、运维手册与归属可用 |
...
1. 把客服情境换成一个受监管的文档分析工作流，找出哪些控制与责任人需要改变。
2. 给 `EvaluationGate` 增加统计置信度与最小样本量。
3. 在机器可读的方案包里把授权与过期来源控制设为硬门禁。
4. 让一位独立评审者找出五条没有测试或责任人的需求。
5. 在一次模拟金丝雀回归之后，记录一次真实的反转决策。

## 关键术语

| 术语 | 人们以为的意思 | 实际的意思 |
|------|-----------------|------------------------|
| 架构方案包 | 一份很长的设计文档 | 相互衔接的决策、契约、证据、控制、归属与恢复 |
| 硬门禁 | 一个高权重指标 | 一个无论平均值如何都阻塞发布的条件 |
| 就绪度 | 代码写完了 | 能证明满足需求并能安全运维失败的能力 |
| 架构答辩 | 演讲技巧 | 有证据支撑地解释选择、后果与被否决备选 |
| 运维责任人 | 部署团队 | 对 SLO、事件、变更与退役负责的角色 |

## 延伸阅读

- [Claude Certified Architect Professional exam guide](https://everpath-course-content.s3-accelerate.amazonaws.com/instructor%2F6nizmqk8tpzpfjvt6qmmav7rh%2Fpublic%2F1783542810%2FClaude+Certified+Architect+%E2%80%93+Professional+Exam+Guide.pdf) — Claude 认证架构师专业级考试指南：领域权重与题型的官方出处
- [Claude Platform documentation](https://platform.claude.com/docs/en/home) — Claude 平台文档：模型、工具与结构化输出的现行事实来源
- [Building effective agents](https://www.anthropic.com/research/building-effective-agents) — 构建有效的 Agent：Anthropic 的编排模式总览
- 架构师专业级路线的全部课程——本毕业设计的直接前置
