# The Messages API Is a State Machine | Messages API 是一台状态机
...
> **【中文解读】** 本课是开发者路线的协议基石：Messages API 是无状态的，会话状态由你的应用持有，一个放错位置的内容块就能打断整个循环。课程把一次请求建模为显式的应用状态迁移：请求进、类型化内容块出、按 stop_reason 分支、工具结果回填、再进入下一轮。框架会替你维护 messages 数组，但认证要求你在便利层之下推理：亲手把裸状态机搭一遍，之后每个 SDK、agent 框架和托管运行时都更好调试。

> **【拓展：无状态 API 的设计哲学】** Messages API 是"纯函数式"的对话接口：请求自带全部上下文，服务端不保存隐藏的聊天对象，这与传统"服务器持有会话"的聊天服务正好相反，把状态管理的责任交回应用层。考试反复考三件事：tool_result 必须连同原始 tool_use 一起回传完整对话序列；stop_reason 是控制信号而不是装饰；SDK 的便利层下面就是这台状态机。后续的 09 课（结构化输出与防御性解析）和 10 课（工具循环与受控委托）都直接建立在本课的迁移图上。
...
**Prerequisites:** [Spend Capability Where Failure Is Expensive](../../02-model-selection-and-token-economics/), [Turn a Request Into a Testable Contract](../../03-prompting-and-task-decomposition/), [Put Each Fact in the Right Kind of Context](../../04-context-knowledge-memory-and-caching/) | **前置知识:** 02 模型选择与 token 经济、03 提示与任务分解、04 上下文类型
...
- Model a Claude request as an explicit application state transition
  中文翻译：把一次 Claude 请求建模为显式的应用状态迁移。
- Choose SDK or raw REST separately from synchronous, streaming, or batch delivery
  中文翻译：把 SDK 与裸 REST 的选择，与同步、流式、批处理交付分开独立决定。
- Construct image and document content blocks with explicit asset boundaries
  中文翻译：用显式的资产边界构造图像与文档内容块。
- Preserve typed response blocks and branch on `stop_reason`
  中文翻译：保留有类型的响应块，并按 `stop_reason` 分支。
- Enforce session, retry, timeout, retention, and context-budget hygiene
  中文翻译：落实会话、重试、超时、保留期和上下文预算的卫生规范。
- Test a complete lifecycle without depending on a live API key
  中文翻译：不依赖线上 API 密钥，测试完整生命周期。

## The Failure That Teaches the Protocol | 教会你协议的那个失败

> **【中文解读】** 这段失败是全课的钩子：工程师只把工具结果放进一个全新请求，没有回传包含原始 tool_use 的 assistant 消息，于是第二次请求失败，或 Claude 表现得像从没请求过工具。原因一点都不神秘：Messages API 是无状态的，tool_result 不是独立的事实，它按 ID 回应一次特定的工具请求，而且必须在由你的代码持有的对话序列里。框架替你维护数组，所以这类错误平时看不见；认证要求你在便利层之下推理，亲手搭一次裸状态机。
...
## One Request, One Transition | 一次请求，一次状态迁移
...
## Choose Two Independent Access Patterns | 选择两个相互独立的访问模式
...
> 表格对照（zh.md 有全译）：官方 SDK 适合语言受支持、想要类型化请求响应模型与流累积助手的时候，但应用状态、stop_reason 策略、重试安全、工具授权、日志和最终校验仍归你；裸 REST 适合没有受支持 SDK、受 constrained 环境禁依赖，或需要自定义 HTTP 传输与协议级 fixture 的时候，认证头、JSON 类型、SSE 分帧、超时、重试、错误映射、前向兼容和连接清理全部自己扛。
...
> 表格对照（zh.md 有全译）：同步消息适合需要完整响应才能继续的单个交互请求；流式消息适合部分展示或首 token 时间重要的场景，完成证据是累积内容加终止 message_stop；Message Batch 适合许多可稍后完成的独立请求，按稳定的 custom_id 逐项对账，但不适合会话式工具循环。
...
## Content Is a Sequence of Typed Blocks | 内容是有类型的块序列
...
## Stop Reasons Are Control Signals | 停止原因是控制信号

> **【中文解读】** 文本会说"我现在就去查"，而响应实际是因为工具请求而停的；文本看起来完整，而生成其实是撞到 token 上限停的。所以要按协议信号分支，不要按文本语气分支。表格给全了七种 stop_reason 的应用侧解读和安全响应。铁律：绝不写 `while stop_reason != "end_turn"`，那会把每个陌生状态都变成又一次请求，造成失控循环；要写穷举分支，外加最大轮数、墙钟截止时间和每工具预算。
...
> 表格对照（zh.md 有全译）：end_turn 表示本轮完成，验证后呈现答案；tool_use 表示请求了客户端工具，走"验证、授权、执行、追加结果、继续"；max_tokens 表示输出预算用尽，视为可能不完整，只在有计划时重试；stop_sequence 是配置的序列触界，确认边界对契约有效；pause_turn 表示服务端操作可能需要延续，按现行功能专属契约处理；refusal 表示模型拒绝了请求，保留拒答并走批准的回退或升级；model_context_window_exceeded 表示填满了上下文窗口，视为截断并重做上下文预算。
...
## The Client Owns Conversation State | 客户端拥有会话状态

> **【中文解读】** 服务端不为 Messages API 保留隐藏的聊天对象，每次调用收到的都是你选择发送的上下文。这给了你控制权，也让会话卫生变成你的活：用户隔离（绝不跨租户复用 messages 数组）、系统分离（可信指令与不可信文档内容分开）、规范存储（持久化类型化块而不是拍平的转录）、上下文预算（在达到上限前压缩，同时保住事实和未了义务）、保留政策（日志前脱敏）、幂等性（网络重试不得在没有稳定操作键的情况下重复付款、发邮件或部署）。压缩长会话时，保住活跃工具请求、用户约束、已验证事实、未决问题、批准状态和来源引用；丢掉"不要发送"这句话的流利摘要，在运营上就是错的。
...
## Multimodal Requests Are Typed Asset Transfers | 多模态请求是有类型的资产传输
...
> 表格对照（zh.md 有全译）：内联 base64 是编码字节随每个请求过界，注意不要记录载荷、显式限制请求保留与大小；URL 是提供方从远端来源取资产，要授权来源、避免带密钥的 URL、计入来源方日志与可用性；Files API 的 file_id 是用标识符引用存在 API 工作区里的字节，要允许列表应用自有的 ID、记录归属与用途、强制工作区隔离、到期即删。
...
## Streaming Changes Delivery, Not Meaning | 流式改变交付方式，不改变语义
...
## Batch, Cache, and Thinking Solve Different Problems | 批处理、缓存与思考解决不同的问题

> **【中文解读】** 这三个功能经常被混为一谈，因为它们都会改变成本或延迟，但目的各不相同：Message Batches 用延迟换吞吐和批价，适合离线分类、抽取、评估、迁移；prompt caching 复用稳定前缀，把持久系统指令、工具定义和共享参考资料放在易变用户内容之前，缓存命中改善首 token 时间和输入成本，但不会扩大上下文窗口，也不会把过期事实变对；extended thinking 为受益的任务分配推理工作量，消耗预算、改变响应块、跨工具轮保留 thinking 块有专属规则，签名内容不可编辑伪造，也不要为简单抽取反射式开启。考试逻辑一句话：从工作负载选机制。
...
## Build the Lifecycle and Asset Boundary Offline | 离线构建生命周期与资产边界
...
- Stores every assistant content block.
  中文翻译：存储每一个 assistant 内容块。
- Executes requested tools.
  中文翻译：执行被请求的工具。
- Returns matching `tool_result` blocks.
  中文翻译：返回匹配的 `tool_result` 块。
...
## Application Lifecycle Beyond One Turn | 超越单轮的应用生命周期
...
> **【中文解读】** 这十二条规则就是本课的考点清单：丢消息先怀疑客户端状态而不是模型记忆；工具结果被拒先查角色顺序和 tool-use ID；输出像被截断先看 stop_reason 和用量再改提示词；要渐进展示选流式不选批处理；几千个能稍后完成的独立任务选 Message Batches；受支持的 SDK 满足传输需求就用它，但生命周期策略留在应用代码；受限运行时用裸 REST 就要为头、错误、SSE、重试和未知字段预算显式测试；资产重复出现就对比内联传输与 Files API 复用加显式删除政策；file_id 没绑定到已认证租户和工作区就在请求前拒绝；长共享前缀反复出现就评估提示缓存；重试可能重复副作用就先要求幂等或对账；出现新 stop_reason 就失效关闭并对照现行文档更新。

- If a scenario loses earlier messages, suspect client-owned state before model memory.
  中文翻译：如果情境中早先的消息丢了，先怀疑客户端持有的状态，再怀疑模型记忆。
- If a tool result is rejected, inspect role ordering and the matching tool-use ID.
  中文翻译：如果工具结果被拒绝，检查角色顺序和匹配的 tool-use ID。
- If output looks cut off, inspect `stop_reason` and usage before changing the prompt.
  中文翻译：如果输出看起来被截断，先看 `stop_reason` 和用量，再改提示词。
- If the user needs immediate progressive display, choose streaming, not batch.
  中文翻译：如果用户需要即时渐进显示，选流式，不选批处理。
- If thousands of independent jobs can finish later, choose Message Batches.
  中文翻译：如果成千上万个独立任务可以稍后完成，选 Message Batches。
...
1. Add a scripted response containing two `tool_use` blocks. Assert that both results appear in one following user message with correct IDs.
   中文翻译：加一个包含两个 `tool_use` 块的脚本化响应。断言两个结果带着正确的 ID 出现在同一条后续 user 消息里。
2. Add explicit handling for `max_tokens`. Return a typed incomplete result instead of displaying partial text as final.
   中文翻译：为 `max_tokens` 加显式处理。返回有类型的"不完整"结果，而不是把部分文本当最终答案展示。
3. Simulate a stream that disconnects before `message_stop`. Record an incomplete attempt and prove that no irreversible action runs.
   中文翻译：模拟一条在 `message_stop` 之前断开的流。记录一次未完成尝试，并证明没有不可逆动作运行。
4. Add tenant and prompt-version metadata to a trace without storing the raw user message.
   中文翻译：给 trace 加租户和提示词版本元数据，但不存原始用户消息。
5. Extend the multimodal fixture with a URL-backed image. Record its origin, authorization, retention, and failure boundaries without making a network call.
   中文翻译：用 URL 来源的图像扩展多模态 fixture。不发网络请求，记录它的来源、授权、保留和失败边界。

## Further Reading | 延伸阅读

- [Messages API reference](https://platform.claude.com/docs/en/api/messages)
  中文翻译：Messages API 参考。
