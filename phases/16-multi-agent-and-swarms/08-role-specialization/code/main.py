...
核心概念：角色专业化流水线——Planner（规格制定）、Executor（代码生成）、Critic（静态审查）、
Verifier（沙箱测试执行），Critic 能捕获语法问题但会被语义正确的错误代码欺骗，
只有 Verifier 通过实际运行测试才能捕获深层 bug
AI 对应：MetaGPT 的 SoftwareCompany 流水线（ProductManager -> Architect -> Engineer -> QA）
采用相同的多角色分工；SWE-Agent (Princeton) 使用 Generate + Verify 两阶段；
Anthropic 的 "Building effective agents" 推荐在 Agent 循环中加入验证步骤；
MAST (Multi-Agent Software Team) 失效模式研究指出纯 LLM 审查不可靠
"""
