...
> ⚠️ **【易错点】** 场景：跳过 schema 验证直接执行 / 后果：弱模型（Haiku、GPT-4o-mini）会幻觉字段（如 `get_weather({ cityy: "Tokyo" })` 拼错 key），执行器要么 KeyError 崩溃要么拿到 None 走错分支 / 修复：在执行器前必加 schema 验证，失败时把错误以 `tool_result` 形式返回给模型让它重试，而不是抛异常给宿主。
...
> Meta 2026 年 agent 安全的"二选一规则"指出，单次交互最多只能组合以下三项中的两项：不可信输入、敏感数据、后果性动作。工具接口是执行该规则的地方——通过拒绝调用、要求用户确认或升级权限范围。完整安全章节见 Phase 13 · 15，agent 级权限策略见 Phase 14 · 09。
...
- [Model Context Protocol — Specification 2026-07-28](https://modelcontextprotocol.io/specification/2026-07-28) — the current stateless, provider-agnostic generalization of the tool interface
  中文翻译：MCP 2026-07-28 规范——当前无状态、供应商无关的工具接口泛化
