...
核心概念：AI 网关路由 + 回退策略——网关前置 OpenAI/Anthropic/Self-hosted 三个供应商，
注入 429/5xx 错误后比较回退效果，单供应商 97% 成功率 -> 两级回退 99.94% ->
三级回退 99.997%，不同网关的延迟开销（Cloudflare 2ms vs Portkey 30ms）
AI 对应：LiteLLM 是最流行的开源 LLM 网关（统一 OpenAI/Anthropic/自托管 API）；
Portkey AI 提供企业级 AI 网关（可观测性 + 路由 + 回退）；
Cloudflare Workers AI 和 Kong AI Gateway 提供边缘级 AI 网关；
OpenRouter 聚合多供应商 API；所有网关都实现 fallback/retry/circuit-breaker 模式
"""
