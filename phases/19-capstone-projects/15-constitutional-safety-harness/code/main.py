...
核心概念：宪法安全线束 + 红队靶场——五层安全管线（输入消毒 → NeMo Guardrails → Llama Guard 4 →
X-Guard 多语言 → PII 输出过滤）+ 六种攻击族（PAIR/TAP/GCG/编码/多语言/多轮）CVSS 评分红队
AI 对应：NVIDIA NeMo Guardrails 是开源的 LLM 对话安全框架；
Llama Guard 4 (Meta) 和 ShieldGemma 是内容安全分类器；
TAP (Mehrotra et al. 2023) 和 GCG (Zou et al. 2023) 是自动化越狱攻击的代表性方法；
CVSS 评分用于量化安全风险严重程度
"""
