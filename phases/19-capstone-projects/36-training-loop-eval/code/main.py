...
核心概念：LLM 训练循环与评估——交叉熵损失 + held-out 评估 + 生成质量探测 +
AdamW 优化器 + 线性 warmup + 余弦衰减学习率 + 梯度裁剪 + JSONL 损失日志
AI 对应：NanoGPT (Karpathy) 的训练循环是此模式的开源参考实现；
AdamW (Loshchilov & Hutter 2017) 是 LLM 训练的标准优化器；
cosine learning rate schedule 被 GPT-3 和 Llama 系列采用；
JSONL 日志是训练可复现性的标准实践
"""
