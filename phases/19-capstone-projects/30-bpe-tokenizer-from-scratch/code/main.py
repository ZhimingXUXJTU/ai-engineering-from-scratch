...
核心概念：字节级 BPE（Byte-Pair Encoding）分词器从零实现——在小语料上训练词汇表，
通过迭代合并最高频字节对构建子词单元，然后编码和解码文本验证分词质量
AI 对应：GPT-2/3/4 使用字节级 BPE 作为分词器（tiktoken 是 OpenAI 的高效实现）；
SentencePiece 被 Llama 和 Mistral 系列采用；HuggingFace tokenizers 库提供 BPE/WordPiece/Unigram
三种算法；分词器质量直接影响 LLM 的多语言能力和推理效率
"""
