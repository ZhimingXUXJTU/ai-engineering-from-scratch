...
> BLEU、ROUGE-L、F1、精确匹配、准确率。五个仍然占已发表 LLM 评测数字大头的指标。把每一个都从第一性原理实现一遍，你才知道那个数字意味着什么。
...
**类型：** 动手构建
**语言：** Python
**前置条件：** Phase 19 Track B 基础；70 课（任务规格格式）
**预计用时：** 约 90 分钟

## 学习目标

- 用显式的分词规则实现 token 级精确匹配、F1 和准确率。
- 从零实现 BLEU-4：修改版 n-gram 精确率、n=1..4 的几何平均、短句惩罚。
- 用最长公共子序列实现 ROUGE-L，以 F-beta 组合精确率与召回率。
- 按 70 课的 metric_name 字段分发，让运行器对指标保持无关。
- 用手工演算的参考向量钉死行为，而不是依赖第三方库。
