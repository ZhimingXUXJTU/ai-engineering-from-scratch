...
核心概念：CodeAct vs JSON tool-call 脚手架对比 —— 两种 Agent 架构使用同一个确定性模型，
对比指标：任务解决数、使用轮数、每操作爆炸半径（可触及文件数）。
核心教训：脚手架（scaffolding）是承重墙，不是装饰。

AI 对应：OpenHands（原 OpenDevin，arXiv:2407.16741）明确选择了 CodeAct（Agent 直接写代码）
而非 JSON 工具调用。Devin、SWE-Agent 也是 CodeAct 路线。而 Claude Code、ChatGPT Code Interpreter
则采用 JSON 工具调用路线。两种路线的选择影响 Agent 的能力边界和安全性。
"""
