...
核心概念：Llama Guard 风格的输入/输出分类器 —— 基于 MLCommons 分类法的关键词检测器，
展示关键词检测的优势（原始滥用）和局限（emoji 走私、同形字替换）。
normalize 管线（NFKC + 同形字映射 + 不可见字符移除）是预处理器对绕过的防御。
输出防护栏（output rail）捕获输入护栏遗漏的风险类别。
AI 对应：Meta 的 Llama Guard 3 和 Prompt Guard 是开源安全分类器的标杆，
MLCommons AI Safety v0.5 分类法是行业标准；NeMo Guardrails（NVIDIA）、
Azure Content Safety 和 OpenAI Moderation API 都提供类似的输入/输出分类。
同形字攻击（homoglyph attack）是 Unicode 安全的经典问题，OWASP 和 NIST 都有相关指南。
"""
