...
> 🔗 **【前置】** 学本节前请先掌握：Phase 15·12（Durable Execution）、Phase 15·15（Propose-then-Commit）、数据库事务（ACID）。本节把"持久执行"+"提议-提交"组合成完整安全网。
> 💡 **【类比】** 检查点回滚 = "游戏的存档与读档"。检查点 = 自动存档（每通过一关存一次）；回滚 = 读档（这关打错了回到上一关）。后果性动作四件套 = 幂等键（防止重启双倍执行）+ 前置条件（重启后世界状态仍符合预期）+ 动作后验证（确认副作用真的发生）+ 失败回滚（恢复到动作前）。这四件套缺一不可，类似数据库事务的 ACID。
> ⚠️ **【易错点】** 只有幂等键没有前置条件检查 → 重启时余额已被其他流程改了仍然执行 → 透支。修复：恢复时必须重检"批准时的世界状态"是否还成立（如"余额 > $1000"在恢复时是否仍成立）。
