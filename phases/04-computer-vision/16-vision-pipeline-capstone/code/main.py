"""
构建完整视觉流水线 — 毕业项目 (Build a Complete Vision Pipeline — Capstone)

核心概念：
- 生产级视觉系统：多模型串联 + 数据契约 + 前后处理规则
- 端到端流水线：数据加载 → 预处理 → 模型推理 → 后处理 → 结果输出
- 工程实践：批处理、异步、缓存、错误处理

AI 对应：
- 实际 AI 产品中，模型只占 20% 的工作量，80% 是数据管道和系统集成
- 自动驾驶系统可能串联：检测→跟踪→分割→深度估计→决策
"""
