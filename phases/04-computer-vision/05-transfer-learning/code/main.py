...
核心概念：
- 特征提取：冻结预训练骨干，只训练分类头（适合小数据集）
- 微调：解冻部分层，使用判别式学习率（浅层小学习率，深层大学习率）
- 三大陷阱：学习率过高破坏预训练特征、BN 统计在小数据集上崩溃、灾难性遗忘

AI 对应：
- 迁移学习是工业界最常用的技术——预训练 ImageNet 模型 + 自定义数据微调
- LoRA（低秩适配）是迁移学习在大模型时代的进化形式
"""
