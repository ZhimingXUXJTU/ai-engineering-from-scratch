...
**类型：** 构建
**语言：** Python
**前置条件：** 阶段 3（深度学习基础），阶段 5 第 10 课（序列到序列）
**时长：** 约 90 分钟
...
分数 = Q @ K^T    形状：(n, n)
              ...
              k1    k2    k3    k4    k5
        +-----+-----+-----+-----+-----+
   q1   | 2.1 | 0.3 | 0.1 | 0.8 | 0.2 |   <- q1 对每个键的关注度
        +-----+-----+-----+-----+-----+
...
print(f"总和:     {softmax(logits).sum():.4f}")
```
...
1. 修改 `scaled_dot_product_attention` 以接受可选的掩码矩阵，在 softmax 前将某些位置设为负无穷（这就是因果/解码器掩码的工作方式）
2. 从零实现多头注意力：将 Q、K、V 拆分为 `n_heads` 块，分别运行注意力，拼接，并通过最终权重矩阵 Wo 投影
...
| 术语 | 人们的说法 | 实际含义 |
|------|-----------|---------|
| 查询 (Query, Q) | "问题向量" | 输入的学习投影，表示这个 token 在寻找什么信息 |
| 键 (Key, K) | "标签向量" | 学习投影，表示这个 token 包含什么信息，与查询匹配 |
| 值 (Value, V) | "内容向量" | 学习投影，携带根据注意力分数聚合的实际信息 |
| 缩放点积注意力 | "注意力公式" | softmax(QK^T / sqrt(dk)) @ V —— 缩放防止高维时 softmax 饱和 |
| 自注意力 | "token 看自己和其他 token" | Q、K、V 都来自同一序列的注意力，让每个位置关注所有其他位置 |
| 注意力权重 | "多少关注" | 位置上的概率分布，由缩放点积上的 softmax 产生 |
