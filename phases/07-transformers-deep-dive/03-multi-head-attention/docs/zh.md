...
![多头注意力拆分、计算、拼接](../assets/multi-head-attention.svg)
...
# scaled_dot_product_attention 在 CUDA 上自动调用 Flash Attention。
...
1. **简单。** 取 `code/main.py` 中的 MHA，在 `d_model=64` 固定的情况下将 `n_heads` 从 1 改为 16。在一个合成复制任务上绘制微型单层模型的损失。更多头有帮助、达到平台期还是有害？
2. **中等。** 实现 MQA（一个 KV 头在所有查询头间共享）。测量与完整 MHA 相比参数量下降多少。计算在 N=2048 的推理时 KV 缓存大小缩减多少。
3. **困难。** 实现迷你版的 Multi-head Latent Attention：将 K,V 压缩为秩 `r` 的隐向量，在 KV 缓存中存储隐向量，在注意力计算时解压。在什么 `r` 值下缓存内存降到完整 MHA 的 1/8 以下，同时质量保持在验证困惑度的 1 bit 以内？

## 术语速查表

| 术语 | 人们的说法 | 实际含义 |
|------|-----------|---------|
| 头 (Head) | "一个注意力电路" | 维度为 `d_head = d_model / n_heads` 的一个 Q/K/V 投影，有自己的注意力矩阵。 |
| d_head | "头维度" | 每个头的隐藏宽度；生产中几乎总是 64 或 128。 |
| 拆分/合并 | "reshape 技巧" | 围绕注意力的 `(N, d_model) ↔ (n_heads, N, d_head)` reshape+transpose。 |
| W_o | "输出投影" | 拼接头后应用的 `(d_model, d_model)` 矩阵；头混合的地方。 |
| MQA | "一个 KV 头" | 多查询注意力：单个共享的 K/V 投影。最小 KV 缓存，有一些质量损失。 |
| GQA | "Llama 2 之后的默认" | 分组查询注意力，`n_kv_heads < n_heads`；重复以匹配 Q。 |
| MLA | "DeepSeek 的技巧" | 多头潜在注意力：K,V 压缩为低秩隐向量，在注意力计算时解压。 |
| 归纳头 (Induction head) | "上下文学习背后的电路" | 一对检测先前出现模式并复制后续内容的头。 |

## 延伸阅读

- [Vaswani 等人（2017）。Attention Is All You Need §3.2.2](https://arxiv.org/abs/1706.03762) —— 原始多头规范。
- [Shazeer（2019）。Fast Transformer Decoding: One Write-Head is All You Need](https://arxiv.org/abs/1911.02150) —— MQA 论文。
- [Ainslie 等人（2023）。GQA: Training Generalized Multi-Query Transformer Models from Multi-Head Checkpoints](https://arxiv.org/abs/2305.13245) —— 训练后如何将 MHA 转换为 GQA。
- [DeepSeek-AI（2024）。DeepSeek-V2 Technical Report](https://arxiv.org/abs/2405.04434) —— MLA 及为何在缓存内存上击败 MHA/GQA。
- [Olsson 等人（2022）。In-context Learning and Induction Heads](https://transformer-circuits.pub/2022/in-context-learning-and-induction-heads/index.html) —— 对头实际功能的机制分析。
