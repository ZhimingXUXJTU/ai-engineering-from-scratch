...
**类型：** 构建
**语言：** Python (标准库， toy embedding-based bias probe)
**前置条件：** Phase 05 (word embeddings), Phase 18 · 01 (instruction following)
**时间：** 约 60 分钟
...
### 三类评估指标 categories (Gallegos et al. 2024)
...
1. 运行 `code/main.py`. Report WEAT-style bias scores before and after the debiasing step. Explain why the metric does not drop to zero.

2. 扩展the probe with an intersectional test: (gender, race) x (career, family). Report cross-axis bias scores.

3. 阅读 An et al. 2025 (PNAS Nexus). Identify the two intersectional effects they report that single-axis gender evaluation would miss.
