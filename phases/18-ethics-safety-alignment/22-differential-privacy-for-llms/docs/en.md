...
> 定义 (epsilon, delta)-差分隐私并说明 DP-SGD 方法。解释 2024-2025 年张力：金丝雀 MIA vs 训练数据提取给出不同图景。描述 PMixED 及为什么推理时私有预测是 DP 训练的替代。描述通过 LLM 反馈的差分隐私逆转攻击。
...
> LLM 会记忆。Carlini 等人 2021 年展示生产语言模型可按需复现逐字训练文本。DP 是形式化防御：训练使输出对任何单个训练样本不可敏感。2024-2025 年证据显示 DP-SGD 是必要的但部署的 ε 值可能与威胁模型不匹配。
...
> 随机算法 M 是 (ε, δ)-DP 的，如果对于任何两个相差一个示例的数据集和任何事件 S：P(M(D) in S) <= e^ε * P(M(D') in S) + δ。
...
> 解释：输出分布足够接近（由 ε 参数化），任何单个个体的贡献都不能被可靠推断，除了概率 δ。
...
> DP-SGD 标准方法：1. 采样小批次。2. 计算逐例梯度。3. 裁剪每个梯度到阈值 C。4. 求和裁剪后的梯度并添加高斯噪声。5. 使用噪声和更新参数。
...
> 隐私成本由计数器跟踪。文献中报告的 ε 值因威胁模型而异；没有普遍"安全"的默认 ε。较低的 ε 通常需要更多噪声并可能增加效用损失。
...
> 全量 DP-SGD 训练前沿模型成本过高。LoRA 限制梯度更新到小型适配器，减少逐例梯度存储。LoRA + DP-SGD 是 2025 年常见配置。DP 保证适用于适配器；基础模型保持固定。
...
> 金丝雀 MIA 报告有限成功，暗示 MIA 困难。训练数据提取报告大量记忆，暗示 MIA 在相关意义上容易。
...
> 2025 年 3 月解决方案：两者测量不同东西。MIA 问"示例 e 在 D 中吗？"，提取问"我能恢复 D 的什么？"——"最可提取的"示例才是隐私的关键。
...
> 新金丝雀设计。无影子模型的基于损失的 MIA。首次对真实数据上具有实际 DP 保证的 LLM 进行非平凡 DP 审计。
...
> 2025 年新兴攻击：使用 DP 训练模型的置信分数作为预言机重新识别个体。即使输出不泄露，置信分布也可能泄露。
...
> Lessons 20-21 是偏见/公平。Lesson 22 是隐私。Lesson 23 是通过水印的来源。Lesson 27 涵盖监管数据来源层。
...
> `code/main.py` 在玩具二分类数据集上模拟 DP-SGD。你可以扫描噪声乘数 σ 和裁剪范数 C，跟踪 (ε, δ) 预算和准确率成本。"金丝雀攻击"插入唯一训练样本并测量对数损失测试是否能在 DP 前后检测到。
...
> 本课产出 `outputs/skill-dp-audit.md`。给定语言模型部署上的 DP 声明，审计：(ε, δ) 值、使用的计数器、MIA 评估协议以及是否评估了置信暴露向量。
