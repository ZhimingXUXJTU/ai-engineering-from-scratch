...
**类型：** 构建
**语言：** Python (标准库， DP-SGD noise-injection and ε-δ accountant demonstration)
**前置条件：** Phase 01 · 09 (information theory), Phase 10 · 01 (large-model training)
**时间：** 约 60 分钟
...
2. 实现a canary insertion and a log-loss test. Measure detection rate before and after DP-SGD at σ = 1.0.

3. 阅读 Nasr et al. 2025 on training-data extraction. Why does extraction success not collapse under moderate ε? What does this imply about MIA-as-evaluation?

4. 设计一个deployment using PMixED (arXiv:2403.15638) that operates entirely at inference time. What is the threat model that PMixED addresses that DP-SGD does not?

5. 草拟the DP Reversal via LLM Feedback attack. Design a countermeasure that limits confidence-score leakage and estimate its deployment cost.
