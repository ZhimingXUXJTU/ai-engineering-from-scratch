...
**类型：** 构建
**语言：** Python (标准库， ArtPrompt token-masking harness)
**前置条件：** Phase 18 · 12 (PAIR), Phase 18 · 13 (MSJ)
**时间：** 约 60 分钟
...
2. 实现a second encoding: base64 for the same target word. Compare the filter-bypass rate against ArtPrompt and the recovery difficulty.

3. 阅读 Jiang et al. 2024 Section 4.3 (five-model results). Propose a reason why Claude's ArtPrompt-resistance is higher than Gemini's on the same benchmark.

4. 设计一个pre-generation defense that detects ASCII-art-shaped regions in the prompt. Measure the false-positive rate on legitimate code, tables, and mathematical notation.
