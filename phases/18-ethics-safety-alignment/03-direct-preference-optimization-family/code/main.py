...
核心概念：DPO 及其变体族（IPO、KTO、SimPO、ORPO、BPO）的成对偏好优化损失函数，
无需训练奖励模型即可直接从偏好数据学习策略，六种方法在同一 toy 数据上对比
AI 对应：DPO (Rafailov et al. 2023) 被 Meta 的 Llama 2/3、Mistral 等广泛采用；
IPO (Azar et al. 2024) 提供理论上更好的 KL 约束；KTO 不需要成对数据；
SimPO 被 Apple 的 OpenELM 采用；ORPO 被用作微软 Phi 系列的训练组件
"""
