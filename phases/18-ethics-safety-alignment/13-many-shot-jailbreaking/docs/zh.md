...
**类型：** 学习
**语言：** Python (标准库， in-context learning vs MSJ simulator)
**前置条件：** Phase 18 · 12 (PAIR), Phase 10 · 04 (in-context learning)
**时间：** 约 45 分钟
...
2. 实现a simple MSJ defense: run a classifier over the full context; if N pattern-match examples of harmful-compliance pairs are detected, truncate or rewrite. Measure the new shot-vs-ASR curve.

3. 阅读 Anil et al. 2024 Figure 3 (power law by category). Explain why violent/deceitful content needs fewer shots to jailbreak than other categories.

4. 设计一个prompt that combines PAIR iteration (Lesson 12) with MSJ. Argue whether the compound attack is worse than MSJ alone, and for which model behaviours.
