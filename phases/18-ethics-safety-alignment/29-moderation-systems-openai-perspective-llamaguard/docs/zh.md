...
**类型：** 构建
**语言：** Python (标准库， three-layer moderation harness)
**前置条件：** Phase 18 · 16 (Llama Guard / Garak / PyRIT)
**时间：** 约 60 分钟
...
1. 运行 `code/main.py`. Run a benign, borderline, and harmful input through all three layers. Report which layer fires for each.

2. 扩展the harness with Perspective-API-style toxicity scoring on a specific category. Compare its threshold behaviour to the category score.

3. 阅读 the OpenAI Moderation API docs and the Llama Guard 3 category list. Map each OpenAI category to the closest Llama Guard categories. Identify three categories that do not cleanly map.

4. 设计一个moderation stack for a code-assistant deployment (e.g., GitHub Copilot). Identify the categories most and least relevant and propose custom rules.
