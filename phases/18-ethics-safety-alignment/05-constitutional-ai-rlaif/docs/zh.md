...
**类型：** 学习
**语言：** Python (标准库， toy self-critique-and-revise loop)
**前置条件：** Phase 18 · 01 (InstructGPT), Phase 18 · 02 (Reward hacking)
**时间：** 约 60 分钟
...
1. 运行 `code/main.py`. Compare the base model's harmful-token rate to the CAI-trained version. How many revision steps are needed to approach zero?
...
3. 设计一个constitution for an AI coding assistant. Specify Tier 1 (catastrophic: destructive commands without approval), Tier 2, Tier 3, Tier 4. Keep each tier to 3-5 principles.
...
5. 阅读 Constitutional Classifiers v2 methodology (if available). Explain why ~1% compute overhead is a qualitatively different safety story than 23.7%.
