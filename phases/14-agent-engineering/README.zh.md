...
**前置条件：** Phase 11（LLM 工程）与 Phase 13（工具与协议）。

**第一课：** [Agent 循环](01-the-agent-loop/)

不需要学完整阶段时，有两条聚焦路线可选：

- [Agent 辅助工程](../../learning-paths/using-coding-agents.json)：覆盖任务框定、仓库证据、执行框架、隔离、验证、评审与持久反馈。
- [产品判断与交付](../../learning-paths/shaping-the-build.json)：覆盖结果定义、工作流发现、假设、切片、规格、指标、分阶段发布与反馈归属。
...
保留命令、退出码、"思考-行动-观察"轨迹、最终答案、轮数，以及结束运行的确切停止条件。

**下一步：** 换一个脚本任务或轮数预算，预测效果，然后继续到 [ReWOO 与规划-执行](02-rewoo-plan-and-execute/)。
...
| 43 | 先定任务，再写代码 | 学习+构建 | ~60min |
| 44 | 从证据出发做计划 | 学习+构建 | ~65min |
| 45 | 带隔离地委托 | 学习+构建 | ~70min |
| 46 | 把反馈变成系统 | 学习+构建 | ~65min |
| 47 | 结果先于产出 | 学习+构建 | ~60min |
| 48 | 发现真实工作流 | 学习+构建 | ~70min |
| 49 | 映射假设，先拆最大风险 | 学习+构建 | ~65min |
| 50 | 选能改变决策的最小切片 | 学习+构建 | ~65min |
| 51 | 写保留判断力的规格 | 学习+构建 | ~75min |
| 52 | 结果出来前，先定成功指标 | 学习+构建 | ~70min |
| 53 | 原型、试点还是生产：三档慎选 | 学习+构建 | ~70min |
| 54 | 反馈棘轮：改进有主，控制可退役 | 学习+构建 | ~75min |

</details>

> **【中文解读】** 43-54 是 2026-09 上游新增的"Agent 工程实践"方法论系列：如何在真实组织里做 Agent 项目——定任务、做计划、委托、收反馈、定指标、选交付档位。它们不写算法，写的是判断力；适合在任何实战项目开始前通读一遍。
