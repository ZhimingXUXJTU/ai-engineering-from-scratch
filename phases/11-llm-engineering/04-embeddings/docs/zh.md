...
**类型：** Build
**语言：** Python
**前置知识：** Phase 11, Lesson 01 (Prompt Engineering)
**时长：** ~75 minutes
**相关：** Phase 5 · 22 (Embedding Models Deep Dive) covers dense vs sparse vs multi-vector, Matryoshka truncation, and per-axis model selection. This lesson focuses on the production pipeline (vector DBs, HNSW, similarity math). Read Phase 5 · 22 before picking a model.
