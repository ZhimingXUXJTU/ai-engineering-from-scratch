...
> **【中文解读】** 学习目标：1) 实现 SFT 将基础模型转化为指令跟随助手；2) 使用聊天模板格式化训练数据（system/user/assistant 角色），并在非 assistant token 上屏蔽损失；3) 理解为什么需要 SFT（基础模型是续写而非回答）；4) 对比基础模型与微调模型的响应质量。
...
> **【中文解读】** GPT-3（基础模型）与 ChatGPT（指令微调）的差距在于 2 万至 10 万条精心制作的 (instruction, response) 对。Stanford Alpaca 用 52,000 条数据、$600 成本微调 Llama 7B，效果接近 ChatGPT。Meta Llama 2 Chat 仅用 ~27,000 条高质量示例做初始 SFT。核心洞察：质量比数量更重要。
...
> **【中文解读】** SFT 不教模型新事实——巴黎是法国首都这种知识在预训练中就学会了。SFT 教的是新行为：看到问题→产生答案；看到指令→产生完成；看到有害请求→产生拒绝。预训练给模型知识，SFT 给模型礼貌。
