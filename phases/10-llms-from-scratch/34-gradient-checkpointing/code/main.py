...
核心概念：
  - 标准反向传播：前向时保存所有中间激活值，反向时直接使用，显存占用 O(layers)
  - 梯度检查点：只保存部分层的激活值（检查点），需要时重新计算其余层
  - 典型策略：每隔 sqrt(n) 层设一个检查点，将显存从 O(n) 降到 O(sqrt(n))
  - 代价：反向传播需要额外 ~33% 的前向计算（被检查的层需要重新计算）

AI 对应：
  - PyTorch 的 torch.utils.checkpoint 就是这个原理
  - GPT-4 / Llama 3 等大模型的训练都使用梯度检查点来降低显存需求
  - 与激活量化、CPU offloading 一起构成"长上下文训练"的三板斧
"""
