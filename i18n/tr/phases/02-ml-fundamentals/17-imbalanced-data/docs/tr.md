...
> %99'ın verileri normal olduğunda doğruluk oranı bir yalan.
...
- SMOTE'yi sıfırdan uygulayın ve sentetik aşırı örnekleme ile rastgele çiftleme arasındaki farkı açıklayın
  SMOTE'yi sıfırdan gerçekleştirmek, yapılmış örnekle yapılmış kopyalama arasındaki farkı açıklamak
...
- Sınıf ağırlığını, eşiğin ayarlanmasını ve yeniden örnekleme stratejilerini karşılaştırın ve verilen dengesizlik oranı için doğru yaklaşımı seçin
   değer düzenleme ve  ağırlık ölçüm stratejisi, doğru yöntem seçmek için belirlenmiş dengesiz oran
- SMOTE, sınıf ağırlıkları ve eşiğin optimize edilmesini birleştiren tam dengesiz bir veri boru hattı oluşturun
  KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ KATSİ Kİ Kİ Kİ Kİ 


> **【中文解读】**
> Dönemsiz veri arasında az sayıda sınıf çok azdır.

> **【拓展：不平衡数据在真实系统中的挑战】**
> PayPal günde yaklaşık 400 milyar işlem işlemeyi halleder, sahtekarlık oranı sadece %0,3'dir, ancak her gün hala yaklaşık 120 milyon sahtekarlık işlemini ifade eder. Doğru oranla değerlendirmek anlamsızdır. %99.7'lik doğru oranı "sahtekarlık dışı" olarak değerlendirilebilir. F1 ̊AUPRC vb. göstergeleri kullanmak zorundadır.
...
> Sen bir sahtekarlık test modeli oluşturduğunuzu... %99.9 doğruluk oranını elde ettiğini... sonra fark ettin ki, her işlem için "sahtekarlıktan uzak" bir tahmin yapıyor.
...
> Bu bir hata değil. Sadece %0,1'lik bir işlem dolandırıcılık olduğunda, bu mantıklı bir davranıştır.
...
> Bu, gerçekten ihtiyaç duyulan yerlerde gerçekleşir. Hastalık teşhisi: %1 陽性率. 网络入侵: 0.01% 攻击.
...
> 准确率失败因为它同等待待待所有正确预测──正确标记合法交易和正确捕获欺诈率的分之一──但捕获欺诈是模型存在的全部原因──我们需要迫使模型关注稀有但重要类别的标志,技术和训练策略──

> **【中文解读】**
> Doğruluk oranı %99.9'dur. Doğru uygulama: 1) F1'yi kullanmak için                                                                                                                                                                                                                                                    
...
> Model, az sayıda aldatmacılığı ve hastalıkları yakaladı. Ama doğruluk oranı %99'u gösterdi.

### Daha İyi Ölçümler

**Precision**TP + FP. Pozitif olarak işaretlenen her şeyden, aslında kaç tane var?

> **精确率**= TP / (TP + FP) ◊ tüm doğru olarak belirlenen örnekler arasında, ne kadar gerçek doğru var?
...
> **召回率**= TP / (TP + FN) ◊ tüm gerçekle doğru örnekler arasında, ne kadar yakaladık?

**F1 Score**= 2 * doğruluk * geri çağırma / (doğrulık + geri çağırma). Harmonik ortalama.

> **F1 分数**= 2 * 精确率 * 召回率 / (精确率 + 召回率) 调和平均值──比算术平均值更严厉地惩罚精确率与召回率之间极端不平衡──
...
> **F-beta 分数**= (1 + beta^2) * 精确率 * 召回率 / (beta^2 * 精确率 + 召回率) ・・・当 beta > 1 时,召回率更重要。当 beta < 1 时,精确率更重要。 F2 在欺诈检测中常用(漏检欺诈比误报更糟) ・・・
...
> **AUPRC**(Dikrarlılık oranı- çağrış oranı eğilimi aşağı面积) ∼ AUC-ROC'ye benzer, ancak dengesiz verilere karşı daha fazla bilgi var.
...
> **马修斯相关系数 (MCC)**= (TP * TN - FP * FN) / sqrt((TP+FP)(TP+FN)(TN+FP)(TN+FN))。 aralığı -1 ila +1─ sadece iki sınıf üzerinde iyi performans gösterdiğinde 才给高分──即使类小差很大也保持平衡──
...
> 对于上述的"始终预测负类"模型:精确率 = 0/0(未定义,通常设为 0),召回率 = 0/10 = 0,F1 = 0,MCC = 0──
...
> 随机过采样复制现有少数类型样本―― bu geçerlidir, ancak riskleri vardır, çünkü model aynı noktayı tekrar görecektir――

SMOTE, inanılmaz ama kopya olmayan yeni sentetik azınlık örnekleri oluşturur.

> SMOTE Yeni bir miktar yapılmış örnek oluşturmak, bunlar mantıklı ama bir çift değil.

1. Her azınlık örneği x için, diğer azınlık örnekleri arasında k'nin en yakın komşularını bul
    Her küçük bir örnek için x, diğer küçük bir örnek arasında yakın komşularını bul
2. Bir komşunu rastgele seç .
   # Bir komşunu seçmek için #
3. X ile komşu arasındaki çizgi segmentinde yeni bir örnek oluştur
   X ve komşu arasındaki çizgi aşamasında yeni bir örnek oluşturun
...
> Bu, gerçek bir az sınıf noktası arasında bir değer oluşturmak, mevcut verileri sadece kopyalamak değil, aynı alanın özellik alanında oluşturmak.
...
- Eksiklikler: Tam kopyalar aşırı uyum sağlıyor, eğitim süresini arttırıyor
  缺点: tamamen aynı副本 sonuçları over-adapted, training time
...
- Eksiler: potansiyel olarak yararlı çoğunluk verilerini atıyor, daha yüksek varyansa
  缺点: kullanışlı bir veri türünün çoğu, daha fazla
...
> Verileri değiştirmek yerine, modellerin hatalarla ilgili davranışını değiştirmek için daha yüksek bir güç verilmiştir.
...
> Normal sınıf 19 kat fazla güç kazanmıştır. Bir normal örneğin fiyatı, yanlış sınıf 19 negatif örneğin fiyatına eşittir.
...
> Sınıf ağırlığı, beklenen olarak, daha fazla örnek matematik fiyatına benzer, ancak yeni veri noktası oluşturmaz. Bu da onları daha hızlı hale getirir ve örneklerin çoğaltılması riskini önler.
...
> Büyük çoğunluk için bir sınıfın çıkış olasılığı vardır. Eğer P (pozitif) >= 0.5 ise, tahmin doğru olur.

Süreç:
1. Bir model eğit
   訓練一個模型
2. Valideleme kümesinde tahmin edilen olasılıkları alın
   Testleme kitlesinde elde edilecek tahmin olasılığı
3. 0,0 ila 1,0 arasında aralıklı sınırlar
   0.0'den 1.0'e kadar 扫描 值
4. Her eşiğinde F1 (veya seçtiğiniz metrik) hesaplayın
   Her değer altında hesap F1 ((( veya seçtiğiniz gösterge)
...
> Model可能对一笔欺诈交易输出P(欺诈) = 0.15──在值 0.5 下, bu 值 0.5 下, bu 值 0.10 下, bu 值 0.10 值 下, bu 值 0.10 值 下, bu 值 0.10 值 下, bu 值 0.10 值 下, bu 值 值  值  值  值  值  值  值  值  值  值  值  值  值  值  值  值  值  值  值  值  值   值   值   值   值   值   值   值    值    值    值     值      值       值                                                                                                                       

### Masraflara Çok Yakışıklı Öğrenme

Klas ağırlıklarının genelleştirilmesi.

> 类权重的推广──不使用统一代价,而分配特定的误分类代价:
...
> Bu, bir HABİRİNİN bir HABİRİNİN bir HABİRİNİN bir HABİRİNİN bir HABİRİNİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİRİRİN bir HABİRİRİN bir HABİRİRİN bir HABİRİN bir HABİRİRİN bir HABİRİN bir HABİRİN bir HABİRİRİN bir HABİRİN bir HABİRİN bir HABİRİN bir HABİN bir HABİRİN bir HABİRİN bir HABİN bir HABİRİN bir HABİRİN bir HABİN bir HABİRİN bir HABİRİN bir HABİN bir HABİN bir HABİNİN bir HABİNİN bir HABİN
...
> Bu, gerçek dünya maliyetini tahmin edebilmeniz için en ilkel yöntemdir. Kanser teşhisinin maliyeti, ekli yaşam denemelerine yol açan yanlış raporlamalarla çok farklıdır.
...
> **【中文解读】**
> SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex Synthesis Minority Synthesis Minority) üretim teknolojisi: SMOTE'nin (Sintex synthesis minority synthesis) üretim teknolojisi: SMOTE'nin (Sintex synthesis) üretim teknolojisi: SMOTE'in (simplementation) yapılması; SMOTE'in (simplementation) yapılması;

> **【拓展：工业级不平衡数据处理的高级技术】**
> Gerçek finansal tempolu kontrolünde, dengesiz verileri işlemek stratejisi SMOTE'den daha karmaşık: odak kaybı kullanmak (Focal Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus Loss, Focus
```figure
class-imbalance
```
...
> Küçük öğrenme ve dengesiz öğrenme, bu teknikler sadece bir kod çizgisine ihtiyaç duyar:
...
> SMOTE, az sayıda sınıfın k-NN 插值――类权重乘以损失―― 值调优是对截断值的循环――没有魔法――
...
1. **Borderline-SMOTE**SMOTE uygulamasını değiştirmek için, yalnızca karar sınırına yakın olan azınlık noktaları için sentetik örnekler oluşturmak (k'nin en yakın komşuları çoğunluk sınıfı örneklerini içerir).
   1. 生成不平衡数据集(1% 正例) ・・・比较始终预测多数类、随机森林(默认)、随机森林(class_weight='balanced')
...
2. **Cost matrix optimization**Bu nedenle, maliyet matrisi bir parametredir. Bir maliyet matrisi alanı oluşturup beklenen maliyeti en aza indirgenen optimum tahminleri gönderir. Farklı maliyet oranları (1:10, 1:100, 1:1000) ile test edin ve hassaslık-içilme bozukluğu nasıl değiştiğini çizin.
   2. Aynı veri kümesi üzerinde SMOTE ile SMOTE karşılaştırma  daha iyi kararlar almanın sınırı gösterir 
...
3. **Threshold calibration**: Platt ölçeklemesini uygulayın (kalibre olasılığını üretmek için modelin ham çıkışlarına lojistik bir geri dönüş uygulayın). Kalibrasyondan önce ve sonra hassaslık-içikleme eğriyi karşılaştırın. Kalibrasyonun sıralamayı değiştirmediğini gösterin (AUC aynı kalır), ancak olasılığı daha anlamlı hale getirir.
   3.  değer ayarını gerçekleştirmek: Logiği geri dönüşü çıkış olasılığını, 0.01 ile 0.99 arasında  değerini taramak, F1 en yüksek  değerini bulmak.
...
> **【中文解读】**
> Çekilme: 1. değerlendirme göstergesi F1/AUPRC/MCC 替代准确率; 2. ağırlıklı analizler; 2. değerlendirmeler; 3. değer duyarlılığı; 3. kayıp işlevi içinde az sayıda kişiye güç kazandırmak.

> **【拓展：Focal Loss——深度学习中的不平衡数据解决方案】**
> Focal Loss(Lin et al., 2017) başlangıçta hedefleme sırasında 极度不均衡而提出的背景像素远于目标像素) ・・・核心思想:降低"容易分类" নমুনা kaybı ağırlığı,让模型聚焦于"难分类" নমুনা ・・・公式:(p) = -(1-p) ^gamma * log(p),gamma=2 时效果最好。RetinaNet kullanmak Focal Loss COCO 检测任务上超越了当时的SOTA 方法。
...
- [imbalanced-learn documentation](https://imbalanced-learn.org/stable/)-- SMOTE çeşitleri, alt örnekleme stratejileri ve boru hattı entegrasyonu ile Python kütüphanesi
  [imbalanced-learn 文档](https://imbalanced-learn.org/)- Python Değişik Öğrenme Kutusu
