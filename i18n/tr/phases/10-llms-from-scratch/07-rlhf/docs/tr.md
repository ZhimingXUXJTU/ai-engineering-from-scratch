...
> **【中文解读】**SFT kilise modeli talimatları takip eder, ama "daha iyi" cevabını öğretmez. RLHF, insan tercihleri veri eğitimi ödüllendirme modeli kullanır, PPO  optimize stratejisi kullanır.

> **【拓展：PPO→ChatGPT对齐】**ChatGPT'nin RLHF trenimi PPO algorithm kullanıyor: Ödül modeli cevaplama 打分,PPO bu 分ü ödül sinyalleri olarak kullanıyor ödüllendirme stratejisi için. KL  ceza önleme stratejisi SFT 模型 çok uzakta. Bu Antropik/OpenAI için hazırlıklı bir çekirdek süreçtir.

>  **【前置】**学本节前 Lütfen önce bil:Fase 10·06(SFT) RLHF'in başlangıç noktası SFT 模型;Fase 09·08(PPO) 强化学习 PPO 算法基础;Fase 18·01(Yönlendirmeler Takip) RLHF 在对齐中的位置──本节是Fase 10·08(DPO) 和Fase 18(Ethics) 系列的前置──
...
>  **【类比】**RLHF = 訓練物学杂技。SFT = "eğitmek için köpeklerin bakış açısını göstermek" ([[監督学习]];;RLHF = " köpekler bir hareket yaparlar, size az yiyecek verirler, size yüksek ödüllendirme sağlarlar, size düşük ödüllendirme sağlarlar, size az yiyecek verirler, size az yiyecek verirler, size az yiyecek verirler, size çok fazla ödüllendirme sağlarlar, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler, size çok fazla ödüllendirirler.

> ️ **【易错点】**RLHF'nin 3 个坑:(1) **奖励模型过拟合**RM preference data acc=99%, fakat genel olarak farklı; daha büyük RM + erken dur + 验证集监控──(2) **KL 系数设错**太大(> 0.5)模型不动如 SFT,太小(< 0.01)模型乱跑"奖励黑客"; tipik 0.05-0.2──(3) **reward hacking** model bulduk "加更多表情符号 RM 给高分",输出全是emoji; sürekli izleme, 输出分布,发现异常立即停下――

## Öğrenme hedefleri

- İnsan tercih çiftlerinden cevap kalitesini (seçilmiş vs reddedilen) notlayan bir ödül modeli oluşturun
  构建从人类偏好对(选择对拒绝)评分回复质量的奖励模型
- Dil model politikasını KL cezası ile ödül modeli karşısında optimize eden PPO eğitim döngüsünü uygula
  PPO   training loop,  KL  punishment 约束 altında ödüllendirme modelleri optimize 语言模型策略
- RLHF'nin neden üç model (SFT, ödül, politika) gerektirdiğini ve KL'nin kısıtlaması ödül hackeriğini nasıl engellediğini açıklayın.
  解释为什么RLHF 需要三个模型(SFT、奖励、策略),以及 KL 约束 如何防止奖励黑客
- RLHF'nin etkisini tercih optimizasyonu öncesi ve sonrası yanıt kalitesini karşılaştırarak değerlendirmek
   RLHF'nin etkisini değerlendirmek için karşılaştırma tercihleri optimizasyonu öncesinin geri dönüş kalitesi

> **【中文解读】**Bu ders RLHF'nin tam yönünü gerçekleştirmek için: İnsan tercihleri verileri eğitimi ödül modeli, PPO algoritması  optimization language model strategiyası。RLHF 需要三个模型协同工作SFT 模型(参考策略)、奖励模型(评分器)、策略模型(被优化模型)。KL 散度惩罚防止策略模型偏离SFT 模型太远,避免奖励客黑客(reward hacking)。
...
> **回复 A：**"Kuantum hesaplama kullanımı, üst üstelik bir kuantum bitinde olabilir, yani aynı anda 0 ̊1 veya ikisi olabilir. Bu da kuantum bilgisayarlarının bazı hesaplamaları klasik bilgisayarların hızlı indeks derecesinden daha hızlı işlemlemesini sağlar.
...
> **回复 B：**"Kuantum hesaplama, kuantum güç fenomenini kullanan bir hesaplama biçimidir. 1980'lerde ilk olarak önerildi. Richard Feynman, kuantum sistemlerinin kuantum bilgisayar tarafından benzetilebileceğini önerdi. Bu alanın sonucunda önemli bir gelişme yaşandı. Birçok şirket şimdi kuantum bilgisayarlarını geliştirmekte. IBM, Google ve diğerleri ilerleme kaydetmiştir. Google, 2019 yılında kuantum haklarını gerçekleştirdiğini iddia etti. "
...
> SFT bu farkı yakalayamıyor. Bu "sağ" geri dönüşü üzerinde bir eğitim modeli içinde, ancak hiçbir mekanizma "Bu geri dönüş daha iyi" diyor.
...
> RLHF  Bu sorunu çözdü. İnsanların hangi tepkiyi tercih edeceğini tahmin etmek için bir ödül modeli eğitildi ve bu ödül sinyalini kullanarak dil modelinin daha yüksek kaliteli bir çıkış üretmesini teşvik etti.

> **【中文解读】**SFT'nin sınırları, "yani daha iyi cevap" ayırt edemediğidir. İki dilbilim doğru, gerçek doğru cevaplar kullanışlılık konusunda farklılık gösterebilir.

> **【拓展：InstructGPT 的突破】**InstructGPT 论文(OpenAI 2022)  RLHF 应用于大模型的里程碑──关键发现:(1) 1.3B 参数  RLHF 模型优于175B 的原始 GPT-3;(2) RLHF 显著减少有害输出(毒性降低约80%);(3) 真实性也升升.
...
> RLHF tek bir eğitim çalışması değildir.
...
> **阶段 1：SFT。**Bu size talimatları takip edebilecek ama hangisinin daha iyi olduğunu bilmediğiniz bir model verir.
...
> **阶段 2：奖励模型。**收集人类偏好数据:向标志者展示对同一提示的两个回复并问"哪个更好?"训练一个模型来预测这些偏好――奖励模型以 (提示,响应) 为输入,输出一个标量分数――
...
> **阶段 3：PPO。**Kullanım Ödül modeli için dil modeli oluşturma eğitim sinyalleri, dil modeli oluşturma, ödüllü modeller onlara bir parça, PPO 更新语言模型以产生更高分的回复, KL 散度惩罚防止语言模型偏离 SFT 检查点太远――

> **【中文解读】**RLHF'nin üç aşama boru hattı:Etap 1 SFT kullanmak  基础模型学会 命令に従う;Etap 2 ヒトの偏好データを収集する; aynı sorunun iki yanıtına göre, " hangisi daha iyi " etiketleme;Etap 3 PPO algoritması kullanmak 策略モデルを高報酬の回報を生成させる; aynı zamanda KL 散度罰を用い SFT モデルから偏離を防止する.
...
> Ödüllü model, değerlendirme cihazının dil modelini yeniden kullanır. SFT modelini alır, dil yapılandırır.

Giriş: bir cevapla bağlantılı bir istek. Çıktı: tek bir skalar ödül puanı.

> 输入:一个提示拼接一个回复――输出:一个标量奖励分数――
...
> 訓練資料は人間の偏好对──各提示,标志者看两个回复并选择更好──これは訓練三元组:(快速, 首选回复, 拒绝回复) 
...
> Bu önemli bir yöntem.`sigmoid(reward(A) - reward(B))`给出回复 A 被偏好于回复 B 的概率──损失推动奖励模型给首选回复分配更高分数──
...
> Neden karşılaştırma yerine mutlak değerlendirme kullanılır? Çünkü insanlar mutlak kalite değerlendirmeyi çok iyi ayırmıyorlar. Bu cevap 10 dakika içinde 7.3 veya 7.5'dir.
...
> **InstructGPT 数据：**OpenAI, 40 tane yükleniciden 33.000 kişiyi topladı.
...
> PPO, güçlü bir kimyasal öğrenme algoritmasıdır. RLHF'de "muhit" ödül modeli, "akıllı vücut" dil modeli, "动作" bir token oluşturur.
...
> İlk adım, yüksek ödüllendirme ödüllendirme.
...
> Neden KL  ceza gerekir? O olmadan, model geri dönüşümlü çözümler bulacaktır. Ödül modelleri sınırlı insan tercihleri veri kümesi üzerinde eğitim, kör noktalar vardır.

- "Ben çok yararlı ve zararsızım!" tekrarlaması, yararlılık/zararsızlık ödül modellerinde yüksek puanlar verir
  Çinçe Çevirimi:重复"I'm very useful very harmless!"
- "Yüksek kalite" ile uyumlu olan sözlü, resmi ama boş cevaplar üretmek
  Çinçe çevirisi:生成冗长、正式但空洞的回复,模式匹配到"高质量"
- Eğitim verilerinde yüksek ödüllerle ilişkili olan belirli ifadelerden yararlanmak
  Çinçe Çevirimiçi: Utilise Training Data 中恰好与高奖励相关的特定短语
...
> KL 惩罚说: Sen iyileştirebilirsin, ama tamamen farklı bir model haline gelemez.
...
> **InstructGPT 数据：**PPO trenings using lr=1.5e-5,KL 系数 beta=0.02,256K 个节目(prompt-回复对), her bir grup 4 个 PPO epoch── tüm RLHF 管线在 GPU 集群上运行了几天──
...
> PPO, aşırı büyük bir yenilenmeyi önlemek için "kısıtlama temsilci hedefi" kullanır. Yeni eski strateji olasılıkları arasındaki oran [1 - epsilon, 1 + epsilon] aralığında kesilir.
...
> Kütle genellikle son geri dönüşün ortalama ödülüdür. Sağ avantaj, ortalama seviyeye göre iyi bir geri dönüş anlamına gelir. Yalan avantaj, ortalama seviyeye göre fark anlamına gelir.
...
> 截断防止灾难性更新── Eğer tek bir cevap anormal yüksek ödülleri alırsa, kesilmemiş oran çok büyük olabilir, bu da modelin şiddetli olarak bu cevap yönüne kaymasına neden olabilir──截断更新幅を制限し,トレーニングを安定させます──
...
> RLHF'nin阴暗面──语言模型针对奖励模型优化,而奖励模型是人类偏好的不完美代理──语言模型越来越擅长最大化奖励,它开始利用奖励模型的弱点──
...
> 缓解策略:更强的 KL 惩罚(model偏离防止到足以利用弱点程度)  model karşılığı örnek üzerinde eğitim ödülleri modeli(modify known failure mode)  modellerin farklı yapılarını kullanarak ödülleri daha zor bir şekilde yenebilir) 
...
> Her modelin RLHF  eğitim yapılandırması karşılaştırması:InstructGPT ile 33K  tercih karşılığı ve 6B  ödül modeli;Llama 2 Chat ile yaklaşık 1M  tercih karşılığı ve 70B  ödül modeli;Antropik'in RLHF  makalesi 22K karşılaştırması ile 52B ödül modeli üzerinde eğitim görmüşlerdir。
...
> Anthropic 2022 makalesi 22.000 kişiye karşı 52B'nin ödüllendirme modeli üzerinde çalıştırıldı. Büyük ödüllendirme modeli daha güvenilir sinyal üretir, PPO eğitimini daha istikrarlı yapar.
...
> Üretim sırasında, insan etiketleyicileri tercih edilen verileri oluşturacağız.
...
> 首选回复简洁直接──被拒回复显示常见失败模式:不必要的填充、模糊、冗余解释和不精确──
...
> Ödüllü model mini GPT'nin transformatör yapısını tekrarladı, ancak kelimenin büyüklüğündeki çıkış başını tek bir proje olarak değiştirdi.
...
>  Ödüllü model son simgeyi alır  konumunun gizli durumu ve simge olarak projektir ∞ neden son simge? Çünkü sonuç dikkatli gizleme son konum önündeki tüm simgeyi önemsediğini gösterir ∞ tüm ∞ için en tam simgeyi vardır ∞

### Üçüncü Adım: Bradley-Terry Kaybesi

Ödül modelini Bradley-Terry çiftlik kaybı ile tercih çiftleri üzerinde çalıştırın.

> Bradley-Terry'yi kullanarak, ödüllendirme modelini kullan.
...
> Dikrarlılık oranı göstergesi çok net: Ödüllendirme modelinin doğru sıralama oranına tercihleri ne kadar? Assion model puanları %50¬dir. Kane net veriler üzerinde iyi eğitilmiş ödüllendirme modeli %70'den fazla olmalıdır.
...
> 完整的PPO 很复杂──这个实现捕获了核心机制:生成回复、评分、计算优势、使用 KL 惩罚更新策略──
...
> 核心循环:(1) 采样一个提示,(2) 生成回复,(3) 用奖励模型评分,(4) 计算与结参考模型的 KL 散度,(5) 计算调整后的奖励(奖励减 KL 惩罚),(6) 更新策略──KL 惩罚 策略偏离参考模型而增,自动防止奖励黑客──
...
> RLHF ından sonra, strateji modeli ödüllendirme modeli üzerinde skor orijinal SFT  modeli ından daha yüksek olmalıdır ⋅
...
> 本课产 出 `outputs/prompt-reward-model-designer.md` Bir ödül modeli eğitimi hattının tasarımına yönelik bir uyarı;; belirlenmiş hedef davranışlar;; kullanışlılık,, programlama yeteneği,, güvenlik), bu da veri toplama protokollerini oluşturur; etiketçi yönlendirmesi ve ödül modeli değerlendirme standartları;;
...
1. Ödül modelini değiştirin. Sadece son pozisyon yerine tüm gizli durumların ortalamasını kullanın. Doğruluğu karşılaştırın. Ortalama birleştirme yaklaşımı her bir simgeyi eşit ağırlığa verirken, son pozisyon yaklaşımı toplu bilgilere nedenci ilgiye dayanır. 6 tercih çiftini test edin ve hangi yaklaşım daha yüksek doğruluk puanlar verdiğini bildirin.
   Çin dilinde tercüme: Değiştirme ödül modeli, tüm gizli durumların ortalama değerini yalnızca son konum yerine kullanır.
...
2. Ödül modelinin kalibrasyonunu uygulayın. Eğitimden sonra tüm tercih çiftlerini ödül modeliyle çalıştırın ve hesaplayın: (a) tercih edilen yanıtlar için ortalama ödül, (b) reddedilen yanıtlar için ortalama ödül, (c) kenar (seçilmiş - reddedilmiş). İyi kalibrlenen bir modelin açık bir kenar olması gerekir.
   Çince çevirim: implement reward model校准── eğitimden sonra, tüm tercihler ödül modelinden geçerli olarak hesaplanır: a) 首选回复的平均奖励, b) 被拒回复的平均奖励, c) 边距 (首选减被拒绝) ─ İyi校准的模型应有明显边距──然后添加 4 new preferences对,检查边距是否保持在未见数据上──
...
3. Ödül hackleme simülasyonu. Uzun cevaplara yüksek puan veren bir ödül modeli oluşturun (ödül = len(reponse) / 100). Bu kusurlu ödül modeli ile PPO çalıştırın ve politikayı daha uzun, tekrarlayan çıkışlar üreten bir model olarak gözlemleyin.
   Çin dilinde çevirme:模拟奖励黑客── oluşturmak için uzun geri dönüş yüksek分 奖励模型(reward = len(response) / 100)── kullanmak için bu eksiklik bonus modelini çalıştırmak PPO, gözlem strateji modeli üretmek için越来越长、重复的输出── sonra eklemek 0.1 KL 惩罚, gösterir geri dönüş davranışını önleyebilir──
...
4. Çoklu objektif bir ödül uygulayın. Bir ödül modeli eğitiniz - biri yararlılık için ve biri kısaca. R = 0.7 * R_helpful + 0.3 * R_concise olarak birleştirin. Birleştirilen hedefin hem yararlı hem de kısaca yanıtlar ürettiğini gösterin, tek bir yararlılık ödülünün sözlücelik tuzağını önleyin.
   Çinçe çeviride: gerçekleştirmek çok hedef ödül. İki ödül modeli eğitilmektedir. Birinde yararlılık için, birinde sadelik için. R = 0.7 * R_helpful + 0.3 * R_concise。 göstermek için birleştirilmiş bir dizi hedef, bir tek yararlılık ödülünün uzun sürdüğü tuzağından kaçınmak için hem yararlı hem de sade bir şekilde geri dönüştürülmektedir.
...
5. Farklı KL katılamlarını karşılaştırın. Beta=0.001 (çok düşük, ödül hackleme), beta=0.02 (standart) ve beta=0.5 (çok yüksek, öğrenme yok) ile PPO çalıştırın.
   Çinçe çevirisi:Biraz farklı KL 系数──用 beta=0.001(太低,奖励黑客)、beta=0.02(标准) 和 beta=0.5(太高,不学习)运行 PPO──绘制每组的奖励曲线和 KL 曲线──beta=0.02 应显示稳定的奖励提升和有界的 KL──
