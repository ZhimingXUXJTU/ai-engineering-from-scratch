# DeepSeek-V3 Arsitektur Gezi Gezi
...
> **【中文解读】**DeepSeek-V3(2024 yıl 12月,671B 总参数,37B 激活) tüm altı yapı dönümünü değiştirdi ve yeni dört tane:MLA(多头潜在注意力) 无辅助损失负载均衡、MTP(多代币 预测) 双管训练──671B/37B oranı, her bir öneride yalnızca %5,5'lik bir parametreni etkinleştirmek anlamına gelir.

> **【拓展：DeepSeek架构→开源大模型】**DeepSeek-V3 2024-2025 yılları için en önemli açık kaynaklı büyük model yapılardan biridir. MLA KV-cache'yi orijinal 1/10, MoE'ye indirerek 671B modelini sadece 37B'nin düşünme maliyetini tüketir. Bu yapı, Çin'in AI laboratuvarlarının büyük model alanında ilerlemenin anahtarını anlamak için kullanılır.

>  **【前置】**Özgür Modeller Arsitürası: 10·14 6 个架构旋概览; 10·15-19 全部(EGLE、Diff Attention、NSA、MTP、DualPipe) DeepSeek-V3'ün her yeniliği.
>  **【类比】**DeepSeek-V3 = " altı边形战士"把 2024 yılın tüm ön kenarını optimize全堆上:MLA(省 KV cache)、MoE(省推理算力)、MTP(送投机解码)、DualPipe(省训练通信)、NSA(省长上下文算力)。671B 总参数但每次只激活37B(5.5%),相当于"功能丰富的瑞士军刀但单次只使用一把刀"。
...
- DeepSeek-V3 yapılandırmasını üstten aşağı okuyun ve her alanı altı GPT-2 düğmesi ve dört DeepSeek özel eklemle açıklayın.
  Baştan sona kadar DeepSeek-V3'yi okuyun. GPT-2'nin altı dönümlüğünü ve dört DeepSeek'i kullanın.
- Toplam parametreler sayısını (671B), aktif parametreler sayısını (37B) ve her birine katkıda bulunan bileşenleri çıkarın.
  推导总参数(671B)、激活参数(37B) ve bileşenleri
- MLA'nın KV önbelleği izini 128k bağlamda hesaplayın ve GQA ile aynı aktif-param yoğun bir modelin neyi ödeyeceğini karşılaştırın.
  计算 MLA 缓存占用 KV 缓存占用 缓存占用 KV 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占用 缓存占
- DeepSeek'e özgü dört yeniliği (MLA, MTP, yardımcı kayıpsız yönlendirme, DualPipe) ve mimarlık/öğretim yığınının her birinin hangi kısmını hedeflediğini belirtin.
  Bu konuda dört farklı konu var: DeepSeek Özellikle yeniliklerle (MLA, MTP, Hassız Kayıp Yolları, DualPipe) ve amaçlı yapı/öğrenme bölümleri.
...
> DeepSeek-V3 ilk yapı olup Llama ailesinin ön kenar açık kaynak modeliyle varlıklı farklılıkları vardır. Llama 3 405B "GPT-2 调调六旋"ıdır.
...
> DeepSeek-V3'ün açık yetki yayınlaması, açık kaynak modelinin "önerlik yetkinliği" anlamını değiştirdi.

## Konsepten bir şey.

> **【中文解读】**DeepSeek-V3 2024 yılının en etkili açık kaynak modelleri arasında birisidir.

> **【拓展：DeepSeek-V3 的经济性突破】**DeepSeek-V3'ün eğitim maliyeti sadece yaklaşık 5.6 milyon dolardır, yaklaşık Llama 3.405B'nin eğitim maliyetinin 1/18'i.
...
> **【拓展：DeepSeek-V3 的 MoE 架构细节】**DeepSeek-V3 256 路由专家,每次激活8 个(加1 共享专家) ⋅671B 总参数但只激活约37B,计算量约5.5%──
...
> 本课产 出 `outputs/skill-deepseek-v3-reader.md`△ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △ △                                                    
...
1. Çık .`code/main.py`- Hesap makinesi toplam parametresi tahminini yayınlanan 671B ile karşılaştırın ve delta'nın nereden geldiğini belirleyin.
   Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri`code/main.py`◊Dizim hesaplayıcılarının toplam parametrelerinin tahminleri ile yayınlanan 671B karşılaştırma, fark kaynağı bulmak için.

2. MLA sıralamasını değiştirmek için MLA sıralamasını değiştirin.
   Çin dili: Modified configure use MLA 秩 256 instead of 512──计算 128K 上下文下 KV 缓存大小──省了多少百分比,代价是什么?
...
3. DeepSeek-V3'ün (256 uzman, üst-8) yönlendirmeyi hipotetik bir (512 uzman, üst-8) varyana karşılaştırın. Toplam parametreler büyüyor; aktif parametreler aynı kalıyor. Teorik olarak ekstra uzman kapasitesi ne satın alır ve sonuçta ne kadar maliyetli olur?
   Çince çevirisi: DeepSeek-V3'ün(256 专家,top-8) yoluyla ile varsayımlar(512 专家,top-8)变体──总参数增长;激活参数不变──额外专家容量理论上买到了什么,推理时代价是什么?
...
4. MLA'ya ilişkin DeepSeek-V3 teknik raporunun (arXiv:2412.19437) 2.1. bölümünü okuyun. K ve V dekompresyon matrislerinin neden sonuçlama zaman verimliliği için sonraki matmul'e "sorb" edilebileceğini üç cümle ile açıklayın.
   Çinçe çevirisi: DeepSeek-V3 技术报告 第2.1 节 MLA içeriği hakkında.
