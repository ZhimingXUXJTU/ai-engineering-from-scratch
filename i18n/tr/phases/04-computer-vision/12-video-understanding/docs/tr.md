# Video Anlama  Zamanlı Modelleme  视频理解  时序建模
...
> **【中文解读】**Video, bir dizi görüntü sırasıdır ve bunların birbirine bağlanmasını sağlar. Bu üç türden bir video modeli vardır.

> **【拓展：视频 AI 应用】**Video anlayışı YouTube/TikTok içeriğini önermeye yöneldi. Güvenlik denetiminin anormal denetlemesi, spor etkinliklerinin otomatik analizi. Sora ve diğer video üretimi modelleri video anlayışını yeni bir seviyeye yönlendirdi.
...
> 30 saniye 30fps video 900 张图像―― basitçe bakmak, video 分类就是把图像分类运行 900 次然后聚聚──当动作在几乎每时都可见时 (体育、、健身视频) Bu geçerlidir, ancak hareket kendiliğinden tanımlandığında ciddi bir başarısızlık: "Bunun soldan sağya atılması" her birinde iki duraklı nesne görünür.
...
> Her video yapısının temel sorusu: zaman, yapı nasıl inşa edilir ve nasıl inşa edilir?
...
> Bu ders, duruşsal görüntü programından daha kısa.
...
> 取一个2D CNN(ResNet、EfficientNet、ViT),在每上独立运行,对每嵌入做平均(或最大池化、注意力池化),将池化后的量送入分类器──
...
- Ucuz: T çerçeveleri * tek görüntü sonucu maliyeti.
  Çeviri: Cost Low: T  × 单推理开销。

Eksiler:
- Hareket modeli yapamıyorum.
  Çinçe Çevirimiçi:无法建模运动──动作 = Dış görünüşün birleştirilmesi──
- Zamanlı birleştirme, düzen değişmez; "açık kapı" ve "kapı kapalı" aynı görünümde.
  Çinçe Çevirisi: zaman序池化是序无关的; "open door" ve "关门" gibi görünüyor.
...
> I3D 技巧: Get a pre-training of 2D ImageNet 模型, "inflation" each 2D 核 along new time axis copying──3x3'nin 2D 卷积 into 3x3x3'nin 3D 卷积── bu da 3D 模型'ın güçlü bir pre-training power gainına, baştan eğitim gerektirmez.

Avantajlar:
- Doğrudan hareket modelleri.
  Çeviri: direct建模运动
- I3D enflasyonı ücretsiz transfer öğrenimi sağlar.
  Çinçe Çevirimi:I3D 膨胀提供免费的迁移学习。

Eksiler:
- T/8'de 2D eşliğinden daha fazla FLOP (sürekli çekirdeğin 3'ü 3 kez yığılmış)
  Çinçe Çevirimiçi:Biy 2D 方案多 T/8'nin FLOPları (Türk için 3 ̊Nükleer büyüklük için 3 ̊D durum)
- Zaman çekirdekleri küçüktür; uzun mesafeli hareket piramit veya çift akımlı bir yaklaşım gerektirir.
  Çinçe çevirisi: 時間核很小;长距離運動需要金字塔或双流方法──

Ne zaman kullanılır: hareket sinyal olduğu hareket tanımı (Something-Something V2, Kinetics with motion-heavy classes).

> 何时使用:运动是关键信号的动作识别(Bir Şey-Bir Şey V2、以运动为主导的动力学 类别) ]]

### Arazi-zamanlı transformatörler

Videoyu bir uzay-zaman çişisi şebekesine işaretleyin ve hepsine katılın.

> Video Swin, VideoMAE, TimeSformer, ViViT, Video Swin, VideoMAE,

Önemli olan dikkat kalıpları:
- **Joint** bir büyük dikkat üzerinde (t, h, w).`T*H*W`- Çok pahalı.
  Çeviri:**联合**对 (t, h, w) 做一次大注意力──复杂度为 `T*H*W`Çekirde;昂贵――
- **Divided** blok başına iki dikkat: bir zaman boyunca, bir uzay boyunca.
  Çeviri:**分离** her blok iki dikkat: bir kez zaman, bir kez uzay.
- **Factorised**Zaman dikkatinin bloklar arasında uzay dikkatinin değişmesi.
  Çeviri:**分解**Zaman ve uzay dikkatinin bloklar arasında değişmesi.

Avantajlar:
- SOTA doğruluğu her önemli referans değerinde.
  Çinçe Çevirisi: tüm ana temel testlerde SOTA 精度'ya ulaşmak.
- Görüntü transformörlerinden (ViT) patch enflasyonu üzerinden aktarmalar.
  Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri: Çeviri Çeviri: Çeviri: Çeviri: Çeviri Çeviri Çeviri Çeviri: Çeviri Çeviri: Ç Ç Ç Ç Çeviri Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç Ç
- Kısıtlı dikkat ile uzun bağlamlı videoları destekler.
  Çinçe Çevirisi: Çıkışlı Dikkat Çekimi

Eksiler:
- Bilgisayar açlığı.
  Çeviri: Kürtçe
- Dikkatli bir dikkatli bir seçim veya koşuş balonu gerektirir.
  Çinçe Çevirimiçi: needs仔细选择注意力模式,否则运行时间膨胀──
...
> 均采样和密集采样器, 列表 (列表) 或视频张量 (视频张量)  için kullanılır.
...
> 两者都回归  İki kişi geri dönüyor`T`个索引用于切片视频张量──

### Adım 2: 2D+pool başlangıç çizgisi

Her çerçeveye 2 boyutlu ResNet-18 çalıştır, ortalama havuz özellikleri, sınıflandırma.

> 2D ResNet-18'i her gün çalıştırmak, ortalama bir özellik oluşturmak ve sonra sınıflandırmak.
...
> Bir milyon parametresi,ImageNet 预训练,逐运行、平均、分类── Bu temel çizgi genellikle normal 3D modellerine göre daha güçlü ImageNet kemik ağını kullanırken 5-10% daha iyi olur.
...
> Yeni zaman boyunca tek bir 2D 卷积 dönüştürülecek.
...
> 3D 卷积分分2D 空间) ve 1D 时间)卷积── aynı algılama alanı, daha az parametre, belirli bir kenarlıkta daha yüksek doğruluk──
...
> 完整的 R(2+1) D 网络就是将 ResNet-18 中的每个3x3 卷积替换为 `Conv2Plus1D`- Evet.
