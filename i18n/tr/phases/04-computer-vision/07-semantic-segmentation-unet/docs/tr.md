# Semantik Bölüm  U-Net 语义分割  U-Net
...
> **【中文解读】**语义分割就是对每个像素进行分类――U-Net 通过配对下采样编码器和上采样解码器,并连接跳跃连接实现这一点――编码器提取特征,解码器恢复分辨率,跳跃连接保留细节信息――

> **【拓展：U-Net 的广泛应用】**U-Net başlangıçta tıbbi görüntü bölme tasarımı için tasarlanmıştı, ancak birçok alanda bir altyapıya dönüştü: Stabil Diffusion U-Net'i kullanmak örneğin, çekirdeği olan gürültü çözme ağı, otomatik olarak kullanılan yol bölümü, U-Net'in gezegen görüntülerinin toprak kapsamı sınıfı, kodlayıcı- çözücü+ at bağlantısı biçimi yaygın olarak kopyalanmıştır.
...
> 分类每张图像输出一个标签――检测每张图像输出几个框――分割每像素输出一个标签――对于大小为`H x W`İçeri girdi, çıkışı şekillidir.`H x W`(语义) veya `H x W x N_instances`Bu, bir değil, her resim için milyonlarca tahmin.

> **【中文解读】**Bölümün temel zorluğu: her bir görüntü üzerinde bir bütün görmek için, ayrıca yerleşik görüntü ayrıntılarını korumak için, bu görüntü yol veya insan yolu) ◊ Standart CNN 通过空间压缩获取上下文时会丢失细节,U-Net 通过跳跃连接同时解决这两个需求.
...
> Bölümsel yapı, neredeyse tüm yoğun öngörülmüş görsel ürünlerin nedenini yönlendirir: tıbbi görüntüler (medicine images) 瘤掩码) 自动驾驶 (autonomous driving) 道路、车道、障壁 (障壁) 卫星 (arşivin döngüsü), işlerin sınırı (object boundary) 文档解析 (布局区域) 机器人 (机器人) 可抓取区域) . Bu görevler, bir nesne etrafındaki resim çerçevesinden geçemez; onlar kesin bir döngü gerektirir──
...
> Yapısal sorun basit ama çözülmesi kolay değildir: İnternet'e ihtiyacınız var görüntülerin tümünü aynı anda görmeniz için. U-Net'in iki tasarımın da aynı anda elde edilmesi için.
...
- **Semantic**"Bu piksel yol, o piksel araba". diyor.
  Çinçe çevirisi:语义分割说" bu biçim yol, o biçim araba"── bitişik iki araç bir bölgeye birleştirilmiştir──
- **Instance**"Bu piksel 3 numaralı bir araba, bu piksel 5 numaralı bir araba" diyor. Arka plan eşyalarını görmezden gelir ("şeriler" = gök, yol, ot).
  Çin dilinde:实例分割说"这个像素是第三辆车,那个像素是第五辆车"──忽略背景类别("物品" = 天空、道路、草地)──
- **Panoptic**her pixel bir sınıf etiketi alır, her örnek de benzersiz bir kimlik alır, her şey ve her şey her ikisi de bölünmüştür.
  Çin dilinde:全景分割统一了两者:每个像素获得类别标签,每个实例获得唯一 ID,背景和前景物体都被分割──

Bu ders semantik konuları kapsar.

> Bu ders, bir dizi farklılıklara yol açtı.
