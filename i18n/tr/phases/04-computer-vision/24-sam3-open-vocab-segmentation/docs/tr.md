...
> **【中文解读】**给模型一个文本提示和一个图像,就能得到所有匹配对象的分割掩码──SAM 3(Segment Anything Model 3) bu süreci tek kez ileri doğru yaymaya dönüştürecek──开放词汇分割突破了传统分割模型只能识别训练类的限制──

> **【拓展：SAM 的革命性影响】**SAM 系列模型 (Meta 发布) bölünme alanındaki "basik model" dir, herhangi bir şeyi bölmek için 零样本可分割任意物体──应用于图像编辑 (图像编辑)                                                                                                                                                                                                                                      
...
> 2023 SAM sadece görsel bir ipucu modeli: bir noktaya tıkladığınızda veya bir çerçeve çizdiğinizde, bir gizlemeyi geri verir. Bu fotoğrafta bulunan tüm 子'lere göre, bir tester gerekir.
...
> SAM 3(Meta,2025 yıl 11 月, ICLR 2026) katlanmış bir sınıf birleştirdi.**可提示概念分割（PCS）**◊结合 2026 年 3 月的 Object Multiplex 更新(SAM 3.1), it can高效地在视频中跟踪同一概念的多个实例──
...
> Bu ders, bu temsil eden yapısal değişimi anlatıyor. 2D bölünme, inceleme ve metin-resim temelleri bir model olarak birleşmiştir. Üretim sorunu artık "ne akış hatlarını birbirine bağladığımı" değil, "neyi gösterimli bir model olarak kullanımıma son verebilirim" sorusu.
...
> "提示概念词" bir kısa isimdir短语 veya bir resim örneği. Model, resim içinde bu kavramın tüm örneklerinin bölünmüş bir gizlemesi, her bir uyumluğun tek örnek kimliği ile birlikte geri döner.

Bu, klasik görsel-sürekli SAM'den üç yönden farklıdır:

> Bu klasik görüntü göstergesiyle üç farklılık vardır:

1. Bir örnek istisna istisna  tek metin istisna tüm eşleşmeleri gönderir.
   Çine dilinde: 无需逐例提示一个文本提示返回所有匹配──
2. Açık sözlük  kavram doğal dilde tarif edilebilir herhangi bir şey olabilir.
   Çinçe Çevirimi: Open Word Word 概念可以是自然语言可描述的任何东西──
3. Tek seferde birden fazla örnek gönderir, tek istek yerine.
   Çinçe Çevirim: Bir kere bir kaç örnek geri dönmek yerine her bir ipucu bir gizleme olarak kullanmak.

### Ana mimari parçalar

- **Shared backbone** tek bir ViT görüntüyi işliyor. Hem dedektör başı hem de hafıza tabanlı izleyici görüntüden okuyor.
  Çeviri:**共享骨干** Tek bir ViT  görüntü işleme  inceleme başları ve bellek tabanlı takip cihazları 
...
- **Decoupled detector-tracker** Resim seviyesindeki tespit ve video seviyesindeki izleme ayrı başlara sahip, böylece müdahale etmezler.
  Çeviri:**解耦的检测器-跟踪器** Görüntü seviyesinin kontrolü ve video seviyesinin takip edilmesi, birbirine karışmamakla birlikte.
- **Memory bank** Video izleme için çerçeveler üzerinde her örnek özelliklerini depolar (sadece SAM 2 mekanizması kullanılır).
  Çeviri:**记忆库**   depolama, video takip için kullanılan her örnekin özellikleri, SAM 2 ile benzer bir mekanizma)
...
- **YOLO-World** Sadece açık sözcük depolama cihazı (mask yok). Gerçek zamanlı. Yüksek fps'lerde kutulara ihtiyaç duyduğunuzda en iyisi.
  Çeviri:**YOLO-World**仅开放词汇检测器(无掩码)。实时──需要高fps 框时的最佳选择──
- **SAM 3** tam segmentasyon + izleme. Daha yavaş ama daha zengin çıkış.
  Çeviri:**SAM 3** 完整分分 + 跟踪──更慢但输出更丰富──
...
> Üretim: YOLO-Dünyası, hızlı su akışını kontrol etmek için kullanılır, SAM 3 gizlenme veya takip etme gereksinimleri için kullanılır.
