# Diffusion stable  Architecture et réglage fin  Diffusion stable  Architecture et micro-réglage
...
> **【中文解读】**La diffusion stable est un modèle de diffusion qui fonctionne dans le potentiel de l'AEV, en utilisant la mise en œuvre de l'analyse de la diffusion de données, en utilisant des modèles de diffusion de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de l'analyse de données à l'aide de données à l'aide de données à l'échelle.

> **【拓展：Stable Diffusion 生态】**La diffusion stable a été dérivé de LoRA (Light Quantity Micro-Module) ControlNet (ControlNet) Control Gest态/Range (IP-Adapter) Image Tip) Lora 微调 (LoRA 微调) Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Low image Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light Light LLLight Light Light LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
...
> 直接在 512x512 RGB 图像上训练 DDPM 很昂贵. Chaque étape de formation doit passer par un U-Net de 3x512x512 = 786,432 输入值反向传播, la sample doit passer par le même U-Net 进行 50+ 次前向传播.
...
> Pour rendre le texte à images plus pratique, il faut savoir comment le faire.**潜空间扩散**(Rombach 等,CVPR 2022)  entraîner un VAE qui va 3x512x512 image de cartographie à 4x64x64 潜张量并恢复, puis faire une expansion dans ce潜空间  calcul量降低 `(3*512*512)/(4*64*64) = 48x`Dans le même GPU, le temps passe de quelques secondes à deux secondes.
...
> La diffusion stable des modèles est un modèle de diffusion de la vidéo.
...
- **VAE**Le décodeur convertit les images en images latences (utilisées pour l'img2img et l'entraînement).
  Le codeur va transformer l'image en un sous-ensemble de l'image, le codeur va transformer l'image en un sous-ensemble de l'image.
...
- **U-Net**Le désignateur possède des couches d'attention croisées qui vont des latences au texte intégré à tous les niveaux de résolution.
  U-Net去噪器──包含交叉注意力层, dans chaque niveau de résolution de la variation du potentiel关订本嵌入──
...
- **Safety checker** filtre NSFW / contenu illégal optionnel sur l'image de sortie.
  Le système de contrôle de sécurité (NSFW) est utilisé pour la production de images.
...
> 纯文本条件化学习 `epsilon_theta(x_t, t, c)`Pour chaque mot`c` CFG  entraînement avec un réseau  10% du temps perdu `c`(en remplacement pour le vide), obtenir un modèle de bruit et de bruit inconditionnels simultanément prévue:
...
> `w`C'est une bonne mesure.`w=0`Il est inconditionnel.`w=1`C'est une condition normale.`w>1`En effet, la production est plus " conforme à la proposition " .`w=7.5`Il y a une autre.
...
> Le CFG est la raison pour laquelle le texte à l'image peut travailler en qualité de production.
...
> Le 4通道潜量 VAE n'est pas seulement une image compressée. Il est un flux, dont l'algorithme fonctionne à grande échelle pour la résolution des variables.

Deux conséquences:

> Les deux derniers résultats:
...
1. **Img2img**= encoder une image en latente, ajouter un bruit partiel, exécuter le dénonciateur, décoder. La structure de l'image survit parce que le codage est presque inversible; le contenu change en fonction de l'invite.
   Le mot grec traduit par " le mot grec "**Img2img**= Coder une image pour une variable, ajouter une partie de bruit, utiliser un déboucheur de bruit, décoder.
2. **Inpainting**= identique à img2img mais le dénonciateur ne met à jour que les régions masquées; les régions non masquées sont conservées au latente codé.
   Le mot grec traduit par " le mot grec "**Inpainting**= similaire à celui de l'img2img, mais le déboucheur ne renouvelle que la zone cache; la zone non cache reste en phase de modification.

### L'architecture U-Net

Le SD U-Net est une grande version du TinyUNet de la leçon 10 avec trois ajouts:

> Le U-Net de SD est la 10e version de TinyUNet, avec trois composants:
...
- **Transformer blocks**à chaque résolution spatiale, contenant une attention personnelle + une attention croisée au texte intégré.
  Traduction anglaise: chaque bloc transformateur à résolution spatiale, contenant l'attention à soi + l'attention à la transition du texte intégré.
- **Time embedding**par MLP sur le codage sinusoïdale.
  En français, le temps de la mise en œuvre est de la même nature que celui de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de la mise en œuvre de mise en œuvre.
- **Skip connections**entre le codeur et le décodeur à résolutions correspondantes.
  Le codeur et le codeur sont en lien entre les résolutions correspondantes.
...
> Le modèle de base est un modèle de base, qui est généralement utilisé en mode micro-réaction de 10 à 50 MB, en train de travailler sur un GPU de consommation unique de 10 à 60 minutes, en fonction de la mise en œuvre de la mise en œuvre de la mise en œuvre.
...
> La LORA est la méthode de distribution de presque tous les micro-organismes communautaires.

### Les horaires que vous verrez

- **DDIM** déterministe, ~50 étapes, simple.
  Le mot "détermination" est traduit par "détermination", en français par "détermination".
- **Euler ancestral** stochastique, 30 à 50 étapes, échantillons légèrement plus créatifs.
  Le style de la création est le plus connu de l'histoire de l'époque.
- **DPM-Solver++ 2M Karras** déterministe, 20 à 30 étapes, production par défaut.
  Le système de gestion des ressources humaines est un système de gestion de ressources humaines.
- **LCM / TCD / Turbo** modèles de consistance et variantes distillées; 1 à 4 étapes au coût d'une certaine qualité.
  Le modèle de conversion de la Turbo est un modèle de conversion de la Turbo.
...
> Dans le`diffusers`Le régulateur de commutation ne nécessite qu'une ligne de code, parfois pas besoin de reentraînement pour pouvoir réparer le problème de l'échantillon.
...
> 本课端到端使用 `diffusers`Il s'agit d'un programme spécialisé dans la production d'API.
...
> `float16`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             `num_inference_steps=25`Également utilisé par le DDIM `num_inference_steps=50`Il y a une autre.
...
> `strength`控制去噪音前添加多少噪音(0.0 = 不变,1.0 = 完全重新生成) ⋅0.5-0.7 est la gamme standard du style de déplacement。
...
> Les images blanches sont à la recherche d'une région de reproduction, les images noires sont conservées.
...
> `lora_scale`控制强度;0.0 = 无效,1.0 = 完全效果──`fuse_lora`L'adaptateur se fusionnera au centre de poids pour augmenter la vitesse, mais empêchera le changement.`pipe.unfuse_lora()`Il y a une autre.

### Étape 6: Formation en LRA (esquisse)

La formation LoRA réelle vit dans `peft`ou `diffusers.training`Le contour:

> La vraie LORA est en train de s' entraîner.`peft`Ou `diffusers.training`Le résumé est le suivant:
...
>  seulement LoRA 矩阵接收梯度;基础 U-Net、VAE 和文本编码器都是结的──批量大小为 1并启用梯度检查点时,8 GB 显存即可运行──
