...
> **【中文解读】**Suivre = 检测 + 关联── pour chaque objectif de recherche, puis traiter les résultats de la recherche actuelle avec les traces de suivi de la première année par l'entremise de l'identification 匹配──多目标跟踪(MOT) est la technologie centrale de la compréhension vidéo, nécessitant de traiter la disparition, la réapparition, etc. des situations complexes──

> **【拓展：MOT 的应用】**Il est également possible de suivre les traces de personnes dans les scènes de sécurité, de contrôle de la circulation, de contrôle des véhicules, d'analyse de la situation, de suivi des joueurs, de suivi des traces de personnes, etc.
...
> Le détecteur vous dit où se trouve l'objet.`t`中哪个检测与第 `t-1`Le test de l'objet est le même. Sans lui, vous ne pouvez pas compter le nombre d'objets qui traversent la ligne.
...
> Suivre les produits de chaque facette vidéo sont essentiels: analyse de la santé, surveillance, conduite autonome, analyse de la médecine, surveillance des animaux sauvages, chiffre d'affaires.
...
> L'année 2026 a apporté deux nouveaux modèles:**SAM 2 基于记忆的跟踪**(Té征记忆替代运动模型关联) et **SAM 3.1 Object Multiplex**(Plusieurs exemples du même concept de partage de mémoire)
...
> Chaque traceur que vous rencontrerez en 2026 est une variante de ce cycle.

- **SORT**(2016): Filtre Kalman + IoU hongrois. Modèle simple, rapide, sans apparence.
  Le mot grec traduit par " le mot grec "**SORT**(2016): Karlmann 波器 + IoU 匈牙利算法──简单、快速、无外观模型──
- **DeepSORT**(2017): SORT + une fonctionnalité d'apparence basée sur CNN par piste (embedding ReID).
  Le mot grec traduit par " le mot grec "**DeepSORT**(2017):SORT + Chaque piste de CNN
- **ByteTrack**(2021): associent les détections de faible confiance à une deuxième étape; aucune caractéristique d'apparence n'est nécessaire mais le meilleur acteur sur MOT17.
  Le mot grec traduit par " le mot grec "**ByteTrack**(2021): le test de faible confiance en tant que deuxième phase de la connexion; sans attributs extérieurs, mais avec le meilleur rendement au MOT17
- **BoT-SORT**(2022): Byte + compensation du mouvement de la caméra + ReID.
  Le mot grec traduit par " le mot grec "**BoT-SORT**(2022):Byte + 相机运动补偿 + ReID。
- **StrongSORT / OC-SORT** Des descendants de ByteTrack avec un meilleur mouvement et une meilleure apparence.
  Le mot grec traduit par " le mot grec "**StrongSORT / OC-SORT**ByteTrack 后代,改进了运动和外观模型──
...
> Carman est en train de maintenir l'état de chaque piste.`(x, y, w, h, dx, dy, dw, dh)`和协方差──每先使用恒速模型**预测**état, reusage de l'état de l'état**更新**◊ prédiction incertaine ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la confiance ◊ augmentation de la pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré pré prédiction.

Chaque traqueur classique utilise un filtre Kalman dans l'étape de prédiction du mouvement.

> Chaque suivi classique utilise un appareil de suivi de la carte.
...
> - Je suis sûr .`M x N`Le prix de la distribution est généralement de 0,5% en moyenne.`1 - IoU(轨迹框, 检测框)`Ou l'extérieur des caractéristiques de la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à la résistance à`scipy.optimize.linear_sum_assignment`- Je suis très rapide.
...
> 标准跟踪器丢弃低置信度检测(< 0.5)。ByteTrack les conservera pour**第二阶段候选**Après avoir effectué des tests de haute confiance, des tests de trajectories non conformes ont été tentés pour une évaluation de la valeur de l'UE relativement plus large.
...
- **MOTA (Multi-Object Tracking Accuracy)** 1 - (interrupteurs FN + FP + ID) / GT. Pise par type d'erreur; une seule métrique qui confond les défaillances de détection et d'association.
  Le mot grec traduit par " le mot grec "**MOTA（多目标跟踪精度）**1 - (FN + FP + ID 切换) / GT── selon le type d'erreur accès;
- **IDF1 (ID F1)** moyen harmonieux de précision et de rappel d'identification. Se concentre spécifiquement sur la façon dont chaque piste de vérité au sol maintient son identité au fil du temps.
  Le mot grec traduit par " le mot grec "**IDF1（ID F1）**Id  précision et réaction 调和平均── 专注衡量每一个真实轨迹随时间保持ID 的一致性── 针对ID 换换敏感任务优于MOTA──
- **HOTA (Higher Order Tracking Accuracy)** se décompose en précision de détection (DetA) et précision d'association (AssA).
  Le mot grec traduit par " le mot grec "**HOTA（高阶跟踪精度）** décomposer pour l'examen de précision (DetA) et l'accès à l'information (AssA) ‒ les normes communautaires les plus complètes depuis 2020 ‒
...
> 监控(谁是谁): rapporter IDF1──运动分析(计数传球):HOTA──一般学术比较:HOTA──
