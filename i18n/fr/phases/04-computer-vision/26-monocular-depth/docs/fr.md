# Approximation de la profondeur monoculaire et de la géométrie
...
> **【中文解读】**La profondeur d'image est une image à travers un seul canal, chaque valeur de la image indiquant la distance entre les images.
...
> La profondeur est l'axe manquant dans la vision 2D de l'ordinateur. Donnée RGB, vous savez où se trouvent les objets sur la plaine de l'image; vous ne savez pas à quel point ils sont éloignés.
...
>  Évaluation de la profondeur  de la profondeur de prédiction  de la RGB  de l'expérience  de l'expérience  de la production de sorties étonnantes  de l'expérience  de l'expérience  de l'expérience  de la production de données  de la profondeur  de la production de données  de la profondeur  de la diffusion  de la production  de la profondeur  de la diffusion  de la production  de la profondeur  de la diffusion  de la production  de la profondeur  de la diffusion  de la profondeur  de la profondeur  de la diffusion  de la profondeur  de la profondeur  de la profondeur  de la profondeur  de la profondeur  de la profondeur  de la profondeur  de la profondeur  de la profondeur 
...
> La profondeur est aussi le pont entre le dépistage 2D et la compréhension 3D: les images du cadre dépisté seront multipliées par la profondeur, vous pourrez alors faire passer les objets 2D à 3D. C'est le cœur de chaque système AR, de chaque évitement de la circulation d'eau et de chaque "capsule".
...
- **Relative depth**- Je vous en prie .`z`"Le pixel A est plus proche que le pixel B, mais le rapport des distances n'est pas ancré aux mètres".
  Le mot grec traduit par " le mot grec "**相对深度** organisé `z`值,没有真实世界单位──"A est plus proche que B, mais le ratio de distance n'est pas déterminé à l'intérieur".""
- **Metric depth** distance absolue en mètres de la caméra.
  Le mot grec traduit par " le mot grec "**度量深度**                                                                                                                                                                                                                                                              
...
> MiDaS et profondeur Tout V3  générer relativement à la profondeur。Marigold  générer relativement à la profondeur。ZoeDepth、UniDepth 和 Metric3D  générer une profondeur de mesure。Modèle de mesure à la surface de l'écran; Modèle de mesure à la surface de l'écran  non sensible。
...
> La profondeur de tout V3 结编码器, seulement entraîner DPT 风格的解码器──编码器提供丰富特征;解码器将它们插值回图像分辨率并回归深度──
...
> Une image 2D contient de nombreux points de vue liés à la profondeur:

- **Perspective** Les lignes parallèles en 3D convergent en 2D.
  Le mot grec traduit par " le mot grec "**透视**3D en milieu de ligne de plan dans 2D en milieu de汇聚──
- **Texture gradient** les surfaces éloignées ont une texture plus petite et plus dense.
  Le mot grec traduit par " le mot grec "**纹理梯度**Les textures de surface sont plus petites que les autres.
- **Occlusion order** Les objets les plus proches occultent les plus éloignés.
  Le mot grec traduit par " le mot grec "**遮挡顺序**Les objets proches sont couverts par les objets éloignés.
- **Size constancy** les objets connus (autos, humains) donnent une échelle approximative.
  Le mot grec traduit par " le mot grec "**大小恒常性** déjà connu les objets 车、人) donner une approximation de mesure.
- **Atmospheric perspective** Les objets éloignés apparaissent plus brumeux et plus bleus dans les scènes extérieures.
  Le mot grec traduit par " le mot grec "**大气透视** Les objets éloignés apparaissent plus en bleu dans la scène extérieure.
...
> Les VT, qui ont été formés sur des milliards d'images, vont intégrer ces indices. Ils disposent de suffisamment de données et de solides bases, sans avoir besoin de toute surveillance 3D explicite pour atteindre une précision raisonnable.
...
> C'est un modèle à utiliser directement en 2026.
...
> Marigold(Ke 等,CVPR 2024) va évaluer la profondeur du tableau en vue de la diffusion de l'image.
