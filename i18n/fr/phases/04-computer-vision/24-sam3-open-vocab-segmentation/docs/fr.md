...
> **【中文解读】**给模型一个文本提示和一个图像,就能得到所有匹配对象的分割掩码──SAM 3(Segment Anything Model 3) Le processus deviendra un seul avant-propos pour se propager──开放词汇 分裂突破了传统分化模型只能识别训练类的限制──

> **【拓展：SAM 的革命性影响】**SAM 系列模型 (Meta 发布) est un " modèle de base " dans le domaine de la division, qui peut être utilisé pour la division de tout objet.
...
> SAM de 2023 est un modèle de pointe visuelle: vous cliquez sur un point ou dessinez un cadre, il renvoie un cache. Pour " me donner cette photo de tout le contenu ", vous avez besoin d'un détecteur pour produire un cadre, puis utiliser SAM pour le diviser.
...
> SAM 3(Meta,2025 11 月, ICLR 2026) a plié le niveau de connexion. Il accepte un exemple de courte durée de courts termes ou d'images comme indication, dans le cas de la propagation de la première et de la seconde fois, de retourner à tous les masques et des exemples correspondants.**可提示概念分割（PCS）**◊结合 2026 年 3 月的 Object Multiplex 更新(SAM 3.1), il peut être très efficace dans le cadre de la vidéo Suivre plusieurs exemples du même concept♦
...
> Cette classe parle de ce qui représente le changement structurel. La 2D de division, de test et de texte-image de base a déjà été combinée en un modèle. La question de production n'est plus "Je mets quelles lignes de courant sont reliées ensemble", mais "quel modèle suggérable peut traiter mon usage de bout en bout".
...
> "提示概念词" est un mot-clé de titre, un mot-clé de titre ou un exemple d'image. Le modèle retourne à l'image pour couvrir les exemples de correspondance de ce concept, ainsi que l'identifiant unique de chaque exemple.

Cela diffère du SAM visuel classique de trois façons:

> Il y a trois différences avec le classique SAM:

1. Aucune demande par instance n'est requise  une demande de texte renvoie toutes les correspondances.
   Une réponse à l'exemple de l'expérience
2. Le concept de vocabulaire ouvert peut être tout ce qui est décrivable dans le langage naturel.
   Le concept de "définition" peut être défini dans la langue naturelle.
3. Retourne plusieurs instances à la fois plutôt qu'un masque par prompt.
   Une fois de retour plusieurs exemples, plutôt que chaque suggestion un masquerode.

### Des pièces architecturales clés

- **Shared backbone** un seul ViT traite l'image.
  Le mot grec traduit par " le mot grec "**共享骨干** Un seul ViT  traitement d'images  Tête de contrôle et de suivi basé sur la mémoire sont tous pris en compte 
- **Presence head** prédit si le concept est présent dans l'image du tout. Découpe "est-ce ici?" de "où est-il?". Réduit les faux positifs sur les concepts absents.
  Le mot grec traduit par " le mot grec "**存在性头** prédiction concept existe-t-il dans l'image ? " ici ? " et " ici où ? "
- **Decoupled detector-tracker** La détection au niveau de l'image et le suivi au niveau de la vidéo ont des têtes séparées afin de ne pas interférer.
  Le mot grec traduit par " le mot grec "**解耦的检测器-跟踪器** Image et vidéo sont indépendants, ne s'interrogent pas.
- **Memory bank** stocke des fonctionnalités par instance sur des cadres pour le suivi vidéo (même mécanisme SAM 2 utilisé).
  Le mot grec traduit par " le mot grec "**记忆库** Traces de stockage de chaque cas, utilisés pour le suivi vidéo (Machine similaire à SAM2)
...
- **YOLO-World** Détecteur de vocabulaire ouvert seulement (pas de masques). En temps réel.
  Le mot grec traduit par " le mot grec "**YOLO-World**仅开放词汇检测器 (→ ) 无掩码 (→ ) 实时──需要高 fps 框时的最佳选择──
- **SAM 3** Segmentation complète + suivi.
  Le mot grec traduit par " le mot grec "**SAM 3** Complete division + suivi.
...
> Produit: YOLO-World utilisé pour le contrôle rapide des flux d'eau, la navigation des appareils, le tableau de bord rapide, le SAM 3 utilisé pour le cacher ou le suivi des scènes.
