...
> **【中文解读】**U-Net n'est pas le secret du modèle de diffusion. U-Net est remplacé par Transformer. U-Net est remplacé par Direct Line Flow.

> **【拓展：DiT 是 2024-2026 的趋势】**Diffusion Transformer (DiT) va Transformer de la capacité de diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de la diffusion de diffusion de la diffusion de la diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diffusion de diff de diffusion de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff de diff
...
> Le programme a été construit en 2020-2023 avec U-Net, qui a généré une diffusion stable de 1,5 et 2,1 ainsi que DALL-E 2,
...
> Chaque modèle de texte à image avancé de 2026 l'a dépassé. La diffusion stable 3、FLUX、SD4、Z-Image、Qwen-Image、Hunyuan-Image n'utilise pas un U-Net. Ils utilisent aussi le Transformer diffusé (DiT) ・SD3 et FLUX.
...
> Cette transformation est importante car elle est basée sur la génération d'images diffusées devenant contrôlables 提示精确 (SD3/SD4 解决文本染) et la raison de la production rapide ⋅ comprendre le DiT + 整流就是理解2026年生成图像技术──
...
- **DiT**(Peebles & Xie, 2023)  remplacer le U-Net par un transformateur semblable à ViT sur des patchs latents. Conditionnement via norme de couche adaptative (AdaLN).
  Le mot grec traduit par " le mot grec "**DiT**(Peebles & Xie,2023)  Utilisation de transformateurs de classe ViT  remplacement U-Net  traitement de la variation de la quantité de suppléments。
- **MMDiT**(SD3, Esser et coll., 2024)  deux flux avec des poids séparés pour les jetons de texte et d'image qui partagent une attention commune.
  Le mot grec traduit par " le mot grec "**MMDiT**(SD3, Esser et autres, 2024)  deux groupes ont le droit de traiter le texte et les symboles d'image, partageant l'attention commune.
- **FLUX**(Black Forest Labs, 2024)  les premiers blocs N double flux comme SD3, les blocs ultérieurs concatenent et partagent des poids (single-stream) pour une efficacité à plus grande profondeur.
  Le mot grec traduit par " le mot grec "**FLUX**(Black Forest Labs, 2024) 前 N 个块像 SD3 一样双流,后续块拼接并共享权重(单流) afin d'améliorer l'efficacité du niveau profond。
- **Z-Image**(2025)  une DiT efficace à flux unique à 6B qui met en cause "l'échelle à tout prix".
  Le mot grec traduit par " le mot grec "**Z-Image**En 2025, le défi de l'expansion des coûts de production est de 60 milliards de dollars.
...
> Le DDPM va définir le processus de pré-orientation comme étant le bruit avec des écarts de fréquence (SDE), parmi lesquels:`x_t`Le retour à l'apprentissage est un deuxième SDE, à travers 1000 petits pas.

Le débit rectifié définit un **straight-line**interpolation entre les données propres et le bruit pur:

> Le flux complet définit la différence entre les données propres et le bruit pur**直线**插值:
...
> 训练网络预测速度 `v_theta(x_t, t) = epsilon - x_0` D'un point de vue direct sur la voie du nettoyage des données à la route du bruit`dx_t/dt`)¬, par exemple, le temps de l'opération de l'opération de détection est réduit de la fréquence du bruit vers la fréquence des données¬, ce qui réduit considérablement le nombre de détections nécessaires à l'opération.

SD3 appelle ça .**Rectified Flow Matching**. FLUX, Z-Image et la plupart des modèles 2026 utilisent le même objectif.

> SD3 sera appelé**整流流匹配** FLUX、Z-Image 和 la plupart des modèles de 2026 utilisent le même objectif。 typique de la réflexion:20-30 步 Euler(确定性) vs 旧 DDPM 方案的50+ 步 DDIM──蒸/turbo/schnell/LCM 变体降至1-4 步──
...
> Je suis passé**自适应层归一化**Conditionnement du temps et des catégories/texts: de la prédiction du temps et du volume des conditions`scale`et `shift`En effet, la mise en œuvre de la norme de couche de fichiers est plus efficace que la mise en place de fichiers de fichiers de couche de fichiers en ligne.
...
> DDPM + U-Net 可行──DiT + 整流流**更好、更快、扩展更干净** Cette transition est similaire à celle de la transition de la RNN à la Transformer en PNL: les deux types d'architectures résolvent le même problème, mais la Transformer est plus étendue et est finalement dominante.
