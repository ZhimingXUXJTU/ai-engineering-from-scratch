# 3D Gaussian Splatting de zéro à zéro réalisation 3D
...
> **【中文解读】**3D 高(3DGS) pour représenter des scènes à travers des millions de 3D 高球. Chaque hauteur a une position, une orientation, une éclosion, une opacité et des couleurs liées au point de vue.

> **【拓展：3DGS vs NeRF】**Le 3D Gaussian Splating est une alternative réelle à la NeRF. La NeRF est lente mais de haute qualité. La 3DGS a réalisé une haute représentation de la 3D en temps réel tout en maintenant une haute qualité.
...
> NeRF va stocker les scènes pour le poids de la chaise de MLP. Chaque image de la chaise est en ligne des centaines de fois.
...
> 3D 高斯(Kerbl、Kopanas、Leimkühler、Drettakis,SIGGRAPH 2023) a remplacé toutes ces images. La scène est un ensemble de 3D 高斯 apparent, avec une GPU de 100+ fps.
...
> Le modèle mental est simple, la mathématique a suffisamment de parties, la plupart des présentations ont commencé par la lumination et ont sauté sur les projections et les fonctions de la sphère.
...
> 旋转 + 缩放构建 3x3 协方差矩阵:`Sigma = R S S^T R^T`                                                                                                                                                                                                                                                              
...
> Un scénario typique a 100 à 500 millions de points de vue. Chaque scénario est stocké à environ 60 points de vue.
...
> Ça ?**与 NeRF 的体积渲染方程相同**L'équation est la raison pour laquelle la qualité de la teinture correspond à la même quantité de champs de radiation.
...
> Chaque étape de la projection, de la distribution de la pièce, de l'analyse de la valeur est relativement petite.`(mu, q, s, alpha, c_lm)`                                                                                                                                                                                                                                                              

### Densation et taille

Un ensemble fixe de Gaussiens ne peut pas couvrir une scène complexe.

> Le nombre fixe de cas ne peut pas couvrir les scénarios complexes.
...
- **Clone**Un Gaussien à sa position actuelle lorsque sa magnitude de gradient est élevée mais son échelle est petite  la reconstruction a besoin de plus de détails ici.
  Le mot grec traduit par " le mot grec "**克隆**L'étage est élevé mais en petit nombre, la reconstruction d'un bâtiment élevé à sa position actuelle nécessite plus de détails.
- **Split**un gaussien à grande échelle en deux plus petits lorsque son gradient est élevé  un gaussien grand est trop lisse pour s'adapter à la région.
  Le mot grec traduit par " le mot grec "**分裂**Quand une grande échelle est élevée, la séparer en deux plus petites une grande échelle est trop élevée pour qu'elle ne soit pas adaptée à la région
- **Prune**Les gaussiens dont l'opacité tombe en dessous d'un seuil ne contribuent pas.
  Le mot grec traduit par " le mot grec "**修剪** l'insécurité est inférieure à la valeur de                                                                                                                                                                                                                                                        
...
> La densité de chaque N fois que les séries sont menées une fois, les scènes commencent généralement à environ 100 000 épisodes de la formation.
