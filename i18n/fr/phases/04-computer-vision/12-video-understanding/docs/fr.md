# La vidéo compréhension  Modélisation temporelle  视频理解  时序建模
...
> **【中文解读】**视频 est un ensemble de séquences d'images qui les relient à des lois physiques. 视频模型的三种流派:3D卷积(时间作为额外维度) 变形机(时间作为序列) 两D+池化(每提取特征重聚) ;; comprendre que la dimension du temps est le défi central de l'IA de la vidéo.
...
> 30 secondes 30fps vidéo est 900 张图像―― simple regarder, vidéo classification est de faire des images classification fonctionner 900 fois puis se regrouper―― lorsque le mouvement est pratiquement visible à chaque fois, mais quand le mouvement est définie par le mouvement lui-même, il échoue sérieusement: " faire des choses de gauche à droite " à chaque fois, il semble que deux objets sont statiques―
...
> La question centrale de chaque structure vidéo est: quand est-elle construite, et comment est-elle construite ?
...
> Le mécanisme de conception de l'image centrale est déjà en place, la compréhension vidéo est principalement liée à l'histoire du processus:
...
> 取一个2D CNN(ResNet、EfficientNet、ViT), dans chaque fonctionnement indépendant, pour chaque emplacement faire une moyenne(((ou la plus grande accumulation、 attention accumulation), va accumuler le volume de l'accumulation après la transmission dans les classes 

Les avantages:
- ImageNet effectue des transferts de pré-entraînement directement.
  Le projet de loi de la Commission européenne sur les droits de l'homme (CEDEAO) a été adopté par le Conseil européen de la protection de l'environnement.
- Le plus simple à mettre en œuvre.
  Le plus simple de réalisation.
- Cheap: T-frame * coût de l'inférence d'une seule image.
  Le prix de vente est bas.

Les inconvénients:
- Je ne peux pas modéliser le mouvement.
  Le mouvement de l'extérieur est un mouvement de l'extérieur.
- Le regroupement temporel est invariable selon l'ordre; "porte ouverte" et "porte fermée" sont identiques.
  Le temps de la séquence est sans ordre; la porte ouverte est comme la porte ouverte.
...
> Pour le nucléaire 2D (H, W) 核替换为 3D (T, H, W) 核──网络在空间和时间进行卷积──早期代表:C3D、I3D、SlowFast──
...
> I3D 技巧: Prendre un pré-entraînement de 2D ImageNet 模型, "inflation" chaque 2D 核沿新时间轴复制──3x3 卷积的2D 卷积变成 3x3x3 卷积──这让3D 模型获得强预训权重,而无需从头训──

Les avantages:
- Il est directement modélisé.
  Le mot "défense" est traduit par "défense".
- L'inflation I3D donne un apprentissage gratuit.
  Le programme de formation de la société de l'information est basé sur la technologie de l'information.

Les inconvénients:
- T/8 plus de FLOP que la contrepartie 2D (pour le noyau temporel de 3 empilés 3 fois).
  Pour le temps, le nucléaire est 3 fois comploté.
- Les noyaux temporaires sont petits; le mouvement à longue portée nécessite une approche pyramidale ou à double courant.
  Temps nucléaire très petit; long distance motion nécessite un métro ou un métro bi-fluent.
...
> 何时使用:运动是关键信号的动作识别(Quelque chose-Quelque chose V2、以运动为主动的动力学 类别) ⋅

### Transformateurs spatiaux-temporaux

Marquez la vidéo dans une grille de patchs spatiotemps et assistez à tous.

> Pour les autres, il faut faire un calcul d'attention entre les deux.

Les schémas d'attention qui comptent:
- **Joint** une grande attention sur (t, h, w).`T*H*W`- Ils sont chers.
  Le mot grec traduit par " le mot grec "**联合** À (t, h, w) faire une fois grande attention―`T*H*W`Le carré est très cher.
- **Divided** deux attentions par bloc: une dans le temps, une dans l'espace.
  Le mot grec traduit par " le mot grec "**分离** chaque bloc deux fois attention: une fois temps, une fois espace, une fois approximation de l'expansion de la ligne.
- **Factorised** L'attention temporelle alternera avec l'attention spatiale à travers les blocs.
  Le mot grec traduit par " le mot grec "**分解** L'attention du temps et de l'espace sont échangées entre les blocs.

Les avantages:
- Accuracité SOTA sur chaque référence majeure.
  En français, la qualité de l'épreuve est de 12 à 12 degrés.
- Transférations à partir de transformateurs d'image (ViT) par inflation des patches.
  Le changement de nom est le plus important dans la vie.
- Supporte la vidéo de long contexte via une attention rare.
  Le film est en cours de réalisation.

Les inconvénients:
- Je suis affamé de calcul.
  Le nombre de personnes qui ont été tuées est de plus en plus élevé.
- Il faut choisir avec soin le modèle d'attention ou les ballons de course.
  Le temps de fonctionnement est inférieur à celui de l'exposition.

Quand utiliser: grands ensembles de données, compréhension vidéo de haute fidélité, tâches vidéo + texte multimodal.

> Quelques années plus tard, le groupe a été créé par le gouvernement de la République de Suède.
...
> Les 10 secondes de 30 fps sont à 300; les 300 sont à tout modèle.

- **Uniform sampling** choisir les cadres T uniformément sur le clip.
  Le mot grec traduit par " le mot grec "**均匀采样**在片段中等间距选 T ──2D+pool 的默认方式──
- **Dense sampling** fenêtre de T-frame contiguë aléatoire. commun pour les convex 3D car le mouvement nécessite des cadres voisins.
  Le mot grec traduit par " le mot grec "**密集采样**随机连续 T 窗口──3D 卷积常用,因为运动需要相邻──
- **Multi-clip** échantillonnage de plusieurs fenêtres de T-frame de la même vidéo, classer chacune, prédictions moyennes au moment du test.
  Le mot grec traduit par " le mot grec "**多片段** De la même vidéo                                                                                                                                                                                                                                                            

T est généralement 8, 16, 32 ou 64. T plus élevé = plus de signal temporel à plus de calcul.

> T est généralement 8、16、32 ou 64。T 越大 = 更多时序信号, mais calcul量更大。

### Évaluation

Deux niveaux:
- **Clip-level accuracy**Le modèle voit un clip de T-frame, rapporte top-k.
  Le mot grec traduit par " le mot grec "**片段级精度**模型看一个T 片段, rapport top-k――
- **Video-level accuracy** prédictions moyennes de niveau de clips sur plusieurs clips par vidéo; plus élevé et plus stable.
  Le mot grec traduit par " le mot grec "**视频级精度** Prévision moyenne de plusieurs clips de la même vidéo; plus élevé et plus stable。
...
- **Kinetics-400 / 600 / 700** l'ensemble de données d'action à usage général. 400 000 clips; URL YouTube (plusieurs sont maintenant morts).
  Le cinéma est un système de communication qui est utilisé pour la communication de données.
- **Something-Something V2** actions définies par mouvement (" déplacement de X de gauche à droite ").
  Le mouvement définit par le mouvement "X va de gauche à droite" (en anglais: Something-Something V2运动定义的动作")
- **UCF-101**- Je suis là .**HMDB-51** plus âgés, plus petits, encore rapportés.
  Le projet de loi de l'Union européenne sur les armes nucléaires (UCF-101)
- **AVA** action *localisation* dans l'espace et le temps; plus difficile que la classification.
  Le nombre de personnes concernées est de plus en plus élevé.
...
> 均采样和密集采样器, utilisée pour la liste 
...
> Ils sont de retour.`T`个索引用于切片视频张量──

### Étape 2: Une ligne de base 2D+Pool

Exécutez un ResNet-18 2D sur chaque cadre, des fonctionnalités moyennes, classez.

> En fonctionnement 2D ResNet-18, les caractéristiques de l'échantillonnage sont ensuite classées.
...
> Une base de données de 1 million de paramètres,ImageNet 预训练, par phase de fonctionnement 平均、分类── Cette base de données sur les tâches de supervision est généralement différente de 5 à 10 points de pourcentage par rapport aux modèles 3D ordinaires parfois même mieux, car elle utilise un réseau de base d'images de plus grande puissance.
...
> À l' exception de`time_kernel` maintenir la largeur de la valeur active relativement inchangée Ceci est important pour la statistique de la regroupement des lots lors de la première pré-diffusion non perturbée
...
> Le même sens est plus faible, la précision est plus élevée sur certains points.
...
> 完整的R(2+1) D 网络就是将ResNet-18 中的每个3x3卷积换为 `Conv2Plus1D`Il y a une autre.
