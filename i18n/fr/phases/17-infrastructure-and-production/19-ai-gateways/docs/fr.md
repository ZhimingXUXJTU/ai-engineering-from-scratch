# Portkey, Kong, Bifrost, et le réseau de l'AI.
...
> **【中文解读】**Ce chapitre présente l'IA 网关LLM Les routes de demande, l'équilibre de charge et la sécurité des réseaux.
...
>  **【前置】**Il s'agit d'un outil de communication technique qui permet de partager les informations et les informations nécessaires pour obtenir un accès à la plateforme.
>  **【类比】**La portée de l'IA = "AI 流量交警"。LiteLLM = 开源 MIT 100+ fournisseur, mais < 500 RPS 适合;Portkey = 控制面(PII 脱敏/越狱检测/审计) $49/月;Kong = 性能王(自家基准比 Portkey 快 228%、比 LiteLLM 快 859%), adapté aux entreprises déjà utilisées de Kong;Bifrost = 自动重试+按回退;Cloudflare/Vercel = 托管运维;;
...
- Enumérer les six caractéristiques de la passerelle principale (routage, retrait, retrait, limites de vitesse, secrets, observabilité, barreaux).
  Le système de gestion de la clé est un système de gestion de la clé.
...
- Citez le point de référence Kong (228% contre Portkey, 859% contre LiteLLM) et expliquez pourquoi il importe pour >500 RPS.
  Il est également important de savoir pourquoi il est important de se concentrer sur les résultats obtenus.
- Choisissez auto-hébergé par rapport à géré en fonction de la résidence des données et du budget des opérations.
  Traduction anglaise: donner des données pour la résidence et le transport, choisir l'autonomie et l'autonomie.

## Le problème , l' introduction du problème

> **【中文解读】**L'AI 网关 se situe entre l'application et le fournisseur de modèles, le problème central résolu est la gestion unifiée de plusieurs fournisseurs. Les produits utilisent simultanément OpenAI、Anthropic 和 autotoublier Llama, chaque fournisseur a différents SDK、 faux modèle、 limite de vitesse et programme d'attestation. Le niveau du réseau va tous les regrouper en une API compatible avec OpenAI, offrant un transfert défectueux, un stockage unifié de certificats, une capacité de mesure unifiée et une limite de vitesse de location.

> **【拓展：2026 年 AI 网关市场】**Les quatre principaux acteurs du marché de l'IA 网关: 1) LiteLLMMIT 开源,100+ fournisseurs, mais en ~2000 RPS 时崩(8GB内存); 2) Portkey 2026 3月开源 Apache 2.0, contrôler面定位(gardails、PII 脱敏、越狱检测、审计追踪),20-40ms 延迟开销; 3) Kong AI Gateway 基于成熟 API 网关产品, Kong 基准测试显示自己的 Portkey 快 228%、比 LiteLLM 快 859%; 4) Cloudflare/Vercel AI Gateway 托管、零运维、边缘部署.
...
> **【中文解读】**Les facteurs déterminants de la décision sont la résidence des données. Les facteurs déterminants de la décision sont la résidence des données. Les facteurs déterminants de la décision sont la résidence des données. Les facteurs déterminants de la décision sont la résidence des données.

> **【拓展：网关 + 可观测性 + 路由的组合】**Phase 17·13(可观测性) + 16(模型路由) + 19(网关) sont dans la production de la même couche.
...
> **【拓展：AI 网关延迟预算分析】**L'AI 网关延迟 directement affecte le TTFT, est un facteur clé de la sélection.
...
> 本课产 出 `outputs/skill-gateway-picker.md`Compte tenu de l'ampleur, de la posture des opérations, de la conformité, du budget de latence, choisit une passerelle.
...
1. On court .`code/main.py`. Configurer le retrait de OpenAI→Anthropic→auto-hébergé. Quel est le taux de succès attendu à 5% de taux d'erreur du fournisseur?
   Le mot " l' économie " est traduit par " l' économie ".`code/main.py` Configuration OpenAI -> Anthropic -> Retour de l'auto-administration.
2. Votre SLA est TTFT P99 < 200 ms sur une ligne de base de 300 ms. Quelles passerelles restent dans le budget ?
   Votre SLA est à 300ms 基线 TTFT P99 < 200ms.
3. Un client de soins de santé a besoin d'un hébergement personnel + de rédaction des données personnelles + d'audit.
   Un client de soins de santé a besoin d'autonomie + PII 脱敏 + 审计――选择 Portkey OSS 或 Kong――
4. Comparer LiteLLM vs Kong: à quel plafond RPS une équipe devrait migrer ?
   Comparer LiteLLM vs Kong: Quelles sont les limites de RPS auquel les équipes devraient déménager ?
