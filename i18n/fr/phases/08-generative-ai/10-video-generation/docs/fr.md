...
> **【中文解读】**图像是2D 张量,视频是3D 张量, théorie est la même mais le calcul est de 10 à 100 fois.

> **【拓展：Sora 的影响】**La production vidéo d'OpenAI est l'un des plus grands événements de l'IA de 2024-2026.
...
>  résoudre ces architectures sont**Diffusion Transformer (DiT)**应用于时空补丁, training on large-scale dataset ∼与第 06 课程相同的扩散损失──
...
> Utilisation de la vidéo en 3D`[T, H, W, C]`Il y a une différence.`[t_p, h_p, w_p]`Les 10 secondes 1080p vidéo compressées pour environ 2 à 10 000 patches.
...
> Transformateur 处理展平的补丁序列──每个补丁有3D 位置编码(时间 + y + x)──注意力通常分解为:

- **Spatial attention**dans les patchs de chaque cadre.
  **空间注意力**Dans chaque patch de l'intérieur.
- **Temporal attention**à travers des cadres dans le même emplacement spatial.
  **时间注意力**La même position spatiale de travers.
- **Full 3D attention**est 16 à 100 fois plus cher; utilisé uniquement à faible résolution ou dans la recherche.
  **完整 3D 注意力**贵 16-100 倍; seulement dans une résolution faible ou dans l'étude.

> **【中文解读】**视频生成的核心技术:(1) 3D VAE 将视频压缩为时空潜在表示;(2) 将潜在表示切分为时空补丁;(3) DiT(Diffusion Transformer) traiter la séquence de patch, en utilisant le code de position 3D;(4) Attention est généralement décomposée en attention spatiale et attention du temps pour réduire la quantité de calcul.

> **【拓展：Sora 架构与 DiT 在视频中的应用】**Le cœur de Sora est de développer le concept de patchage de ViT dans le domaine du vidéo. Le projet de vidéo est considéré comme un " processus de patchage de temps et de l'espace ".
...
> L'écriture de la série est un outil de référence pour la production de textes.
...
> Le poids des sources ouvertes réduit plus rapidement la distance que dans le domaine des images.
...
> Le coût de la vidéo par seconde a diminué de 20 fois entre 2024 et 2026:
...
## Note de production: les latences vidéo sont un problème de bande passante mémoire.
