...
> **【中文解读】**LLM 推理分两阶段:预填充(并行处理快速,计算密集) 和码(逐代币 生成,显存带宽密集) ――KV-cache、连续批处理、投机解码 sont destinés à l'optimisation de ces deux phases。

> **【拓展：推理优化→生产部署】**VLLM, TensorRT-LLM, TGI et autres cadres de calcul ont permis de réaliser la gestion de cache KV, le traitement de lot continu, l'attention payée et d'autres améliorations.

>  **【前置】**Le projet de loi de l'Université de Londres (Lille) a été créé en 1999 et a été créé en 2008.

>  **【类比】**LLM 推理 = 读书 + 写读书笔记。**Prefill**(处理 prompt) = 快速翻完书(并行扫所有代币,计算密集);**Decode**(生成) = 逐字写笔记(一次写一个字,显存带宽密集)**KV cache**= 翻书时做书签(Récouvrez chaque page de la note, écrivez votre note sans avoir à la réécrire);**continuous batching**= 同时给多人读书会服务 (→ Les mots en anglais)
...
- Implémenter KV-cache pour éliminer les calculs redondants lors de la génération de jetons autorégressifs
   réaliser KV  cache pour éliminer les déductions de calcul dans le processus de production des jetons auto-retour
...
- Implémenter des concepts de batchage continu et PagedAttention pour maximiser l'utilisation de la GPU en cas de requêtes simultanées
  实现连续批处理和 PagedAttention 概念, maximiser le taux d'utilisation de la GPU en cas de requête
- Comparer les techniques d'optimisation des déductions (cache KV, décoding spéculatif, attention flash) et leurs compensations de débit/latence
  Comparer avec les techniques de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de la capacité de calcul de calcul de la capacité de calcul de calcul de la capacité de calcul de calcul de la capacité de calcul de calcul de la capacité de calcul de calcul de la capacité de calcul de calcul de la capacité de calcul de calcul de la capacité de calcul de calcul de calcul de la capacité de calcul de calcul de calcul de la capacité de calcul de calcul de calcul de la capacité de calcul de calcul de calcul de calcul de la capacité de calcul de calcul de calcul de calcul de calcul de la capacité de calcul de calcul de calcul de calcul de calcul de la capacité de calcul de calcul de calcul de calcul de calcul de calcul de calcul de la capacité de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de calcul de

> **【中文解读】**Le projet de loi de l'Université de la Californie (LDC) a été créé en 2008 pour la réalisation de la mise en œuvre de la technologie de l'information et de la communication (LDC) et a été créé en 2008 par le gouvernement de l'État de Californie.
...
> Vous êtes installé sur 4xA100 GPUs pour déployer Llama 3 70B. Un seul utilisateur obtient environ 50 jetons/seconde.
...
> Le modèle lui-même n'a pas changé entre un utilisateur et 100 utilisateurs. Le même poids, la même structure, la même mathématiques. La différence est la façon dont vous réglez le travail. La logique simple gaspille 90%+ du GPU disponible en calcul.
...
> Il s'agit d'une question de régulation, non de développement.$25k 推理账单降为服务相同流量的 $5k 账单的关键──
...
> VLLM en 4xA100-80GB 上服务 Llama 3 70B, low并发时达到约50 tokens/second/user, via continuous batch processing and PagedAttention en 100并发请求维持15-25 TPS/user。

## Le concept de base.

> **【中文解读】**Les deux phases de la logique de calcul ont des différences nettes: le pré-remplissage est limité aux GPU FLOPS (environ la quantité de calcul), le décode est limité au grand stockage (environ la taille du modèle) et la complexité du processus de décode est réduite de O (environ 2 fois) à O (environ 3 fois).

> **【拓展：vLLM 和 PagedAttention】**L'introduction de PagedAttention à la vLLM gère le cache KV comme une mémoire virtuelle, en utilisant un stockage de pages de taille fixe KV à la taille, résolvant le problème des fragments de stockage. Cela rend la capacité de débit de vLLM de 2 à 4 fois supérieure à celle de HuggingFace TGI.
...
> Chaque demande de formation en LLM a deux phases différentes.
...
> **解码**个别生成输出代币――每个新代币 关注所有前代币,但每次前向传播只产生一个代币――权力矩阵与预填充时的尺寸相同,但你使用单向量而不是矩阵与相乘――GPU 核心在微秒内完成,然后等待下一批权重从内存到内存.瓶是内存宽度:你能以多快速速度将模型权重从HBM传输到计算单元――A100 有2TB/s 带宽――FP16 的70B 模型是140GB――读完整模型需要70ms这是单次解码步骤的下限.
...
> 核心洞察:* Décode est de type mémoire intensive, car vous lisez tout le modèle pour générer un seul jeton*―. Chaque optimisation ci-dessous soit réduit la quantité de lecture, soit augmenté par chaque lecture de traitement de jeton 批次, soit évité complètement de lire―.
...
> En calculant attentivement, chaque requête de jeton se préoccupe de tous les jetons précédents de la clé et de la valeur de la clé et de la valeur de la clé et de la valeur de la clé. Sans stockage, générer des jetons N 需要重新计算所有 N-1 个先前的 token的键和值投投投.
...
> KV 缓存存储所有前代币的键和值投投影──生成代币 N 时,你只计算代币 N 的键和值,然后与缓存的代币 1到N-1 的 K/V 拼拼拼──
...
> Llama 3 70B a une seule 128K de KV, la consommation de 40 GB de KV 缓存半张 A100 de内存──100 发发用户每 4K token 时,只有 KV 缓存就需要128 GB──这就是为什么 KV 缓存管理是推理优化的核心挑战──
...
> 静态批处理等一批 N 个请求到达,一起处理,等*所有*完成后再接受新请求――如果一个请求需要500个代币 另一个需要10个,短请求在完成后置490个解码步骤――
...
> 连续批处理(也叫代级批处理) Dans chaque requête, une nouvelle requête sera immédiatement insérée dans la demande.
...
> La longueur moyenne du traitement continue est la même que celle de l'état de traitement fixe. La longueur du traitement continue est la même que celle du traitement statique.
...
> Chaque requête KV 缓存 est un bloc de l'économie de temps continu. Avec l'arrivée et la sortie de la requête, l'économie de temps se décompose en morceaux comme dans le système d'exploitation.
...
> PagedAttention (en anglais: PagedAttention) est un logiciel de gestion de données qui est utilisé pour la mise en cache de données. Il est utilisé pour la mise en cache de données.
...
> C'est parce que c'est l'ordre de générer un jeton, de revenir, de générer le suivant... mais si vous pouvez deviner 5 jetons, puis vérifier tout ça une fois ?
...
> 投机解码 utiliser petit et rapide**草稿模型**生成 K 个候选标签──**目标模型**Ensuite, dans une seule phase de propagation, tous les K 个候选人 sont traités. Ils semblent être remplis en même temps.
...
> Accélération dépend**接受率** Projet de modèle de prédiction多频繁地匹配目标──Llama 3 8B 为 Llama 3 70B Lors de la réalisation du projet, le taux d'acceptation en langage naturel est généralement de 70 à 85%── ce qui se traduit par un accélération de la résolution de 2 à 3 fois.
...
> Le décompte est mathématiquement précis, la distribution de sortie est la même que celle du modèle objectif. Il n'est pas approximatif.
...
> Avant de recevoir une demande, le système rédige une copie de la demande ou un devis de la demande.
...
> Pour toutes les requêtes partagées, le système de 2,000-tokens est instantanément utilisé, et la cache pré-récharge supprime chaque requête de 400 ms.
...
> - `outputs/skill-inference-optimization.md` Diagnostication et optimisation des compétences en matière de gestion des services
...
1. Modifiez le profil de cache KV pour comparer la quantification de cache KV FP16 vs FP8 vs INT4 pour Llama 3 70B dans un contexte 4K, calculer le maximum d'utilisateurs concurrents pour chacun sur 4xA100-80GB. La quantification KV à INT4 devrait être environ 4 fois la capacité de l'utilisateur.
   Pour les 4K, le Llama 3 70B, calculé 4xA100-80GB, est le plus grand nombre d'utilisateurs de chaque format.
...
2. Extension du simulateur de batchage continu pour suivre l'utilisation de la GPU (fraction des fentes de batch remplies par étape).
   Le taux d'utilisation du GPU est de > 80% en fonction de la taille de la gamme de traitement.
...
3. Implémenter une version de la cache KV de l'attention à requête groupée (GQA) où `num_kv_heads < num_query_heads`Llama 3 70B utilise 64 têtes de requête mais seulement 8 têtes de KV. Compute l'économie de mémoire par rapport à l'attention multi-tête complète (8 fois la réduction de la taille du cache KV).
   Le KV 缓存, parmi lesquels `num_kv_heads < num_query_heads`◊ Llama 3 70B avec 64 têtes de requête mais seulement 8 têtes de KV ⋅ calculé par rapport à la totalité de la prise de conscience ⋅ KV 缓存大小减少8倍) ⋅
...
5. Élargir le simulateur de décoding spéculatif pour mettre en œuvre une spéculation basée sur des arbres (style EAGLE-2). Au lieu d'une chaîne unique de jetons de projet K, générer un arbre de candidats (par exemple, 2 branches à chacun des 3 niveaux = 8 candidats à feuilles). Comparer le total des jetons acceptés par tour de vérification par rapport à la spéculation linéaire.
   Le projet de loi de la Commission européenne sur les droits de l'homme (CEDEAO) a été adopté par le Conseil européen de l'emploi et de la justice.
