# RLHF: Modèle de récompense + PPO basé sur l'apprentissage de la force humaine contre la force.
...
> **【中文解读】**Le modèle de l'Église SFT suit les instructions, mais ne l'enseigne pas à répondre " mieux "―RLHF utilise le modèle de récompense de formation de données préférentielles humaines, reutilise le modèle de PPO  stratégie d'optimisation pour générer des réponses plus conformes au jugement humain― c'est ce qui fait que Claude a besoin de 、 GPT 礼貌的核心技术―

> **【拓展：PPO→ChatGPT对齐】**Le RLHF de ChatGPT  entraînement en utilisant l'algorithme PPO : le modèle de récompense donne des points de réponse, le PPO utilise ce point comme signal de récompense pour optimiser la stratégie。

>  **【前置】**Je vous invite à maîtriser la phase 10·06 (SFT) RLHF du début est le modèle SFT; phase 09·08 (PPO) 强化学习 PPO 算法基础; phase 18·01 (Instruction suivante) RLHF 在对齐中的位置──本节是阶段 10·08 (DPO) 和阶段 18 (Éthique) 系列的前置──
...
>  **【类比】**RLHF = "trainer" 物学杂技。SFT est " faire démonstration faire chien照着学"(监督学习)。RLHF est " chien faire un mouvement, vous donner des零食( haute récompense) ou ignorer( faible récompense), chien慢慢学会讨零食的动作"(强化学习)。 récompense modèle = "expression sur votre visage( prédiction de quelles actions vous allez récompenser),PPO = 狗调整动作策略。KL 惩罚 = "别太离谱"狗不能为了零食转圈咬自己尾巴──

> ️ **【易错点】**3 个坑: 1)**奖励模型过拟合**RM acc=99% sur les données préférentielles, mais généralisation différente; avec plus de RM + préarrêt + 验证集监控──(2) **KL 系数设错**太大(> 0.5)模型不动如 SFT,太小(< 0.01)模型乱跑"奖励黑客"; typique 0.05-0.2──(3) **reward hacking** modèle trouvé "加更多表情符号 RM 给高分",输出全是emoji; continuation de surveillance de la distribution de sortie, découverte de l'arrêt immédiat de l'arrêt.
...
- Mettre en œuvre la boucle de formation PPO qui optimise une politique de modèle de langue par rapport au modèle de récompense avec une pénalité KL
  réaliser le cycle de formation de l'OPP, dans le cadre de la KL 惩罚约束
- Expliquer pourquoi la RLHF exige trois modèles (SFT, récompense, politique) et comment la contrainte KL empêche le piratage des récompenses
   expliquer pourquoi RLHF  besoin de trois modèles                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
- Évaluer l'effet du RLHF en comparant la qualité de la réponse avant et après l'optimisation des préférences
   par rapport à la préférence d'optimisation de la qualité de la réaction précédente pour évaluer l'effet de RLHF

> **【中文解读】**Le RLHF est une méthode de récompense pour les données de l'Homme, utilisée par les algorithmes de PPO pour optimiser les méthodes de langage.

## Le problème , l' introduction du problème

Demandez à un modèle "Expliquer l'informatique quantique" et il pourrait produire:

> 问模型"解释量子计算", il peut se produire:
...
> **回复 A：**" L'utilisation du calcul quantique peut être sur des bits quantiques de superposition, ce qui signifie qu'ils peuvent être simultanément 0 、 1 ou les deux. Ceci permet au traitement de certains calculs par ordinateur quantique par rapport aux ordinateurs classiques.
...
> **回复 B：**" Le calcul quantique est un type de calcul qui utilise des phénomènes de la quantité. Il a été proposé pour la première fois dans les années 1980 . Richard Feynman a suggéré que les systèmes quantiques puissent être imités par des ordinateurs quantiques.
...
> 两回复事实上都正确――语法上都无问题――都遵循命令――但回复 A 明显更好――更简洁、更有信息量、结构更好――人类每次都会选A―
...
> SFT ne peut pas saisir cette différence. Il est dans le modèle de formation "correct" reprise, mais aucun mécanisme ne dit "ce reprise est meilleur que celui-là" il considère chaque modèle de formation comme le même bon.
...
> RLHF a résolu ce problème. Il a formé un modèle de récompense pour prédire quelles réponses les humains préfèrent, puis a utilisé ce signal de récompense pour promouvoir la production de produits de langue de meilleure qualité.

> **【中文解读】**La limite de la SFT réside dans le fait qu'elle ne peut pas distinguer "quel est le meilleur" deux phrases correctes  faits précises  Réponses correctes  sur l'utilité possible  différence de terrain  RLHF                                                                                                                                                                                                                               

> **【拓展：InstructGPT 的突破】**Le RLHF est utilisé pour le grand modèle. Il est essentiel de constater que: 1) le RLHF est supérieur à la GPT de 175B; 3) le RLHF réduit considérablement les produits nocifs; 3) la toxicité est réduite d'environ 80%; 3) la réalité est également améliorée.
...
> RLHF n'est pas un seul entraînement en cours.

**Stage 1: SFT.**Prenez un modèle de base sur les paires d'instructions et de réponses (leçon 06).

> **阶段 1：SFT。**En instruction-réponse à la base de l'entraînement supérieur (第六课) .
...
> **阶段 2：奖励模型。**收集人类偏好数据:向标标记者展示对同一提示的两个回复并问"哪个更好?"训练一个模型来预测这些偏好――奖励模型以 (prompte, réponse) 为输入,输出一个标量分数――
...
> **阶段 3：PPO。**Utilisation de modèles de récompense pour les modèles de langue générer des signaux de formation. Modèles de langue générer des récompenses. Modèles de récompense pour les éditer.

> **【中文解读】**La phase 1 utilise le SFT 让基础模型学会跟随指令; la phase 2 collecte des données de préférence humaine ((à deux réponses du même prompt, marquer "quel est le meilleur") modèle de récompense d'entraînement; la phase 3 utilise l'algorithme PPO 让策略模型生成高奖励的回复, tout en utilisant KL 散度惩罚防止偏离 SFT 模型;; la KL 惩罚 est la clé de l'anti-préhension du hacker 没有它,策略会找到奖励模型的漏洞而不是真正改善输出质量;;
...
> Le modèle de récompense est utilisé à nouveau comme modèle de langage de l'équipe d'évaluation.

Entrée: une requête concatenée avec une réponse. sortie: un seul score de récompense scalaire.

> 输入: un prompt 拼接一个回复──输出: un échantillon de récompense分数──
...
> Pour chaque prompt, le markant voit deux réponses et choisit mieux. Ceci crée un training.
...
> C'est le chemin le plus important.`sigmoid(reward(A) - reward(B))`给出回复 A 被偏好于回复 B 的概率──损失推动奖励模型给首选回复分配更高分数──
...
> Pourquoi utiliser des notes de comparaison plutôt que des notes de qualité ? Parce que les humains ne sont pas très doués pour répartir les notes de qualité de comparaison.
...
> **InstructGPT 数据：**OpenAI a recueilli 33 000 comparaisons auprès de 40 entrepreneurs. Chaque comparaison a coûté environ 5 minutes.
...
> Le PPO est un type d'algorithme de formation en force. Dans le RLHF, l'environnement est un modèle de récompense, le intelligent est un modèle de langage, l'action est la génération d'un jeton.
...
> Première étape: promouvoir le modèle générer des récompenses élevées.
...
> Pourquoi faut-il KL  puni? Sans elle, le modèle trouvera une solution dégradée. Le modèle de récompense se former sur un ensemble de données limité de préférences humaines, avec des points aveugles. Le modèle de langue utilisera ces points aveugles pour trouver des résultats élevés mais sans sens réels sur le modèle de récompense.

- Répéter "Je suis si utile et inoffensif!" donne de bons résultats sur les modèles de récompense pour l'aide et l'inutilité
  Le mot "je suis utile et sans danger" est traduit par "je suis utile et sans danger".
- Produire des réponses verbes, formelles mais vides qui correspondent à un modèle de " haute qualité "
  Génération冗长、 formal mais空洞的回复,模式匹配到"高质量"
- Exploitation de phrases spécifiques qui se sont associées à une grande récompense dans les données de formation
  Traduction chinoise: utilisation de données de formation dans le cadre de la formation
...
> KL 惩罚说: vous pouvez améliorer, mais ne pouvez pas devenir un modèle complètement différent― maintenir dans la version SFT déjà raisonnable 附近―偏离太远则 KL 成本会主导奖励―
...
> **InstructGPT 数据：**L'entraînement de PPO à l'aide de lr=1.5e-5, KL est un nombre de bêta=0.02,256K épisodes, par lot 4 épisodes de PPO.
...
> Le PPO utilise un "objectif de déconnexion" pour prévenir les mises à jour trop importantes. Le ratio entre les probabilités de nouvelles stratégies est déconnecté dans la plage de [1 - epsilon, 1 + epsilon], dont l'epsilon est généralement de 0,2 epsilon.
...
> Le positionnement signifie une bonne réaction par rapport à la moyenne; le négatif signifie une différence par rapport à la moyenne. Le PPO augmente la probabilité de réaction supérieure à la moyenne, réduit la probabilité de réaction inférieure à la moyenne.
...
> 截断防止灾难性更新── si une seule répétition obtient des récompenses anormalement élevées, le taux de non-interruption peut être très élevé, entraînant un déplacement dramatique du modèle vers cette répétition──截断限制更新幅度, maintenir l'entraînement stable──
...
> Le modèle linguistique de la RLHF s'optimise en fonction du modèle de récompense, tandis que le modèle de récompense est un représentant imparfait des préférences humaines.
...
> 缓解策略:更强的 KL 惩罚(pour empêcher le modèle de se détourner de suffisamment pour exploiter son faible niveau)  entraîner le modèle de récompense sur le modèle de lutte contre le modèle de récompense ((rectifier le modèle de défaillance connu)  utiliser plusieurs modèles de récompense de différentes structures ((更难同时攻破所有模型) 
...
> RLHF de formation de différents modèles Configuration par rapport:InstructGPT utilisant 33K  préférence par rapport à 6B  récompense modèle;Llama 2 Chat utilisant environ 1M  préférence par rapport à 70B  récompense modèle;Anthropic de RLHF 论文在 22K comparativement entraîné par rapport à 52B 奖励模型──
...
> Le thème de l'Anthropic 2022 a été comparé à 22 000 personnes et a entraîné 52B à des modèles de récompense. Un modèle de récompense plus grand produit un signal plus fiable, rendant l'entraînement PPO plus stable.
...
> En production, les étiquettes humaines créent des données préférentielles. Nous allons créer des données synthétiques, dont la " première sélection " répond à l'objectif de mieux être.
...
> 首选回复简洁直接──被拒回复显示常见失败模式:不必要的填充、模糊、冗余解释和不精确── ceci est vrai que le SFT est impossible à saisir mais que le RLHF peut faire la différence.
...
> Le modèle de récompense a utilisé la transformation mini GPT, mais a remplacé la production de la taille de la carte de référence par une projection de taille unique.
...
> Le modèle de récompense prend le dernier jeton et le projette en quantité. Pourquoi est-ce le dernier jeton ? Parce que l'inconvénient signifie que la dernière position a déjà pris en compte tous les jetons précédents.
...
> Utilisez le modèle Bradley-Terry pour les pertes et les pertes en préférence pour les récompenses d'entraînement.
...
> L'indicateur de précision est très intuitif: quelle est la proportion de préférence du modèle de récompense pour une bonne classification ? Le modèle de récompense donne 50% de points. Le modèle de récompense bien entraîné sur des données propres doit dépasser 70%. Le modèle de récompense de l'InstructGPT atteint environ 72% de précision par rapport à la comparaison de conservation, semble faible mais en fait très bon.
...
> Le PPO complet est très complexe. Cette réalisation a capturé le mécanisme central: générer des réponses, évaluer des avantages de calcul, utiliser des stratégies de réparation.
...
> Après RLHF, le score du modèle de stratégie dans le modèle de récompense devrait être supérieur à celui du modèle SFT original.
...
> 本课产 出 `outputs/prompt-reward-model-designer.md` Une mise à jour rapide de la ligne de formation du modèle de récompense.
...
1. Modifiez le modèle de récompense pour utiliser la moyenne de tous les états cachés au lieu de la dernière position. Comparer la précision. L'approche de mise en commun moyenne donne à chaque jeton un poids égal, tandis que l'approche de dernière position repose sur l'attention causale à l'information agrégée. Testez sur les 6 paires de préférences et rapportez quelle approche marque une précision plus élevée.
   Le modèle de récompense modifié utilise la moyenne de tous les états cachés et non seulement la dernière position.
...
2. Implémenter l'étalonnage du modèle de récompense. Après la formation, procéder à l'exécution de toutes les paires de préférences à travers le modèle de récompense et calculer: a) la récompense moyenne pour les réponses préférées, b) la récompense moyenne pour les réponses rejetées, c) la marge (favorée moins rejetée). Un modèle bien calibré doit avoir une marge claire.
   Après l'entraînement, tous les préférences seront calculées par le biais du modèle de récompense: a) le prix moyen de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense de la récompense.
...
3. Simuler le piratage de la récompense. Créer un modèle de récompense qui donne des scores élevés aux réponses longues (récompense = len(réponse) / 100). Exécuter PPO avec ce modèle de récompense défectueux et observer le modèle de politique générant des sorties de plus en plus longues et répétitives. Puis ajouter une pénalité KL de 0,1 et montrer qu'il empêche le comportement dégénéré.
   Créer un modèle de récompense pour un long retour à un haut score (récompense = len) / 100) ⋅ utiliser ce modèle de récompense avec défauts pour gérer le PPO, observer le modèle de stratégie pour générer des sorties de plus en plus longues ⋅ répétées ⋅ puis ajouter 0,1 de KL ⋅ punition, montrant qu'il peut prévenir le comportement de dégradation ⋅
...
4. Mettre en œuvre une récompense multi-objectif. Exercer deux modèles de récompense - un pour l'utilité et un pour la concision. Combiner comme R = 0,7 * R_helpful + 0,3 * R_concise. Montrez que l'objectif combiné produit des réponses qui sont à la fois utiles et concises, évitant le piège verbosité d'une seule récompense d'utilité.
   Le modèle de récompense est composé de deux modèles: un pour la utilité, un pour la simplicité. Le modèle de récompense est composé de R = 0,7 * R_helpful + 0,3 * R_concise.
...
5. Comparez différents coefficients KL. Exécutez PPO avec beta=0.001 (trop bas, hacking de récompense), beta=0.02 (standard) et beta=0.5 (trop élevé, pas d'apprentissage).
   Le nombre de récompenses est de 0,001 (exemple: +/- 0,001) et de 0,02 (exemple: +/- 0,002).
