# Récupération de la mise en marche et de l' activation.
...
> **【中文解读】**Réponse: Le problème principal est de savoir quelles sont les réponses qui ne sont pas "toutes".

> **【拓展：梯度检查点→大模型训练】**Le point de contrôle de la température est la réalisation de cette idée. Dans les 128K de formation, le point de contrôle sélectif peut économiser 60% de la réserve.

>  **【前置】**Les techniques de formation sont les techniques essentielles de formation de l'équipe de formation de base.
>  **【类比】**梯度检查点 = 出差只带必要文件──反向传播──工作完成后回顾所有材料;不检查点 = 把出差路上所有的材料都背着(占满行李箱);全检查点 = 只带护照,需要时再印(重算成本高);选择性检查点 = 只扔可重算的(如草稿),保留难重建的(如合同) 平衡存储和速度显著──
...
>  formation transformateur , chaque couche nécessite un stockage à l'envers de propagation de chaque entrée opérationnelle: attention entrée  Q/K/V  projection softmax  output FFN  input 归化输出和残差流`d`、序列长度 `L`、 les lots `B`C'est à peu près à chaque étage.`12 * B * L * d`Il y a des points.
...
> Pour le`d=8192, L=8192, B=1`,BF16 下每层800 MB──64 层模型的激活值为51 GB`L^2`), n'est pas encore prévu dans la partie suivante de la même ligne.
...
> 双面账单:BF16 权重加优化器状态可能放入80GB,但激活值让你超出──梯度检查点(又称激活重计算) est un programme de révision standard── en abandonnant la majeure partie de la valeur active; en faisant de nouveau la pré-diffusion pour les récupérer pendant la propagation inverse──价格: FLOPs supplémentaires──收益:显存按检查段段数与总层数的比例下降──
...
> **【中文解读】**梯度检查点(Gradient Checkpointing) utilise le calcul du changement de stock: avant de propagation ne conserve pas la valeur active moyenne, contre la propagation le recalcul de la nécessité de l'activation. Ceci sera à un coût de 30% environ de calcul supplémentaire de la vente, la valeur active de stockage diminuera d'environ 70%。

> **【拓展：梯度检查点在训练中的关键作用】**Le point de contrôle de la gradience est la technique de marquage du grand modèle de formation. Pour Llama 3 70B, aucun point de contrôle ne nécessite environ 16 Go de valeur active de stockage, puis est réduit à environ 5 Go.
...
> **【拓展：梯度检查点与 FSDP 的配合】**Les points de contrôle de la gradience sont généralement utilisés en combinaison avec FSDP/ZeRO. Les paramètres de la gradience de la gradience de la gradience de la gradience sont utilisés pour réduire la valeur d'activation de la gradience de l'optimisateur.
...
> 本课产 出 `outputs/prompt-activation-recompute-policy.md` un modèle accepté configuration (nombre de niveaux, dimension cachée, longueur de séquence, lot de temps) et GPU disponible en interne et sortie pour chaque niveau de stratégie de calcul (nombre de niveaux, taille de séquence, taille de déchargement) 
...
1. Vérifiez la précision.`model_forward`+ `model_backward`(activations complètes)`model_forward_checkpointed`+ `model_backward_checkpointed`Les gradients de paramètre doivent être identiques à la précision de la machine.
   Le mot "réalisation" est traduit par "réalisation".`model_forward`+ `model_backward`(完整激活) avec `model_forward_checkpointed`+ `model_backward_checkpointed`(分段) ◊ Les paramètres doivent être en accord avec la précision de l'appareil.

2. Taille du segment de balayage `k`de 1 à `L`- Trouvez le genou de la courbe.
   Le grand épisode de la série`k`De 1 à 1`L`◊ dessiner FLOP 开销和内存── trouver des courbes de la courbe
...
3. Mettre en œuvre un point de contrôle sélectif: stocker l'entrée du module d'attention mais pas ses intermédiaires. Mesurer le point de contrôle FLOP en fonction de la charge supérieure par rapport au point de contrôle à couche complète pour un modèle de 32 couches à seq=8192.
   Le résultat de la mise en œuvre de la méthode de calcul de la valeur de l'échantillon est de 32 niveaux.
...
4. Ajouter le déchargement. Enregistrer les entrées de segment dans un "buffer du processeur" simulé (une liste distincte). Mesurer "largeur de bande PCIe" en octets/temps et trouver le point d'équilibre entre le déchargement et le recomptage.
   Le code de déchargement est un code de déchargement de données qui est utilisé pour la mise à jour de données.
...
5. Réservez un véritable transformateur PyTorch avec et sans `torch.utils.checkpoint`. Mesurer la mémoire (via `torch.cuda.max_memory_allocated`) et le temps de marche.
   Le transformateur PyTorch est utilisé pour la transformation de pyTorch.`torch.utils.checkpoint`进行基准测试──测量内存(通过 `torch.cuda.max_memory_allocated`Il y a beaucoup de temps.
