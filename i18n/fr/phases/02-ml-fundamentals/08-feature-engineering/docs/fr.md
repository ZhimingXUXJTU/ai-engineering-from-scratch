...
- Implémenter des transformations numériques (standardisation, mise à l'échelle minimale, transformation log, encastrement) et expliquer quand chacune est appropriée
  实现数值变换(标准化、Min-Max 缩放、对数变换、分箱) et expliquer leurs propres scénarios d'utilisation
- Construire un code unique, une étiquette et un code cible pour les caractéristiques catégoriques et identifier le risque de fuite de données dans le code cible
  Construire un code unique, un code d'étiquette et un code objectif, identifier le code objectif risque de fuite de données
- Construire un vectorificateur TF-IDF à partir de zéro et expliquer pourquoi il dépasse le nombre de mots bruts pour la classification du texte
  De la conception TF-IDF vers le quantificateur, expliquer pourquoi il est meilleur que le nombre de fréquences
- Appliquer la sélection des caractéristiques basée sur le filtre (troisons de variance, corrélation, informations mutuelles) pour réduire la dimensionnalité
  application basée sur des caractéristiques de sélection √√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√√


> **【中文解读】**
> La méthode de traitement des données est la plus longue de la méthode de traitement des données. La méthode de traitement des données est la plus longue de la méthode de traitement des données.

> **【拓展：特征工程 vs 深度学习的自动特征学习】**
> Le principal avantage de l'apprentissage en profondeur est le caractère d'apprentissage automatique, mais il est toujours essentiel de le faire. Les champions de compétition passent souvent 80% de leur temps sur le caractère. Le programme gagnant du Netflix Prize contient des centaines de caractéristiques de conception manuelle.
...
> Puis quelqu'un a transformé les données originales en meilleures caractéristiques, un simple retour logique a battu le degré de régulation de l'intégration.
...
> Dans le classique ML, la représentation des données est plus importante que le choix des algorithmes. Un modèle de prix avec "l'espace" et "numéro de chambre" va à l'encontre du modèle de "l'adresse des fichiers originaux", peu importe la complexité du machine à apprendre.
...
> Le trait de construction est le processus de transformation des données originales en des modèles plus faciles à trouver par le modèle. Le choix des traits est le processus de rejet des traits qui ne font que augmenter le bruit et ne augmentent pas le signal.

> **【中文解读】**
> " Les données et les caractéristiques déterminent la limite supérieure de ML, le modèle et l'algorithme ne sont que près de cette limite supérieure. " Les bonnes caractéristiques permettent de faire en sorte que les modèles simples battent les modèles complexes.
...
> Le nombre de nombres primitifs est très peu utilisé directement dans le modèle.
...
> **缩放：**Les caractéristiques seront mises dans la même gamme, en fonction de l'algorithme de distance K-Means、KNN、SVM) équivalent à tous les caractéristiques.
...
> **对数变换：**La population est une population de plus en plus nombreuse.
...
> **分箱：**La relation entre les caractéristiques et les objectifs est non-lineaire mais utile en classe de niveau.
...
> **多项式特征：**Créer des modèles de x^2、x^3、x1*x2 项── faire en sorte que les modèles de x^2、x^3、x1*x2 项──
...
> **独热编码：**Pour chaque catégorie, créer une deuxième catégorie: "color = red/blue/green" 变成三列:is_red、is_blue、is_green。
...
> **标签编码：**Pour chaque classe de cartes, le nombre total est: rouge = 0, bleu = 1, vert = 2, introduit un faux ordre de répartition.
...
> **目标编码：**Remplacer chaque catégorie par la valeur moyenne des variables cibles de cette catégorie.
...
> **词频向量化：**計算每个词在文档中出现的次数──"le chat s'est assis sur le tapis" 变成 {le: 2, le chat: 1, sat: 1, sur: 1, mat: 1}──
...
- **Drop rows:**Seulement lorsque les données manquantes sont rares et aléatoires
  **删除行：** seulement lorsque la disparition de données est rare et occasionnelle
- **Mean/median imputation:**Simple, préservant la forme de distribution (la moyenne est plus robuste aux écarts)
  **均值/中位数填充：**简单, maintenir la forme de distribution
- **Mode imputation:**Pour les caractéristiques catégoriques
  **众数填充：**Utilisé pour les caractéristiques de classe
- **Indicator column:**Ajouter une colonne binaire "was_this_missing" avant d'imputer.
  **指示列：**填充前添加二进制列"était_ce_disparu"── la carence de données elle-même peut être informative
- **Forward/backward fill:**Pour les données de séries temporelles
  **前向/后向填充：**Utilisé pour les données de séquences de temps
...
> Il existe une relation entre les composants. Le "tallure" et le "poids" sont uniquement des composants de "BMI = poids / hauteur de taille" et sont donc des composants de "tallure" et de "poids" de "poids" et de "poids" de "poids" et de "poids" de "poids" et de "poids" de "poids" et de "poids" de "poids" et de "poids" de "poids" et de "poids" de "poids" et de "poids" de "poids" et de "poids" et de "poids" de "poids" et de "poids" de "poids" et de "poids" de "poids" et de "poids" de "poids" de "poids" et de "poids" de "poids" de "poids" de "poids" de "poids" et de "poids" de "poids" de "poids" de "poids" de "poids" et "pour cela, l'utilisation du savoir choisir correct composant de "poison".
...
- Corrélation: supprimer les caractéristiques fortement corrélées entre elles (rédundantes)
  相关性: déplacement hautement相关的特征(冗余)
- Informations mutuelles: mesure la mesure dans laquelle la connaissance d'une caractéristique réduit l'incertitude quant à l'objectif
  互信息: Mesurer savoir un trait peut réduire l'objectif combien l'incertitude
- Térubin de variance: supprimer les caractéristiques qui varient à peine
  方差值: déménagement des caractéristiques pratiquement inchangées

**Wrapper methods (model-based):**
- L1 régularisation (Lasso): entraîne des poids de caractéristiques irrélevants à zéro exactement
  L1 L'équilibre: sera sans rapport avec le pouvoir de poids
- Élimination de la fonction récursive: entraînement, suppression de la fonction la moins importante, répétition
  递归特征消除: entraînement, déplacement des caractéristiques les plus insignifiantes, répétition
...
> **为什么选择很重要：**Un modèle avec 10 bonnes caractéristiques gagne généralement 10 bonnes caractéristiques plus 90 caractéristiques sonores.

## Construisez-le et mettez-le en œuvre.

> **【中文解读】**
> De la mise en œuvre de la transformation des caractéristiques habituelles: standardisation (en)                                                                                                                                                                                                                                                     
```figure
feature-scaling
```
...
> Troisième étape: la fréquence de chaque mot dans le document est multipliée par la fréquence du mot dans le document, réduisant le poids du mot courant, augmentant le poids du mot rare.`TfidfVectorizer`C'est la version de production réalisée.
...
> La valeur moyenne de remplissage est la valeur moyenne de remplissage pour les données de distribution correcte, la moyenne est la valeur moyenne pour les données de distribution correcte, la moyenne est la valeur moyenne pour les données de distribution correcte, la moyenne est la valeur moyenne pour les données de distribution correcte.
...
> La première étape: sélection de caractéristiques. Les facteurs associés à la mesure des relations interlignées (de 1 à +1); l'information mutuelle peut capturer des relations non-lignées, plus complètes mais qui doivent être dispersées; la différence entre les caractéristiques est presque inchangée; la relation est supérieure à la relation redondante (la différence est supérieure à la relation de la relation).`SelectKBest`- Je suis là.`VarianceThreshold`C'est la méthode de production de ces produits.
...
> Le colonneTransformer + pipeline de sklearn est la meilleure pratique de l'ingénierie des caractéristiques: traiter séparément les caractéristiques numériques et les caractéristiques de catégorie, composer une ligne de flux d'un bout à l'autre. Cela permet d'utiliser des ensembles de formation et de test entièrement identiques, d'éviter les fuites de données. Dans les projets de Kaggle, Pipeline est une pratique standard qui permet de reproduire le code, de le déployer, de le maintenir.

Avec scikit-learn, ces transformations sont des pipelines composables:

> Utilisez un peu de l'apprentissage, ces changements peuvent être combinés pour la ligne de conduite:
...
> De la version zéro, il est précisé ce qui se passe à l'intérieur de chaque changement. La version de la série ajoute le traitement des situations de bordure, le support de la matrice rare et le raccordement des tuyaux, mais le principe mathématique est le même.
...
> - `outputs/prompt-feature-engineer.md`- un mot de référence à partir des caractéristiques de l'ingénierie de la systématisation des données

> **【拓展：自动化特征工程——Featuretools 和 AutoML】**
> Les outils de fonctionnalité sont une base de données ouverte d'ingénierie de caractéristiques automatisées, capable de générer automatiquement des milliers de caractéristiques à partir de données relationnelles. Les outils de synthèse de caractéristiques profondes peuvent être modifiés en fonction de la base de la composition pour générer des caractéristiques profondes. Bien que l'apprentissage en profondeur ait réduit la demande de l'ingénierie de caractéristiques manuelles, dans les données de tableau, l'ingénierie de caractéristiques automatisées + le modèle de bois reste l'une des combinaisons les plus fortes.

> **【中文解读】**
> TF-IDF ({{lang-IDF}}) est une méthode classique de l'ingénierie des caractères de texte:TF 衡量词在文档中的频率,IDF 衡量词在所有文档中的稀少程度,如"的"、"是")IDF 低,特征词(如"量子"",区块链")IDF 高──TF-IDF 虽然简单,但在文本分类中至今仍然有效的基线特征.
...
1. Ajouter une mise à l'échelle robuste (en utilisant la plage médiane et interquartile au lieu de la moyenne et de l'écart standard) aux transformations numériques.
   1. En effet, les données de la valeur moyenne sont plus nombreuses que les données de la valeur moyenne.
2. Implémenter un codeur cible exclusif: pour chaque ligne, calculer la moyenne cible en excluant la valeur cible de cette ligne.
   2. 实现留一法目标编码: calculer la valeur moyenne de l'objectif de chaque ligne, en excluant la valeur moyenne de l'objectif de cette ligne.
3. Construisez un pipeline de sélection automatique de fonctionnalités qui combine le seuil de variance, le filtrage de corrélation et le classement des informations mutuelles. Appliquez-le à l'ensemble de données de logement et comparez les performances du modèle (utilisez une régression linéaire simple) avec toutes les fonctionnalités par rapport aux fonctionnalités sélectionnées.
   3. Construire des caractéristiques automatisées de sélection de tuyaux, combiner des différences de valeur, des correlations et des séquences d'informations mutuelles. Appliquer à un ensemble de données de logements, comparer les performances des modèles utilisant toutes les caractéristiques et les caractéristiques sélectionnées.
...
- [Feature Engineering and Selection (Max Kuhn & Kjell Johnson)](http://www.feat.engineering/)- livre en ligne gratuit couvrant l'ensemble du paysage de l'ingénierie des fonctionnalités
  [Feature Engineering and Selection (Max Kuhn & Kjell Johnson)](http://www.feat.engineering/)- couvrant les caractéristiques du projet
- [scikit-learn Preprocessing Guide](https://scikit-learn.org/stable/modules/preprocessing.html)- référence pratique pour toutes les transformations standard
  [scikit-learn 预处理指南](https://scikit-learn.org/stable/modules/preprocessing.html)- Tous les changements de normes
