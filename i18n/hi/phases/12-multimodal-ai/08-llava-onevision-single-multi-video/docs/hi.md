# LLaVA-OneVision: एकल छवि, बहु-छवि, वीडियो एक मॉडल में

> LLaVA-OneVision से पहले (Li et al., अगस्त 2024) ओपन-वीएलएम दुनिया में अलग-अलग वंशज थेः एकल छवियों के लिए LLaVA-1.5, मंटिस और विला जैसे मल्टी-इमेज मॉडल, वीडियो-एलएलएवीए और वीडियो-एलएएमए जैसे वीडियो मॉडल। प्रत्येक ने अपना बेंचमार्क जीता और दूसरों में असफल रहा। LLaVA-OneVision का तर्क था कि एक एकल पाठ्यक्रम एक मॉडल को तीनों परिदृश्यों पर हावी होने के लिए प्रशिक्षित कर सकता है, और कि उभरते कार्य-प्रसार प्रभाव (एक छवि कौशल को वीडियो में निर्यात किया जाता है, बहु छवि तर्क को एक छवि में निर्यात किया जाता है) विशेषज्ञों के योग से अधिक है। यह नुस्खा बहुत सरल हैः एक दृश्य टोकन बजट जो परिदृश्यों में स्थिर रहता है, साथ ही एक स्पष्ट पाठ्यक्रम जो एकल छवि से OneVision (बहु-छवि) से वीडियो में स्थानांतरित होता है। यह पाठ बजट, पाठ्यक्रम और उभरते व्यवहार को पढ़ता है।

> **【中文解读】**LLaVA-OneVision का मुख्य योगदानः एक एकीकृत दृश्य टोकन के साथ  बजट (~ 3000-4000 टोकन) और एक तीन चरण के पाठ्यक्रम को सीखना (~ एक ही समय में तीन प्रकार के दृश्यों के मॉडल को बेहतर बनाने का प्रशिक्षण) ।

> **【拓展：统一多模态模型的产业价值】**वास्तविक उत्पादों में, उपयोगकर्ता एक ही समय में एक ही चित्र, एक ही चित्र और एक ही वीडियो अपलोड कर सकता है। तीन स्वतंत्र मॉडल का रखरखाव करना एक एकीकृत मॉडल से अधिक महंगा है।

**Type:** Build  | **类型：构建**
**Languages:** Python (stdlib, token budget solver + curriculum planner)  | **语言：Python（标准库，token预算求解器 + 课程规划器）**
**Prerequisites:** Phase 12 · 05 (LLaVA), Phase 12 · 06 (any-resolution)  | **前置：阶段12第05课（LLaVA）、阶段12第06课（任意分辨率）**
**Time:** ~180 minutes  | **时长：约180分钟**

>  **【前置】**学本节前 कृपया पहले समझेंःPhase 12·05(LLaVA 投影器) 、Phase 12·06(AnyRes/NaFlex 分辨率调度) 、Phase 11·07(Curriculum Learning 课程学习) ⋅本节是LLaVA 系列的集大成一个模型干三件事──
>  **【类比】**LLaVA-OneVision = "सर्वशक्तिमान स्विस सैन्य刀"── अन्य VLM = 专门的单功能刀(单图刀、多图刀、视频刀)── स्विस सैन्य刀 प्रत्येक कार्य विशेष विशेष刀 की तरह नहीं है, लेकिन अज्ञात परिदृश्य से निपट सकता है; एक एकीकृत टोकन बनाए रखना 预算(3000-4000) = 子 और刀片 की कुल लंबाई तय है, परिदृश्य के आधार पर मुख्य कार्य करते हैं──

## सीखने के लक्ष्य

- एक दृश्य-टोकन बजट डिजाइन जो एकल छवि, बहु-छवि, और वीडियो इनपुट पर स्थिर रखता है।
- एक प्रशिक्षण पाठ्यक्रम आदेश है कि आपदा के बिना वीडियो में एक छवि से कौशल स्थानांतरित करने के लिए प्रशिक्षण पाठ्यक्रम आदेश.
- समझाएं कि एक मॉडल एक ही पैरामीटर की गणना पर विशेषज्ञों को क्यों हराता है जब पाठ्यक्रम सही ढंग से किया जाता है।
- LLaVA-OneVision द्वारा रिपोर्ट की गई तीन उभरती क्षमताओं का नाम देंः मल्टी-कैमरा तर्क, सेट-ऑफ-मार्क प्रॉम्प्टिंग, iPhone स्क्रीनशॉट एजेंट।

## समस्या  समस्या पृष्ठभूमि

छवि, मल्टी-इमेज और वीडियो प्रत्येक मॉडल को अलग तरह से तनाव देते हैं।

एकल छवि उच्च रिज़ॉल्यूशन टोकन (AnyRes, ~ 2880 दृश्य टोकन) की आवश्यकता होती है ताकि ओसीआर और बारीक विवरण को पकड़ सकें। प्रति नमूना बजटः एक छवि, 2880 टोकन।

मल्टी-इमेज मध्यम रिज़ॉल्यूशन (~576 टोकन प्रत्येक) पर कई छवियों की आवश्यकता होती है ताकि छवियों के बीच तर्क संदर्भ में फिट बैठ सके। प्रति नमूना बजटः 4-8 छवियां, 576 प्रत्येक, 2300-4600 टोकन।

वीडियो को समय की गतिशीलता को कैप्चर करने के लिए कम रिज़ॉल्यूशन पर कई फ्रेम (बॉलिंग के बाद प्रति फ्रेम ~ 196 टोकन) की आवश्यकता होती है। प्रति नमूना बजटः 8-32 फ्रेम, 196 प्रत्येक, 1600-6200 टोकन।

> **【中文解读】**तीन प्रकार के टोकन  बजट की मांग अलग-अलग है: एक चित्र उच्च रिज़ॉल्यूशन के साथ (~2880 टोकन), एक चित्र मध्यम रिज़ॉल्यूशन के साथ (~576 टोकन), एक वीडियो कम रिज़ॉल्यूशन के साथ (~196 टोकन) 

यदि आप अलग-अलग मॉडल को प्रशिक्षित करते हैं, तो आप एक बजट चुनते हैं। यदि आप एक मॉडल को प्रशिक्षित करते हैं, तो आपको संदर्भ को उड़ाने के बिना परिदृश्यों के बीच समझदारी से पैमाने पर बजट की आवश्यकता होती है।

पूर्व-वनविज़न, डिफ़ॉल्ट उत्तर था "एक परिदृश्य को प्रशिक्षित करें, दूसरों को अनदेखा करें।" वीडियो-एलएवीए ने अतिरिक्त प्रशिक्षण चरणों के साथ एक छवि मॉडल पर वीडियो को रीफॉर्मेट किया। एलएवीए-नेक्स्ट ने टाइलिंग के साथ मल्टी-इमेज समर्थन जोड़ा। कोई भी तीनों को साफ ढंग से संभाला।

## अवधारणा का मूल अवधारणा

### OneVision टोकन बजट OneVision टोकन बजट

LLaVA-OneVision प्रति नमूना लगभग 3000-4000 टोकन का एक एकीकृत दृश्य टोकन बजट चुनता है, जो प्रति परिदृश्य अलग-अलग आवंटित होता हैः

- एकल छवि / 单图: AnyRes-9 (3x3 टाइल + थंबनेल), प्रत्येक टाइल 384 पर 729 पैच के साथ, प्रति टाइल 2x2 → 182 प्रति आक्रामक द्विआधारी पूलिंग। कुलः 9 * 182 + 182 = 1820 टोकन। या AnyRes-4 प्रति टाइल 729- पर = 2916 + 729.
- मल्टी-इमेज / 多图: प्रत्येक छवि मध्यम रिज़ॉल्यूशन (384, कोई टाइलिंग नहीं), 729 टोकन बिना बंडलिंग के। बजट 6 छवियां → 4374 टोकन।
- वीडियो / 视频: आक्रामक 3x3 द्विआधारी पूल के साथ 384 रिज़ॉल्यूशन पर 32 फ्रेम → प्रति फ्रेम 81 टोकन। कुलः 32 * 81 = 2592 टोकन।

आवंटन लगभग निरंतर कुल टोकन बनाए रखता है। एलएलएम कभी भी एक बैच नहीं देखता है जो इसके संदर्भ को उड़ाता है। एन्कोडर प्रति परिदृश्य अलग-अलग ज्यामिति का उत्पादन करता है, लेकिन एलएलएम एक ही बजट का उपभोग करता है।

> **【中文解读】**核心思想:总标标题 预算保持恒定(约3000-4000), लेकिन वितरण विधि परिदृश्य से भिन्न है。 एकल图片 AnyRes 片+池化,多图片中等分辨率无池化,视频用低分辨率+激进池化但更多──LLM 见的总标题数始终在预算内──

### तीन चरणों का पाठ्यक्रम

तीन चरणों में LLaVA-OneVision ट्रेनेंः

1. एकल छवि एसएफटी (चरण एसआई) / 单图指令微调. सभी डेटा एकल छवि-प्लस-टेक्स्ट हैं। उच्च रिज़ॉल्यूशन एनीरेस इनपुट पर प्रशिक्षित करें। यह धारणा, ओसीआर और बारीक-अमल की समझ सिखाता है। एलएवीए-नेक्स्ट डेटा प्लस वनविजन-विशिष्ट एकल छवि डेटा का उपयोग करता है।
2. OneVision SFT (स्टेज OV) / 统一指令微调. एकल छवि + बहु-छवि + वीडियो (समान रूप से नमूने फ्रेम) मिश्रण करें। एकीकृत टोकन बजट पर प्रशिक्षण दें। इससे मॉडल को विषम बैच आकारों को संभालने के लिए सिखाया जाता है। कोई वजन रीसेट नहीं होता है  स्टेज एसआई से जारी है।
3. कार्य हस्तांतरण (चरण टीटी) / 任务迁移. लक्ष्य कार्य मिश्रण के साथ जारी रखें, आमतौर पर उत्पाद के आधार पर मल्टी-इमेज या वीडियो पर भारी। तैनाती के लिए वैकल्पिक ठीक-ठाक।

महत्वपूर्णः पाठ्यक्रम क्रम मायने रखता है। वीडियो-पहले या मल्टी-इमेज-पहले प्रशिक्षण एक ही डेटा के साथ भी एकल-इमेज-पहले की तुलना में खराब छवि प्रदर्शन पैदा करता है। पेपर इसे स्पष्ट रूप से खारिज करता है।

> ️ **【易错点】**स्व प्रशिक्षण एकीकरण VLM 时课程序列搞反了(先训视频再训单图)→ 单图性能大幅下降──原因:视频低分辨率输入让模型先学到"模糊是正常的",再训高分辨率单图时模型适应不过来──修复:必须单图 → 多图 → 视频的序列,先学精细再学粗──
> 🤔 **【困惑】**प्रश्नः क्यों फिक्स्ड टोकन  बजट इतना महत्वपूर्ण है? क्योंकि LLM की उपर्युक्त निम्न विंडो निश्चित है, एक ही चित्र अचानक 5000 टोकन  वीडियो 10000 टोकन  होगा बैचिंग को नुकसान और अनुमानित बजट  फिक्स्ड बजट = अनुमानित अनुमानित लागत, उत्पाद तैनाती की कुंजी है

> **【中文解读】**क्रम क्रम महत्वपूर्ण: पहले एक चित्र, पुनः एक चित्र+ वीडियो, अंतिम कार्य स्थानांतरण। यदि पहले एक चित्र या एक चित्र का प्रशिक्षण किया जाता है, तो एक चित्र का प्रदर्शन कम हो जाता है।

### क्यों पाठ्यक्रम काम करता है

एकल-छवि प्रशिक्षण धारणा आधार बनाता है। पैच टोकन में बारीक-खरेबी दृश्य विशेषताएं होती हैं; एलएलएम उन्हें पाठ के साथ एकीकृत करना सीखता है। बहु-छवि और वीडियो संरचनात्मक चुनौतियों (कौन सी छवि है, कौन सी पहली हुई) को पेश करते हैं जो एक मजबूत धारणा आधार के बिना सीखना मुश्किल है।

यदि आप सभी परिदृश्यों को खरोंच से एक साथ प्रशिक्षित करते हैं, तो मॉडल धारणा (प्रति बैच सीमित एकल-छवि डेटा) और ओवरफिट संरचना (बहुत सारे मल्टी-छवि / वीडियो डेटा) से कम है। परिणामः एक मॉडल जो क्रॉस-छवि तर्क पैटर्न का अनुसरण करता है लेकिन दृश्य रूप से क्षैतिज है।

पाठ्यक्रम क्रम आपको चरण एसआई से धारणा शक्ति देता है, फिर चरण ओवी से संरचनात्मक/समय तर्क देता है, बिना किसी को खोए।

> **【中文解读】**यदि सभी परिदृश्यों को एक साथ प्रशिक्षित किया जाए, तो मॉडल अनुभूति क्षमता के लिए उपयुक्त नहीं होगा। प्रत्येक बैच में एकल चित्र डेटा सीमित है।

### क्रॉस-सेंसर कौशल उभरती है क्रॉस-सेंसर क्षमता

LLaVA-OneVision पेपर में तीन उभरती क्षमताओं की रिपोर्ट की गई हैः

1. मल्टी-कैमरा तर्क / 多摄像头推理. मल्टी-इमेज + वीडियो पर अलग से प्रशिक्षित; निष्कर्ष पर, मल्टी-कैमरा ड्राइविंग दृश्य के बारे में तर्क करने के लिए कहा गया। मॉडल प्रशिक्षण में कभी भी सटीक प्रारूप नहीं देखने के बावजूद सही ढंग से विचारों को एकीकृत करता है।
2. सेट-ऑफ-मार्क प्रॉम्प्टिंग / 标记提示. उपयोगकर्ता अंकित चिह्नों के साथ एक छवि में वस्तुओं को नोट करता है; मॉडल तर्क "चिह्न 3 चिह्न 7 के सापेक्ष क्या कर रहा है" के बारे में।
3. iPhone-स्क्रीनशॉट एजेंट / 手机截图代理. उपयोगकर्ता एक iPhone स्क्रीन का स्क्रीनशॉट प्रदान करता है और अगले क्लिक की योजना बनाने के लिए कहता है। यूआई स्क्रीनशॉट, उपयोगकर्ता वर्कफ़्लो के वीडियो और जोड़े से पहले / बाद की बहु-छवि पर प्रशिक्षित। एजेंट उपयोग के मामले में सामान्यीकरण।

ये प्रशिक्षित कार्य नहीं हैं; वे पाठ्यक्रम की संरचनात्मक संरचना से उत्पन्न होते हैं।

> **【拓展：涌现能力的工程启示】**涌现能力 का अर्थ है एक एकीकृत मॉडल का मूल्य विभिन्न विशेषज्ञों से परे है और। कई कैमरा तर्क क्षमता सुरक्षा निगरानी, स्वचालित ड्राइविंग के लिए उपयोग किया जा सकता है; निशान सुझाव छवि चिह्न उपकरण के लिए उपयोग किया जा सकता है; मोबाइल काटने एजेंट यूआई स्वचालन परीक्षण के लिए उपयोग किया जा सकता है। ये स्पष्ट रूप से प्रशिक्षित नहीं हैं, बल्कि पाठ्यक्रम सीखने के "उपयोगी उत्पाद" हैं।

### दृश्य-सकेत को जोड़ना

टोकन बजट को pooling की आवश्यकता होती है। OneVision 2D पैच ग्रिड पर द्विआधारी अंतराल का उपयोग करता हैः 24x24 = 576 पैच 12x12 = 144 (2x कारक) या 8x8 = 64 (3x कारक) बन जाता है। स्थानीयता को संरक्षित करने के लिए पैच-ग्रिड स्थान में pooling किया जाता है, टोकन स्थान नहीं।

प्रति परिदृश्य को जोड़ने के कारक का चयन स्वयं एक हाइपरपैरामीटर है। कम जोड़ना = अधिक टोकन = समृद्ध प्रतिनिधित्व। अधिक जोड़ना = कम टोकन = अधिक फ्रेम / छवियां फिट।

> **【中文解读】**池化在2D 补丁网格空间进行(而非 टोकन 空间),以保留空间局部性──池化因子是每个场景的超参数:少池化=更多 टोकन=更丰富表示;多池化=更少 टोकन=可容纳更多/图像──

### LLaVA-OneVision-1.5

2025 के अनुवर्ती (LLaVA-OneVision-1.5, arXiv 2509.23661) प्रशिक्षण डेटा, मॉडल वजन और कोड में "पूरी तरह से खुला" है। कुछ बेंचमार्क पर स्वामित्व अंतर से मेल खाता है और नुस्खा को लोकतांत्रिक बनाता है। एक ही पाठ्यक्रम, अधिक डेटा, बेहतर आधार LLM। कोई वास्तुकला परिवर्तन नहीं।

### Qwen2.5VL के साथ तुलना Qwen2.5VL के साथ तुलना

Qwen2.5-VL (लक्ष 12.09) अलग-अलग विकल्प बनाता है। यह निश्चित पूलिंग के बजाय M-RoPE और गतिशील FPS का उपयोग करता है। इनपुट के साथ इसका बजट स्केल  1 मिनट का वीडियो 5 सेकंड के वीडियो की तुलना में अधिक टोकन का उपयोग करता है। LLaVA-OneVision बजट को तय करता है और पूलिंग को स्केल करता है। दोनों काम करते हैं; वे पूर्वानुमानिता के लिए विन्यासशीलता का आदान-प्रदान करते हैं।

> **【中文解读】**Qwen2.5VL के साथ M-RoPE 和动态率, टोकन  बजट के साथ-साथ इनपुट संकुचन;LLaVA-OneVision 固定 बजट、调整池化── दो प्रकार की रणनीतियों में प्रत्येक में फायदे होते हैंः पहला जीवंत लेकिन लागत अप्रत्याशित, दूसरा लागत नियंत्रण योग्य लेकिन संभावित बर्बाद या अभाव──
```figure
l5-onevision-budget
```

## इसका प्रयोग करें

## इसे प्रयोग करें .

`code/main.py`एक वनविजन शैली VLM के लिए एक पाठ्यक्रम और बजट नियोजक है। प्रति नमूना टोकन बजट और एक लक्ष्य परिदृश्य मिश्रण (कहें 40% एकल छवि, 30% बहु-छवि, 30% वीडियो) को देखते हुए, यहः

- प्रत्येक परिदृश्य के लिए संकल्प, पूर्णांक कारक और फ़्रेम आवंटित करता है।
- जाँचें कि क्या प्रत्येक परिदृश्य साझा बजट के भीतर फिट बैठता है।
- रिपोर्ट अपेक्षित टोकन संख्या, एलएलएम फ्लोप, और क्या परिदृश्य कम टोकन हैं.
- चरण-दर-चरण प्रशिक्षण कार्यक्रम प्रिंट करता है।

## इसे भेजें।

यह सबक हमें फल देता है`outputs/skill-onevision-budget-planner.md`. लक्ष्य कार्य वितरण और प्रति नमूना बजट को देखते हुए, यह एनीरेस कारक, प्रति फ्रेम पूलिंग, वीडियो फ्रेम की संख्या और पाठ्यक्रम चरण वजन जारी करता है। जब भी आप एक एकीकृत परिदृश्य VLM को प्रशिक्षित या ठीक से समायोजित करते हैं तो इसका उपयोग करें।

> **【中文解读】**इस वर्ग में एक दृष्टि  बजट नियोजन उपकरण  दिया गया लक्ष्य कार्य वितरण और प्रत्येक नमूना बजट, आउटपुट AnyRes कारक  वर्ग इकाइकरण  वीडियो  संख्या और पाठ्यक्रम चरणों का वजन  प्रशिक्षण या सूक्ष्म विनियमन के लिए उपयुक्त VLM

## अभ्यास विषय

1. आपके उत्पाद में 80% एकल-छवि, 10% मल्टी-छवि (2-4 छवियां), 10% वीडियो (8-16 फ्रेम) का समर्थन है। टोकन बजट डिजाइन करें। भारी मल्टी-छवि नहीं करने से आप जो अतिरिक्त बजट बचाएंगे, उसे आप कहां डालेंगे?
   | 产品支持 80% 单图、10% 多图（2-4张）、10% 视频（8-16帧）。设计 token 预算。从轻量多图中省下的预算放在哪？

2. LLaVA-OneVision सेक्शन 4.3 (उत्पन्न क्षमताएं) पढ़ें। पाठ्यक्रम में संभवतः एक चौथा उभरता हुआ कौशल का प्रस्ताव दिया जाएगा लेकिन पेपर ने रिपोर्ट नहीं की।
   | 阅读 LLaVA-OneVision 第 4.3 节（涌现能力）。提出课程学习可能解锁但论文未报告的第四种涌现技能。

3. पाठ्यक्रम क्रम को बदलें  पहले मल्टी-इमेज ट्रेन, फिर एकल-इमेज, फिर वीडियो। भविष्यवाणी करें कि कौन से बेंचमार्क गिराए जाते हैं और क्यों।
   | 交换课程顺序——先多图，再单图，最后视频。预测哪些基准会下降以及原因。

4. पेपर में वीडियो बेंचमार्क की रिपोर्ट है जो प्रति नमूना केवल 8 फ्रेम पर प्रशिक्षित किए गए हैं। क्या यह निष्कर्ष पर 30 सेकंड के वीडियो में सामान्य हो जाता है? क्या पहले टूट जाता है  टोकन बजट या समय तर्क?
   | 论文报告视频基准只用每样本8帧训练。这对推理时的30秒视频泛化吗？先崩溃的是 token 预算还是时序推理？

5. 24x24 पैचों को 12x12 तक द्विआधारी पूलिंग प्रति dim में 4x कमी है। stdlib पायथन में पूलिंग को लागू करें और सत्यापित करें कि प्रत्येक 2x2 ब्लॉक पर औसत द्विआधारी आउटपुट से मेल खाता है।
   | 将 24x24 补丁双线性池化为 12x12 是每维 4 倍缩减。用标准库 Python 实现池化，验证每个 2x2 块的均值与双线性输出一致。

## Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key Terms  Key

| Term | What people say | What it actually means | 中文含义 | 中文释义 |
|------|-----------------|------------------------|----------|---------|
| OneVision scenario | "Single-image, multi-image, or video" | One of three input shapes the unified VLM handles; the budget stays constant across | 统一 VLM 处理的三种输入形态之一，预算在场景间保持恒定 | |
| Token budget | "How many tokens per sample" | Total visual tokens the LLM sees per training / inference sample, typically 3000-4000 | LLM 每样本看到的总视觉 token 数，通常 3000-4000 | |
| Curriculum | "Training order" | Stage ordering (single-image → multi-image → video) chosen for emergent transfer | 课程学习：按单图→多图→视频顺序训练，促进技能迁移 | |
| Bilinear pooling | "Token shrink" | Applying bilinear interpolation to the patch grid (2D) to reduce token count while preserving locality | 在补丁网格上做双线性插值，减少 token 数并保留空间局部性 | |
| Emergent skill | "Not trained, still works" | Capability that appears at inference without matching training data, due to curriculum composition | 课程学习组合带来的未训练即涌现的能力 | |
| AnyRes-k | "k-tile setup" | k sub-tiles of fixed resolution plus one thumbnail, typical k ∈ {4, 9} | k 个固定分辨率子切片加一个缩略图 | |
| Task transfer | "Cross-scenario generalization" | Skills learned on single-image that apply to video (and vice versa) via shared backbone | 通过共享骨干网络，单图技能迁移到视频（反之亦然） | |

## आगे पढ़ना 延伸閱讀

- [Li et al. — LLaVA-OneVision (arXiv:2408.03326)](https://arxiv.org/abs/2408.03326)➡️LLaVA-OneVision
- [LLaVA-OneVision-1.5: Fully Open Framework (arXiv:2509.23661)](https://arxiv.org/abs/2509.23661) पूरी तरह से खुला स्रोत संस्करण
- [Lin et al. — Video-LLaVA (arXiv:2311.10122)](https://arxiv.org/abs/2311.10122)∙ वीडियो-LLAVA  वीडियो बहुआयामी
- [Lin et al. — VILA (arXiv:2312.07533)](https://arxiv.org/abs/2312.07533) विला बहु चित्र मॉडल
- [Wang et al. — Qwen2-VL (arXiv:2409.12191)](https://arxiv.org/abs/2409.12191) Qwen2-VL तुलना करें
