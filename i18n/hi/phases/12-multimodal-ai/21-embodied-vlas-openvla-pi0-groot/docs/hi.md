# अवतार VLAs: RT-2, OpenVLA, π0, GR00T 具身 VLA: विज़न-भाषा-आकार मॉडल और मशीन नियंत्रण

> पहली बार एक मॉडल ने एक वेबसाइट से एक नुस्खा पढ़ा और इसे रसोई की रोबोट में निष्पादित किया RT-2 (Google DeepMind, जुलाई 2023) था। RT-2 ने टेक्स्ट टोकन के रूप में कार्यों को गुप्त रखा, वेब डेटा और रोबोट-एक्शन डेटा पर एक VLM को सह-अपरिभाषित किया, और साबित किया कि वेब-स्केल दृष्टि-भाषा ज्ञान रोबोट नियंत्रण में स्थानांतरित होता है। OpenVLA (जून 2024) ने खुला 7B संदर्भ भेजा। फिजिकल इंटेलिजेंस की π0 श्रृंखला (2024-2025) ने प्रवाह-समानता कार्रवाई विशेषज्ञों को जोड़ा। एनवीआईडीआईए के GR00T N1 (मार्च 2025) ने मानवयुक्त रोबोटों के लिए दोहरी प्रणाली (सिस्टम 1 / सिस्टम 2) नियंत्रण प्रदान किया। वीएलए आदिम  दृष्टि-भाषा-क्रिया, एक एकल मॉडल जो देखता है, पढ़ता है, और कार्य करता है  इस चरण के समझ मॉडल और चरण 15 में स्वायत्त प्रणालियों के बीच पुल है।

> **【中文解读】**RT-2  प्रथम प्रमाण नेटवर्क स्तर विज़ुअल भाषा ज्ञान को यंत्र नियंत्रण में स्थानांतरित किया जा सकता हैः将关节动作分散化为文本代币,与VLM 联合微调――OpenVLA 是开源 7B 参考,π0 引入流匹配动作专家,GR00T N1 实现双系统(快思考/慢思考) 人形机器人控制──VLA(视觉-语言-动作) 连接多模态理解和自主系统的桥梁──

> **【拓展：Embodied VLA 到机器人产业】**VLA मॉडल प्रयोगशाला से उद्योग की ओर बढ़ रहा हैः Tesla Optimus, Figure 01、1X Technologies, आदिम रूप के मशीनों की कंपनी VLA पर आधारित नियंत्रण प्रणाली के विकास में जुड़ी हुई है। औद्योगिक परिदृश्य में, VLA भंडारण सामग्री के लिए उपयोग किया जा सकता है, उपकरण उपकरण, स्थापना लाइन ऑपरेटिंग मशीन आदि।

**Type:** Learn
**Languages:** Python (stdlib, action tokenizer + VLA inference skeleton)
**Prerequisites:** Phase 12 · 05 (LLaVA), Phase 15 (Autonomous Systems, referenced)
**Time:** ~180 minutes

>  **【前置】**学本节前 कृपया पहले समझेंःPhase 12·05(LLaVA VLM 基础) Phase 15·01(एजेंट 循环)  नियंत्रण सिद्धांत आधार (关节空间、末端执行器位姿) VLA = VLM 输出文本变成机器人动作,是Phase 12到Phase 15(自主系统) के पुल──
>  **【类比】**VLA = "给机器人装大脑和眼睛"──传统机器人 = 程序员写死 if-else 规则(见红色就停下);VLA = 像人看图说话做事("把红杯放到桌上"→见杯→规划路径→控制关节执行)──RT-2 把动作分散 टोकन में डाल = 动作作为文字写进即时;π0 流匹配 = 输出连续动作而非分散 टोकन,更精确──

## सीखने के लक्ष्य

- क्रिया टोकनकरण का वर्णन करेंः डिस्क्रेट बिन एन्कोडिंग (RT-2), फास्ट कुशल क्रिया टोकन, निरंतर प्रवाह-संगत क्रियाएं (π0).
  中文翻译:描述动作分词化:离散 बिन 编码(RT-2) 、FAST 高效动作符号、连续流匹配动作(π0)。
- वेब + रोबोट डेटा पर सह-अच्छी ट्यूनिंग से नए कार्यों में सामान्य ज्ञान हस्तांतरण को क्यों संरक्षित किया जाता है, इसका वर्णन करें।
  चीनी अनुवादः समझाएँ क्यों वेब पेज+ मशीन डेटा पर संयुक्त सूक्ष्म调能 सामान्य ज्ञान को नए कार्य में स्थानांतरित करने की क्षमता को बरकरार रखने में सक्षम है।
- एक ही रोबोट कार्य पर OpenVLA (खुला 7B Llama+VLM), π0 (प्रवाह-अनुरूपता), और GR00T N1 (द्वितीय-प्रणाली) की तुलना करें।
  中文翻译:在同一机器人任务上比较 OpenVLA(开放 7B Llama+VLM) 、π0(流匹配) और GR00T N1(双系统) ✿
- ओपन एक्स-एम्बॉडीमेंट डेटासेट और आरटी-एक्स प्रशिक्षण कॉर्पस के रूप में इसकी भूमिका का नाम बताइए।
  中文翻译:列举 ओपन एक्स-एम्बॉडीमेंट 数据集及其作为 RT-X 训练语料的角色──

## समस्या  समस्या परिचय

एक रोबोट जो प्राकृतिक भाषा के निर्देशों से घर का काम करता है, 1970 के दशक से एक शोध लक्ष्य रहा है। 2020 के उत्तरः एक दृष्टि-भाषा-क्रिया (वीएलए) मॉडल। उसी वीएलएम वास्तुकला का उपयोग वीक्यूए के लिए किया जाता है, लेकिन आउटपुट पाठ के बजाय क्रियाएं (संयुक्त टोक़, अंत-इफेक्टर मुद्राएं, अलगाव आदेश) हैं।

> विजुअल लैंग्वेज-आकारों (VLA) मॉडल, लेकिन आउटपुट एक ही VLM संरचना का उपयोग करते हैं, लेकिन यह एक ही है।

VLA के लिए विशिष्ट चुनौतियांः

> VLA के लिए विशेष चुनौतियांः

1. क्रिया स्थान निरंतर (संयुक्त कोण, बल) और उच्च आयामी हैं (7-DOF हाथ + 3-DOF पकड़ = 30 हर्ट्ज पर 10 dims) ।
   中文翻译:动作空间是连续的(关节角度、力) 且高维(7自由度臂 + 3自由度爪 = 30Hz 下 10 维) ⋅
2. रोबोट-विशिष्ट प्रशिक्षण डेटा दुर्लभ है। ओपन एक्स-एम्बॉडीमेंट में ~ 1M पटरियां हैं; वेब टेक्स्ट-इमेज 5B + है।
   中文翻译:机器人专用训练数据稀缺──Open X-Embodyment 约100万条轨迹;网页文本-图像有50亿+──
3. नियंत्रण आवृत्ति मायने रखता है. 30 हर्ट्ज नियंत्रण लूप प्रति कार्रवाई 33ms बजट का मतलब है.
   中文翻译:控制频率很重要──30Hz 控制回路意味着每动作 33ms 预算──
4. सुरक्षा. गलत कार्रवाई हार्डवेयर, इंसानों या संपत्ति को नुकसान पहुंचाती है।
   चीनी अनुवादः सुरक्षा।

## अवधारणा का मूल अवधारणा

> **【中文解读】**具身視觉-语言-动作模型(VLA) 让机器人理解语言指令和视觉场景后执行物理动作──OpenVLA 是开源VLA,pi0(भौतिक बुद्धिमत्ता) और NVIDIA Groot 是具身智能的代表性模型──VLA = 视觉编码器 + LLM + 动作解码器──

> **【拓展：具身智能的进展**OpenVLA-7B Google रोबोट पर लगभग 80% के मिशन सफलता दर को पूरा करता है। प्रवाह मिलान का उपयोग करके निरंतर गतिशीलता का निर्माण किया जाता है।


### कार्रवाई टोकन (RT-2)

RT-2 की चालः प्रत्येक संयुक्त लक्ष्य को एक क्वांटिज़्ड टेक्स्ट टोकन के रूप में प्रस्तुत करें। सामान्यीकृत [-1, 1] रेंज को 256 बिन्स में विभाजित करें, प्रत्येक बिन्स को शब्दावली आईडी के लिए मैप करें। प्रत्येक नियंत्रण चरण पर 10-DOF कार्रवाई 10 टोकन बन जाती है।

> RT-2 के टिप्स: 将每个关节目标表示为量化文本代币――将归结化的 [-1, 1] 范围离散化为 256 个bin,每个bin 映射到一个词汇 ID──10自由度动作在每个控制步骤变成10个 token──

एक मिश्रण पर एक PaLM-X VLM को सह-अच्छी तरह से समायोजित करेंः

> मिश्रित डेटा पर संयुक्त सूक्ष्म समायोजन PaLM-X VLM:

- वेब छवि-पाठ जोड़े (शीर्षक, VQA) ।
  中文翻译:网页图文对(描述、VQA) ⋅
- रोबोट प्रदर्शन, टोकन के रूप में कार्रवाई।
  中文翻译:机器人演示,动作为标志──

मॉडल "लाल घन" (भाषा) → छवि (दृष्टि) → 10-टोकन एक्शन अनुक्रम (विज्ञप्त संयुक्त लक्ष्य) देखता है। वेब प्री-ट्रेनिंग सामान्य ज्ञान हस्तांतरण को संरक्षित करता हैः RT-2 "त्वरित गतिशील वस्तु की ओर कदम" का पालन कर सकता है भले ही "त्वरित गतिशील" प्रशिक्षण डेटा में नहीं है।

> 模型看"拿起红色方块" (→ 图像) 视觉) → 10 टोकन 动作序列 (→ 动作序列)  离散关节目标)  网页预训练 ने सामान्य ज्ञान迁移 (→ 转移) को बरकरार रखाःRT-2 能执行"移向快速移动的物体", भले ही"快速移动" प्रशिक्षण डेटा में शामिल न हो।

आरटी-2 पेपर में 3-5 हर्ट्ज पर इन्फेरेंस, VLM ऑटोरेग्रेसिव डिकोड द्वारा सीमित।

> RT-2 论文中推理速度 3-5 Hz, VLM से ही सीमित है

### OpenVLA  खुला 7B संदर्भ

ओपनवीएलए (किम और सहयोगियों, जून 2024) ओपन-वेट आरटी-2 समकक्ष है। 7 बी लामा रीढ़ की हड्डी, DINOv2 + सिगलिप ड्यूल विजन एन्कोडर, 256 बिन्स पर एक्शन टोकनाइज़ेशन।

> OpenVLA is open权重的RT-2等价物──7B लामा 主干,DINOv2 + SigLIP 双视觉编码器,256 बिन 动作分词化──

ओपन एक्स-एम्बॉडीमेंट (970 हजार पटरियों पर 22 रोबोट) पर प्रशिक्षित। नए रोबोट के अनुकूलन के लिए लोरा बारीक-ट्यूनिंग समर्थन वाले जहाज।

> ️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️

इन्फेरेंसः क्वांटिज़ेशन के साथ A100 पर 4-5 हर्ट्ज. धीमी हेरफेर के लिए पर्याप्त तेज, उच्च आवृत्ति नियंत्रण के लिए नहीं।

> 推理:A100 上量化后 4-5 Hz──对慢速操作足够,不适合高频控制──

### फास्ट टोकनराइज़र  तेजी से कार्रवाई डिकोड

Pertsch et al. (2024) ने दिखाया कि डिस्क्रिट-बिन टोकनाइजेशन अक्षम है  बिन-स्पेस के एक छोटे क्षेत्र में अधिकांश एक्शन क्लस्टर। FAST (फ्रिक्वेन्सी-डोमेन एक्शन सीक्वेंस टोकनाइज़र) DCT के माध्यम से एक्शन अनुक्रमों को संपीड़ित करता है और गुणांक को मात्राबद्ध करता है।

> Pertsch 等人(2024) ने दिखाया कि विभाजन की गतिशीलता की दर कम है अधिकांश आंदोलनों का एक साथ संचय होना 空间 के छोटे क्षेत्र में है FAST(频域动作序列分词器) द्वारा DCT 压缩动作序列并量化系数──

एक 30-चरण कार्रवाई की प्रक्षेपवक्र 300 विवश-बिन टोकन के बजाय ~ 10 फास्ट टोकन बन जाता है। गुण हानि के बिना तर्क 3-5 गुना तेज होता है।

> 30 步动作轨迹 लगभग 10 快速 टोकन बन गया, 300 散 bin टोकन नहीं बल्कि  गति वृद्धि 3-5 倍, गुणवत्ता हानि नहीं 

### π0 और प्रवाह-समानता कार्य

फिजिकल इंटेलिजेंस का π0 (ब्लैक एट अल, अक्टूबर 2024) एक प्रवाह-अनुरूपता कार्रवाई विशेषज्ञ के साथ अलग-अलग कार्रवाई टोकन को बदल देता हैः

> फिजिकल इंटेलिजेंस का π0 उपयोग कर प्रवाह匹配动作专家替代离散动作 टोकन:

- एक छोटा एक्शन ट्रांसफार्मर VLM की छिपी हुई अवस्थाओं को पढ़ता है और सही प्रवाह के माध्यम से एक निरंतर 50-चरण एक्शन अनुक्रम आउटपुट करता है।
  中文翻译:小型动作 ट्रांसफार्मर 读取 VLM 隐藏状态,通过正流输出连续的50 步动作序列──
- एक्शन हेड प्रवाह-अनुरूपता हानि के साथ ट्रेन करता है; VLM पूर्व-प्रशिक्षण अपरिवर्तित रहता है।
  中文翻译:动作头用流匹配损失训练;VLM 预训练不变──
- इन्फरेंसः ~5 डीनोइंग स्टेप्स में उत्सर्जित पूर्ण एक्शन सीक्वेंस, प्रभावी रूप से 50 हर्ट्ज नियंत्रण।
  中文翻译:推理:完整动作序列在约5 步去噪音中输出,等效 50Hz 控制──

π0 का दावाः एक व्यापक सेट में हेरफेर कार्यों पर OpenVLA और Octo से बेहतर है। निरंतर कार्रवाई के सूत्र को चिकनाई बनाए रखता है जो विवशता नष्ट करता है।

> π0  दावा: व्यापक परिचालन कार्य में OpenVLA और Octo को हराया।

> **【中文解读】**π0 उपयोग प्रवाह संगत प्रतिस्थापन विघटन गति टोकन: एक छोटा गति ट्रांसफार्मर 读取 VLM 隐藏状态,通过正流输出连续的50 步动序列──推理时只需约5 步去噪音,实现等效50Hz 控制频率──连续动作表达保留离散会破坏的动作平滑性──

π0.5 और π0-FAST क्रमिक उन्नयन हैं. π0-FAST FAST टोकनकरण को प्रवाह मिलान के साथ जोड़ता है।

> π0.5 तथा π0-FAST वृद्धि वृद्धि है।

### मानवयुक्तों के लिए GR00T N1  दोहरी प्रणाली

NVIDIA का GR00T N1 (मार्च 2025) मानवयुक्त रोबोट (>30 DOF, पूर्ण शरीर) के लिए बनाया गया हैः

> NVIDIA का GR00T N1 为人形机器人设计 ((>30 स्वतंत्रता度, संपूर्ण):

- प्रणाली 2: एक बड़ा VLM पढ़ने दृश्य + निर्देश, ~ 1 हर्ट्ज पर उच्च स्तर के उप-लक्ष्य का उत्पादन।
  中文翻译:系统 2: बड़े पैमाने पर VLM 读取场景+ निर्देश,以约1Hz 生成高层子目标──
- प्रणाली 1: एक छोटा एक्शन हेड ट्रांसफार्मर जो उप-लक्ष्य पर कंडीशनिंग वाले कम-स्तरीय 50-100 हर्ट्ज संयुक्त कमांड का उत्पादन करता है।
  中文翻译:系统 1:小型动作头 ट्रांसफार्मर 根据子目标生成底层 50-100Hz 关节命令。

कैनेमैन के तेज और धीमी सोच के लिए विभाजित नक्शेः सिस्टम 2 योजनाएं, सिस्टम 1 कार्य करता है। लाभः धीमी VLM आकार की योजना तेजी से नियंत्रण को अवरुद्ध नहीं करती है; सिस्टम 1 विलंबता के लिए छोटा रहता है।

> इस प्रकार का विभाजन काणिमैन के त्वरित विचार में होता हैः प्रणाली 2 规划, प्रणाली 1 执行―― लाभः धीमी गति VLM 级规划不会阻阻快速控制; प्रणाली 1 保持小规模以保证低延迟――

GR00T N1.7 (अंतिम 2025) डेटा स्केलिंग में सुधार करता है। GR00T ओम्निवर्स से सिम-टू-रियल डेटा के साथ ठीक-ठाक करता है।

> GR00T N1.7 ((2025 साल के अंत) ने डेटा विस्तार को सुधार दिया।

### खुला एक्स-बॉडीमेंट

प्रशिक्षण डेटा। RT-X (अक्टूबर 2023) ने 22 रोबोटों पर 1M पटरियों को कवर करने वाले 22 डेटा सेट इकट्ठा किए। ओपन एक्स-एम्बॉडीमेंट सभी का उपयोग करने वाला कॉर्पस हैः

> 训练数据──RT-X(2023 साल 10 月) ने 22 DATABET को एक साथ जोड़ा, 22 机器人 के 100,000 条轨迹 को कवर किया।

- ALOHA / ब्रिज V2 / Droid / RT-2 किचन / भाषा तालिका।
  中文翻译:ALOHA / ब्रिज V2 / ड्रोइड / RT-2 किचन / लैंग्वेज टेबल。
- प्रत्येक नमूनाः (रोबोट की स्थिति, कैमरा दृश्य, निर्देश, कार्रवाई अनुक्रम) ।
  中文翻译: प्रत्येक样本:(机器人状态、摄像头视图、指令、动作序列)
- प्रशिक्षण स्वच्छताः एक्शन स्पेस को एकजुट करें, संयुक्त रेंज को सामान्य बनाएं, कैमरों का आकार बदलें।
  中文翻译: प्रशिक्षण规范:统一动作空间、归一化关节范围、统一摄像头分辨率──

ओपन एक्स-एम्बॉडीमेंट पर ओपनवीएलए और पी0 ट्रेन। किसी भी विशिष्ट रोबोट के लिए डोमेन अंतर 100-1000 कार्य-विशिष्ट डेमो पर लोरा फाइन-ट्यूनिंग द्वारा बंद हो जाता है।

> OpenVLA 和 π0 में Open X-Embodiment 上 प्रशिक्षण── विशिष्ट यंत्रों के साथ क्षेत्र अंतर के माध्यम से 100-1000 个任务 विशिष्ट प्रदर्शन के लोरा 微调来弥合──

### केवल रोबोट के साथ सह-अच्छी ट्यूनिंग

सह-अच्छी ट्यूनिंग वेब VQA डेटा को रोबोट पटरियों के साथ मिलाती है। अनुपात मायने रखता हैः बहुत अधिक VQA और मॉडल कार्यों को भूल जाता है; बहुत अधिक रोबोट डेटा और मॉडल सामान्य ज्ञान खो देता है।

> 联合微调将网页 VQA 数据与机器人轨迹混合──比例很重要:VQA 太多模型忘记动作;机器人数据太多模型失去通用知识──

RT-2 का अनुपातः ~1:1. OpenVLA: ~0.5:1 वेब-टू-रोबोट. π0: समान। सटीक अनुपात डेटासेट आकार के अनुसार ट्यून करने के लिए एक हाइपरपैरामीटर है।

> RT-2 का अनुपात लगभग 1:1 है। OpenVLA  लगभग 0.5:1 है।

केवल रोबोट प्रशिक्षण कार्य-विशिष्ट मॉडल उत्पन्न करता है जो वितरण के बाहर निर्देशों पर विफल रहता है। सह-अच्छी ट्यूनिंग "लाल घन (डेमो में) उठाएं" और "बाएं से तीसरा सबसे बड़ा वस्तु उठाएं (नवीन वाक्यांश) " के बीच अंतर है।

>  केवल मशीनों के प्रशिक्षण से विशिष्ट कार्य मॉडल उत्पन्न करने में विफलता, वितरण पर निर्देश पर विफलता  संयुक्त सूक्ष्म调度 है "रेड ब्लक को उठाने में) " और "बाएं से तीसरे सबसे बड़े वस्तु को उठाने में) " के बीच अंतर

### सुरक्षा और कार्रवाई की सीमाएं

प्रत्येक उत्पादन VLA जहाजों के साथः

> प्रत्येक उत्पादन श्रेणी VLA शहर के लिए तैयारः

- हार्ड ज्वाइंट सीमा (स्पेसिफिकेशन के बाद टॉर्क नहीं कर सकता) ।
  中文翻译:硬性关节限制(不能超过规格的力矩)
- गति सीमा (नरम काटने)
  中文翻译:速度限制 (速度限制)
- कार्यक्षेत्र की सीमाएँ (अंत-इफेक्टर तालिका से बाहर नहीं निकल सकता है) ।
  中文翻译:工作空间边界 (末端执行器不能离开桌面)
- नए कार्यों के लिए मानव-सक्रिय अनुमोदन।
  中文翻译:新任务的人工审批──

ये VLA के बाहर नियंत्रण परत की जांच के रूप में बैठते हैं। VLA का आउटपुट एक सुझाव है, एक आदेश नहीं है।

> इन VLA के बाहर स्थित नियंत्रण स्तर की जांच करें।

## इसे फ्रेमवर्क के साथ लागू करें
```figure
mm-action-tokens
```

## इसका प्रयोग करें

`code/main.py`:

- 256-बिन एक्शन टोकनाइजेशन और डिकॉनाइजेशन को लागू करता है।
  中文翻译:实现 256 बिन 动作分词化和反分词化──
- DCT + क्वांटिज़ेशन के आधार पर FAST टोकनाइज़र का स्केच।
  中文翻译:基于DCT + 量化勾勒 FAST 分词器──
- प्रति क्रिया चरण पार (विशिष्ट-बिन, फास्ट, निरंतर-प्रवाह) के लिए टोकन-कउंट की तुलना करता है।
  中文翻译:比较离散 बिन、FAST、连续流三种方式的每步符号 数──
- RT-2 → OpenVLA → π0 → GR00T का वंशावली सारांश छापता है।
  中文翻译:打印 RT-2 → OpenVLA → π0 → GR00T 的谱系摘要──

## इसे भेजें उत्पाद

यह सबक हमें फल देता है`outputs/skill-vla-action-format-picker.md`. एक रोबोट कार्य (मॅनिपुलेशन, नेविगेशन, ह्यूमनॉइड पूरे शरीर) को देखते हुए, डिस्क्रिट-बिन + RT-2, फास्ट + ओपनवीएलए, प्रवाह-मिलान + π0, या डबल-सिस्टम + GR00T के बीच चयन करें।

> 本课产 出 `outputs/skill-vla-action-format-picker.md`◊ given given machine tasks(操作、导航、人形全身), में अलग अलग बिन+RT-2、FAST+OpenVLA、流匹配+π0 या दो सिस्टम+GR00T 之间选择──

## अभ्यास विषय

1. 10 डीओएफ हाथ 30 हर्ट्ज नियंत्रण दर पर। 256 डिब्बे पर विवश-बिन टोकनकरण प्रति सेकंड कितने टोकन उत्सर्जित करता है? क्या एक 7 बी वीएलएम साथ रह सकता है? 10 स्वतंत्रता मशीन हथियार, 30 हर्ट्ज नियंत्रण आवृत्ति, 256 डिब्बेट बीन। प्रति सेकंड कितने टोकन उत्पन्न होते हैं? 7 बी वीएलएम 能跟上吗?

2. फास्ट टोकनकरण 30 चरणों के ट्रैक को ~ 10 टोकन तक संपीड़ित करता है। यदि ट्रैक में उच्च आवृत्ति आंदोलन (जैसे, ड्रमिंग) होता है तो उपयोगकर्ता क्या खोता है? FAST 30 步轨迹压缩为约 10 टोकन──如果轨迹包含高频运动(如击鼓),会丢失什么?

3. π0 का प्रवाह-अनुरूपता सिर ~5 चरणों में denoises। OpenVLA के ऑटोरेग्रेसिव डिकोड के साथ 4-5 हर्ट्ज पर आउटपुट की तुलना करें। π0 का प्रवाह匹配头在约5 步去噪点──对比OpenVLA 4-5 हर्ट्ज自归解码的吞吐量──

4. GR00T के सिस्टम 1 / सिस्टम 2 ने Kahneman के नक्शे को विभाजित किया। एक अलग विभाजन (सिस्टम 3?) का प्रस्ताव करें जो द्विपाद चलने में मदद कर सकता है। GR00T के सिस्टम1/ सिस्टम2 से अलग है।

5. ओपन एक्स-एम्बॉडीमेंट सेक्शन 4 को पढ़ें डेटासेट क्यूरेशन पर। डोमेन लीक को रोकने के लिए तीन क्यूरेशन नियम का नाम दें।

## कीवर्ड्स  शब्द खोज तालिका

| Term | What people say | What it actually means | 中文释义 |
|------|-----------------|------------------------|---------|
| VLA | "Vision-language-action" 视觉-语言-动作模型 | Model that takes image + instruction and outputs action commands 接受图像+指令并输出动作命令的模型 | |
| Action tokenization | "Discrete bins" 离散 bin 编码 | Quantize continuous joint targets into 256 bins per dim, each a vocab ID 将连续关节目标量化为每维 256 个 bin，每个 bin 对应一个词表 ID | |
| FAST tokenizer | "Frequency action tokens" 频域动作 token | DCT + quantize to compress 30-step trajectories to ~10 tokens 用 DCT + 量化将 30 步轨迹压缩为约 10 个 token | |
| Co-fine-tune | "Mix web + robot" 混合微调 | Train on web VQA data alongside robot demos to preserve general knowledge 在网络 VQA 数据和机器人演示上联合训练以保留通用知识 | |
| Flow-matching action head | "pi0 continuous output" 流匹配动作头 | Small transformer that outputs a 50-step action sequence via rectified flow 通过矫正流输出 50 步连续动作序列的小型 Transformer | |
| System 1 / System 2 | "Dual-system control" 双系统控制 | Large VLM plans slowly, small action head acts quickly; GR00T pattern 大 VLM 慢规划，小动作头快执行；GR00T 模式 | |
| Open X-Embodiment | "RT-X dataset" 开放具身数据集 | 1M-trajectory cross-robot dataset; the training corpus 100 万轨迹跨机器人数据集；标准训练语料 | |

## आगे पढ़ना 延伸閱讀

- [Brohan et al. — RT-2 (arXiv:2307.15818)](https://arxiv.org/abs/2307.15818)
- [Kim et al. — OpenVLA (arXiv:2406.09246)](https://arxiv.org/abs/2406.09246)
- [Black et al. — π0 (arXiv:2410.24164)](https://arxiv.org/abs/2410.24164)
- [NVIDIA — GR00T N1 (arXiv:2503.14734)](https://arxiv.org/abs/2503.14734)
- [Open X-Embodiment Collab — RT-X (arXiv:2310.08864)](https://arxiv.org/abs/2310.08864)
