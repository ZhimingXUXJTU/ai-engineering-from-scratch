# लोड टेस्टिंग LLM एपीआई  क्यों k6 और टिड्डियों झूठ बोल रहे हैं  लोड टेस्टिंग API क्यों LLM 美国
...
> **【中文解读】**इस भाग में LLM API लोड टेस्टिंग  मूल्यांकन 推理 सेवाएं उच्च लोड में प्रदर्शन करने के परीक्षण विधि का परिचय दिया गया है।
...
>  **【前置】**学本节前कृपया पहले समझेंःचरण 17·08(指标)、चरण 17·03(GPU 扩缩)。 पारंपरिक लोड परीक्षण यंत्र नहीं为流式响应+变长输出设计。
>  **【类比】**LLM 负载测试 = "测自动驾驶 vs 测传统车"──两个陷:(1) GIL 陷:Locust 在 Python GIL 下做代币化,与请求生成抢锁→报告的代币 间延迟虚高(客户端是瓶不是服务端);(2) त्वरित 一致性陷:循环同步快速只测分布一个点,真流量有多样化前匹配──LLMPerf 用 `--mean-input-tokens+stddev`修复──2026 工具:GenAI-Perf/LLMPerf/LLM-Locust(LLM 专用) + k6 v2026.1(流式+K8s) + Vegeta(Go 常速率) + Locust(仅配 LLM-Locust 扩展)
...
- दो विरोधी पैटर्न (जीआईएल जाल, शीघ्र-समानता जाल) को समझाएं जो एलएलएम एपीआई के लिए सामान्य लोड परीक्षकों को झूठ बोलते हैं।
  中文翻译:解释两个反模式(GIL 陷、提示均性陷), ये सामान्य भार परीक्षण उपकरण को गलत दिशा प्रदान करते हैं।
...
- चार लोड पैटर्न (स्थिर, रैंप, स्पाइक, भिगो) डिजाइन करें और प्रत्येक कैच के विफलता मोड का नाम दें।
  चीनी अनुवादः डिजाइन चार प्रकार के लोड मोड (अर्थात्, चार प्रकार के लोड मोड)
- निश्चित लंबाई के बजाय इनपुट टोकन के औसत + stddev का उपयोग करके एक यथार्थवादी शीघ्र वितरण बनाएं।
  中文翻译: उपयोग输入 टोकन के औसत मूल्य + 标准差构建真实提示分布,而非固定长度──

## समस्या  समस्या परिचय

> **【中文解读】**傳統負載測試工具不是LLM 設計的它们不支持流式响应、可变输出长度、token 级指标或 GPU 和度──两个常见陷:(1) GIL 陷Locust का टोकन 级测量在Python GIL 下运行分词,高并发时代 टोकन化 队列膨胀,虚报 延迟 延迟(你的客户端是瓶,不是服务器);2) 均性陷循环测试中使用相同提示,前缓存中命率接近100%,吞吐量看起来很好但完全不反映真流量.

> **【拓展：LLM 负载测试的四种模式】**2026 साल LLM  लोड परीक्षण के चार प्रकारः 1) स्थिर स्थिति (stable-state) 恒定 RPS 持续 30-60 मिनट, पकड़ की लाइन प्रदर्शन गिरावट; 2) 渐增)  रैंप)  0 线性 से बढ़कर लक्ष्य RPS, पकड़ क्षमता खंड और पूर्व ताप असामान्य; 3) 突发 (spike)  अचानक 3-10x RPS 持续 2 मिनट फिर वापस落, परीक्षण स्वचालित विस्तार प्रतिक्रिया 队列和和和冷启动影响; 4) 时间 (soak)  स्थिर स्थिति (stable state) 持续 4-8 घंटे, पकड़内存漏漏、连接池漂移和可观测性溢出.
...
> **【拓展：Python GIL 对 LLM 负载测试的影响】**Python GIL(全局解释器锁) LLM 负载测试的影响:Locust Python 运行客户端分词,在高并发时代代代码化 队列排在请求生成后面――报告的间代码 延迟包含客户端代码化 积压你以为是服务器慢,其实是测试工具的瓶──解决方案:(1) LLM-Locust 扩展将代码化 移动到独立进程;(2) 使用编译语言工具kkk) 
...
> **【中文解读】**2026 साल LLM 负载测试工具选择:(1) LLMPerf(Anyscale)Rust-backed 分词 + 流式感知,性能测试的默认选择;(2) NVIDIA GenAI-PerfNVIDIA 参考工具,注意其 ITL 不含 TTFT;(3) LLM-Locust(TrueFoundry)Locust 扩展,修复 GIL 问题;(4) k6 v2026.1.0 + k6 ऑपरेटर 1.0 GA(2025 साल 9 月) Go编译、无 GIL、流式、知感感网-原生 分布测试,CI/CD गेट 最佳选择──

> **【拓展：CI/CD 中的 SLA Gate】**CI में k6 का उपयोग SLA गेट 配置: प्रति बार PR 运行 30-50 次代,gate 指标包括 P50/P95 TTFT、5xx < 5%、TPOT 在值以下──违规构建失败──使用真实提示分布(mean + stddev of input tokens) बजाय तय लंबाईLLMPerf उपयोग `--mean-input-tokens 500 --stddev-input-tokens 150`                                                                                                                                                                                                                                                              
...
> 本课产 出 `outputs/skill-load-test-plan.md`. कार्यभार और एसएलए को देखते हुए, उपकरण चुनता है और चार भार पैटर्न डिजाइन करता है।

## अभ्यास विषय

1. दौड़ें`code/main.py`. समरूप बनाम यथार्थवादी वितरण की तुलना करें  अंतर कहाँ है?
   中文翻译:运行 `code/main.py` तुलना औसत बनाम वास्तविक वितरणP99 TTFT 差异在哪里?
...
3. आपके विसर्जन परीक्षण में मेमोरी 50 एमबी/घंटे बढ़ रही है. तीन कारणों का नाम दें और उनके बीच चयन करने के लिए उपकरण।
   Chinese: Your Bubble Test shows内存 प्रति घंटे 50MB बढ़ता है।
4. 10 आरपीएस से 100 आरपीएस तक स्पाइक परीक्षण। यदि कारपेन्टर + वीएलएलएम उत्पादन-स्टैक (चरण 17 · 03 + 18) लागू है तो अपेक्षित पुनर्प्राप्ति समय क्या है?
   中文翻译: 10 आरपीएस 尖峰测试 से 100 आरपीएस ⋅ अगर कार्पेन्टर  45 सेकंड की आपूर्ति की आवश्यकता होती है, तो प्रत्याशित पुनर्प्राप्ति समय कितना है?
