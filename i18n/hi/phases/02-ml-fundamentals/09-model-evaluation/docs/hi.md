...
> 模型 का अच्छा बुराई आप इसे कैसे मापते हैं पर निर्भर करता है।
...
- K-fold और stratified K-fold क्रॉस-वैलिडेशन को खरोंच से लागू करें और समझाएं कि असंतुलित डेटा के लिए stratification क्यों मायने रखता है
  शून्य से प्राप्त K 折和分层 K 折交叉验证, समझाएं कि क्यों विभाजन स्तर के लिए असंतुलन डेटा महत्वपूर्ण है
- सटीकता, रिकॉल, F1, AUC-ROC और रेग्रिशन मीट्रिक (MSE, RMSE, MAE, R-क्वायर) को खरोंच से गणना करें
  शून्य गणना सटीकता दर、召回率、F1、AUC-ROC 和归归指标(MSE、RMSE、MAE、R-क्वाड)
...
- डेटा लीक, गलत मीट्रिक चयन और परीक्षण सेट की दूषितता सहित सामान्य मूल्यांकन त्रुटियों की पहचान करना
  识别常见评估错误, डेटा लीक सहित


> **【中文解读】**
> 模型评估 उत्तर मॉडल到底好不好──准确率、精确率、召回率、F1、AUC-ROC 分类指标;MSE、MAE、R^2 归归指标──交叉验证防止过拟评估──sklearn 中的 cross_val_score/classification_report──

> **【拓展：模型评估失误导致的生产事故】**
> अमेज़ॅन की भर्ती एआई उपकरण (एआई उपकरण) क्योंकि मूल्यांकन अपर्याप्त है (अर्थात् प्रशिक्षण डेटा पर मूल्यांकन नहीं) महिलाओं के उम्मीदवारों के लिए प्रणालीगत भेदभाव का कारण बनता है, अंततः मजबूर हो जाता है।

## समस्या  समस्या परिचय

आपने एक मॉडल को प्रशिक्षित किया है, यह आपके डेटा पर 95% सटीकता प्राप्त करता है।

> आप एक मॉडल को प्रशिक्षित किया है. यह अपने डेटा पर प्राप्त 95% सटीकता दर. यह अच्छा है?
...
> शायद अच्छा, शायद बुरा। यदि 95% डेटा एक श्रेणी में आता है, तो हमेशा भविष्यवाणी करें कि उस श्रेणी के मॉडल को 95% सटीकता दर मिलती है लेकिन पूरी तरह से बेकार है। यदि आप अभ्यास किए गए डेटा पर मूल्यांकन करते हैं, तो 95% यह संख्या कोई मायने नहीं रखती है, क्योंकि मॉडल केवल उत्तर को याद करता है।
...
> 模型评价是大多数 ML 项目出错的地方──错误的标志让坏模型看起来好──错误的分分别让模型作弊──错误的比较让你选择更差的模型──正确评价不是可选项它是生产中有效的模型与遇到真实数据的模型之间的区别──

> **【中文解读】**
> 模型 मूल्यांकन सबसे आसान तीन गलतियाँ करने के लिए हैः 1) प्रशिक्षण डेटा पर मूल्यांकन करना; 2) गलत संकेतकों का उपयोग करना; जैसे कि असंतुलित डेटा सटीकता दर; 3) डेटा लीक करना; 3) प्रशिक्षण प्रक्रिया में डेटा लीक करना; सही मूल्यांकन के लिए स्वतंत्र डेटा विभाजन की आवश्यकता होती है; व्यापार लक्ष्य के संकेतकों के अनुरूप;
...
> तीन प्रकार के उपयोगः

- **Training set**इस प्रकार, मॉडल इन आंकड़ों से सीखता है। यह प्रशिक्षण के दौरान इन उदाहरणों को देखता है।
  **训练集**: मॉडल से मध्य सीख── प्रशिक्षण में इन नमूनों को देखें──
- **Validation set**इस मॉडल को कभी भी इस डेटा पर प्रशिक्षित नहीं किया जाता है, लेकिन आपके निर्णय इसके द्वारा प्रभावित होते हैं।
  **验证集**: सुपर पैरामीटर और मॉडल में चयन को विनियमित करने के लिए उपयोग किया जाता है। मॉडल इस डेटा पर प्रशिक्षण नहीं करता है, लेकिन आपके निर्णय प्रभावित होते हैं।
- **Test set**यदि आप परीक्षण प्रदर्शन को देखते हैं और फिर अपने मॉडल को बदलने के लिए वापस जाते हैं, तो यह अब एक परीक्षण सेट नहीं है। यह एक दूसरा सत्यापन सेट बन गया है।
  **测试集**: जब आप अंतिम बार ही स्पर्श करते हैं, तो अंतिम प्रदर्शन रिपोर्ट करते हैं। यदि आप परीक्षण प्रदर्शन को देखते हैं तो फिर से मॉडल को संशोधित करने के लिए जाते हैं, तो यह अब परीक्षण संग्रह नहीं है।
...
> 测试集是您的保留保证, सुनिश्चित करें कि रिपोर्ट का प्रदर्शन वास्तविक अप्रत्याशित डेटा पर मॉडल के प्रदर्शन को दर्शाता है।
...
>  लघु डेटा संग्रह के लिए, एकल प्रशिक्षण/परीक्षण विभाजन व्यय डेटा और ध्वनि अनुमान प्रदान करते हैं।
...
1. समान आकार के K फोल्ड में डेटा विभाजित करें
   डेटा को K 个大相等 के खंड में विभाजित करना
2. प्रत्येक तह के लिए, K-1 तह पर ट्रेन करें और शेष तह पर सत्यापित करें
   प्रत्येक मोड़ के लिए, K-1 मोड़ पर प्रशिक्षण, शेष एक मोड़ पर परीक्षण
...
> K=5 या K=10 मानक चयन हैं। प्रत्येक डेटा बिंदु का प्रयोग एक बार किया जाता है।
...
> **分层 K 折**यदि आपका डेटासेट 70% ए श्रेणी और 30% बी श्रेणी है, तो प्रत्येक खंड में लगभग समान अनुपात होगा।

> **分层 K 折**: प्रत्येक खंड में श्रेणी वितरण बनाए रखें। यदि डेटा संग्रह 70% श्रेणी ए और 30% श्रेणी बी है, तो प्रत्येक खंड में लगभग समान अनुपात होगा। यह असंतुलित डेटा संग्रह के लिए महत्वपूर्ण है, क्योंकि किसी भी समय विभाजन संभव है कि सभी अल्पसंख्यक प्रकार के नमूने एक ही खंड में डाल दिए जाएंगे।

### वर्गीकरण मेट्रिक्स

**Confusion matrix**द्विआधारी वर्गीकरण के लिएः

> **混淆矩阵**आधार ः ः ःदो वर्ग के लिए:
...
> इस रक्शन्स से सभी अन्य संकेतकों को बाहर निकालेंः

> इस रक्शन्स से, सभी अन्य संकेत इस प्रकार से हैंः
...
- **Precision**= टीपी / (टीपी + एफपी) । सभी भविष्यवाणी की गई चीजों में से कितने वास्तव में सकारात्मक थे? जब झूठे सकारात्मक महंगे होते हैं (जैसे, स्पैम फ़िल्टर वास्तविक ईमेल को स्पैम के रूप में चिह्नित करता है) का उपयोग करें।
  **精确率**= TP / (TP + FP) ◊ सभी पूर्वानुमानों में से, क्या वास्तविक में कितना सही है?
- **Recall**(संवेदनशीलता) = टीपी / (टीपी + एफएन) सभी वास्तविक सकारात्मकताओं में से, हमने कितने पकड़े? जब झूठे नकारात्मक लागत वाले होते हैं (जैसे, कैंसर स्क्रीनिंग में ट्यूमर गायब होता है) उपयोग करें।
  **召回率**(संवेदनशीलता) = टीपी / (टीपी + एफएन)  सभी वास्तविक वैक्सीन के नमूने में, हमने कितना कैप्चर किया है?
- **F1 score**= 2 * सटीकता * याद / (सटीकता + याद) सटीकता और याद का सामंजस्यपूर्ण औसत। दोनों को संतुलित करता है जब स्पष्ट रूप से कोई भी प्रभुत्व नहीं रखता है।
  **F1 分数**= 2 * 精确率 * 召回率 / (精确率 + 召回率) ◊精确率和召回率调和平均── इन दोनों में से कोई स्पष्ट रूप से लाभदायक संतुलन नहीं है──
- **AUC-ROC**: रिसीवर ऑपरेटिंग कैरेक्टेरिस्टिक वक्र के तहत क्षेत्र। विभिन्न वर्गीकरण सीमाओं पर सच्ची सकारात्मक दर बनाम झूठी सकारात्मक दर का पता लगाता है। AUC = 0.5 का अर्थ है यादृच्छिक अनुमान, AUC = 1.0 का अर्थ है सही अलगाव। सीमा-स्वतंत्रः यह मापता है कि मॉडल नकारात्मक से ऊपर सकारात्मक को कितनी अच्छी तरह से रैंक करता है, चाहे आप किस कटऑफ का चयन करें।
  **AUC-ROC**:ROC 曲线下面积── भिन्न वर्गों में वास्तविक दर और झूठी सही दरों के नीचे चित्रण करें──AUC = 0.5 का प्रयोग करते हुए अनुमान लगाने,AUC = 1.0 का प्रयोग करते हुए सही अंतरण का प्रयोग करें──

### प्रतिगमन माप

- **MSE**(मीडियन स्क्वायर त्रुटि) = mean((y_true - y_pred) ^ 2). बड़े त्रुटियों को quadratically दंडित करता है। असाधारण के प्रति संवेदनशील।
  **均方误差 (MSE)**= mean(((y_true - y_pred) ^2)。二次惩罚大误差──对异常值敏感──
- **RMSE**(रूट मीड स्क्वायर त्रुटि) = sqrt(MSE) लक्ष्य चर के समान इकाइयां। एमएसई की तुलना में व्याख्या करना आसान है।
  **均方根误差 (RMSE)**= वर्ग (MSE) ◊ लक्ष्य परिवर्तन इकाई के साथ समान ◊ से MSE 更易解释
- **MAE**(मध्यम पूर्ण त्रुटि) = औसत y_true - y_pred ) सभी त्रुटियों को रैखिक रूप से व्यवहार करता है। एमएसई की तुलना में अधिक मजबूत से असाधारण।
  **平均绝对误差 (MAE)**= औसत (अर्थात् वास्तविक)  सभी त्रुटियों का संसाधित करना
- **R-squared**= 1 - SS_res / SS_tot, जहां SS_res = योगफल((y_true - y_pred) ^2) और SS_tot = योगफल(((y_true - y_mean) ^2) । मॉडल द्वारा समझाए गए भिन्नता का अंश। R^2 = 1.0 एकदम सही है। R^2 = 0.0 का अर्थ है कि मॉडल हमेशा औसत की भविष्यवाणी करने से बेहतर नहीं है। R^2 नकारात्मक हो सकता है यदि मॉडल औसत से बदतर है।
  **决定系数 (R-squared)**= 1 - SS_res / SS_tot, जिसमें SS_res = योगफल(((y_true - y_pred) ^2),SS_tot = योगफल((((y_true - y_mean) ^2);; मॉडल व्याख्या का अनुपात अनुपात。R^2 = 1.0 完美──R^2 = 0.0 表示模型不比始终预测均值好──R^2 可以为负,如果模型比预测均值还差──

### सीखने की वक्रता

प्रशिक्षण सेट के आकार के आधार पर प्रशिक्षण और सत्यापन स्कोरः

> चित्रण प्रशिक्षण अंक और सत्यापन अंक प्रशिक्षण के साथ बड़े बदलाव की वक्रताः

> प्रशिक्षण के बड़े कार्य के रूप में प्रशिक्षण अंक और परीक्षण अंक का चित्रण करेंः

- **High bias (underfitting)**एक और डेटा जोड़ने से मदद नहीं मिलेगी। आपको एक अधिक जटिल मॉडल की आवश्यकता है।
  **高偏差（欠拟合）**: दो खंभे प्राप्त करने के लिए कम分── अधिक डेटा बढ़ाने में मदद नहीं मिलेगी── आपको अधिक जटिल मॉडल की आवश्यकता है──
- **High variance (overfitting)**प्रशिक्षण स्कोर उच्च है लेकिन सत्यापन स्कोर बहुत कम है। इन दोनों के बीच अंतर बड़ा है। अधिक डेटा जोड़ने से मदद मिलेगी।
  **高方差（过拟合）**प्रशिक्षण अंक उच्च हैं, लेकिन परीक्षण अंक बहुत कम हैं।

### सत्यापन वक्र

हाइपरपरमैटर के कार्य के रूप में प्लॉट प्रशिक्षण और सत्यापन स्कोरः

> चित्रण प्रशिक्षण अंक और सत्यापन अंक के साथ सुपर पैरामीटर परिवर्तन की वक्रः

> प्रशिक्षण अंक और परीक्षण अंक के रूप में सुपर पैरामीटर के कार्य चित्रणः
...
- उच्च जटिलता परः प्रशिक्षण स्कोर उच्च रहता है लेकिन सत्यापन स्कोर गिरता है (ओवरफिटिंग)
  复杂度高时: प्रशिक्षण分数保持高但验证分数下降(过拟合)

इष्टतम हाइपरपरमैटर मान उस स्थान पर होता है जहां सत्यापन स्कोर चरम होता है।

> अधिकतम अति-परमानुष मूल्य है सत्यापन अंक की शिखर मूल्य पर पहुंचने की स्थिति।
...
> **数据泄漏**:测试集信息泄漏到训练中── उदाहरण: 始终先划分,再预处理──
...
> **类别不平衡**:99% के लेनदेन कानूनी हैं, 1% धोखाधड़ी हैं।
...
> **不使用分层划分**: असंतुलित आंकड़ों के लिए, जब भी विभाजन संभव हो, तो बहुत कम नमूने सत्यापन में डाल दिए जाएंगे, अनिश्चित अनुमान दिए जाएंगे।

**Testing too often**: जब भी आप परीक्षण प्रदर्शन को देखते हैं और समायोजित करते हैं, तो आप परीक्षण सेट के लिए ओवरफिट करते हैं।

> **测试过于频繁**: प्रत्येक बार जाँच परीक्षण प्रदर्शन में सुधार, आप बस परीक्षण सेट के लिए अनुकूलित किया गया है।

## इसे बनाओ, इसे पूरा करो।

> **【中文解读】**
> शून्य से प्राप्त交叉验证 (K-fold 和分层 K-fold) 分类指标 (精确率、召回率、F1、AUC-ROC) 及归归指标 (MSE、RMSE、MAE、R2) 交叉验证 (K-fold 和分层 K-fold) 分类指标 (分类指标) 精确率、召回率、F1、AUC-ROC) 及归归指标 (MSE、RMSE、MAE、R2) 交叉验证 (K-fold 和分层 K-fold) 分层 K-fold 确保每折中类比例一致,对不平衡数据至关重要──

> **【拓展：学习曲线——诊断模型问题的利器】**
> सीखने की वक्र) प्रशिक्षण त्रुटि और सत्यापन त्रुटि को रेखांकित करना प्रशिक्षण डेटा मात्रा के परिवर्तन के साथ, उच्च विकृति/उच्च विकृति के मुद्दों का निदान करने का एक सीधा उपकरण है। प्रशिक्षण त्रुटि और सत्यापन त्रुटि भी बहुत अधिक है।
```figure
...
> तीसरा चरण:混矩阵和分类指标── शून्य से TP/TN/FP/FN 计数, पुनः推导出准确率、精确率、召回率、F1──ROC 曲线扫描所有可能值,记录每点的 (FPR, TPR),AUC 是曲线下面积(
...
> चौथा चरण: लौटने का संकेत। एमएसई (%) औसत त्रुटि (%) बड़ी त्रुटि (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे दण्ड (%) दोहरे) दोहरे दण्ड (%) दोहरे) के लिए एक ही (%) दोहरे)
...
> 第五步:学习曲线──逐步增加训练数据量,记录训练分数和验证分数──两条曲线都低→高偏差(欠拟合);训练高但验证低→高方差(过拟合)── यह नैदानिक मॉडल की समस्या का सबसे सीधा सा साधन──
...
> 第六步: एक सरल तार्किक归归归分类器, प्रयोग में टेस्ट मूल्यांकन कोड──手动实现前向传播(线性组合 + sigmoid) 梯度下降更新── उसके बाद पूर्व-परिभाषित交叉验证、指标、学习曲线完整评估这个模型──
...
> प्रयोग स्किट-लर्न, मूल्यांकन内置于工作流中:
...
> शून्य संस्करण से सटीक रूप से दिखाया गया है कि किस प्रकार के पारगमन का परीक्षण किया गया है (केवल चक्र और सूचकांक का पता लगाने के लिए कोई जादू नहीं है) ✓ प्रत्येक सूचक कैसे गणना किया जाता है (केवल गणना TP/FP/TN/FN) ✓ तथा किनकी विभाजन स्तर महत्वपूर्ण है (केवल प्रति खंड में वर्ग अनुपात बनाए रखना) ✓ पुस्तकालय संस्करण में वृद्धि हुई है ✓ अधिक मूल्यांकन विकल्प और पाइपलाइन इंजेक्शन ✓
...
> 本课产出:
> - `outputs/skill-evaluation.md`- वर्गों और वापसी मॉडल मूल्यांकन रणनीति को कवर करने के कौशल

> **【拓展：A/B 测试——模型评估的终极标准】**
> उद्योग जगत में, ऑफ़लाइन मूल्यांकन सूचक (अक्षमता दर, एयूसी आदि) केवल संदर्भ है, वास्तविक मूल्यांकन ऑनलाइन ए/बी परीक्षणों में है। Google प्रति वर्ष 10,000 से अधिक बार ए/बी परीक्षणों को चलाता है ताकि खोज एल्गोरिदम में सुधार का मूल्यांकन किया जा सके; नेटफ्लिक्स का उपयोग करके ए/बी परीक्षणों का उपयोग करके यह तय किया जा सकता है कि क्या एल्गोरिदम ऑनलाइन है; यूबर का उपयोग करके ए/बी परीक्षणों का मूल्यांकन करने के लिए गतिशील मूल्य निर्धारण रणनीति। ए/बी परीक्षणों का मूल है रैंडम प्रवाहन और सांख्यिकीय स्पष्टता परीक्षण।

> **【中文解读】**
> आरओसी 曲线绘画不同值下 TPR(真率) बनाम FPR(假正率),AUC 曲线下面积(0.5=随机,1.0=完美)。AUPRC(精确率-召回率曲线下面积) में असंतुलित डेटा पर AUC-ROC के मुकाबले अधिक जानकारी मात्रा में。MCC(马修斯相关系数) असंतुलित डेटा पर समग्र सूचक है, मिक्सवॉल्यू矩阵 के सभी चार को ध्यान में रखते हुए。
...
1. सटीकता-पुनर्प्राप्त वक्रों को लागू करेंः विभिन्न सीमाओं पर ग्राफ सटीकता बनाम पुनर्प्राप्त। औसत सटीकता (पीआर वक्र के नीचे क्षेत्र) की गणना करें। असंतुलित डेटासेट पर पीआर वक्र की तुलना करें और समझाएं कि प्रत्येक कब अधिक जानकारीपूर्ण है।
   1. 实现精确率-召回率曲线: भिन्न值下绘制精确率 vs.召回率――计算平均精确率(PR 曲线下面积)――在不平衡数据集上将PR 曲线与ROC 曲线比较,解释各自何时有更多信息──
2. एक घोंसले गए क्रॉस-वैलिडेशन लूप बनाएंः बाहरी लूप मॉडल प्रदर्शन का मूल्यांकन करता है, आंतरिक लूप हाइपरपरपैरामीटर को समायोजित करता है। इसका उपयोग मूल्यांकन में वैलिडेशन डेटा लीक किए बिना दो मॉडल की तुलना करने के लिए करें।
   2. 构建嵌套交叉验证循环: बाहरी循环评估模型性能,内循环调优超参数―― इसे दो मॉडल की तुलना में निष्पक्ष रूप से करें, यह मूल्यांकन के दौरान डेटा लीक का परीक्षण नहीं करेगा――
3. मॉडल तुलना के लिए एक परमुटੇਸ਼ਨ टेस्ट लागू करेंः लेबल को मिलाएं, रीट्रेन करें और प्रदर्शन मापें। शून्य वितरण बनाने के लिए 100 बार दोहराएं। इस वितरण के खिलाफ देखे गए मॉडल प्रदर्शन के लिए p-मूल्य की गणना करें।
   3. 实现模型比较的置换检验:打乱标签,重新训练,测量性能──重复 100 बार建立零分布──计算观测模型性能对这个分布的p 值──
...
- [scikit-learn Model Selection Guide](https://scikit-learn.org/stable/model_selection.html)- क्रॉस-वैलिडेशन, मेट्रिक्स और हाइपरपरमैटर ट्यूनिंग पर व्यापक संदर्भ
  [scikit-learn 模型选择指南](https://scikit-learn.org/stable/model_selection.html)- 交叉验证、指标和超参数调优 के व्यापक संदर्भ
...
- [A Survey of Cross-Validation Procedures (Arlot & Celisse, 2010)](https://projecteuclid.org/journals/statistics-surveys/volume-4/issue-none/A-survey-of-cross-validation-procedures-for-model-selection/10.1214/09-SS054.full)- विभिन्न सीवी रणनीतियों का कब और क्यों काम होता है, इसका सख्त इलाज
  [A Survey of Cross-Validation Procedures (Arlot & Celisse, 2010)](https://projecteuclid.org/journals/statistics-surveys/volume-4/issue-none/A-survey-of-cross-validation-procedures-for-model-selection/10.1214/09-SS054.full)- विभिन्न पारस्परिक सत्यापन रणनीति किस समय प्रभावी कठोर विश्लेषण
