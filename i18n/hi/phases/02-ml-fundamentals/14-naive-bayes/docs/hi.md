...
> "सरल" का परिकल्पना गलत है, लेकिन यह अभी भी उपयोग में है।
...
- पाठ वर्गीकरण के लिए लैपलेस चिकनाई के साथ मल्टीनोमीअल नाईव बेय को खरोंच से लागू करें
  शून्य से कार्यान्वयन लेपलेस के साथ सरल बहुवचन सरल उपयोग में आता है
- समझाएं कि क्यों साफ़-साफ़ स्वतंत्रता धारणा गणितीय रूप से गलत है लेकिन व्यवहार में सही वर्ग रैंकिंग का उत्पादन करती है
   समझाएँ कि साधारण स्वतंत्रता का परिकल्पना गणित में गलत क्यों है, लेकिन अभ्यास में सही श्रेणी श्रेणी का उत्पादन क्यों होता है
- मल्टीनोमीअल, बर्नौली और गौशियन नाइव बेय के संस्करणों की तुलना करें और किसी दिए गए विशेषता प्रकार के लिए सही एक का चयन करें
  बहुवचन, बर्नूली और उच्च सादगी बेलेज़ भिन्न, विशिष्ट प्रकार के लिए सही प्रकार के चयन के लिए भिन्न
- उच्च-आयामी दुर्लभ डेटा पर लॉजिस्टिक प्रतिगमन के खिलाफ नाइव बेय का मूल्यांकन करें और काम पर पूर्वाग्रह-वियरिएंस ट्रेडऑफ की व्याख्या करें
  उच्च स्तर की दुर्लभता के आंकड़ों पर आकलन सरलता और तर्कसंगत वापसी, व्याख्या अंतर-अंतर-अंतर भार


> **【中文解读】**
> 朴素贝叶斯基于贝叶斯定理 +特征独立性假设──虽然假设很强,但在文本分类垃圾邮件过) 中出奇地好──sklearn 中的多边NB/GaussianNB──

> **【拓展：朴素贝叶斯在垃圾邮件过滤和情感分析中的应用】**
> पहले के स्पाममेल फिल्टर (जैसे स्पैम अससिन) ने बहुत से सादे बेयस का उपयोग किया, क्योंकि यह विशेषता आयाम में सैकड़ों हज़ार तक पहुंच सकता था) लेकिन प्रशिक्षण नमूना कम परिदृश्यों में प्रदर्शन किया गया।
...
> आपको पाठ को वर्गीकृत करने की आवश्यकता है। मेल को कचरा या गैर-कचरा मेल में विभाजित किया गया है। ग्राहक टिप्पणी को सकारात्मक या नकारात्मक में विभाजित किया गया है।
...
> अधिकांश वर्गीकरण यंत्र यहाँ बसे हुए हैं। तर्कात्मक पुनरावृत्ति के लिए पर्याप्त रूप से पर्याप्त है। यह एक ही शब्द पर एक ही समय में विभाजित और गंभीर रूप से overfitting निर्णय के पेड़ है।
...
> 朴贝叶斯能处理这种情况――它做了一个数学错误假设 (((给定类别下每个特征相互独立),但在文本分类上仍然优于"更聪明"的模型,特别是在小训练集上――它只需要一次遍历数据就能训练――它扩展到数百万特征――它产生概率估计 (((尽管独立假设通常校准不佳) )
...
> यह समझना कि गलत परिकल्पनाएँ अच्छी भविष्यवाणी का उत्पादन क्यों कर सकती हैं, आपके मशीन सीखने में कुछ बुनियादी बातें बताती हैंः सबसे अच्छा मॉडल सबसे सही मॉडल नहीं है, बल्कि आपके डेटा के लिए सबसे अच्छा मॉडल है।

> **【中文解读】**
> 朴素贝叶斯 का "朴素" यह मान लेना है कि सभी विशेषताएं किसी दिए गए वर्ग में एक दूसरे से स्वतंत्र हैं, जो वास्तविकता में लगभग मान्य नहीं है, जैसे कि "免费" और "优惠" कचरे में अत्यधिक संबंधित हैं, लेकिन यह क्यों उपयोग किया जा सकता है? क्योंकि वर्गों को केवल विभिन्न वर्गों की संभावनाओं को सही रैंक करने की आवश्यकता होती है, सटीक संभावना मूल्य की आवश्यकता नहीं होती है।
...
- `P(features | class)`-- इस वर्ग के दस्तावेजों में इन शब्दों को देखने की संभावना
  `P(features | class)`-- इस श्रेणी में दस्तावेजों में इन शब्दों की तरह दिखता है
...
- `P(features)`-- सबूत, सभी वर्गों के लिए एक ही, तो हम तुलना करते समय इसे अनदेखा कर सकते हैं
  `P(features)`-- प्रमाण, सभी श्रेणियों के लिए समान, तुलना में अनदेखा किया जा सकता है

उच्चतम वर्ग के साथ `P(class | features)`जीतता है।

> `P(class | features)`सर्वोच्च श्रेणी जीत
...
> आपको एक असंभव संयुक्त वितरण का अनुमान लगाने की आवश्यकता नहीं है, बल्कि एक साधारण विशेषताओं के लिए एक साधारण वितरण का अनुमान लगाने की आवश्यकता है।
...
> यह परिकल्पना स्पष्ट रूप से गलत है। "मशीन" और "लर्निंग" किसी भी दस्तावेज़ में स्वतंत्र नहीं हैं। लेकिन वर्गीकरण को सटीक संभावना अनुमान की आवश्यकता नहीं है। यह सही रैंकिंग की आवश्यकता है। किस श्रेणी की संभावना सबसे अधिक है।
...
1. **Ranking over calibration.**वर्गीकरण केवल शीर्ष रैंक वर्ग सही होने की आवश्यकता है। भले ही P(स्पैम) = 0.99999 जब वास्तविक संभावना 0.7 है, तो भी वर्गीकरणकर्ता अभी भी स्पैम सही चुनता है। हमें सही संभावनाओं की आवश्यकता नहीं है। हमें सही विजेता की आवश्यकता है।
   **排名优于校准。**वर्ग केवल प्रथम श्रेणी की आवश्यकता है सही श्रेणी। यहां तक कि P(स्पैम) = 0.99999 जबकि वास्तविक संभावना 0.7 है, वर्ग अभी भी सही प्रकार की कचरा चुनें।
...
2. **High bias, low variance.**स्वतंत्रता परिकल्पना एक मजबूत पूर्व है। यह मॉडल को बहुत सीमित करता है, जो ओवरफिटिंग को रोकता है। सीमित प्रशिक्षण डेटा के साथ, एक मॉडल जो थोड़ा गलत है लेकिन स्थिर है, सैद्धांतिक रूप से सही है लेकिन बहुत अस्थिर है। यह कार्रवाई में पूर्वाग्रह-भिन्नता व्यापार है।
   **高偏差、低方差。**独立性假设 एक मजबूत पूर्व प्रयोग है  यह गंभीर रूप से बाध्यकारी मॉडल है, जो अत्यधिक अनुकूलन को रोकता है  सीमित प्रशिक्षण डेटा के तहत, एक थोड़ा गलत लेकिन स्थिर मॉडल सिद्धांत रूप में सही लेकिन बहुत अस्थिर मॉडल से अधिक है  यह अंतर-अंतर-वज़न के वास्तविक संचालन है 
...
3. **Feature redundancy cancels out.**संबद्ध विशेषताएं अधिमानतः सबूत प्रदान करती हैं। वर्गीकरणकर्ता इस सबूत को दोगुना गिनता है, लेकिन यह इसे सही वर्ग के लिए भी दोगुना गिनता है। यदि "मशीन" और "लर्निंग" हमेशा एक साथ दिखाई देते हैं, तो दोनों "तकनीकी" वर्ग के लिए सबूत प्रदान करते हैं। एनबी उन्हें दो बार गिनता है, लेकिन यह उन्हें सही वर्ग के लिए दो बार गिनता है।
   **特征冗余相互抵消。**相关特征提供冗余证据――分类重复计数这些证据,但正确类别的证据也被重复计数―― यदि "机器"和"学习" 总是出现, दोनों "技术"类提供证据――简单贝叶斯数两次,但正确类别数两次――
...
> चौथा वास्तविक कारणः सरल बेयिस बहुत तेजी से। प्रशिक्षण एक बार में डेटा गणना की आवृत्ति है। भविष्यवाणी एक रैंक गुणा है। आप कुछ ही सेकंड में एक मिलियन दस्तावेजों को प्रशिक्षित कर सकते हैं। इस गति का मतलब है कि आप धीमी गति से मॉडल का उपयोग करने की तुलना में अधिक तेजी से कर सकते हैं।
...
>  सरल बेलीज में तीन प्रकार के भिन्न होते हैं  प्रत्येक प्रकार के लिए `P(feature | class)`建模方式不同──
...
> प्रत्येक विशेषता को गणना के रूप में रखना। सबसे उपयुक्त विशेषता शब्द频 या TF-IDF 值 के पाठ डेटा है।
...
> `alpha`यह परिवर्तन है लेख में मुख्य शक्ति

#### गौसीयन साफ़ बेय

मॉडल्स प्रत्येक विशेषता के रूप में एक सामान्य वितरण. निरंतर विशेषता के लिए सबसे अच्छा।

> प्रत्येक विशेषता को व्यवस्थित रूप से वितरित करें।
...
> प्रत्येक श्रेणी में प्रत्येक विशेषता के लिए अपना औसत मूल्य और अंतर होता है।

#### बर्नौली साफ़ बेयज़

प्रत्येक विशेषता को द्विआधारी (वर्तमान या अनुपस्थित) के रूप में मॉडल। लघु पाठ या द्विआधारी विशेषता वेक्टर के लिए सबसे अच्छा।

> प्रत्येक विशेषता को दो मानों में रखना (उत्पत्ति या अनुपस्थिति) ⋅ सबसे उपयुक्त लघु文本 या दो मानों के लिए विशेषता के आकार ⋅
...
> कई प्रकार के विपरीत, बर्नुली स्पष्ट दंड शब्द का कोई उल्लेख नहीं करता है। यदि "मुक्त" आमतौर पर कचरे के मेल में दिखाई देता है, लेकिन यह मेल नहीं है, तो बर्नुली इसे कचरे के खिलाफ सबूत के रूप में मानता है।
...
> यदि एक शब्द परीक्षण डेटा में दिखाई देता है, लेकिन किसी वर्ग के प्रशिक्षण डेटा में कभी दिखाई नहीं देता है, तो क्या होगा?
...
> न बढ़ानाः`P(word | class) = 0/N = 0` एक शून्य गुणा तक पूरे गुणाङ्क में होगा `P(class | features) = 0`, अन्य सबूतों के बावजूद, एक अदृश्य शब्द पूरे पूर्वानुमान को नष्ट कर देता है, अन्य सबूतों के बावजूद, समर्थन करता है।

लैप्लेस चिकनाई एक छोटी संख्या जोड़ती है `alpha`(आमतौर पर 1) प्रत्येक विशेषता संख्या के लिएः

> प्रत्येक विशेषता को एक छोटा सा अंक जोड़कर एक साइडलाइन दें`alpha`(आमतौर पर 1 के लिए):
...
> alpha=1 时, प्रत्येक शब्द कम से कम एक बहुत छोटा सा संभावना प्राप्त करते हैं।
...
> अधिक उच्च अल्फा का अर्थ है अधिक मजबूत समतल (अधिक निकटतम औसत) वितरण (अधिक निकटतम) ।
...
> सौ संभावनाओं को जोड़ना (प्रत्येक को 1) से छोटा करके फ्लेव बिंदु पर उतार-चढ़ाव पैदा करता है।
...
> यह पूर्वानुमान एक बिंदु बन गयाः
...
> 矩阵乘法── यही कारण है कि सादगी बेयज़ का अनुमान इतना तेजी से होता है यह और एकतरफा रैखिक मॉडल एक ही ऑपरेशन हैं

### साफ़ बेयज़ बनाम लॉजिस्टिक रिग्रेशन

दोनों ही पाठ के लिए रैखिक वर्गीकरण हैं। अंतर उनके मॉडल में है।

> दोनों ही ग्रंथों के लिए लाइनर हैं।
...
>  अनुभव विधि: पहले सरल उपयोग करें. यदि डेटा पर्याप्त है और NB प्रदर्शन रुक गया है, तो परिवर्तन तर्क वापस लौट आएँ।
...
> **【中文解读】**
> शून्य से बहुवचन सरल बेयस (बहुवचन NB, ग्रंथ में उपयोग के लिए) और उच्च सरल बेयस (Gaussian NB, निरंतर विशेषता के लिए) से। बहुवचन संस्करण प्रत्येक श्रेणी में प्रत्येक शब्द की आवृत्ति का आकलन करता है।

> **【拓展：朴素贝叶斯在现代 NLP 中的角色演变】**
>  यद्यपि ट्रांसफार्मर 模型(BERT、GPT) ने एनएलपी 任务 पर काफी हद तक सरल बेयसे से परे चला गया है, लेकिन सरल बेयसे का अभी भी अपना उपयोग हैः वास्तविक समय की कचरा मेल over(毫秒级延迟)  बड़े पैमाने पर ग्रन्थ पूर्व分类(成本极低) 作为 आधार 快速验证特征工程效果── चिकित्सा निदान में, उच्च सरल बेयसे के कारण आउटपुट संभावना स्पष्ट और व्यापक रूप से उपयोग किया जाता है डॉक्टरों को "कितना बड़ा पता होना चाहिए", न कि ब्लैक बॉक्स आउटपुट।
```figure
naive-bayes
```
...
1. **Smoothing experiment.**0.01, 0.1, 1.0, 10.0, और 100.0 के अल्फा मानों के साथ पाठ डेटा पर मल्टीनोमीअलएनबी को प्रशिक्षित करें। प्लॉट सटीकता बनाम अल्फा। प्रदर्शन का शिखर कहां है? बहुत उच्च अल्फा क्यों चोट करता है?
   1. **平滑实验。**अल्फा मूल्य 0.01 、0.1 、1.0 、10.0 、100.0 में अभ्यास करें।
...
2. **Feature independence test.**एक वास्तविक पाठ डेटासेट लें. दो शब्दों को चुनें जो स्पष्ट रूप से सहसंबंधित हैं ("मशीन" और "शिक्षा") गणना P  word1  वर्ग * P  word2  वर्ग) और तुलना P  word1 और word2  वर्ग के साथ. कितना गलत है स्वतंत्रता परिकल्पना? यह वर्गीकरण की सटीकता को प्रभावित करता है?
   2. **特征独立性测试。**取一个真文本数据集. 选择两个明显相关词 (如机器和学习) 计算 P 词1  वर्ग) * P 词2  वर्ग) 并与 P 词1 词2  वर्ग) तुलना करें 独立性假设错误多少?
...
3. **Bernoulli implementation.**एक BernoulliNB वर्ग के साथ कोड का विस्तार करें. शब्द के बैग को बाइनरी (वर्तमान / अनुपस्थित) में परिवर्तित करें और पाठ डेटा पर मल्टीनोमीअलएनबी के साथ सटीकता की तुलना करें। Bernoulli कब जीतता है?
   3. **伯努利实现。**扩展代码添加 BernoulliNB 类──将词袋转为二值(存在/不存在)并与多数组NB 在文本数据上比较准确率──伯努利何时赢?
...
4. **NB vs Logistic Regression.**पाठ डेटा पर दोनों को प्रशिक्षित करें. 100 प्रशिक्षण नमूनों से शुरू करें और 10,000 तक बढ़ें। दोनों के लिए प्लॉट सटीकता बनाम प्रशिक्षण सेट आकार। लॉजिस्टिक रिग्रेशन ने नैव बेयज़ को किस बिंदु पर आगे बढ़ाया है?
   4. **NB vs 逻辑回归。**अभ्यास के 100 से लेकर 10,000 तक बढ़ना शुरू हो गया।
...
5. **Spam filter.**एक पूर्ण स्पैम वर्गीकरण बनाएंः कच्चे ईमेल पाठ को टोकन बनाएं, शब्दावली बनाएं, शब्द के बैग सुविधाएँ बनाएं, मल्टीनोमीलएनबी को प्रशिक्षित करें, सटीकता के साथ मूल्यांकन करें और याद रखें (न केवल सटीकता - क्यों?
   5. **垃圾邮件过滤器。**构建完整的垃圾邮件分类器:分词原始邮件文本、构建词汇表、创建词袋特征、训练 बहुपदNB、精确率和召回率评估──

> **【中文解读】**
> 朴素贝叶斯的三种变体:(1) 多项式朴素贝叶斯(MultinomialNB) 适用于词频/TF-IDF特征文本分类;(2) 伯努利朴素贝叶斯(BernoulliNB) 适用于二值特征(词是否出现);(3) 高斯朴素贝叶斯(GaussianNB) 适用于连续特征,假设每类内特征服从正态分布――Laplace 平滑加) 不会防止零概率问题遇到训练集中没见过的词时,概率为零──
...
- [scikit-learn Naive Bayes docs](https://scikit-learn.org/stable/modules/naive_bayes.html)-- गणित विवरण के साथ सभी तीनों संस्करणों
  [scikit-learn 朴素贝叶斯文档](https://scikit-learn.org/stable/modules/naive_bayes.html)- तीन प्रकार के परिवर्तन एवं गणित विवरण
- [McCallum and Nigam, A Comparison of Event Models for Naive Bayes Text Classification (1998)](https://www.cs.cmu.edu/~knigam/papers/multinomial-aaaiws98.pdf)-- पाठ के लिए बहुपद बनाम बर्नौली की क्लासिक तुलना
  [McCallum and Nigam (1998)](https://www.cs.cmu.edu/~knigam/papers/multinomial-aaaiws98.pdf)- बहुवचन vs बर्नूली ग्रंथ वर्गों की तुलना
- [Rennie et al., Tackling the Poor Assumptions of Naive Bayes Text Classifiers (2003)](https://people.csail.mit.edu/jrennie/papers/icml03-nb.pdf)-- पाठ के लिए एनबी में सुधार
  [Ng and Jordan (2001)](https://ai.stanford.edu/~ang/papers/nips01-discriminativegenerative.pdf)- प्रमाणन NB में कम डेटा प्राप्त करने में तेजी
