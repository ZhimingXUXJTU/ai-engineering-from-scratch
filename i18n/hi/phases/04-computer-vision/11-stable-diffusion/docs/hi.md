# स्थिर विसारण  वास्तुकला और ठीक-ठीक ट्यूनिंग  स्थिर विसारण  वास्तुकला और सूक्ष्म
...
> **【中文解读】**स्थिर विसारण एक प्रसार मॉडल है जो पूर्व प्रशिक्षण VAE के संभावित अंतरिक्ष में संचालित होता है, पार ध्यान के माध्यम से) पाठ की स्थिति को स्वीकार करता है, तेजी से निश्चितता वाले ODE का उपयोग करता है।

> **【拓展：Stable Diffusion 生态】**स्थिर विसारण प्रवाह से LoRA (Light Quantity Micro调) ControlNet (ControlNet) 控制姿态/边缘) IP-Adapter (图像提示)  आदि समृद्ध पारिस्थितिकी  LoRA 微调 (Low)  केवल कुछ चित्रों और खपत स्तर के GPU  即可定制风格 की आवश्यकता होती है, जिससे AI 绘画进入了千万户──SDXL、SD3、FLUX等后续版本不断推动图像质量升级──
...
> 直接在 512x512 RGB 图像上训练 DDPM 很昂贵. प्रत्येक प्रशिक्षण चरण को एक U-Net में 3x512x512 = 786,432 输入值 देखने के लिए एक U-Net में विपरीत दिशा में प्रसारण करने की आवश्यकता होती है, नमूना को उसी U-Net में 50+ बार पूर्व दिशा में प्रसारण करने की आवश्यकता होती है।
...
> इसे चित्रों में खोलने के लिए उपयोगी तकनीक है**潜空间扩散**(Rombach 等,CVPR 2022) ∙ प्रशिक्षण एक VAE 3x512x512 图像映射 4x64x64 潜张量并恢复, फिर उस潜空间 में विस्तार करना ∙ गणना मात्रा घटाना `(3*512*512)/(4*64*64) = 48x`एक ही GPU पर उपरोक्त नमूना कुछ सेकंड से दो सेकंड के भीतर नीचे आया
...
>  लगभग प्रत्येक आधुनिक छवि उत्पादन मॉडल SDXL、SD3、FLUX、HunyuanDiT、Wan-Video सभी पनरोक विस्तार मॉडल हैं, स्वयं编码器、去噪器(U-Net या DiT) और पाठ की स्थिति पर कुछ अंतर है।
...
> 纯文本条件化学习 `epsilon_theta(x_t, t, c)`प्रत्येक शब्द के लिए`c`◊CFG प्रशिक्षण एक नेटवर्क के साथ 10% समय बर्बाद`c`(बदला-बदला के लिए रिक्त एम्बेड), एक साथ भविष्यवाणी शोर और बिना शोर के मॉडल प्राप्त करेंः
...
> `w`यह एक दिशा है।`w=0`है बिना शर्त उत्पन्न,`w=1`है सामान्य परिस्थितियों उत्पन्न,`w>1`विविधता के बलिदान में उत्पादन को बढ़ावा देना अधिक "अनुपालन में"`w=7.5`
...
> CFG है कि सामग्री के लिए चित्रों को उत्पादन स्तर की गुणवत्ता के तहत काम करने के कारणों में से एक है।
...
> VAE का 4 मार्ग潜量量 न केवल संपीड़ित छवि है। यह एक प्रवाह है, इसके ऊपर का गणितीय परिचालन काफी हद तक प्रतिक्रिया के लिए है।
...
1. **Img2img**= छवि को लटेंट में एन्कोड करें, आंशिक शोर जोड़ें, डेनोइज़र चलाएं, डिकोड करें। छवि संरचना जीवित रहती है क्योंकि एन्कोडिंग लगभग उलटनीय है; प्रॉम्प्ट के आधार पर सामग्री बदलती है।
   中文翻译:**Img2img**= छवि को परिवर्तित करने के लिए छवि को कोडित करना, कुछ शोर जोड़ना, शोर निकालने के लिए काम करना, को हटाना, छवि संरचना को बरकरार रखना, क्योंकि कोड को लगभग उल्टा किया जा सकता है; सामग्री के अनुसार सुझाव शब्द परिवर्तन करना।
2. **Inpainting**= img2img के समान लेकिन denoiser केवल छिपे हुए क्षेत्रों को अपडेट करता है; अनमास्केड क्षेत्रों को एन्कोडेड लटेंट पर रखा जाता है।
   中文翻译:**Inpainting**= इमेज2 इमेज के समान, लेकिन ध्वनि निकालनेवाला केवल छिपे हुए क्षेत्र को अपडेट करता है; छिपे हुए क्षेत्र को कोड के बाद के संभावित परिवर्तनों के लिए बनाए रखता है।

### यू-नेट वास्तुकला

एसडी यू-नेट टिनीयूनेट का एक बड़ा संस्करण है जो तीन जोड़ों के साथ पाठ 10 से आता हैः

> SD का U-Net TinyUNet का 10वां संस्करण है, जिसमें तीन घटक शामिल हैंः

- **Transformer blocks**प्रत्येक स्थानिक संकल्प पर, स्वयं-विचार + पाठ एम्बेडिंग पर पार-विचार शामिल है।
  中文翻译: प्रत्येक अंतरिक्ष के संकल्प पर ट्रांसफार्मर 块, जिसमें स्वयं ध्यान + पाठ में सम्मिलित के लिए पारस्परिक ध्यान शामिल है
...
> SD 1.5  कुल तत्वों की संख्या लगभग 8.6 अरब है।
...
> स्थिर विसारण के पूर्ण मात्रा में सूक्ष्म समायोजन की आवश्यकता है 20+ GB 显存并更新 8.6 亿参数。LoRA(低秩适应) बनाए रखने के लिए बुनियादी मॉडल结, ध्यान के स्तर में छोटे क्रम में विभाजित矩阵注入──SD के LoRA 适配器 आमतौर पर केवल 10-50 MB, एक ही चेंज में खपत श्रेणी GPU ऊपर प्रशिक्षण 10-60 मिनट, सुझाव के रूप में तत्काल उपयोग में संशोधन लोड──
...
> लोरा लगभग सभी समुदायों में एक अलग तरीके से विकसित होता है।

### कार्यक्रम आप देखेंगे

- **DDIM** निर्धारक, ~50 कदम, सरल।
  中文翻译:DDIM确定性, लगभग 50 步,简单――
- **Euler ancestral** स्टोकास्टिक, 30-50 कदम, थोड़ा अधिक रचनात्मक नमूने।
  中文翻译:ईलर पूर्वज随机性,30-50 步,样本更具创意──
...
> `diffusers`मध्य स्विच समायोजक को केवल एक लाइन कोड की आवश्यकता होती है, कभी-कभी पुनः प्रशिक्षण की आवश्यकता नहीं होती है।
...
> 本课端到端使用 `diffusers`इसके बजाय शून्य से पुनर्निर्माण स्थिर प्रसारण से (आपको पुनर्निर्माण की आवश्यकता है) विभिन्न विशेष पाठ्यक्रमों के लिए (वैकल्पिक)
...
> `float16`प्रमाणित डिपॉजिट उपयोग और कोई स्पष्ट गुणवत्ता हानि नहीं `num_inference_steps=25`डीडीआईएम के उपयोग के समान`num_inference_steps=50`
...
> 调度器状态与 U-Net 权重解──आप किसी भी调度器 से प्रशिक्षण ले सकते हैं
...
> `strength`控制去噪音前添加多少噪音(0.0 = 不变,1.0 = 完全重新生成) ⋅0.5-0.7 है风格迁移的标准范围──
...
> `lora_scale`控制强度;0.0 = 无效,1.0 = 完全效果──`fuse_lora`वेट में तेजी लाने के लिए एडाप्टर को संश्लेषित करें, लेकिन स्विच को रोकें।`pipe.unfuse_lora()`

### चरण 6: लोरा प्रशिक्षण (स्केच)

वास्तविक लोरा प्रशिक्षण जीवन में `peft`या `diffusers.training`. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .

> असली लोरा प्रशिक्षण में`peft`या `diffusers.training`निम्नानुसार किया गया हैः
...
>  केवल लोरा 矩阵接收梯度; आधार U-Net、VAE 和文本编码器 सभी 结的──批量大小为 1 并启用梯度检查点时,8GB 显存即可运行──
