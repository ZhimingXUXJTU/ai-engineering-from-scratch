# दृष्टि-भाषा मॉडल  ViT-MLP-LLM पैटर्न  दृश्य भाषा मॉडल  ViT-MLP-LLM 模式
...
> **【中文解读】**视觉编码器将图像转换为代币,MLP 投影层将这些代币映射到LLM के嵌入空间,语言模型完成剩余工作──ViT-MLP-LLM 2026 में सभी उत्पादन स्तरीय视觉语言模型 (VLM) का मानक मॉडल है  GPT-4V、Claude 3 Vision、LLaVA आदि 

> **【拓展：VLM 的应用】**VLM है बहुमुखी एआई का मूल:GPT-4V(图像问答) Claude Vision(文档分析) ✓LLaVA(开源视觉对话) ✓Google Gemini(多模态推理) ✓ वित्तीय क्षेत्र में, VLM का उपयोग वित्तीय आलेखों को समझने, अनुबंधों की समझ, बुद्धिमान समीक्षा, स्वतः प्रसंस्करण के लिए किया जा सकता है
...
> CLIP( चौथा चरण 18 课) आपको छवियों और पाठ के साझा करने के लिए एक स्थान देता है, जो कि एक नमूना विभाजन और जांच करने के लिए पर्याप्त है। यह जवाब नहीं दे सकता है "इस चित्र में कुछ लाल कारें हैं? " क्योंकि CLIP पाठ नहीं उत्पन्न करता है। यह केवल समानता का मूल्यांकन करता है।
...
> 视觉语言模型(VLM) Qwen3-VL、InternVL3.5、LLaVA-Next、GLM-4.6V CLIP परिवार के इमेज कोडिंग मशीन को पूर्ण भाषा मॉडल से कनेक्ट करेगा──模型 देखें इमेज जोड़ना समस्या और उत्पन्न उत्तर── 2026 में, ओपन सोर्स VLM में बहु-मॉडल基准上匹敌或击败 GPT-5 和 Gemini-2.5-Pro──
...
> तीन घटक (ViT, प्रोजेक्टर, एलएलएम) मानक विन्यास हैं। मॉडल के बीच अंतर यह है कि किस ViT, किस प्रोजेक्टर का उपयोग करें, किस LLM, प्रशिक्षण डेटा और तैयार कार्यक्रमों में से एक है।
...
1. **Vision encoder** एक पूर्व प्रशिक्षित ViT (CLIP-L/14, SigLIP, DINOv3 या एक ठीक से ट्यून संस्करण) ।
   中文翻译:**视觉编码器**预训练的 ViT(CLIP-L/14、SigLIP、DINOv3 अथवा微调变体) 产生补丁代号──
2. **Projector** एक छोटा मॉड्यूल (2-4 परत MLP, या एक Q-former) जो LLM के एम्बेडिंग आयाम में दृष्टि टोकन का नक्शा बनाता है। यह वह जगह है जहां अधिकांश बारीक-बारी से ट्यूनिंग होती है।
   中文翻译:**投影器**小型模块 (~2 लेयर MLP या Q-former),将视觉 टोकन 映射到LLM के嵌入维度── अधिकांश लघु调调发生在这里──
3. **LLM** एक केवल डेकोडर भाषा मॉडल (Qwen3, Llama, Mistral, GLM, InternLM) । यह दृश्य + पाठ टोकन को क्रमशः पढ़ता है, पाठ उत्पन्न करता है।
   中文翻译:**LLM**仅解码器语言模型(Qwen3、Llama、Mistral、GLM、InternLM) ・・・顺序读取视觉 + 文本代币,生成文本。
...
> तीन भागों के सिद्धांतों पर प्रशिक्षण प्राप्त होता है। अभ्यास में, विज़ुअल एडिटर और एलएलएम केवल प्रोजेक्टर को प्रशिक्षित करते हैं जो सस्ते लागत पर अरबों पैरामीटर के संकेत प्राप्त करते हैं।
...
> 朴素投影只使用ViT 最后一层――DeepStack(Qwen3-VL) कई ViT गहराई采样特征并堆叠――更深层携带高级语义;更浅层携带细粒度空间和纹理信息――将两者都送进 LLM 弥合了"图像包含什么" (图像包含什么) 语义) और"具体在哪里" (空间定位) के बीच का अंतर――
...
1. **Alignment** विटी और एलएलएम को फ्रीज करें. केवल प्रोजेक्टर को छवि-कैप्शन जोड़े पर प्रशिक्षित करें। प्रोजेक्टर को भाषा स्थान में दृष्टि स्थान का नक्शा बनाने के लिए सिखाता है।
   中文翻译:**对齐**结 ViT 和 LLM── केवल छवि-शीर्षक पर प्रशिक्षण प्रोजेक्टर──शिक्षण प्रोजेक्टर将视觉空间映射到语言空间──
2. **Pre-training** सब कुछ मुक्त करें। बड़े पैमाने पर इंटरलेटेड छवि-पाठ डेटा (500M+ जोड़े) पर प्रशिक्षित करें। मॉडल के दृश्य ज्ञान का निर्माण करता है।
   中文翻译:**预训练**解一切── बड़े पैमाने पर संबन्धित छवियों-पाठों के डेटा पर प्रशिक्षण5 अरब+ के लिए)── निर्माण मॉडल का दृश्य ज्ञान
3. **Instruction tuning** क्यूरेट किए गए (छवि, प्रश्न, उत्तर) ट्रिपल पर ठीक-ठीक ट्यून करना। बातचीत व्यवहार और कार्य प्रारूप सिखाता है। यह एक "दृष्टि-जागरूक एलएम" को एक उपयोगी सहायक में बदल देता है।
   中文翻译:**指令微调**在精选的 (图像,问题,答案) 三元组上微调──教会对话行为和任务格式── यह "दृश्य संज्ञानात्मक एलएम" को उपलब्ध सहायक में बदल देगा──
