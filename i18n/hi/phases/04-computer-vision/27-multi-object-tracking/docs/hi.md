# मल्टी-ऑब्जेक्ट ट्रैकिंग और वीडियो मेमोरी  बहु-उद्देश्य ट्रैक और वीडियो मेमोरी
...
> **【中文解读】**अनुसरण = 检测 + 关联── प्रत्येक 检测目标, फिर वर्तमान 检测 के परिणामों को ऊपर के 追踪轨迹 के साथ ID 匹配──多目标跟踪(MOT) वीडियो समझने की मूल तकनीक है, जिसमें छिपने, गायब होने, फिर से आने आदि जटिल परिस्थितियों को संभालने की आवश्यकता है──

> **【拓展：MOT 的应用】**कई उद्देश्यों का पालन करना सुरक्षा निगरानी में करना है, जैसे कि:
...
> 检测器 आपको बताता है वस्तुओं के एक में स्थान में                                                                                                                                                                                                                                                        `t`中哪个检测与第 `t-1` में से कौन सा परीक्षण एक ही वस्तु है  इसके बिना, आप रेखा पार करने वाले वस्तुओं की संख्या का गणना नहीं कर सकते  ढोए गए गेंद का पीछा कर सकते हैं, या जानते हैं "4 नंबर की गाड़ी इस रास्ते पर 8 सेकंड से है"
...
> प्रत्येक वीडियो के लिए महत्वपूर्ण हैंः शारीरिक विश्लेषण, निगरानी, स्वचालित ड्राइविंग, चिकित्सा वीडियो विश्लेषण, वन्यजीव निगरानी, व्यापारिक अंकन।
...
> 2026 में दो नए मॉडल आएंगे।**SAM 2 基于记忆的跟踪**(特征记忆替代运动模型关联) और **SAM 3.1 Object Multiplex**(एक ही अवधारणा के कई उदाहरण साझा स्मृति) 本课先走经典,然后走基于记忆的方法
...
- **ByteTrack**(2021): कम आत्मविश्वास की पहचान को दूसरे चरण के रूप में जोड़ता है; कोई उपस्थिति सुविधाओं की आवश्यकता नहीं है लेकिन MOT17 पर शीर्ष प्रदर्शनकर्ता।
  中文翻译:**ByteTrack**(2021):将低信度检测作为第二阶段关联;无需外观特征但在MOT17上表现最佳――
- **BoT-SORT**(2022): बाइट + कैमरा गति मुआवजा + रीआईडी।
  中文翻译:**BoT-SORT**(2022):बाइट + 相机运动补偿 + रीआईडी
- **StrongSORT / OC-SORT** बेहतर गति और उपस्थिति के साथ बाइटट्रैक वंशज।
  中文翻译:**StrongSORT / OC-SORT**बाइटट्रैक पछली पीढ़ी, सुधार किया गया आंदोलन और बाहरी मॉडल
...
> कार्लमैन 波器 प्रत्येक पटरियों की स्थिति बनाए रखने `(x, y, w, h, dx, dy, dw, dh)`和协方差──每先恒速模型**预测**स्थिति, पुनः प्रयोग匹配的检测**更新**预测不确定性高时更新更信任检测── यह एक चिकनी रफ्तार पैदा करता है, यह एक क्षणिक छाया से गुजर सकता है1-5 )

हर क्लासिक ट्रैकर गति-पूर्वानुमान चरण में एक Kalman फ़िल्टर का उपयोग करता है।

> प्रत्येक क्लासिक ट्रैकर में खेल पूर्वानुमान चरणों में सभी का उपयोग कर रहे हैं
...
> 给定 `M x N`                                                                                                                                                                                                                                                              `1 - IoU(轨迹框, 检测框)`या बाहरी दृश्य विशेषता का负余弦相似度──运行时间为 O(((M+N) ^3);M、N 约1000 以内在Python中通过 `scipy.optimize.linear_sum_assignment`足足足快――
...
> 标准跟踪器丢弃低置信度检测(< 0.5) ――बाइटट्रैक उन्हें बनाए रखेगा**第二阶段候选**: उच्च विश्वासता जांच के बाद, अपर्याप्त ट्रैक की कोशिश की गई है कि थोड़ा हल्का IoU मूल्य कम विश्वासता जांच के साथ मेल खाए।
...
- **MOTA (Multi-Object Tracking Accuracy)** 1 - (FN + FP + ID स्विच) / GT। त्रुटि प्रकार द्वारा वजन; एक एकल मीट्रिक जो पता लगाने और संघ दोषों को मिलाता है।
  中文翻译:**MOTA（多目标跟踪精度）**1 - (FN + FP + ID 切换) / GT── त्रुटि प्रकार के अनुसार अतिरिक्त अधिकार;将检测和关联失败混混的单一指标──
- **IDF1 (ID F1)** आईडी सटीकता और याद करने का सामंजस्यपूर्ण औसत। यह विशेष रूप से इस बात पर केंद्रित है कि प्रत्येक ग्राउंड-सत्य ट्रैक समय के साथ अपनी आईडी को कैसे रखता है। आईडी-स्विच-संवेदनशील कार्यों के लिए एमओटीए से बेहतर।
  中文翻译:**IDF1（ID F1）**ID 精确率和召回率调和平均―― ध्यान प्रत्येक वास्तविक रस्ते को समय के साथ ID 精确率  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें  ध्यान केंद्रित करें
- **HOTA (Higher Order Tracking Accuracy)** पता लगाने की सटीकता (DetA) और संघ सटीकता (AssA) में विघटित होता है। 2020 से सामुदायिक मानक; सबसे व्यापक।
  中文翻译:**HOTA（高阶跟踪精度）**分解为检测精度(DetA) 和关联精度(AssA) ・2020 साल से सबसे व्यापक सामुदायिक मानक
...
> 监控(谁是谁): रिपोर्ट IDF1──运动分析(计数传球):HOTA──一般学术比较:HOTA──
