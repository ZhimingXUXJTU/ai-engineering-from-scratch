# उत्पादन एमएलए आवेदन का निर्माण
...
> **【中文解读】**आपने सुझाव, एम्बेड,RAG, फ़ंक्शन समायोजन, कैशिंग और रखरखाव को अलग-अलग बनाया है। इस कोर्स में सभी घटक एक उत्पादन स्तर की सेवा में एकीकृत किए जाएंगे जो वास्तविक प्रवाह, सुरुचिपूर्ण गिरावट, प्रवाह आउटपुट, ट्रैकिंग लागत, पहली मात्रा में 10,000 उपयोगकर्ताओं को संभाल सकेगी।

> **【拓展：生产化→AI工程全栈】**यह चरण 11 का एक एकीकृत विकास पाठ्यक्रम है। यह "एआई एपीआई का उपयोग करने वाले" से "निर्मित उत्पादन प्रणाली बनाने" तक के लिए महत्वपूर्ण कदम है।

>  **【前置】**यह चरण 11 का कैपस्टोन है, चरण 11 के पहले अध्ययन के लिए आवश्यक है।

**Type:** Build (Capstone) | **类型:** 构建（顶点课）
**Languages:** Python | **语言:** Python
**Prerequisites:** Phase 11 Lessons 01-15 | **前置知识:** Phase 11 · 01-15
**Time:** ~120 minutes | **时间:** ~120 分钟
**Related:**चरण 11 · 14 (MCP) एक साझा प्रोटोकॉल के साथ कस्टमाइज़ेड टूल योजनाओं को बदलने के लिए; चरण 11 · 15 (प्रॉम्प्ट कैशिंग) स्थिर उपसर्गों पर 50-90% लागत में कमी के लिए। दोनों को 2026 के प्रत्येक गंभीर उत्पादन स्टैक में अपेक्षित है।**相关:**चरण 11 · 14 (MCP) साझाकरण समझौते के साथ विशेष उपकरण योजना के प्रतिस्थापन; चरण 11 · 15 (提示缓存) में स्थिर पूर्व वृद्धि में 50-90% की कमी है 成本── दोनों 2026 वर्ष के कठोर उत्पादन के मानक विन्यास──

## सीखने के लक्ष्य

- सभी चरण 11 घटकों (प्रॉम्प्ट, आरएजी, फ़ंक्शन कॉल, कैशिंग, गार्डरेल्स) को एक एकल उत्पादन-तैयार सेवा में जोड़ें
  चरण 11 को सभी घटकों में जोड़ें (提示、RAG、函数调用、缓存、护)
- स्ट्रीमिंग टोकन वितरण, अनुग्रहपूर्ण त्रुटि प्रसंस्करण को लागू करें, और समय सीमा प्रबंधन का अनुरोध करें
  实现流式 टोकन 交付、优雅错误处理和请求超时管理
- एप्लिकेशन में अवलोकनशीलता का निर्माण करेंः अनुरोध लॉगिंग, लागत ट्रैकिंग, विलंबता प्रतिशत और त्रुटि दर डैशबोर्ड
  构建应用可观测性: अनुरोध日志、 लागत ट्रैकिंग、 देरी प्रतिशत और त्रुटि दर उपकरण盘
- स्वास्थ्य जांच, दरों की सीमा और प्रदाता के विराम के लिए एक बैकअप रणनीति के साथ आवेदन को लागू करें
  स्वास्थ्य जांच, सीमित प्रवाह और प्रदाता के वापसी रणनीति सहित आवेदन

> **【中文解读】**इस वर्ग का उद्देश्य: LLM को मूल प्रकार से उत्पादन वातावरण तक लागू करना होगा।

>  **【类比】**डेमो बनाम 生产 = 学生作业 बनाम 银行系统――डेमो:单进程、单用户、不持久化、无监控、API 失败就崩──生产:多进程、并发限流、状态持久化、全链路监控、API 失败自动倒退、灰度发布、版本回滚──差异是工程量,不是AI 能力──

> ️ **【易错点】**生产部署 के 3 个坑: ((1) **没做 provider fallback**OpenAI 机 पूरा उत्पाद लटक गया; using LiteLLM या खुद लिख एक परत,OpenAI 失败自动转人类――(2) **没用流式输出**长回答让用户等 10s 才看到第一个字;用 `stream=True`+ एसएसई,首字延迟降至500ms──(3) **没追踪成本**上线 3 天烧光一个月预算; प्रति API 调用记符号 数 + 成本到兰福斯/ हेलिकॉन,设日均预算告警──
...
> एक LLM का निर्माण करने के लिए केवल एक दोपहर की आवश्यकता होती है।
...
> आज प्रत्येक उत्पादन वातावरण में LLM  अनुप्रयोग कड़वाहट कर्सर ChatGPT नशन AI都 ने इन समस्याओं को हल किया है―
...
> यह शीर्ष बिंदु पाठ्यक्रम है। आप एक पूर्ण उत्पादन स्तर LLM सेवा का निर्माण करेंगे, एकीकृत सुझाव प्रबंधन, एम्बेडिंग और वेक्टर खोज, फ़ंक्शन समायोजन, मूल्यांकन, कैशिंग, देखभाल, प्रवाह आउटपुट, त्रुटि प्रसंस्करण, दृश्यता और लागत ट्रैकिंग।

## अवधारणा का मूल अवधारणा

> **【中文解读】**एपीआई 网关  भार संतुलन 推理引擎 量数据库 缓存层 监控警警 
...
> प्रत्येक LLM के आवेदन एक ही प्रक्रिया का पालन करते हैं।
...
> अनुरोध एपीआई के माध्यम से 网关进入,网关处理认证和限流──提示路由器选择合适模板前,输入护检查提示注入和违规内容──语义缓存检查近期是否回答过类似问题──缓存未定中时,启动流式调用 LLM──输出护验证响应──评估日志器记录质量指标──成本追踪器核算每个代币──响应流式返回客户端──

सात घटक, प्रत्येक एक सबक है जो आपने पहले ही पूरा कर लिया है। इंजीनियरिंग तारों में है।

> 七组件── प्रत्येक में आपके द्वारा पूरा किए गए पाठ──工程在连线中──
...
> GPT-5 生成 500 输出 टोकन का प्रतिक्रिया आवश्यकता 3-8 सेकंड──不流式时用户全程着转圈──流式时首个 टोकन 在 200-500ms到达──总时间相同──感知延迟下降90%──
...
> एसएसई है डिफ़ॉल्ट चयन──OpenAI、Anthropic 和 Google सभी एसएसई 流式──आपका सर्वर एलएलएम एपीआई से 接收块并作为 एसएसई 事件转发给客户端──客户端用`EventSource`(浏览器) या `httpx`(पाइटन) खपत流──
...
> उत्पादन LLM  आवेदन में तीन अलग-अलग असफलता के तरीके हैं। प्रत्येक को अलग-अलग पुनर्प्राप्ति रणनीति की आवश्यकता है।
...
**第一层：API 失败。**LLM प्रदाता वापसी 429  सीमा प्रवाह) 、500  सर्वर त्रुटि)  या超时── समाधान:带动的指数退避── 1 से 2 सेकंड से, प्रति बार पुनः प्रयास करें,加随机动防惊群效应── अधिकतम 3 बार पुनः प्रयास करें──
...
**第二层：模型失败。**模型返回形式错误 JSON、幻觉函数名或产生验证失败的输出── समाधान: सुधार के सुझाव के साथ पुनः प्रयास──把错误包含在重试消息中让模型自我纠正──
...
> **回退模型链。**मुख्य मॉडल अनुपयोगी, क्रॉस-चेन वापस लौटेंः
...
> प्रत्येक चरण में गुणवत्ता में परिवर्तन उपलब्धता है।
...
**结构化日志。**प्रत्येक अनुरोध उत्पन्न JSON 日志条目: अनुरोध आईडी、 उपयोगकर्ता आईडी、提示模板名、所用模型、输入 टोकन、输出 टोकन、延迟(ms)、缓存命中/未中、护通过/失败、成本(USD)及任何错误──
...
**指标仪表盘。**प्रत्येक LLM  टीम के लिए चिंताओं के पांच अंकः
...
> 提示 को पूरा करने में सक्षम नहीं है, यह डेटा के साथ साबित होता है कि यह सफल विकल्प है।
...
**影子模式。**100% ट्रैफ़िक पर नए सुझाव चलाए लेकिन केवल रिकॉर्ड परिणाम उपयोगकर्ताओं को नहीं दिखाए गए️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️️
...
**百分比灰度。**10% की आवाजाही को नए संकेतों तक ले जाएँ।
...
> उपयोगकर्ता आईडी की निश्चितता है, कोई भी विकल्प नहीं है। यह सुनिश्चित करता है कि प्रत्येक उपयोगकर्ता एक ही प्रयोग में अनुरोध अनुभव को एक जैसा महसूस करता है।
...
**Perplexity。**उपयोगकर्ता पूछताछ प्रवेश करें। खोज इंजन पूछताछ 10-20 网页。网页分块、嵌入、重排。 शीर्ष 5 ब्लॉक बन गए RAG 上下文。LLM 生成带引用的答案,实时流式返回── दो मॉडल:快模型用于搜索查询重写,强模型用于答案合成── अनुमानित 5000+ मिलियन 查询/天──
...
**Cursor。**打开的文件、周围文件、近期编辑和终端输出构成上下文──提示路由器决定:小模型做自动补充(Cursor-small, लगभग 20ms),大模型做聊天(Claude Sonnet 4.6 / GPT-5, लगभग 3s) ⋅上下文被激进压缩仅相关代码段,非整个文件──代码库嵌入提供长程上下文──投机编辑流式不同,非整个文件──MCP 集成让第三方工具插入而无需按工具修改代码──
...
**ChatGPT。**插件、 फ़ंक्शन调用和 MCP 服务器让模型访问网络、运行代码、生成图像和查询数据库──路由层决定调用哪些能力──记忆跨会话持久化用户偏好──系统提示是1,500+ टोकन के व्यवहार नियम,通过提示缓存──多模型服务不同功能:GPT-5做聊天、GPT-Image做图像、Whisper做语音、o4-mini做深度推理──
...
- **Async everywhere.**LLM कॉल पर कभी भी वेब सर्वर के धागे को ब्लॉक न करें।`asyncio`और `httpx.AsyncClient`. .
  **处处异步。**永不LLM 调用上阻塞网 服务器线程──用 `asyncio`和 `httpx.AsyncClient`
- **Queue-based processing.**वास्तविक समय के लिए कार्य (संक्षेप, विश्लेषण) के लिए, एक कतार (रेडिस, एसक्यूएस) पर धक्का दें और श्रमिकों के साथ प्रक्रिया करें। नौकरी आईडी वापस करें, ग्राहक सर्वेक्षण को छोड़ दें।
  **基于队列的处理。**गैर वास्तविक समय का कार्य (अंशिका, विश्लेषण) कटाई में लाया गया (Redis, SQS)
...
- **Horizontal scaling.**LLM एप्लिकेशन I/O बाध्य हैं, सीपीयू बाध्य नहीं। एक एकल असिनक्रॉन सर्वर 100+ समवर्ती अनुरोधों को संभालता है। स्केल सर्वर, कोर नहीं।
  **水平扩展。**LLM  अनुप्रयोग 密集 है और CPU 密集 नहीं है  एकल भिन्न चरण सर्वर प्रसंस्करण 100+ और प्रक्षेपण अनुरोधों  विस्तार सर्वर, गैर-केंद्रित
...
> यह शीर्ष बिंदु है एक फाइल सभी घटक एक साथ जुड़े हैं
...
- अनुरोध प्रति लागत ट्रैकिंग और समग्र
  प्रत्येक अनुरोध और एकत्रित अनुवर्ती
...
> 基础――配置、日志 तथा प्रत्येक घटक पर निर्भर डेटा संरचना――
...
> 版本化提示模板含A/B 测试支持── प्रत्येक模板名、版本和模板字符串──路由器根据请求上下文和实验分配选择──
...
> 编排器──接收原始用户请求, प्रत्येक घटक पर रन, पुनः संरचनात्मक परिणाम──
...
> उपरोक्त प्रस्तुति में स्क्रिप्ट परिचालन ः उत्पादन में फास्टएपीआई  पैकेजिंग में औपचारिक अंत बिंदु ः
...
> 作为真实服务器运行:取消注释并装依赖 `pip install fastapi uvicorn`访问 `http://localhost:8000/docs`स्वचालित रूप से उत्पन्न एपीआई 文档──

### वास्तविक एपीआई एकीकरण

वास्तविक प्रदाता एसडीके के साथ अनुकरण LLM कॉल की जगह लें।

> उपयोग करें वास्तविक प्रदाता एसडीके 替换模拟 LLM 调用。
...
> चार कर्मचारी── प्रत्येक प्रसंस्करण में अलग-अलग चरण I/O── एक मशीन 4 कर्मचारी  सेवा 400+ 并发 LLM अनुरोध, क्योंकि सभी अन्य नेटवर्क I/O और CPU नहीं──
...
> 本课产 出 `outputs/prompt-architecture-reviewer.md`对照生产清单审查任何 LLM 应用架构的可复用提示──给它你的系统描述,返回差距分析──
...
> और उत्पादन`outputs/skill-production-checklist.md`把LLM 应用上线生产的决策框架, इस वर्ग के प्रत्येक घटक को कवर करना, जिसमें विशिष्ट मूल्य एवं पारित/ असफल मानदंड शामिल हैं
...
1. **Add RAG integration.**20 दस्तावेजों के साथ एक सरल इन-मेमोरी वेक्टर स्टोर बनाएं। जब टेम्पलेट `rag_answer`, क्वेरी एम्बेड करें, सबसे समान 3 दस्तावेजों को खोजें, और उन्हें संदर्भ के रूप में इंजेक्ट करें। मापें कि RAG संदर्भ के साथ और बिना प्रतिक्रिया की गुणवत्ता कैसे बदलती है। एलएलएम विलंबता से अलग से पुनर्प्राप्त करने की विलंबता का ट्रैक करें।
   **添加 RAG 集成。**20 दस्तावेजों का उपयोग करके सरल इनमेज स्टोरेज के लिए किया गया है।`rag_answer`时,嵌入查询,找3 个最相似文档,作为上下文注入.
...
3. **Build a cost alerting system.**प्रति उपयोगकर्ता प्रति दिन लागत का पता लगाएं। जब एक उपयोगकर्ता से अधिक $0.50/day, switch them to `gpt-4o-mini`. When total daily cost exceeds $100, आपात स्थिति मोड सक्रिय करेंः दोहराए गए प्रश्नों के लिए केवल कैश प्रतिक्रियाएँ, `gpt-4o-mini`और बाकी सब कुछ के लिए, 2,000 इनपुट टोकन से अधिक अनुरोधों को अस्वीकार करें. एक अनुकरणित यातायात की वृद्धि के साथ परीक्षण करें.
   **构建成本告警系统。** ट्रैकिंग प्रति उपयोगकर्ता प्रति दिन लागत $0.50/天时切换到 `gpt-4o-mini`。每日总成本超 $100 时激活紧急模式:重复查询仅缓存、其他全用 `gpt-4o-mini`、 अस्वीकार >2,000 输入 टोकन अनुरोध.
...
5. **Add OpenTelemetry tracing.**प्रत्येक घटक (कैश खोज, गार्डरेल जांच, एलएलएम कॉल, लागत गणना) को एक अलग अवधि के रूप में उपयोग करें। प्रत्येक अवधि अपनी अवधि रिकॉर्ड करती है। कंसोल में निशान निर्यात करें। एक एकल अनुरोध के लिए पूर्ण निशान दिखाएं, प्रत्येक घटक के योगदान के साथ कुल विलंबता दिखाई देगी।
   **加 OpenTelemetry 追踪。**प्रत्येक घटक को एक स्वतंत्र अवधि के रूप में जोड़ें। प्रत्येक अवधि को रिकॉर्ड समय में खींचें।
...
- [FastAPI Documentation](https://fastapi.tiangolo.com/)-- इस पाठ में इस्तेमाल असिनक्रॉन पायथन फ्रेमवर्क, मूल एसएसई स्ट्रीमिंग और स्वचालित ओपनएपीआई दस्तावेजों के साथ
  फास्टएपीआई 文档本课所需异步पायथन 框架,含原生SSE 流式和自动OpenAPI 文档
- [OpenAI Production Best Practices](https://platform.openai.com/docs/guides/production-best-practices)-- सबसे बड़ी एलएलएम एपीआई प्रदाता से दर सीमा, त्रुटि प्रबंधन और स्केलिंग मार्गदर्शन
  OpenAI उत्पादन सर्वोत्तम प्रथा अधिकतम LLM API प्रदाता का सीमित प्रवाह、 त्रुटि प्रसंस्करण एवं विस्तार मार्गदर्शिका
- [Anthropic API Reference](https://docs.anthropic.com/en/api/messages-streaming)-- क्लाउड के लिए स्ट्रीमिंग कार्यान्वयन विवरण, जिसमें सर्वर द्वारा भेजे गए घटनाएं और स्ट्रीमिंग के दौरान उपकरण का उपयोग शामिल है
  मानव एपीआई 参考Claude के प्रवाह के कार्यान्वयन विवरण, जिसमें SSE और प्रवाह के मध्य उपकरण का उपयोग शामिल है
- [OpenTelemetry Python SDK](https://opentelemetry.io/docs/languages/python/)-- वितरित ट्रैकिंग के लिए मानक, LLM पाइपलाइन के प्रत्येक घटक के उपकरण के लिए उपयोग किया जाता है
  OpenTelemetry Python SDK वितरित ट्रैकिंग मानक, LLM 流水线 प्रत्येक घटक में प्लग करने के लिए उपयोग किया जाता है
- [Semantic Caching with GPTCache](https://github.com/zilliztech/GPTCache)-- उत्पादन अर्थशास्त्र कैशिंग पुस्तकालय जो इस पाठ से अवधारणाओं को पैमाने पर लागू करता है
  GPTCache 语义缓存 बड़े पैमाने पर इस वर्ग अवधारणा के उत्पादन语义缓存库 को लागू करना
- [Hamel Husain, "Your AI Product Needs Evals"](https://hamel.dev/blog/posts/evals/)-- एमएलएम अनुप्रयोगों के लिए मूल्यांकन-चालित विकास पर अंतिम मार्गदर्शिका, जो इस कपास्टोन में मूल्यांकन घटक को पूरक करती है
  हमेल हुसैन "आपकी एआई  उत्पाद मूल्यांकन की आवश्यकता है"
- [Eugene Yan, "Patterns for Building LLM-based Systems"](https://eugeneyan.com/writing/llm-patterns/)-- प्रमुख प्रौद्योगिकी कंपनियों में उत्पादन LLM तैनाती के दौरान देखे जाने वाले वास्तुशिल्प पैटर्न (गार्डरेल, RAG, कैशिंग, रूटिंग)
  यूजीन यान "कन्स्ट्रक्शन LLM 系统的模式"大科技公司生产 LLM 部署 में देखे गए ढांचे के ढांचे के मॉडल ([[护、RAG、缓存、路由)
...
- [Hugging Face TGI](https://huggingface.co/docs/text-generation-inference/index)-- पाठ पीढ़ी इन्फरेंसः निरंतर बैचिंग, फ्लैश ध्यान, और मेदुसा अनुमानात्मक डिकोडिंग के साथ जंग सर्वर; वीएलएलएम के लिए एचएफ-नेटिव विकल्प।
  Hugging Face TGI文本生成推理:Rust 服务器配连续批处理、Flash Attention 和 Medusa 投机解码;vLLM का HF 原生替代──
- [NVIDIA TensorRT-LLM documentation](https://nvidia.github.io/TensorRT-LLM/)-- एनवीआईडीए हार्डवेयर पर उच्चतम आउटपुट पथ; क्वांटिज़ेशन, उड़ान में बैचिंग, और उद्यम तैनाती के लिए एफपी8 कर्नल।
  एनवीआईडीए TensorRT-LLM 文档NVIDIA 硬件 पर उच्चतम संचलन पथ; मात्राकरण, उड़ान थोक प्रसंस्करण और FP8 内核, उद्यम部署 हेतु
- [Hamel Husain -- Optimizing Latency: TGI vs vLLM vs CTranslate2 vs mlc](https://hamel.dev/notes/llm/inference/03_inference.html)-- मुख्य सेवा फ्रेमवर्क के बीच आउटपुट और विलंबता की मापी तुलना।
  Hamel Husain延迟优化:TGI vs vLLM vs CTtranslate2 vs mlc 跨主要服务框架的吞吐和延迟测量对比──
