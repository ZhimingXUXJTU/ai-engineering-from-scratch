# मूल्यांकन और परीक्षण LLM आवेदनों का मूल्यांकन और परीक्षण
...
> **【中文解读】**ऑनलाइन वेब अनुप्रयोगों पर परीक्षा नहीं होगी, लेकिन अधिकांश टीमों को लगता है कि ऑनलाइन एमएलएम अनुप्रयोगों पर " 10 आउटपुट गलत" हैं। यह मूल्यांकन नहीं है, यह उम्मीद है।

> **【拓展：LLM评估→AI工程质量】**एलएलएम के अनुप्रयोगों की अनिश्चितता पारंपरिक सॉफ्टवेयर से परे है। स्वचालित मूल्यांकन (अभिलेखित सटीकता, प्रासंगिकता, सुरक्षा, वापसी परीक्षण) एआई इंजीनियरिंग के लिए एक महत्वपूर्ण कारक है।

>  **【前置】**学本节前请先掌握:(1) चरण 11·01(प्राम्प्ट इंजीनियरिंग)、चरण 11·09(फंक्शन कॉलिंग);(2) Pytest या unittest 基础评估集本质是测试用例;(3) CI/CD 概念(GitHub Actions、GitLab CI) 』会用 `pytest``langfuse`या `promptfoo`

**Type:** Build | **类型:** 构建
**Languages:** Python | **语言:** Python
**Prerequisites:** Phase 11 Lesson 01 (Prompt Engineering), Lesson 09 (Function Calling) | **前置知识:** Phase 11 · 01 (提示工程)、09 (函数调用)
**Time:** ~45 minutes | **时间:** ~45 分钟
**Related:**चरण 5 · 27 (LLM मूल्यांकन  RAGAS, DeepEval, G-Eval) ढांचे-स्तर की अवधारणाओं (एनएलआई आधारित निष्ठा, न्यायाधीश माप, RAG चार) को कवर करता है। चरण 5 · 28 (लंबे संदर्भ मूल्यांकन) संदर्भ-लंबे regression के लिए NIAH / RULER / LongBench / MRCR को कवर करता है। यह पाठ इस पर केंद्रित है कि क्या है LLM इंजीनियरिंग-विशिष्ट हैः CI / CD एकीकरण, लागत-गेटेड मूल्यांकन रन, regression डैशबोर्ड।**相关:**चरण 5 · 27 (LLM 评估RAGAS、DeepEval、G-Eval) ढांचे के वर्ग अवधारणाओं को कवर करती है।

## सीखने के लक्ष्य

- अपने LLM आवेदन के लिए विशिष्ट इनपुट-आउटपुट जोड़े, रुब्रिक्स और किनारे मामलों के साथ एक मूल्यांकन डेटासेट बनाएं
  आदान-प्रदान, इनपुट, आउटपुट, मूल्यांकन मानकों और LLM अनुप्रयोगों के लिए सीमांत उपयोग के उदाहरणों सहित मूल्यांकन डेटासेट का निर्माण
- LLM-as-judge, regex matching और deterministic affirmation checks का उपयोग करके स्वचालित स्कोरिंग लागू करें
  स्वचालित मूल्यांकन, न्यायकर्ता के रूप में LLM का उपयोग करना समानता और निश्चितता के निष्कर्षों की जांच
- प्रमाणीकरण, मॉडल या मापदंडों में परिवर्तन होने पर गुणवत्ता गिरावट का पता लगाने के लिए रेग्रिशन परीक्षण स्थापित करें
  建立归归测试, में提示、模型或参数变更时检测质量下降
- डिजाइन मूल्यांकन मीट्रिक जो आपके उपयोग के मामले के लिए क्या मायने रखता है (सटीकता, स्वर, प्रारूप अनुपालन, विलंबता) को कैप्चर करते हैं
  设计评测指标, उदाहरणों को पकड़ने के लिए

> **【中文解读】**इस कोर्स का उद्देश्य: एमएलएम के लिए 评测体系 का निर्माण करना 不只是评估模型本身, बल्कि संपूर्ण प्रणाली ((prompt + 模型 + RAG + 工具) के प्रदर्शन का मूल्यांकन करना है।

>  **【类比】** मूल्यांकन LLM  आवेदन जैसे करने के लिए एथलीटों को शारीरिक जांच नहीं कर सकते केवल "आज के परिणाम" देखने के लिए, एक समूह संकेतकों के रुझानों को देखने के लिए (( गति, शक्ति, धीरज, हृदय दर) ⇒ LLM 系统 भीः एक एकल संकेतकों को मूल्यांकन करना ((जैसे सटीकता दर) पर्याप्त नहीं है, एक समूह को मापना होगा (( सटीकता दर + पूर्णता + सुरक्षा + देरी + 成本), प्रत्येक परिवर्तन के लिए त्वरित रफ्तार पूर्ण मात्रा मूल्यांकन, प्रवृत्ति के मुकाबले।

> ️ **【易错点】**LLM-as-judge के 3 个坑:(1) **位置偏见**आध्याय  प्राथमिकता पहला या अंतिम उत्तर;修复:随机化答案顺序,跑两次取平均──(2) **冗长偏见**आध्यायकर्ता 偏好长答案 (बहुत कम है) 修复:**自吹偏见** G-4 评判 G-4 的输遇过度宽容;修复: 用更强模型(GPT-5 评判 Claude 输出) या不同家族模型(Claude 评判 GPT 输出)
...
> आप ग्राहक के लिए एक RAG Chatboter बनाया है। प्रदर्शन अच्छा है। आप इसे प्रकाशित किया है। दो सप्ताह बाद, किसी ने कल्पना को कम करने के लिए सिस्टम टिप्स को बदल दिया है।

11 दिनों तक किसी ने ध्यान नहीं दिया, सेल्फ सर्विस चैनल से राजस्व गिर गया, समर्थन टिकटों में तेजी आई।

> 11 लोगों ने ध्यान दिया कि स्व सहायता सेवा के माध्यम से आय में कमी आई है।
...
> यह भावनात्मक मूल्यांकन का मानकीकृत परिणाम है। LLM का आउटपुट आकस्मिक है। 5 परीक्षण उपयोग मामलों में प्रभावी सुझाव 6 में विफल हो सकता है।
...
> 修复方法"更小心" नही है। 修复方法 स्वचालित मूल्यांकन है प्रत्येक परिवर्तन के दौरान संचालन,对照评分标准评分,计算置信区间,质量回归时阻止部署
...
> **【中文解读】**LLM 工程 में मूल्यांकन और मॉडल प्रशिक्षण मूल्यांकन भिन्न है आपको पूरी प्रणाली के प्रदर्शन का मूल्यांकन करने की आवश्यकता है, न केवल मॉडल स्वयं को।

> **【拓展：LLM 应用的评测框架】**RAGAS 框架专门评测 RAG 系统(निष्ठा, प्रासंगिकता, संदर्भ सटीकता) ――LLM-as-Judge प्रयोग强模型(जैसे GPT-4) आकलन कमजोर मॉडल आउटपुट。LangSmith 和 LangFuse 提供追踪和评测平台──生产系统中通常需要建立黄金数据集一套标注好的答案对用于归归测试──
...
> LLM  मूल्यांकन में तीन प्रकार के होते हैं।
...
> **自动化指标**एल्गोरिथ्म से आउटपुट पाठ और संदर्भ उत्तर तुलना करें。BLEU  मापने n-ग्राम 重叠(मूलतः मशीन अनुवाद डिजाइन के लिए)。ROUGE  मापने संदर्भ n-ग्राम की कॉल-आउट दर(मूलतः सारांश डिजाइन के लिए)。BERTScore का उपयोग करके BERT  में सम्मिलित करें  मापने की भाषाई समानताएँ。 ये जल्दी और सस्ते कुछ सेकंड में 1 हजार आउटपुट का मूल्यांकन कर सकते हैं。 लेकिन वे छोटे अंतरों को अनदेखा करते हैं。 दो उत्तर शून्य शब्द भारी हो सकते हैं सही。 एक उत्तर ROUGE उच्च लेकिन उपरोक्त में नीचे में पूरी तरह से गलत है。
...
> **LLM-as-judge**प्रयोग बल मॉडल ((GPT-5、Claude Opus 4.7、Gemini 3 Pro) द्वारा रेटिंग मानक के अनुसार आउटपुट देने के लिए।$8，Claude Opus 4.7 约 $25) लेकिन मानव न्याय से संबंधित 82-88% ((उत्कृष्ट डिजाइन मूल्यांकन मानक)
...
> **人工评估**यह स्वर्ण मानक है, लेकिन सबसे धीमी सबसे महंगी है।
...
> यह आपके 90% समय का उपयोग करने वाली मूल्यांकन विधि है। यह विधि बहुत सरल हैः इनपुट, आउटपुट, चयन योग्य संदर्भ उत्तर और मूल्यांकन मानक प्रदान करें।
...
**正确性**(1-5): सूचना क्या तथ्य सत्य है?
...
**有用性**(1-5): क्या उपयोगकर्ता उपयोगी पाएगा?1 分无价值──5 分用户可立即根据此行动──
...
- **5**: इसका उत्तर तथ्यात्मक रूप से सही है, सीधे प्रश्न को संबोधित करता है, विशिष्ट विवरण या उदाहरण शामिल करता है, और कार्रवाई योग्य जानकारी प्रदान करता है।
  **5**: उत्तर तथ्य सही  सीधे प्रश्न का उत्तर  विशिष्ट विवरण या उदाहरण  प्रदान करने योग्य जानकारी
- **4**: इसका उत्तर तथ्यात्मक रूप से सही है और प्रश्न को संबोधित करता है लेकिन इसमें विशिष्ट विवरण का अभाव है या थोड़ा शब्दहीन है।
  **4**उत्तर: तथ्य सही, प्रश्न का उत्तर दिया गया, लेकिन विशिष्ट विवरण या थोड़ा लंबा नहीं है।
- **3**: उत्तर ज्यादातर सही है लेकिन इसमें मामूली गलतियाँ हैं या प्रश्न का उद्देश्य आंशिक रूप से गायब है।
  **3**उत्तरः उत्तर लगभग सही है लेकिन इसमें छोटी-छोटी गलतियाँ या भाग से विचलित प्रश्न शामिल हैं।
- **2**: उत्तर में महत्वपूर्ण तथ्यात्मक त्रुटियां हैं या केवल प्रश्न से संक्षेप में संबंधित हैं।
  **2**उत्तर में भारी-भरकम गलतियाँ या केवल संक्षेप में संबंधित प्रश्न हैं।
- **1**: इसका उत्तर तथ्यगत रूप से गलत, विषय से बाहर या हानिकारक है।
  **1**उत्तर: तथ्य गलत, प्रश्न या हानि
...
> **成对比较**यह एक विकल्प है: दो आउटपुट देखें, पूछें कि कौन बेहतर है। यह मानक परिशुद्धता के मुद्दे को समाप्त करता है।
...
> **Best-of-N**प्रत्येक इनपुट उत्पन्न करने के लिए N 个 आउटपुट, 评判挑最好―― यह आपके सिस्टम की ऊपरी सीमा को मापता है―― यदि 5 में से सर्वश्रेष्ठ लगातार सर्वश्रेष्ठ से अधिक जीतता है, तो कई बार पुनः चयन संभव है।

### ईवल पाइपलाइन

प्रत्येक मूल्यांकन एक ही 6-चरण पाइपलाइन का अनुसरण करता है।

> प्रत्येक मूल्यांकन एक ही 6 चरणों के साथ किया जाता है।
...
**提示**: परिभाषित परीक्षण उपयोग उदाहरणों में से प्रत्येक उपयोग उदाहरण में एक प्रविष्टि है।
...
**运行**: मॉडल निष्पादन提示──收集输出──若想测方差, प्रत्येक उपयोग उदाहरण रन 1-3 बार──

**Collect**: इनपुट, आउटपुट और मेटाडेटा (मॉडल, तापमान, टाइमस्टैम्प, शीघ्र संस्करण) को स्टोर करें।
**收集**: भंडारण इनपुट, आउटपुट और युन डेटा (आधार)

**Score**: अपने मूल्यांकन विधि लागू करें -- स्वचालित माप, LLM-जैसे-जजज, या दोनों।
**评分**: आवेदन मूल्यांकन विधि स्वचालन सूचक、LLM-जैसे-जिज या दो
...
**比较**: के साथ तुलनात्मक संख्याएँ। के साथ तुलनात्मक संख्याएँ।

**Decide**: यदि नया संस्करण सांख्यिकीय रूप से बेहतर (या बदतर नहीं) है, तो इसे भेजें। यदि यह पीछे हटे, तो ब्लॉक करें।
**决定**यदि नई संस्करण की संख्या काफी बेहतर है, तो इसे रोकें।
...
>  मूल्यांकन डेटासेट अच्छे या बुरे में से उपयोग के उदाहरणों पर निर्भर करता है।
...
**Golden 测试集**(50-100 उपयोग उदाहरण): चयन करें इनपुट आउटपुट के लिए, प्रतिनिधित्व कोर उपयोग उदाहरण।
...
**对抗样本**(20-50 उपयोग उदाहरण): प्रणाली के इनपुट को नष्ट करने के लिए डिज़ाइन किया गया।
...
**分布样本**(100-200 उपयोग उदाहरण): वास्तविक उत्पादन प्रवाह से यादृच्छिक रूप से लिया गया है।
...
> यदि 50 प्रयोग उदाहरण के तहत मूल्यांकन 90%,95% 置信区间是 [78%, 97%]──यह 19 个点的范围──आप 80% के सिस्टम और 96% के सिस्टम में अंतर नहीं कर सकते──

200 मामलों में 90% सटीकता के साथ, विश्वास अंतराल को घीराकर [85%, 94%] कर दिया गया है। अब आप निर्णय ले सकते हैं।

> 200 उदाहरण 90% 准确率 के नीचे, विश्वास区间收紧到 [85%, 94%]── अब आप निर्णय ले सकते हैं──
...
> प्रत्येक पीआर रन 200 उदाहरण मूल्यांकन सेट के साथ जीपीटी-5-मिनी प्रत्येक बार के बारे में$4。若团队每周合并 10 个 PR，就是 $160/月── प्रति比让用户满意度崩方 11 天的回归成本──
...
**凭感觉评估。**"मैंने 5 आउटपुट देखे, गलत देखा। "आपको नहीं पता कि 5% का गुणात्मक रिटर्न क्या है।
...
**在训练例上测试。**यदि उपयोग के उदाहरणों का मूल्यांकन सुझाव या सूक्ष्म डेटा में उदाहरणों के साथ होता है, तो आप स्मृति का मूल्यांकन करते हैं, न कि सामान्यीकरण।
...
**单一指标执念。**केवल सही और उपयोगी को अनदेखा करके, सरल, तकनीकी रूप से सटीक, लेकिन बेकार उत्तर प्राप्त होते हैं।
...
**无基线评估。**4.2/5 分孤立看无意义――昨天比较好还是差?竞标提示比较好还是差?始终比较――
...
**用弱评判。**GPT-3.5 करना है, जो कि ध्वनि उत्पन्न करते हैं, असंगत की संख्या। GPT-4o या क्लाउड सोनेट के साथ।

### वास्तविक उपकरण

आपको सब कुछ खरोंच से बनाने की ज़रूरत नहीं है। ये उपकरण मूल्यांकन बुनियादी ढांचा प्रदान करते हैंः

> यह आवश्यक नहीं है कि सब कुछ शून्य से निर्माण से हो।
...
> इस वर्ग में आपको हर स्तर को समझने में मदद मिलेगी।
...
> इस प्रकार के मूल्यांकन मॉडल के अनुसार मूल्यांकन मानक के अनुसार आउटपुट में भाग लेने के लिए उपलब्ध है।
...
> 統計ात्मक कठोरता वास्तविक मूल्यांकन और संवेदनाओं में अंतर को स्पष्ट करती है।
...
> शीघ्रपूर्ति शून्य से मूल्यांकन प्रवाह के लिए सबसे तेज़ मार्ग है।
...
> डीपईवल और पिटेस्ट 集成──运行 `deepeval test run test_evals.py`मूल्यांकन को परीक्षण सूट के भाग के रूप में निष्पादित करना--- इसमें 14 अंतर्निहित संकेतकों, जिसमें भ्रम परीक्षण, पूर्वाग्रह और विषाक्तता शामिल हैं।
...
> प्रत्येक में शामिल सुझाव या LLM कोड के PR पर प्रतिक्रिया मूल्यांकन. किसी भी मानक पर लौटने से अधिक मूल्य पर प्रतिक्रिया को रोकने के लिए।
...
> 本课产 出 `outputs/prompt-eval-designer.md` डिजाइन मूल्यांकन मानक का दोहराया जा सकता है सुझाव ढाँचा. इसे अपने LLM  अनुप्रयोग विवरण दें, यह अनुकूलित मूल्यांकन मानक का उत्पादन करता है।
...
> और उत्पादन`outputs/skill-eval-patterns.md`उपयोग के उदाहरणों, बजट और गुणवत्ता आवश्यकताओं के आधार पर उपयुक्त मूल्यांकन रणनीति के निर्णय ढांचे का चयन करना
...
1. **Add BERTScore.**शब्द एम्बेडिंग कॉसिन समानता का उपयोग करके एक सरलीकृत BERTScore लागू करें। यादृच्छिक 50-आयामी वेक्टरों पर मैप किए गए 100 सामान्य शब्दों का एक शब्दकोश बनाएं। संदर्भ और परिकल्पना टोकन के बीच जोड़ी के साथ कॉसिन समानता मैट्रिक्स की गणना करें। सटीकता, याद और F1 की गणना करने के लिए लालची मिलान का उपयोग करें (प्रत्येक परिकल्पना टोकन अपने सबसे समान संदर्भ टोकन से मेल खाता है) ।
   **加 BERTScore。**उपयोग शब्द में सम्मिलित अतिरिक्त स्ट्रिंग समानता को प्राप्त करने के लिए सरल संस्करण BERTScore── बनाना 100 个常用词映射到随机50 维向量的字典──计算参考和假设符号 之间成对余弦相似度矩阵──使用贪心匹配(每个假设符号 匹配最相似参考符号)计算精度、回忆 和 F1──
...
2. **Build pairwise comparison.**एक ही इनपुट और दो आउटपुट को देखते हुए, न्यायाधीश को यह बदलना चाहिए कि कौन सा आउटपुट बेहतर है और क्यों। अपने परीक्षण सूट के माध्यम से बेसलाइन-v1 बनाम बेसलाइन-v2 के साथ जोड़ी तुलना करें और आत्मविश्वास अंतराल के साथ जीत दर की गणना करें।
   **构建成对比较。**परिवर्तन मूल्यांकन दो मॉडल आउटपुट की तुलना में इसे एक साथ निकालने के लिए करते हैं न कि एक ही अंक को।
...
3. **Implement stratified analysis.**समूह परीक्षण मामले श्रेणी (वास्तविक, तकनीकी, सुरक्षा, कोडिंग, सारांश) द्वारा और विश्वसनीयता अंतराल के साथ प्रत्येक श्रेणी के स्कोर की गणना करें। पहचानें कि कौन सी श्रेणियां सुधार और कौन सी त्वरित संस्करणों के बीच पीछे हटी हैं। एक प्रणाली एक विशिष्ट श्रेणी पर पीछे हटी हुई समग्र सुधार कर सकती है।
   **实现分层分析。**按类别 (facts, techniques, security, programming, abstract) 分组测试用例,计算每类分数配置信区间间.
...
4. **Add inter-rater reliability.**प्रत्येक परीक्षण मामले पर LLM न्यायाधीश को 3 बार चलाएं (अलग न्यायाधीश "रेटर" सिमुलेशन) तीन रन के बीच कोहेन के कप्पा या क्रिपेंडॉर्फ के अल्फा की गणना करें। यदि सहमति 0.7 से कम है, तो आपका रूब्रिक बहुत अस्पष्ट है - इसे फिर से लिखें।
   **加评分者间信度。**प्रत्येक परीक्षण प्रयोग उदाहरण रन LLM 评判 3 次(模拟不同评判"评分者") ⋅ गणना三次运行间的 कोहेन कप्पा या क्रिपेंडॉर्फ अल्फा──若一致性低于0.7,评分标准太模糊重写──
...
5. **Build a cost tracker.**प्रत्येक न्यायाधीश कॉल के टोकन उपयोग और लागत का ट्रैक करें। न्यायाधीश को प्रत्येक इनपुट में मूल प्रॉम्प्ट, मॉडल आउटपुट और रूबिक (~ 500 टोकन इनपुट, ~ 100 टोकन आउटपुट) शामिल हैं। अपने परीक्षण सूट में कुल मूल्यांकन लागत की गणना करें और प्रति सप्ताह 10 मूल्यांकन रन का अनुमान लगाकर मासिक लागत का अनुमान लगाएं।
   **构建成本追踪。**追踪 प्रत्येक मूल्यांकन调用 टोकन उपयोग और लागत── प्रत्येक मूल्यांकन输入含原始提示、模型输出和评分标准(लगभग 500 टोकन 输入,लगभग 100 टोकन 输出) ──计算测试套件总评估成本,按每周10次评估运行推算月度成本──
...
- [Zheng et al., 2023 -- "Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena"](https://arxiv.org/abs/2306.05685)-- अन्य LLM का न्याय करने के लिए LLM का उपयोग करने पर आधारभूत पत्र, MT-Bench और जोड़ी तुलना प्रोटोकॉल की शुरूआत
  Zheng 等 2023 LLM 评判 अन्य LLM के आधारभूत सिद्धांत, MT-Bench 和成对比协议 की शुरूआत
- [promptfoo Documentation](https://promptfoo.dev/docs/intro)-- यॅमएल कॉन्फ़िग, 15+ प्रदाता, एलएलएम-जैसे-जजज और आईसी एकीकरण के साथ सबसे व्यावहारिक ओपन सोर्स मूल्यांकन ढांचा
  शीघ्रपू 文档 सबसे व्यावहारिक ओपन सोर्स मूल्यांकन ढांचा, जिसमें YAML 配置、15+  प्रदाता、LLM-as-judge、CI 集成 शामिल हैं
- [DeepEval Documentation](https://docs.confident-ai.com)-- 14+ मेट्रिक्स के साथ पायथन-नेटिव मूल्यांकन ढांचे, पायटेस्ट एकीकरण, और भ्रम का पता लगाने
  डीपईवल 文档पायथन 原生评估框架,14+ 指标、Pytest 集成、幻觉检测
- [Braintrust Eval Guide](https://www.braintrust.dev/docs)-- प्रयोगों का पालन, स्कोरिंग फ़ंक्शंस और डेटासेट प्रबंधन के साथ उत्पादन मूल्यांकन मंच
  ब्रेनट्रस्ट  मूल्यांकन  उत्पादन मूल्यांकन मंच, जिसमें प्रयोगों का पालन, मूल्यांकन फ़ंक्शन और डेटासेट प्रबंधन शामिल है
- [Ribeiro et al., 2020 -- "Beyond Accuracy: Behavioral Testing of NLP Models with CheckList"](https://arxiv.org/abs/2005.04118)-- LLM मूल्यांकन के लिए लागू व्यवस्थित व्यवहार परीक्षण पद्धति (न्यूनतम कार्यक्षमता, अपरिवर्तनीयता, दिशात्मक अपेक्षाएं)
  रिबेरो 等 2020 व्यवस्थित व्यवहार परीक्षण विधि ((कम से कम कार्यक्षमता、 अपरिवर्तकता、 दिशात्मक अपेक्षा), LLM  मूल्यांकन के लिए उपयुक्त
- [LMSYS Chatbot Arena](https://chat.lmsys.org)-- लाइव मानव मूल्यांकन मंच जहां उपयोगकर्ता मॉडल आउटपुट पर वोट करते हैं, LLM के लिए सबसे बड़ा जोड़ी तुलना डेटा सेट
  LMSYS चैटबॉट एरेना  वास्तविक समय कृत्रिम मूल्यांकन मंच, उपयोगकर्ता मॉडल पर आउटपुट मतदान, सबसे बड़ी LLM 成对比较数据集
- [Es et al., "RAGAS: Automated Evaluation of Retrieval Augmented Generation" (EACL 2024 demo)](https://arxiv.org/abs/2309.15217)-- आरएजी के लिए संदर्भ मुक्त माप (निष्ठा, उत्तर प्रासंगिकता, संदर्भ सटीकता/हला); मूल्यांकन पैटर्न जो लेबलर के बिना प्रोड करने के लिए स्केल करता है।
  इस प्रकार "RAGAS" (EACL 2024 डेमो) RAG का निर्दिष्टीकरण  निष्ठा  उत्तर संबंधी  ऊपर नीचे सटीकता/पुनर्विचार);
- [Liu et al., "G-Eval: NLG Evaluation using GPT-4 with Better Human Alignment" (EMNLP 2023)](https://arxiv.org/abs/2303.16634)-- विचार श्रृंखला + एक न्यायाधीश प्रोटोकॉल के रूप में फॉर्म भरना; माप और पूर्वाग्रह परिणाम हर न्यायाधीश-निर्माता की जरूरत है।
  लियू 等 "जी-ईवल" (EMNLP 2023) 思维链 + 表单填写作为评判协议; प्रत्येक评判 निर्माताओं के लिए आवश्यक校准和偏差结果──
- [Hugging Face LLM Evaluation Guidebook](https://huggingface.co/spaces/OpenEvals/evaluation-guidebook)-- ओपन एलएलएम लीडरबोर्ड को बनाए रखने वाली टीम द्वारा डेटा प्रदूषण, मीट्रिक चयन और पुनरुत्पादनशीलता पर व्यावहारिक सलाह दी जाती है।
  Hugging Face LLM  मूल्यांकन पुस्तिका 维护 ओपन LLM Leaderboard  टीम डेटा प्रदूषण, पहचान विकल्प और व्यवहार्यता के बारे में व्यावहारिक सुझावों
- [EleutherAI lm-evaluation-harness](https://github.com/EleutherAI/lm-evaluation-harness)-- स्वचालित बेंचमार्क (एमएमएलयू, हेलास्वाग, ट्रुथफुलक्यूए, बिग-बेंच) के लिए मानक ढांचा; ओपन एलएलएम लीडरबोर्ड के पीछे इंजन।
  एलेथेरएआई आईएम-मूल्यांकन-हर्नेस स्वचालन基准(एमएमएलयू、हेलास्वाग、सत्यQA、बिग-बेंच) के मानक ढांचे;ओपन एलएलएम लीडरबोर्ड 背后的引擎──
