# स्केलिंगः वितरित प्रशिक्षण, FSDP, डीप स्पीड
...
> **【中文解读】**1.24 अरब मॉडल एकल जीपीयू पर प्रशिक्षण देते हैं। लेकिन 70 अरब पैरामीटर मॉडल में कोई अंतर नहीं है।

> **【拓展：DeepSeek-V3的2048卡训练】**डीपसेक-वी3 का उपयोग 2048 张 H800 GPU  प्रशिक्षण, डुअल पाइप 流水线并行 + एमओई 专家并行── समझ वितरण प्रशिक्षण का उपयोग करने के लिए अग्रिम में बड़े मॉडल को कैसे प्रशिक्षित किया जाता है, इसका आधार है।

>  **【前置】**学本节前कृपया पहले हावी हो जाओःचरण 10·04(प्रारंभिक प्रशिक्षण मिनी जीपीटी)单 GPU 训练基础;PyTorch DDP 概念;多 GPU 通信(NCCL、AllReduce)。本节是चरण 10·19(DualPipe) औरचरण 10·20(DeepSeek-V3 Walkthrough) के前置──
...
>  **【类比】**वितरण प्रशिक्षण = कई लोग सहयोग बड़े बॉक्स को स्थानांतरित करना**数据并行**(DP) = 4 个人各搬一个相同的小箱子(同模型不同数据, परिणाम AllReduce 平均) **张量并行**(TP) = 4 个人一起抬起一个大箱子的四角**流水线并行**(PP) = 4 个人流水线,第 1 人搬一层传给第二 人(模型按层切分)**FSDP**= डीपी + 把模型切片分到各卡,用时再聚聚合 (省显存) ;; उत्पादन场景一般 3种组合用 (3D समानांतर) ;;

> ️ **【易错点】**3 个坑 分布式训练的:**batch size 设错**单卡 bs=8,4 卡应该 bs=32(4×8)而非 bs=8; प्रभावी बैच आकार के साथ 计算 lr――(2) **AllReduce 瓶颈**卡间通信比 GPU 计算慢10倍,bs 太小会让 GPU等通信;bs 至少32+才划算──(3) **没设 seed** प्रत्येक समय में प्रारम्भिकता भिन्न होती है, परिणाम अपरिवर्तनीय होते हैं;`torch.manual_seed(42) + torch.cuda.manual_seed_all(42)`

## सीखने के लक्ष्य

- समानांतर के तीन प्रकार (डेटा, टेंसर, पाइपलाइन) और मॉडल और क्लस्टर आकार के आधार पर प्रत्येक आवश्यक होने पर समझाएं
  解释三种并行类型(数据并行、张量并行、流水线并行) और इसके अनुकूल परिदृश्य
- कई GPU के बीच ग्रेडिएंट सिंक्रनाइज़ेशन के साथ PyTorch DDP का उपयोग करके डेटा-समान प्रशिक्षण लागू करें
  PyTorch DDP का उपयोग कई GPU डेटा को प्राप्त करने के लिए
- न्यूनतम हार्डवेयर निर्धारित करने के लिए एक दिए गए मॉडल आकार (वजन + अनुकूलक राज्यों + ग्रेडिएंट + सक्रियण) के लिए मेमोरी बजट की गणना करें
  计算给定模型大小的显存预算(权重 + 优化器状态 + 梯度 + 激活值), न्यूनतम हार्डवेयर आवश्यकता निर्धारित करें
- जीपीयू पर मॉडल राज्यों को टुकड़े करने के लिए FSDP या डीप स्पीड ZeRO चरणों को कॉन्फ़िगर करें और एकल-जीपीयू मेमोरी से अधिक फिट मॉडल
  फ़ॉरफ़ॉर्मिंग FSDP या डीपस्पीड ZeRO चरण को एक टुकड़ा मॉडल की स्थिति में विभाजित करने के लिए, ताकि एकल-कार्ड प्रदर्शित मॉडल से अधिक प्रशिक्षित किया जा सके

> **【中文解读】**इस कोर्स में LLM प्रशिक्षण के मूल बोतल पर ध्यान केंद्रित किया गयाः प्रकट भंडारण。7B  मॉडल में FP16 नीचे केवल वजन पर 14GB की आवश्यकता है, इसके अतिरिक्त एडम  अनुकूलक स्थिति(28GB) और ग्रेड(14GB), कुल 56GB अभी तक सक्रियण मूल्य नहीं है。आप तीन प्रकार की समक्रमण रणनीति सीखेंगे( डाटा并行、张量并行、流水线并行) तथा FSDP/ZeRO 分片技术来突破单卡显存限制──
...
> 7B 参数模型 में FP16 नीचे केवल वजन पर 14GB की आवश्यकता होती है।

एनवीडीए ए100 में 80 जीबी मेमोरी है।

> NVIDIA A100 80GB के साथ स्पष्ट भंडारण
...
> 80GB में 56GB का उपभोग किया गया है। शेष 24GB सक्रियण मूल्य के लिए उपयोग किए जाते हैं। इसके लिए 2048 टोकन क्रम और 4096 维 मॉडल के लिए, एक-स्तरीय सक्रियण मूल्य लगभग 64MB है। 32 स्तरों को प्रति नमूना 2GB की आवश्यकता है।
...
> Llama 3 405B में 16,384 张 NVIDIA H100 GPU पर प्रशिक्षण, गणना लागत अनुमानित लगभग 1 बिलियन डॉलर है।
...
> इस कक्षा में बड़े पैमाने पर प्रशिक्षण के लिए चार रणनीतियाँ शामिल हैंः डेटा并行、张量并行、流水线并行和完全分片数据并行── आप शुद्ध पायथन का उपयोग करके प्रत्येक को अनुकरण करेंगे, संपर्क वितरित प्रशिक्षण ढांचे से पहले इसकी तंत्र को समझेंगे──
...
> निम्नलिखित वास्तविक मॉडल के स्पष्ट रूप से जीवित गणितीय गणना हैं। प्रत्येक संख्या का अनुमान नहीं, बल्कि अनुमान है।
...
> "आदम  स्टेट"列是致命的──आदम प्रत्येक घटक भंडारण के लिए एक परिचालन औसत मूल्य ((m) और एक परिचालन अंतर ((v), सभी FP32──70B 模型 के लिए, अर्थात् 70B x 4 字节 x 2 = 560GB── केवल अनुकूलक पर ही सात张 A100──
...
> 单张H100有80GB──Llama 3 405B至少需要61张H100来放置权重、优化器和梯度──加上激活值数字更大──Meta 使用16384张GPU不是因为他们想而是因为他们必须──

> **【中文解读】**显存预算是LLM 训练的第一道关卡──以 Llama 3 70B उदाहरण:FP16 权重 140GB + एडम 优化器状态 560GB + 梯度 140GB = 840GB(不含激活值)。单张H100 只有80GB,至少需要11张GPU才能放下这些状态──Llama 3 405B 总需求高达4,860GB──Adam 优化器是显存杀手它为每个参数存储两个 FP32 的动量估计节目m 和 v),参数 x 8 字 x 2──

> **【拓展：Llama 3 的 16384 GPU 训练】**Llama 3 405B में 16,384 张 H100 GPU पर प्रशिक्षण, 3D और साथ में डेटा कोर्सेस + 张量并行 + 流水线并行) ⋅ प्रशिक्षण की कुल लागत अनुमानित लगभग 1 बिलियन डॉलर है।
...
> सबसे सरल वितरण रणनीति── पूरे मॉडल को N 张 GPU में कॉपी करना── प्रत्येक प्रशिक्षण को N 等份 में विभाजित करना── प्रत्येक GPU को अपने डेटा विभाजन के लिए आगे और पीछे प्रसारित करना── सभी GPU के बीच औसत तरंगों में प्रसारित होने के बाद── प्रत्येक GPU को उसी औसत तरंगों के साथ अपने वजन के प्रति अद्यतन करना, सभी प्रतिओं को एक साथ रखना──
...
> **优点：**吞吐量线性扩展──N 张 GPU प्रत्येक चरण में N 倍 के डेटा का प्रसंस्करण──通信 केवल औसत स्तर तक सीमित है, जो गणना के साथ ओवरलैप हो सकता है──
...
> **缺点：**प्रत्येक GPU  के पास मॉडल  के अनुकूलक स्थिति और स्तर की पूर्ण प्रतिलिपि  के लिए 70B  मॉडल, प्रत्येक GPU  की आवश्यकता 840GB  के लिए डेटा  के लिए  के भंडारण  को कम नहीं करता  यह केवल प्रशिक्षण समय  को कम करता है
...
> **数学：**有效批量大小 = प्रतिGPU批量 x N。 N=64 张 GPU के लिए, प्रतिGPU 批量 16,有效批量 1,024 ∙ Llama 3 उपयोग प्रति चरण 16000000 टोकन के प्रभावी批量。
...
> एक बार एक वर्ग गुणांक को एक वर्ग GPU में विभाजित किया जाता है, प्रत्येक गणना भाग परिणामों में से एक है।
...
> 考虑前层中形为 (8192, 8192) के权重矩阵── 4 路张量并行, प्रत्येक GPU 持有 (8192, 2048) के分片── प्रत्येक GPU को अपने分片 के गुणा में इनपुट किया जाएगा, कुछ परिणामों का उत्पादन किया जाएगा── कुछ परिणामों के माध्यम से पूर्ण归约 या पूर्ण संग्रह संयोजन के लिए पूर्ण आउटपुट──
...
> **优点：**减模型权重的每 GPU 显存──70B 模型分为8张 GPU
...
> **缺点：**प्रत्येक स्तर के बाद तेजी से GPU 间通信 ⋅ प्रत्येक रेंज के बाद कुल归约 बढ़ी देरी ⋅ यह NVLink में है ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅ ⋅  ⋅ ⋅                                                                                                                                         
...
> **实际使用：**मेगाट्रॉन-एलएम ने पहले चेंज मात्रा के साथ-साथ चलना प्रस्तावित किया।

> **【中文解读】**张量并行 (टेंसर समानांतरवाद) एकल स्तर के矩阵乘法 को कई张 GPU ऊपर में विभाजित करेगा। उदाहरण के लिए, एक (8192, 8192) के वजन矩阵 में 4 路并行 के नीचे, प्रत्येक GPU को केवल भंडारण की आवश्यकता है (8192, 2048) । लेकिन कमी यह है कि प्रत्येक स्तर को पूर्ण रूप से कम करने की आवश्यकता है।

> **【拓展：流水线并行的气泡问题】**流水线并行将模型层次分为不同 GPU (जैसे GPU1 跑 1-8层, GPU2 跑 9-16层) 但会产生"气泡"GPU空等候──GPipe 通过微批次(微批次) 重叠计算来缓解:4 个阶段使用16 个微批次,气泡率降至 (4-1)/16 = 18.75%──
...
> **优点：**GPU 间通信最小 केवल स्तर सीमा के सक्रियण मूल्य, तरंग या भार के साथ तुलना में बहुत छोटा, क्योंकि बैंडविड्थ की मांग कम है।
...
> **缺点：**流水线气泡── जब GPU 4 में गणना की जाती है 1 के पहले के प्रसारण के समय, GPU 1、2、3 空(ये पहले से ही अपना हिस्सा पूरा कर चुके हैं)──反向传播时模式相反──朴素流水线下,N 个阶段 के GPU उपयोग दर केवल 1/N──
...
> **GPipe 和 PipeDream**通过将批次分为微批次来解决气泡问题──GPU 1 一完成微批次 1 的前向传播就开始微批次 2──这在流水线阶段间重叠计算──M 个微批次和N 个阶段,气泡比例降至 (N-1) /M──M=16 个微批次和N=4 个阶段,气泡为3/16 = 18.75% 空时间──
...
> FSDP डेटा समानांतरों की विस्तारशीलता और विभाजन की स्पष्ट भंडारण दक्षता को जोड़ता है। प्रत्येक GPU में पूर्ण मॉडल प्रतिलिपि नहीं है, बल्कि केवल 1/N के पैरामीटर, डिग्री और अनुकूलक की स्थिति है।
...
> एक परत के पूर्ववर्ती प्रसार से पहले, FSDP ने सभी GPU से पूर्ण पैरामीटर एकत्र करने से लेकर प्रत्येक GPU के स्पष्ट भंडारण तक पूरे संग्रह संचालन को चलाया। पूर्ववर्ती प्रसार के बाद, प्रत्येक GPU ने गैर-स्थानिक पैरामीटर को त्याग दिया।
...
> 没有FSDP,70B 模型不能放入单张80GB GPU──使用FSDP在8张 GPU上,每张 GPU用105GB等等等,还是放不下──你需要至少16张 GPU 才能降至每卡80GB以下,或将FSDP与激活检查点结合(反向传播时重新计算激活值而不是存储它们) ⋅
...
> 通信 लागत सामान्य डेटा संचयन से अधिक है, क्योंकि प्रत्येक स्तर से पहले पूर्ण संग्रह की आवश्यकता होती है।

> **【中文解读】**FSDP( पूर्ण विभाजन डेटा एकीकरण) डेटा एकीकरण और विभाजन के संयोजन है। प्रत्येक GPU केवल 1/N के तत्वों, तरंगों और अनुकूलक की स्थिति को संग्रहीत करता है।

> **【拓展：DeepSpeed ZeRO 的三个阶段】**ZeRO स्टेज 1 分片优化器状态(省 4x 显存),स्टेज 2 加上梯度分片(省 8x),स्टेज 3 加上参数分片(省 N 倍,N 为 GPU 数) ・FSDP 本质上是 ZeRO स्टेज 3 के PyTorch 原生实现──Llama 3 405B के 16,384 GPU 训练使用FSDP + 张量并行 + 流水线并行 3D组合──
...
> डीपस्पीड का ZeRO (零冗余优化器) अवधारणा में FSDP के समान है, लेकिन माइक्रोसॉफ्ट द्वारा स्वतंत्र रूप से विकसित किया गया है। यह तीन चरणों को परिभाषित करता है, प्रत्येक चरण अधिक सक्रिय है।
...
> ZeRO-3 FSDP के समान है। इसका नाम अलग है, तंत्र एक ही है।
...
> डीपस्पीड ने भी ZeRO-Offload को पेश किया है। यह CPU में अपलोड करने के लिए बेहतर होगा।
...
> **【拓展：3D 并行与 MoE 的经济性】**लामा 3 405B प्रयोग 3D并行:节点间数据并行 + 节点内 8 路张量并行 + 节点流水线并行。DeepSeek-V3 则使用 MoE(混合专家) वास्तुकला लागत कम प्रति वारंवार अग्रिम प्रसारण केवल सक्रिय 37B 参数(总参数 671B), प्रशिक्षण लागत केवल लगभग 560 मिलियन डॉलर, है लामा 3 के 1/18।
...
> इस तरह के 3D विभाजन(8 x 16 x 128 = 16,384) है आप कैसे विस्तार करने के लिए हजारों GPUs के तरीके। प्रत्येक GPU देखें अलग डेटा विभाजन (Data并行) ✓ प्रत्येक स्तर का एक टुकड़ा है (张量并行) ✓ गणना विभिन्न समूह के स्तर (流水线并行) ✓
...
> डीपसीक V3 ने अलग-अलग तरीके अपनाए हैं। उनके मिश्रित विशेषज्ञ वास्तुकला प्रत्येक टोकन केवल 671B 参数 में से 37B को सक्रिय करता है। इसका मतलब है कि प्रत्येक GPU को केवल गणना की आवश्यकता है।
...
> प्रत्येक GPU को अपने विभाजन पर गणना करने के लिए आगे बढ़ेगा।
...
> कुल परिग्रहण संचालन ({{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:{{lang-en:en:{{lang-en:en:{{lang-en:en:{{lang-en:en:{{lang-en:en:{{lang-en:en:{{lang-en:en:{{lang-en:en:en:{{lang-en:en:{{lang-en:en:{{lang-en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:en:
...
> 误差应恰好为零(或机器epsilon) ⋅张量并行在数学上是精确的它产生与在一张GPU上计算完整矩阵乘法相同的结果──分离沿输出维度进行,每张GPU 产生不同的列块,拼接重建完整结果──
...
> 对于列并行线性层(分分输出维度),你拼接──对于行并行(分输入维度),你求和──在变压器 FFN में,第一个线性(扩展) 列并行,第二个线性(收缩) 列并行──这避免了两层之间的全归约──
...
> मॉडल के स्तर को वर्चुअल जीपीयू में विभाजित करें। प्रारंभिक चरण के बाद के चरण के बाद के चरण के बाद के समय के बारे में बताएं।
...
> 4 चरणों और 1 लघु बैचों में, ग्लिप बुलबुला अनुपात 75% है।
...
> यह कैलकुलेटर प्रत्येक एमएल  इंजीनियर प्रश्न का उत्तर देता हैः "मुझे कितने चेंग GPU की आवश्यकता है?
...
> अधिकांश लोगों के लिए सबसे बड़ी अप्रत्याशितताः मिश्रण सटीकता में आधे से अधिक की कमी नहीं होगी।
