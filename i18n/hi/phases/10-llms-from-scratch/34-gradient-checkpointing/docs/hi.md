# ग्रेडिएंट चेकपॉइंटिंग और सक्रियण पुनः गणना .
...
> **【中文解读】**प्रतिवर्ती प्रसार को सभी मध्य सक्रियणों को सहेजने की आवश्यकता है──70B 参数 + 128K 上下文 = प्रति卡 3TB  सक्रियण──梯度检查点使用计算换显存:丢弃部分 सक्रियण,需要时重新计算──关键问题是丢弃哪些答案不是"全部"──

> **【拓展：梯度检查点→大模型训练】**梯度检查点是训练大模型的标标技术──PyTorch के टॉर्च.उपयोग.चेकपॉइंट、DeepSpeed के सक्रियण चेकपॉइंटिंग सभी इस विचार की प्राप्ति हैं──128K 上下文训练中, छवियुक्त检查点 60%+ की बचत कर सकता है।

>  **【前置】**学本节前 कृपया पहले सीखेःचरण 10·04(प्रारंभिक प्रशिक्षण GPT);反向传播算法;GPU 显存层次(SRAM vs HBM) ⋅本节是训练超大模型 + 长上下文时的关键技术──
>  **【类比】**梯度检查点 = 出差只带必要文件──反向传播 = 工作完成后回顾所有材料;不检查点 = 出差路上所有的材料都背着(占满行李箱);全检查点 = 只带护照,需要时重印(重算成本高);选择性检查点 = 只扔可重算的(如草稿),保留难重建的(如合同) 平衡存储和速度显著──
...
>  प्रशिक्षण ट्रांसफार्मर 时, प्रत्येक स्तर को प्रतिवर्ती प्रसार में प्रत्येक क्षुद्र परिचालन के इनपुट के भंडारण की आवश्यकता होती हैः ध्यान इनपुट  Q/K/V 投影、softmax 输出、FFN 输入、归化输出和残差流──`d`、序列长度 `L`、 मात्रा `B`                                                                                                                                                                                                                                                              `12 * B * L * d`个浮点数──
...
>  के लिए `d=8192, L=8192, B=1`,BF16 नीचे प्रति परत 800 MB──64 स्तर मॉडल का सक्रियण मूल्य 51 GB है जो भी छोटे बैच आकार में गुणा नहीं किया गया है, भी नहीं बढ़ाया ध्यान softmax मध्य मूल्य( प्रति टाई`L^2`), जो कि एक ही लाइन के भाग में शामिल नहीं है।
...
> 双面账单:BF16 权重加优化器状态可能放入80GB, लेकिन सक्रियण मूल्य आपको अधिक बनाता है।
...
> **【中文解读】**梯度检查点(Gradient Checkpointing) का उपयोग करके गणना में परिवर्तन की बचतः पूर्व दिशा में प्रसारित होने पर मध्य सक्रियण मूल्य नहीं बचाया जाता है, विपरीत दिशा में प्रसारित होने पर पुनः गणना की आवश्यकता की सक्रियण।

> **【拓展：梯度检查点在训练中的关键作用】**梯度检查点是训练大模型的标标技术──Llama 3 70B के लिए, कोई जांच बिंदु नहीं है, प्रत्येक नमूना के लिए लगभग 16GB  सक्रियण मूल्य का उपयोग करना आवश्यक है, प्रारंभ के बाद लगभग 5GB तक कम हो जाता है──FSDP और मिश्रित सटीकता के संयोजन में,梯度检查点 सीमित GPU 显存内 प्रशिक्षण बड़े मॉडल को संभव बनाता है──
...
> **【拓展：梯度检查点与 FSDP 的配合】**梯度检查点通常FSDP/ZeRO के साथ 配合 उपयोग ⋅FSDP 分片参数和优化器状态,梯度检查点减少活值显存,两者互补 ⋅FSDP + 梯度检查点 + 混合精度使8xA100 में ऊपर प्रशिक्षण संभव हो गया ⋅
...
> 本课产 出 `outputs/prompt-activation-recompute-policy.md` एक स्वीकार मॉडल विन्यास (~ लेयर संख्या,  छिपा आयाम, 序列 लम्बाई, 批次) और उपलब्ध GPU 内存并输出 प्रत्येक लेयर重计算策略 (~ बिना / 选择性 / 完全 / 卸载) का शीघ्र 
...
1. सही होने की पुष्टि करें।`model_forward`+ `model_backward`(पूर्ण सक्रियण) vs `model_forward_checkpointed`+ `model_backward_checkpointed`पैरामीटर ग्रेडिएंट मशीन सटीकता के समान होना चाहिए।
   中文翻译:验证正确性──运行 `model_forward`+ `model_backward`(完整激活)`model_forward_checkpointed`+ `model_backward_checkpointed`(分段) ️ तत्वों की तरंग यंत्र की सटीकता से मेल खाती है️

2. पोंछ खंड आकार `k`1 से `L`. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
   中文翻译:将段大小 `k`1 से ् ा तक ् `L`                                                                                                                                                                                                                                                              
...
3. चयनात्मक चेकपोइंटिंग लागू करेंः ध्यान मॉड्यूल इनपुट को स्टोर करें लेकिन इसके मध्यवर्ती नहीं। 32 परत मॉडल के लिए FLOP ओवरहेड बनाम पूर्ण-परत चेकपोइंटिंग को seq=8192 पर मापें।
   中文翻译:实现选择性检查点: भंडारण ध्यान मॉड块输入但不存中间结果──测量 32 层模型在 seq=8192 下与全层检查点的 FLOP 开销比较──
...
4. अपलोड जोड़ें. अनुकरण "सीपीयू बफर" (अलग सूची) में खंड इनपुट सहेजें। "पीसीआई बैंडविड्थ" को बाइट्स/समय के रूप में मापें और अपलोड और पुनः गणना के बीच ब्रेक-ईक्वेंस बिंदु ढूंढें।
   中文翻译:添加卸载──将段输入保存到模拟的"CPU 缓冲区" (CPU 缓冲区) 单独的列表) 以字节/时间测量"PCIe 带宽"并找到卸载和重计算之间的平衡点──
...
5. एक वास्तविक PyTorch ट्रांसफार्मर के साथ और बिना बेंचमार्क `torch.utils.checkpoint`. स्मृति का मापन (via `torch.cuda.max_memory_allocated`) और कदम समय।
   中文翻译:对真实 PyTorch ट्रांसफार्मर 分别使用和不使用 `torch.utils.checkpoint`进行基准测试──测量内存(通过 `torch.cuda.max_memory_allocated`) और步长时间──
