...
> **【中文解读】**التدريب على الـ LLM 推理分两个阶段:预填充(并行处理快速,计算密集) 和码(逐代币 生成,显存带宽密集) KV-cache、连续批处理、投机解码都是针对这两个阶段的优化──

> **【拓展：推理优化→生产部署】**تمت إدارة الاحتياطي الكهربائي المتخزين والمتسلسلات والمتسلسلات المتسلسلة والمتسلسلطة والتي تسمح بتحسين هذه التقنيات.

>  **【前置】**学本节前请先掌握:Phase 10·01-08(تحول 架构、Attention);GPU 计算 vs 显存带宽概念。本节是Phase 17(بنية التحتية والإنتاج)

>  **【类比】**ماجستير في التدريس = 读书 + 写读书笔记。**Prefill**(处理 prompt) = 快速翻完书(并行扫所有代币,计算密集);**Decode**(生成) = 逐字写笔记 ((一次写一字,显存带宽密集)**KV cache**= كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة كتابة**continuous batching**= 同时给多人读书会服务(动态拼组)
...
- تنفيذ KV-Cache للقضاء على الحسابات الزائدة خلال توليد الرمز التدريجي
  实现 KV 缓存 لتحقيق التخلص من إعادة التأمين الذاتي
- شرح مراحل التعبئة المسبقة مقابل عملية فك الكود في استنتاج ماجستير في العلوم العليا ولماذا كل منها له عوارض زجاجة مختلفة (مرتبطة بالحوسبة مقابل المرتبطة بالذاكرة)
  解释 LLM 推理的预填充与解码阶段及其不同瓶(计算密集vs 访存密集)
- تنفيذ مفاهيم الإعداد المستمر ومصطلحات PagedAttention لتحقيق أقصى استغلال لـ GPU تحت الطلبات المتزايدة
  实现连续批处理和 PagedAttention 概念, 在并发请求下最大化GPU利用率
- مقارنة تقنيات تحسين الاستنتاجات (KV-cache، فك التفكيرات، الاهتمام الفلاشي) وتداولاتها في التكامل / التأخير
  تقارن مع تقييم تحسين التقنية ((KV 缓存、投机解码、Flash Attention) ومقدار الانتفاع/التأخير

> **【中文解读】**本课焦 LLM 推理优化──两个核心概念:(1) Prefill 阶段并行处理提示, محدود إلى GPU 计算量(FLOPS);(2) Decode 阶段 per token 生成, محدود إلى显存带宽──KV-cache 避免重复计算已生成的代币的K/V,连续批处理最大化 GPU 使用率,PagedAttention 解决 KV-cache 的存片片问题显──
...
> تمت استخدام 4xA100 GPU على تعيين Llama 3 70B── مستخدم واحد يحصل على حوالي 50 رمزا/ ثانية── تشعر أنه يسير── ثم 100 مستخدم في نفس الوقت يزور نقطة نهاية── انخفض التناثر إلى 3 رمزا/ ثانية── تمتلك 25000 دولار شهريًا في حسابات GPU ‬
...
> النموذج نفسه لا يتغير بين مستخدم واحد و 100 مستخدم. نفس الوزن ، نفس البنية ، نفس الرياضيات. التغير هو كيفية ضبط العمل. التفكير البسيط يضيع 90% + من GPU المتاحة في الحساب. ينتظر المستخدم 47 رمزا يحتل المجموعة الكاملة من المراكز ، بينما GPU يظهر بين المراكز المتعددة. في الوقت نفسه ، يمكن استخدام طلب 2,000 رمزا للمستخدم الجديد لتحليل مفيد لملء ذلك المرحلة المميتة.
...
> هذه ليست مشكلة توسيع، بل مشكلة تحديث.$25k 推理账单降为服务相同流量的 $5K 账单的关键──
...
> vLLM في 4xA100-80GB 上 خدمة Llama 3 70B، انخفاض في التوزيع عند الوصول إلى حوالي 50 رموز/ ثانية/ مستخدم، من خلال سلسلة التوزيع والتمويل المخصص للخدمات في 100 ووضع في طلب تحت الحفاظ على 15-25 TPS/ مستخدم.

## المفهوم الأساسي

> **【中文解读】**المرحلتين من تقرير LLM هناك بطاقتين مختلفة بشكل واضح: المرحلة السابقة محدودة بـ GPU FLOPS (حجم الحساب) ، المرحلة القصدية محدودة بـ显存带宽 (المتخزنة) ؛ كل إنتاج يجب أن يقرأ كل النموذج (الوزن) ؛ الكشوف الكهربائي هو أساسي تحسين (المتخزن) من رمز الخصم المولد (المناسبة) ، لتجنب إعادة الحساب ، سوف تنخفض تعقيد المرحلة من O (أ) n^2) إلى O (أ) n (أ) ✿

> **【拓展：vLLM 和 PagedAttention】**في الوقت نفسه ، يتم استخدام الجهاز التجاري لتحقيق المعلومات المختلفة في مجال الجهاز التجاري. في الوقت نفسه ، يتم استخدام الجهاز التجاري لتحقيق المعلومات المختلفة.
...
> كل طلب لتقديم ماجستير في إدارة الأعمال له مرحلتين مختلفتين
...
> **预填充**处理整个输入提示──所有代币 已知,所以注意力可以在完整序列上并行计算──这是大型矩阵乘法GPU 核心保持忙碌──瓶是计算:硬件每秒能提供多少 FLOPS──A100做 312 TFLOPS(BF16)──70B 模型上 4,096-代币提示的预填充在单张 A100 上约需要400ms──
...
> **解码**个别生成输出代币――每个新代币 关注所有前代币,但每次前向传播只产生一个代币――权力矩阵与预充时大小相同,但你使用单向而不是矩阵与相乘――GPU 核心在微秒内完成,然后等下一批权重从内存到内存带宽:你能以多快的速度将模型权重从HBM传输到计算单元――A100 有2TB/s 带宽――FP16 的70B 模型是140GB――读完整模型需要70ms这是单次解码步骤的下限.
...
> 核心洞察:* إن فك الشفرة كثيفة الذاكرة، لأنك تقرأ النموذج بأكمله لإنتاج رمز واحد*― كل تحسين من أدناه إما تقليل حجم القراءة، أو زيادة كل مرة تقرأ معالجة رمز 批次، أو تجنب القراءة تماماً―
...
> في حساب الاهتمام، كل طلب من الرمز يتركز على جميع الرمز السابقين والقيمة والحجم. لا يوجد تخزين عند، إنتاج الرمز N 需要重新计算 جميع N-1 个前 الرمز الرمزية والقيمة الإلقاء. إنتاج الرمز 2 时投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投投
...
> KV 缓存存储所有前代币的键和值投投投影──生成代币 N 时,你只计算代币 N 的键和值,然后与缓存的代币 1到N-1 的 K/V 拼拼音──
...
> إضافة إلى ذلك ، فإن إضافة إضافية إلى إضافة إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافة إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافة إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافة إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافة إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافة إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافة إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية إلى إضافية إضافية.
...
> 静态批处理等一批 N 个请求到达,一起处理,等*所有*完成后再接受新请求──如果请求需要500 个代币 另一个需要10 个,短请求在完成后置490 个解码步骤──
...
> 连续批处理(也叫代级批处理) في أي طلب يتم الانتهاء فوراً سيتم إدخال طلب جديد في المجموعة بعد ذلك.
...
>  ارتفاع التشغيل يعتمد على مقدار تغير طول المخرج                                                                                                                                                                                                                                                       
...
> كل طلب من KV 缓存 هو قطعة من سلسلة الاحتفاظ بالانخراط والانخراط في التخزينات. كل طلب من KV 缓存 هو قطعة من سلسلة الاحتفاظ بالانخراط والانخراط. كل طلب من KV 缓存 هو قطعة من سلسلة الاحتفاظ بالانخراط والانخراط. كل طلب من طلبات KV 缓存 هو قطعة من سلسلة الاحتفاظ بالانخراط والانخراط. كل طلب من طلبات KV 缓存 هو قطعة من سلسلة الاحتفاظ بالانخراط والانخراط. كل طلب من طلبات KV 缓存 هو قطعة من سلسلة الاحتفاظ بالانخراط والانخراط. كل طلب من طلبات KV 缓存 هو قطعة من سلسلة الاحتفاظ بالانخراط والانخراط.
...
> الصفحة الاهتمام ((( من vLLM) سوف يتم تطبيق النظام التشغيلي风格 الافتراضي内存 ل KV 缓存。 لا لتوزيع كل طلب بلاك تسلسل، ولكن توزيع ثابتة الحجم " الصفحة "( عادة في كل صفحة 16 رمز)。 الصفحة يمكن أن تكون في أي موقع في الجي بي يو في الاحتفاظ الفيزيائي‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬
...
> 解码慢是因为它是序列的生成一个代币,反回去,生成下一个...... ولكن إذا كنت تستطيع تخمين رخيص التالي 5 رموز, ثم مرة واحدة التحقق من كل هذا؟
...
> 投机解码 استخدام صغير و سريع**草稿模型**生成 ك 个候选标志──大型**目标模型**ثم في التوزيع الواحد المقبل يُعامل جميع المرشحين ك 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个
...
> 加速取决于**接受率**توقعات نموذج المخططات多频繁地匹配目标──Llama 3 8B 为 Llama 3 70B عند صنع المخططات، عادة ما تكون معدل قبول اللغة الطبيعية 70-85%── هذا يتحول إلى تسريع التفريغ 2-3 مرات──
...
> 投机解码 هو *المنطقة المحددة *المنطقة المحددة *المنطقة المحددة من النتائج هي نفسها تماماً من النتائج التي تمتلكها النموذج المستهدف.
...
> السابق 缓存存储常见前的 KV 缓存并在请求间复用──当新请求带有已知前到达时,系统复制(或引用) KV 条目缓存,只计算唯一后的 KV──
...
> لجميع الطلبات المشتركة من 2,000 رمز نظام على الفور، قامت الحفظة السابقة بإزالة كل طلب من 400ms من التميز المسبق. في 100 طلب/ ثانية، تموّل كل ثانية 40 ثانية من عمل GPU.
...
> 本课产出:
> - `outputs/skill-inference-optimization.md` التشخيص والتحسين للمهارات في مجال القانون  تقديم الخدمات
...
1. قم بتعديل ملف تعريف الاحتفاظ بالخزنة KV للمقارنة بين FP16 vs FP8 vs INT4 KV كوانتيزة الاحتفاظ بالخزنة. لـ Llama 3 70B في سياق 4K ، احسب أقصى مستخدمين متزامنين لكل واحد على 4xA100-80GB. يجب أن يكون تعدد الكوانتيزة KV إلى INT4 حوالي 4x قدرة المستخدم.
   中文翻译:修改 KV 缓存分析器比较 FP16 مقابل FP8 مقابل INT4 KV 缓存量化──对于4K 上下文的Llama 3 70B,计算 4xA100-80GB 上每种格式最大并发用户数──INT4 KV 量化应大约 4倍用户容量──
...
2. توسيع محاكاة الحزم المستمرة لتتبع استخدام GPU (جزء من فتحات الحزم المملئة لكل خطوة). استخدام الحزم مع مرور الوقت لكل من الحزم الثابتة والمتواصلة مع 50 طلبًا تتبع طول الخروج توزيع باريتو (الشكل = 1.5 ، المقياس = 20) يجب أن يحافظ الحزم المستمر على الاستخدام > 80٪.
   中文翻译:扩展连续批处理模拟器跟踪GPU利用率(每步批次槽位填充比例) ・・・绘制静态和连续批处理随时间的利用率图,50 个请求的输出长度遵循帕累托分布(形=1.5 ,规模=20) ・・・连续批处理应维持 >80%利用率──
...
3. تنفيذ نسخة من الجهاز التخزيني للقاعدة المشتركة`num_kv_heads < num_query_heads`. Llama 3 70B يستخدم 64 رأس استفسار ولكن 8 رأس KV فقط. احسب مدخرات الذاكرة مقابل الاهتمام الكامل متعدد الرؤوس (8 ضعف الحد من حجم ذاكرة الاحتفاظ بالكيف).
   中文翻译:实现分组查询注意力(GQA) version of KV 缓存, من بينها `num_kv_heads < num_query_heads`◊Llama 3 70B استخدام 64 查询头但只有 8 KV 头──计算相比完整多头注意力的内存节省(KV 缓存大小减少8倍)。
...
4. قم ببناء مخزن مخزني يستخدم إزالة LRU. حدد ماكس_ادراجات إلى 500 وتوليد 1000 طلب حيث يشارك 60% واحدة من 5 مخزنيات شائعة. قياس معدل ضربات ومقارنة مع مخزن مخزني غير محدود. مع الإزالة الجيدة، يجب أن يبقى معدل ضربات فوق 55%.
   中文翻译:构建使用 LRU 淘汰的前缓存──设置 max_entries 为 500,生成 1,000 个请求,其中 60% 共享 5 个常见前之一──测量命中率并与无限缓存比较──良好淘汰下命中率应保持在 55%以上──
...
5. توسيع محاكاة تشخيص المضاربة لتنفيذ المضاربة القائمة على الشجرة (نمط EAGLE-2). بدلاً من سلسلة واحدة من رموز مسودة K ، قم بتوليد شجرة من المرشحين (على سبيل المثال ، فروع 2 في كل من مستويات 3 = مرشحات ورقة 8) مقارنة إجمالي رموز القبول لكل جولة التحقق مقابل المضاربة السطحية.
   中文翻译:扩展投机解码模拟器实现树状投机(EAGLE-2风格) ・・・ ليس K 个草稿代币的单链,而是生成候选树(例如 3层每层 2 分支 = 8个叶选) ・・・比较每轮验证接受代币 总数与线性投机──
