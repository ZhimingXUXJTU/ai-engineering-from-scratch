...
> **【中文解读】**RLHF 需要训练三个模型(SFT、奖励模型、策略), أيضاً للتعامل مع عدم استقرار PPO──DPO 直接在偏好对上优化语言模型不需要奖励模型,不需要 PPO,一个训练循环,效果相当──

> **【拓展：DPO→简化对齐】**DPO هو برنامج بديل لـ RLHF الذي طرحه ستانفورد في عام 2023 ، وقد أصبح أول نموذج مفتوح للعديد من النماذج مثل Zephyr、Tulu) على المدى المحدد.

>  **【前置】**学本节前请先掌握:Phase 10·07(RLHF) 理解RLHF 流程和它的问题(3 个模型 + PPO 不稳定);PyTorch 监督学习基础──DPO هو "RLHF بدون RL" رياضي优雅,理解为什么有效需要看原文推导──
...
>  **【类比】**دبو مقابل رلهف = 直接教育 vs 训练动物。RLHF:先训一个" معلم"(RM) أعط الطلاب打分,再使用 PPO 让学生讨老师欢心3 个模型 + 复杂训练。DPO: مباشرة给学生看"好答案"和"坏答案"对照,让它自己学1 个模型 + 简单交叉──数学 DPO 证明"做分类在偏好数据上做分类"等于"最优解的RLHF",省去了显而易的 RM──

> ️ **【易错点】**دبي 3 个坑: ((1) **偏好数据质量决定一切**DPO 不像RLHF 有 RM平滑噪声,标注错的偏好对直接学错;务必做标注质量控制──(2) **β 系数设错**太大(> 0.5)模型不变,太小(< 0.05)模型偏离 SFT 太远;典型 0.1──(3) **没做 reference model**خسارة DPO 需要相对 SFT 模型的日志-prob 差分,忘记加载ref 模型 会训练崩;用 `AutoModelForCausalLM.from_pretrained(sft_path)`إشارة
...
- استنباط وظيفة خسارة DPO وتفسير كيف أنها تمثل ضمنيا نموذج مكافأة من خلال احتمالات سجل السياسة
  推导 DPO 损失函数, شرح كيفية تمثيل نموذج مكافأة من خلال استراتيجية
- مقارنة DPO مقابل RLHF من حيث استقرار التدريب وتكلفة الحساب وعدد النماذج المطلوبة
  مقارنة DPO مع RLHF من حيث استقرار التدريب ‬التكلفة الحسابية وعدد النماذج المطلوبة
- ضبط المعيار بيتا للتحكم في مدى الانفصال من السياسة المدربة من نموذج المرجع
  调节 بيتا 参数, التحكم في استراتيجيات التدريب عن النموذج المرجعي

> **【中文解读】**هذا هو المفهوم الذي يستخدم في تطوير DPO (باشر التحضيريات المباشرة) ، والبدائل البسيطة لـ RLHF.
...
> أنت في الدورة السابعة بنيت خطة RLHF 管线── ثلاث مراحل── ثلاث نماذج──SFT 模型、 ماذج مكافأة 模型 ومودل استراتيجية لتحسين PPO  فقط ماذج مكافأة تحتاج إلى آلاف من الفردات المفضلة على و دورة تدريبية منفصلة──PPO  تحتاج إلى تحديد دقيق KL في عدد، معدل التعلم٬ نسبة القطع والعصر عدد──
...
> في الممارسة العملية، تدريبات الـPPO غير مستقرة تسمية. تغيرات صغيرة في العناصر الفائقة يمكن أن تؤدي إلى انتشار التدريب. النموذج المكافئ هو وكيل غير كامل لتحيز البشر.
...
> هذه التعقيدات هي السبب في صعوبة استخدام RLHF لسنوات بعد إصدار InstructGPT.
...
> 2023 مايو، رفائيل رافائيلوف، أرتشيت شرما وزملاء ستانفورد نشروا "تحسين التفضيلات المباشرة: نموذج لغتك هو بشكل سري نموذج مكافأة"。
...
> سوف يسهل DPO RLHF إلى خطوات تعلم منفردة للإشراف على الموديل.

> **【中文解读】**حل DPO ثلاثة نقاط صعوبة RLHF: 1) لا تحتاج إلى نموذج مكافأة تدريب فردي؛ 2) لا تحتاج إلى معالجة عدم الاستقرار والتحسينات الفائقة للشكل الفائق للشكل الفائق للشكل الفائق؛ 3) من ثلاث نموذجات التبنيات المبسطة لتبني دورة تدريبية واحدة.

> **【拓展：DPO 在开源社区的普及】**أصبح DPO 已成为开源模型对齐的首选方法──HuggingFace TRL 库原生支持 DPO,Zephyr、Tulu、OpenHermes 等知名开源模型都使用 DPO 替代 PPO──相比 RLHF,DPO التدريب تكلفة حوالي 1/3 من الأخير (((قل من نموذج مكافأة) ، وتدريب أكثر استقرار──Llama 3 的对齐训练结合 DPO 和其他方法──
...
> من بينها R هو نموذج المكافأة، pi هو استراتيجية، pi_ref هو نموذج المرجح، beta هو KL في العدد.

أظهرت ورقة DPO أن هذا الهدف لديه حلًا مثاليًا في شكل مغلق. بالنسبة لأي وظيفة مكافأة R ، فإن السياسة المثلى هي:

> يظهر مقال DPO أن هذا الهدف لديه أفضل حل في شكل مغلق. بالنسبة لأي وظيفة مكافأة R، أفضل استراتيجية هي:
...
> هذا هو التمرير. المكافأة تعتمد تماما على احتمالية نموذج الاستراتيجية ومعرفة احتمالية نموذج المرجح.

استبدال هذا في نموذج تفضيل برادلي تيري:

> أبدله في نموذج برادلي تيري
...
> Z(x) 项消去了, لأن الردتين هما نفس اللحظة x كشرط.
...
> **【中文解读】**يعد مركز وظيفة DPO 损失 () "تباين احتمالية النسب مع احتمالية النسب":beta * (log pi(y_w) /pi_ref(y_w) - log pi(y_l) /pi_ref(y_l))。 إنها تقيس نموذج استراتيجية مقارنة مع نموذج مرجع أفضل من المختار 回复、更不偏好 رفض 回复的程度。beta 控制策略偏离参考模型的程度(شبه KL 系数 في RLHF), نموذج 0.1-0.5。

> **【拓展：DPO 的数学直觉】**ثمة اختراق في DPO في إعلان وظيفة مكافأة "خفية" في RLHF للتعبير عن نموذج الاستراتيجية وموديل المرجح: R(x,y) = بيتا * log(pi(yyyx) / pi_refyyyx)) + const. هذا يعني عدم الحاجة إلى تدريب واضح نموذج مكافأة نموذج الاستراتيجية نفسه هو نموذج مكافأة.
...
> %`log pi(y|x) / pi_ref(y|x)`يُعَدُّ نسبة احتمالية العدد. عندما يكون هذا النسبة صحيحاً، فإن النموذج الحالي يعطي ردّاً و يُوزع احتمالية أعلى من المرجح.
...
> DPO  خسارة دفع النموذج زيادة احتمالية الرد على العدد الأول من الاختيارات، انخفاض احتمالية الرد على الرد على الرد على العدد من الاختيارات.
...
> يتطلب تدريب DPO اثنين من النماذج في الذاكرة النماذج الحالية والماذج المرجعية للخروج  RLHF  تحتاج إلى ثلاثة أو أربعة: استراتيجية  المرجعية  نموذج المكافأة، فضلا عن المفتاح القيمة المختار  بالنسبة إلى 70B  النموذج، كل نسخة في FP16  140GB‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬
...
> **小型数据集。**هناك 5,000-20,000 من المفضلة مقابل الوقت، وغالبا ما يتناسب أو يتجاوز نموذج مكافأة RLHF.
...
> **有限算力。**DPO 大约只需要完整 RLHF 三分之一的计算量(دورة تدريب وليس ثلاثة) .
...
> **快速迭代。**想尝试 10 مختلف محطات بيانات المفضلة انظر ما هو أفضل نموذج؟DPO 让你在几小时内运行每个实验.
...
> **大规模训练。**على نطاق GPT-4 أو Claude، يمكن لنموذج مكافأة مستقل لـ RLHF أن يلتقط إشارات تحفيز أكثر تفصيلا.
...
> **复杂奖励信号。**عندما يتعلق "أفضل" بعدة أبعاد (استخدامية، لا ضرر، ولا نزاهة) ، يمكن لنموذج المكافأة تعلم هذا الوزن المتعدد الأهداف.
...
> **迭代对齐。**يمكن أن تستخدم RLHF 管线 استراتيجية الحالية لتوليد ردود فعل جديدة، والقيام بتقييمات الإنسان، وإعادة تدريب نموذج مكافأة في الدورة على الإنترنت.

### خارج الـ DPO: KTO، ORPO، SimPO

ألهمت DPO عائلة من طرق التنظيم المبسطة.

> بدأ مكتب الإدارة الإدارية سلسلة من الأساليب المُبسطة للتأهيل
...
> **KTO（Kahneman-Tversky 优化，2024）：**أنت حتى لا تحتاج إلى الموازنة. استخدام KTO بدون الموازنة فقط يجب أن تُعَلِّم كل ردّ فعل على أنه "جيد" أو "سيئ"، لا حاجة إلى مقارنة مع البدائل. هذا يُبسط بشكل كبير جمع البيانات.
...
> **ORPO（赔率比偏好优化，2024）：**في خطوات التدريبية الوحيدة ، يتم دمج SFT و على وجه الخصوص. ORPO  تغيير SFT  الخسارة لتحتوي على إشارة التحيز ، بدلاً من القيام أولاً SFT و إعادة القيام DPO  الخسارة لديها اثنين: المعيار الأول للرد على التوقيت التالي  التوقيت الخسارة ، بالإضافة إلى زيادة الميزان المزايد لفرق احتمالية الرد على التوقيت والرفض.
...
> **SimPO（简单偏好优化，2024）：**完全 القضاء على النموذج المرجعي.‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬
...
> التوجه واضح جدا: كل طريقة لتحقيق القيادة عن التعقيدات. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
...
> **Zephyr-7B（HuggingFace，2023 年 10 月）：**تمتد على الـ Mistral 7B  البنيسة على الـ UltraChat ((200K  عينات) ، ثم تمتد على الـ DPO في الـ UltraFeedback ((60K  تفضيل على) ، تمتد على الـ MT-Bench 6.47 عندها أعلى نموذج 7B  بالمقارنة مع Llama 2 Chat 70B تمتد على 6.86، مما يعني أن Zephyr فقط باستخدام DPO على الـ ZZil وصل إلى مستوى 94% من نموذجها الكبير 10 مرات.
...
> **Llama 3（Meta，2024 年 4 月）：**في مرحلة RLHF الاولى بعد استخدام DPO. هذا المكون يظهر DPO و RLHF يمكن أن يضاف RLHF.
...
> **Neural Magic / nm-chat（2024）：**تطبيق DPO على العديد من نماذج مفتوحة، في إطار التوازن على المراجع، يظهر ارتفاع 5-15% من المراجع SFT النقي.
...
> 格式与 RLHF 相同(فوري، 首选، 被拒绝) 三元组──DPO 直接消费这些数据,无需中间的奖励模型──
...
> DPO 损失需要计算给定提示下回复的总对数概率──这意味着运行模型在完整的(快速 + 回复) 序列上,并对每回复代币的对数概率求和──
...
> هذه الوظيفة هي قوة الرئيسية لـ DPO. للنسبة لكل تحفيز على، فإنها تعمل أربع مرات: النموذج على رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد رد
...
> `preferred_ratio`和 `rejected_ratio`يُعطى النموذج الحالي احتمالية التوزيع الأولية للردود أعلى احتمالية عندما يتم رفض التوزيع أقل احتمالية للردود على النموذج المرجعي، وذلك لسبب الصواب والخسارة المنخفضة.
...
> `implicit_preferred_reward`和 `implicit_rejected_reward`ويمكنك استفادة من هذه المكافآت لتحقق من فعالية التدريب، والفرق بين المكافأة الأولى والمنفض يجب أن تزداد خلال عملية التدريب.
...
> مقارنة مع RLHF، دورة التدريب بسيطة جداً.

### الخطوة 5: مقارنة DPO مقابل RLHF

قياس هامشات المكافأة الضمنية وتحولات احتمالية السجل مقارنة DPO مع نموذج RLHF من الدروس 07.

> قياس الحدود المكافأة المحتملة والتحول المحتملة للعدد، سيتم مقارنة DPO مع نموذج RLHF في الدورة السابعة.
...
> الجوانب البيتا هي في DPO مساوية لعدد RLHF KL في الجوانب. يمكن أن يبعث نموذج التحكم عن النموذج المرجعي إلى حد ما.
...
> 小 beta(0.01) جعل النموذج حرّاً عن التأثير تعلمت بسرعة ولكن كان هناك حلّ متداخل.
...
> 本课产出 `outputs/prompt-alignment-method-selector.md` لمساعدتك على اختيار طريقة التزامن الصحيحة (SFT,RLHF,DPO,KTO,ORPO,SimPO) على سبيل المثال.
...
1. تنفيذ KTO (Kahneman-Tversky Optimization) KTO لا تحتاج إلى أزواج - فقط وضع علامة على كل رد "جيد" أو "سيئ". الخسارة لرد جيد هي`-log(sigmoid(beta * log_ratio))`وبالنسبة للرد السيء هو`-log(1 - sigmoid(beta * log_ratio))`مع مضاعف ردة الفعل الخسارة (عادة 1.5x) على خسارة الاستجابة السيئة. قم بتدريب نفس البيانات (اعالج أفضل باعتبارها "جيدة" ورفضها "سيئة" بشكل مستقل) وقارن الدقة مقابل DPO.
   中文翻译:实现 KTO(Kahneman-Tversky 优化) ・・・KTO 不需要配对只需将每回复标记为"好"或"坏"──好回复的损失是`-log(sigmoid(beta * log_ratio))`, سوء تعويض الخسارة هو`-log(1 - sigmoid(beta * log_ratio))`،并对坏回复损失应用损失厌恶乘数 ((عادة 1.5 倍) ⋅ في نفس البيانات التدريب ((( سيتم الاختيار الأول على أنه " جيد " ، يتم رفضه على أنه "坏 ") ،并与DPO比较准确率──
...
2. تنفيذ DPO المعتاد على الطول. بدلاً من احتمالات السجل الخام، قم بتقسيمها بعد رموز الاستجابة: `normalized_logprob = total_logprob / num_tokens`هذا يمنع النموذج من تفضيل الاستجابات القصيرة (التي لديها أعلى إجمالي السجلات). مقارنة هامش المكافأة الضمنية مع ولا دون التطبيع.
   中文翻译:实现长度归归一化 DPO──不使用原始对数概率,而是除以回复代币 数:`normalized_logprob = total_logprob / num_tokens` هذا يمنع تحويل النموذج نحو ردود الفعل الأقصر                                                                                                                                                                                                                                                     
...
3. قم ببناء خسارة مزودة على النمط ORPO. أضف خسارة التنبؤ القياسية للبرمجة التالية على الاستجابة المفضلة لخسارة DPO: `L = L_sft(preferred) + alpha * L_dpo`. حاول قيم الألفا من 0.1، 0.5، و 1.0. يجب أن تنتج الخسارة المشتركة نموذج يتبع كل منهما التعليمات (من مصطلح SFT) ويفضل الاستجابات الأفضل (من مصطلح DPO) ، مما يزيل الحاجة إلى مرحلة منفصلة SFT.
   中文翻译:构建 ORPO 风格的组合损失──在 DPO 损失上添加首选回复的标准 下一个标志 预测损失:`L = L_sft(preferred) + alpha * L_dpo` تجربة الفا 值 0.1 、0.5 和 1.0  تخسير المجموعة يجب أن يحدث既能遵循指令(من SFT 项) و أيضاً أفضل أفضل أفضل回复(من DPO 项) النموذج، لتحقيق الحد من الحاجة إلى SFT 阶段
...
4. تنفيذ DPO التكراري. تشغيل DPO لمدة 3 حقول، ثم إنشاء استجابات جديدة من النموذج المدرب، وتزاوجها مع الاستجابات المفضلة الأصلية كزوجات تفضيل جديدة، وتشغيل DPO مرة أخرى. جولتين من هذه العملية "اللاعب الذاتي". مقارنة دقة تفضيل بعد الجولة 1 والجولة 2 لمعرفة ما إذا كان التكرير التكراري يساعد.
   ترجمة باللغة الصينية: تنفيذ代 DPO── تنفيذ代 DPO 3  عصر، ثم من خلال نموذج بعد التدريب إنتاج جديدة، سوف تقوم بتطبيقها مع الأصل الأول الأول الأول الأول الأول الأول  مرة أخرى كـ تطبيق جديد، مرة أخرى تنفيذ  دوة دورة "自我对" عملية── مقارنة 1 轮 والمرحلة الثانية  بعد تطبيق تصميم تحديد، انظر إذا كان التطبيق مفيد‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬
