...
> **【中文解读】**70 مليار نموذج معين في FP16 需要 140GB 显存(两张A100)  تقييم إلى FP8 需一张80GB GPU, تقييم إلى INT4 يمكن تشغيلها على MacBook  تقييم هو باستخدام التكنولوجيا الأساسية للدقة في التغيير في الاحتفاظ والسرعة‬

> **【拓展：量化→llama.cpp/GGUF】**llama.cpp 和 GGUF 格式让大模型能在消费级硬件上运行──GPTQ、AWQ、GGUF 等量化方法是将70B+ 模型部署到本地设备的关键──理解量化是理解大模型部署的基础──

>  **【前置】**學本节前请先掌握:Phase 10·01-10(LLM 基础);浮点数表示(FP32/FP16/BF16/INT8/INT4);numpy 矩阵运算。

>  **【类比】**量化 = 压缩图片──原照片(FP16) في كل صورة 16 位,肉眼分辨不出和 8 位(FP8) في الفارق, ولكن文件大小一半──再压到4 位(INT4)
...
- تنفيذ التعريف الكمي التناغمي وغير التناغمي من FP16 إلى INT8 و INT4 بما في ذلك تحديد حجم كل تنسور وكل قناة
  تحقيق الكميات المرتبطة/غير المرتبطة من FP16 إلى INT8 و INT4 ، بما في ذلك الكميات المتعددة وتكثيف الممرات
- حساب مدخرات الذاكرة من الكميات وتحديد الدقة التي تناسب VRAM من GPU معينة
  الحساب الكميات الكبيرة من الحفاظ على الاحتفاظ، تحديد دقة معينة من الاحتفاظ على الجهاز المعمول به
- شرح الفرق بين تعريف الكم بعد التدريب (PTQ) وتدريب معرفة الكم (QAT)
  解释 تدريب بعد الكمية PTQ) و تقييم التعرف على التدريب QAT)
- تطبيق GPTQ أو AWQ لقياس النموذج الحقيقي وقياس التنازل بين الدقة والذاكرة على مقياس مقارنة
  طبيق GPTQ أو AWQ قياس النموذج الحقيقي، و على أساس قياس دقة-مظهر الوزن

> **【中文解读】**هذا الدور لتحقيق تقنية الكمياتية باستخدام الدقة لتغيير الظهور والسرعة──النهج الأساسي: كمياتية/غير كمياتية、 كمياتية/كمياتية تتراوح في كل اتجاه،PTQ(التدريب بعد الكمياتية) مقابل QAT(التدريب على الإدراك الكمياتية)──القياس هو طريق الإرسال القياسي لكل نموذج أكبر من 7B──
...
> لاما 3 70 ب لديه 700 مليار عنصر. كل عنصر هو 16 بيت浮点数. أي 1400 مليار عنصر.
...
> ولكن كل عنصر 16 بيعت ضائعة. معظم الوزن في شبكة العصبية تجمع في مكان قريب.
...
> قياس باستخدام أرقام دقة منخفضة استبدال أرقام دقة عالية. فب 16 إلى فب 8  خفض النمو الضوئي. فب 16 إلى إن تي 4  خفض إلى أربعة أجزاء. 140 جيجابايت  نموذج يصبح 35 جيجابايت. يمكن إدخالها في واحد 张消费级 جي بي يو.
...
> 代价是精度──每移除一个都破坏信息──问题在于你损失多少精度以及在哪里损失──模型INT4 量化良好在大多数基准上保留原质量的95-99%──简单INT4 量化可能完全摧毁模型──差异在技术──
...
> 社区 باستخدام GPTQ سوف يحدد Llama 3 إلى INT4, في WikiText 上 فقط فقدان حوالي 1-2 个困惑度点──Mistral 发布 Mixtral 8x22B FP8 检查点,MMLU 上几乎零质量损失──GGUF 格式驱动 llama.cpp,在MacBook M 系列芯片上运行 70B 模型──量化不是黑客──它是每个超过 7B 模型的标准部署路径──

> **【中文解读】**FP16 لكل عنصر 16 بكتة،70B  النموذج يحتاج 140GB  ولكن 95% من الوزن يركز بين -0.1 إلى +0.1  باستخدام 16 بكتة يشير إلى هذه القيم ضائعة جدا.  مقياس إلى INT4 سوف ينخفض الاحتياجات الحفظية إلى 35GB، يمكن أن تعمل على مستوى الاستهلاك على GPU  جيد INT4  الحفاظ على النموذج الأصلي 95-99% من الجودة  GPTQ  الحفاظ على الجودة اللاما 3 إلى INT4  فقدان 1-2 نقطة حيرة فقط، FP8 من Mistral  مقياس في MMLU  تقريبا صفر خسارة 

> **【拓展：量化生态】**量化生态已非常成熟:GPTQ(بناء على تقارب ثانوية المعلومات على بعد بعد بعد) 、AWQ(تحريك الحكم على الاعتبار، الحماية الوزن البارز) 、GGUF(llama.cpp                                                                                                                                                                                                                                
...
> كلّ نقطةٍ تُحْصَلُ على ثلاثة أجزاء: رمز، مؤشر، وتُطلق عليها أيضاً رقمًا فعالًا.
...
> **FP32**هو كامل الدقة. 23 位尾数给你约7 位十进制精度.
...
> **FP16**سوف يقلل الوزن إلى نصفها. سيتم تقليص الوزن إلى 5 بضع. سيتم تقليص الوزن إلى 5 بضع.
...
> **BF16**(Brain Float 16) الحفاظ على مؤشر FP32 من 8 نقاط ولكن سيتم تقليص عدد التوالي إلى 7 نقاط. المدى هو أقل من FP32 بالمثل، والحدد أقل من FP16. Google  خصيصا للتعلم العميق التصميم. 直觉: بالنسبة للشبكات العصبية المدى هو أكثر أهمية من الدقة.
...
> **FP8**هناك نوعان من المتغيرات. E4M3(4 مؤشر، 3 尾数) يستخدم في التفكير في وزن وتفعيل. E5M2(5 مؤشر، 2 尾数) يستخدم في التدريب في درجة، في الوقت الحالي نطاق أكثر أهمية من الدقة.
...
> **INT8**هو النمط الكامل. ليس لديه مؤشر. ليس لديه عدد. فقط 256 ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪ ٪
...
> عمليات النواة بسيطة جدا. خذ نقطة تحويل قيمة حجم، والعثور على العاملات المكثفة، ضربها، أربعة إلى خمسة إلى عدد كامل أقرب، ثم تخزين العاملات المكثفة.
...
> الاختلاف هو أربعة خمسة خُطاء. كل قيمة أقصى حد لها.`scale / 2` التخلفات الإجمالية في الطبقة تعتمد على كمية الوزن لديك ومدى حساسية النموذج للتأثيرات على هذه الوزن.
...
> **逐张量 vs 逐通道量化。**张量对整体权重矩阵使用缩小因子――简单但有损: إذا كان هناك قيمة كبيرة في صف آخر، فإن القيمة الصغيرة ستفقد معظم الدقة── 张量对整体权重矩阵的每行或每列) باستخدام عنصر缩小── 开销更大(储存N 个缩小因子而不是1 个) ولكن ال质量显著更好── 采用每个生产量化方法逐步通道或更大的细粒子──
...
> **非对称量化**添加零点偏移:`quantized = round(tensor / scale) + zero_point` هذا التعامل ليس على التوزيع مركزيا على الصفر. على سبيل المثال، ReLU  قيمة النشاط总是非负的. مقابل الكمياتية سوف تضيع نصف نطاق الكامل على قيمة سلبية غير ظهرة. غير المقارنة الكمياتية سوف تظهر النطاق الفعلي [min, max] إلى نطاق الكاملات.

### الهرم الحساس

ليس كل شيء في النموذج يتسامح مع التعريف الكمي على قدم المساواة. هناك Hierarchy واضحة.

> الموديل ليس من جميع الأجزاء نفس التسامح مع الكميات.
...
> **权重（最鲁棒）。**تغير الوزن النموذجي ببطء خلال التدريب، ويتبع تقريباً التوزيع القوي المتمركز على الصفر.
...
> **KV 缓存（高敏感度）。**键值缓存存储所有前代币的注意状态――在长上下文长度下,KV 缓存主导内存──70B 模型在 32K 上下文下,只KV 缓存就有40GB FP16──将KV 缓存量化到FP8或INT8 节省大量内存但任何差异会在所有后代注意计算中累积──质量影响随序列长度增加──
...
> **注意力 logits（最敏感）。**يُمكن أن يغير خطأ الكميات 0.01 في المنطق السابق للكميات 0.01 بشكل كبير توزيع الاهتمام. معظم برامج الكميات حتى إذا كان جميع الأجزاء الأخرى كمياتية، فإنها تحافظ على الاهتمام في الحساب بدقة أعلى.
...
> **量化感知训练（QAT）**في التدريب في التوزيع الاولى إدخال عمليات الكمياتية الزائفة. سوف يضع الموديل المجموعة الوزن على وضع أربع خمسة في خلفات صغيرة.
...
> **GPTQ**هو طريقة واحدة PTQ  إنها تدريبية لتقييم الوزن ، باستخدام مجموعة بيانات صغيرة من المقررات  عادة 128 عينة) قياس هيسين  عن الناتج على حساسية كل وزن  معلومات ثانوية  هيسين 指示重量重量更仔细化‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬
...
> **AWQ**观察到少部分权重 ((约1%) لأن مع معدل تنشيط كبير مضاعفة وخاصة جدا. AWQ استخدام تحديد البيانات لتحديد هذه الوزن الكبير و تقييم قبل زيادةها.  ثم تقليل لتحديد النشاط المرتبط) 
...
> **GGUF**هو صيغة الملفات التي تستخدم في llama.cpp وبيئتها. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
...
> كيف يمكنك أن تقرر أن نموذجك الكمي لا يزال مفيدًا؟
...
> **困惑度。**أهم المقاييس المستخدمة: 0.5-1.0 جيد، 1.0-2.0 معظم المهام مقبولة،> 2.0 شرح المشاكل.
...
> **任务特定基准。**في MMLU、HumanEval、GSM8K أو مجموعة تقييمات تعريفك الخاصة، تمارس النموذج الكمي مقارنة مع النموذج الأصلي، والقياس الكمي على تأثير المهارات المختلفة غير متساوي، والمهام الرياضية والرمزية أكثر حساسية لخسارة الدقة من المعرفة العامة.
...
> **输出比较。**من اثنين من النماذج في نفس اللحظة 上生成回复并比较.
...
> **延迟和吞吐量。**الهدف من وجود الكميات هو جعل النموذج أسرع وأكثر تكلفة. قياس كل ثانية رمز عدد، أول رمز  الوقت والذكرة الاستخدام.
...
> 模式:FP8 几乎免费──INT4 代价 1-2  MMLU نقاط ولكن الانتفاع مضاعف、 الحمية انخفضت إلى أربعة في المائة── هذا الوزن لجميع عمليات التنفيذ تقريبا يستحقها──

### الأرقام الحقيقية

FP16 إلى FP8 على H100: 30-50% تسريع الاستنتاج، < 0.1% فقدان الجودة. هذا هو الكمية بدون دماغ. كل نشر H100 يجب أن تستخدم.

> H100 فوق FP16 إلى FP8:30-50% 推理加速,< 0.1% 质量损失──这是不用想的量化──每H100 部署都应使用──
...
> FP16 إلى INT8 ((LLM.int8()): 2 倍内存减少,< 0.5% 质量损失──混合精度方法保持离群特征在 FP16而其他全部量化到INT8──

FP16 إلى INT4 (GPTQ / AWQ): 4x تقليل الذاكرة ، خسارة الجودة 1-3% اعتمادًا على النموذج والطريقة. تمكين نماذج 70B على GPU واحد 48GB.

> FP16 إلى INT4(GPTQ/AWQ): 4 倍内存减少,1-3% 质量损失,取决于模型和方法──使70B 模型可在单张48GB GPU 上运行──
...
> FP16 إلى INT4(GGUF Q4_K_M):3.5 倍内存减少,1-2% 质量损失──为CPU 推理优化──Q4_K_M 的 70B 模型大约40GB,在64GB M3 Max 上以 10-15代币/秒运行──
...
> FP16 إلى INT2:8 倍内存减少,5-15% 质量损失──仅适用于特定任务的可容忍退化──研究前沿,非通用生产就绪──
...
> تشكيل كل نوع من أشكال تعبيرات الدرجة، انظروا بشكل واضح ما تفعله كل رمز، مؤشر، وأعداد نهاية.
...
> عمليات الكميات الأساسية. الكمية بالتنسيق تستخدم عنصر التكثيف لكل صف أو كل صف باستخدام عنصر التكثيف.
...
> القياس الكمي قد دمّر الكثير من المعلومات‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬‬
...
> في مختلف الدرجات (٢、٣、4、8、16) ، قم بتقييم نفس الدرجة وتقييم جودة كل درجة.
...
> 本课产出 `outputs/skill-quantization.md`، إطار اتخاذ القرار لتحديد استراتيجية قياس صحيحة. تم إعطاء حجم النموذج والصناعة المستهدفة والمتطلبات الجيدة، فإنه يخبرك عن أي نوع من الأشكال والطرق والخطوات التحقق من استخدامها.
...
1. قم بتطبيق كمية المجموعات. بدلاً من مقياس واحد لكل قناة، استخدم مقياس واحد لكل مجموعة من 128 وزناً داخل قناة. هذا ما تستخدمه GPTQ و AWQ في الواقع. مقارنة أحجام المجموعات من 32، 64، 128 و 256 على نفس المصفوفة الوزن. توفر المجموعات الصغيرة جودة أفضل ولكن تكلفة تخزين أكبر لعوامل القياس.
   ترجمة باللغة الصينية:实现分组量化── لا تستخدم كل اتجاه عنصر تكميل، ولكن تستخدم كل 128 权重一组缩缩因子── هذا هو GPTQ 和 AWQ 实际使用── في نفس الوزن الموجات مقارنة 32、64、128 و 256 组大小── حجم الحجم الأصغر أفضل ولكن تخزين العوامل التكميل أكبر──
...
2. قم ببناء كمية دقيقة مختلطة. قم بتقييم الطبقات الأولى والأخيرة من شبكة متعددة الطبقات في INT8 بينما تقوم بتقييم الطبقات الوسطى في INT4. قم بمقارنة نوعية الخروج من نهاية إلى نهاية مع INT4 متساوية و INT8 متساوية. قم بتقييم وفورات الذاكرة مقارنة مع كل-INT8.
   中文翻译:构建混合精度量化器──将多层网络首尾层量化为INT8,中层量化为INT4──将端到端输出质量与统一INT4 和统一INT8比较──测量相比全INT8的内存节省──
...
3. تنفيذ مقياس المباشر (STE) للتدريب على الوعي بالكميات. إدراج عمليات الكمية/القطع الكمية المزيفة في المضي قدما لشبكة بسيطة من طبقتين تدرب على مهمة تراجع. مقارنة الخسارة النهائية بين نموذج تدرب بشكل طبيعي (ثم PTQ إلى INT4) مقابل نموذج تدرب مع QAT من البداية.
   ترجمة باللغة الصينية: تنفيذ عملية تقييم الحسّاسة القياسية في عملية التدريب المباشر ((STE)  في عملية التدريب المباشر للعودة إلى المهمة البسيطة من شبكة المستويين المباشرة للعمل في عملية تقييم الحسّاسة القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القياسية القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة القصة الق
