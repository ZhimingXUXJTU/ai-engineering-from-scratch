...
- تنفيذ طرق التصفية (عدول التغيرات، المعلومات المتبادلة، المربع الصفري) وطرق الغلاف (RFE، الاختيار الأمامي) من الصفر
  من صفر تحقيق 法 ((方差 值、互信息、卡方检查) و 包装法 ((RFE、前向选择)
- شرح لماذا يتم التقاط العلاقات غير الخطية بين الميزات والهدف التي تفوت التواصل
  شرح لماذا يمكن للمعلومات المتبادلة أن تستخدم العلاقة بين الصفات غير السريعة والصفات المستهدفة
- مقارنة تنظيم L1 (اختيار مدمج) مع RFE (اختيار الملفات) وتقييم التنازلات الحاسوبية الخاصة بهم
  مقارنة L1 正则化 (嵌入选择) مع RFE (包装选择) ، تقييمها
- بناء خط أنابيب اختيار الميزات التي تجمع بين أساليب متعددة وتظهر تحسين التعميم على البيانات التي تم احتفاظها
  تشكيل مجموعة من الطرق المختلفة لخيار الصفائح، وتظهر التحسينات العامة على البيانات المتبقية


> **【中文解读】**
> خصائص اختيار من بين العديد من الخصائص اختيار الأكثر فائدة من الجمعيات التابعة.

> **【拓展：特征选择在工业界的重要性】**
> في نموذج التحكم في النمو، يمكن تفسير ما يطلبه الهيئة التنظيمية يجب أن يكون قادرًا على تفسير كل سمة لماذا يتم اختيارها L1 正则化 Lasso) سوف يتم تلقائيًا تقليص الوزن على الخصائص غير المهمة إلى الصفر، مع تحقيق اختيار الخصائص وتدريب النموذج في تحليل التعبير الجيني، يتم اختيار 50 جينًا رئيسًا من بين 20،000 جين ليس فقط لتحسين أداء النموذج، ولكن أيضًا لإعطاء إشارة لدراسة الآلية المرضية في برنامج فوز جائزة Netflix، سيتم تقليص خيار الخصائص المفروضة إلى مئات الملايين 
...
> لديك 500 خصائص. تمرين نموذج بطيء. يستمر أكثر من المعدل. لا أحد يستطيع تفسير ما تعلمته.
...
> هذا هو العمل الفعلي للقياس العلامة. مع زيادة عدد الخصائص، فإن حجم الخصائص الفضائية يتفجر. أصبحت النقاط البيانية نادرة.
...
> خصائص اختيار هو حل الدواء. إزالة الضجيج. إزالة اللويات. الحفاظ على خصائص المعلومات التي تحمل هدفها الحقيقي. النتيجة هي: تدريب أسرع.
...
> **【中文解读】**
> خصائص اختيار ثلاث فئات من الطرق لديها كل من مزايا: فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء فراء  فراء  فراء                                                                                                           
...
> كل نوع من الخصائص الاختيارية تنتمي إلى واحدة من هذه المجموعات الثلاث:
...
> **过滤法**استخدام الحصص الاحتمالية بشكل مستقل لكل خصائص تعريفات.
...
> **包装法**訓練模型來評估特征子集──使用模型性能作為分數──結果更好,但成本高,因為需要多次重新訓練模型──
...
> **嵌入法**في عملية تدريب النموذج يتم اختيار الصفات. L1 يتم تقليد الوزن إلى صفر. تم تقسيم شجرة القرار على أكثر الصفات المفيدة. يحدث اختيار في عملية التأهيل، وليس كخطوة منفصلة.

### عتبة التباين

إن كان المصفاة الأسهل، إذا كانت ميزة ما تختلف بالكاد بين العينات، فإنها لا تحمل تقريبا أي معلومات.

> أسهل أجهزة التصفية. إذا كانت صفة واحدة في النموذج لا تتغير تقريباً، فإنها لا تحمل تقريباً معلومات.

فكر في ميزة هي 0.0 ل 999 من 1000 عينات. تغيرها قريب من الصفر. لا يمكن لأي نموذج استخدامها للتمييز بين الفئات. إزالتها.

> 考虑一千个样本中999个值为0.0的特征──它的方差接近零──没有模型可以用它来区分类──移除它──
...
> إذا X 和 Y 独立,p(x,y) = p(x) * p(y) ، لذلك على عدد المواد للخروج من الصفر،I(X;Y) = 0。X 告诉你关于Y的信息越多,互信息越高。
...
> مقارنة بالعلاقة المفتاحية: التعرف على المعلومات المتبادلة على العلاقات غير السريرية.
...
> 对于连续特征,先离散为分盒(基于直方图的估计) ――分盒数影响估计太少分盒丢失信息,太多箱增加噪声──常见选择:sqrt(n) 个分箱或 Sturges 规则 (1 + log2(n))。
...
> RFE 考虑特征交互, لأن النموذج يرى جميع الخصائص المتبقية معا.
...
> 代价:你需要训练模型 N - 目标 次数──500 个特征和目标 10 个,就是 490 个训练次──对于昂贵的模型,这是很慢──你可以通过每步移动多个特征来加速──如每轮移动底部10%)──
...
> الرفا العناصر التحكم في الخصائص يتم تحسين درجة التقطيع.
...
> لماذا تحديد للصفر؟ L1 惩罚 في الفضاء الوزني خلق 形约束区域──最优解倾向于落在形的角上,在那里一个或多个权重为零──L2 正则化(Ridge) خلق 圆形约束,权重缩小小但很少变为零──

هذا هو اختيار ميزات مدمجة: يتعلم النموذج أثناء التدريب الميزات التي يجب تجاهلها. يتم إزالة الميزات التي لا تزن.

> هذا هو اختيار الخصائص المضمنة: يتم إزالة الخصائص التي يتم تجاهلها في عملية التدريب.
...
> الحد: فقط تطبق على النموذج الحي. لا يمكن أن تتمكن من فهم أهمية الخصائص غير الحي.
...
> 决策树及其集成 (随机森林、梯度提升) الطبيعية على صفات الترتيب.
...
> انتباه: أهمية نموذج الشجرة تعكس العديد من الميزات ذات القيمة الفريدة.

### أهمية التحول

طريقة نموذجية:

> طريقة غير مرتبطة بالموديل:

1. تدريب النموذج وتسجيل أداء خط الأساس على بيانات التحقق من التحقق
   訓練模型并记录 على بيانات التحقق
2. لكل ميزة: خليط قيمها عشوائياً، قياس انخفاض الأداء
   لكل خصائص: مع انخفاض قيمة القياس
3. كلما زاد حجمها، كلما زاد أهمية الميزة
   أسفل أكبر، الخصائص أكثر أهمية

إذا لم يؤثر التشويش على أداء النموذج، فإن النموذج لا يعتمد عليه. إذا انهارت الأداء، فإن هذه الميزة حاسمة.

> إذا كان التشويش في صفة لا يؤثر على الأداء، فإن النموذج لا يعتمد عليه. إذا كان الأداء ينهار، فإن هذه الصفة هي المفتاحية.
...
> أهمية التبديل تجنب الاختلافات الأساسية في أهمية نموذج الشجرة. ولكنها بطيئة جدا: كل خصائص مرة واحدة لتقييم كامل، ولتثبيت تحتاج إلى تكرار عدة مرات.
...
> **【中文解读】**
> من التنفيذ صفر ثلاث قسمات من الخصائص طريقة اختيار:过法(方差值、互信息、卡方检查独立评估每个特征) 、包装法(递归特征消除RFE反复训练模型消除最不重要的特征) 、嵌入法(L1 正规化 Lasso训练时自动将不重要特征权重压缩为零) 通过合成数据(已知哪些特征有用) 验证各方法的效果──

> **【拓展：特征选择在 LLM 时代的新意义】**
> على الرغم من أن تعريف التعلم المتعمق على أنه "ميزة التعلم الآلي"، إلا أن الاختيار في المشهد التالي لا يزال مهمًا: 1) تعريف البيانات تخصيص الاختيار يمكن تحسين أداء وتجربة XGBoost / LightGBM ؛ 2) متطلبات تفسيرية  تحتاج مجالات الطب والمالية إلى شرح أي خصائص يتم استخدامها ؛ 3) 嵌入空间 حتى المحول ، أيضا في حاجة إلى إضافة 维度 على "تخصيص الخصائص" تخصيص التركيز هو في جوهري اختيار الخصائص الحركية) 
```figure
f3-feature-prune
```
...
> نعرف الحالة الحقيقية: الصفات 0-4 هي معلومات ((اضافة 3 و 4 هي 0 و 1 نسخة ذات صلة)) ، الصفات 5-9 ذات صلة مع الصفات المعلوماتية ، الصفات 10-19 هي صوت نقي.
...
> الملفات الكودية تعمل على مجموعة بيانات واحدة المجمعة من جميع الطرق الخمسة، وتطبق جدول مقارنة يظهر ما هي الخصائص التي اختارها كل طريقة.

## استخدمها في إطار التنفيذ

مع scikit-learn، يتم إدراج اختيار الميزات في خط الأنابيب:

> استخدام المعلمة الصغيرة، خصائص اختيار داخلية في خط الأنابيب:
...
> من الصفر إلى التحقق يظهر ما حدث داخل كل طريقة.`var(X, axis=0)`و تطبيق مخفياتها. وتتفاعل المعلومات هي التردد المشترك والمتعدد الحدودي في جدول المجموعات الحسابية. RFE هي دورة تدريبية وتصفية وتقطيع الأغصان. L1 هي مع تراجع درجة التدابير التدريبية.
...
> التطبيقات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المعلومات المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة: المشتركة:
...
1. **Forward selection**: تنفيذ عكس RFE. ابدأ مع صفرا من الميزات. في كل خطوة، أضف الميزة التي تحسن أداء النموذج الأكثر. توقف عند إضافة الميزات لم تعد تساعد. مقارنة الميزات المختارة مع نتائج RFE. أي أسرع؟ أي يعطي أفضل النتائج؟
   1. تم إنشاء 100 مجموعة بيانات عن الصفات المحددة، 10 منها تتعلق بالهدف، 90 منها هي الضجيج.
...
3. **Multicollinearity detection**: حساب مصفوفة التواصل لجميع الميزات. تنفيذ وظيفة التي، نظراً إلى عتبة التواصل (مثل 0.9), يزيل ميزة واحدة من كل زوج متواصلة للغاية (حفاظ على تلك التي لديها معلومات متبادلة أعلى مع الهدف). اختبار على مجموعة البيانات الاصطناعية والتحقق منها يزيل الميزات المتواصلة الزائدة.
   3. في نفس المجموعة البيانية مقارنة L1 و RFE نتائج اختيارها. هل تختار نفس الصفات؟ متى سيكون مختلفا؟
...
> **【中文解读】**
> استراتيجية عملية اختيار الميزات: ((1) قبل استخدام الاختلافات  قيمة إزالة الخصائص العادية ((الاختلافات القريبة من الصفر تعني عدم وجود معلومات) ؛ ((2) باستخدام المعلومات المتبادلة  اختيار الخصائص ذات الصلة بالهدف (((يمكن استخدام المعلومات المتبادلة للاستيعاب غير السلكي ، أكثر شاملة من الخصائص ذات الصلة)) ؛ ((3) باستخدام RFE أو L1 正则化精细选择 (((اعتبار التفاعل بين الخصائص)))).

> **【拓展：递归特征消除（RFE）的工业应用】**
> يستخدم RFE على نطاق واسع في علم الجينوميات  من بين 20،000 إعلان جيني لتحديد 50-100 جين أكثر قدرة على التنبؤ ، ليس فقط لتحسين أداء النموذج ، ولكن أيضا للاكتشاف من علامات الأمراض. في مجال التحكم في المالية ، RFE  ساعد على اختيار من بين مئات من خصائص المرشحين  من بين خصائص الدخول النهائية. RFE + RFECV  مع اختبار التسجيل  يمكن أن تحدد تلقائيًا عدد أفضل خصائص ، وهو واحد من أدوات اختيار الخصائص الأكثر شيوعاً في الممارسة.
...
- [An Introduction to Variable and Feature Selection (Guyon & Elisseeff, 2003)](https://jmlr.org/papers/v3/guyon03a.html)--البحث الأساسي حول طرق اختيار الميزات، لا تزال تستخدم على نطاق واسع
  [Guyon & Elisseeff: An Introduction to Variable and Feature Selection (2003)](https://jmlr.org/papers/v3/guyon03a.html)- خصائص اختيار المعلومات
