# هندسة السياق: Windows, Budgets, Memory, and Retrieval  上下文工程: Window, Budget, Memory and Retrieval
...
> **【拓展：上下文工程→Claude生态】**بروتوكول كلود MCP هو في جوهرها تنفيذ المعايير المحددة للعمل على النظام التنفيذي من خلال نموذج إدارة بروتوكول موحد للدوائر والموارد والنصائح الملاحظة.

>  **【前置】**学本节前请先掌握:(1) المرحلة 11·01-02(التجنيات السريعة、CT القليل من اللقطات);(2) المرحلة 11·04(التركيبات) و المرحلة 11·06(RAG)  فهم检索 كيفية الحصول على الملفات؛(3) الوهم 概念本节重度讨论 الوهم 预算── إذا لم تكن تعرف "فندوق سياق 200K" 指什么,先看 المرحلة 10──

**Type:** Build | **类型:** 构建
**Languages:** Python | **语言:** Python
**Prerequisites:** Phase 10 (LLMs from Scratch), Phase 11 Lesson 01-02 | **前置知识:** Phase 10（从零理解 LLM）、Phase 11 Lesson 01-02
**Time:** ~90 minutes | **时间:** ~90 分钟
**Related:**المرحلة 11 · 15 (التخزين السريع)  التخطيط الصديق للتخزين هو امتداد للهندسة السياقية. المرحلة 5 · 28 (تقييم السياق الطويل) لكيفية قياس الخسارة في الوسط مع NIAH / RULER. **相关:**المرحلة 11 · 15(提示缓存) 缓存友好布局是上下文工程的延伸──Phase 5 · 28(长上下文评估)介绍 كيفية استخدام NIAH/RULER 测量"中间丢失"──

## أهداف التعلم

- حساب ميزانيات الوهم عبر جميع مكونات نافذة السياق (تشارك النظام، الأدوات، التاريخ، الوثائق المكتسبة، مساحة توليد)
  跨所有上下文窗口组件(系统提示、工具、历史、检索文档、生成余量) حساب رمز 预算
- تنفيذ استراتيجيات إدارة نافذة السياق: تخفيض، وجملة، ونفذة زلقة لتاريخ المحادثة
  实现上下文 窗口管理策略:截截截,摘要,滑动 窗口管理对话历史
- تحديد الأولويات وتنظيم مكونات السياق لتعظيم اهتمام النموذج على المعلومات الأكثر أهمية
   حسب الترتيب الأولوي على المكونات التالية، تعظيم النموذج على الاهتمام بأهم المعلومات ذات الصلة
- قم ببناء مجموعة سياقية تقوم بتخصيص الرموز بشكل ديناميكي بناء على نوع الاستفسار ومساحة النافذة المتاحة
   بناء على نوع الاستفسار و المتاحة النوافذ الفضاء تحرك التوزيع رمز

> **【中文解读】**هدف هذا الدورة: تجاوز الهندسة السريعة، إدارة النظامية دخول النموذج على جميع المعلومات التالية.
...
> كلود أوبوس 4.7 لديها 200K رمز  نافذة(بيتا إصدار 1M) ・・・ جيبت-5 لديها 400K ・・・ جيمين 3 برو لديها 2M ・・・ هذه الأرقام تبدو كبيرة جدا حتى تملأها。
...
> هذا هو التفصيل الحقيقي للمساعد في البرمجة. النظام تلميح: 500 رمز. 50 个工具定义: 8,000 رمز.

>  **【类比】**上下文窗口像书桌桌面200K رمز 听起来很大,但放上"教科书(系统提示) "+"参考书(恢复文件) "+"草稿纸(历史) "+"计算器(工具) "就快满了。**Lost in the Middle**现象像寻找东西: عندما تكون مكتبة مكتظة بالأشياء، فإن أسهل شيء يتم تجاهلها هو التركيب في الوسط.

> ️ **【易错点】**上下文管理的3 个坑: ((1) **历史无限增长** حوار越长 تاريخ 越大,最终撞窗口;修复: باستخدام الختصار(每 N 轮缩成摘要) أو النافذة المنزلقة((حافظ فقط على الاختصار القريب K 轮 + 第一轮) 』2) **工具定义重复发送** كل مرة تدخل 50 个工具 schema 全发一遍;修复:用快速缓存(Phase 11·15),Claude / OpenAI 都支持,省 90% cost。(3) **检索文档全塞**دعو مرة أخرى 50 个块 全塞 prompt 模型迷失;修复:top-5 高质量块 + 交叉编码重排。
...
> لكن الاهتمام لن يتزايد بشكل خطي مع طول التقاطع. أهمًا ، فإن معدل تحديد البحث ينخفض. يظهر اختبار "البحر العالي" أن النموذج صعب العثور عليه في الموقع الوسطى للمعلومات على التقاطع.
...
> التدريب العملي: هناك 200K رمز قابل الاستخدام لا يعني استخدام 200K رمز هو فعال.
...
> كل رمز وضعته في النافذة كان يحتوي على رمز يمكن أن يحمل معلومات أكثر صلة. كل أداة غير صلة تعريف. كل دورة من المحادثات السابقة. كل بلاج من الأسئلة.

## المفهوم الأساسي

> **【中文解读】**上下文工程(Context Engineering) هو مفهوم الهندسة السريعة على الفور ليس مجرد كتابة السريعة، ولكن نظامية إدارة جميع المعلومات على النموذج على نافذة النص التالي: نتائج الوصول ‬التاريخ المحادثة‬الأدوات المخرج‬إرشادات النظام ‬الخ‬.

> **【拓展：上下文窗口的有效利用】**GPT-4o لديها رمز 128K على النافذة التالية، ولكن الدراسات أظهرت أن النموذج على المعلومات في الموقع الوسطى" تركيز هبوط" ((Lost in the Middle 问题) ").
...
> ضع النوافذ فوقها على أنها ريم وليس قرص نقدي.
...
> كل عنصر يسعى للحصول على مساحة. تعريف المزيد من الأدوات يعني مساحة تاريخية أقل.
...
> أهم أشكال عملية في مجال الأعلى والإنجليزية. النموذج يتمتع بتركيز أفضل على المعلومات التي تظهر في بداية والنهاية.
...
> ليو وغيرهم ((2023) اختبروا هذا الظاهرة بشكل منهجي. قاموا بوضع المستندات ذات الصلة في 20 موقع مختلف في المستندات غير ذات الصلة، وقاسوا معدل الصلة للجوابات.
...
- ضع الأهم المعلومات أولاً (تعليمات النظام السريعة، التعليمات الحرجة)
  سأضع المعلومات المهمة على الموقع
- ضع الاستطلاع الحالي والسياق الأكثر أهمية في الأخير (تساعد التحيز الأخير)
  سوف تحصل على استفسارات حالية وأكثر علاقة على النص التالي إلى الأخير
- تعامل وسط السياق كمناطق ذات الأولوية المنخفضة
  سوف نضع الوسط في المنطقة ذات الأولوية الأدنى
- إذا كان عليك إضافة معلومات في الوسط، قم بتكرار النقطة الرئيسية في النهاية
  إذا كان يجب أن تضع المعلومات في الوسط، في النهاية
...
> **工具定义**: كل أداة 50-200 رمز ((اسم ∙ وصف ∙ مخطط العنصر) ∙ 50 工具 لكل 150 رمز، يعادل 7500 رمز قبل بدء المحادثة تم استخدامها ∙ اختيار الأدوات الحركية ∙ يحتوي فقط على الأدوات المتعلقة بالمسألة الحالية ∙ يمكن تقليصها 60-80% ∙
...
> **检索上下文**: من المستندات من قاعدة بيانات النطاقات 搜索结果 文件内容──检查质量直接决定响应质量──坏的检查比不检查更糟它用噪声填满窗口并主动误导模型──
...
> **对话历史**: جميع المستخدمين السابقين والمساعدين الإجابة.
...
> **少样本示例**: نموذج من إدخال / إخراج إحتمالات التصرفات المحددة بإنتباه: 2-3 نموذج من إختيارات دقيقة غالباً ما تكون أكثر قدرة على تحسين جودة الإخراج من آلاف الرموز.
...
> **生成预算**: فان النموذج لا يملك مساحة للإجابة على الامتثال.
...
> **历史摘要**:而非逐字保留所有前序轮次,定期总结对话──"لقد ناقشنا X, قرر Y, المستخدم يريد Z"باستخدام 100 رمز بدل 10 轮 2,000 رمز──当历史超过值(如 5,000 رمز)时运行摘要──
...
> **相关性过滤**: سوف كل بحث وثائق مع الاختبار الحالي ضرب分، وترك أقل من 值.
...
> **工具裁剪**: قسم استفسار المستخدم، يحتوي فقط على أدوات ذات صلة بهذا النية.
...
> **递归摘要**: على الصفحة الخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة والخامسة.

### أنظمة الذاكرة

هندسة السياق تمتد على ثلاثة آفاق زمنية

> أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أعلى أع أع

**Short-term memory**: المحادثة الحالية. يتم تخزينها في نافذة السياق مباشرة. ينمو مع كل جولة. يتم إدارةها عن طريق التجميع والقصر.

> **短期记忆**: حالي حوار. مباشرة تخزين في نافذة أعلى و أدناه.
...
> **长期记忆**:跨对话持久的事实和偏好──"مفضل المستخدم TypeScript──""项目用PostgreSQL──"存储在数据库,会话开始时检索──Claude Code 存在CLAUDE.md 文件──ChatGPT 存在其内存 功能──
...
> **情景记忆**:可能相关的特定过去交互──"上周二我们在作者模块调试过类似问题──"خزنة للدخول، الحوار الحالي مطابقة الحالات الماضية عند الاختبار──"
...
5. إضافة مثالات قليلة التي تتطابق مع نوع المهمة
   إضافة نموذج صغير يتناسب مع نوع المهمة
...
> هذا هو الفرق بين تطبيق الذكاء الاصطناعي الرائع و التطبيق الذكاء الاصطناعي الرائع.
...
> لا يمكنك القيام بجدول لعدد لا يمكن قياسه. بناء جهاز قياس الوهم بسيط.
...
> 实现重排策略: أهم المواد الموضحة في الاولى والاخيرة، وأهم غير المهمة في الوسط
...
> فقط تتضمن أدوات ذات صلة بالتحقيقات الحالية.
...
> وضع كل شيء على اتصال مع بعضها البعض
...
> كلويد كود باستخدام تدابير مختلفة على إدارة النص. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
...
> 关键工程决策:كلود كود لا ينمو إلى القائمة الكودية كلها على النص.

### تحميل السياق الديناميكي للكورسور
...
> يقوم cursor بإدخال جميع قوائم المرجع في داخلها. عند استفسار الدخول، استخدم حجم التشابهات لتحقيق الملفات والبلوكز الأكثر ارتباطاً.

هذا هو النمط: تضمين كل شيء، استرداد على الطلب، تضمين فقط ما يهم.

> هذا هو النموذج: إدخال كل شيء، حسب الحاجة، فقط يتضمن المهمة.
...
> سيتم تخزين "ChatGPT" تفضيلات المستخدم والحقيقة لفترة طويلة من الذاكرة. عند بدء كل حوار، يتم استرداد الذاكرة ذات الصلة وتحتوي على نصيحة النظام.
...
> 检索增强生成是上下文工程的形式化──不把知识塞进模型权重 () 训练) 或系统提示 () ),而在查询时检索相关文档并注入上下文窗口──整个RAG管线分块、嵌入、检索、重排存在就是为了解决一个问题:把正确信息放入上下文窗口──
...
> 本课产出 `outputs/prompt-context-optimizer.md` مراجعة على استراتيجية التجميع و توصي بتحسين النصائح القابلة للاستخدام.
...
> مع الوقت المنتج`outputs/skill-context-engineering.md` بناء على نوع المهام  على النظام القياسي للقيام بالقرار على النظام التجاري للقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام بالقيام.
...
1. إضافة "كشف النفايات الوهمية" إلى فئة ContextBudget. يجب أن يضع علامة على المكونات التي تستخدم أكثر من 30% من الميزانية ويقترح استراتيجيات الضغط المحددة لكل نوع من المكونات (التلخيص التاريخ، أدوات القص، إعادة تصنيف الوثائق).
   给 ContextBudget 添加"مراقبة التوقيت 浪费检测器"──应标记使用超过30% 预算组件,并建议针对每个组件类型的压缩策略(摘要历史、剪裁工具、重排文档)
...
2. تنفيذ التخفيضات التفصيلية للسياق المسترد. إذا كانت الوثائق المستردة متشابهة أكثر من 80% (من خلال التداخل الكلي أو التشابه الكوني في إدخالها) ، فاحتفظ فقط بالتي تتجاوز درجة. قياس كمية ميزانية رمزية تسترد هذه.
   实现检索上下文的语义去重──若 اثنين من مستندات الاختبار تزيد عن 80% 相似性 ({\displaystyle \mathbb {\\mathbb {{\mathbb}}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathbb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{\mathb}{{{mathb}}}{{{{b}}}}{b}{b}}{b}{b}}{b}{b}{b}}{b}{b}}{b}{b}{b}}{b}{{b}}}{{b}}}{b}}{{{{b}}}}}}}{{{{{{{}}}}}}}}}}{{{{{}}}}}}}}}}}{{{{{{{{{}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}
...
4. قم بتنفيذ اختيار أداة قائمة على الأولويات. بدلاً من إدراج / استبعاد ثنائي، قم بتعيين كل أداة درجة أهمية للمسألة الحالية. قم بإدراج الأدوات في ترتيب أهمية ينخفض حتى يتم استنفاد ميزانية الأداة. مقارن أداء المهام مع أدوات 5، 10، 20، و 50 متضمنة.
   تطبيقات الاختيار الأدوات القائمة على الأولوية. لا تحتوي على اثنين أو إقصاء، ولكن تفرق كل أداة على الارتباط مع الاستفسارات الحالية.
...
5. قم ببناء مضغط سياق متعدد الاستراتيجيات. تنفيذ ثلاث استراتيجيات الضغط (التخفيض، التخفيض، استخراج الجمل الرئيسية) ووضع مقارنة عليها على مجموعة من 20 وثيقة. قياس التنازل بين نسبة الضغط والاحتفاظ بالمعلومات (هل ما زال النسخة المضغوطة تحتوي على الإجابة على السؤال؟)
   构建多策略上下文压缩机.实现三种压缩策略. 截断,摘要,关键句提取) ، على 20 مجموعة من المستندات على اختبار基准.
...
- [Liu et al., 2023 -- "Lost in the Middle: How Language Models Use Long Contexts"](https://arxiv.org/abs/2307.03172)-- الدراسة النهائية حول الاهتمام المعتمد على الموقف، التي تظهر أن النماذج تكافح مع المعلومات في وسط السياقات الطويلة
  ليو 等، "خسرت في الوسط" ((2023)  موقع متعلقات الاهتمام دراسة السلطة، تظهر أن النموذج صعب التعامل مع长上下文中间的信息
- [Anthropic's Contextual Retrieval blog post](https://www.anthropic.com/news/contextual-retrieval)-- كيف أنثروبيك تقترب من استرداد قطع واعية للسياق، وتقلل من فشل الاسترداد بنسبة 49٪
  الانثروبات 上下文检索博客أنثروبات  كيفية التعامل على الانثروبات 感知分块检索,将检索失败减少49%
- [Simon Willison's "Context Engineering"](https://simonwillison.net/2025/Jun/27/context-engineering/)-- المدونة التي تسمي هذا التخصص وتتميزه عن الهندسة السريعة
  "هندسة السياق" سمون ويلسون"سميت هذا المجال و تمييزه مع تجهيز الهندسة
- [LangChain documentation on RAG](https://python.langchain.com/docs/tutorials/rag/)-- التنفيذ العملي لإنتاج المعلومات المُتزايد من الاسترداد كنمط هندسي سياقي
  لانغ تشين RAG 文档 سوف تستكشف زيادة الناتج كتحقيق عملي للنموذج المعماري على
- [Greg Kamradt's Needle in a Haystack test](https://github.com/gkamradt/LLMTest_NeedleInAHaystack)-- المؤشر الذي كشف عن فشل في الاسترداد يعتمد على الموقع في جميع النماذج الرئيسية
  كشف Greg Kamradt's Big Sea捞针测试 كل الموديل الرئيسية المرتبطة الموقع
- [Pope et al., "Efficiently Scaling Transformer Inference" (2022)](https://arxiv.org/abs/2211.05102)-- لماذا طول السياق يقود الذاكرة والبطء، وكيف KV مخزن، MQA، و GQA تغير حساب الميزانية.
  وبوب 等، "توسيع الكفاءة إضفاءة المحولات" (2022) 为何上下文长度驱动内存和延迟، وكذلك KV cache、MQA、GQA 如何改变预算计算──
- [Agrawal et al., "SARATHI: Efficient LLM Inference by Piggybacking Decodes with Chunked Prefills" (2023)](https://arxiv.org/abs/2308.16369)-- مرحلتين من الاستنتاج التي تجعل الطلبات الطويلة مكلفة في TTFT ولكن رخيصة في TPOT؛ الحقيقة القاعدة وراء التداولات التعبئة السياق.
  أغروال 等، "ساراتي" ((2023)  التفكير في مرحلتين يجعل النصيحة طويلة في TTFT 上昂贵但 TPOT 上便宜; 上下文打包权衡背后的真相──
- [Ainslie et al., "GQA: Training Generalized Multi-Query Transformer Models from Multi-Head Checkpoints" (EMNLP 2023)](https://arxiv.org/abs/2305.13245)ورقة الاهتمام المجموعة التي تقطع ذاكرة KV 8x في مُشفرات الإنتاج دون فقدان الجودة.
  أينسلي وغيره، "GQA" ((EMNLP 2023) 分组查询注意力论文، في إنتاج المفصلات المحمولة في KV خفضت 8 مرات دون خسارة الجودة.
