# Preencher/Descodificar Desagregado  NVIDIA Dynamo e llm-d 分离式 预填充 解码 LLM
...
> **【中文解读】**Esta secção apresenta a separação de preenchimento/descodificação que se realizará em diferentes hardware.
**Type:** Learn
...
> - Não .**【前置】**学本节前请先掌握:Fase 17·04(vLLM)、Fase 17·08(指标)。分离式 prefill/decode = 把两种瓶阶段(计算密集 vs 内存密集) 分到不同 GPU 池──
> - Não .**【类比】**O mesmo GPU 跑两者 = 厨师又做又菜端菜,浪费某种资源──Dynamo/llm-d 把两者分池,KV cache 通过NIXL(RDMA) 传输──NVIDIA 发布:DeepSeek-R1 在 GB200+Dynamo 提速 ~6 倍;GB300+Dynamo MoE 吞吐最高50 倍──$2M 推理账单可省 30-40%（即 $600-800K/ano)。短请求(<512 token) 不值得──
...
- Explique por que o preenchimento e a decodificação têm diferentes atribuições ótimas de GPU e quantifique o resíduo sob colocação.
  Tradução do inglês: Explique por que o pre-empenho e o código têm diferentes GPUs de melhor distribuição, e quantificar o gasto de recursos do mesmo lugar.
- Diagrama da arquitetura desagregada: pré-reempimento, decodificação, transferência de KV através de NIXL, roteador.
  Tradução do inglês para tradução inglesa: drawings separated from the original:
- Cita a condição em que a desagregação NÃO dá resultados (indicações curtas, saídas curtas).
  Tradução do inglês para o inglês: 短提示、短输出)
- Distinguir o NVIDIA Dynamo (pilha acima) do llm-d (nativo Kubernetes) e combinar cada um com um contexto operacional.
  No entanto, o que não é verdade é que o que não é verdade é que o que não é verdade.

## O problema é o problema da introdução

> **【中文解读】**Prefill é computado limitado, decode é de memória limitada, em função da mesma GPU em cima de ambos desperdiçam um recurso. Em carga de trabalho misturada, 20-40% do tempo de perda de GPU em um recurso errado.

> **【拓展：分离式推理的经济学】**Economia de separação: dados internos abrangentes mostram que a mudança de serviço comum para Dynamo pode ser economizada em 30-40% sob a premissa de manter o mesmo P99  atraso SLA$2M 年支出可节省 $600-800K/ano) ――Baseten  relatório Dynamo KV roteamento 带2x 更快 TTFT 和 61% 更高吞吐──关键条件:提示 >512 tokens + 输出 >200 tokens 时才值分离短提示的 KV 传输开销超过收益──
...
> **【中文解读】**
> Prefill é um tipo de cálculo intenso, processar todo o prompt, decodificar é um tipo de cálculo intenso, cada token faz apenas pouca quantidade de cálculo.
...
> **【中文解读】**两种分离式推理框架对比:(1) NVIDIA Dynamo位于 vLLM/SGLang/TRT-LLM 之上的编排器,Rust 核心 + Python 扩展,Planner Profiler 自动配置预填:decode 比例,在 GB200 NVL72 + DeepSeek-R1 MoE 上报告 6x 吞吐提升;(2) llm-d(Red Hat + AWS) Kubernetes 原生,prefill/decode/router 作为独立服务,per-role HPA,`packDomain: rack`确保同一机架内高带宽 KV 传输──选 Dynamo 如果想要托管编排器,选 llm-d 如果承诺到 CNCF 生态──
...
> **【中文解读】**MoE(xp) modelo em Blackwell é o maior beneficiário da teoria de separação. O roteamento de especialistas em MoE em pre-reempimento estágio é calculado intenso, em decodificação estágio é de cache de memória intenso, portanto, a separação é dupla receita.

> **【拓展：分离式推理与缓存路由的协同】**O roteador de raciocínio de separação é KV-cache-aware de [[Fase 17·11) ⋅ solicitação de até ao decodificação 池时, se já houver cache , pode ser directamente copiado e sem necessidade de passar por preenchimento 池命中率 com o dividendo de implementação ⋅ NIXL é NVIDIA de node间传输层, usando RDMA/InfiniBand(可用时) ou TCP 回归──4K-prompt KV em 70B FP8 ⋅ Este é o motivo de um atraso de transmissão de cerca de 20-80ms ⋅
...
> **【中文解读】**
> O sistema de transferência de KV não vale a pena. Apenas o sistema de transferência de KV é um sistema de transferência de KV.

> **【拓展：分离式推理→下一代基础设施】**A teoria de separação é a principal direção da infraestrutura de LLM de 2026 ⋅ NVIDIA GB200 NVL72 机架专门为预填/解码 分离设计, NVLink 带宽足够在 GPU 间传输 KV cache── Red Hat  llm-d 项目把这个能力带到了 Kubernetes 生态──对于大规模的MoE 模型(如 DeepSeek-V3), a teoria de separação pode economizar 30-40% de custos de argumentação──
```figure
...
> 本课产 出 `outputs/skill-disaggregation-decider.md`- Tendo em conta a carga de trabalho e o cluster, decide se se desagrega.

## Exercícios.

1. Corra .`code/main.py`A que ponto a desagregação supera a colocação?
   Tradução: 运行`code/main.py`◊ Em que ponto de longo prazo a distribuição de serviços é melhor do que a mesma?
2. Desenhar o pré-reempimento e o decodificador para um serviço RAG com comprimento de prefixo P99 8K, saída 300.
   Tradução do inglês para inglês: For P99 前长度 8K、300 输出 RAG 服务设计预填充池和码池。
3. Dynamo vs llm-d: escolha um para uma loja pura Kubernetes sem preferência de Python.
   Por favor, não me esqueça de dizer que eu não sou um homem.
