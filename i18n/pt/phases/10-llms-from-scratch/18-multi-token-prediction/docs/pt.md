# Multi-token Prediction (MTP) 更多代码预测
...
> **【中文解读】**传统 LLM Cada posição apenas prevê um token. DeepSeek-V3 em cada posição aumenta uma segunda perda.

> **【拓展：MTP→DeepSeek-V3创新】**O MTP é uma das quatro grandes inovações da DeepSeek-V3 (MLA + MoE + MTP + DualPipe) e também melhorou o treinamento (MLA + MoE + MTP + DualPipe).

> - Não .**【前置】**學本節前請先掌握:Fase 10·04(Pre-Training GPT);Fase 10·15(Dekodificação especulativa) MTP 在推理时变成草稿器;因果链(causal chain)概念──本节是Fase 10·20(DeepSeek-V3 Walkthrough) 的前置──
> - Não .**【类比】**传统下语标语时一次记一个单词――MTP = 同时记"这个单词"和"下一个常用搭配"训练时学更多结构(梯度信号丰富),推理时把"搭配预测器"当草稿器免费加速(80% 接受率)
...
- Estabelecer o objetivo de treinamento MTP e derivar a perda conjunta em profundidades de previsão.
  Explicar o objetivo de treinamento MTP, adicionar a profundidade de perda conjunta
- Explique a diferença entre os cabeças MTP paralelas de Gloeckle et al. (2024) e os módulos MTP sequenciais de DeepSeek-V3 e por que o projeto sequencial preserva a cadeia causal.
   Explicar a diferença entre a sequência de MTP head e a sequência de MTP 模块 do Gloeckle, bem como por que a sequência de design mantém a sua cadeia de resultados
- Calcular o parâmetro e a carga de memória da adição de módulos MTP a uma corrida pré-treinamento.
   calcular em pre-treinamento adicionar MTP 模块 de parâmetros e memória
- Implementar um módulo MTP a partir do zero: a incorporação compartilhada, o bloco de transformador por profundidade, a projeção e a cabeça de saída compartilhada.
  Desde zero a implementação de um MTP 模块: compartimentação de inserção  por profundidade Transformer 块、 projeção e saída de partilha
...
> O primeiro token 预测是标准 LLM 训练目标――每个隐藏状态被监督预测恰好一个事:紧接下来的下一个 token―― é um sinal fraco surpreendente―― a maior parte da informação da sequência se estende a um único token 之外结构"",连贯性"",事实性"",算术流程―― o modelo deve passar em milhões de tokens 累积许多单 token 信号来学习这些――
...
> MTP 问: Se cada estado oculto fosse monitorado ao mesmo tempo, como seria?Gloeckle 等人(Meta,2024) mostrou que isso ajuda. Suas realizações colocam em cima da rede de base várias saídas independentes, cada previsão com diferentes desvios.
...
> **【中文解读】**Dois tokens  pré-determinar que um modelo prevenda simultaneamente vários tokens futuros (em vez de apenas prever o próximo), fornecer um sinal de treinamento mais rico.

> **【拓展：Meta 的多 token 预测研究】**Meta em um artigo publicado em 2024 mostrou que o 4-token  prediction é significativamente melhor do que o padrão seguinte em tarefas de geração e raciocínio de código  prediction。DeepSeek-V3 também usou vários tokens auxiliares  prediction heads para aumentar o treinamento de sinais。 Este método tem uma ligação natural com o desbloqueio de dados  Modelo já aprendeu a prever vários tokens no futuro。
...
> **【拓展：多 token 预测与推理加速的联系】**Modo token  previsão de treinamento modelo natural adequado para o jogo  porque já aprendeu a prever futuros vários tokens── Pesquisas de Meta mostram que o modelo de 4 tokens  previsão de treinamento aumenta em código gerado em comparação com o treinamento padrão 5-10%──
...
> 本课产 出 `outputs/skill-mtp-planner.md`△ given determination pre training运行规范(模型大小、数据、算力), ele retorna ao plano de MTP integrado: profundidade D、`lambda`调度、内存开销和推理时投机解码连线──
...
1. Corra .`code/main.py`. Mostre que a perda por profundidade diminui monotonicamente à medida que o sinal sintético se fortalece. Modifique o sintético para usar um padrão fixo e verifique a convergência das perdas de profundidade-1 e profundidade-2.
   Tradução: 运行`code/main.py` mostrar cada perda de profundidade com sinal de síntese reforçar o seu modo de distribuição  modificar os dados de síntese usando um modelo fixo e verificar a perda de profundidade 1 e profundidade 2 
...
2. Compute o custo de parâmetro para um modelo 70B denso (escondido 8192, 80 camadas) com módulo D = 1 MTP. Compare com o custo de parâmetro 14B relatado pelo DeepSeek-V3. Explique por que o número do DeepSeek é maior: o bloco de transformador MTP herda a mesma estrutura MoE, inflacionando a contagem de parâmetros por módulo.
   Chinese Translation:计算密集 70B 模型(hidden 8192,80 层)加D=1 MTP 模块的参数开销。与DeepSeek-V3 报告的14B 开销比较。解释为什么DeepSeek 的数字更高:MTP transformador 块继承了相同的MoE 结构,膨胀了每模块参数──
...
3. Implementar D=2 no brinquedo: adicionar um segundo módulo MTP que toma h^(1) e prevê `t_{i+2}`Verifique a perda conjunta e a contabilidade dos parâmetros coincide com as equações 19-21 do papel do DeepSeek.
   Tradução do inglês: In plaything model implement D=2: Add a second MTP 模块,取 h^(1) 并预测 `t_{i+2}` Verificação de perda conjunta e cálculo de parâmetros com DeepSeek 论文公式 19-21 匹配──
...
4. Mudar o brinquedo para MTP paralelo (estilo de Gloeckle): adicionar cabeças de saída D em cima do estado oculto principal, cada uma prevendo um offset diferente. Medir como as perdas por profundidade comparam com a versão sequencial no mesmo sinal sintético. A versão sequencial deve produzir menor perda de profundidade k para k > 1 porque condiciona as previsões intermediárias.
   Tradução em inglês:将玩具模型换成并行 MTP(Gloeckle风格): adicionar D 个输出头, em estado de oculto principal, cada previsão diferentemente desviada。 medir cada perda de profundidade com a versão em ordem em comparação no mesmo sinal de síntese。 a versão em ordem em k > 1 时应产生更低的深度 k 损失,因为它在中间预测为条件──
...
5. Utilize o módulo MTP formado como um esboço de estilo EAGLE: convoque o módulo k para propor `t_{i+k}`Se você atingir 50% + no brinquedo, você reproduz a propriedade empírica MTP-as-draft.
   Tradução do inglês para inglês: will train of MTP 模块用作 EAGLE 风格草稿:在推理时调用模块 k 提出 `t_{i+k}` Medir estes tokens de esboço na sequência de reservas para a taxa de aceitação da previsão do modelo principal.
