# Quantização: fazer modelos adequados

> Um modelo 70B em FP16 precisa de 140GB. Dois A100s apenas para pesos. Quantize para FP8: uma GPU de 80GB. INT4: um MacBook.

> **【中文解读】**O modelo de 70 bilhões de parâmetros em FP16 necessita de 140 GB de armazenamento e velocidade.

> **【拓展：量化→llama.cpp/GGUF】**llama.cpp 和 GGUF 格式让大模型能在消费级硬件上运行──GPTQ、AWQ、GGUF 等量化方法是将70B+ 模型部署到本地设备的关键──理解量化是理解大模型部署的基础──

> - Não .**【前置】**O curso de formação em matemática é um curso de formação em matemática.

> - Não .**【类比】**量化 = 压缩图片──原照片(FP16) por imagem 16 bits,肉眼分辨不出和 8 位(FP8) de diferença, mas文件大小一半──再压到4位──INT4)肉眼开始看到(精度损失),但文件小4 倍──模型量化同理:FP16→INT4 大小变 1/4,性能损失通常 <5%──GGUF 格式让你在 MacBook 上跑 Llama-70B──
...
- Implementar quantização simétrica e assimétrica de FP16 para INT8 e INT4, incluindo escalação por tensor e por canal
   Realizar quantificação de peso/não-peso de FP16 para INT8 e INT4, incluindo quantificação de peso e encolhimento de canal
- Calcule a economia de memória da quantização e determine qual precisão se encaixa na VRAM de uma determinada GPU
   calcular quantificar o volume de armazenamento de dados, determinar a precisão de uma GPU
- Explique a diferença entre a quantização pós-formação (PTQ) e a formação consciente da quantização (QAT)
  解释训练后量化(PTQ) e 量化感知训练(QAT)
- Aplicar GPTQ ou AWQ para quantificar um modelo real e medir a compensação entre precisão e memória em um índice de referência
   aplicar GPTQ ou AWQ quantificar o modelo real, e em base à medição de precisão-evidência

> **【中文解读】**Este curso realiza a técnica de quantificação com precisão de mudança de aparência e velocidade── método central: quantificação/não-proporção、 quantificação/conclusão de cada canal、PTQ(treinamento pós- quantificação) vs QAT( quantificação de percepção)── quantificação é um padrão de implementação de cada modelo maior que 7B──
...
> Llama 3 70B tem 700 bilhões de parâmetros. Cada parâmetro é de 16 bits.
...
> Mas cada parâmetro 16 é um desperdício. A maioria dos pesos de rede neural se agrupam em zero. O alcance do FP16 está quase totalmente desprovido. Se você medir a distribuição real do peso de Llama 3 70B, 95% está entre -0,1 e +0,1 . Você pode usar 16 pontos para representar o valor de 4 pontos.
...
> Quantização com números de baixa precisão substituir números de alta precisão. FP16 para FP8  Reduzir a metade de armazenamento. FP16 para INT4  Reduzir a quarta parte. 140GB  Modelo transformado em 35GB                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
...
> O problema é a precisão e a perda que você perde. O modelo INT4 bem quantificado na maioria dos modelos de base retém 95-99% da qualidade original.
...
> 社区GPTQ vai quantificar Llama 3 para INT4, em WikiText  仅损失约1-2个困惑度点──Mistral 发布了Mixtral 8x22B 的 FP8 检查点,MMLU 上几乎零质量损失──GGUF 格式驱动 llama.cpp,运行在MacBook M 系列芯片上70B 模型──量化不是黑客──它是每个超过7B 模型的标准部署路径──

> **【中文解读】**FP16 por parâmetro 16 bits,70B  modelo precisa de 140GB  Mas 95% do peso concentra-se entre -0,1 a +0,1  usando 16 bits para indicar que estes valores foram muito desperdiçados  Quantizado para INT4 irá reduzir a demanda de armazenamento para 35GB, disponível em GPU de nível de consumo  Boa quantização de INT4                                                                                                                                                                                                                                                                                                                                                                                                                                   

> **【拓展：量化生态】**量化生态已非常成熟:GPTQ(baseado em aproximação de segundo grau de quantificação de informação) 、AWQ(activate percepção de peso, proteção de peso saliente) 、GGUF(llama.cpp 量化格式, suportar 2-8 bits 混合精度) 、llama.cpp 让70B 模型在MacBook M 系列芯片上运行,催生本地大模型部署的生态(Ollama、LM Studio等) ⋅
...
> Cada número de pontos de um ponto tem três partes: símbolo, índice e número final (também chamado de número válido).
...
> **FP32**É a precisão total. O número de pontos de 23 dá-lhe cerca de 7 pontos de precisão.
...
> **FP16**A redução do peso é um problema, mas a atividade e a escala de aumento no treino são muito perigosas.
...
> **BF16**(Brain Float 16) retém o índice de 8 bits do FP32 mas reduzirá o número de尾数 para 7 bits. O alcance é inferior ao FP32.
...
> **FP8**Há dois tipos de variações: E4M3 (4, 3 尾数) para a avaliação do peso e da atividade: E5M2 (5, 2 尾数) para a avaliação da graduação do treino:
...
> **INT8**É um formato de números inteiros. Não há índice, não há fim. Apenas 256 valores de intervalos, de -128 a 127 . Você precisa de reduzir o fator para que o valor do ponto seja mais rápido.
...
> **INT4**Mais adiante: apenas 16 个可能值: 缩放因子承担重任: 质量完全取决于你如何选择缩放比例和量化哪些权重: 最先进的INT4 方法:  GPTQ、AWQ) conserva o modelo original 95%+ de质量:
...
> 核心操作很简单──取一个浮点值张量,找到缩缩因子,乘以它,四舍五进到最近整数,然后存储整数加上缩缩因子──
...
> 误差是四舍五进误差── cada valor é o máximo de desvio `scale / 2` A diferença total entre as camadas depende de quanto peso você tem e de como o modelo é sensível à perturbação dessas pesos.
...
> **逐张量 vs 逐通道量化。**张量对整体权重矩阵使用缩小因子――简单但有损: se uma série tiver um grande valor em outra série tiver um pequeno valor, o pequeno valor perderá a maior parte da precisão―― por linha ou por linha em cada saída do矩阵 () usando um factor de缩小因子――开销更大――储存 N 个缩小因子而不是 1 个) mas o massa é significativamente melhor―― cada método de quantificação de produção é usado em fases ou por mais porções――
...
> **非对称量化**添加零点偏移:`quantized = round(tensor / scale) + zero_point`◊ Este tratamento não é distribuído em zero-centrado. Por exemplo, o valor ativo do ReLU 总是非负的.

### Hierarquia de sensibilidade

Nem tudo num modelo tolera quantização de forma igual.

> O modelo não tem todas as partes da mesma tolerância à quantificação.
...
> **权重（最鲁棒）。**O peso do modelo varia lentamente durante o treinamento, seguindo a distribuição de alta centrada em zero. Eles quantificam o efeito.
...
> **激活（中等敏感度）。** Activar é o valor médio da rede de logística.  Eles têm um alcance de movimento mais amplo do que o peso e não contêm o valor do grupo.  Individual attention heads can generate a ativar um valor de atividade superior a 100 vezes o valor médio.  Estes valores do grupo são essenciais para a qualidade do modelo.  Simplicidade da quantificação irá destruir informações.  Solução: manter o caminho do grupo em maior precisão (LLM.int8) ), usando token por token ou ação de ativar um caminho em menor aceleração.
...
> **注意力 logits（最敏感）。**A suavidade no seu sistema de atenção é altamente sensível às pequenas variações de sua entrada. O erro de quantificação de 0,01 no logite pré-softmax pode alterar significativamente a distribuição da atenção. A maioria dos programas de quantificação, mesmo que todas as outras partes sejam quantificadas, também mantém a atenção calculada com maior precisão.
...
> **量化感知训练（QAT）**Em um treinamento, a equipe de pesquisa irá colocar o peso na posição de quatro quadros de erro menor.
...
> **GPTQ**É um método de PTQ de uma só vez. É um método de quantificação de peso por camadas, usando o pequeno conjunto de dados de calibração.
...
> **AWQ**Observe a menor parte do peso (cerca de 1%) porque é muito importante em relação ao grande valor de ativação.
...
> **GGUF**É o formato de arquivo usado em llama.cpp e seu ambiente. É suportado por misturação: diferentes níveis obtêm diferentes níveis de largura.
...
> Como julgar que o seu modelo de quantificação ainda é útil?
...
> **困惑度。**O valor diferencial diz-lhe quanto a quantificação destruiu informações. O valor diferencial é: < 0,5 优秀,0.5-1.0 良好,1.0-2.0 大多数任务可接受,> 2.0 说明有问题──
...
> **任务特定基准。**Em MMLU、HumanEval、GSM8K ou em seu conjunto de avaliação de auto-definição, o modelo quantificado é executado em comparação com o modelo original.
...
> **输出比较。**De dois modelos em um mesmo prompt 上生成回复并比较──LLM-as-judge (第十课) aqui muito eficaz──计算胜率: quantization model in how much proportion of prompt 上匹配或超过原始模型?
...
> **延迟和吞吐量。**Quantização existe para fazer o modelo mais rápido e mais barato. Messa por segundo token                                                                                                                                                                                                                                                    
...
> Modelo:FP8  quase gratuito INT4 代价 1-2  MMLU pontos mas a produção duplicou 内存 reduziu-se a um quarto  Este peso vale para quase todas as implementações 

### Números reais

FP16 a FP8 no H100: 30-50% de velocidade de inferência, < 0,1% de perda de qualidade. Esta é a quantização sem cérebro.

> H100 acima FP16 até FP8:30-50%                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
...
> FP16 até INT8 ((LLM.int8()): 2 倍内存减少,< 0.5% 质量损失──混合精度方法保持离群特征在FP16而其他全部量化到INT8──

FP16 a INT4 (GPTQ/AWQ): 4x redução de memória, perda de qualidade de 1-3% dependendo do modelo e método.

> FP16 até INT4 ((GPTQ/AWQ) 4: 倍内存减少,1-3% 质量损失,取决于模型和方法──使70B 模型可在单张48GB GPU 上运行──
...
> FP16 até INT4(GGUF Q4_K_M):3.5 倍内存减少,1-2% 质量损失──为 CPU 推理优化──Q4_K_M 的 70B 模型约40GB,在64GB M3 Max 上以 10-15 tokens/秒运行──
...
> FP16 até INT2:8 倍内存减少,5-15% 质量损失――仅适用于可容忍退化的特定狭窄任务――研究前沿,非通用生产就绪――
...
> Construir cada tipo de forma de representação de grau, ver claramente o que os símbolos, índices e números finais têm a ver com o efeito.
...
> 基本量化操作──逐张量对整个矩阵使用一个缩缩因子──逐通道对每行或每列使用一个缩缩因子──
...
> ∆a quantificação destruiu quantas informações¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬
...
> Em diferentes níveis de largura ((2、3、4、8、16)), a quantificação da mesma quantidade e a quantificação da qualidade de cada nível.
...
> VLLM Orig生支持 AWQ 和 GPTQ 模型── é usado durante o quadrado de matrizes para processar a quantificação, e é usado em divisões para controlar a concentração de atenção KV 缓存── em H100 para usar FP8 时,添加 `--dtype float8_e4m3fn`- Não.
...
> 本课产 出 `outputs/skill-quantization.md`, um quadro de decisão para escolher uma estratégia de quantificação correta.
...
1. Implemente quantização de grupo. Em vez de uma escala por canal, use uma escala por grupo de 128 pesos dentro de um canal. É o que GPTQ e AWQ realmente usam. Compare os tamanhos de grupo de 32, 64, 128 e 256 na mesma matriz de peso. Grupos menores dão melhor qualidade, mas mais custo de armazenamento para fatores de escala.
   Tradução do inglês para "GPTQ" em inglês: implementar divisão de peso quantificação. Não usar um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de envelhecimento por meio de um fator de fator de envelhecimento por meio de fator de fator de envelhecimento.
...
2. Construa um quantificador de precisão mista. Quantize as primeiras e últimas camadas de uma rede de várias camadas no INT8 enquanto quantiza camadas médias no INT4. Compare a qualidade de saída de ponta a ponta com a INT4 uniforme e a INT8 uniforme.
   Chinese Language Translation: Construir um sistema de medição de precisão mista. O primeiro e último nível da rede de várias camadas será mediado para INT4 e o segundo nível será mediado para INT4.
...
3. Implementar o estimador direto-a-cara (STE) para treinamento consciente de quantização. Insira operações de quantização/dequantização falsas no passo para a frente de uma rede de duas camadas simples treinada em uma tarefa de regressão. Compare a perda final entre um modelo treinado normalmente (então PTQ para INT4) versus um modelo treinado com QAT desde o início.
   Tradução do inglês para tradução do inglês: implementing quantization sensory training's direct通 estimator ((STE) ⋅ in training return task's simple two-tier network's forward-spreading in insertion pseudo quantization/ant quantization operation― comparar o modelo PTQ to INT4 após o treinamento normal com a perda final do modelo do treinamento QAT ⋅
