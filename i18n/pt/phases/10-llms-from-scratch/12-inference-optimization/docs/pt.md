...
> **【中文解读】**LLM 推理分两阶段:预填充(并行处理快点,计算密集) 和码(逐代币 生成,显存带宽密集) ――KV-cache、连续批处理、投机解码都是针对这两个阶段的优化――

> **【拓展：推理优化→生产部署】**VLLM, TensorRT-LLM, TGI e outros frameworks de avaliação foram implementados em KV-cache management, continuidade de batchamento, PagedAttention, etc.

> - Não .**【前置】**O primeiro é o primeiro período de desenvolvimento de um programa de ensino superior.

> - Não .**【类比】**LLM 推理 = 读书 + 写读书笔记。**Prefill**(处理 prompt) = 快速翻完书(并行扫所有代币,计算密集);**Decode**(生成) = 逐字写笔记 ((一次写一个字,显存带宽密集)**KV cache**= 翻书时做书签(记住每页关键信息,写笔记时不需要重翻);**continuous batching**= 同时给多人读书会服务 (→ "título em português")
...
- Implementar conceitos de batch contínuo e PagedAttention para maximizar a utilização de GPU sob solicitações simultâneas
  实现连续批处理和 PagedAttention 概念, maximizar a taxa de utilização da GPU em fase de desenvolvimento
- Comparar técnicas de otimização de inferências (cache KV, descodificação especulativa, atenção flash) e as suas compensações de rendimento/latença
  Comparar o teor de optimização de tecnologia (KV 缓存、投机解码、Flash Attention) e seu teor de

> **【中文解读】**O processo de processamento de dados é um processo de processamento de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de
...
> Você está em 4xA100 GPUs em Llama 3 70B── utilizador individual obtém cerca de 50 tokens/s── sentimento muito fluido── então 100 utilizadores visitam o terminal simultâneo── a capacidade de produção diminui para cada utilizador 3 tokens/s── utilizador de US$ 25.000 por mês  GPU                                                                                                                                                                                                                                                                                                                                                                                                                                                     
...
> O modelo em si não varia entre um usuário e 100 usuários. O mesmo peso, a mesma estrutura, a mesma matemática. A variação é como você trabalha a regulação. O simples raciocínio desperdiça 90% + de GPUs disponíveis.
...
> Não é uma questão de expansão, mas de regulação.$25k 推理账单降为服务相同流量的 $5k 账单的关键──
...
> VLLM em 4xA100-80GB 上服务 Llama 3 70B, baixa并发时达到约50 tokens/秒/user, através de contínua bateria de processamento e PagedAttention em 100并发请求维持15-25 TPS/user;;

## O conceito central.

> **【中文解读】**As duas fases do LLM 推理 具有截然不同的瓶:Prefill 阶段限于GPU FLOPS (FLOPS) 计算量 (计算量),Decode 阶段限于显存带宽 (显存带宽)  每次生成都要读取整个模型权重 (KV-cache)  KV-cache é a principal optimização 缓存已生成代币的关键/值 (Key/Value)  KV-cache é a principal optimização 缓存已生成代币的关键/值 (KV-cache) 缓存已生成代币的关键/值 (KV-cache) 缓存已生成代币的关键/值 (KV-cache) 缓存是最基本优化 缓存已生成代币的关键/值 (KV-cache) 缓存是最基本的优化 缓存的关键/值 (KV-cache) 缓存的关键/值 (KV-cache) 缓存的关键/值 (KV-cache) 缓存的关键/值 (KV-cache) 缓存的关键) 缓存的关键 (KV-de-de-de-code 缓存) 级的复数量 (C) 缓存的复数量 (C) 量) 缓存的复数 (C) 量 (C) 缓存的数量 (C) 量) 量 (C) 量 (C) 量 (C) 量) 量 (C) 量 (C) 量 (C) 量 (C) 量) 量 (C) 量 (C) 量 (C) 量 (C) 量) 量 (C)

> **【拓展：vLLM 和 PagedAttention】**A introdução de PagedAttention vLLM irá gerenciar o cache de KV como o virtual内存, usando o cache de página de tamanho fixo KV向量, resolvendo o problema de fragmentos de armazenamento. Isso torna a throughput de vLLM superior ao HuggingFace TGI 2-4 vezes.
...
> Cada pedido de Mestrado em Direito Jurídico tem duas fases diferentes.
...
> **预填充**处理整个输入快速――所有代币 已知,所以注意力可以在完整序列上并行计算――这是大型矩阵乘法GPU 核心保持忙碌――瓶是计算:硬件每秒能提供多少FLOPS──A100做 312 TFLOPS(BF16)──70B 模型上 4,096-token prompt 的预填充在单张A100 上约需要400ms──
...
> **解码**个别生成输出代币――每个新代币 关注所有前代币,但每次前向传播只产生一个代币――权力矩阵与预填时大小相同,但你使用单向量而不是矩阵与相乘――GPU 核心在微秒内完成,然后等下一批权重从内存到内存到内存带宽:你能以多快的速度将模型权重从HBM传输到计算单元――A100 有 2 TB/s 带宽――FP16 的 70B 模型是140 GB――读完整模型需要70ms这是单次解码步骤的下限度.
...
> 核心洞察:* O código de resolução é de tipo de memória intenso, porque você lê todo o modelo para gerar um único token*―. Cada melhoramento abaixo é reduzir a quantidade de leitura, ou aumentar o lote de leitura processada por token, ou completamente evitar a leitura―.
...
> Em cálculo de atenção, cada token de consulta se concentra em todos os principais e valores dos tokens anteriores. Sem cache, gerar token N precisa recalcular todos os N-1 个前 token de chave e valor de projeção.
...
> KV 缓存存储所有前代币的键和值投投影──生成代币 N 时,你只计算代币 N 的键和值,然后与缓存的代币 1 到 N-1 的 K/V 拼拼拼──
...
> Llama 3 70B de 128K de um único bloco de conversas                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              
...
> 静态批处理等一批 N 个请求到达,一起处理,等*所有*完成后再接受新请求――如果一个请求需要500代币 另一个需要10代币,短请求在完成后置490个解码步骤――
...
> 连续批处理(也叫代级批处理) em qualquer pedido concluído imediatamente, um novo pedido será inserido em lote.
...
> Tototúmulo de aumento depende de quanto a saída de longo variação.Totúmulo de longo variação, continuidade de loteamento é igual ao loteamento estático.Totúmulo de longo variação, continuidade de loteamento pode fornecer 2-5 vezes a touto, pois GPU 槽位 ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇ ̇  ̇ ̇ ̇   ̇ ̇   ̇ ̇           ̇                        
...
> Cada pedido de KV 缓存 é um bloco de contínuo内存── com o pedido de chegada e saída, o cache se fragmenta assim como o RAM fragmentação no sistema operacional── 4K token de pedido precisa de 1.28 GB 连续空间── mesmo que no total 2 GB 空, não pode haver 1.28 GB *连续*空间── você vai perder o cache ou recusar o pedido──
...
> PagedAttention( proveniente de vLLM) vai aplicar o tipo de sistema operacional virtual de memória para KV 缓存── não para cada pedido distribuir um bloco continual, mas distribuir um "página" fixo de tamanho fixo (normalmente por página 16 tokens)── página pode ser localizada em qualquer local da memória do GPU físico── página de cada pedido será mapeada para a posição da página física──
...
> O que é que é lento é porque é uma ordem de gerar um token, de volta para trás, gerar o próximo... mas se você puder adivinhar os próximos 5 tokens, então, uma vez verificar tudo?
...
> 投机解码使用小而快的**草稿模型**生成 K 个候选标签──**目标模型**Então, em uma única fase de pré-propagação, você aceita todos os K 个候选人. Se ele estiver na posição j não concordar, você aceita o token 1 a j-1 e não perde o restante.
...
> A velocidade depende**接受率** Projetos de modelo                                                                                                                                                                                                                                                            
...
> 投机解码是*数学精确*输出分布与目标模型的分布完全相同――它不是近似――验证步骤确保每个接受的代币 恰恰具有目标模型会分配的概率――
...
> Antes de ser aprovado, o sistema de registro de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados de dados
...
> Para todos os pedidos compartilhados de 2.000 tokens, o cache anterior eliminou o preenchimento de cada pedido em cerca de 400ms. Em 100 pedidos/segundo, a economia de 40 segundos por segundo de GPU foi calculada em mais de um GPU.
...
> 本课产出:
> - `outputs/skill-inference-optimization.md` diagnóstico e otimização de competências de Mestrado em Direito
...
1. Modifique o perfil do cache KV para comparar a quantização do cache FP16 vs FP8 vs INT4 KV. Para Llama 3 70B em contexto 4K, calcule o máximo de usuários simultâneos para cada um em 4xA100-80GB. Quantização de KV para INT4 deve ser aproximadamente 4x a capacidade do usuário.
   Modificar KV 缓存分析器 Comparar FP16 vs FP8 vs INT4 KV 缓存量化── Para 4K 上下文的Llama 3 70B,计算 4xA100-80GB 上每种格式最大并发用户数──INT4 KV 量化应大约4倍用户容量──
...
2. Extenda o simulador de lotes contínuos para rastrear a utilização da GPU (fração de slots de lotes preenchidos por etapa). Utilização de parcelas ao longo do tempo para lotes estáticos e contínuos com 50 solicitações cujos comprimentos de saída seguem uma distribuição Pareto (forma = 1.5, escala = 20).
   China文翻译:扩展连续批处理模拟器跟踪 GPU utilization rate(per step批次槽位填充比例)。绘制静态和连续批处理随时间的利用率图,50 个请求的输出长度遵循帕累托分布(形=1.5,规模=20)。连续批处理应维持 >80%利用率──
...
3. Implementar uma versão de atenção de consulta agrupada (GQA) do cache KV onde `num_kv_heads < num_query_heads`Llama 3 70B utiliza 64 cabeças de consulta, mas apenas 8 cabeças de KV. Calcule a economia de memória vs atenção completa de cabeças múltiplas (8 vezes a redução do tamanho do cache de KV).
   Tradução do inglês para tradução do inglês: implement分组查询注意力(GQA)`num_kv_heads < num_query_heads`❖ Llama 3 70B Utilizando 64 cabeças de consulta mas apenas 8 cabeças de KV ⋅ Calculação em relação ao total de cabeças de atenção ⋅ KV 缓存大小减少8倍) ⋅
...
4. Construa um cache de prefixos que use o despejo LRU. Configure o máximo de entradas para 500 e gerar 1.000 solicitações onde 60% compartilham um dos 5 prefixos comuns. Messa a taxa de acidentes e compare com o cache ilimitado. Com um bom despejo, a taxa de acidentes deve permanecer acima de 55%.
   Construir usando LRU 淘汰的前缓存── configurar max_entries 为 500, gerar 1.000 个请求, 60% deles 共享 5 个常见前之一──测量命中率并与无限缓存比较──良好淘汰下命中率应保持在 55% 以上──
...
5. Expanda o simulador de decodificação especulativo para implementar especulação baseada em árvore (estilo EAGLE-2). Em vez de uma única cadeia de tokens de projeto K, gerar uma árvore de candidatos (por exemplo, 2 ramos em cada um dos 3 níveis = 8 candidatos de folha). Comparar os tokens totais aceitos por rodada de verificação vs especulação linear.
   Não é uma cadeia única de tokens de K 个草稿, mas gerar um candidato. Por exemplo, 3 níveis por nível 2 分支 = 8 个叶选) ⋅ Comparar cada rodada de teste de tokens aceites 总数与线性投机──
