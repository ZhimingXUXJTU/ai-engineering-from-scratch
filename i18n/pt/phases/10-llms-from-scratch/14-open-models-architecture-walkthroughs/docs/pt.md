# Modelos abertos: Arquitetura Passagens .
...
> **【中文解读】**Você em quarto curso construiu GPT-2 Small. Em 2026 anos, o modelo de código aberto fez apenas cinco modificações: RMSNorm  substituir LayerNorm. SwiGLU  substituir GELU. RoPE  substituir código de posição de aprendizagem. GQA/MLA  substituir toda a atenção.

> **【拓展：DeepSeek架构】**A nova tecnologia é a de um modelo de desenvolvimento de grandes dimensões, que pode ser usado para a construção de um sistema de controle de dados.

> - Não .**【前置】**學本節前請先掌握:Fase 10·04(Pre-Training Mini GPT) 理解原始 GPT 架构;Fase 10·05(Scaling);Fase 10·12(Inference Optimization) 理解 KV cache 才能懂 MLA──本节是后续 15-22 节 (架构创新详解) 的概览──

> - Não .**【类比】**Olha 2026 开源模型 = 看不同品牌的电动车;;Llama 3 = 特斯拉(务实,GQA + RoPE 标准配置);DeepSeek-V3 = 比亚迪;;堆料王,MLA + MoE + MTP + DualPipe 全套);Mixtral = 大众 ID(MoE 老牌);Qwen = 小(多尺寸覆盖);Gemma = 沃尔沃(精简安全);;Bottom layer are Transformer(电动车底盘),差异在 5-6 个关键模块──
...
- Nomear a mudança arquitetônica específica feita por cada modelo em relação ao GPT-2 Small e justificar a partir dos primeiros princípios
  Explicar cada modelo em relação a GPT-2 Pequenas modificações de estrutura específica, e argumentar a partir dos princípios básicos
- Contagem de parâmetros de computação, tamanho do cache KV e memória de ativação para qualquer modelo aberto apenas a partir de sua configuração
   apenas a partir de configuração calcular o número de parâmetros de qualquer modelo de código aberto  KV  cache grandeza e activar memória
- Escolha o modelo aberto certo para um alvo de implantação dado o atraso, memória e restrições de capacidade
  De acordo com o atraso, a memória e a capacidade, para a implantação, o objetivo é escolher um modelo de código aberto adequado.
...
> Você escreveu 350 linhas numpys na quarta aula sobre um modelo de forma GPT-2  Llama 3 405B tem um relatório técnico de 200 páginas♦ O seu intuito diz que são coisas completamente diferentes♦ Na verdade não  200 páginas descrevem o mesmo objeto, apenas cinco ou seis modificações com movimentos suficientes, adicionadas a mil detalhes sobre a implementação da expansão♦ Estrutura  embutidos  transformadores  blocos  atenção  MLP  regeneração  saída  sem mudança♦
...
> Este curso é uma diferença. Para cada família de modelos de origem aberta, nós listamos em relação ao GPT-2 até o final do que mudou, por que mudou, o preço é o que.
...
> **【中文解读】**O estudo foi realizado em uma área de estudos de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa de pesquisa em pesquisa em pesquisa de pesquisa em pesquisa em pesquisa de pesquisa em pesquisa em pesquisa de pesquisa em pesquisa em pesquisa em pesquisa em pesquisa de pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa em pesquisa

> **【拓展：架构演进的趋势】**Tendências de arquitetura de 2024-2025: RoPE  rotativo de posição codificação substituído pelo código de posição absoluta, GQA 分组查询注意力) reduziu o cache KV 大小, SwiftGLU 替代 ReLU 升级性能, RMSNorm 替代 LayerNorm 降低计算开销──MoE 混合专家) através de activação rara reduzir o custo de sugestão,DeepSeek-V3 MoE por cada vez apenas activa cerca de 37B/671B 参数──
...
> 本课产 出 `outputs/skill-open-model-picker.md` dado o objetivo de implantação (GPU 类型,显存,上下文长度,延迟预算) e de atribuição de tarefas (Chatting,代码,推理,长上下文), ele recomenda o modelo de código aberto, o esquema de quantificação da décima primeira classe e o esquema da décima segunda classe, e dá uma raciocínio clara de seis arquiteturas rotativas.
...
2. O DeepSeek V3 usa 256 especialistas com roteamento de 8 principais. Calcule a relação de especialistas ativados com especialistas totais e compare com o Mixtral 8x7B com o top-2 de 8. O que a mudança de escassos (25%) para mais densos escassos (3%) implica sobre a capacidade por FLOP?
   O que significa a mudança para cada capacidade de FLOP de 255% para 3%?
...
3. Compute o cache KV para Llama 3 405B em contexto 128k em FP8 e BF16. Em FP8 é metade do número BF16. Quantas sequências paralelas você pode servir em um único nó 8xH100 (80GB cada = 640GB total, menos memória de peso)?
   O FP8 é uma parte da BF16 e é uma parte da BF16.
...
5. Encontre um modelo aberto de fronteira recente que foi lançado após esta lição ter sido escrita. Identifique qual dos seis botões que ele escolheu e se introduziu um sétimo botão. O currículo vai sentir-se desatualizado no momento em que uma nova arquitetura lança - o objetivo é atualizar sua mesa sem reconstruir seu modelo mental.
   Chinese Translation: encontrar um artigo sobre o modelo de código aberto mais recente publicado após a redação do livro. Identificar quais dos seis rotos ele escolheu e se introduziu o sétimo. O objetivo do curso é renovar o seu quadro e não reconstruir o modelo mental.
