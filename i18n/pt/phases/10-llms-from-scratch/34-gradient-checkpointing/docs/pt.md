...
> **【中文解读】**O que é necessário para a transmissão é manter todos os elementos activados em meio.

> **【拓展：梯度检查点→大模型训练】**梯度检查点是训练大模型的标配技术──PyTorch's torch.utils.checkpoint、DeepSpeed's activation checkpointing 都是这一思想的实现──在128K 上下文训练中,选择性检查点可以节省60%+的显存──

> - Não .**【前置】**學本節前請先掌握:Fase 10·04(Pre-Training GPT);反向传播算法;GPU 显存层次(SRAM vs HBM)。本节是训练超大模型 + 长上下文时的关键技术。
> - Não .**【类比】**梯度检查点 = 出差只带必要文件──反向传播── 工作完成后回顾所有材料;不检查点 = 把出差路上所有的材料都背着(占满行李箱);全检查点 = 只带护照,需要时再印(重算成本高);选择性检查点──只扔可重算的(如草稿),保留难重建的(如合同) 平衡存储和速度显――
...
>                                                                                                                                                                                                                                                               `d`、序列长度 `L`、 volume `B`De um nível, é quase de cada nível.`12 * B * L * d`- Não.
...
> Para o`d=8192, L=8192, B=1`,BF16 下每层800 MB──64 层模型的激活值为51 GB这还没有乘以微批次大小,还没有加重注意软max 中间值(每头`L^2`), ainda não se planeja a inserção de uma parte do volume de operações.
...
> O estado do sistema de otimização pode ser de 80 GB, mas o valor de ativação faz com que você exceda.
...
> **【中文解读】**梯度检查点(Gradient Checkpointing) com o cálculo de mudança de armazenamento: antes de divulgação não conservar o valor ativo médio, depois de divulgação re-computar a necessidade de divulgação.

> **【拓展：梯度检查点在训练中的关键作用】**O ponto de verificação de gradiente é a técnica de sinalização do modelo de treinamento. Para o Llama 3 70B, não há ponto de verificação, cada amostra precisa de cerca de 16 GB de valor ativado de armazenamento, reduzido para cerca de 5 GB após o início.
...
> **【拓展：梯度检查点与 FSDP 的配合】**Os pontos de verificação de gradiente são geralmente usados em conjunto com o FSDP/ZeRO. Os pontos de verificação de gradiente são utilizados em parâmetros e em estado de optimização, os pontos de verificação de gradiente reduzem o valor de ativação e os dois se complementam.
...
> 本课产 出 `outputs/prompt-activation-recompute-policy.md` um modelo de configuração aceitação (level number 隐藏维度 序列长度 批次) e GPU disponível (in-stock并输出) de cada nível de estratégia de cálculo (无 / 选择性 / 完全 / 卸载)
...
1. Verifique a correcção.`model_forward`+ `model_backward`(ativações completas) vs `model_forward_checkpointed`+ `model_backward_checkpointed`Os gradientes dos parâmetros devem ser idênticos à precisão da máquina.
   Tradução do inglês: 验证正确性.`model_forward`+ `model_backward`(完整激活)`model_forward_checkpointed`+ `model_backward_checkpointed`(分段) ◊ A graduação dos parâmetros deve ser de acordo com a precisão do aparelho.

2. Dimensão do segmento de varredura `k`de 1 a `L`- Traçar o plano de cabeça e memória.
   Tradução do português:将段大小`k`De 1 a 1 .`L`◊ desenhar FLOP 开销和内存── encontrar curvas de curvas──

3. Implementar um ponto de verificação seletivo: armazenar a entrada do módulo de atenção, mas não os seus intermediários.
   Tradução do inglês para tradução do inglês: implementar selectividade checkpoint: storage attention module input but not preserved middle result;; medida 32 层模型在 seq=8192 下与全层检查点的 FLOP 开销比较;;
...
4. Adicionar descarga. Salvar entradas de segmento para um "buffer de CPU" simulado (uma lista separada). Medir "PCIe largura de banda" como bytes/tempo e encontrar o ponto de equilíbrio entre descarga e recomputada.
   Chinese: 添加卸载. 将段输入保存到模拟的"CPU缓冲区" (CPU 缓冲区) 单独的列表)  以字节/时间测量 (PCIe 带宽) 并找到卸载和重计算之间的平衡点.
...
5. Marque de referência um verdadeiro transformador PyTorch com e sem `torch.utils.checkpoint`. Medir a memória (via `torch.cuda.max_memory_allocated`) e tempo de passagem.
   Tradução do inglês para tradução do inglês para tradução do inglês para tradução do inglês para tradução do inglês para tradução do inglês para tradução do inglês para inglês para tradução do inglês para o inglês para tradução do inglês para o inglês para tradução do inglês para o inglês para tradução do inglês para o inglês para o inglês para tradução do inglês para o inglês para o inglês para o inglês`torch.utils.checkpoint`进行基准测试──测量内存(通过 `torch.cuda.max_memory_allocated`E o tempo é muito longo.
