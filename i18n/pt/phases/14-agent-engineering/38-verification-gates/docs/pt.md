# Verificação Gates Verificação controlada
...
> - Não .**【前置】**學本節前請先掌握:Fase 14·33(Regras) 、Fase 14·36(Cálculo) 、Fase 14·37(Feedback) 3 个前置都做完才能学验证门控。
> - Não .**【类比】**O agente próprio diz "I do do complexo" não conta o número de gate  independent check" test se passa scope 否守住、规则是否满足、diff 是否合理"──门控失败,任务不算完成──这是阶段14·39(Reviewer Agent) 简化版gate 是脚本,reviewer 是 LLM──
...
> 验证门(Verification Gates) em Agente 工作流的关键节点设置检查点──每个门验证前一步的输出是否满足要求,不满足则触发修复流程──
...
> 工作台的修复是一个单一的验证门控,读取 Agente 已产生的工件并做出判断──门控是确定性的──门控在版本控制中──门控连接CI──Agent 无法它──
