# O banco de trabalho num repositório real.
...
> - Não .**【前置】**學本節前請先掌握:Fase 14·32-40 全部(Workbench 系列 9 節) ∼本節是工作台的实战综合在真实小项目上跑-only vs workbench-guided对比──
> - Não .**【类比】**O primeiro é o primeiro tempo em que um carro saiu de casa e você não pensa em problemas.
...
> A demonstração de tarefas de brinquedo não convence ninguém. A razão da plataforma de trabalho é executar uma tarefa de sentido real em um armazém de sentido real, com menos falhas, menos voltas e na próxima reunião, que podem ser usadas para a produção de embalagens.

Esta lição envia o repo real e executa a mesma tarefa através de ambos os canais.

> Esta aula fornece um armazém de sensação real, e atravessa dois canais de operação da mesma tarefa. O resultado é um relatório de comparação anterior e posterior que pode ser entregue ao duvidoso.


> **【中文解读】**Esta secção apresenta as melhores práticas e estratégias de implementação do ambiente de produção.
...
> **【中文解读】**A empresa de gestão de dados e de dados (Agen 工作台) é uma empresa de gestão de dados e de dados que é responsável por criar e criar um sistema de gestão de dados e de dados.
...
> Real Warehouse Workbench colocará o Agente em um real código de armazenamento para realizar testes e melhorias.
