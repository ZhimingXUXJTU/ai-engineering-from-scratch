...
> - Não .**【前置】**學本節前 請先掌握:Fase 14·38(Verification Gate) ⋅本节是 gate of LLM 版用第二代理 审查第一代理的产出──
> - Não .**【类比】**Construtor + Reviewer = Autor + Editor, autor escreveu o seu primeiro livro facilmente auto-fecundado, "Eu acho muito claro", editor em leitura visual, "Este é o nome do autor".
...
> Você fez o Agente corrigir um bug. Editou quatro documentos, executou testes, completou o relatório.`passed: true`Dois dias depois descobriste que a reparação só resolveu metade do bug.
...
> O auditor pergunta se a aceitação é necessária, mas não é suficiente. O avaliador pergunta se a aceitação é impossível: se isso resolveu o problema corretamente? se o alcance foi ampliado sem marcação? se a suposta supostação de que o trabalho deveria ser questionado foi registrada? se a plataforma de trabalho permanecerá em um estado de acesso à próxima reunião?


> **【中文解读】**O Agente de revisor é um Agente independente que é especializado em revisar outros Agentes. Ele verifica: 1) Qualidade do código: leitura; 2) Realidade; 2) Erro logico; 3) Condições de fronteira; 3) Segurança: Introdução a lacunas; • Fugas de dados sensíveis.
...
> 审查者 Agent (revisor) Especialmente revisar as saídas de outros agentes.
