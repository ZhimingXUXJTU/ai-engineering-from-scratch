# Inicialização Escrits para Agentes
...
> - Não .**【前置】**学本节前请先掌握:Fase 14·32(Minimal Workbench)、Fase 14·34(Repo Memory)。本节讲"boot脚本"让代理不用每次冷启动重新探索 repo。
> - Não .**【类比】**O primeiro dia de trabalho do novo empregado não precisa de tocar-se "Test How to Run"",Depending on what's installed"",Environmental Change is what" (O primeiro dia de trabalho não precisa de tocar-se)
...
> 打开一会话──Agent 猜测 Python 版本──猜测测试命令──列出仓库根目录五次来找到入口点──试导入一个未安装的包──问用户配置文件在哪里──Quando realmente faz o editing, 10 000 tokens 已经花在本应的脚本的设置工作上──
...
> Repair é um script de inicialização, executado antes de um agente fazer qualquer coisa, e escrito para um agente ao iniciar.`init_report.json`- Não.


> **【中文解读】**O primeiro livro foi publicado em 2003 e publicado em The New York Times, em 15 de janeiro de 2007. O primeiro livro foi publicado em 2003 e foi publicado em The New York Times, em 15 de janeiro de 2007.
...
> O script de inicialização é para o Agente, que define as condições de inicialização.
