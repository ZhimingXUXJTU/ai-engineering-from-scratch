...
> Normalmente, é fácil de definir.
...
- Implementar métodos de detecção de anomalias florestais de zero em termos de pontuação Z, RQI e isolamento
  Desde zero a realização de Z-score, IQR e isolamento floresta  métodos de inspecção anormal
- Distinguir entre anomalias pontuais, contextuais e coletivas e selecionar o método de detecção adequado para cada
  区分点异常、上下文异常和集合异常, para cada seleção de métodos de exame adequados
- Explicar por que a detecção de anomalias é enquadrada como modelagem de dados normais em vez de classificação de anomalias
  Explicar por que os exames anormais são criados para construir dados normais e não para classificar as anormalidades
- Comparar a detecção de anomalias não supervisionadas com a classificação supervisionada e avaliar a compensação entre a cobertura de anomalias novas e a precisão
  Comparar a pesquisa de anomalias sem supervisão com a classificação de supervisão, avaliar a taxa de cobertura e a taxa de precisão das novas anomalias


> **【中文解读】**
> 异常检测找出不同数据点──信用卡欺诈检测、设备故障预警、网络入侵检测都依赖于──Isolation Forest 和 One-Class SVM é um método comum──sklearn 中的 IsolationForest──

> **【拓展：异常检测在金融和网络安全中的核心应用】**
> O sistema de detecção de fraude em tempo real da Visa processa cerca de 76.000 transações por segundo, usando métodos mistos de detecção de irregularidades + supervisão de aprendizagem, em cerca de 150 milissegundos para julgar se é fraude. O sistema de segurança da rede do Google usa detecção de irregularidades para detectar ataques de DDoS e comportamento de registro anormal. O sistema de gestão de baterias da Tesla usa detecção de anomalia para antecipar falhas de baterias. O desafio central do teste de irregularidades é a extrema desequilíbrio. A taxa de fraude geralmente é inferior a 0,1%, tornando a aprendizagem de vigilância difícil de usar diretamente.
...
> Uma tarjeta de crédito no meio-dia 2 horas em Nova York, depois 2:05 horas em Tóquio.
...
> Estas são as coisas que não são normais. Descobrir-as é muito importante.
...
> O desafio é: você raramente tem uma marca de marca abnormalizada. A fraude ocupa apenas 0,1% das transações. A falha de equipamentos ocorre apenas algumas vezes por ano. Você não pode treinar classificação padrão, porque na categoria "abnormalidade" há quase nada a aprender. Mesmo com algumas marcas, as anomalias que você vê também não são o único tipo que você encontrará.
...
> O teste de anomalia transformou o problema. Não aprendemos o que é anormal, mas aprendemos o que é normal. Qualquer desvio de algo normal é suspeito.
...
- **Point anomalies.**Um único ponto de dados incomum, independentemente do contexto.$50,000 from an account that normally spends $- 50 anos.
  Por isso, não importa o que seja, não há dados normais.
- **Contextual anomalies.**Um ponto de dados incomum dado o seu contexto. Uma temperatura de 90 graus é normal no verão, anormal no inverno.
  Nota de dados em inglês: "Nota de dados em inglês" (90°)
...
> A maioria dos métodos de exame de pontos de anomalia.
...
> Em classificação padrão, você tem duas categorias de etiquetas. Em exames anormais, você normalmente encontra uma das três situações seguintes:
...
1. **Fully unsupervised.**Não há rótulos, encaixas o detector em todos os dados e espero que as anomalias sejam raras o suficiente para não corromper o modelo "normal".
   完全无监督──完全没有标签── Você está em todos os dados, esperamos que as anomalias sejam pequenas o suficiente para não destruir o modelo "normal"──
2. **Semi-supervised.**Você tem um conjunto de dados limpo de dados normais apenas. Você encaixa neste conjunto limpo e pontua tudo o resto. Esta é a configuração mais forte quando possível.
   半监督──you only have one clean 常常的正 常的数据集──you fit on this clean 净集合, then to all other data 打分──is this the most powerful setup when possible──
3. **Weakly supervised.**Tem algumas anomalias rotuladas. Usa-as para avaliação, não para treinamento. Treina sem supervisão, depois mede precisão/recall no subconjunto rotulado.
   弱监督──you have some markings of anomalies──will use them for assessment rather than training──无监督训练, then measure precision rate/callback rate on the markings collection── você tem algumas anomalias de marcas, use-as para avaliação e não para treinamento, use-as para treinamento, use-as para treinamento, use-as para avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, use-as para a avaliação, e use-as para a avaliação.

A principal ideia: a detecção de anomalias é fundamentalmente diferente da classificação.

> 关键洞察: A análise anormal é fundamentalmente diferente da classificação.
...
- Taxa de falsos positivos mais elevada (não tudo o que é incomum é ruim)
  Mais alta taxa de falsidade (não tudo o que não é normal é mau)
- Mais robusto para a mudança de distribuição
  Para a distribuição de mudanças
...
> Na prática, o melhor sistema combina dois: inspeção sem supervisão para ampla cobertura, supervisão de modelos para conhecidos tipos de alta prioridade anormal, revisão artificial para modelos de duas possibilidades.
...
> O método mais simples é calcular o valor médio e o diferencial padrão de cada característica.
...
> O valor padrão é de 3,0 ((99,7% da distribuição de alta data normal caiu em 3 padrões de diferença)

**Strengths:**Simples, rápidos, interpretáveis ("este valor é 4,5 desvios padrão do normal").

> **优势：**简单――快速――可解释(" este valor é de 4,5 标准差")
...
> **劣势：**假设数据服从正态分布――对训练数据中的异常值敏感异常值会偏移平均值并膨胀标准差,使它们更难被检测) 
...
> **适用场景：**Data grande de apresentação de horário de monitorização de características únicas.
...
> **失效场景：**Dois escritórios têm diferentes temperaturas de base) 偏斜数据 (o volume de transações de 1000 dólares é raro, mas não é raro) 訓練集中有异常值的数据──

### Método de RIC

É mais robusto do que o Z-score, usa o interquartile em vez de desvio médio e padrão.

> A diferença entre o valor médio e o padrão é mais elevada.
...
> **优势：**Para o valor anormal de um índice de percentagem não é influenciado pelo valor extremo.
...
> **实践提示：**O fator de 1,5 no IQR é o de correspondência à caixa de imagem. O fator de 1,5 é o de potencial anormalidade.
...
> 关键洞察: as anomalias são muito pequenas e diferentes. Em uma divisão de dados, as anomalias são mais fáceis de separar.
...
> **为什么有效：**O ponto comum está localizado em uma área densa. É necessário muitas divisões arbitrárias para separar um ponto de seus vizinhos.
...
>  Número de pontos anormais baseado na média de comprimento de caminho entre todas as árvores, regroupado pela longitude de caminho esperada da árvore de busca binária:
...
> Entre eles `c(n)`É o tempo de espera de um modelo. O tempo de espera é de 0,005 segundos.
...
> **优势：**无分布假设──适用高维──扩展性好样本量亚线性,因为 cada árvore usa um filhote de 采样)──处理混合特征类型──

**Weaknesses:**Luta com anomalias em regiões densas (efeito de enmascaramento).

> **劣势：**難處理密集區域異異 (異異) 遮蔽效應 (遮蔽效應)                                                                                                                                                                                                                                                  
...
- `n_estimators`Numero de árvores. 100 é geralmente suficiente. Mais árvores dão pontuações mais estáveis, mas com uma computação mais lenta.
  `n_estimators`Número de árvores: 100, normalmente suficiente, mais árvores fornecem uma percentagem mais estável, mas o cálculo é mais lento.
- `max_samples`O número de amostras por árvore. 256 é o padrão no papel original. Valores menores tornam árvores individuais menos precisas, mas aumentam a diversidade. A sub-sampulação é o que torna a Floresta de Isolamento rápida - cada árvore vê uma pequena fração dos dados.
  `max_samples`O estudo original afirma que 256... menor valor faz com que um único árvore não seja preciso, mas aumentem a diversidade...
- `contamination`: Fracção esperada de anomalias. Utilizada apenas para a fixação do limiar.
  `contamination`O número de variações é de aproximadamente 0,00%.
...
> LOF comparará a densidade local em torno de um ponto com a densidade do vizinho em torno de um local.

**How it works:**
1. Para cada ponto, encontrar os seus vizinhos mais próximos k
   Para cada ponto, encontrar seu próximo vizinho
2. Calcule a densidade de acessibilidade local (quão densa é a vizinhança)
   计算局部可达密度 (número de áreas com densidade)
3. Compare a densidade de cada ponto com a densidade de seus vizinhos
   Comparar a densidade de cada ponto com a densidade de seus vizinhos
4. Se um ponto tem uma densidade muito menor que os seus vizinhos, é um outlier
   Se a densidade de um ponto é muito menor que a de seu vizinho, é um ponto anormal.

**LOF score:**
- LOF próximo a 1,0 significa densidade semelhante à dos vizinhos (normal)
  LOF  aproximado de 1.0 significa similar à densidade vizinha
- LOF superior a 1,0 significa menor densidade do que os vizinhos (potencialmente anômalo)
  LOF maior que 1.0 significa densidade inferior ao vizinho (((可能异常)
- LOF muito maior que 1,0 (por exemplo, 2,0+) significa densidade significativamente menor (anomalia provável)
  LOF 远大于1.0 (como 2.0+) significa densidade significativamente menor (três vezes é anormal)
...
> "Local" é fundamental. Considere um conjunto de dados com duas classes de aglomeração: um aglomeração concentrada de 1000 pontos e um aglomeração de 50 pontos.

**Strengths:**Detecta anomalias locais (pontos incomuns em sua vizinhança, mesmo que não sejam incomuns globalmente).

> **优势：**检测局部异常 (extensão de pontos não comuns em áreas vizinhas, mesmo que não sejam comuns em toda a área) 
...
> **劣势：**Em grande data collection velocidade lenta (n^2)) ⋅ para a seleção de k ⋅ em muito alta dimensão não é bom (n^2) ⋅
...
>  avaliar os exames anormais mais difíceis do que avaliar os classificadores:

- **Extreme class imbalance.**Com anomalias de 0,1%, prever "normal" para tudo dá 99,9% de precisão.
  Em uma taxa de desordem de 0,1% em uma classe extrema, todas as previsões são normais, com 99,9% de precisão.
- **AUROC is misleading.**Com um desequilíbrio pesado, o AUROC pode parecer bom mesmo quando o modelo não apresenta a maioria das anomalias nos limiares práticos.
  AUROC 具有误导性──在严重不平衡时,即使模型在实用值下错过了大部分异常,AUROC parece ainda não ser errado──
- **Better metrics:**Precision@k (dos itens marcados com o sinal k superior, quantas são as anomalias reais), AUPRC (área sob curva de recall de precisão) e recall a uma taxa fixa de falso positivo.
  Melhor indicador:Precision@k(排名前 k 个标项中有多少是真正的异常) 、AUPRC(精确率-召回率曲线下面积) 和固定假阳性率下召回率──
...
> Na prática, o teste anormal segue os seguintes processos de trabalho:

1. **Collect baseline data.**Idealmente, um período em que você sabe que não há anomalias (ou muito poucas).
   收集基线数据── idealmente, é um período de períodos anormais que você sabe não existem.
...
> 管线永远不会"完成"―― dados distribuídos desviados, novos tipos de anomalias aparecem, 值需要调整――将异常检测视为一个活系统,而不是一次性模型――

## Construí-lo e realizei-o.

> **【中文解读】**
> A partir de zero, três métodos de detecção de anomalias são realizados: Z-score (baseado em valores médios e diferença de padrão, adaptado a dados de distribuição de estado normal)  IQR (baseado em quatro pontos de distância, sobre um valor anormal)  Floresta de isolamento (随机选择特征和分点隔离数据点,异常点平均需要更少的分离次数)  A floresta de isolamento é o método de detecção de anomalias mais comum de uso na indústria, sem supervisão e sem supervisão.

> **【拓展：异常检测在 AIOps 和制造业中的应用】**
> O Microsoft Azure Monitor usa exames anormais para detectar automaticamente as anomalias de desempenho de serviços em nuvem; a Netflix usa exames anormais para monitorar os vários indicadores do serviço de streaming ([[tardo]], taxa de erro, etc.), exames diários de bilhões de pontos de dados; a Fujicom utiliza exames anormais na linha de produção para detectar falhas de dispositivos, reduzindo o tempo de paragem em 30%. O desafio fundamental do exame anormal é controlar a taxa de erro de comunicação.
```figure
f3-anomaly-fence
```
...
> Desde o zero, a construção é feita de forma individual.
...
> A extensão do caminho separado de um ponto determina o seu número de pontos anormais.
...
> 归一化因子 `c(n)`é a longitude do caminho de busca esperado de n 个元素的二叉搜索树中不成功搜索的期望路径── é igual a `2 * H(n-1) - 2*(n-1)/n`, entre os `H`Esta classificação garante que os números de dados entre diferentes conjuntos de dados sejam comparáveis.
...
1. **Single cluster with outliers.**Um aglomerado Gaussiano 2D com anomalias injetadas longe do centro.
   单聚类加异常值――a 2D 高聚类, injectada em centros distantes em abnormalidades―― todos os métodos neste cenário devem funcionar
2. **Multimodal data.**Três aglomerados de tamanhos e densidades diferentes. Os pontos entre os aglomerados são anómalos.
   Dois pontos entre as colunas de tamanho e densidade diferentes foram extraordinários.
3. **High-dimensional data.**50 características, mas as anomalias diferem em apenas 5 delas.
   Alto nível de dados: 50 características, mas as anomalias diferem apenas em 5 características:

Cada demonstração compara todos os métodos usando precisão, recall, F1, e Precision@k.

> Cada apresentação usa a precisão, a recitação, a F1 e a precisão, comparando todos os métodos.

## Use-o com o framework implementado.

Com sklearn (usando implementações de biblioteca, não a partir do zero):

> Utilize sklearn(使用库实现,非从零实现):
...
> Atenção .`contamination`setup expected of abnormal proportion── correct setting is important  too low  miss check abnormal, too high  err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err err
...
> `anomaly_detection.py`O código central é comparado com o do zero realizado em dados similares.
...
> Os dois produzem o mesmo número de variações.`iso_5`标记前 5% , e `iso_10`Se você não sabe a taxa de anomalia real, normalmente não sabe, a contaminação será definida como "auto" e será utilizada diretamente com o número de pontos originais.

### Mudanças de controlo de segurança

Outro detector de anomalias não supervisionado que vale a pena conhecer.

> Outro teste de anomalia sem supervisão que vale a pena entender.
...
> `nu`参数近似异常比例――One-Class SVM é eficaz em pequenos conjuntos de dados, mas não pode se expandir para grandes dados ([[núcleo de matrizes]] em segundo crescimento)―
...
> O auto-editor é uma rede neuronal que aprende a comprimir e reedificar dados.

Isto é coberto na Fase 3 (Depth Learning), mas o princípio é o mesmo: modelo o que é normal, marca o que desvia.

> Esta é a fase 3 (dependência) da discussão, mas o princípio é o mesmo: construir o que é normal, marcado por desvio.
...
> Assim como o método de integração para melhorar o segmento (§§11), a combinação de vários exames anormais pode melhorar o exame.
...
2. Normalize as pontuações de cada detector para [0, 1]
   Para que cada testeiro seja dividido em [0, 1]
...
> Isso reduz a falsidade positiva, pois diferentes métodos têm diferentes padrões de falha.
...
> Integrar mais complexos, com base na estimativa de confiabilidade de cada testeiro, aumentando o poder de medição, se possível, em conjuntos de testes de anomalias conhecidas)

### Considerações de produção

1. **Threshold drift.**A distribuição de dados muda, o limite fixo fica ultrapassado.
   Valor漂移── Com a distribuição de dados, o valor fixo Value becomes obsolete── Monitoring of the distribution并 periodically adjustment──
...
4. **Feature engineering.**As características brutas raramente são suficientes. Adicione estatísticas de rodamento, proporções, tempo desde o último evento e características específicas de domínio.
   Características de engenharia. Características originais são muito poucas.
5. **Feedback loop.**Quando os operadores examinam os itens sinalizados e os confirmam ou rejeitam, enviam-nos de volta ao sistema.
   Quando o operador confirma ou exclui o sinal de inquérito, o sistema de entrada será controlado.

## Envia-o . Produto .

Esta lição produz:
- `outputs/skill-anomaly-detector.md`- uma habilidade de decisão para escolher o detector certo
  `outputs/skill-anomaly-detector.md` 选择合适检测器's habilidades de decisão
- `code/anomaly_detection.py`-- Z-score, IQR e Isolação Forest desde zero, com comparação sklearn
  `code/anomaly_detection.py` Desde zero a realização do Z-score, IQR e Isolação Forest,
...
- **Equipment maintenance.**Um falso alarme significa um desligamento desnecessário que custa muito .$50,000. A missed failure means a $500 mil reparos, fixa o limite para equilibrar os custos.
  设备维护――误报意味着不必要的停机,成本50.000 美元――漏检故障意味着500.000 美元的维修――configurar值以平衡这些成本――

Em ambos os casos, o limite ideal depende da relação de custos entre falsos positivos e falsos negativos.

> Em ambos os casos, o valor ideal depende da relação de custo entre falso positivo e falso negativo.
...
1. **Batch training, online scoring.**Treinar o modelo periodicamente (diariamente, semanalmente) com dados normais recentes.
   批量训练,在线打分──定期(每天、每周) em modelos de treinamento em dados normais de curto prazo──每新观测到达时打分──
2. **Feature computation must match.**Se você treinou com estatísticas de rolamento durante 30 dias, precisa de 30 dias de história para calcular características para uma nova observação.
   Características de cálculo devem ser combinadas. Se você usar 30 dias de treinamento de estatística de rolagem, você precisa de 30 dias de história para obter os novos caracteres de cálculo de observação.
3. **Score distribution monitoring.**Seguir a distribuição das pontuações de anomalia ao longo do tempo. Se a pontuação média deslocar-se para cima, ou os dados estão a mudar ou o modelo está obsoleto.
   Segurança de distribuição de dados. Se o número de dados estiver em mudança, o modelo já está ultrapassado.
4. **Explainability.**Quando marcar uma anomalia, diga porquê. Z-score: "Figura X é de 4,2 desvios padrão acima do normal". Isolação Floresta: "Este ponto foi isolado em 3,1 divisões em média (pontos normais tomam 8,5). "
   "Isolamento Floresta:":"该点平均被 3.1次分隔离(正常点需要8.5次) ").

## Exercícios.

1. **Threshold tuning.**Execute o detector de pontuação Z com limiares de 1.0 a 5.0 em etapas de 0.5.
   1. Em dados normais, insira-se uma proporção diferente de anomalias (1% ∼5% ∼10%) ∼ Comparar Z-score ∼IQR e taxa de precisão e recomposição da floresta de isolamento ∼
...
2. **Multivariate anomalies.**Criar dados 2D onde cada característica individual parece normal, mas a combinação é anómala (por exemplo, pontos longe da diagonal do cluster principal). Mostrar que o Z-score por característica perde estes, mas Isolation Forest os pega.
   2. 生成一个上下文异常数据集 (normal valor em inverno e diferente de verão)  mostrar simples Z-score em inverno, classificar o valor normal no verão como anormal  Adicionar a seguinte característica após reexame 
...
3. **LOF from scratch.**Implementar o Local Outlier Factor usando os vizinhos mais próximos de k. Compare com o LocalOutlierFactor de sklearn nos mesmos dados. Use k=10 e k=50 - como a escolha de k afeta os resultados?
   3. Construir a Foresta de Isolamento: treinar 10 árvores de isolamento, obter a duração média do caminho.
...
4. **Streaming anomaly detection.**Modifique o detector de pontuação Z para funcionar em uma configuração de streaming: atualize a média e a variância em execução à medida que novos pontos chegam (algoritmo online de Welford).
   4. Usar autoencoder 思路实现异常检测: treinar um simples modelo de construção, marcar o erro de construção elevado de pontos para as coisas estranhas.
...
> **【中文解读】**
> Avaliação de exames de anomalia com Precision@K(排名前 K 个可疑案例中有多少是真正的异常) e AUPRC(精确率-召唤率曲线下面积) Comparado com a precisión率 é mais significativo。Isolation Forest's core insight:
...
- [Liu et al., Isolation Forest (2008)](https://cs.nju.edu.cn/zhouzh/zhouzh.files/publication/icdm08b.pdf)-- o papel original de Isolamento Floresta
  [Liu et al.: Isolation Forest (2008)](https://ieeexplore.ieee.org/document/4781136)- Isolamento Floresta
