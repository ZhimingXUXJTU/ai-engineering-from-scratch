...
> A suposição de "pureza" é errada, mas ainda pode ser usada.
...
- Implementar Bayes Naívo Multinômio a partir do zero com suavizamento Laplace para classificação de texto
  Desde zero realizando com Laplace plano de múltiplas formas simples de base para uso em textos
- Explique por que a suposição ingênua de independência é matematicamente errada, mas produz classificações de classes corretas na prática
  Explicação de por que a simples independência supõe erros matemáticos mas na prática produz classificação de classes correta
- Compare as variantes multinomial, bernoulli e gaussiana de Bayes ingênua e selecione a certa para um determinado tipo de característica
  Comparar variantes de vários tipos, Bernuli e Goss Simple Bayes, para determinar o tipo de variante escolhida corretamente
- Avaliação de Bayes Ingênuo contra a regressão logística em dados escassos de alta dimensão e explicação da compensação de variação de viés no trabalho
  Em alta dimensão de dados raros, avaliar o simples teor e a lógica de regresso, explicar o peso de diferença-diferência
...
> Os primeiros filtros de lixo de correio (como SpamAssassin) usaram muito simples Bayes, pois ele era de características de alta dimensão (expresso pode chegar a centenas de milhares de milhares) mas o treinamento mostrava-se em cenários muito pequenos.
...
> Você precisa de fazer uma divisão de texto em categorias. O e-mail é dividido em lixo ou não. Os comentários dos clientes são divididos em positivos ou negativos.
...
> A maioria dos classificadores aqui está em jogo. A regressão lógica precisa de um estilo suficiente para estimar com confiança milhares de pesos.
...
> 朴贝叶斯能处理这种情况――它做了一个数学错误假设 (((给定类别下每个特征相互独立),但在文本分类上仍然优于"更聪明"模型,特别是小训练集上――它只需要一次遍历数据就能训练――它扩展到数百万特征――它产生概率估算 (((尽管由于独立性假设通常校准不佳) ︎
...
> Entender por que as suposições erradas podem produzir boas previsões ensinará algumas coisas básicas em sua aprendizagem de máquina: o melhor modelo não é o modelo mais correto, mas o melhor modelo para a balança de diferença-diferença de seus dados.

> **【中文解读】**
> O "pureza" de Bayez consiste na suposição de que todas as características são independentes entre si em determinadas categorias. Isto é quase impossível na realidade. Mas por que pode ser usado? Porque as categorias só precisam de classificação correta das probabilidades de cada categoria, não precisam de valor de probabilidade preciso.
...
- `P(features | class)`-- a probabilidade de ver estas palavras em documentos desta classe
  `P(features | class)`- Veja estas palavras no arquivo.
...
- `P(features)`- a evidência, igual para todas as classes, para que possamos ignorá-la quando comparamos
  `P(features)`- Evidências, para todas as categorias, iguais, podem ser ignoradas quando comparadas
...
> 精确计算 `P(features | class)`需要估估所有特征的联合概率──对于10,000个词表,你需要估估2^10,000种可能组合上的分布──不可能──
...
> Você não precisa estimar uma distribuição conjunta impossível, mas estimar n ̇s simples distribuições individuais.
...
> Esta hipótese é obviamente errada. "máquina" e "aprendizagem" não são independentes em qualquer documento. Mas as categorias não precisam de uma estimativa de probabilidade correta.
...
1. **Ranking over calibration.**A classificação só precisa da classe de topo para ser correta. Mesmo que P(spam) = 0,99999 quando a probabilidade verdadeira é 0,7, o classificador ainda escolhe o spam corretamente. Não precisamos de probabilidades corretas. Precisamos do vencedor correto.
   **排名优于校准。**Até mesmo o P(spam) = 0,99999 e a probabilidade real é 0,7, os classificadores ainda escolhem o lixo. Não precisamos de probabilidade correta.
...
2. **High bias, low variance.**A suposição de independência é um forte antecedente. Ele restringe fortemente o modelo, o que impede o excesso de encaixe. Com dados de treinamento limitados, um modelo que é ligeiramente errado, mas estável, supera um modelo que é teoricamente correto, mas muito instável.
   **高偏差、低方差。**A hipótese de independência é um forte antecedente. É um modelo de restrição grave, impedindo o excesso de adaptação. Em dados de treinamento limitado, um modelo um pouco errado mas estável vence o modelo teoricamente correto mas extremamente instável.
...
3. **Feature redundancy cancels out.**As características correlacionadas fornecem evidências redundantes. O classificador contagia duas vezes essas evidências, mas também as contagia para a classe correta. Se "máquina" e "aprendizagem" aparecem sempre juntas, ambas fornecem evidências para a classe "tecnológica". NB as contagia duas vezes, mas as contagia duas vezes para a classe correta.
   **特征冗余相互抵消。**相关特征提供冗余证据――分类器重复计数 这些证据,但正确类别的证据也被重复计数――如果"máquina"和"aprendizagem"总是出现一次,两者都为"技术"类提供证据――朴贝叶斯计数两次,但为正确类别计数两次――
...
> A quarta razão real é que o treinamento é uma frequência de contagem de dados única. A previsão é uma multiplicidade de matrizes. Você pode treinar um milhão de documentos em poucos segundos. Esta velocidade significa que você pode experimentar mais características, executar mais experimentos.
...
> Simplesmente, há três variantes.`P(feature | class)`Construir de maneira diferente.
...
> Use cada característica como um número. O mais adequado é o texto de dados de um valor de frequência ou TF-IDF.
...
> `alpha`É o que é o "Laplace" (Platform)

#### Gaussian Naive Bayes

Modela cada característica como uma distribuição normal.

> Colocar cada característica em uma distribuição normal.
...
> Cada categoria tem seu próprio valor médio e diferença em cada característica.
...
> Colocar cada característica em um segundo valor () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () () ()
...
> Diferentemente de muitos outros, Bernouli não aparece a palavra "livre" em seu e-mail, mas não o faz, ele considera isso como uma prova de que ele não é o mesmo.
...
> Se uma palavra aparecer nos dados de teste, mas nunca aparecer em dados de treinamento de uma determinada categoria, o que acontece?
...
> Não-completo:`P(word | class) = 0/N = 0`◊ um zero multiplicado até toda a multiplicidade`P(class | features) = 0`Não importa como outras provas provem, uma palavra que não foi vista destruiu toda a previsão.

O suavizamento de laplace acrescenta uma pequena contagem .`alpha`(geralmente 1) para cada número de características:

> A posição é plana para cada característica e um pequeno número .`alpha`(normalmente para 1):
...
> Alpha=1 时, cada palavra pelo menos obtém uma probabilidade extremamente pequena.

O alfa superior significa suavização mais forte (distribuições mais uniformes).

> Alfa superior significa mais forte de plano (ou mais próximo da distribuição média) e alfa inferior significa modelo mais confiante em dados.
...
> Colocar centenas de probabilidades (cada um menor que 1) fazendo que o valor real seja um número positivo muito pequeno, o multiplicador em baixo também se torna zero.

A solução: trabalhar no espaço de log. Em vez de multiplicar as probabilidades, adicionar os seus logaritmos:

>  solucion: em relação ao número de espaço de cálculo, 把概率相乘以对数相加:
...
> A previsão se transformou em ponto:
...
> 矩阵乘法── é por isso que a previsão de Bayes é tão rápida.

### Bayes Ingênuo vs Regressão Logística

Ambos são classificadores lineares para texto. A diferença é no que eles modelam.

> Os dois são classificadores de texto lineares. A diferença é no objeto que os construem.
...
> 经验法则: primeiro usar simples Bayesian.
...
> **【中文解读】**
> A partir do zero, a maioria dos números é utilizada para classificar os números de números (em inglês: "multiplex", em inglês: "multiplex"), e a maioria dos números é usada para classificar os números de números (em inglês: "multiplex", em inglês: "multiplex"), e a maioria dos números é usada para classificar os números de números (em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", em inglês: "multiplex", "multiplex", em inglês: "multiplex", "multiplex" ou "multiplex" e "multiplex" em inglês: "multiplex" e "multiplex" em inglês: "most-allong" em "most-

> **【拓展：朴素贝叶斯在现代 NLP 中的角色演变】**
> Embora o Transformer 模型(BERT、GPT) tenha superado significativamente a simples Bayes na tarefa de PNL, mas a simples Bayes ainda tem seu lugar de uso:
```figure
naive-bayes
```
...
1. **Smoothing experiment.**Treinar MultinomialNB em dados de texto com valores alfa de 0.01, 0.1, 1.0, 10.0 e 100.0.
   1. **平滑实验。**Utilize o valor alfa 0.01、0.1、1.0、10.0、100.0 em texto em dados treinamento MultinomialNB── desenhar taxa de precisão versus alfa── onde está o pico de desempenho?
...
2. **Feature independence test.**Pegue um conjunto de dados de texto real. Escolha duas palavras que são obviamente correlacionadas ("máquina" e "aprendizagem"). Compute P  palavra1  classe) * P  palavra2  classe) e compare com P  palavra1 E palavra2  classe. Quão errada é a suposição de independência? Afeta a precisão da classificação?
   2. **特征独立性测试。**取一个真文本数据集. 选择两个明显相关词 (如机器和学习) 计算 P                                                                                                                                                                                                                                                   
...
4. **NB vs Logistic Regression.**Treinar ambos com dados de texto. Comece com 100 amostras de treinamento e aumente para 10.000. A precisão da trama vs tamanho do conjunto de treinamento para ambos. Em que ponto a Regressão Logística ultrapassa o Bayes Ingénuo?
   4. **NB vs 逻辑回归。**Em dados de texto, os dois treinamentos começaram a aumentar de 100 a 10.000 exemplares de treinamento.
...
> **【中文解读】**
> 朴素贝叶斯的三种变体:(1) 多项式朴素贝叶斯(MultinomialNB) 适用于词频/TF-IDF特征文本分类;(2) 伯努利朴素贝叶斯(BernoulliNB) 适用于二值特征(词是否出现);(3) 高斯朴素贝叶斯(GaussianNB) 适用于连续特征,假设每类内特征服从正态分布――Laplace 平滑加 alpha) 不会防止零概率问题遇到训练集中没见过的词时,概率为零──
...
- [McCallum and Nigam, A Comparison of Event Models for Naive Bayes Text Classification (1998)](https://www.cs.cmu.edu/~knigam/papers/multinomial-aaaiws98.pdf)-- a comparação clássica de Multinomial vs Bernoulli para texto
  [McCallum and Nigam (1998)](https://www.cs.cmu.edu/~knigam/papers/multinomial-aaaiws98.pdf)- Comparar vários tipos de textos clássicos
- [Rennie et al., Tackling the Poor Assumptions of Naive Bayes Text Classifiers (2003)](https://people.csail.mit.edu/jrennie/papers/icml03-nb.pdf)-- Melhorias no texto
  [Ng and Jordan (2001)](https://ai.stanford.edu/~ang/papers/nips01-discriminativegenerative.pdf)- prova NB em pouco dados recebido rapidamente em LR
