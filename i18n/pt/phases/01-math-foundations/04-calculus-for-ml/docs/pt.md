...
> O número de direções diz-te onde é a direção de descer.
...
- Computação de derivados numéricos e analíticos para funções ML comuns (x^2, sigmoide, entropia cruzada)
  計算常见 ML 函数(x^2、sigmoid、交叉) de números de valores e de dados
- Implementar descida de gradiente a partir do zero para minimizar uma função de perda em 1D e 2D
  De zero a nível de redução, na função de perda mínima entre 1D e 2D
...
> **【中文解读】**
> O número de direção diz-lhe "para onde deve ir" e pode fazer o erro ser pequeno. A rede neuronal tem milhões de parâmetros, cada parâmetro é um "torno", o número de bits diz-lhe em que direção deve ir.

> **【拓展：微积分与神经网络】**
> - **梯度下降**O algoritmo central do treinamento de redes neurais é o de atualizar os parâmetros em direção ao contrário da gradiência.
> - **SGD/Adam**A evolução do sistema de aprendizagem de cada um dos grupos de aprendizagem é um processo de aprendizagem de cada um dos grupos de aprendizagem.
> - **学习率**梯度下降的步长──太大则跳过最小值,太小则收太慢──

## O problema é o problema da introdução

> **【中文解读】**A formação é encontrar cada rotação que deve virar em que direção. A formação é encontrar cada rotação que deve virar em que direção.

## O conceito central.

> **【拓展：偏导数就是"只动一个旋钮看效果"]**Função de perda de rede de nervos L(w1, w2, ..., wn) Há milhões de variações.
...
> 导数量衡变化率──对于函数 y = f(x),导数 f'(x) 告诉你: se x 微小变化, y 变化多少?

Geometricamente, a derivada é a inclinação da linha tangente em um ponto.

> Em geometria, o número de direção é a inclinação de um ponto de linha.
...
> Quando x é igual a 2, a inclinância é de 4... se você vai x para a direita mover um ponto, y aproximadamente aumentar a quantidade de movimento 4 vezes... quando x é igual a 0, a inclinância é de 0... você está no "teso da tigela".
...
> Em código, saltar além do limite, direto usando um pequeno h para aproximar-se.
...
> A função de perda de rede neuronal depende de milhares de pesos. O número de parâmetros mantém as outras variações inalteradas, só para uma variação.
...
> Cada número de direção responde: se eu apenas ligar este peso, a perda varia?
...
> A função forma uma forma de bolha, igual à linha alta é com o círculo central. O valor mínimo é (0, 0)
...
> É o que se vê na figura de um gradiente de descenso.
...
> 訓練神经网络就是优化──损失函数 L(w1, w2, ..., wn) 衡模型有多"错", você deve minimizá-lo──
...
> 梯度下降规则: 新权重 = 旧权重 - 学习率 × 梯度──重复:1) calcular o número de direções de cada peso;2) deduzir um pequeno multiplicador do peso;3) 代数百万次──

A taxa de aprendizagem controla o tamanho dos passos, é grande demais e você supera, é pequeno demais e você rasteja.

> O índice de aprendizagem controlou o progresso.
...
> O mínimo local é o mínimo do vale do vale do vizinho, mas não o mínimo local; a gradiência desce de qualquer ponto de partida ao longo da ladeira para baixo.
...
> 梯度下降沿坡下行── pode ser um problema real, mas em alto espaço, isso raramente se torna um problema real──
...
> Número de valores: usando definição de aproximação.
...
> Os números de direção são mais lentos, mas são aplicáveis a qualquer função.
...
> Em x<0 时导数为负(函数递减), x=0 时导数为零( atinge o mínimo valor), x>0 时导数为正(函数递增) ⋅
...
> Para o w o resultado é x o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o o

### A regra da cadeia

Quando as funções são compostas, a regra da cadeia diz-lhe como diferenciar.

> Quando a função é composta, a lei do enlace diz-lhe como pedir orientação.
...
> A rede de neurônios é uma cadeia funcional: entrada -> linha -> ativar -> linha -> ativar -> perda;.

### A Matriz Hessiana

O gradiente diz-te a inclinação.

> A gradiência diz-te a gradiência, a Hessian é a matrizes que te dizem a gradiência.

O Hessiano é a matriz de derivadas parciais de segunda ordem. Para uma função f ((x1, x2, ..., xn), a entrada (i, j) do Hessiano é:

> Hessian é um segundo grau de direção dos números compostos em uma matriz.
...
> Hessian está em ponto de clínica (梯度为 0 处) diz-te: se é o valor mínimo local, o valor máximo local ou o ponto ──
...
> Hessian em ML: Newton usa Hessian 修梯度方向, fazendo direção de pequeno passo 平坦方向走大步, తద్వారా比梯度下降更快收──
...
> Newton 更新:w_new = w_old - H−1 × gradiente。 é usado para a taxa de curvatura de reconduzir a gradiente。
...
> Newton recebeu mais rápido, porque Hessian "reencontrair" gradiente  direção de um pequeno passo, plano direção de um grande passo.
...
> O problema está em: N 个参数的网络, Hessian é N×N──million参数模型需要万亿规模矩阵这就是为什么我们使用近似像亚当 L-BFGS)──
...
> Não é diferente de um optimizador: gradiente de baixa em relação a um nível de baixa em relação ao outro; Newton usa o Hessiano completo (O3), rápido, mas muito caro; L-BFGS usa o gradiente histórico similar ao Hessiano; Adam usa o gradiente Hessiano similar ao cada um dos parâmetros; Natural gradiente usa o Fisher 信息矩阵。
...
> Na prática, Adam é um mecanismo de otimização padrão de aprendizagem profunda.

### Aproximação da série Taylor

Qualquer função lisa pode ser aproximada localmente por um polinômio:

> Qualquer função plana pode ser localmente usada em múltiplas aproximações.
...
> Taylor 级数在ML中的意义:一阶 Taylor = 线性近似 = 梯度下降;二阶 Taylor = 二次近似 = Newton 法;MSE 和交叉的平滑性不是巧合平滑损失让优化可预测──
...
> Próximo 阶数与优化方法:0 阶只用值(随机搜索);1 阶用斜率(梯度下降);2 阶用曲率(Newton法);更高阶在 ML muito pouco uso。
...
> 关键洞见: todas as melhorias baseadas em gradientes são, em essência, em função local de perda aproximada, e depois passam para esse ponto de menor valor aproximado.
...
> Em ML você tem poucas mãos para calcular, mas o conceito de积分 está presente:
...
> **概率**Para a continuidade, a densidade da função p (x) é a probabilidade de ocorrer em uma área.
...
> **期望**A expectativa de perda na distribuição de dados é de um积分, o treino minimiza sua experiência.
...
> **KL 散度**A diferença entre duas distribuições é medida.
...
> **归一化常数**Em teoria de Beyes, a divisão é o ponto de todos os valores dos parâmetros possíveis, normalmente não pode ser calculado.
...
> 积分概念在 ML 中体现:曲线下面积(densidade função procura probabilidade) 期望(损失 função) 、KL 散度(VAE/蒸) 归结化(贝叶斯后验/softmax 分母) 边际似然(模型比较/ELBO) ⋅
...
> A lei da cadeia de variações não se aplica apenas às funções de quantidade linear. Em redes neuronais, as variações se dividem e se juntam.
...
> Cada arco multiplicado por um número de direções locais. A gradiência de qualquer parâmetro = multiplicidade de todas as direções locais no caminho do parâmetro. Quando o caminho se divide e se junta, é necessário fazer a contribuição para obter e obter o número de cadeias.
...
> Todo o que se pode dizer de contra-direção é: aplicar o sistema de cadeias de dados em diagramas de cálculo, de saída a entrada.
...
> Quando a função traz o veículo mapeado para o veículo, como a camada de rede de neurônios, seu direcionamento é uma matriça Jacobiana que contém cada saída para cada entrada.
...
> Você não vai calcular a JacobianPyTorch de uma rede de neurônios. Mas saber que ela existe pode ajudá-lo a entender a forma na transmissão: se a camada coloca R^n 映射到R^m, seu Jacobian é m×n, gradiente através de sua transmissão em direção à transmissão.

### Por que isso importa para redes neurais

Cada peso numa rede neural recebe um gradiente.

> Cada peso na rede neuronal tem uma escala, que diz-te como ajustar esse peso para reduzir a perda.
...
> Cada peso de nova: W = W - lr × dL/dW── anterior para calcular previsão e perda, inversos para calcular cada peso de gradiente, cada peso ao longo da gradiente direção negativa.
...
> Antes de divulgar o cálculo da previsão e perda, depois de divulgar o cálculo da gradiência de cada peso, então cada peso percorre um pequeno passo em direção ao nível negativo.
...
> Usar o método de diferença central para realizar o número de valores de referência.

A derivada numérica corresponde à analítica com muitos pontos decimais.

> A coesão entre a direção de valores numérica e a direção de resolução em vários pontos após a pequena diferença verificou a veracidade da fórmula da diferença central.
...
> Número de valores: para cada dimensão de um centro de dependência de um determinado ponto de orientação, formação de um gradiente de orientação, avaliação f ((x,y) = x2 + 3xy + y2 em (1,2) 处的梯度为 [8, 7]。
...
> De x=5 para sair, cada passo é mais próximo de x=0 ((minimum value) ⋅ taxa de aprendizagem 0,1 ⋅ deixe x ⋅ diminuição gradual até 0⋅
...
> 2D 梯度下降: de (4, 3) 出发, cada passo update point -= lr × grad, passo a passo rece do (0, 0) ⋅
...
> Em relação a 5 funções comuns em x=2 em relação a uma função de valores de referência e a uma função de resolução: x2、x3、sin(x)、e^x、1/x──, os erros geralmente ocorrem em 1e-10 níveis, a correlação do método de verificação de valores de referência.
...
> O valor de caracteres hessianos da função de pontos é 2 和 -2 ((一正一负, 定点); o valor de caracteres hessianos da função de bolsas são 2 ((均正,定值最小) ⋅
...
> Em geral, a taxa de aprendizagem é de 0,00%, mas a taxa de aprendizagem é de 0,00%.
...
> Cada ciclo de treinamento baseado em gradientes segue este modelo: pré-estima →  cálculo perda →  cálculo gradiente →  renovação de peso.

## Use-o com o framework implementado.

Com o NumPy, as mesmas operações são mais rápidas e concisas:

> Utilize NumPy 重写: igual运算更简洁更快――矢量化 evitou o ciclo Python, fazendo a gradiência calcular mais高效――
...
> NumPy 向量化版本: 用 `np.mean`替代Python 循环求平均,更快更简洁──矢量化 é uma técnica de otimização central do cálculo numérico──

A PyTorch automatiza o cálculo do gradiente, mas o ciclo de atualização é idêntico.

> Você acaba de conseguir a descida da escala desde o zero.`w -= lr * dw`Esta linha nunca mudará.
...
1. Implementação `numerical_second_derivative(f, x)`usando`numerical_derivative`Verifique se a segunda derivada de x^3 em x=2 é 12.
    realização `numerical_second_derivative(f, x)`,调用两次 `numerical_derivative` Experiência x^3 em x=2 处的二阶导数为12──
2. Use a descida de gradiente para encontrar o mínimo de f ((x, y) = (x - 3) ^ 2 + (y + 1) ^ 2. Comece a partir de (0, 0). A resposta deve convergir para (3, -1).
   Us梯度下降找 f(x, y) = (x - 3)2 + (y + 1)2 de menor valor, de (0, 0) 出发,应收到 (3, -1)。
3. Adicionar impulso ao ciclo de descida de gradiente: manter um vetor de velocidade que acumula gradientes passados. Compare a velocidade de convergência com e sem impulso em f ((x) = x^4 - 3x^2.
   给梯度下降循环加动量: mantenha uma acumulação de velocidades de velocidade do gradiente passado.
...
> 术语速查:Derivada (→) 导数/斜率) ‧Derivada parcial (→) 偏导数, fixa outras variações) ‧Gradiente (→), todas as derivações compostas por direções, orientadas para a maior direção ascendente) ‧Descenso (→) ‧Gradiente (→) ‧Descenso (→) ‧Trading rate (→) ‧Learning rate (→) ‧Learning rate (→) ‧Rate (→) ‧Chain rule (→) ‧Chain rule (→) ‧Chain rule (→) ‧Chain rule (→) ‧Chain rule (→) ‧Chain rule (→) ‧Chain rule (→) ‧Chain rule (→) ‧Chain rule (→) ‧Chain line (→) ‧Chain line (→) ‧Line (→) ‧Line (→) ‧Line (→) ‧Line (→) ‧Line (→) ‧Line) ‧Line (→) ‧Line (→) ‧Line (→) ‧L) ‧
