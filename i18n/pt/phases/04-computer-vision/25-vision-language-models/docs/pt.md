# Modelos de linguagem de visão  O padrão de ViT-MLP-LLM 
...
> **【中文解读】**O visual editor irá transformar imagens em tokens, o MLP 投影层 irá mapear esses tokens para o espaço de inserção do LLM, o modelo de linguagem completar o restante trabalho.

> **【拓展：VLM 的应用】**O VLM é o núcleo da IA: GPT-4V(图像问答) Claude Vision(文档分析) ✓ LLaVA(开源视觉对话) ✓ Google Gemini(多模态推理) ✓ No domínio financeiro, o VLM pode ser usado para o desenvolvimento de informações sobre o quadro de entendimento, contrato, análise inteligente, processamento automático de dados.
...
> CLIP(Quarta fase 18 课) dá-lhe espaço de partilha de imagens e texto, suficiente para fazer zero amostragens de categorização e pesquisa. Não pode responder "Há alguns carros vermelhos neste quadro?" porque CLIP não gera texto.
...
> 视觉语言模型(VLM) Qwen3-VL、InternVL3.5、LLaVA-Next、GLM-4.6V vai conectar o código de imagens da família CLIP ao modelo de linguagem completo── modelo ver imagens adicionais e gerar respostas── em 2026 ano, VLM em vários modelos irá competir ou vencer o GPT-5 e Gemini-2.5-Pro──
...
> Três componentes (ViT, Projector, LLM) são as configurações padrão. A diferença entre os modelos é em que ViT, projector, LLM, treinamento e data de conjunto. Uma vez que você entende esse modelo, substituir qualquer componente é uma operação mecânica.
...
1. **Vision encoder** um ViT pré-treinado (CLIP-L/14, SigLIP, DINOv3 ou uma variante de sintonia fina).
   Tradução:**视觉编码器** Pre-training de ViT(CLIP-L/14、SigLIP、DINOv3 或微调变体) 产生补丁代号──
2. **Projector** um pequeno módulo (2-4 camadas MLP, ou um Q-former) que mapeia os tokens de visão na dimensão de incorporação do LLM. É aqui que a maior parte do ajuste fino acontece.
   Tradução:**投影器**小型模块 ((2-4 层 MLP ou Q-former),将视觉 token 映射到LLM 嵌入维度── a maior parte das pequenas modificações ocorrem aqui──
3. **LLM** um modelo de linguagem apenas para decodificadores (Qwen3, Llama, Mistral, GLM, InternLM).
   Tradução:**LLM**仅解码器语言模型(Qwen3、Llama、Mistral、GLM、InternLM) ・・・顺序读取视觉 + 文本代币,生成文本。
...
> Em prática, o programação visual e o Mestrado em Matemática (LLM) são apenas projetores que são treinados para obter bilhões de parâmetros de sinais a baixo custo.
...
> 朴素投影只使用ViT 最后一层――DeepStack(Qwen3-VL) de vários ViT profundidade采样特征并堆叠――更深层携带高级语义;更浅层携带细粒度空间和纹理信息――将两者都送进 LLM 弥合了"图像包含什么" (图像包含什么) 和"具体在哪里" (空间定位) 之间的差距――
...
1. **Alignment** congelar ViT e LLM. Treinar apenas o projetor em pares de imagem-capção. Ensinar o projetor a mapear o espaço de visão no espaço de linguagem.
   Tradução:**对齐**结 ViT 和 LLM── apenas em imagem-título sobre o projeto de treinamento superior── ensinar o projeto de projeção vai o espaço visual mapear para o espaço de linguagem──
2. **Pre-training**- Descongelar tudo. Treinar em grande escala dados de imagem-texto entrelaçados (pares de mais de 500 milhões). Construir o conhecimento visual do modelo.
   Tradução:**预训练**解一切── formação em dados de imagens-texto de grande escala  5 mil milhões +                                                                                                                                                                                                                                                   
3. **Instruction tuning** sintonização fina em triples curados (imagem, pergunta, resposta). Ensina o comportamento de conversação e formatos de tarefas.
   Tradução:**指令微调**在精选的(图像,问题,答案) 三元组上微调──教会对话行为和任务格式──这将" LM do sentido visual" se transformar em assistente disponível──
