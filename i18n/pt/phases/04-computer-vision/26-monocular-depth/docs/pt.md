# Profundeza monocular e geometria Estimação .
...
> **【中文解读】**A profundidade do gráfico é uma imagem de um único canal, cada imagem representa o valor da distância da câmera. A profundidade de imagem de um único gráfico RGB previsão de profundidade foi considerada impossível, mas o ViT de 2026  codificador + de resolução de menor quantidade já conseguiu alcançar uma precisão próxima ao valor real.
...
> A profundidade é o eixo que falta na visão de computador em 2D. Dado RGB, você sabe a posição dos objetos na superfície da imagem; você não sabe o quanto eles estão longe.
...
>  Profundidade de estimação  de um único RGB  Proficiência de previsão  passado produzido模糊、outputes não confiáveis。 até 2026 anos grandes prep training codificadores alteraram este ponto: Profundidade Qualquer coisa V3 usando结  DINOv2 骨干并产生跨室内、室外、医学和卫星领域泛化的深度图──Marigold irá redefinir profundidade como condição de expansão questão──ZoeDepth retornar à verdadeira distância pública─
...
> A profundidade é também um ponto de ligação entre a pesquisa 2D e a compreensão 3D: se as imagens do quadro detectado forem multiplicadas pela profundidade, você poderá elevar os objetos 2D para 3D.
...
- **Relative depth**- Encomendado .`z`"O pixel A é mais próximo do que o pixel B, mas a relação de distâncias não está ancorada a metros".
  Tradução:**相对深度**                                                                                                                                                                                                                                                              `z`值,没有真实世界单位──"A imagem A é superior a B, mas a taxa de distância não está determinada até nós──"
- **Metric depth** distância absoluta em metros da câmera. Requer que o modelo tenha aprendido a relação estatística entre as indicações da imagem e a distância real.
  Tradução:**度量深度** Desse modo, os modelos precisam aprender a relação estatística entre a imagem e a distância real.
...
> MiDaS 和 Depth Qualquer coisa V3 产生对深度──Marigold 产生对深度──ZoeDepth、UniDepth 和 Metric3D 产生度深度──度模型对相机内参敏感;相对模型则不敏感──
...
> Depth Anything V3 结编码器, apenas treinar DPT 风格的解码器──编码器提供丰富特征;解码器将它们插值回图像分辨率并回归深度──
...
> A ViT, que treinou em bilhões de imagens, irá encapsular esses encaminhos. Com dados e fortes bases, a profundidade visual única não requer qualquer supervisionamento 3D evidente para alcançar uma precisão razoável.
...
> É um modelo de 2026 que precisa de profundidade.
...
> Marigold(Ke 等,CVPR 2024) vai profundidade estimativa reformulado para condições imagem até imagem de expansão.
