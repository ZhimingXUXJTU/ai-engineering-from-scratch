# Transformadores de difusão e fluxo rectificado . Transformador de expansão e fluxo completo .
...
> **【中文解读】**U-Net não é o segredo do modelo de expansão. U-Net é usado para substituir o U-Net por Transformador.

> **【拓展：DiT 是 2024-2026 的趋势】**Transformador de difusão (DiT) vai Transformador de cómputo de dilatação (DiT) vai Transformador de cómputo de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformador de dilatação (DiT) vai Transformação (DiT) vai Transformação (DiT) vai Transformação (DiT) vai Transformação (DiT) vai Transformação (DiT) vai Transformação) vai Transformador de dilatação (DiT) vai Transformação (DiT) vai Transformação) vai Transformador de dilatação (DiT) vai Transformação (DiT) vai Transformação) vai Transformação (D) vai Transformação (D)
...
> No curso de 2020-2023, o programa foi desenvolvido com o uso de um dispositivo de ruído U-Net, que produziu uma difusão estável de 1,5 e 2,1 e DALL-E 2, respectivamente.
...
> Cada modelo de texto até imagem avançado de 2026 já superou isso. Estabilidade Diffusão 3、FLUX、SD4、Z-Image、Qwen-Image、Hunyuan-Image não usa um U-Net. Eles usam Transformador Difuso (DiT) ・SD3 和 FLUX também usam o fluxo inteiro para substituir a modulação de ruído DDPM, que irá passar de ruído para os dados através de um caminho direto, e através de uma união ou um vapor de variação realizar 1-4 passos.
...
> Esta transformação é importante, porque é baseada na generação de imagens de expansão tornar-se controlada 提示精确 (SD3/SD4 解决文本染) e na produção rápida 原因──理解DIT + 整流就是理解2026年生成图像技术──
...
- **DiT**(Peebles & Xie, 2023)  substituir a U-Net por um transformador semelhante a ViT em parches latentes. Condicionamento através de norma de camada adaptativa (AdaLN).
  Tradução:**DiT**(Peebles & Xie,2023)  Usar o tipo de Transformador de ViT  substituir U-Net  processar o potencial de mudança de volume 
- **MMDiT**(SD3, Esser et al., 2024)  dois fluxos com pesos separados para tokens de texto e imagem que compartilham uma atenção comum.
  Tradução:**MMDiT**(SD3,Esser etc., 2024)  dois grupos têm o direito de processar textos e imagens em tokens, compartilhamento de atenção comum.
- **FLUX**(Black Forest Labs, 2024)  primeiros blocos N duplo-fluxo como SD3, blocos posteriores concatenar e compartilhar pesos (fluxo único) para a eficiência em maior profundidade.
  Tradução:**FLUX**(Black Forest Labs, 2024) 前 N 个块像 SD3 一样双流,后续块拼接并共享权重(单流) para melhorar a eficiência do nível profundo。
- **Z-Image**(2025)  uma DiT eficiente de fluxo único a parâmetros 6B que desafia a "escalação a todo custo".
  Tradução:**Z-Image**(2025)  60 mil milhões de parâmetros de alta eficiência e fluxo único de TI, desafiando a "expanção do custo não contabilístico"
...
> DDPM vai definir o processo de direção para o ruído com micro-quebradação (SDE), entre os quais:`x_t`渐渐被损坏──学习到的反向是第二个SDE,通过1000小步求解──

O fluxo retificado define um **straight-line**Interpolação entre dados limpos e ruído puro:

> O fluxo completo define entre dados e ruído puro**直线**插值:
...
> 训练网络预测速度 `v_theta(x_t, t) = epsilon - x_0` da direcção de dados de qualidade ao ruído`dx_t/dt`O ODE é mais próximo da linha reta, portanto, o número de passos de acumulação necessários é muito reduzido.

O SD3 chama isto .**Rectified Flow Matching**. FLUX, Z-Image e a maioria dos modelos 2026 usam o mesmo objetivo.

> SD3 será denominado**整流流匹配** FLUX、Z-Image 和大多数 2026 年模型使用相同目标──典型推理:20-30 步 Euler(确定性)vs 旧 DDPM 方案的50+ 步 DDIM──蒸/turbo/schnell/LCM 变体降至1-4 步──
...
> DiT                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            **自适应层归一化**Para a condicionalização de tempo e de categorias/texto: de condicionalização de volume de previsão`scale`和 `shift`, em aplicação posterior à LayerNorm, é um padrão de cada DiT moderno, em comparação com a FiLM 风格调制更干净, na U-Net.
...
> DDPM + U-Net 可行──DiT + 整流流**更好、更快、扩展更干净** Esta transformação é semelhante à transição da RNN para o Transformer na PNL: duas estruturas resolvem o mesmo problema, mas o Transformer é mais extensivo e mais finalmente dominante.
