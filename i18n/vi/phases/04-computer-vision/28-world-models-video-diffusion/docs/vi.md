# World Models & Video Diffusion Ưu hình và video phổ biến trên thế giới
...
> **【拓展：世界模型的前沿】**Sora(OpenAI) và Genie(DeepMind) là đại diện của mô hình thế giới.
...
> 视频生成和世界建模于2026年融合了──一个能生成一分连贯视频的模型在某种意义上学会了世界如何运动:物体持久性,重力,因果关系,风格──如果在预测上以动作为条件 ((向左走、开门),视频模型就变成了一个可学习的模拟器,可以替代游戏引擎、驾驶模拟器或机器环境──
...
> 利害关系是具体的──Genie 3 Từ đơn张图像生成可玩环境──Runway GWM-1 Worlds 合成无限可探索场景──Sora 2 生成带同步音频和物理建模的分钟级视频──NVIDIA Cosmos-Drive、Wayve Gaia-2 和 Tesla DrivingWorld 为自动驾驶训练数据生成真实驾驶视频──世界模型范式然接管机器人的真实对现――
...
> Bài học này là chương trình " toàn cảnh" giai đoạn thứ tư. Nó sẽ tạo ra hình ảnh, video hiểu và lý luận về các cơ thể thông minh kết nối với các mô hình cấu trúc đang được nghiên cứu.
...
- **Sora 2**là sản xuất video hoàn toàn dựa trên các yêu cầu không có giao diện hành động bạn không thể "định hướng" nó giữa triển khai
  Trung ngữ翻译:**Sora 2**Đó là một video tự do, điều kiện hóa vào lời khuyên. Không có động tác.
- **Genie 3**- **GWM-1 Worlds**- **Mirage / Magica**là các mô hình thế giới điều chỉnh hành động. Nhập vào các hành động ẩn trong video được quan sát, sau đó điều chỉnh dự đoán khung hình tương lai về các hành động.
  Trung ngữ翻译:**Genie 3****GWM-1 Worlds****Mirage / Magica**Đó là mô hình thế giới điều kiện động tác. Từ video được quan sát, suy đoán về động tác tiềm năng, sau đó sử dụng động tác điều kiện tương lai.
- **DreamerV3**và gia đình mô hình thế giới RL cổ điển dự đoán trong một không gian ẩn với điều kiện hành động rõ ràng, được đào tạo trên một tín hiệu phần thưởng. ít hình ảnh hơn; hữu ích hơn cho RL hiệu quả mẫu.
  Trung ngữ翻译:**DreamerV3**和经典 RL 世界模型家族在潜空间预测,带显式动作条件化,在奖励信号上训练――视觉性较弱;对样本高效的RL更有用――
...
- **Full joint** tất cả các token tham gia vào tất cả các token. O ((N ^ 2) với N token. cấm cho video dài.
  Trung ngữ翻译:**全联合** tất cả các biểu tượng 注意 tất cả các biểu tượng。O(N^2)。对长视频不可行。
- **Divided** sự chú ý thời gian thay thế (nơi không gian tương tự, qua thời gian: `(H*W) * T^2`) và sự chú ý không gian (một bước thời gian, trên không gian: `T * (H*W)^2`Được sử dụng bởi TimeSformer và hầu hết các video DiT.
  Trung ngữ翻译:**分离**交替时间注意力(同一个空间位置,跨时间) 和空间注意力(同一个时间步,跨空间) ――TimeSformer 和大多数视频 DiT 使用──
- **Window** cửa sổ địa phương trong (t, h, w). Được sử dụng bởi Video Swin.
  Trung ngữ翻译:**窗口**(t, h, w) 中的局部窗口──Video Swin 使用──
...
> Mỗi mô hình phát triển video năm 2026 đều sử dụng một trong ba mô hình này, cộng với AdaLN  điều kiện hóa (第 23 课) và toàn bộ dòng chảy.
...
> Chúng thay thế thu thập dữ liệu thực tế đắt tiền để xử lý tình huống lề đường: người đi bộ đêm xuyên đường, ngập đường băng, loại xe bất thường, nếu không cần hàng triệu dặm lái xe.
...
> Nó thay thế hình dạng và mẫu phong phú của RL. mô hình thế giới chịu trách nhiệm tưởng tượng; ngược động lực học đóng vòng để thực hiện.

### Đánh giá

- **Visual quality** FVD (Fréchet Video Distance), nghiên cứu người dùng.
  Trung ngữ翻译:**视觉质量**FVD(Fréchet 视频距离) 、用户研究。
- **Prompt alignment** CLIPS score per frame, đánh giá theo kiểu VQA.
  Trung ngữ翻译:**提示对齐** Mỗi  CLIPScore、VQA 风格评估。
- **Physical plausibility** đánh giá tay trên một bộ điểm chuẩn (Sora 2 điểm chuẩn nội bộ, VBench).
  Trung ngữ翻译:**物理合理性**在基准套件上人工评分(Sora 2 内部基准、VBench)
- **Controllability**(đối với các mô hình thế giới tương tác)  hành động → sự nhất quán quan sát; bạn có thể quay lại trạng thái trước đây không?
  Trung ngữ翻译:**可控性**(交互式世界模型) 动作→观测一致性;能否回到前前的状态?
