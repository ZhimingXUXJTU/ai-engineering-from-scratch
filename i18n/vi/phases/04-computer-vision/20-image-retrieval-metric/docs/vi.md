# Khám ảnh lấy lại & Métric Learning  Khám ảnh tìm kiếm và đo học
...
> **【中文解读】**检索系统通过嵌入空间中的距离对候选图像排序――度学习就是塑造这个空间,使距离反映你想要的语义关系相似图像靠近,不相似图像离远――对比损失 (相对损失) 和三元组损失 (三元组损失) 是核心方法――

> **【拓展：检索系统的应用】**以图搜图(Google Images、淘宝拍照搜) 、人脸识别(FaceNet)、推系统(Pinterest) đều phụ thuộc vào quy mô học;.
...
> 检索在生产视觉中无处不在:重复检测、反向图像搜索、视觉搜索(" tìm kiếm sản phẩm tương tự") 的人脸重识别、监管人员重识别、电商实例级匹配──产品问题总是相同:"给定这张查询图像,对我的目录排序──"
...
> Hai quyết định thiết kế tạo nên toàn bộ hệ thống. Đúng vào trong một mô hình tạo ra khối lượng. Đúng vào trong một mô hình tạo ra khối lượng.

Việc hình thành là học metric. Đó là một kỷ luật nhỏ nhưng có đòn bẩy cao.

> Đó là một loại hình học tập vừa phải.
...
> Đối với hầu hết các trường hợp sản xuất, từ đầu tiên đào tạo, chỉ khi kết hợp hiện tại được tích hợp vào tập hợp thử nghiệm của bạn không hiệu quả tốt mới có thể tăng cường độ học tập.
...
>   `a`拉近正样本 `p`, Chu远负样本 `n`, dùng`margin`确保间隔──三图像结构可推广到任何相似度排序──
...
> 挖掘 rất quan trọng:简单三元组`n` đã xa `a`(Trong phần đầu tiên, các công ty đã tạo ra một hệ thống kỹ thuật và kỹ thuật để tạo ra các hệ thống kỹ thuật và kỹ thuật kỹ thuật).`n`比 `p`远但在边缘内) là chương trình của FaceNet năm 2016, cho đến nay vẫn chiếm ưu thế.

### Sự tương đồng của cosine so với L2

Hai chỉ số, hai quy ước:

> 2 loại đo, 2 loại quy định:

- **Cosine**: góc giữa các vector. yêu cầu các embeddings L2 chuẩn hóa.
  Trung ngữ翻译:**余弦**:向量间角──需要 L2 归结嵌入──
- **L2**: khoảng cách Euclidean. hoạt động trên các nhúng nguyên liệu hoặc chuẩn hóa, nhưng thường được kết hợp với L2- chuẩn hóa + L2 vuông.
  Trung ngữ翻译:**L2**: 欧氏距离;; thích hợp cho việc nhúng nhập gốc hoặc nhúng nhập, nhưng thường dùng với L2 归化 + 平方 L2 配对使用;;
...
> Đối với hầu hết các mạng hiện đại,`||a|| = ||b|| = 1`时,`||a - b||^2 = 2 - 2 cos(a, b)`                                                                                                                                                                                                                                                              
...
> Và排 report recall@1、@5、@10──recall@10 超过0.95 nhưng recall@1 低于0.5 nghĩa là được đặt trong cấu trúc không gian đúng nhưng排序有噪音尝试更长的微调或重排序步骤──
...
> Đối với重复检测,precision@K quan trọng hơn, vì mỗi giả tích là lỗi của người dùng có thể nhìn thấy. Đối với video tìm kiếm, remember@K là tín hiệu sản phẩm.
...
> Facebook AI tương tự tìm kiếm.

- `IndexFlatIP`- `IndexFlatL2` lực thô, chính xác, không có huấn luyện.
  Trung ngữ翻译:`IndexFlatIP`- `IndexFlatL2` Tìm kiếm bạo lực, xác định, không cần đào tạo.
- `IndexIVFFlat` chia thành các tế bào K, tìm kiếm chỉ những tế bào gần nhất.
  Trung ngữ翻译:`IndexIVFFlat` chia thành K 个单元, chỉ tìm kiếm vài đơn vị gần đây hơn.
- `IndexHNSW` dựa trên đồ thị, nhanh nhất cho nhiều truy vấn, kích thước chỉ mục lớn.
  Trung ngữ翻译:`IndexHNSW` dựa trên hình ảnh, nhiều truy vấn, nhanh nhất, chỉ số lớn hơn lớn hơn.
...
> 100.000 đô la dùng`IndexFlatIP`余弦相似度即可──10 triệu dùng `IndexIVFFlat`❖ 1 tỷ người hơn cộng tác`IndexIVFPQ`(■)

### Khám phá cấp độ giai đoạn so với cấp độ danh mục

Hai vấn đề rất khác nhau với cùng tên:

> Hai từ giống nhau nhưng rất khác nhau vấn đề:
...
- **Category-level** "đ tìm mèo trong danh mục của tôi". Sự tương đồng theo các điều kiện lớp học; các nhúng CLIP / DINOv2 không sẵn có hoạt động tốt.
  Trung ngữ翻译:**类别级**" trong danh mục của tôi tìm mèo"──类别条件相似度;现成的 CLIP / DINOv2 嵌入即可──
- **Instance-level** "đ tìm * sản phẩm chính xác này* trong danh mục của tôi". Cần phân biệt tinh vi giữa các đối tượng tương tự về mặt trực quan trong cùng một lớp; các bản nhúng ra khỏi kệ kém hiệu quả; điều chỉnh tinh tế với các vấn đề học tập métric.
  Trung ngữ翻译:**实例级**" trong danh mục của tôi tìm * sản phẩm cụ thể này*"── cần phân biệt độ phân biệt giữa các loại hình ảnh tương tự vật thể; hiện tại được nhúng không tốt; độ học tập nhỏ rất quan trọng──

Luôn hỏi bạn đang giải quyết vấn đề nào trước khi chọn một mô hình.

> Trong mô hình lựa chọn trước, cần phải hỏi rõ bạn đang giải quyết vấn đề nào.
...
> Một行代码──适用于 L2 归一化或原始嵌入──

### Bước 2: khai thác bán cứng

Với một loạt các nhúng và nhãn, tìm âm bán cứng khó nhất cho mỗi neo.

> Đặt một loạt các mục nhập và nhãn, cho mỗi điểm tìm thấy một nửa khó khăn nhất.
...
> Mỗi điểm đạt được các mẫu thực khó nhất trong cùng lớp và một mẫu thực so với nhau nhưng trong biên giới là một nửa các mẫu tiêu cực.
...
> L2 归结嵌入内积 top-k等于余弦 top-k―― báo cáo ít nhất có một tỷ lệ trung bình của các câu hỏi của hàng xóm chính xác――
...
> Sau vài trăm bước, các lớp được tích hợp thành từng loại một.
