# Phân tích phân đoạn  Mask R-CNN 实例分割  Mask R-CNN

> Thêm một nhánh nạ nhỏ vào máy dò R-CNN nhanh hơn và bạn có phân đoạn ví dụ. Phần khó là RoIAlign, và nó khó hơn nó trông.

> **【中文解读】**Trong R-CNN nhanh hơn kiểm tra viên thêm một bộ phận ẩn chứa được phân chia ví dụ. Điểm khó khăn là RoIA align sẽ có các khu vực ứng cử viên khác nhau kích thước để phù hợp với các mô hình đặc trưng của kích thước cố định, và giữ độ chính xác không gian.

> **【拓展：实例分割的应用】**Mask R-CNN 广泛 được sử dụng tự lái 区分不同车辆和行人) 机器人抓取 识别单个物体轮) 视频编辑 精确图)  RoIAlign 技术后来也被用于视觉变压器的适配器中──
...
> 语义分割给你每类一个掩码――实例分割给你每类一个掩码,即使两个物体属于同一类――数个体、跨跟踪和测量东西――墙壁中的每块的边界框、显微镜图像中每个细胞) đều cần các实例分割――
...
> Mask R-CNN (Hô hình 2017) thông qua việc phân chia các thí dụ được định nghĩa lại để kiểm tra và ẩn chứa để giải quyết vấn đề này.
...
> 困难的工程问题是采样: bạn làm thế nào để cắt cắt trong khung tranh cử không giống với biên giới hình ảnh một khu vực đặc trưng cố định?
...
1. **Backbone**ResNet-50 hoặc ResNet-101 được đào tạo trên ImageNet.
   Trung ngữ翻译: 在 ImageNet 上训练的ResNet-50或ResNet-101──生成步幅为4、8、16、32的层级特征图──
2. **FPN (Feature Pyramid Network)** kết nối phía trên xuống + bên cung cấp cho mỗi kênh cấp C các tính năng giàu ngữ nghĩa.
   Từ trên và xuống tiếp nối, cho mỗi cấp cung cấp C 个通道的语义丰富特征──检测时查询与物体大小匹配的 FPN 层──
3. **RPN (Region Proposal Network)** một đầu con nhỏ, ở mỗi vị trí neo, dự đoán "có một đối tượng ở đây không?" và "Tôi tinh chỉnh hộp như thế nào?".
   Trung ngữ翻译:一个小的卷积头, 在每个点位置预测"这里没有物体?"和"如何修改边界框?"── mỗi张图像产生约1000个候选区──
4. **RoIAlign** lấy mẫu một vá đặc tính kích thước cố định (ví dụ: 7x7) từ bất kỳ hộp nào ở bất kỳ cấp độ FPN nào.
   Trung ngữ翻译: Từ bất kỳ khung bất kỳ của cấp độ FPN 采样固定大小(如 7x7) 个性块──双线性采样,无量化──
5. **Heads** đầu hộp hai lớp tinh chế hộp và chọn một lớp, cộng với một đầu con nhỏ mà ra ra một `28x28`Mặt nạ nhị phân cho mỗi đề xuất.
   Trung ngữ翻译: hai tầng khung đầu được sử dụng để sửa đổi khung biên giới và chọn loại, cộng thêm một khối nhỏ để mỗi khu vực ứng cử xuất `28x28` 2   
...
> Bản gốc Fast R-CNN sử dụng RoIPool, nó sẽ phân chia khung ứng cử thành lưới, lấy giá trị đặc điểm lớn nhất trong mỗi khung, và sẽ đưa tất cả các biểu tượng vào số nguyên.
...
> RoIAlign trong COCO 上免费提升掩码 AP 3-4 个点──每个关心定位精度检测器现在都使用它YOLOv7 seg、RT-DETR、Mask2Former 无一例外──
...
> Trong mỗi vị trí trong biểu đồ đặc trưng đặt K 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 个 
...
> Đối với mỗi khu vực ứng cử viên (RoIAlign 之后), ẩn đầu là một FCN nhỏ: bốn 3x3 卷积, một 2x反卷积, một cuối cùng của 1x1 卷积, trong`28x28`phân giải lên tạo ra`num_classes`个输出通道―― chỉ giữ lại các đường lối đối phó với các loại dự đoán; các đường lối còn lại bị bỏ qua―― điều này sẽ che giấu các đường lối dự đoán và phân loại giải pháp──

Hãy lấy mẫu mặt nạ 28x28 với kích thước pixel ban đầu của đề xuất để tạo ra mặt nạ nhị phân cuối cùng.

> Tạo mẫu trên lớp ẩn 28x28 để tạo kích thước hình ảnh ban đầu của khu vực ứng cử, tạo ra lớp ẩn 2 giá trị cuối cùng.

### Khối thối

Mask R-CNN có bốn lỗ cộng lại:

> Mask R-CNN có 4 hàm mất tích cộng:
...
> Đây là thành phần duy nhất trong Mask R-CNN sử dụng mã hóa dễ hiểu hơn so với chữ.
...
> Mỗi số lượng đều nằm ở vị trí nhị phân. Không có trượt vào, không có định lượng, không có độ mất.
...
> Khi đó`sampling_ratio=1`且 `aligned=True`时, sự khác biệt giữa hai người`1e-5`Trong
...
> 46000000参数,91 个类别(COCO)。第一个类别(id 0) là背景;模型实际检测所有类别从id 1 开始。
...
> 掩码张量形为 `(N, 1, H, W)`❖ Nhận được các giá trị 2 của mỗi vật thể:
...
> 常见的微调方案: tái sử dụng骨干网络、FPN 和 RPN; thay thế hai phân loại đầu.
...
> `num_classes`必须包含背景类, vì vậy có 4 loại vật liệu sử dụng tập hợp dữ liệu `num_classes=5`
...
> Trong tập hợp dữ liệu nhỏ, chỉ có mục tiêu của RPN + trở lại và hai phần tham gia học.
