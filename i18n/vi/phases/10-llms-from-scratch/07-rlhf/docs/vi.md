# RLHF: Mô hình phần thưởng + PPO dựa trên học tập tăng cường chống lại con người
...
> **【中文解读】**SFT Giáo hội mô hình theo hướng dẫn, nhưng không dạy nó trả lời" tốt hơn "―RLHF sử dụng mô hình thưởng đào tạo dữ liệu ưu tiên của con người, sử dụng lại PPO  tối ưu hóa chiến lược để mô hình tạo ra các câu trả lời phù hợp hơn với phán quyết của con người― đây là để Claude có dụng、GPT 礼貌的核心技术―

> **【拓展：PPO→ChatGPT对齐】**ChatGPT's RLHF training using PPO 算法: Mô hình thưởng cho trả lời打分,PPO sử dụng số này như một tín hiệu thưởng để tối ưu hóa chiến lược。KL 惩罚防止策略偏离 SFT 模型太远── đây là một quy trình cốt lõi của Antropic/OpenAI đối với tập luyện。

>  **【前置】**学本节前请先掌握:Phase 10·06(SFT) RLHF 的起点是 SFT 模型;Phase 09·08(PPO) 强化学习 PPO 算法基础;Phase 18·01(Instruction 后续) RLHF 在对齐中的位置──本节是Phase 10·08(DPO) 和Phase 18(伦理) 系列的前置──
...
>  **【类比】**RLHF = 训练物学杂技。SFT = "làm biểu tình để chó chăm sóc học"(监督学习)。RLHF = "con chó làm một động thái, bạn cho零食(lợi khen cao) hoặc bỏ qua(lợi khen thấp), chó慢慢学会讨零食的动作"(强化学习)。奖励模型 = biểu hiện trên khuôn mặt bạn(预测哪些动作你会奖励),PPO = 狗调动动策略。KL 惩罚 = "别太离谱"狗不能为了零食转圈咬自己尾巴──

> ️ **【易错点】**RLHF của 3 个坑:(1) **奖励模型过拟合**RM trong dữ liệu ưu tiên acc=99%, nhưng biến đổi toàn diện; sử dụng RM + 早停 + 验证集监控──(2) **KL 系数设错**太大(> 0.5)模型不动如SFT,太小(< 0.01)模型乱跑"奖励黑客";典型 0.05-0.2──(3) **reward hacking** Mô hình tìm thấy "加更多表情符号 RM 给高分",输出全是emoji; liên tục giám sát phân phối输出, tìm thấy bất thường ngay lập tức dừng lại.
...
- Thực hiện vòng đào tạo PPO tối ưu hóa chính sách mô hình ngôn ngữ so với mô hình phần thưởng với hình phạt KL
  Thực hiện vòng tập luyện PPO, trong KL 惩罚约束 nhắm vào mô hình thưởng tối ưu hóa mô hình ngôn ngữ chiến lược
- Giải thích tại sao RLHF yêu cầu ba mô hình (SFT, phần thưởng, chính sách) và cách hạn chế KL ngăn chặn việc tấn công phần thưởng
  解释 tại sao RLHF  cần ba mô hình ((SFT、奖励、策略), và KL 约束 làm thế nào để ngăn chặn奖励黑客
- Đánh giá hiệu quả của RLHF bằng cách so sánh chất lượng phản ứng trước và sau khi tối ưu hóa ưu tiên
  Thông qua so sánh ưu tiên tối ưu hóa trước sau của chất lượng phản hồi để đánh giá hiệu quả của RLHF

> **【中文解读】**本课实现 RLHF的完整管线: từ mô hình thưởng huấn luyện dữ liệu ưu tiên của con người, đến sử dụng PPO 算法优化语言模型策略。RLHF 需要三个模型协同工作SFT 模型(参考策略)、奖励模型(评分器)、策略模型(被优化模型)。KL 散度惩罚防止策略模型偏离SFT 模型太远,避免奖励客黑客(奖励黑客)。

## Vấn đề  vấn đề giới thiệu

Hãy hỏi một mô hình "Thiết lý máy tính lượng tử" và nó có thể tạo ra:

> 问模型"解释量子计算", nó có thể tạo ra:
...
> **回复 A：**"Việc sử dụng tính toán lượng tử có thể nằm trong các bit lượng tử của trạng thái chồng lên, có nghĩa là chúng có thể đồng thời là 0 ≠ 1 hoặc hai người. Điều này giúp máy tính lượng tử xử lý một số tính toán nhanh hơn các máy tính cổ điển. Các thuật toán quan trọng bao gồm các thuật toán Shor được phân tích bằng số lớn và các thuật toán Grover được sử dụng để tìm kiếm trong cơ sở dữ liệu không trật tự".
...
> **回复 B：**"Quantum computing là một loại tính toán sử dụng hiện tượng về lực lượng. Nó lần đầu tiên được đưa ra vào những năm 1980 Richard Feynman đề xuất rằng hệ thống lượng tử có thể được mô phỏng bởi máy tính lượng tử.
...
> 两个回复事实上都正确――语法上都无问题――都遵循命令――但回复 A 明显更好――更简洁、更有信息量、结构更好――人类每次都会选 A――
...
> SFT không thể nắm bắt sự khác biệt này. Nó trong mô hình đào tạo "trực sự" trả lời, nhưng không có cơ chế nói "đây trả lời tốt hơn so với đó"
...
> RLHF đã giải quyết vấn đề này. Nó đào tạo một mô hình thưởng để dự đoán sự lựa chọn của con người, sau đó sử dụng tín hiệu thưởng này thúc đẩy mô hình ngôn ngữ tạo ra kết quả chất lượng cao hơn.

> **【中文解读】**Hạn chế của SFT nằm trong việc nó không thể phân biệt "những câu trả lời tốt hơn" hai ngôn ngữ chính xác, thực tế chính xác có thể khác nhau về hữu ích. RLHF thông qua mô hình khuyến khích được đào tạo để dự đoán sự thích của con người, sử dụng lại mô hình khuyến khích này để tạo ra sản lượng chất lượng cao hơn.

> **【拓展：InstructGPT 的突破】**Bài luận của InstructGPT 论文(OpenAI 2022) là RLHF được sử dụng trong mô hình lớn. 里程碑──关键发现:(1) 1.3B 参数 của RLHF  mô hình tốt hơn 175B của GPT gốc-3;(2) RLHF  giảm đáng kể tác hại xuất khẩu(毒性 giảm khoảng 80%);(3) thực chất cũng được nâng cao.
...
> RLHF không phải là một tập luyện đơn lẻ. Nó là một chuỗi chuỗi ba giai đoạn, mỗi giai đoạn được xây dựng trên một giai đoạn trước.

**Stage 1: SFT.**Thực hiện một mô hình cơ bản trên các cặp hướng dẫn-đáp ứng (Dạy 06).

> **阶段 1：SFT。**Trong hướng dẫn-đổi lại đối với mô hình cơ bản tập luyện trên (第六课) .
...
> **阶段 2：奖励模型。**收集人类偏好数据:向标标签者展示对同一提示的两个回复并问"哪个更好?"训练一个模型来预测这些偏好――奖励模型以 (quan, phản ứng) 为输入,输出一个标量分数――
...
> **阶段 3：PPO。**Sử dụng mô hình thưởng cho mô hình ngôn ngữ tạo ra tín hiệu đào tạo. Mô hình ngôn ngữ tạo ra phản hồi. Mô hình thưởng cho chúng một phần.

> **【中文解读】**RLHF's Three Stage Pipeline:Stage 1 sử dụng SFT 让基础模型学会遵循指令;Stage 2 收集人类偏好数据(对同一提示的两个回复,标记"哪个好") 训练奖励模型;Stage 3 sử dụng PPO 算法让策略模型产生高奖励的回复,同时使用 KL 散度惩罚防止偏离 SFT 模型;;KL 惩罚是反奖励黑客的关键没有它,策略会找到奖励模型的漏洞而不是真正改善输出质量;;
...
> Mô hình thưởng là một mô hình ngôn ngữ được sử dụng lại như một mô hình ngôn ngữ của một nhà đánh giá.

Input: một prompt kết nối với một câu trả lời. Output: một điểm thưởng scalar duy nhất.

> 输入: một prompt 拼接一个回复──输出: một标量奖励分数──
...
> 训练数据是人类偏好对──对于每一个提示,标签者看两个回复并选择更好──这创建了训练三元组:(快速, 首选回复, 拒绝回复)──
...
> Đó là cách quan trọng.`sigmoid(reward(A) - reward(B))`给出回复 A được ưu tiên hơn trả lại B 概率──损失推动奖励模型给首选回复分配更高分数──
...
> Tại sao sử dụng đánh giá đối với so với điểm tuyệt đối? Bởi vì con người không giỏi phân phối điểm đánh giá chất lượng tuyệt đối (((" Câu trả lời này là 7.3 trong 10 phút hoặc 7.5?") nhưng rất giỏi so với so sánh ((("A so với B tốt hơn?")  Mô hình Bradley-Terry sẽ tương đối chuyển đổi thành hệ thống đánh giá tuyệt đối phù hợp.
...
> **InstructGPT 数据：**OpenAI đã thu thập 33.000 người so sánh đối với 40 nhà thầu. Mỗi so sánh đã tốn khoảng 5 phút.
...
> PPO là một loại thuật toán học mạnh mẽ. Trong RLHF, " môi trường " là mô hình thưởng, "智能体 " là mô hình ngôn ngữ, "动作 " là tạo ra một token.
...
> Thứ nhất thúc đẩy mô hình tạo ra phần thưởng cao回复── thứ hai(KL 散度惩罚) ngăn chặn mô hình偏离 SFT 检查点太远──
...
> Tại sao cần KL  trừng phạt? Không có nó, mô hình sẽ tìm ra giải pháp bị hủy hoại. mô hình thưởng sẽ tìm ra kết quả cao nhưng thực sự vô nghĩa trên mô hình thưởng.

- Lặp lại "Tôi rất hữu ích và vô hại!" ghi điểm cao trên các mô hình phần thưởng hữu ích / vô hại
  Trung ngữ翻译:重复"我很有用很无害!" trên mô hình thưởng hữu ích / vô hại điểm cao
- Tạo ra những câu trả lời có âm thanh chính thức nhưng trống rỗng phù hợp với "bất lượng cao"
  Trung ngữ翻译:生成冗长、正式但空洞的回复,模式匹配到"高质量"
- Sử dụng cụm từ cụ thể xảy ra liên quan đến phần thưởng cao trong dữ liệu đào tạo
  Trung ngữ翻译:利用训练数据中恰好与高奖励相关的特定短语
...
> KL 惩罚说: Bạn có thể cải thiện, nhưng không thể trở thành mô hình hoàn toàn khác biệt.
...
> **InstructGPT 数据：**PPO  đào tạo sử dụng lr=1.5e-5, KL 系数 beta=0.02,256K 个节目(quan-回复对), mỗi lô 4 个 PPO epoch── toàn bộ RLHF 管线 trên GPU 集群运行了几天──
...
> PPO sử dụng "đánh chặn mục tiêu đại lý" để ngăn chặn quá nhiều cập nhật. tỷ lệ giữa tỷ lệ tỷ lệ có thể bị cắt ngang trong phạm vi [1 - epsilon, 1 + epsilon], trong đó epsilon thường là 0,2 .
...
> 基线 thường là phần thưởng trung bình hồi đáp gần đây. n lợi nghĩa là trả lời tốt hơn mức trung bình; n lợi nghĩa là chênh lệch so với mức trung bình. n lợi nghĩa là tăng tỷ lệ trả lời cao hơn mức trung bình, giảm tỷ lệ trả lời thấp hơn mức trung bình.
...
> 截断防止灾难性更新── nếu một lần trả lời nhận được phần thưởng cao bất thường, tỷ lệ không bị cắt có thể rất lớn, dẫn đến mô hình chuyển hướng mạnh mẽ sang đó.
...
> Mô hình ngôn ngữ của RLHF được tối ưu hóa nhằm vào mô hình thưởng, trong khi mô hình thưởng là một đại diện không hoàn hảo của sự thích nghi của con người.
...
> 缓解策略:更强的 KL 惩罚( ngăn chặn mô hình chuyển hướng đến đủ để sử dụng mức độ yếu điểm)  tập luyện trên mô hình phản đối mô hình thưởng mô hình (修复已知失败模式)  sử dụng nhiều mô hình thưởng khác nhau cấu trúc (更难同时攻破所有模型) 
...
> Các mô hình RLHF  đào tạo cấu hình đối với:InstructGPT sử dụng 33K  ưu tiên đối với và 6B  mô hình thưởng;Llama 2 Chat sử dụng khoảng 1M  ưu tiên đối với và 70B  mô hình thưởng;Anthropic của RLHF 论文 trong 22K so với trên đào tạo đối với mô hình thưởng 52B ⋅
...
> Bài luận của Anthropic 2022 đã so sánh 22.000 người trên đào tạo mô hình thưởng 52B. mô hình thưởng lớn hơn tạo ra tín hiệu đáng tin cậy hơn, làm cho đào tạo PPO ổn định hơn. mô hình thưởng nhỏ được đào tạo bằng mô hình ngôn ngữ lớn có nguy cơ. mô hình thưởng không có đủ dung lượng để nắm bắt sự khác biệt nhỏ giữa phản hồi tốt và phản hồi xấu.
...
> Trong quá trình sản xuất, người đánh dấu người tạo ra dữ liệu ưu tiên. Chúng tôi sẽ tạo ra các đối tượng tổng hợp, trong đó "đầu tiên chọn" phản hồi trên mục tiêu tốt hơn.
...
> 首选回复简洁直接──被拒回复显示常见失败模式:不必要的填充、模糊、冗余解释和不精确── đây chính là SFT không thể nắm bắt nhưng RLHF có thể phân biệt sự khác biệt──
...
> Mô hình thưởng đã sử dụng cấu trúc biến thể mini GPT, nhưng sẽ thay thế từ表大小的输出头为单个标量投影──
...
> Mô hình thưởng lấy biểu tượng cuối cùng  vị trí ẩn trạng thái并 chiếu như biểu tượng. Tại sao là biểu tượng cuối cùng? Vì sự chú ý ẩn ý có nghĩa là vị trí cuối cùng đã quan tâm đến tất cả các biểu tượng trước đó. Nó đối với toàn bộ chuỗi có biểu tượng hoàn chỉnh nhất.

### Bước 3: Bradley-Terry Loss

Tập mô hình phần thưởng trên cặp ưu tiên bằng cách sử dụng tỷ lệ mất đôi Bradley-Terry.

> Sử dụng Bradley-Terry 成 đối với lỗ trong ưu tiên đối với trên tập luyện mô hình thưởng.
...
> Chỉ số tỷ lệ chính xác rất trực tiếp: tỷ lệ ưu tiên đối với tỷ lệ xếp hạng đúng của mô hình thưởng là bao nhiêu? Mô hình tự động có điểm số 50%. Mô hình thưởng được đào tạo tốt trên dữ liệu sạch nên vượt quá 70%. Mô hình thưởng của InstructGPT đạt tỷ lệ chính xác khoảng 72% so với so sánh giữ, nghe có vẻ thấp nhưng thực tế rất tốt.
...
> 完整的PPO 很复杂──这一实现捕获了核心机制:生成回复、评分、计算优势、使用 KL 惩罚更新策略──
...
> RLHF 后, điểm số của mô hình chiến lược trên mô hình thưởng 应高于原始 SFT 模型的回复──
...
> 本课产 出 `outputs/prompt-reward-model-designer.md` Một lời nhắc được sử dụng để thiết kế mô hình thưởng đào tạo ống dẫn.
...
1. Thay đổi mô hình phần thưởng để sử dụng trung bình của tất cả các trạng thái ẩn thay vì chỉ vị trí cuối cùng. So sánh độ chính xác. Cách tiếp cận tích hợp trung bình cho mỗi mã thông báo trọng lượng bằng nhau, trong khi cách tiếp cận vị trí cuối cùng dựa trên sự chú ý nguyên nhân đến thông tin tổng hợp.
   Trung ngữ翻译: sửa đổi mô hình thưởng sử dụng trung bình của tất cả các trạng thái ẩn chứ không chỉ là vị trí cuối cùng.
...
2. Thực hiện hiệu chuẩn hóa mô hình phần thưởng. Sau khi đào tạo, chạy tất cả các cặp ưu tiên qua mô hình phần thưởng và tính toán: (a) phần thưởng trung bình cho các câu trả lời ưu tiên, (b) phần thưởng trung bình cho các câu trả lời bị từ chối, (c) biên (họ thích trừ bị từ chối). Một mô hình được hiệu chuẩn tốt nên có biên rõ ràng. Sau đó thêm 4 cặp ưu tiên mới và kiểm tra xem biên giữ dữ liệu không được nhìn thấy hay không.
   Trung ngữ翻译:实现奖励模型校准――训练后,将所有偏好对通过奖励模型计算:(a) 首选回复的平均奖励,(b) 被拒回复的平均奖励,(c) 边距(首选减被拒绝) ―― 良好校准的模型应有明显边距――然后添加 4 个新偏好对,检查边距是否保持在未见数据上──
...
3. Mô hình thưởng tạo điểm cao cho các phản ứng dài (trả thưởng = len( phản ứng) / 100). chạy PPO với mô hình thưởng sai lầm này và quan sát mô hình chính sách tạo ra kết quả lặp đi lặp lại ngày càng dài. Sau đó thêm một hình phạt KL 0,1 và cho thấy nó ngăn chặn hành vi thoái hóa.
   Trung ngữ翻译:模拟奖励黑客── tạo một mô hình thưởng cho dài回复高分的奖励模型(奖励 = len(响应) / 100) ・・・ sử dụng mô hình thưởng có lỗ hổng này để vận hành PPO, quan sát mô hình chiến lược tạo ra越来越长、重复的输出──然后添加 0.1 的 KL 惩罚,展示它能防止退化行为──
...
4. Thực hiện một phần thưởng đa mục tiêu. Cử lý hai mô hình phần thưởng - một cho sự hữu ích và một cho sự ngắn gọn. Kết hợp chúng như R = 0,7 * R_helpful + 0,3 * R_concise. Cho thấy mục tiêu kết hợp tạo ra các phản ứng có ích và ngắn gọn, tránh bẫy từ ngữ của một phần thưởng hữu ích duy nhất.
   Trung ngữ翻译:实现多目标奖励──训练两个奖励模型一个用于有用性,一个用于简洁性──组合为R = 0.7 * R_helpful + 0.3 * R_concise──展示组合目标产生既有用又简洁的回复,避免单一有用性奖励的冗长陷──
...
5. So sánh các hệ số KL khác nhau. chạy PPO với beta = 0,001 (quá thấp, reward hacking), beta = 0,02 (thực kỳ), và beta = 0,5 (quá cao, không học).
   Trung ngữ翻译:比较不同 KL系数──用beta=0.001(太低,奖励黑客)、beta=0.02(标准) 和beta=0.5(太高,不学习)运行 PPO──绘制每个组的奖励曲线和 KL 曲线──beta=0.02 应显示稳定的奖励提升和有界的 KL──
