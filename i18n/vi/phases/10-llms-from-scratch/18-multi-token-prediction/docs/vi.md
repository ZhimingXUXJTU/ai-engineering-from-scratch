# Multi-Token Prediction (MTP) 更多Token 预测
...
> **【中文解读】**传统 LLM Mỗi vị trí chỉ dự đoán một token tiếp theo――DeepSeek-V3 ở mỗi vị trí tăng một lỗ thứ hai: dự đoán dưới một token tiếp theo――额外14B 参数通过梯度流蒸回主模型, được đào tạo tốt MTP đầu trong suy luận được sử dụng như một đầu tư giải mã bản thảo khi đưa ra dự đoán(80%+ 接受率),1.8 倍吞吐提升免费获得──

> **【拓展：MTP→DeepSeek-V3创新】**MTP là một trong bốn cấu trúc mới của DeepSeek-V3 (MLA + MoE + MTP + DualPipe) ⋅ nó cũng cải thiện đào tạo (more丰富的梯度信号) và đưa ra các ý tưởng (免费获得投机解码草稿器), là ví dụ của "trenning时投资"", đưa ra các ý tưởng" ⋅

>  **【前置】**学本节前请先掌握:Phase 10·04(Pre-Training GPT);Phase 10·15(Sự giải mã suy đoán)MTP 在推理时变成草稿器;因果链(链因果) 概念──本节是Phase 10·20(DeepSeek-V3 Walkthrough) 的前置──
>  **【类比】**传统下标语时时记一个单词――MTP = 同时记"这个单词"和"下一个常用搭配"训练时学更多结构(梯度信号丰富),推理时把"搭配预测器"当草稿器免费加速(80% 接受率)
...
- Cụ thể mục tiêu đào tạo MTP và rút ra sự mất mát chung qua độ sâu dự đoán.
  Nói rõ MTP  tập luyện mục tiêu,推导跨预测深度的联合损失
- Giải thích sự khác biệt giữa các đầu MTP song song (2024) của Gloeckle et al. và các mô-đun MTP theo trình tự của DeepSeek-V3 và lý do tại sao thiết kế theo trình tự bảo tồn chuỗi nguyên nhân.
  Giải thích sự khác biệt giữa các mô hình MTP của Gloeckle và các mô hình MTP của DeepSeek-V3, cũng như lý do tại sao các mô hình MTP được thiết kế để giữ chuỗi kết quả
- Xét các tham số và bộ nhớ trên của việc thêm các mô-đun MTP vào một cuộc chạy trước khi tập luyện.
  计算 trong dự kiến đào tạo thêm MTP 模块 của các tham số và bộ nhớ
- Thực hiện một mô-đun MTP từ đầu: nhúng chung, khối biến đổi độ sâu, chiếu và đầu đầu ra chung.
  Từ zero thực hiện một MTP 模块: chia sẻ nhúng  mỗi độ sâu Transformer 块  chiếu và chia sẻ đầu ra
...
> 下一代币 预测是标准 LLM 训练目标――每个隐藏状态被监督预测恰好一个事:紧接下来的下一个代币――这是一个惊人的弱信号――序列中的大部分信息延伸到单个代币――外结构"",连贯性"",事实性"",算术流程――模型必须通过在亿代币上积累许多单代币 信号来学习这些――
...
> MTP 问: Nếu mỗi trạng thái ẩn được giám sát đồng thời dự đoán nhiều token tương lai 会怎样?Gloeckle 等人(Meta,2024) cho thấy điều này có ích. Việc thực hiện của họ ở trên đỉnh mạng xương rồng đặt ra một số đầu ra độc lập, mỗi dự đoán có sự chuyển hướng khác nhau.
...
> **【中文解读】**Nhiều token  dự đoán để mô hình đồng thời dự đoán nhiều token trong tương lai (trừ việc chỉ dự đoán một token tiếp theo), cung cấp tín hiệu đào tạo phong phú hơn. Nghiên cứu của Meta cho thấy nhiều token  dự đoán không chỉ tăng tốc đào tạo, mà còn có thể nâng cao khả năng lập kế hoạch và tính toán của mô hình.

> **【拓展：Meta 的多 token 预测研究】**Meta đã công bố một bài báo năm 2024 cho thấy, 4-token  dự đoán trong nhiệm vụ tạo mã và suy đoán rõ ràng hơn tiêu chuẩn tiếp theo của một token  dự đoán. DeepSeek-V3 cũng sử dụng hỗ trợ nhiều token  dự đoán đầu để tăng cường tín hiệu đào tạo.
...
> **【拓展：多 token 预测与推理加速的联系】**Nhiều token  dự đoán mô hình đào tạo tự nhiên thích hợp với đầu tư giải mã vì nó đã học được dự đoán nhiều token trong tương lai── Nghiên cứu của Meta cho thấy, mô hình 4 token  dự đoán đào tạo trong code generation tăng lên 5-10% so với chuẩn đào tạo──
...
> 本课产 出 `outputs/skill-mtp-planner.md`△给定预训运行规范(模型大小、数据、算力), nó trở lại tập hợp kế hoạch của MTP: độ sâu D、`lambda`调度、内存开销和推理时投机解码连线──

## Tập luyện bài tập

1. Đi chạy`code/main.py`. Cho thấy mất độ sâu giảm một cách đơn giản khi tín hiệu tổng hợp tăng cường.
   Trung ngữ翻译:运行 `code/main.py` hiển thị mỗi mất độ sâu cùng với tín hiệu tổng hợp tăng cường đơn调递减;; sửa đổi dữ liệu tổng hợp sử dụng mô hình cố định并验证 độ sâu 1 và độ sâu 2 của mất đều nhận được🏻
...
2. Xét số lượng trên của một mô hình 70B dày đặc (bỏ 8192, 80 lớp) với mô-đun D=1 MTP. So sánh với số lượng trên 14B được báo cáo của DeepSeek-V3. Giải thích lý do tại sao số lượng của DeepSeek cao hơn: khối biến đổi MTP thừa hưởng cấu trúc MoE tương tự, làm tăng số lượng các tham số mỗi mô-đun.
   Trung ngữ翻译:计算密集 70B 模型( ẩn 8192,80 层)加D=1 MTP 模块的参数开销。与DeepSeek-V3 报告的14B 开销比较。解释为什么DeepSeek 的数字更高:MTP biến thể 块继承相同的MoE 结构,膨胀了每个模块参数。
...
3. Thực hiện D=2 trong đồ chơi: thêm một mô-đun MTP thứ hai lấy h^(1) và dự đoán `t_{i+2}`- Kiểm tra sự mất mát chung và tính toán các tham số phù hợp với phương trình 19-21 của bài DeepSeek.
   Trong mô hình chơi game thực hiện D=2: thêm một MTP thứ hai, lấy h^(1) 并预测 `t_{i+2}` Thử nghiệm kết hợp lỗ hổng và số lượng kiểm toán với DeepSeek 论文公式 19-21 匹配──
...
4. Chuyển đồ chơi sang MTP song song (Gloeckle-style): thêm đầu đầu ra D trên đỉnh trạng thái ẩn chính, mỗi dự đoán một sự bù đắp khác nhau. đo cách các tổn thất theo độ sâu so sánh với phiên bản theo trình trên cùng một tín hiệu tổng hợp. phiên bản theo trình nên tạo ra tổn thất độ sâu k thấp hơn cho k > 1 vì nó điều kiện trên các dự đoán trung gian.
   Trung ngữ翻译:将玩具模型换为并行 MTP(Gloeckle风格): trong trạng thái ẩn chính, thêm D 个输出头, mỗi dự đoán khác biệt chuyển động.
...
5. Sử dụng mô-đun MTP được đào tạo như một bản thảo kiểu EAGLE: gọi mô-đun k để đề xuất `t_{i+k}`đo tỷ lệ chấp nhận của các mã thông báo dự thảo này so với dự đoán của mô hình chính trên một chuỗi được giữ. Nếu bạn đạt 50% + trên đồ chơi, bạn đã tái tạo thuộc tính MTP-as-draft thực nghiệm.
   Trung文翻译:将训练的 MTP 模块用作 EAGLE 风格草稿:在推理时调用模块 k 提出 `t_{i+k}` Đánh giá các mã bản thảo trên chuỗi lưu giữ đối với tỷ lệ chấp nhận dự đoán mô hình chủ sở hữu. Nếu mô hình đồ chơi đạt 50%+, bạn đã có được tính năng kinh nghiệm của MTP-as-draft.
