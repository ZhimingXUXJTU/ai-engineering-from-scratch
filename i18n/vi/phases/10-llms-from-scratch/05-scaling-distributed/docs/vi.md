# Scaling: Đào tạo phân tán, FSDP, DeepSpeed  扩展: Đào tạo phân tán, FSDP, DeepSpeed
...
> **【中文解读】**1.24 tỷ mô hình được đào tạo trên một GPU. Nhưng 70 tỷ mô hình không được lưu trữ, một máy đào tạo dữ liệu cần vài tuần.

> **【拓展：DeepSeek-V3的2048卡训练】**DeepSeek-V3 sử dụng 2048 张 H800 GPU 训练, sử dụng DualPipe 流水线并行 + MoE 专家并行;; hiểu được phân phối tập luyện là hiểu được tiền tuyến mô hình lớn được đào tạo ra như thế nào nền tảng;;

>  **【前置】**学本节前请先掌握:Phase 10·04(Pre-Training Mini GPT)单 GPU 训练基础;PyTorch DDP 概念;多 GPU 通信(NCCL、AllReduce)。本节是Phase 10·19(DualPipe) 和Phase 10·20(DeepSeek-V3 Walkthrough) 的前置──
...
>  **【类比】**Ưu điểm:**数据并行**(DP) = 4 个体各搬一个相同的小箱子(同模型不同数据,结果 AllReduce 平均) ⋅**张量并行**(TP) = 4 người cùng nhau mang một hộp lớn trong 4 góc cùng lớp phân chia đến 4 卡)**流水线并行**(PP) = 4 个人流水线,第 1 人搬一层传给第二 人(模型按层切分)**FSDP**= DP + 把模型切片分成各卡,用时再聚合 (省显存) ・生产场景一般 3种组合用 (三维对行) ・

> ️ **【易错点】**分布式训练的3 个坑: ((1) **batch size 设错**单卡 bs=8,4 卡应该 bs=32(4×8)而非 bs=8; sử dụng kích thước lô hiệu quả 计算 lr―(2) **AllReduce 瓶颈**卡间通信比 GPU 计算慢 10 倍,bs 太小会让 GPU 等通信;bs 至少 32+才划算──(3) **没设 seed** mỗi lần bắt đầu khác nhau, kết quả không thể tái tạo;`torch.manual_seed(42) + torch.cuda.manual_seed_all(42)`
...
- Giải thích ba loại song song (dữ liệu, tensor, đường ống dẫn) và khi nào mỗi loại cần thiết dựa trên mô hình và kích thước cluster
  解释三种并行类型(数据并行、张量并行、流水线并行) và các trường hợp ứng dụng
- Thực hiện đào tạo song song dữ liệu bằng cách sử dụng PyTorch DDP với sự đồng bộ hóa gradient trên nhiều GPU
  Sử dụng PyTorch DDP  thực hiện nhiều GPU dữ liệu và tập luyện và trình độ
- Xét ngân sách bộ nhớ cho kích thước mô hình nhất định (nặng + trạng thái tối ưu hóa + gradient + kích hoạt) để xác định phần cứng tối thiểu
  计算给定模型大小的显存预算(权重 + 优化器状态 + 梯度 + 激活值), xác định nhu cầu phần cứng tối thiểu
- Cài đặt FSDP hoặc DeepSpeed ZeRO giai đoạn để phân mảnh các trạng thái mô hình trên GPU và các mô hình phù hợp vượt quá bộ nhớ GPU đơn
  Chuẩn bị FSDP hoặc DeepSpeed ZeRO 阶段 để phân chia trạng thái mô hình, để vượt qua mô hình hiển thị đơn

> **【中文解读】**本课聚焦 LLM 训练的核心瓶:显存──7B 模型在FP16 下仅权重就需要14GB,加上亚当 优化器状态(28GB) 和梯度(14GB),总数56GB还没有算激活值──你将学习三种并行策略──数据并行、张量并行、流水线并行) 以及FSDP/ZeRO 分片技术来突破单卡显存限制──
...
> 7B 参数模型在FP16 下仅权重就需要14GB──Adam 优化器为每参数额外存储两副副本──一阶和二阶矩估算),又需要28GB──反向传播的梯度再加14GB──在存储任何激活值之前,你已经达到56GB──

Một chiếc NVIDIA A100 có bộ nhớ 80GB.

> NVIDIA A100 có 80GB lưu trữ.
...
> 80GB đã tiêu thụ 56GB. Còn lại 24GB được sử dụng để kích hoạt giá trị tiền向传播中计算的中间值, phải dành cho ngược向传播保留. Đối với 2048 token 序列和4096 维模型, một tầng kích hoạt giá trị khoảng 64MB.
...
> 现在试试 70B 参数――仅权重:FP16 下 140GB──放不进一张 GPU──至少需要2张 A100(2 x 80GB = 160GB)才能放下权重──加上优化器状态和梯度,需要更多:最少3+张 GPU,实际需要8-16张,取决于分片策略──
...
> Llama 3 405B trong 16,384 张 NVIDIA H100 GPU trên đào tạo, chi phí tính toán ước tính khoảng 1 tỷ USD.
...
> Bài học này bao gồm bốn chiến lược cho phép đào tạo quy mô lớn: dữ liệu và lượng dữ liệu và dòng chảy dữ liệu và các bộ phận dữ liệu hoàn toàn. Bạn sẽ sử dụng Python để mô phỏng từng mô hình, trước khi tiếp xúc với khuôn khổ đào tạo phân tán để hiểu cơ chế của nó.
...
> Dưới đây là các tính toán toán học hiển nhiên của mô hình thực tế. Mỗi số được tính toán, không phải là ước tính.
...
> "Adam  trạng thái"列是致命的──Adam cho mỗi số liệu lưu trữ một vận hành trung bình giá trị(m) và một vận hành方差(v), là FP32── đối với 70B 模型, tức là 70B x 4 字节 x 2 = 560GB── chỉ cần 7张 A100──
...
> 单张H100 có 80GB──Llama 3 405B ít nhất cần 61张H100 để đặt trọng lượng, tối ưu hóa và thang độ──加上激活值数字更大──Meta sử dụng 16,384张 GPU không phải vì họ nghĩ mà vì họ phải──

> **【中文解读】**显存预算 là khóa học đầu tiên của LLM 关卡. Ví dụ:FP16 权重 140GB + Adam 优化器状态 560GB + 梯度 140GB = 840GB(không chứa kích hoạt giá trị) 单张 H100 只有80GB,至少需要11张 GPU才能放下这些状态.

> **【拓展：Llama 3 的 16384 GPU 训练】**Llama 3 405B trên 16,384 张 H100 GPU, được đào tạo bằng 3D并行 (Data并行 + 张量并行 + 流水线并行) ⋅ đào tạo tổng chi phí ước tính khoảng 1 tỷ USD.
...
> Chiến lược phân phối đơn giản nhất:  sẽ sao chép toàn bộ mô hình thành N 张 GPU  sẽ phân chia từng tập số thành N 等份  Mỗi GPU sẽ vận hành các phân đoạn dữ liệu của mình về phía trước và ngược chiều truyền  Sau khi truyền ngược chiều, trên tất cả GPU 间 trung bình gradient Mỗi GPU sử dụng cùng gradient trung bình để cập nhật bản quyền trọng lượng của mình, giữ tất cả các bản sao đồng thời 
...
> **优点：**吞吐量线性扩展──N 张 GPU Mỗi bước xử lý dữ liệu N 倍──通信 chỉ giới hạn ở mức trung bình, có thể được tính toán chồng chất──
...
> **缺点：**Mỗi GPU  nắm giữ mô hình ∞ trạng thái và độ của máy tối ưu hóa ∞ cho mô hình 70B, mỗi GPU ∞ cần 840GB ∞ dữ liệu ∞ không giảm mỗi GPU ∞ lưu trữ ∞ chỉ giảm thời gian đào tạo ∞
...
> **数学：**Có hiệu quả khối lượng lớn = Mỗi GPU khối lượng x N。 đối với N=64 张 GPU, mỗi GPU khối lượng 16, lượng hiệu quả là 1.024。 Llama 3 Sử dụng mỗi bước 1600.000 token khối lượng hiệu quả。
...
> Để phân chia một tầng đơn thành nhiều GPU. Một lần các矩阵乘法 được phân bổ thành nhiều GPU.
...
> 考虑前层中形为 (8192, 8192) 的权重矩阵──使用 4 路张量并行, mỗi GPU 持有 (8192, 2048) 的分片──每个 GPU 将输入乘以其分片,产生部分结果──部分结果通过全归约或全收集组合为完整输出──
...
> **优点：**Giảm trọng lượng của mỗi GPU 显存──70B 模型 phân chia thành 8 GPU nghĩa là mỗi GPU  nắm giữ khoảng 8.75B 参数 trọng lượng──
...
> **缺点：**Mỗi tầng sau đó cần GPU nhanh 间通信。 mỗi矩阵乘法后的全归约增加延迟。 đây là ở NVLink(同节点 GPU 间 900 GB/s) có hiệu quả tốt, nhưng ở InfiniBand(400 Gb/s, khoảng 50 GB/s) kết nối có hiệu quả khác nhau trên các跨节点──张量并行几乎总是限于单节点内(8张 GPU)。
...
> **实际使用：**Megatron-LM  tiên phong đề xuất 张量并行──Llama 3 405B 在每个节点内使用 8 路张量并行──

> **【中文解读】**张量并行(Tensor Parallelism) sẽ phân chia các mô hình mô hình của một tầng đơn thành nhiều GPU trên. Ví dụ: một (8192, 8192) của mô hình mô hình trọng lượng nằm trong 4 đường并行, mỗi GPU chỉ cần lưu trữ (8192, 2048) của một phân đoạn.

> **【拓展：流水线并行的气泡问题】**流水线并行将模型按层分配到不同 GPU (如 GPU1 跑 1-8层,GPU2 跑 9-16层)  但会产生"气泡"GPU空等.
...
> **优点：**GPU 间通信最小 chỉ có giá trị kích hoạt của giới hạn cấp, rất nhỏ so với thang độ hoặc trọng lượng.
...
> **缺点：**Khi GPU 4 trong tính toán các khối lượng nhỏ của 1 được chuyển tiếp, GPU 1、2、3 空(các bộ phận đã hoàn thành phần của mình)。反向传播时模式相反──朴素流水线下,N 个阶段 GPU sử dụng chỉ là 1/N。
...
> **GPipe 和 PipeDream**通過將批次分分成微批次來解決氣泡問題──GPU 1 一完成微批次 1 的前向传播就开始微批次 2──这在流水线阶段间重叠计算──M 个微批次和N 个阶段,气泡比例降至 (N-1) /M──使用M=16 个微批次和N=4 个阶段,气泡为3/16 = 18.75% 空时间──
...
> FSDP kết hợp khả năng mở rộng và hiệu quả lưu trữ của các đoạn dữ liệu cùng nhau. Mỗi GPU không có bản sao mô hình hoàn chỉnh, mà chỉ có các tham số, độ và trạng thái tối ưu hóa 1/N.
...
> Trước khi truyền tải một tầng, FSDP chạy toàn bộ hoạt động thu thập từ tất cả các GPU  thu thập các tham số hoàn chỉnh đến lưu trữ hiển thị của mỗi GPU. Sau khi truyền tải trước, mỗi GPU  bỏ qua các tham số không địa phương.
...
> Không có FSDP,70B mô hình không thể đặt vào một GPU 80GB. Sử dụng FSDP trên 8 GPU, mỗi GPU sử dụng 105GB, vv, hoặc bạn cần ít nhất 16 GPU để giảm xuống 80GB mỗi thẻ dưới đây, hoặc sẽ kết hợp FSDP với điểm kiểm tra kích hoạt khi phản truyền tính toán lại giá trị kích hoạt thay vì lưu trữ chúng)

Chi phí giao tiếp cao hơn sự song song dữ liệu vanilla vì tất cả các bộ sưu tập trước mỗi lớp.

> Chi phí giao thông cao hơn so với các bộ dữ liệu thông thường, vì mỗi tầng trước đó cần phải thu thập đầy đủ.

> **【中文解读】**FSDP( hoàn toàn phân đoạn dữ liệu và kết hợp) là kết hợp của dữ liệu và phân đoạn. Mỗi GPU chỉ lưu trữ các tham số 1/N, thang độ và trạng thái tối ưu hóa.

> **【拓展：DeepSpeed ZeRO 的三个阶段】**ZeRO Stage 1 分片优化器状态(省 4x 显存),Stage 2 加上梯度分片(省 8x),Stage 3 加上参数分片(省 N 倍,N 为 GPU 数) ・FSDP 本质上是 ZeRO Stage 3 的 PyTorch 原生实现──Llama 3 405B 的 16,384 GPU 训练使用FSDP + 张量并行 + 流水线并行 3D组合──
...
> ZeRO của DeepSpeed (không có gì khác) về khái niệm giống với FSDP, nhưng được phát triển độc lập bởi Microsoft. Nó xác định ba giai đoạn, mỗi giai đoạn được phân chia tích cực hơn:
...
> ZeRO-3 giống như FSDP. Tên khác nhau, cơ chế giống nhau. PyTorch đã xác nhận khái niệm sau đó FSDP sẽ được thực hiện như một phần tử thực tế.
...
> DeepSpeed cũng giới thiệu ZeRO-Offload (tạm dịch: "tạm dịch: ZeRO-Offload") sẽ nâng cấp tình trạng tải xuống CPU 内存, rẻ hơn và lớn hơn) và ZeRO-Infinity (tạm dịch: "tải xuống NVMe SSD").

### Việc đào tạo chính xác hỗn hợp

Việc đào tạo hiện đại sử dụng nhiều định dạng điểm nổi cùng một lúc:

> 现代训练同时 sử dụng nhiều kiểu floating point:
...
> **【中文解读】**混合精度训练是现代 LLM 训练的标准做法:前向传播用 BF16(16位),优化器维护 FP32 主权重(32位),损失缩放防止梯度下溢──BF16 có cùng một chỉ số phạm vi với FP32 ((8位), nhưng độ精度 giảm(7位尾数 vs 23位), hầu như không cần mất tích缩放──业界已从 FP16 全面显度转向 BF16──混合精为 7B 模型节省约28GB ⋅

> **【拓展：3D 并行与 MoE 的经济性】**Llama 3 405B sử dụng 3D và đường:节点间数据并行 + 节点内 8 路张量并行 + 跨节点流水线并行。DeepSeek-V3 sử dụng MoE(混合专家) cấu trúc giảm chi phí mỗi lần chuyển tiếp truyền tải chỉ kích hoạt khoảng 37B 参数(总参数 671B), chi phí đào tạo chỉ khoảng 560 triệu USD, là 1/18 của Llama 3
...
> Đây là một cách để mở rộng thành hàng ngàn GPU. Mỗi GPU có một đoạn dữ liệu khác nhau.
...
> DeepSeek V3  đã sử dụng một cách khác nhau. Các cấu trúc chuyên gia hỗn hợp của họ cho mỗi token chỉ kích hoạt 671B 参数 trong 37B. Điều này có nghĩa là mỗi GPU chỉ cần tính toán.
...
> Để phân chia các khối lượng thành GPU có mô hình trên. Mỗi GPU trên phân đoạn của nó được tính toán trước và truyền đi.
...
> Hoạt động hoàn chỉnh: mỗi GPU sẽ gửi 1/N của một thang đến hàng xóm, từ hàng xóm khác nhận 1/N,N-1 bước sau mỗi GPU đều có giá trị trung bình hoàn chỉnh. Tổng số lượng giao tiếp: 2 x 梯度大小 x (N-1) /N, đối với N lớn gần 2 lần 梯度大小.
...
> 将权重矩阵分为多张 GPU──每张 GPU 计算部分矩阵乘法──组合结果──
...
> 误差应该恰好为零(或机器epsilon) ――张量并行在数学上是精确的它产生与在一张GPU上计算完整矩阵乘法相同的结果──拆分沿输出维度进行,每个GPU 产生不同的列块,拼接重建完整结果──
...
> Đối với các dòng chảy, các dòng chảy và các dòng chảy khác nhau, các dòng chảy và các dòng chảy khác nhau, các dòng chảy và các dòng chảy khác nhau, các dòng chảy và các dòng chảy khác nhau, các dòng chảy và các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy và các dòng chảy khác nhau, các dòng chảy và các dòng chảy khác nhau, các dòng chảy và các dòng chảy khác nhau, các dòng chảy và các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác nhau, các dòng chảy khác, các dòng chảy khác, các dòng chảy khác, các dòng chảy khác, và các dòng chảy khác, các dòng chảy khác, và các dòng chảy khác, trong các dòng chảy khác, trong các dòng chảy khác, trong các dòng chảy này tránh được chuyển về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về về
...
> Để phân chia các lớp của mô hình thành GPU ảo trên, hãy trình bày các vấn đề về các loại khí trong giai đoạn đầu và giai đoạn sau.
...
> 4 giai đoạn và 1 khối nhỏ, tỷ lệ búm là 75% trong 4 phần ba GPU  bất cứ lúc nào đều trống 🏼 16 khối nhỏ giảm xuống khoảng 19% 🏼 loại bỏ giá trị của búm là hiển nhiên: bạn phải cùng lúc lưu trữ tất cả các khối nhỏ để thực hiện giá trị kích hoạt 🏻
...
> Máy tính này trả lời câu hỏi của mỗi kỹ sư ML:"Tôi cần bao nhiêu GPU?"
...
> Đối với hầu hết mọi người, sự bất ngờ lớn nhất: độ chính xác hỗn hợp sẽ không giảm một nửa trong số lượng hiển thị.
