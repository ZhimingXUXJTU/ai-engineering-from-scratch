...
> **【拓展：推理优化→生产部署】**vLLM、TensorRT-LLM、TGI 等推理框架都实现 KV-cache 管理、连续批处理 (continuous batching)、PagedAttention等优化──理解这些技术是将大模型部署到生产环境的关键──

>  **【前置】**Học本节前请先掌握:Phase 10·01-08(Transformer 架构、Attention);GPU 计算 vs 显存带宽概念──本节是Phase 17 (Phase 17 (Phase 17 (Phase 1) 基础设施 &生产) 的前置生产 LLM 部署核心──

>  **【类比】**LLM 推理 = 读书 + 写读书笔记。**Prefill**(处理 prompt) = 快速翻完书(并行扫所有代币,计算密集);**Decode**(生成) = 逐字写笔记 ((一次写一个字,显存带宽密集)**KV cache**= 翻书时做书签(记住每页关键信息,写笔记时不用重翻);**continuous batching**= 同时给多人读书会服务 (动态拼组)
...
- Thực hiện KV-cache để loại bỏ tính toán dư thừa trong quá trình tạo token tự động
  实现 KV 缓存 để loại bỏ các token tự quay trở lại trong quá trình sinh sản
...
- Thực hiện các khái niệm batching liên tục và PagedAttention để tối đa hóa việc sử dụng GPU dưới các yêu cầu đồng thời
  实现连续批处理和 PagedAttention 概念, 在并发请求下最大化 GPU利用率
- So sánh các kỹ thuật tối ưu hóa suy luận (KV-cache, giải mã suy đoán, chú ý flash) và sự thỏa hiệp thông qua/sự trễ của chúng
  比较推理优化技术(KV 缓存、投机解码、Flash Attention) và lượng lượng/tần số chậm

> **【中文解读】**本课聚焦 LLM 推理优化──两个核心概念:(1) Prefill 阶段并行处理提示,限于 GPU 计算量(FLOPS);(2) Decode 阶段 từng token 生成,限于显存带宽──KV-cache 避免重复计算已生成的代币的K/V,连续批处理最大化 GPU利用率,PagedAttention 解决 KV-cache 的存片问题显――
...
> Bạn đang sử dụng 4xA100 GPU trên Llama 3 70B── một người dùng nhận được khoảng 50 token/ giây── cảm giác rất chảy── sau đó 100 người dùng cùng thời gian truy cập vào điểm kết thúc── dung lượng giảm xuống còn mỗi người dùng 3 token/ giây── bạn mỗi tháng 25.000 đô la GPU 账单 cung cấp hơn người dùng打字还慢的响应──
...
> Mô hình tự nó không thay đổi giữa 1 người dùng và 100 người dùng. cùng trọng lượng, cùng cấu trúc, cùng toán học. Thay đổi là làm thế nào bạn điều chỉnh công việc. Lý do đơn giản lãng phí 90% + của GPU có thể sử dụng trong tính toán.
...
> Đây không phải là vấn đề mở rộng, mà là vấn đề điều chỉnh.$25k 推理账单降为服务相同流量的 $5k 账单的关键──
...
> vLLM trong 4xA100-80GB 上服务 Llama 3 70B, thấp并发时 đạt khoảng 50 token/ giây/ người dùng, thông qua tiếp tục xử lý và PagedAttention trong 100并发请求维持 15-25 TPS/ người dùng. Không có những tối ưu hóa này, cùng một phần cứng trong đó并发下 chỉ phục vụ 5 TPS/ người dùng.

## Khái niệm cốt lõi

> **【中文解读】**LLM 推理 có hai giai đoạn khác nhau: Prefill 阶段 bị giới hạn trong GPU FLOPS (tỷ lệ tính toán), Decode 阶段 bị giới hạn trong hiển thị lưu trữ带宽 (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ lưu trữ) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ lệ phí) (tỷ phí)

> **【拓展：vLLM 和 PagedAttention】**VLLM 引入 PagedAttention sẽ quản lý bộ nhớ cache KV như bộ nhớ trong ảo, sử dụng bộ nhớ trang KV chiều kích cố định, giải quyết vấn đề các mảnh vỡ lưu trữ hiển thị. Điều này làm cho dung lượng của vLLM cao hơn HuggingFace TGI 2-4 lần.
...
> Mỗi yêu cầu đề xuất LLM có hai giai đoạn khác nhau.
...
> **预填充**处理整个输入快速――所有代币 已知,所以注意力可以在完整序列上并行计算――这是大型矩阵乘法GPU 核心保持忙碌――瓶是计算:硬件每秒能提供多少FLOPS──A100做 312 TFLOPS(BF16)──70B 模型上 4,096-代币快速的预填在单张A100 上约需要400ms──
...
> **解码**个别生成输出代币――每个新代币 关注所有前代币,但每次前向传播只产生一个代币――权力矩阵与预填时大小相同,但你用单向量而不是矩阵与相乘――GPU核心在微秒内完成,然后等下一批权重从内存到内存.瓶是内存宽度:你能以多快的速度将模型权重从HBM传输到计算单元――A100 有 2 TB/s 带宽――FP16 的 70B 模型是140 GB――读完整模型需要70ms这是单次解码步骤的下限.
...
> 核心洞察:* giải mã là kiểu bộ nhớ dày đặc, bởi vì bạn đọc toàn bộ mô hình để tạo ra một token đơn lẻ. Mỗi lần tối ưu hóa dưới đây hoặc giảm lượng đọc, hoặc tăng từng lần đọc xử lý token 批次, hoặc hoàn toàn tránh đọc.
...
> Trong tính toán chú ý, mỗi token được truy vấn liên quan đến tất cả các token trước đây và giá trị của các token. Không có lưu trữ, tạo token N cần phải tính lại tất cả các N-1 个 token trước đây và giá trị của các token.
...
> KV 缓存存储 tất cả các token trước tiên của khóa và giá trị dự báo ⋅ tạo token N ⋅, bạn chỉ tính toán các token N của khóa và giá trị, sau đó với token của cache 1 đến N-1 của K / V 拼接──
...
> Llama 3 70B 128K trên 下文对话消耗 40 GB KV 缓存半张 A100 的内存──100 发发用户每 4K token 时,只 KV 缓存就需要 128 GB──这就是为什么 KV 缓存管理是推理优化的核心挑战──
...
> 静态批处理 chờ một lô N 个 yêu cầu đến, cùng xử lý, chờ* tất cả* hoàn thành sau khi tiếp nhận yêu cầu mới. Nếu một yêu cầu cần 500 token 另一个需要 10 token,短请求在完成后置 490 个解码步骤――

Các gói liên tục (còn gọi là gói cấp lặp) đưa ra các yêu cầu mới vào gói ngay khi bất kỳ yêu cầu nào hoàn thành.

> 连续批处理(也叫代级批处理) 在任何请求完成后立即将新请求插入批次──批次在每个解码步骤重新评估──10 代币 完成后的请求立即被等待中的请求替换──
...
>  Tăng lượng tăng phụ thuộc vào sự thay đổi độ dài của sản xuất.  Tăng lượng trung bình, tiếp tục xử lý hàng loạt giống như xử lý hàng tĩnh.
...
> Mỗi yêu cầu KV 缓存 là một khối bộ nhớ liên tục. Khi yêu cầu đến và rời đi, bộ nhớ bị phân mảnh như trong hệ điều hành.
...
> PagedAttention( từ vLLM) sẽ sử dụng bộ nhớ ảo của hệ điều hành风格 để sử dụng KV 缓存。 không dành cho mỗi yêu cầu phân phối một khối liên tục, mà phân phối một "đường" trang (từ 16 mã thông báo)。 trang có thể được hiển thị ở bất kỳ vị trí nào trong bộ nhớ GPU vật lý。 trang bảng sẽ hiển thị vị trí của mỗi yêu cầu về các logic.
...
> 解码慢是因为它是顺序的生成一个代币,反回去,生成下一个――但如果你能低价猜下一个5代币,然后一次验证所有吗?
...
> 投机解码 sử dụng nhỏ và nhanh**草稿模型**生成 K 个候选标志──大型**目标模型**Sau đó trong một lần phát tán, xử lý tất cả các ứng cử viên K (đáng như là dự án)  cùng dòng  tính toán dày đặc  cao hiệu quả)  Nếu mô hình mục tiêu đồng ý với dự đoán mô hình sơ đồ, bạn sẽ chấp nhận tất cả các mã K trong thời gian phát tán. Nếu nó ở vị trí j không đồng ý, bạn sẽ chấp nhận mã 1 đến j-1 và không bỏ qua phần còn lại.
...
> 加速取决于**接受率** Dự đoán mô hình bản thảo多频繁地匹配目标──Llama 3 8B 为 Llama 3 70B Khi làm bản thảo, tỷ lệ chấp nhận trên ngôn ngữ tự nhiên thường là 70-85%── điều này chuyển thành 2-3 lần tăng tốc giải mã──
...
> 投机解码 là * toán học chính xác * phân bố đầu ra hoàn toàn giống nhau với phân bố mô hình mục tiêu.
...
> 缓存存储常见前的 KV 缓存并在请求间复用──当新请求带有已知前到达时,系统复制(或引用) KV 条目缓存,只计算唯一后的 KV──
...
> Đối với tất cả các yêu cầu chia sẻ 2.000 token  hệ thống nhanh chóng, dự trữ trước đã loại bỏ mỗi yêu cầu khoảng 400ms của prefill. Trong 100 yêu cầu / giây, mỗi giây tiết kiệm 40 giây GPU tính toán  hơn một GPU của công suất.
...
> 本课产 出:
> - `outputs/skill-inference-optimization.md` Chẩn đoán và tối ưu hóa các kỹ năng dịch vụ LLM
...
1. Thay đổi trình viền cache KV để so sánh lượng lượng cache FP16 vs FP8 vs INT4 KV. Đối với Llama 3 70B trong bối cảnh 4K, tính toán số lượng người dùng đồng thời tối đa cho mỗi người trên 4xA100-80GB.
   Trung文翻译:修改 KV 缓存分析器比较 FP16 vs FP8 vs INT4 KV 缓存量化。 đối với 4K 上下文的Llama 3 70B,计算 4xA100-80GB 上每种格式最大并发用户数──INT4 KV 量化应约4倍用户容量──

2. Tăng bộ mô phỏng đợt phát triển liên tục để theo dõi việc sử dụng GPU (phần nhỏ các khe đợt được lấp đầy mỗi bước).
   Trung文翻译:扩展连续批处理模拟器跟踪 GPU利用率(每步批次槽位填充比例) ・绘制静态和连续批处理随时间的利用率图,50 个请求的输出长度遵循帕累托分布(形=1.5,规模=20) ・连续批处理应维持 >80%利用率──
...
3. Thực hiện phiên bản chú ý truy vấn nhóm (GQA) của bộ nhớ cache KV khi `num_kv_heads < num_query_heads`Llama 3 70B sử dụng 64 đầu truy vấn nhưng chỉ có 8 đầu KV. Xét tiết kiệm bộ nhớ so với sự chú ý đa đầu đầy đủ (8 lần giảm kích thước cache KV).
   Trung文翻译:实现分组查询注意力(GQA) phiên bản của KV 缓存, trong đó `num_kv_heads < num_query_heads`❖ Llama 3 70B dùng 64 đầu truy vấn nhưng chỉ có 8 đầu KV ⋅ tính toán so với toàn bộ nhiều đầu chú ý trong bộ nhớ tiết kiệm ⋅ KV 缓存大小减少8倍) ⋅
...
4. Xây dựng một bộ nhớ cache tiền đề sử dụng việc xả LRU. Đặt tối đa_entries lên 500 và tạo ra 1.000 yêu cầu nơi 60% chia sẻ một trong 5 tiền đề phổ biến. đo tỷ lệ hit và so sánh với bộ nhớ cache không giới hạn. Với việc xả tốt, tỷ lệ hit nên ở trên 55%.
   Trung文翻译:构建使用 LRU 淘汰的前缓存──设置 max_entries 为 500,生成 1,000 个请求, trong đó 60% 共享 5 个常见前之一──测量命中率并与无限缓存比较──良好淘汰下命中率应保持在 55%以上──
...
5. Tăng cường mô phỏng giải mã đầu cơ để thực hiện đầu cơ dựa trên cây (tương tự EAGLE-2). Thay vì một chuỗi đơn lẻ của các mã dự thảo K, tạo ra một cây ứng cử viên (ví dụ, 2 nhánh ở mỗi cấp độ 3 = 8 ứng cử viên lá). So sánh tổng số mã thông báo được chấp nhận cho mỗi vòng xác minh so với đầu cơ tuyến tính.
   Trung文翻译:扩展投机解码模拟器实现树状投机(EAGLE-2风格) ・・・ không phải là một chuỗi đơn của mã thông báo K 个草稿, mà là tạo ra cây ứng cử viên(ví dụ 3 tầng mỗi tầng 2 分支 = 8 个叶选) ・・・ so sánh mỗi vòng kiểm tra nhận được mã thông báo 总数与线性投机。
