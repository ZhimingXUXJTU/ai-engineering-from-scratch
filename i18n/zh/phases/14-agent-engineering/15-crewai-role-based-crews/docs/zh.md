...
> 采用多代理框架的团队都会遇到相同的墙面――"自主合作"在演讲中听起来很棒――然后客户提出了一个错误,你需要确定性重新放――或者财务部门问一次,

纯粹的DAG回答所有,但失去探索形式一个脑风暴代理需要.

> 简单的答案是可以回答的,但会失去脑海的探索性结构.


> **【中文解读】**机组人员采用角色模板模型每个机组人员都有角色 (角色) 目标 (目标) 和背景故事 (背景故事) . 机组人员组成 协作完成任务. 机组人员的独特特点是将组织管理理论 (角色定义,团队协作) 映射到机组人员设计中,使非技术用户也能够定义机组人员.

> **{【拓展：CrewAI 是 2024-2025 年增长最快的 Agent 框架之一（GitHub 20k+ s...】}**员工AI是 2024-2025年增长最快的代理框架之一. 吉特哈布20万+星) ⋅其核心销售点是低代码代理合作 通过YAML 配置文件定义代理角色和任务流. 员工AI 支持两种模式:员工 (预定义角色团队) 和流动 (动态工作流) ⋅企业用户 (尤其是非技术团队) 特别青其直观角色定义方式──
团队的分离是诚实的. 团队为合作,基于角色,探索工作. 活动驱动,代码所有,可审计的生产流动. 同样的框架,两个形状,每表面选择.

> 机组人员的分离诚实面对了这一权力.机组人员使用协作式,基于角色的探索性工作.流动使用事件驱动,代码控制,可审核的生产环境.

>  **【前置】**必须先掌握:阶段14·12 (人类工作流程模式) CrewAI的流程就是这些模式的实现,Crew是"自主版"工作流;以及阶段14·14 (演员模型) Crew内部的代理 协作本质上是消息传递――还需要理解 Pydantic (结构化输出验证),因为任务的`output_pydantic`是核心协议.
...
> 机组人员将多 机组人员 协作 建模为角色团队 机组人员 事件驱动流程 流程 两种模式 四种原语:机组人员 角色+目标+背景故事 任务 任务 机组人员 团队容器 过程 执行策略

- **Agent.** `role + goal + backstory + tools + (optional) llm`后台故事是承载性的.它塑造音调,判断,当代理停止.工具是代理可以调用的功能 (下面更多).

>  **【类比】**工作人员的代理 定义像招聘JD(职位描述):角色是职位名 (("资深数据分析师"),目标是KPI (("查找数据中的关键趋势"),背景是企业文化洗脑 (("你是IBM 工作20年资深专家,重点严谨...") ").**关键洞察**后背故事 不是装饰,它真的会影响LLM的判断风格 把同一个代理写成"严谨学者"或"激进创业者",输出风格完全不同.
...
> 代理人之间不直接见面. 任务引用代理人. 工作人员排列任务序列. 过程决定谁执行下一个任务.
...
> ️ **【易错点】**查看CrewAI文档示例都使用了等级模式就跟随了.**后果**管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 管理员的职责变化: 变化: 变化: 变化: 变化: 变化**一行修复**默认使用序列 (以下简称: 序列) 只有任务顺序真的依赖前一步输出 (如客服分流)才升级到等级级.

> 在五步运行中,代币成本可能增加三倍.只有你需要路由时才值得付费.
...
> 文档 2026 年的生产建议:从流动开始.`Crew.kickoff()`调用嵌入式 流动 步骤中. 流动 提供审计追踪, 工作人员 提供探索能力.
...
> 结构化输出使用Pydantic──在任务上传入 `output_pydantic=MyModel`◎根据模特的校验, ◎应对不匹配则强制转换或重试.`expected_output`字符串使用──自由文本输出适合草稿;结构化输出才是下游流 可以消费的──
...
> 在船员上通过`memory=True`根据类型配置来启用. 通过您配置的嵌入式 提供商支持. 默认的OpenAI,可替换为本地模型. 记忆是CrewAI相比较轻量化的框架的优势之一.
...
>  **【困惑】**问: 工作人员说"生产环境从流动开始",但我看YouTube教程全是工作人员例,到底该信谁?**可重放、可审计、可监控**这些只有流量能给给.建议路径:用船员做原型验证想法 ((一两小时搞定),稳定后用流量重写为生产版本.
...
> 不依赖于LangChain──支持Python 3.10到3.13──使用`uv`包管理――AWS Bedrock 集成已有文件;供应商基准测试报告称,在QA 工作负载上有显著的加速,但方法论(数据集、硬件、评估指标) 未公开,因此将框架供应商的数据仅作为参考.
...
> **背景故事导致的提示膨胀。**每个代理2000字的背景故事,加上五个代理团队,在第一次调用之前就已经耗尽了下文预算.
- **Manager-LLM token tax.**在一个五任务组上,这是六个LLM电话而不是五个,而管理员电话带有完整的任务列表加上之前的输出. 切换到序列,除非路由取决于输出.

> **管理者 LLM 的 Token 税。**层级模式在每次专家调用前增加一次管理员LLM调用.五任务的团队变成了六次LLM调用而不是五次,而且管理员调用携带完整的任务列表加上之前的输出.
- **Brittle handoffs.**任务N的`expected_output`任务N+1读为`context`法律法师生产了四个. 下游代理的广告.`output_pydantic`在任务N上,所以任务N+1读取输入的对象,而不是自由文本.

> **脆弱的交接。**任务N 的`expected_output`是"一个纲要"――任务N+1将其作为`context`读取并尝试解析三个部分.`output_pydantic`修复任务 N,让任务 N+1 读取类型化对象而非自由文本.
- **Crew-as-prod.**无轮装机出货. 输出可变性很高;重播是不可能的; 在调用时不能分辨一个坏跑与一个好的. 装用流.

> **Crew 直接上生产。**没有包装成流程 就将自由形式 机组人员 发布到生产环境――输出变异性高;无法重放;值班人员无法对抗好坏运行――用流程 包装――
...
> `code/main.py`标准库实现了两种形态以及一个三种代理团队.
...
> 具体演示:研究员、作者、编辑组成团队,生成一篇关于"代理工程2026"的简报──研究员获取(模拟的) 来源──作者起草──编辑精炼──同一个团队通过流动 运行以展示确定性形态──
...
> `outputs/skill-crew-or-flow.md`为一个任务选择船员还是流动,并构建最小实现.
...
> **把背景故事当装饰。**它实际上塑造出口. 每个代理测试三个变体;差异是真实的.
- **Skipping `expected_output`.**没有每项任务的合同,下游任务都会接收到 LLM所产生的任何东西.

> **跳过 `expected_output`。**没有每个任务的契约,下游任务会接收LLM 生成的任何内容――
- **Memory always-on.**长期写每一个运行. 矢量DB增长. 检索变得杂. 范围写到事实持续的任务.

> **记忆始终开启。**长期记忆每次运行都写入. 量数据库不断增长. 检索变得杂.
- **Manager prompt drift.**如果路由变得奇怪,放下语音模式,然后读.

> **管理者提示漂移。**层级模式的管理者提示是隐式的.如果路由变得奇怪,
- **Tool side effects in Crews.**邮件,删除,支付属于流动步骤,从来没有一个船员工具.

> **Crew 中的工具副作用。**机组人员可能比预期更多调用工具──POST、DELETE、支付等操作应该放在流动步骤中,永远不要放在机组人员工具中──
