...
>  **【前置】**学习本节前请先掌握:阶段14·32(最小工作台) ――本节讲如何使用仓库本身(不是聊天历史) 作为代理的"长期记忆"――
>  **【类比】**聊天历史 = 短期记忆(睡觉就忘了), 记忆记忆 = 笔记本(写下来长期保留) ・ 完成长期任务时不能依赖于短期记忆上下文窗口会满,需要把关键信息写到 repo 里如`STATE.md`,我知道.`DECISIONS.md`),下次启动时从这些文件读起.
...
> 代理完成一次会议――聊天关闭――下一次会议开启并问从哪里开始――模型说"让我检查文件",读过的笔记,重新做已经完成的工作――或者更糟糕的是,它重新写了一个已经完成的文件,因为没有人告诉它文件已经完成――

工作台的修复是 repo 存储器:状态在 repo 中的 JSON 文件中存在,写在一个方案下,保持在原子上,在代码审查中保持差异.聊天是一个过渡的传输; repo 是记录系统.

> 工作台的修复是仓库记忆:状态存在在仓库中的JSON文件中,在下图中写入,原子化持久化,在代码审查中友好差异;聊天是瞬态流;仓库是记录的系统。


> **【中文解读】**仓库记忆与状态管理让代理维护对代码库的理解――两种记忆: 1) 结构性记忆 文件树、依赖关系、API 接口; 2) 语义性记忆 代码意图、设计决策、变更历史――状态管理确保代理在多轮交互中保持一致的代码库理解――
