...
>  **【前置】**开始讲"运行时反"让代理看到命令的真实输出而不是自己猜测.
>  **【类比】**反循环 = 闭眼做饭与 眼做饭.盲猜"煮5分钟应该熟了"= 不看输出;眼看"水还没开"= 看真实反再决定.
...
> 实际情况是没有运行任何测试. 代理想象出口,或者运行命令但从未读取结果,或者读取结果但默默截断失败行.

反运行器消除了这一差距.每个命令通过运行器.每个记录都包含命令,捕获的 stdout 和 stderr,出口代码,墙钟的持续时间,以及一个线条的代理注释. 代理在下一个转折上读取记录.验证门在任务结束时读取记录.

> 反运行器消除了这一差距――每个命令都通过运行器――每条记录带着命令、捕获的 stdout 和 stderr、退出码、挂钟时间和一行 代理备注―― 代理 在下一轮读取记录――验证门控在任务结束时读取记录――


> **【中文解读】**运行时反循环让代理在执行过程中持续获取反──三种反类型: 1) 工具反命令输出,编译结果,测试结果; 2) 用户反中途断断和纠正; 3) 系统反资源使用,误率,超时──反循环是代理自适应调整的基础──
...
> 运行时反循环 通过监控代理行为并检测到问题时即时干预,提高成功率.
