...
>  **【前置】**学本节前请先掌握:阶段14·33(规则) 、阶段14·36(范围) 、阶段14·37(反) 3 个前置都做完才能学验证门控。
>  **【类比】**检查 检查 独立检查 测试是否通过,范围是否守住,规则是否满足,差是否合理"......门控失败,任务不算完成.
...
> 验证门(验证门) 在代理工作流的关键节点设置检查点──每个门验证前一步的输出是否满足要求,不满足则触发修复流程──
...
> **【中文解读】**验证门在代理执行的关键节点插入自动检查――常见验证门:(1) 编译检查代码修改后必须通过编译;(2) 测试检查提交前必须通过相关测试;(3) 链接检查代码必须符合风格规范;(4) 安全检查必须引入已知漏洞――验证门防止错误累积――

工作台修复是一个单个验证门,它读取代理已经制作的文物并进行电话.门是确定性的.门是版本控制.门是有线到CI.代理不能钱.

> 工作台的修复是一个单一的验证门控,读取代理 已产生的工件并做出判断.门控是确定性的.门控在版本控制中.门控连接CI.
