...
>  **【前置】**学本节前请先掌握:阶段14·32(最低工作台) 、阶段14·34(回复记忆)。本节讲"启动脚本"让代理不需要每次冷启动重新探索回复──
>  **【类比】**项目 README的可执行版──新员工进入职务第一天不用摸索"测试怎么运行"",依赖装备哪些"",环境变量是什么"init脚本自动完成写到状态文件──代理 每次启动先运行 init,省下10-30%的代币预算──
...
> 打开一个会话―― 代理 猜测 Python 版本―― 猜测测试命令―― 列出仓库根目录五次来找到入口点―― 尝试导入一个未安装的包―― 问用户配置文件在哪里―― 当它真正进行编辑时,一万个代币已经花在本应的脚本设置工作上――

解决方案是一个初始化脚本,在代理做任何其他事情之前运行,`init_report.json`代理在启动时读取.

> 修复是一个初始化脚本,在代理做任何事情之前运行,并写入一个代理在启动时读取的.`init_report.json`,我知道.


> **【中文解读】**首先,在 Agent 会话开始时设置环境和上下文──包括:(1) 环境探测检查依赖、语言版本、工具链;(2) 项目分析扫描文件结构、识别框架和约定;(3) 记忆载从上次会话恢复上下文──好的初始脚本是 Agent 成功的前提──
...
> 始始脚本为代理会话设置起始条件――包括环境配置、工具装载、上下文初始化和安全策略设置――
