...
>  **【前置】**学本节前请先掌握:阶段14·32-40 全部(工作台 系列9节) ⋅本节是工作台的实战综合在真实小项目上运行,只运行快速对比与工作台指导对比――
>  **【类比】**预期: 预期: 工作位 版本在真实 repo 上仍然比快速出口强 30-50%──
...
> 玩具任务的演示没有说服任何人.工作台的理由是在一个真实感的仓库上执行一个真实感的任务,以减少失败,减少回滚和下一次会话可以使用包投入生产.

通过两条管道,这次课程将实现真正的回应,并通过两条管道执行相同的任务.

> 这一课提供了真实的仓库,并通过两个管道运行相同的任务.


> **【中文解读】**本节介绍了生产环境部署策略和运维最佳实践.
...
> **【中文解读】**将代理工作台应用于真码仓库.真码仓库带来的挑战: 1) 规模数千文件的代码库超出下文窗口; 2) 复杂性多语言、多框架、多依赖; 3) 约定特定项目编码风格和架构模式; 4) 测试不确定的测试套件和脆弱的CI──
