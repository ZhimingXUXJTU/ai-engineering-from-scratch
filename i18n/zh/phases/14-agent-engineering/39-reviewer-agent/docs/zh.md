...
>  **【前置】**学本节前请先掌握:阶段14·38(验证门) ――本节是门的LLM版 用第二个代理 审查第一个代理的产出――
>  **【类比】**编辑以读者视角挑刺 ("这段逻辑跳跃"",那个名词没解释") 让同一个代理兼任两者等于没有审查人类作家都不行,LLM更不行。
...
> 你让代理修复一个错误. 它编辑了四个文件,运行测试,报告完成.`passed: true`两天后,你发现修复只解决了半个错误.

接受是必要的,不够的. 审查者提出了接受不能提出的问题:这是否解决了正确的问题?

> 验收是必要的,但不充分的.评审者问验收是无法问的问题:这是否正确解决问题?是否在没有标记的情况下扩大范围?是否记录了应被质疑的假设?是否将工作台留在下一次会议上可以接手的状态?


> **【中文解读】**审查员 代理是专门用于审查其他 代理工作的独立代理. 它检查: 1) 代码质量可读性、可维护性、性能; 2) 正确性逻辑错误、边界条件、错误处理; 3) 安全性注入漏洞、敏感数据泄露──独立审查员提供第二双眼──
