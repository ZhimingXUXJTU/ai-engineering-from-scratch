...
>  **【前置】**学本节前请先掌握:阶段14·31-41 全部 11 节 工作台 课程。本节是阶段14的标题石;;顶点课),把所有工作台 表面打包成可复制目录。
>  **【类比】**现在的厨房套件.前11节学到了"刀板调料"分别怎么用,本节把它们放进一个盒子你拿到任何仓库.`cp -r`现在,我们可以使用,第二天, 代理人可以靠谱地工作.
...
> 一个存在于谷歌文档,聊天历史和三半记忆脚本中的工作台是一个每季度必须重建的工作台.修复是一个版本化包:一个包含表面,方案,脚本和一个命令安装器的仓库或目录.

你将结束这个课程`outputs/agent-workbench-pack/`发送在磁盘上`bin/install.sh`这将它放入任何目标回报.

> 你将在课结束时在磁盘上交付.`outputs/agent-workbench-pack/`和一个`bin/install.sh`它们可以放入任何目标仓库.


> **【中文解读】**代理工作台 顶点项目:将将前面 11 课的所有概念整合到一个完整的编码 代理 中――该 代理能: 1) 了解项目结构和约定; 2) 执行范围受限的修改; 3) 运行验证门检查; 4) 通过审查员 代理质检; 5) 跨会话保持状态――这是最小可行的生产代理的设计――
...
> 经纪人工作台 毕业项目综合运用14期 所有知识,在真实代码仓库中构建一个生产级经纪人.
