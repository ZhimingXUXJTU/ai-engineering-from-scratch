...
>  **【前置】**学生节前请先掌握:阶段14·32(最低工作台) 阶段14·33(规则作为限制)
>  **【类比】**经理不懂边界" (反正都是这个项目代码,我顺手把那个bug也修了), 经理让经理知道哪些文件可以改,哪些不能,越界就报警了.
...
> 代理会蔓延──任务是"修复登录错误"──不同的是登录路由,邮件辅助函数,数据库驱动,README和发布脚本──每次触及都有合理的理由──但它们与审查的变化不同──

范围爬行是代理工作中最少监控的故障模式,因为代理以诚信叙述每个步骤.修复不是更严格的提示.修复是磁盘上的合同,上面写着所承诺的内容,以及对比结果与承诺的检查.

> 范围蔓延是代理工作中最缺乏监控的失败模式,因为代理诚实地讲述每一步.修复不是更严格的提示.


> **【中文解读】**范围契约定义 代理可以做什么和不能做什么. 显然的范围契约包括: 1) 文件范围 代理可以修改哪些文件和目录; 2) 操作范围 可以执行哪些命令; 3) 根据范围 可以安装哪些包.
...
> 范围契约(范围合同) 定义代理的操作边界――通过明确指定代理可以和不可以做什么,防止越界操作――
