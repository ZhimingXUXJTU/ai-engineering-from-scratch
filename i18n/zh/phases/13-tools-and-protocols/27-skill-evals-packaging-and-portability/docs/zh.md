...
> **【中文解读】**一个技能 只有同时做到五点才算"完成":包结构通过 lint 通过正确的请求被触发路由 带来真实升级 在可量任务 始终待在授权边界 没有失效的其他宿主 诚实地降低. 本课是代理技能 小系列 ((第13 阶段 · 22-27) 的收官课:把以上验收标准落成六层可执行的评测和发布门禁.

> **【拓展：Skill 生态→软件工程成熟度】**技能常被视为"写得漂亮的标记",本课程的核心主张恰恰相反:技能是带概率性路径和执行层的小型软件包,需要与任何生产接口相同的关注点分离静态结构,路径行为,任务效果,脚本质量,安全边界,跨运行可移植性,一层都不能省略.

>  **【前置】**学本课前请先掌握:第13阶段 · 22阶段 经理技能的技能.md 契约与运行时边界) 第13阶段 · 24阶段 · 渐进式披露) 第13阶段 · 25阶段 ·触发与路由) 第13阶段 · 26阶段 ·权限、沙箱与信任)  本课是这四课的综合毕业设计,也是全迷你轨道的产出验证课程.
...
- 通过分离判断,定制计算,参考和输出合同,将专家的工作流程变成一种技能.
  中文翻译:通过分离判断性工作、确定性计算、参考资料和输出契约,把专家工作流提炼为技能──
- 测试包结构,触发路由,任务行为,脚本正确性,安全性和可移植性作为单独的层次.
  中文翻译:把包结构"",触发路由"",任务行为"",脚本正确性"",安全性和可移植性作为相互独立的层分别测试.
- 测量引发精确和回忆,使用正面,清晰的负面,
  中文翻译:用正例、明确负例和近失例度触发的精确率(精确性) 与召回率(回忆) ⋅
- 通过重复跑步进行表现的比较,
  中文翻译:跨多次重复运行,比较有技能与没有技能 两种条件下表现.
- 建立和执行跨运行时间能力矩阵和完整技能捆绑的释放门.
  中文翻译:为完整的技能 包构建并强制执行跨运行时的能力矩阵和发布门禁.

## 问题 问题引入

> **【中文解读】**演示中,一切正常,依赖于四个巧合:用户巧合使用描述中的原话、作者自己知道打开哪个参考、脚本得到的是干净输入、主机恰好了解每一个自定义字段──真实使用中,这四个巧合同时消失,而"注册看起来不错"也发现不了──本节列出了七种典型翻车方式,说明了技能为什么必须像生产软件一样分层验收──

技能在一个演示中工作.用户询问了描述中使用的句子,作者知道哪个引用要打开,脚本看到清洁输入,预期的主机识别了每个自定义字段.

> 一个技能在一次演示中可以跑通.用户问的恰好是描述使用过的短语,作者知道该打开哪个参考,脚本看到的是干净输入,预期的宿主知道每个自定义字段.

然后真正的使用开始.

> 然后真实使用开始了. 下面的这个组失败场景是:模型把它误触发到相邻任务;另一个说法有效请求没有被识别;正文只讲做什么却没说什么产品算完成;脚本在空格、重复执行、残留状态下崩;安装器只复制SKILL.md 丢失引用文件;另一个运行时忽略调用标志和工具授权;一次成功、三次等价格运行运行进入不同分支.
...
> 这些失败没有一个可以被"标记看起来不错"捕获.

## 概念的核心概念

> **【中文解读】**本节讲技能提取与打包的三条方法论: 1) 从真实工作流而不是"主题"发发发"诊断某种部署为什么不可用并产出事故报告"是可用的技能 范围, "Kubernetes技能"不是; 2) 判断性工作交给模型,确定性工作交给脚本; 3) 依赖顺序编写包先定产品契约和验证方式,最后才磨练正文措辞.

### 开始从一个真正的工作流程,而不是一个主题.

库伯内特斯不用用,它包含数百项任务,其工具,风险和输出不同.

> "创建一个库伯内特技能"不是一个可用的范围.

"诊断一个部署为什么没有达到可用,收集证据而不改变集群,并制作排名事件报告"是技能候选人.它有:

> "诊断某种部署为什么一直未到达 状态"",在不变集群的前提下收集证据"",并产生一个按优先级排序事件报告"才是合格的技能候选人. 它具有:触发边界"",稳定的取证步骤序列"",需要判断的决策点"",可编写的命令"",明确的产品",以及"只读诊断"的安全边界.
...
> 采用这十个问题进行提议访谈, 事件触发点, 类似的请求不应触发, 首先收集什么证据, 哪些决策依赖证据, 哪些步骤可编写, 哪些领域规则值得参考, 哪些行动需要批准或必须排除, 什么产品证明完成, 独立评审如何检查, 哪些步骤依赖特定运行时) 回答同时决定结构和评测集.
...
> 模型判断用于分类,排序优先级,综合与处理差异义;脚本或工具用于解析,计数,校验,转换,查询类型化API 和强制不变量.

设置一个模拟手动解析的80行技能,是脆弱的.试图做出主观的建筑决定的脚本是不透明的.

> 正文里塞 80 行"人肉模拟的解析逻辑"是脆弱的;让脚本做主观的结构决策则是难以测试的.

### 根据依赖顺序编写包

首先不要抛光散文,而是从内在的合同中建立起.

> 不要从打磨措辞开始――从可观察的契约向外往内构建――十步顺序:产物契约 → 验证方式 → 取证工具 → 决策图 → 参考资料 → 进口正文 →描述 → 运行时适配器 → 六层评测 → 安装后测试)――这个顺序让文字服务于可测试的系统,而不是在演示 跑通后再推出成功标准――
...
> 每层回答一个不同的问题:结构层问"包的形状对不对",路由层问"该触发时触发、不该触发时弃绝吗",行为层问"任务真的变得好吗",脚本层问"确定性部分是合格软件吗",安全层问"没有越权吗",可移植层问"换个宿主还诚实吗"――通过其中一层不能替代其他任何一层――

## 包装结构.

> **【中文解读】**静态 lint 负责验证一切不需要模型的事实:SKILL.md 存在且前面材料可解析、名称与目录名称一致、必需字段齐全且不超限、非核心字段都在运行时扩展白名单里、每个引用都能在包内解析、文件后和字节数符合发布策略、没有符号链接和特殊文件、正文不超字符预算、秘密模式、扫描空以及输出合同和失败行为 两个部分 非空.注意:先做物理树预检(拒绝符号链接根目录等) 再解析内容,否则会抹消检查所需的证据.
...
> 在解析中`SKILL.md`、评测数据、证据、主机固定或表现 之前,先做一次物理树预检:在任何内容读取之前,拒绝符号链接的根目录、符号链接的父目录或入口、缺失的必需普通文件以及特殊文件,然后再运行内容感知策略 lint──如果在预检前就把捆绑路径解决掉,会抹掉根符号链接检查所需的证据──
...
> 静态 lint 证明包的形状,不能证明模型会选择或遵循这个技能

## 层2:触发路由

> **【中文解读】**路由评测量是"该触发时触发、不应触发时弃权") 关键纪律:先建标签案例集,再反复改写,否则等于对训练集调参.
...
> 把案例分为开发集与验证集:在开发集上调解描述,使用验证集判断修改后的描述是不是泛化;如果发布决策足够重要,再留下最终保留集 (置集) 了.
...
> 对于目录式路径,还需要测量前一个命中率,弃权,弃权,弃权,弃权等技能之间的混.

### 路由评测必须运行在目标运行时

词汇模拟器是解释指标和捕捉明显的重叠的有用.它不能证明模型驱动的生产路由器如何行为.在声称运行时间质量之前,运行标记的集合通过实际主机,模型,目录序列化和政策配置.

> 词法模拟器适合解释标志和发现明显的重叠,但不证明模型驱动的生产路由器的真实行为. 在宣称"运行时质量"之前,必须把标志集运行过真实主机,模型,目录序列和策略配置.

>  **【类比】**这样驾驶科目一和实际路考的区别.词法模拟器是"科目一"在纸上判断请求和描述的关键词重度;真实主机路由是"路考"模型在完整下文,目录序列和策略配置下做真实选择.科目一满分不代表开车.

## 层3:指令和工艺品行为.

> **【中文解读】**触发正确只是进入门票,技能 必须让任务真的变好――方法学是 A/B 对照实验组:基线组:
...
> 保持模型,温度或采样策略,工具集,任务设置和预算全部不变,否则你无法将差异归因于技能.
...
> 项目校验检查结构,领域检查校验候选版本与证据路径,人或校准过的评审员评估"建议是否由证据推出"――

## 层4:脚本正确性.

> **【中文解读】**技能的脚本是普通软件,必须在模型运行之外进行独立测试.最小的用例集包括:正常/空/形输入,Unicode与空格与路径边界,重复执行,超时或依赖失败,上次运行的残留产品,输出大小上限,干跑行为,结构化退出与错误契约.
...
> **【中文解读】**安全评测问题是:包裹是否始终存在于它授予的权限内. 十类必测场景包括:用户请求的范围之外;参考输入中的恶意指示;逃离包装的资源路径;逃离允许根目录工作区符号链接;未声明的网络目的地依赖;依赖环境证书的命令;未经批准的破坏性行动;超大输出或死循环;技能之间的循环调控;可能重复副作用的恢复执行.
...
> **【中文解读】**可移植性不问"宿主支不支持技能"这个布尔值,而是逐项问"哪些行为支持"――本节给出三个事情: 1) 发布测试必须安装到干净目标位置后对"装备副本"进行验证只测源码树发现不需要安装器丢文件,丢执行位,压平引用,改名和残留旧文件; 2) 显示使用SHA-256 逐文件测试,检测漂移,但哈希不等于认证,表现的真实性必须依赖于外部可信的信念; 3) 矩阵每个必要能力记得为"原生支持/适配器档案文档化回退/不支持不失安装"默降级是消灭可移植性错误的.

### 设置目录作为一个单元,将目录作为一个整体安装.

释放测试应安装在清洁的目的地,然后与安装的副本进行验证.

> 一个发布测试应该先安装到干净的目标位置,然后对"装好副本"运行验证.
...
> 让我`assets/manifest.json`保存给表现出来的数据,并把它排除在自己的身上.`files`映射之外文件不能在自身内部携带"自身完整当前内容"的稳定哈希――校验其余的每个打包文件;表现的真实性必须通过外层可信信信信信信信信号签名发布或可信登记记录) 建立――信封只接受`manifestVersion: 1`和 `algorithm: "sha256"`已是规范的相对 POSIX 路径:`./SKILL.md`、反斜、绝对路径和父目录段都会被拒绝而不是归结.

哈希检测漂移.版本号码传达兼容性. 无论是不验证表格或在升级之前取代一个完整的diff和 eval运行.

> 哈希检测漂移,版本号传达兼容性,两者都不能认证表现,也不能替代升级前的完整差异和评测运行――

### 移植性是一种能力矩阵.

不要问主机是否"支持技能"作为一个布尔式.

> 不要问一个主机"支不支持技能"这样一个问题,要逐项问它支持哪些行为.
...
> 每条能力声明都应指向一个测试或现行官方契约.主持人行为会变化,所以要将适配器版本和测试日期写入兼容性报告.
...
> 据GitSkills 数据集论文报道,在2026年7月的一次爬取中,共有382,200个库存中,共3797,117个技能类文件,重量为1,877,981种不同字节内容;按字节级度量约为50.5%的匹配文件是完全相同的副本.

这些数字表明,技能文物存在于存储器规模,并且对数据集构建,搜索,来源和升级分析来说,重复是重要的. 它们并没有显示,一半的技能是好的或坏的,技能提高了任务的性能,任何呼叫领域是普遍的, 这篇论文是数据集研究,而不是有效性或安全性基准.

> 这些数字说明了技能 工件已经达到仓库级别的规模,重复副本将影响数据集的构建,搜索,追溯源和升级分析;但它们不能说明一半的技能是好坏,技能是否提高任务表现,哪个调用字段是通用的,哪种沙箱设计是安全的. 它是一个数据集研究,不是效果或安全基准.

通过生态系统计数来激励排序和来源.

> 通过生态规模数据来证明重复和追溯的必要性;通过自己的评价来支持质量声明.

## 连续运行与不确定性

> **【中文解读】**模型和路由行为自然有差异,所以每个行为用例都必须在生产采样策略下重复运行多次.观察通过率 k/n 只是起点:必须保留每次运行的原始痕迹70% 通过率可能是相同的一致失败,也可能是几个不相关的失败,修改完全不同.对比基线和实验组按任务单项比,不能只看汇率;平均变化好,但个别任务还要回报.
...
> 任务对比基线与实验组,不要只看汇总平均值;即使平均分数改善,也必须报告回归.

## 释放盖茨

> **【中文解读】**发布门禁就是给六层各设一个值:结构零错误、路由精度≥0.95/回忆≥0.90、近失误触发≤1、行为契约通过率≥0.90 且对基线无回归、脚本单测全过、安全用例100% 通过、可移植性要求无静降级、安装树与表现一致.
...
> 失败必须能够定位到层次和证据. 不要把路由,行为,安全压力成分数,

### 区分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分分

> **【中文解读】**只有证明"禁令机制运转正常",证明没有目标运行时真的选择了这个技能.`fixturePassed`(使用声明的确定性装置 走过全部门层)`localEvidenceReady`(四类捕获模式标签有非空源,并符合SHA-256与本地证据)`productionReady`(还需要一个来自包外的收件证书 绑定整个`evidenceRoot`终结的`passed`只是跟着`productionReady`证书的期望摘要必须经过外信 (CI秘密,签名发布记录或注册表决) 提供,否则只是另一个本地重算的哈希.
...
> 它还需要通过.`--trusted-attestation-sha256`提供证书 字节精确的SHA-256――这个期望摘要必须来自带外可信策略,CI秘密,签名发布记录或注册表决;将它存在于同一捆绑里,只将此检查退化为另一个本地可重复的哈希――评估器会拒绝缺失,在捆绑内,符号链接,形,不匹配或版本不支持的证书――

## 建立它,实现它.

> **【中文解读】** `code/main.py`实现了整个迷你轨道的发布带:物理树预检,`lint_package`静态检查、带完整原始痕迹的触发评测、分类指标、重复运行通过率、产品合约校验、证据检查、追溯源与三级判断(固定/本地完整性/生产)、显现 构建与校验、能力矩阵、以及保层的最终门禁――演示 打包毕业技能 跑完整评测:`checks_passed`与`fixture_passed`为了真实,而`local_evidence_ready`,我知道.`trust_anchor_valid`,我知道.`production_ready`,我知道.`passed`保持虚假,这是本节三边界演示.
...
> 报告只需要通过历史证据, 版本与包装版本和评测器 版本一起存档旧模型,旧宿主或旧技能.
...
> 修哪一层取决于失败出哪一层. 真正的问题是"安装器丢引用"或"沙箱暴露在家目录"时,往往.`SKILL.md`里堆更多文字没有用.

## 实际主机可移植检查点

> **【中文解读】**确定性固定 证明是门禁机制;这个检查点证明"一个真实主 finally founded something 了什么 了什么 允许什么 删除了什么"......六个步骤:先运行固定演示 并记下固定 通过≠主体结果;使用`npx skills add`装备完整包;使用三种提示分别探测显式调用、隐式选择与近失不触发;再使用"评测通过就发布"探测审批边界(预期:不发布);换第二个主机或如实标签未经验证/未支持;最后测升级和卸载(包括卸载不再被发现) ⋅所有后观察都记入证书表文档读档或看网页不构成可移植性证书.
...
> **【中文解读】**本课产出发 `skill-release-gate`一个完整的毕业包:SKILL.md、一个参考文件、只读评测脚本、宿主 fixture、标记好的触发案例集和产物契约。生产路径是:把所有 fixture 换成捕获值、重建保留的表、通过独立的发布基础设施获得认证 及其可信摘要,再运行评估器──只有六层门禁、本地证据完整性和外部信任全部通过,命令才成功退出本地重注、重算哈希的 fixture 仍然是"非生产".―
