...
> **【中文解读】**2026年的音频生成几乎都是基于代币的.EnCodec、SNAC、Mimi、DAC将连续波形转换为离散序列,让变压器可以预测──语义-声学代币 分离.

> **【拓展：音频 token 化】**就像文本有BPE代币器将文字变成代币,音频有EnCodec等编码器将声波变成代币――这使得音频可以像文本一样被大语言模型处理――Moshi、AudioLM等模型都依赖于这种代币化――
...
> 语言模型处理离散代币――音频是连续的――如果你想为语音/音乐构建LLM 风格的模型音乐Gen、莫希、芝麻CSM、VibeVoice、奥尔菲斯你首先需要一个**神经音频编解码器**编码器将频频分散成小词表标记,加上匹配的解码器重建波形.
...
> 2024-2026 年的洞察:**纯重建编解码器在从文本生成时给你模糊的语音。**编解码代币 上部LLM必须同时学习语言结构和声学结构在同一码本中,这无法扩展.将它们分开为语义码本0,声学码本1-N正是Moshi和芝士CSM成功的关键.
...
> 由于使用一个大型码本 (高质量需要数百万码),所有现代音频编解码器都使用**RVQ**编码器输出第一码本量化编码器输出第二码量化残差以此类推为每码本1024个码本,8个码本 = 有效词表1024^8 =10^24。

在推断时,解码器将每个框架中所选的所有代码总和起来,以重建.

> 推理时,解码器将每选中的所有码本向量求和重建波形.

### 2026年有关键的四个编程器

**EnCodec (Meta, 2022).**基线. 波形加码器-解码器,RVQ瓶. 24 kHz,32个代码库可能,默认4个代码库 @ 1.5 kbps. 使用 `1D conv + transformer + 1D conv`建筑,由音乐Gen使用.

> **EnCodec（Meta，2022）。**基线──波形上的编码器-解码器,RVQ 瓶──24 kHz,最多32个码本,默认4个码本 @ 1.5 kbps──使用 `1D conv + transformer + 1D conv`架构――音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐 音乐

**DAC (Descript, 2023).**随着L2标准化的代码簿,定期激活功能,损失改善. 任何开放代码的最高重建忠实度有时无法与原始语音区分,有12个代码簿. 44.1 kHz 频段.

> **DAC（Descript，2023）。**采用L2 归化码本、周期性激活函数和改进损失的RVQ──开源编码器中重建保证最高12个码本时有时与原始语音无法区分──44.1 kHz 全频带──

**SNAC (Hubert Siuzdak, 2024).**多尺度RVQ 粗代码书在较细的图像速度下运行.有效地以语音层次模型:粗的"sketch"在 ~ 12 Hz 加上50 Hz 细节.Orpheus-3B使用,因为层次结构很好地映射到基于LM的生成.

> **SNAC（Hubert Siuzdak，2024）。**多尺寸RVQ粗码本以细码本比较低的率运行.有效地分层建模音频:约12Hz的粗草图加50Hz的细节.
...
> **Mimi（Kyutai，2024）。**极低的游戏率,8个码本 @ 4.4 kbps的码本 0 **从 WavLM 蒸馏**训练来预测波LM的语音内容特征──码本 1-7 是声学残差──这种分离驱动了莫希 (第15课) 和芝麻CSM──
...
> 在12.5Hz的率下,一段10秒的语音只有125个编解码变压器可以轻松预测.
...
> 自归 LM 先预测语义符号(以文本为条件),再预测声学符号(以语义 + 说话人参考为条件) ・・・这种分解正是现代TTS 能够零样本克隆声音的原因:语义模型处理内容,声学模型处理音色。
...
> 传统编码器 (如Opus) 在每比特感知质量仍然胜出.**离散 token**产生的产量和**生成模型质量**为了赢得胜利,我可以用这些符号做什么.
...
> 秒速6 kbps 下`n_codebooks=8`△每码取值 0-1023(10 比特) △
...
> 语义码本 0 与波音LM对齐.你可以训练一个文本→语义 变体词表比直接到音频小得多.然后一个独立的声学→波形解码器以说话人参考为条件.
...
1. **Easy.**跑步`code/main.py`它实现了玩具的尺度量化器+残余量化器,并测量了重建错误,
   **简单。**运行`code/main.py` 实现了玩具标量+残差量化器,测量随着代码本增加的重建差距.
2. **Medium.**安装`encodec`通过使用一个长时间的语音片段,将1,4,8,32个代码簿进行比较.
   **中等。**装备`encodec`图表: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签: 标签
3. **Hard.**装载米米.编码一段片段.用随机整数取代代代码书0;解码.然后类似地取代代码书7.比较两种腐败代码书0腐败应该破坏理解性;代码书7腐败几乎不应该改变任何东西.
   **困难。**加载 Mimi──编码一段音频──随机整数换码本 0;解码──然后类似地换码本 7──比较两种损坏码本 0 损坏应破坏可理解性;码本 7 损坏几乎没有变化──
