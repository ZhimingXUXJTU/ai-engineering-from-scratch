...
> **【中文解读】**3D高 (高) 通过数百万的3D高球来表示场景.每个高都有位置,方向,缩放,不透明度和视角相关的颜色.

> **【拓展：3DGS vs NeRF】**3D高质量分化是NeRF的实时替代方案.NeRF 染色慢但质量高,3DGS 通过显而易见的3D高高表示实现实时染色,同时保持高质量.
...
> 对于每一个图像都是沿线上数百次 MLP 查询. 训练需要几个小时,训练需要几秒,权重无法编辑.
...
> 3D 高斯(Kerbl、Kopanas、Leimkühler、Drettakis,SIGGRAPH 2023) 取代了所有这些.场景是一组显而易见的3D 高斯.染色是100+fps的GPU光化.训练只需几分钟.编辑是直接的:平移部分高斯就移动了椅子. 到2026年,克罗诺斯集团已经批准高斯的GLTF扩展,使用OpenUSD 26.03 附高斯模式,Zillow 和 Apartments.com它们染色房地产,大多数新的3D重建研究论文都是核心3DGS思想的变化.

心理模型很简单,数学有足够的移动部分,大多数介绍都从化开始,然后跳过投影和球状和.这个课程首先构建了整个东西 2D版本,然后是3D扩展.

> 心智模型很简单,数学有足够的部分,大多数介绍从光化开始跳过投影和球函数――本课程构建整个东西首先是2D版本,然后是3D扩展――
...
> 旋转 + 缩放构建3x3 协方差矩阵:`Sigma = R S S^T R^T`△这是高斯在3D中的形状. 球函数让颜色随着观察方向变化. 高光,微妙的光泽,视角相关发光. 没有需要存储的每视角纹理.

一场场景通常有1-500万个高斯人.每个场景都存储约60个浮动 (3 + 4 + 3 + 1 + 48 + 混合).这为5百万个高斯人场景的240MB,远小于每个点纹理的等级点云,并且大小量小于NeRF的MLP重量,高分辨率重新呈现.

> 一个典型场景有100-500万高斯. 每个场景存储60个浮点数.
...
> 五步,全部GPU 友好――无需每像素MLP查询――单张RTX 3080 Ti 以 147 fps 染色 600 万──
...
- **Clone**现在的位置是高斯的,但它的尺度很小.
  翻译: 中文**克隆**高梯度高但缩小小时,在当前位置重建高梯度需要更多细节.
- **Split**高梯度时,一个大高梯度太平滑,不能适合该区域.
  翻译: 中文**分裂**大尺度高,高梯度高,分为两个较小的一个大尺度高,平滑不适合区域.
- **Prune**光率下降到值以下的高斯人,
  翻译: 中文**修剪**不透明度低于值高斯不贡献,修剪掉.

密度运行每次N代.一个场景通常从100k初始高斯人 (从SfM点种植) 增长到训练结束时的1-5M.

> 密集化每N次代运行一次――场景通常从约10万初始高斯 (从SfM 点播种) 增长到训练结束时的100-500万――
