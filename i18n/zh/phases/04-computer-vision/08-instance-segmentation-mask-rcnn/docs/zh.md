...
> **【中文解读】**在Faster R-CNN检测器上添加一个掩码分支就得到了实例分支.难点在于,RoIA align将不同规模的候选区对齐到固定尺寸的特征图,并保持空间精度.实例分支与语义分支的区别在于:实例分支可以区分与类别不同的人物 (如图中的每个人) ).

> **【拓展：实例分割的应用】**面具R-CNN 广泛用于自动驾驶 (区分不同车辆和行人) 机器人抓取 (识别单个物体轮) 视频编辑 (精确图) ⋅RoIAlign 技术后来也被用于视觉变换器的适配器中。
...
> 语义分给你每个类别一个掩护――实例分给你每个物体一个掩护――即使两个物体属于同一类别――数个体、跨跟踪和测量东西――墙壁中的每个块的边界框、显微镜图像中的每个细胞) 都需要实例分离――

面具R-CNN (He等,2017) 通过重新构建实例细分化为检测加面具来解决这一问题.设计非常清洁,在接下来的五年里几乎每个实例细分纸都是面具R-CNN变体,并且火视觉实现仍然是小型到中等数据集的生产默认.

> 通过将实例分区重新定义为检测加掩码来解决这个问题.设计如此干净,直到接下来的五年,几乎每一个实例分论文都是 Mask R-CNN的变体,火视觉实现仍然是中小型数据集的生产默认选择.

设计难题是采样:如何从一个没有与像素界限一致的角落的提案框中切割一个固定尺寸的特征区域?

> 难题的工程问题是:你如何从角点不同于像素边界对齐的候选框中剪切固定大小的特征区域?
...
1. **Backbone** ResNet-50或 ResNet-101 在 ImageNet 上训练. 在步骤 4, 8, 16, 32 产生功能地图的等级.
   中文翻译:在ImageNet上训练的ResNet-50或ResNet-101──生成步幅为4、8、16、32级特征图──
2. **FPN (Feature Pyramid Network)**上下 + 侧接,给每个级别C道含有语义丰富的特性.检测查询 FPN 级别匹配对象大小.
   中文翻译:自上而下加横向连接,为每个层面提供C 个通道的语义丰富特征――检测时查询与物体大小匹配的FPN层级――
3. **RPN (Region Proposal Network)**一个小的头,在每个头位置,预测"这里有什么对象吗?"和"我如何精炼盒子?"每张图片产生1000个建议.
   中文翻译:一个小的卷积头,在每个点位置预测"这里没有物体?"和"如何修改边界框?"──每张图像产生约1000个候选区──
4. **RoIAlign**从任何FPN级别的任何盒子中采用固定尺寸 (例如7×7) 功能补丁.
   中文翻译:从任意FPN级的任意框中采样固定大小(如7x7) 的特征块──双线性采样,无量化──
5. **Heads**两层盒头,精炼盒子,选择一个类,加上一个小的卷头,输出一个`28x28`每个提议都会有双面膜.
   中文翻译:两层框头用于修改边界框并选择类别,加上一个小卷积头为每个候选区输出`28x28`的二值掩码.
...
> 原始的快速R-CNN使用RoIPool,它将候选框分成网格,取每个格子中最大特征值,并将所有坐标四舍五入整数.这种坐标将特征图和输入图像坐标错位多达一个特征图像在224x224图像上影响很小,但当特征图的步幅为32时则是灾难性的.
...
> 现在每个关心定位精度检测器都使用它YOLOv7 段、RT-DETR、Mask2Former 无一例外――
...
> 在特征图中的每个位置放置K个不同大小和形状的框――为每个框预测一个目标分数和一个回归偏移量,将框转换为更适合的边界框――保留最高得分的约1000个框,在IoU 0.7 下应用NMS,将幸存者交给后头部――RPN用自己的小损失函数训练结构和第6课的YOLO损失相同,只有两个类型物体/非物体) ――

### 面具头

对于每一个提案 (RoIAlign之后) 面具头是一个小的FCN:四个3x3conv,一个2xdeconv,最后一个1x1conv产生`num_classes`输出频道`28x28`只有与预测类相应的频道保持,而其他频道则被忽视.

> 对于每个候选区 (RoIAlign 之后),掩码头是一个小的FCN:四个3x3卷积,一个2x反卷积,一个最终的1x1卷积,在`28x28`分辨率上生成`num_classes`个输出通道――只保留与预测类对应的通道;其余通道被忽略――这将掩盖预测与分类解――

通过将28x28面具调整到原始的像素尺寸,

> 将28x28 掩码的样本采用到候选区的原始图像大小,生成最终的二值掩码.
...
- `L_box_reg`在头盒上精细的L1.
  中文翻译:头部边界框修改的平滑 L1 损失──
- `L_mask`每像素的二进制交叉值在28x28面具输出.
  中文翻译:28x28 掩码输出上的逐个像素二元交叉损失

每个损失都有自己的默认重量;火视觉实现将它们作为构造论据暴露出来.

> 每个损失都有自己的默认权重;实现将它们暴露为构建函数参数.
...
> 每个数值都位于双线性采用位置.没有收入,没有量化,没有梯度丢失.
...
> 当 当`sampling_ratio=1`且`aligned=True`时,两者之间的差异在`1e-5`在内
...
> 掩码张量形状为`(N, 1, H, W)`△以0.5为值获得每个物体的二值掩码:
...
> 常见的微调方案:复用骨干网络、FPN 和 RPN;替换两个分类器头──
...
> `num_classes`必须包含背景类,因此使用4个物体类的数据集.`num_classes=5`,我知道.

### 步骤 6: 结不需要训练的东西

在小数据集中,冷脊椎和FPN.只有RPN对象性+回归,两个头学习.

> 在小型数据集中,只有RPN的目标性+回归和两个主题参与学习.
