...
> **【中文解读】**语义分分就是对每个像素进行分类. 通过配对下采样编码器和上采样解码器,并连接它们之间跳跃连接实现这一点.

> **【拓展：U-Net 的广泛应用】**网最初是为医学影像分区设计而设计的,但已成为许多领域的基础架构:稳定传播使用U-Net作为核心无声网络,自动驾驶的道路分区,卫星图像的土地覆盖分类,其编码器-解码器+跳跃连接模式已被广泛复制.
...
> 分类每张图像输出一个标签――检测每张图像输出几个框――分分每一个图像输出一个标签――对于大小为`H x W`的输入,输出是形状为`H x W`语义) 或`H x W x N_instances`张量――那就是每张图像数百万个预测,而不是一个.

> **【中文解读】**分类输出标签,检测输出几框,分输出每个像素的标签. 分的核心难题是:既要看到全局上下文 ((这是什么场景),又要保留局部像素细节 ((这个像素是路还是人行道) 标准CNN 通过空间缩小获取下文时会丢失细节,U-Net 通过跳跃连接同时解决了这两个需求.

细分结构是为什么它支持几乎所有密集预测视觉产品:医疗成像 (瘤面具),自动驾驶 (道路,车道,障碍),卫星 (建筑脚印,作物边界),文件分析 (布局区),机器人 (可抓住区域).这些任务都不能通过将盒子围绕物体解决;它们需要精确的模样.

> 分类结构是驱动几乎所有密集预测视觉产品的原因:医学成像 (瘤掩码) 无人驾驶 (道路,车道,障碍) 卫星 (建筑轮,作物边界) 文档解析 (布局区域) 机器人 (可抓取区域) 这些任务都不能通过物体周围的画框来解决;它们需要精确的轮.

建筑问题很简单,而难以解决:您需要网络同时看到图像的全球背景 (这是什么样的场景) 和本地像素细节 (正确的是哪个像素是道路与路面).标准的CNN空间压缩以获得背景,这会丢弃细节.U-Net是设计得到了这两者.

> 架构问题说起来简单,但解决起来不简单:你需要网络同时看到图像的全局下文.
...
- **Semantic**这里写着:"这像素是道路,那像素是汽车".
  中文翻译:语义分割说"这个像素是路,那个像素是车"――相邻的两个车辆被合并成一个区域――
- **Instance**无视背景的东西 ("东西" = 天空,道路,草).
  中文翻译:实例分割说"这个像素是第三辆车,那个像素是第五辆车"――忽略背景类别("东西" = 天空、道路、草地) ・・・
- **Panoptic**每个像素都得到了类标签,每一个实例都得到了独特的ID,
  中文翻译:全景分割统一了两者:每个像素获得类别标签,每个实例获得唯一的ID,背景和前景物体都被分割了。

接下来的课程 (面具R-CNN) 涵盖实例.

> 本课覆盖语义分割──下一课(面具R-CNN)覆盖实例分割──
