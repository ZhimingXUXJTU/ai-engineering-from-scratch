...
> **【中文解读】**通过变压器取代U-Net,使用直线流 (直流流) 取代噪声调节,就得到了SD3、FLUX等最新一代文本生成图像模型──整流让采样路径更直,减少推理步数──

> **【拓展：DiT 是 2024-2026 的趋势】**变频生成: 稳定变频 3、FLUX 都采用了变频架构.
...
> 第10课 用U-Net去噪音器构建了DDPM.该方案在2020-2023年占主导地位:U-Net +beta调度 +噪音预测损失.
...
> 每个2026年最先进的文本到图像模型都已经超越了它.稳定传播3、FLUX、SD4、Z-图像、Qwen-图像、Hunyuan-图像没有使用U-Net.它们使用扩散变压器 (DiT) ・SD3 和FLUX也使用整流取代了DDPM噪声调节,它将从噪声到数据的路径拉直,并通过一致性或蒸变量实现1-4步推推.

转变是重要的,因为它是基于扩散的图像生成成为可控制,快速准确的原因 (SD3/SD4解决文本染),以及生产快速的原因.理解DIT+修改流程是理解2026年生成图像堆.

> 这种转变很重要,因为它是基于扩散图像生成变得可控的提示精确的 (SD3/SD4 解决文本染色) 和生产快速的原因.
...
- **DiT**在隐藏的补丁上用一种类似VIT的变压器取代U-Net.通过适应性层标准 (AdaLN) 调 conditioning.
  翻译: 中文**DiT**通过自适应层归结 (AdaLN) 进行条件化.
- **MMDiT**两个流,对共享关注的文本和图像代币有不同的权重.
  翻译: 中文**MMDiT**两流分别有权重处理文本和图像代币,共享联合注意力.
- **FLUX**(黑森林实验室, 2024) 最初的N块像SD3一样双流,后来的块连接并共享重量 (单流) 以提高高深度的效率.
  翻译: 中文**FLUX**后续块拼接并共享权重 (单流) 提高深层的效率.
- **Z-Image**在6B参数上有效的单流DT,挑战"无论如何的规模".
  翻译: 中文**Z-Image**经济增长的发展,经济增长的发展,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长,经济增长等等等.

### 一段时间内调整的流量

未来的过程是杂的SDE,`x_t`学习的反向是第二个SDE,通过1000个小步骤解决.

> 声与微分方程 (SDE) 之间,`x_t`逐渐被损坏. 学习到的反向是第二个SDE,通过1000个小步求解.

调整流量定义了**straight-line**清洁数据与清洁噪音之间的插射:

> 整流定义干净数据和纯噪音之间的**直线**插值:
...
> 训练网络预测速度`v_theta(x_t, t) = epsilon - x_0`从干净数据到噪音直线路径的前向方向`dx_t/dt`采样时,反向积分的速度从噪音向数据而走.

SD3叫这么说**Rectified Flow Matching**,Z-图像和大多数2026模型都使用相同的目标.典型的推断:旧的DDPM模式中20-30个欧勒步骤 (确定性) 与50多个DDIM步骤.蒸/轮/快速/LCM变体将其降至1-4个步骤.

> SD3将被称为**整流流匹配**△FLUX、Z-Image 和大多数2026年模型使用相同的目标──典型推理:20-30步 尤勒(确定性)vs旧DDPM方案的50+步 DDIM──蒸/turbo/schnell/LCM 变体降至1-4步──

### 适应性调节

通过 时间步骤和课程/文本的DIT条件**adaptive layer norm**预测`scale`其他`shift`它们比U-Nets中的FiLM式调节更清洁,而且是每个现代的DIT中默认的.

> 通过**自适应层归一化**对于时间步骤和类别/文本进行条件化:从条件向量预测`scale`和 `shift`对于每一个现代的DIT,这是一个默认方案.
...
> 化技术+U-Net 可行──DiT+整流流**更好、更快、扩展更干净**△这种转变类似于NLP从RNN到变压器的转型:两种架构都解决了相同的问题,但变压器更具可扩展性并最终主导.
