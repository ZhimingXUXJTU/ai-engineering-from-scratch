...
> **【中文解读】**专家分布在节点间――跨节点通讯让GPU一半时间在等待――双管是双向流水线,将向前/反向计算与向前通讯重叠,减少气泡、提升吞吐――

> **【拓展：DualPipe→大规模训练】**双管是深度搜索V3高效训练的关键:它将使MoE的通信开销与前向/反向计算重叠,使2048卡的训练效率接近线性扩展.

>  **【前置】**学本节前请先掌握:阶段10·05(扩展分布式);MOE(专家混合) 概念;流水线并行(管道平行) 基础;通讯模式――本节是阶段10·20(深度寻找-V3通行) 的并行策略详解――
...
- 两管前后面的四个组件,以及为什么每个组件都有自己的窗口.
  解释双管前向反向块的四个组件及其各自的重叠窗口原因
- 解释了管道泡问题,以及"无泡"在实践中与营销中意味着什么.
  解释大规模流水线气泡问题以及实践中"无气泡"与营销说法之间的区别
- 通过手动追踪双管的时间表,为8个PP级和16个微批量,并确认前向和反向流填补彼此的空.
  手动追踪 8 个PP 阶段和 16 个微批次的双管调度,确认正向和反向流填补彼此空槽位
- 声明DualPipeV (Sea AI Lab, 2025) 所做的交易:在专家平行性不活跃时,以稍大泡的成本降低2倍参数复制.
  说明双管V(海人工智能实验室, 2025) 的权衡:取消 2x 参数复制,代价是专家平行不活跃时气泡稍大
...
> 在2KH800GPU上训练671BMOE模型会遇到三个复合瓶:

1. **内存压力。**每个GPU都拥有一个模型的片段.
   中文翻译:每张GPU 持有模型的一个片段──8k序列长度、61层、128头的激活显存量是巨大的──
2. **流水线气泡。**传统流水线并行(GPipe、1F1B) 让GPU在等待其阶段的输入或梯度时空──8个阶段,即使使用1F1B调度,GPU的约12%的时间可能是气泡──
   中文翻译:传统流水线并行让GPU在等待时空──8个阶段时,GPU的约12%的时间是气泡──
3. **跨节点全互联。**带专家并行的MOE将专家分散到各节点. 每次前向传播触发一次全互联网将分发代币给专家,另一次将结果合并.
   中文翻译:MoE 专家并行每次前向传播触发两次全互联通信,在2kGPU时计算通信比率接近 1:1──
...
> **【中文解读】**双管是DeepSeek-V3提出的双向流水线并行算法,通过在前向和反向传播之间重叠计算来最大化GPU利用率.

> **【拓展：DualPipe 与 DeepSeek-V3 的效率】**通过使用双管+MOE 专家并行+FSDP──双管将流水线气泡率降至5%左右 (传统方法约20%),是DeepSeek-V3以560万美元左右的训练的关键技术之一.
...
> **【拓展：流水线并行的工程细节】**双管在同一方向上交换前向和反向传播,气泡率将从约20%降至约5%――这对大规模训练成本产生巨大影响.
...
> 本课产出发 `outputs/skill-dualpipe-planner.md`△给定训练集群规范(GPU 数量、拓、互连、模型形状),它推流水线并行策略、调度算法和目标规模下的预期气泡占比──
...
1. 跑步`code/main.py`现在`(P=8, micro_batches=16, schedule=dualpipe)`其他`(P=8, micro_batches=16, schedule=1f1b)`计算GPU使用率差异,并以每百万训练代币的GPU-小时计算.
   中文翻译:在`(P=8, micro_batches=16, schedule=dualpipe)`和 `(P=8, micro_batches=16, schedule=1f1b)`上运行 `code/main.py`△计算GPU利用率差异并表示每百万训练代币回收的GPU小时数量.

2. 绘制时间表`(P=4, micro_batches=8, schedule=dualpipe)`标记每一个时间区块,用微批次的ID和方向. 确定泡缺失的第一个时间区块.
   中文翻译:手动画出 `(P=4, micro_batches=8, schedule=dualpipe)`调度表――标记每一个时间槽的微批次ID和方向――找到气泡消失的第一时间槽――

3. 阅读深度搜索-V3技术报告的5个图 (arXiv:2412.19437). 确定双管前部件内部的全向发送的重叠窗口.解释计算时间表如何隐藏它.
   中文翻译:阅读DeepSeek-V3 技术报告图 5――识别双管 前向块内所有调度的重叠窗口――解释计算调度如何隐藏它――

4. 计算双管的2倍参数上费用,用于70B密集型型号,P=8管道阶段和671BMoE模型,P=16管道阶段. 显示为什么MoE外费用比较小 (大多数参数是专家,分为大型EP组).
   中文翻译:计算双管对70B密集模型 ((P=8流水线阶段) 和671B MoE模型 ((P=16阶段) 的2倍参数开销――展示为什么MoE情况的开销比例更小――大多数参数是专家,跨大型EP组分片)

5. 根据该文件的3.4节,确定双管没有的两个特定属性.
   中文翻译:比较双管与金机 (双管与金机) 详见双管 添加的金机 没有两个特定属性,使用论文第 3.4 节作为参考.
