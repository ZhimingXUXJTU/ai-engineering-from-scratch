...
> **【中文解读】**1,24亿模型在单个GPU上训练――但70亿参数模型没有进入显现存储,单机训练数据需要数周――分布式训练是规模化的唯一途径:数据并行、张量并行、流水线并行――

> **【拓展：DeepSeek-V3的2048卡训练】**采用2048张H800GPU训练,采用双管流线并行 +MOE专家并行.理解分布式训练是理解前沿大模型如何被训练出来的基础.

>  **【前置】**学本节前请先掌握:阶段10·04(预训练迷你GPT) 单 GPU 训练基础;PyTorch DDP 概念;多 GPU 通信(NCCL、AllReduce) ⋅本节是阶段10·19(双管) 和阶段10·20(深度搜索-V3通行) 的前置──
...
>  **【类比】**分布式训练 = 多人合作搬大箱子──**数据并行**(DP) = 4 个人搬到一个相同的小箱子,结果全部减少平均)**张量并行**,我很高兴能看到你.**流水线并行**个人流水线,第1 人搬一层传给第二 人**FSDP**制作场景一般用3D平行性组合)

> ️ **【易错点】**分布式训练的3个坑:**batch size 设错**单卡 bs=8,4 卡应该 bs=32(4×8) 而不是 bs=8;使用有效批量大小计算 lr。(2) **AllReduce 瓶颈**卡间通信比GPU计算慢10倍,bs太小会让GPU等通信;bs至少32+才划算――(3) **没设 seed**每次的初始化不同,结果不可复制;`torch.manual_seed(42) + torch.cuda.manual_seed_all(42)`,我知道.
...
- 使用 PyTorch DDP 实现数据平行训练,并通过多个GPU进行梯度同步
  使用PyTorch DDP 实现多个GPU数据并行训练和梯度同步
- 计算给定模型大小的内存预算 (重量+优化状态+梯度+激活) 确定最小硬件
  计算给定模型大小的显存预算(权重 + 优化器状态 + 梯度 + 激活值),确定最低硬件需求
- 配置FSDP或DeepSpeed ZeRO阶段,将模型状态分成GPU和超越单GPU内存的适应模型
  配置FSDP或DeepSpeed ZeRO 阶段分成模型状态,使超出单卡显现模型的模型可以训练

> **【中文解读】**本课聚焦 LLM 训练的核心瓶:显存──7B 模型在FP16 下仅重量就需要14GB,加上亚当 优化器状态(28GB) 和梯度(14GB),总数56GB还没有算激活值──你将学习三种并行策略──数据并行、张量并行、流水线并行) 以及FSDP/ZeRO 分片技术来突破单卡显存限制──

## 问题 问题引入

在FP16中,一个7B参数模型只需要14GB的权重.亚当优化器存储每个参数的两个额外副本 (第一和第二时刻估计).这又是28GB.后延的基梯子增加了14GB.在单次激活之前,您在56GB上存储.

> 7B 参数模型在FP16下仅权重需要14GB──亚当 优化器为每个参数额外存储两副本──一阶段和二阶段估计),还需要28GB──反向传播的梯度再加上14GB──在存储任何激活值之前,你已经达到56GB──
...
> 80GB 中已消耗56GB──剩下的24GB 用于激活值前向传播中计算的中间值,必须为反向传播保留──对于2048代币序列和4096维模型,单层激活值约为64MB──32层需要每样本2GB──批量大小8需要16GB──你有24GB──批量大小12将爆发.──

现在试试70B参数. 单独的重量:FP16中的140GB. 不适合一个GPU. 仅仅需要保持重量,至少需要2个A100 (2 x 80GB = 160GB). 添加优化状态和梯度,你需要更多的:3+GPU最小,而实际上8-16取决于碎片策略.

> 现在试试70B参数――仅权重:FP16 下140GB──放不进一张GPU──至少需要2张A100(2 x 80GB =160GB)才能放下权重──加上优化器状态和梯度,需要更多:至少3+张GPU,实际需要8-16张,取决于分片策略──
...
> 拉马3405B 在 16,384张NVIDIA H100 GPU上训练,计算成本估计约为10亿美元.

本课程涵盖了使大规模训练成为可能的四种策略:数据平行,子平行,管道平行和完全碎片化的数据平行.

> 本课程覆盖大规模训练成为可能的四种策略:数据并行,张量并行,流水线并行和完全分片数据并行.你将使用纯Python模拟每一种,在接触分布式训练框架之前理解其机制.
...
> "亚当状态"列是致命的.亚当为每个参数存储一个运行平均值 (m) 和一个运行方差 (v),都是FP32.

一个H100拥有80GB.Llama 3 405B需要至少61个H100来控制权重,优化器和梯度.加上激活,数量会进一步增加.Meta使用了16384个GPU,不是因为他们想要,因为他们必须.

> 单张H100 有80GB──Llama 3 405B 至少需要61张H100来放置权重、优化器和梯度──加上激活值数字更大──Meta 使用16384张GPU 不是因为他们想而是因为他们必须──

> **【中文解读】**显存预算是LLM训练的第一关卡. 以Llama 3 70B为例:FP16权重140GB + Adam 优化器状态560GB +梯度140GB = 840GB(不含激活值) 单张H100 只有80GB,至少需要11张GPU才能放下这些状态.

> **【拓展：Llama 3 的 16384 GPU 训练】**拉马3405B在 16,384张H100 GPU上训练,使用3D并行(数据并行 +张量并行 +流水线并行) ⋅训练总成本估计约10亿美元──作为比较,DeepSeek-V3使用MoE架构(每次只激活部分参数) 和双管流水线,训练了大约560万美元的同等能力模型──

### 数据平行

简单的分布式策略.将整个模型复制成NGPU.将每个训练批量分为N等部分.每个GPU在数据片段上运行前后传递.后传递后,平均所有GPU的梯度.每个GPU更新其重量副本,保持所有副本的同步.

> 最简单的分布式策略――将整个模型复制到N 张GPU――将每个训练批次分为N 等部分――每个GPU对其数据片分运行前向和反向传播――反向传播后,在所有GPU间平均梯度――每个GPU使用相同的平均梯度更新其权重副本,保持所有副本同步――

**The good:**线性吞吐量扩展.NGPU每步处理N倍的数据.通信仅限于梯度平均值,它与计算重叠.

> **优点：**吞吐量线性扩展――N 张GPU 每步处理N 倍的数据――通信仅限于梯度平均,可与计算重叠――

**The bad:**每个GPU都包含模型的完整副本,优化状态和梯度.对于70B模型,每个GPU需要840GB.数据平行化没有任何作用,以减少每GPU内存.它只减少了训练时间.

> **缺点：**每张GPU 拥有模型,优化器状态和梯度的完整副本.对于70B模型,每张GPU 需要840GB.数据并行不减少每个GPU 显存量.

**The math:**有效批量大小 = per_gpu_batch_size x N.对于每GPU批量为16个GPU的N=64GPU,有效批量为1,024. Llama 3使用了每步的有效批量大小为1600万代币.

> **数学：**有效批量大小 = 每GPU批量 x N。对于 N=64张GPU,每GPU批量16个,有效批量为 1,024个.
...
> 将单层分成多张GPU──一次矩阵乘法被分配到多张GPU,每张计算部分的结果──

考虑一个重量矩阵 (8192,8192) 在一个输送前进层.通过4个方向的子平行性,每个GPU都保持一个 (8192,2048) 片段.每个GPU乘以其片段输入,产生部分结果.部分结果被结合 (通过全部减小或全部收集) 来产生全部输出.

> 考虑前层中形状为 (8192, 8192) 的权重矩阵――使用4 路张量并行,每张GPU 持有 (8192, 2048) 的分片――每张GPU 将输入乘以其分片,产生部分结果――部分结果通过全归约或全收集组合为完整输出――

**The good:**减少模型重量的每GPU内存. 70B模型分为8个GPU,意味着每个GPU都具有8.75B参数的重量.

> **优点：**减少模型权重的每一个GPU显存量――70B模型分为8张GPU,这意味着每个GPU拥有约8.75B参数权重――
...
> **缺点：**每层后需要快速GPU间通信――每次矩阵乘法后的全归约增加延迟――这在NVLink(同节点GPU间900GB/s) 上效果良好,但在InfiniBand(400 Gb/s,约50GB/s) 连接的跨节点上效果差异――张量并行几乎总是仅限于单节点内(8张GPU) ――

**Real usage:**梅加特龙-LM是子平行论的先驱.Llama 3 405B使用每个节点内的8个方向子平行论.

> **实际使用：**梅加特龙-LM 首先提出张量并行――Llama 3 405B 在每个节点内使用8路张量并行――

> **【中文解读】**张量并行 (压平行主义) 将单层矩阵乘法分成多张GPU 上――例如一个 (8192,8192) 的权重矩阵在4路并行下,每 GPU 只需要存储 (8192,2048) 的分片――但缺点是每个层都需要全归约 (全减) 通信,因此几乎只限于 NVLink 连接单节点内,8张GPU)――Llama 3 405B 就使用8路张量并行――

> **【拓展：流水线并行的气泡问题】**流水线并行将模型按层次分为不同的GPU (如GPU1 运行 1-8层,GPU2 运行 9-16层) .但会产生"气泡"GPU空等.

### 管道平行

按层次分类模型.GPU 1运行层次1-8.GPU 2运行层次9-16.GPU 3运行层次17-24.GPU 4运行层次25-32.数据通过管道流动:GPU 1计算其层次,并发送激活到GPU 2,该层次计算并发送到GPU 3,等.

> 按层拆分模型──GPU 1 跑 1-8层──GPU 2 跑 9-16层──GPU 3 跑 17-24层──GPU 4 跑 25-32层──数据流流水线:GPU 1 计算其层并将激活值发送给 GPU 2,GPU 2 计算其层并发送给 GPU 3,以此类推──

**The good:** GPU 之间的通信最小,只是在层边界的激活,这些激活与梯度或重量相比较小.

> **优点：** GPU 间通信最小只有层边界的激活值,与梯度或权重相比很小.

**The bad:**管道泡.当GPU4计算了微批次1的前进传递时,GPU1,2和3是空置的 (它们已经转发了部分).在后进传递时,模式反转.在天真的管道线下,GPU的使用率为N管道阶段的1/N.

> **缺点：**流水线气泡. 当GPU 4 在计算微批次1的前向传播时,GPU 1、2、3 空(它们已经完成了自己的部分) ⋅反向传播时模式相反──朴素流水线下,N个阶段的GPU利用率仅为1/N。
...
> **GPipe 和 PipeDream**通过将批次分为微批次来解决气泡问题.GPU 1 一完成微批次 1 的前向传播就开始微批次 2.这在流水线阶段间重叠计算.M 个微批次和N 个阶段,气泡比例降至 (N-1) /M.

### FSDP:完全分碎的数据并行

FSDP 结合数据平行化的扩展性和碎片化的内存效率.而不是每个GPU都持有模型的完整副本,每个GPU只持有1/N的参数,梯度和优化状态.

> FSDP 结合了数据并行可扩展性和分片的显存效率. 每个GPU 不拥有完整的模型副本,而是仅拥有1/N的参数,梯度和优化器状态.

在一个层向前传递之前,FSDP运行一个**all-gather**后续传输后,每个GPU都会丢弃非本地参数.在倒退期间,全集再次运行,以重建梯度计算参数.**reduce-scatter**它们只存储1 N的梯度.

> 在一层前向传播之前,FSDP运行了全集操作从所有GPU收集完整参数到每个GPU的显存中. 在前向传播后,每个GPU丢弃非本地参数.
...
> 没有FSDP,70B模型无法放入单张80GB的GPU.使用FSDP在8张GPU上,每张GPU使用105GB等等,还是放不下.你需要至少16张GPU才能降至每卡80GB以下,或者将FSDP与激活检查点结合起来.反向传播时重新计算激活值而不是存储它们)

由于每个层前的全部收集,通信成本比尼拉数据平行性更高.

> 通信成本高于普通数据并行,因为每个层需要全部收集.

> **【中文解读】**FSDP (完全分片数据并行) 是数据并行和分片的结合. 每个GPU只存储1/N的参数,梯度和优化器状态.

> **【拓展：DeepSpeed ZeRO 的三个阶段】**泽罗阶段1 分片优化器状态(省4x 显存),阶段2 加上梯度分片(省8x),阶段3 加上参数分片(省N倍,N 为GPU 数) ・FSDP 本质上是泽罗阶段3的PyTorch原生实现──Llama 3 405B的16,384GPU训练使用FSDP+张量并行+流水线并行3D组合──
...
> 根据其概念,它与FSDP相似,但由微软独立开发. 它定义了三个阶段,每个阶段更为激进:
...
> **【中文解读】**混合精度训练是现代 LLM 训练的标准做法:前向传播用BF16(16位),优化器维护FP32 主权重(32位),损失缩放防止梯度下溢――BF16与FP32有相同指数范围(8位),但精度降低(7位尾数对比23位),几乎不需要损失缩放――业界已从FP16 全面显度转向BF16──混合精度为7B模型节省约28GB ⋅

> **【拓展：3D 并行与 MoE 的经济性】**拉马3405B 使用3D并行:节点间数据并行 + 节点内 8 路张量并行 + 跨节点流水线并行。DeepSeek-V3则使用MoE(混合专家) 架构降低成本每次前向传播仅激活约37B 参数(总参数671B),训练成本仅约560万美元,是拉马3的1/18──
...
> 按数量分成模拟的GPU 上面. 每个GPU 在其分片上计算前向传播.
...
> 实际上,这使用NVIDIA GPU上的NCCL库,实现了环形全归约:每张GPU将1/N的梯度发送给邻居,从另一个邻居接收1/N,N-1 步骤后每个GPU都具有完整的平均值.

### 步骤 2:模拟度平行

每个GPU计算一个部分矩阵乘法. 结合结果.

> 将权重矩阵分为多张GPU──每张GPU 计算部分矩阵乘法──组合结果──
...
> 误差应该恰好为零(或机器的比伦) ⋅张量并行在数学上是精确的它产生与在一个GPU上计算完整矩阵乘法相同的结果──分开沿输出维度进行,每个GPU 产生不同的列块,拼接重建完整结果──

在一个变压器 FFN 中,第一个线性 (扩展) 使用列式并行,第二个线性 (合同) 使用列式并行. 这避免了两层之间的全部减少.

> 对于列并行线性层 (分行输出维度),你拼接.对于行并行 (分行输入维度),你求和. 在变压器 FFN 中,第一个线性 (扩展) 使用列并行,第二线性 (收缩) 使用行并行.

### 步骤3:模拟管道平行

显示泡问题,早期阶段停留在空中,而后期阶段计算.

> 将模型层分成虚拟GPU上部. 展示早期阶段空后期计算时气泡问题.
...
> 4个阶段和1个微批次,气泡比例为75%四分之三的GPU 任何时候都空──16个微批次时降至约19%.
...
> 这个计算器回答每一个ML工程师的问题:"我需要多少张GPU?"输入模型大小看是否放下.
...
> 对大多数人来说最大的意外:混合精度不会减半显存量――优化器状态 (Adam's m 和 v) 不管精度如何保持FP32――对于7B模型,FP32 训练使用112GB――混合精度使用84GB――这是25%的减少,而不是50%――优化器占主导地位――
