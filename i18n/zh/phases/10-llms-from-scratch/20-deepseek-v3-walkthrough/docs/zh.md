...
> **【中文解读】**查-V3(2024年12月,671B 总参数,37B 激活) 转动了所有六个架构旋并新增了四个:MLA(多头潜在注意力) 无辅助损失负载平衡、MTP(多代币预测) 双管训练──671B/37B 的比例意味着每次推测只激活5.5%的参数──

> **【拓展：DeepSeek架构→开源大模型】**深度搜索V3是2024-2025年最重要的开源大模型架构之一.MLA将KV缓存压缩到原来的1/10/10,MoE让671B模型只耗耗37B的推理成本.

>  **【前置】**学本节前请先掌握:阶段10·14(开放模型架构) 6个架构旋概览;阶段10·15-19 全部(EGLE、Diff Attention、NSA、MTP、DualPipe) DeepSeek-V3的每个创新──本节把它们组装在一起,是阶段10的后半段的综合──
>  **【类比】**把2024年所有前沿优化全堆上:MLA(省 KV缓存) 莫E(省推理算力) 、MTP(送投机解码) 、双管 (DualPipe) 省训练通信) 、NSA(省长上下文算力) ⋅671B 总参数但每次只激活37B(5.5%),相当于"功能丰富的瑞士军刀但单次只使用一把刀"──
...
- 阅读DeepSeek-V3配置的顶部到底,并用六个GPT-2按和四个DeepSeek特定的添加来解释每个字段.
  从头到尾阅读DeepSeek-V3配置,使用GPT-2的六个旋转加上四个DeepSeek特有的创新解释每个段
...
- 说明DeepSeek特定的四项创新 (MLA,MTP,辅助无损路由,DualPipe) 以及每个目标的建筑/培训堆的一部分名称.
  描述四项深度搜索特有创新 (MLA,MTP,无助损失路由,DualPipe) 和其针对的架构/训练环节
...
> 深V3是第一个与拉马家族有实质性差异的前沿开源模型.拉马3405B是"GPT-2调六旋".

学习的回报:DeepSeek-V3的开放权重版本改变了"边界能力"在开放模型中意味着什么. 架构是许多2026年培训运行所复制的蓝图.理解它是任何涉及边界LLM培训或推断的角色的桌子.

> 学习它的收益:DeepSeek-V3的开放权重发布改变了开源模型"前沿能力"的含义. 它的架构是许多2026年训练运行正在复制的蓝图.

## 概念的核心概念

> **【中文解读】**密搜索V3是2024年最具影响力的开源模型之一:671B 总参数(37B 激活参数) 的MoE架构,使用MLA(多头潜在注意力) 压缩KV缓存,使用双管 优化流水线并行,以约560万美元的训练成本达到GPT-4级性能──

> **【拓展：DeepSeek-V3 的经济性突破】**果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行成本: 果电脑系统的运行量: 果电脑系统的运行量: 果电脑系统的运行量: 果电脑系统的运行量: 果电脑系统的运行量: 果电脑系统的运行量: 果电脑系统的运行量: 果电脑系统的运行量: 果电脑系统的运行量: 果电脑电脑系统的运行量: 果电脑系统的运行量:
...
> **【拓展：DeepSeek-V3 的 MoE 架构细节】**探V3有256个路由专家,每次激活8个,加上1个共享专家.
...
> 本课产出发 `outputs/skill-deepseek-v3-reader.md`△给定DeepSeek 系列模型 ((V3、R1或任何未来变体),它产生了各组件的架构解读,命名配置的每个字段,按组件推导参数数,并识别模型使用了四个DeepSeek 特定创新中的哪些──

## 练习题

1. 跑步`code/main.py`计算器的总参数估计与已发表的671B相比较,并确定 delta来自哪里.
   中文翻译:运行 `code/main.py`△将计算器的总参数估计与已发表的671B比较,找出差异来源――论文第二节有完整分项――

2. 修改配置,以使用MLA级别 256而不是512. 在128k文本中计算出所产生的KV缓存大小.它购买了多少个百分比的减小,每头表达性成本是多少?
   中文翻译:修改配置使用MLA 排名 256而不是512──计算 128K 上下文下 KV 缓存大小──省了多少百分比,代价是什么?

3. 根据 DeepSeek-V3 的推测,它可以使用一个假设的 (512 专家, 8 专家) 变异.
   中文翻译:比较DeepSeek-V3的(256 专家,顶-8)路由与假设的(512 专家,顶-8)变体──总参数增长;激活参数不变──额外专家容量理论上买到了什么,推理时代价是什么?

4. 阅读深度搜索-V3技术报告 (arXiv:2412.19437) 的2.1节. 解释为什么K和V解压矩阵可以"吸收"后续的子中,以推断时间效率.
   中文翻译:阅读DeepSeek-V3 技术报告第2.1节关于MLA的内容.
