...
> **【中文解读】**简单的GPT-2小有1.24亿参数:12层变压器,12个注意力头,768个维嵌入.

> **【拓展：大模型三阶段】**大模型训练三阶段:(1) 预训练(海量无标注数据,学习语言表示)→ (2) SFT(指令微调,学会跟随指令)→ (3) RLHF/DPO(对齐人类偏好)。本课是第一阶段,GPT-2 是所有GPT 系列的原型。

>  **【前置】**学本节前请先掌握:(1) 阶段10·01-03(分词器、数据管线) 理解代币ID 序列如何输入模型;(2) 变压器 架构(05阶段) 自我注意、LayerNorm、FFN;(3) 矩阵运算、反向传播手算(03阶段微积分与链式法则);(4) 交叉损失函数的梯度推导――本课**用 numpy 实现**现在,我不需要依赖 PyTorch Autograd,`backward()`,我知道.
...
- 从零开始实现完整的GPT-2架构 (124M参数):代币嵌入,定位嵌入,变压器块和语言模型头
  从零实现完整的GPT-2架构(124M参数):代码 嵌入,位置嵌入,变压器块和语言模型头
- 通过使用交叉缩损失下一个代码预测来训练GPT模型在文本体内
  使用下一代标志 预测和交叉损失在文本语料上训练 GPT 模型
- 实现自动降低文字生成,采用温度采样和顶k/topp过
  实现带温度采样和顶-k/top-p 过的自归文本生成
- 监测训练损失曲线,验证模型学习一致的语言模式
  监控训练损失曲线,验证模型学到了连贯的语言模式

> **【中文解读】**本课程使用纯正的编号从零实现GPT-2小号 (小号) 参数 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) 号) 号 (小号) 号 (小号) 号 (小号) 号 (小号) (小号) 号) (小号) (小号) (小号) 号) (小号) (小号) (小号) 号) (小号) (小号) (小号) (小号)

## 问题 问题引入

你知道变压器是什么. 你已经阅读了图表. 你可以说"注意力就是你需要的"并画一个白板上标记的框"多头注意力".

> 你知道什么是变压器. 你看过图表. 你能背诵"注意力就是你需要的"并在白板上画出标记的"多头注意力"框.
...
> GPT-2 小有124,438,272个参数 (含权重共享) ⋅每个参数都通过训练循环设定:前向传播、计算损失、反向传播、更新权重──12个变压器块,每块12个注意力头,768个维嵌入空间,50,257个词表──每次生成的代币,所有1.24亿个参数链参与一个矩阵乘法,将代币ID序列转换为下一个代币的概率分布率──

如果你从未自己构建过这个模型,你就在使用黑盒子.你可以使用API.你可以调整.但是当某种东西发生错误时 - - 当模型幻觉,重复,拒绝遵循指令时 - - 你没有什么心理模型为什么.

> 如果你从未亲自构建过这个模型,你就在使用一个黑盒子.你可以调用API,可以微调.

这一课构建了GPT-2小从零开始.不是在 PyTorch.在 numpy. 每个矩阵乘法都可见. 每个梯度都由你的代码计算.你会看到12400万个数字如何阴谋预测下一个词.

> 本课从零构建GPT-2小──不用 PyTorch──用 numpy──每矩阵乘法都可见──每梯度都由你的代码计算──你将看到1.24亿个数字如何协作预测下一个词──
...
> 这就是整个模型――没有卷积――没有循环――只有嵌入,注意力,前网络和层归结堆积12次――

>  **【类比】**GPT 像一台"流水线打字机":纸带送进代币ID → 印章 1(代币嵌入)盖出 768 维向量 → 印章 2(位置嵌入) 叠加位置 → 12 道工人(变压器块) 层次修改这个向量 → 末端喷墨头(LM头) 在 50257个候选词上喷概率分布 → 选择最高概率的词输出 → 把新词同时再送回纸带开头,循环――一切都在那 12 道工人头上秘藏着"修改"向量而自我注意力就是工人只用 12 眼睛注意力) 的看序列里其他代币能力.
...
> 剩差连接是关键的.没有它们,梯度在反向传播到第1块时会消失.有它们,梯度可以通过"跳跃"路径直接从损失流到任何层面.

> **【中文解读】**GPT 架构的核心是变压器 解码器块的堆积――每个块包含:LayerNorm → 多头自注意力 → 残差连接 → LayerNorm → 前网络(MLP)→残差连接――GPT-2 使用预规则(先归化再注意力),而不是原始变压器的后规则――残差连接是关键没有它,梯度在12层反向传播后会消失,无法训练深层网络――

> **【拓展：GPT 系列的架构演进】**GPT-2 小(124M,12层 768维)→ GPT-2 中型(355M,24层 1024维)→ GPT-2 大(774M,36层 1280维)→ GPT-2 XL(1.5B,48层 1600维)→ GPT-3(175B,96层 12288维)─架构基本相同,只是层数和维度不断扩大──GPT-4的具体参数未公开,但推测使用了约120层和MoE(混合专家) 架构──

### 注意:核心机制

自我注意让每个代币看看之前的代币,

> 自注意让每个代币 看到每个之前的代币,并决定给每个代币 多少关注.
...
> 因果掩码使GPT成为自归模型.位置5可以注意到位置0-5但不能注意到6、7、8等.

**Multi-head attention**根据"一头"的定义,一个头可以追踪语法关系 (主体-动词协议).另一个头可以追踪语义相似性 (同义词).另一个头可以追踪位置接近性 (近距离的词).所有12个头的输出都是连锁的,并投射到768个维度.

> **多头注意力**将 768 维空间分为 12 个 64 维的头──每个头学习不同的注意力模式──一个头可能追踪句法关系(主谓一致)──另一个可能追踪语义相似性(同义词)──另一个可能追踪位置邻近性(相邻词)──所有 12 个头的输出被拼接并投影回 768 维──
...
> 除以平方 (d_k) sqrt)  (64) = 8是缩放.没有它,点积在高维向量上会变得很大,将软max 推进梯度接近零的区域.

### 基维缓存:为什么推理是快速的

在训练过程中,你一次处理整个序列.在推断过程中,你一次生成一个代币.没有优化,生成代币N需要重新计算所有N-1前代币的注意力.这就是每生成的代币O(N^2) 或为长度N的序列O(N^3) 总数.

> 训练时,你一次处理整个序列――推理时,你个别生成代币――没有优化的话,生成代币 N 需要为所有 N-1 个之前的代币 重新计算注意力――这是每个生成代币的 O(N^2),或长度 N 的序列总共 O(N^3)。
...
> 缓存解决了这个问题――计算每个代币的 K 和 V 后存储它们――生成代币 N+1 时,你只需要计算新代币的 Q 并查找所有之前的代币 缓存的 K 和 V――这将使 K 和 V 计算的每个代币 成本从 O(N) 降至 O(1);;注意分数计算仍然是 O(N),因为你注意到所有之前的位置,但避免了输入上的冗余矩阵乘法――
...
> 对于12层12头的GPT-2,KV 缓存每个代币存储2(K + V) x12层 x12头 x64维 = 18,432个值――对于1024个代币序列,在FP32中约为75MB――对于128层的Llama 3 405B,单个序列的KV 缓存可超过10GB――这就是为什么长上下文推理是内存受限的――

### 预填与解码:两阶段的推理

当你向法学士发送提示时, 推断发生在两个不同的阶段.

> 当你发送到LLM时,建议发生在两个不同阶段.

**Prefill**处理整个提示符并行.所有代币都已知,所以模型可以同时计算所有位置的注意力.这个阶段是计算的 - - GPU在完成全吞吐量矩阵乘法.对于一个A100上的1000代币提示符,预填需要大约20-50ms.

> **预填充（Prefill）**并行处理整个提示――所有代币已知,所以模型可以同时计算所有位置的注意力――这个阶段是计算密集型GPU以全吞吐量做矩阵乘法――在A100上处理1000代币的提示,预填需要约20-50ms――
...
> **解码（Decode）**个别生成代币――每个新代币取决于所有之前的代币――这个阶段是访问存储密集型瓶从GPU内存读取模型权重和KV缓存,而不是矩阵运算本身――GPU的计算核心大部分时间都在等待内存读取――对于GPT-2,每个解码步骤的时间大致相同,无论矩阵乘法需要多少FLOP,因为内存宽度是有限的――

这种区别对于生产系统来说很重要.使用GPU计算预填输量 (更多FLOPS =更快的预填).用存储带宽 (更快的存储器 =更快的解码) 解码输量.这就是为什么NVIDIA的H100专注于提升存储带宽比A100 - 它直接加快代币生成.

> 这种区别对生产系统很重要.预充吞吐量与GPU计算能力比例. 更多的FLOPS = 更快预充. 解码吞吐量与内存宽度比例.
...
> 训练 LLM 就是下一代币 预测――给定代币 [0, 1, 2, ..., N-1],预测代币 [1, 2, 3, ..., N]――损失函数是模型预测的概率分布与实际下一代币 之间的交叉──
...
> 学习率调度比你想象的更重要. 在前2000步的 GPT-2 开始从0 预热到峰值学习率,然后按余弦曲线衰退.
...
> **【中文解读】**GPT-2 的参数分布:代币嵌层占 38.6M(50257 x 768),12个变压器块各占 7.1M,最终 LayerNorm 仅 1.5K──权重共享(重量绑定) 让输出投影层复用代币 嵌入矩阵,减少38M参数的同时还提升了性能因为输入和输出被强制使用相同表示空间──
...
> 符号嵌入将50,257个可能的符号,各映射到一个768维向量.
...
> 标准差的初始化来自GPT-2论文. 太大则初始前向传播产生极端值,破坏训练稳定性.

### 步骤2: 用因果面具自卫

原因面具在软max之前将未来位置设置为负无限,确保每个位置只能关注自己和以前的位置.

> 首先做单头注意力. 原因掩盖在软max 前将未来位置设为负无限,确保每个位置只能注意到自己和更早的位置.
...
> 软max 实现在取指数前减去最大值――没有这个,exp(大_数) 会溢出到无穷大――这是一个数值稳定性技巧,不改变输出,因为对任意常数 c,软max(x - c) =软max(x)。

### 步骤3:多头注意力

根据768维输入的数据,分为12个,每个头有64维度.每个头独立计算注意力.

> 将768维输入分为12个64维的头.每个头独立计算注意力.
...
> 张量变为 (批量, seq_len, 12, 64),然后 (批量, 12, seq_len, 64) 现在 12个头各有自己的 (seq_len, 64) 矩阵来运行注意力.

### 步骤4:变压器块

一个完整的变压器块:LayerNorm,多头注意力与残余,LayerNorm,向与残余.

> 一个完整的变压器块:LayerNorm、带残差的多头注意力、LayerNorm、带残差的前网络──
...
> 前网络将 768 维输入扩展到 3,072 维(4倍),应用非线性,然后投影回 768 . 这种扩缩模式给模型一个"更宽"的内部表示在每个位置工作.

### 步骤5:完整的GPT模型

堆12个变压器块,在前面添加嵌入层,后面添加输出投影.

> 堆叠12个变压器块. 前面加嵌层,后面加输出投影.
...
> 注意权重共享:`logits = x @ self.embedding.token_embed.T`△输出投影复用代币 嵌入矩阵 (转置) △这不仅仅是节省参数的技巧. 它意味着模型使用相同的量空间来理解代币 (嵌入) 和预测代币 (预测代币) △输出 (输出) △

### 第六步:训练循环

对于一个真正的训练运行在124M参数,你需要一个GPU和PyTorch.这个训练循环展示了一个小型模型的机械,它运行在纯粹的 numpy.我们使用一个小模型 (4层, 4头, 128 个) 让它可处理.

> 对于 124M 参数的真正训练运行,你需要GPU和PyTorch. 这个训练循环在纯粹的形运行小模型上演示机制.
...
> 损失初始接近 ln(vocacab_size) 对于 256 个代币的字节级词表,即 ln(256) = 5.55。随机模型给每个代币 分配等概率──随着训练进行,损失下降,因为模型学会预测常见模式:"t" 后面是"th",句号后面是空格等。

在生产中,你会使用亚当优化器,加上梯度,学习速度加热,和梯度剪辑.前进-通过-损失-后退更新循环是相同的.优化器更复杂.

> 在生产中,你会使用亚当优化器配合梯度累积,学习率预热和梯度剪切. 前向传播-损失-反向传播-更新的循环是相同的.优化器更复杂.

### 步骤7: 创建文字

生成使用训练模型以一次预测一个代币.每个预测都是从输出分布中采样 (或贪地用 argmax)

> 生成使用训练好的模型逐个预测代币――每个预测从输出分布中采样(或贪心地取 argmax) 』
...
> `tokens[-seq_len:]`窗口是必要的,因为模型有最大上下文长度 ((GPT-2 为 1024) ⋅一旦超出,你必须丢弃最旧的标志――这是大家说的"上下文窗口"――
...
> 在小语料和小模型上,生成的文本充满其量是半连贯的.它从训练文本中学到一些字节级模式,但无法像GPT-2在40GB的训练数据和完整的124M参数架构上那样泛化.关键不在输出质量中.关键在于你可以追踪每一步:嵌入查找,注意力计算,前变化,逻辑投影,软max和采样.每个操作都是可见的.

## 运送它.

这一课产生了`outputs/prompt-gpt-architecture-analyzer.md`通过GPT模型的结构选择进行分析,给它提供模型卡或技术报告,

> 本课产出发 `outputs/prompt-gpt-architecture-analyzer.md`一个任意的分析 GPT 风格模型架构选择的提示──输入模型卡或技术报告,它将分解参数分配、注意设计和缩小决策──
