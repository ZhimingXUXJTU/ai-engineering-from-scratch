...
> **【中文解读】**预沿 LLM 生成一个代币 需要一次完整的数十亿参数前向传播――投机解码使用小模型猜测接下来 3-5个代币,大模型只需要验证――猜对时 5个代币只算一个代币――EAGLE-3将接受率推向到4.5个代币/验证,4-5倍加速――

> **【拓展：投机解码→推理加速】**投机解码是2025-2026年推理加速的标准技术――DeepSeek-V3的MTP头在推理时被重复用于草稿器,接受率80%以上――vLLM、TensorRT-LLM 都已内置支持――

>  **【前置】**学本节前请先掌握:阶段 07·16(投机解码数学证明);阶段 10·12(推理优化基础) ――本节是阶段 10·15(EAGLE-3) 的更早版本(基础+EAGLE),阶段 10·15 是 V3 升级版。
...
> **【中文解读】**投机解码的核心思想:使用一个小型草稿模型快速生成 k 个候选标记,再使用目标大模型并行验证这个 k 个标记――被接受的标记直接保留(零额外延迟),被拒绝从第一个错误处重新生成――数学证明:输出分布与直接使用大模型生成完全相同――

> **【拓展：投机解码的工程实践】**投机解码的加速比取决于草稿模型与目标模型的分布匹配度和批量处理的并行效率.
...
> **【拓展：投机解码的数学保证】**投机解码的输出分布与目标模型完全一致,而不是近似,而是数学上严格等价.
...
> 本课产出发 `outputs/skill-speculative-tuning.md`一个分析目标模型工作负载并选择草稿模型、K(草稿长度) 、树宽度、温度以及何时回归普通解码的技能──

## 练习题

1. 执行确切的拒绝规则,经验验证它.`speculative_decode`通过简单的目标样本采集,计算两个输出分布之间的电视距离. 应为<0.01.
   中文翻译:实现精确拒绝规则并经验验证──通过 `speculative_decode`和普通目标采样各运行 10K样本;计算两个输出分布之间的电视距离──应 < 0.01──

2. 计算加快公式,给定了`α`其他`K`图表预期的代币每目标前进. 找出α ∈ {0.5,0.7,0.9} 的最佳K.
   中文翻译:计算加速公式――给定固定`α`和 `K`绘制每次目标前向传播的期望标志 数――找到 α ∈ {0.5, 0.7, 0.9} 的最佳 K――

3. 训练一个小的草稿. 拿一个124MGPT-2目标,并在100M代币上除一个30MGPT-2草稿.`α`预期:0.6至0.7.
   中文翻译:训练微型草稿模型──以 124M GPT-2 为目标,在 100M标志上使用 KL 损失蒸 30M GPT-2 草稿──在保留文本上测量 `α`◎预期:0.6-0.7─

4. 执行EIGLE样式的树草图. 代替链条,将草图输出的每一个深度上三个分支. 构建树注意力面具. 检查目标接受最长正确的分支.
   中文翻译:实现 EAGLE 风格树状草稿──不是链式,而是让草稿在每个深度输出前三分支──构建树力掩码──验证目标接受最长正确分支──
