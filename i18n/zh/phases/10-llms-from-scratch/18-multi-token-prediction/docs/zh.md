...
> **【中文解读】**传统的LLM每一个位置只预测下一个代币――DeepSeek-V3 在每个位置增加第二个损失:预测下下一个代币――额外的14B参数通过梯度流蒸回主模型,训练有素的MTP头在推理时被用于投机解码的草稿器(80%+接受率),1.8倍吞吐提升免费获得――

> **【拓展：MTP→DeepSeek-V3创新】** MTP 是DeepSeek-V3的四大架构创新之一. 它同时改善了训练.

>  **【前置】**学本节前请先掌握:阶段10·04(预训GPT);阶段10·15(推测解码) MTP 在推理时变成草稿器;因果链(因果链) 概念──本节是阶段10·20(深度寻找-V3 走路) 的前置──
>  **【类比】**传统下一个标志 = 学英语时一次记一个单词――MTP = 同时记"这个单词"和"下一个常用搭配"训练时学更多结构(梯度信号丰富),推理时把"搭配预测器"当草稿器免费加速(80% 接受率) ⋅ 一举两得──
...
> **【中文解读】**多代币预测让模型同时预测多个未来代币 (而不是仅预测下一个),提供更丰富的训练信号――Meta的研究表明多代币预测不仅加速训练,还可以提高模型的规划能力和推理性能――

> **【拓展：Meta 的多 token 预测研究】**预测4代币在代码生成和推理任务上显著优于下一个代币标准预测.DeepSeek-V3也使用辅助多代币预测头来增强训练信号.
...
> **【拓展：多 token 预测与推理加速的联系】**多代币预测训练的模型自然适合投机解码因为它已经学会了预测未来多代币――Meta的研究表明,4-代币预测训练的模型在代码生成上提高了5-10%比标准训练――
...
> 本课产出发 `outputs/skill-mtp-planner.md`△给定预训运行规范(模型大小、数据、算力),它返回集成MTP的计划:深度数 D、`lambda`调度,内存开销和推理时投机解码连接

## 练习题

1. 跑步`code/main.py`显示合成信号强化时,每深度损失单调减少. 修改合成以使用固定模式,并验证深度-1和深度-2损失相近.
   中文翻译:运行 `code/main.py`△显示每次深度损失随着合成信号增强单调递减――修改合成数据使用固定模式并验证深度1和深度2的损失都收到了──

2. 计算密集70B模型 (隐藏8192,80层) 的参数上层费用.与D=1MTP模块的DepSeek-V3报告的14B上层费用进行比较.解释为什么DepSeek的数量更高:MTP变压器块继承了相同的MoE结构,从而增加了每个模块参数数数.
   中文翻译:计算密集 70B 模型(隐藏 8192.80 层) 加 D=1 MTP 模块的参数开销――与 DeepSeek-V3 报告的 14B 开销比较――解释为什么 DeepSeek 的数字更高:MTP 变压器块继承了相同的 MoE 结构,膨胀了每个模块的参数――

3. 运用 D=2 在玩具中:添加第二个MTP模块,它取 h^(1) 并预测`t_{i+2}`检查联合损失和参数会计符合深度搜索论文19-21的方程.
   中文翻译:在玩具模型中实现 D=2:添加第二个MTP模块,取 h^(1) 并预测 `t_{i+2}`△验证联合损失和参数核算与深度搜索论文公式 19-21 匹配──

4. 切换玩具为平行MTP (Gloeckle式):在主要隐藏状态之上添加D输出头,每个都预测不同的偏移.测量每深度的损失与同一合成信号的连续版本相比较.连续版本应该产生较低的 k > 1的深度损失,因为它会对中间预测进行条件.
   中文翻译:将玩具模型转换为并行MTP(Gloeckle风格):在主隐藏状态上添加D个输出头,每个预测不同偏移――测量每次深度损失与顺序版本在同一合成信号上的比较――顺序版本在k > 1 时应产生更低的深度 k 损失,因为它在中间预测为条件――

5. 使用训练有素的MTP模块作为EAGLE样式的草案:调用模块 k提出 `t_{i+k}`根据模型的预测,这些图标的接受率是对待的. 如果在玩具上达到50%以上,你将复制了经验性MTP-as-draft属性.
   中文翻译:将训练的MTP模块用作 EAGLE风格草稿:在推理时调用模块 k 提出 `t_{i+k}`△在保留序列中测量这些草稿标志对主模型预测的接受率. 如果玩具模型达到50%+,你已经复现了MTP作为草案的经验特性.
