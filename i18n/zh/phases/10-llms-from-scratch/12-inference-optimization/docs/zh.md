...
> **【中文解读】**预填,并行处理快速,计算密集) 和解码 (单个代币生成,显存带宽密集) 设置:

> **【拓展：推理优化→生产部署】**实现了KV缓存管理连续批量处理 (连续批量处理) 页面关注等优化.

>  **【前置】**转型器架构、注意力;GPU 计算与显存带宽概念──本节是基础设施和生产的第十七阶段.

>  **【类比】**读书 + 写读书笔记。**Prefill**快速翻完书 (处理提示)**Decode**逐字写笔记一次写一个字,显存带宽密集) 』**KV cache**记住每页关键信息,写笔记时不需要重复);**continuous batching**现在,我在读书中写的文章.
...
- 实现KV缓存,以消除自动降低代币生成期间冗余计算
  实现 KV 缓存以消除自归代币 生成过程中的冗余计算
- 解释LLM推理的预填与解码阶段以及为什么每个阶段都有不同的瓶 (计算与记忆)
  解释 LLM 推理的预填充和解码阶段及其不同瓶(计算密集vs访存密集)
- 实现连续批量和PagedAttention概念,以最大限度地在同时请求下利用GPU
  实现连续批处理和页面关注 概念,在并发请求下最大化GPU利用率
- 比较推断优化技术 (KV缓存,投机解码,闪存注意力) 和其吞吐量/延迟权衡
  比较推理优化技术(KV 缓存、投机解码、闪光注意)及其吞吐量/延迟权衡

> **【中文解读】**本课聚焦 LLM 推理优化――两个核心概念:(1) 预填 阶段并行处理提示,限于GPU 计算量(FLOPS);(2) 解码 阶段以代币生成,限于显存带宽――KV-缓存 避免重复计算已生成的代币的K/V,连续批量处理最大化GPU利用率,PagedAttention 解决KV-缓存的存片问题显――

## 问题 问题引入

您在 4xA100 GPU 上部署Llama 3 70B. 一个用户每秒获得约50个代币.感觉很快. 然后100个用户同时达到终端点. 吞吐量下降到3个代币/秒/用户. 您的每月25,000美元的 GPU 账单服务响应速度比人类类型慢.

> 你在4xA100GPU上部署Llama 3 70B──单个用户获得约50个代币/秒──感觉很流──然后100个用户同时访问端点──吞吐量降至每用户3个代币/秒──你每月25,000美元的GPU账单提供比人类打字还慢的响应──
...
> 模型本身在 1 用户和 100 用户之间没有变化.相同权重,相同结构,相同的数学.变化是你如何调整工作.简单的推理浪费了可用的GPU计算的 90%+. 等待第 47 个代币的用户占据了整个批次槽位,而GPU 显存总线在矩阵乘法之间置. 与此同时,一个新用户的 2,000 个代币提示可以使用有用的计算来填满那段时间.

这不是扩展问题. 这是一个规划问题. 这课中的技术 - - KV缓存,连续批量,页面注意,推测解码,前缓存 - - 是分开一个$25k/month inference bill from a $平均每月5千辆,服务于同一个交通.

> 这不是扩展问题,而是调度问题. 本课程的技术KV 缓存,连续批量处理,$25k 推理账单降为服务相同流量的 $五千个账单的关键.
...
> 在4xA100-80GB上服务 Llama 3 70B,低发发时达到约50个代币/秒/用户,通过连续批量处理和 PagedAttention 在 100 并发请求下维持15-25 TPS/用户──没有这些优化,相同硬件在该发发下只服务 5 TPS/用户──相同的GPU、相同的模型、4倍吞吐量──

## 概念的核心概念

> **【中文解读】**推理的两个阶段有截然不同的瓶:前填阶段限于GPU FLOPS (计算量),解码阶段限于显存带宽 (显存带宽) 对于每个生成都要读取整个模型权重 (重量) ;;KV-缓存是最基本的优化缓存已生成的代币的关键/值,避免重复计算,将解码阶段的复杂性从O (n) 降至O (n) ⋅

> **【拓展：vLLM 和 PagedAttention】**引入的VLLM 页面注意力将KV缓存像虚拟内存一样管理,使用固定的大小页面存储KV向量,解决显着存储碎片问题──这使得VLLM的吞吐量比 HuggingFace TGI高2~4倍──连续批量处理 (连续批量处理) 如果有请求完成时立即插入新请求,将GPU利用率从约30%升至90%以上──
...
> **预填充**处理整个输入提示――所有代币已知,所以注意力可以在完整序列上并行计算――这是大型矩阵乘法GPU 核心保持忙碌――瓶是计算:硬件每秒能提供多少FLOPS――A100做 312 TFLOPS(BF16)─70B 模型上 4,096-代币提示的预填充在单张A100上需要约400ms――
...
> **解码**个别生成输出代币――每个新代币――关注所有前代币,但每次向前传播只产生一个代币――权力矩阵与预填充时的尺寸相同,但你使用单个向量而不是矩阵与相乘――GPU核心在微秒内完成,然后等待下一批权力从内存到内存.瓶是内存宽度:你能以多快的速度将模型权力从HBM传输到计算单元――A100 有 2 TB/s 带宽――FP16 的 70B 模型是 140 GB――读完整模型需要 70ms这是单次解码步骤的下限.
...
> 核心洞察:*解码是内存密集型的,因为你读取整个模型来生成单个代币*──下面的每个优化要么减少读取量,要么增加每次读取处理的代币批次,或者完全避免读取──

### 存储器

在注意期间,每个代币的查询都会关注每个前代币的关键和值向量.没有缓存,生成代币N需要重新计算所有N-1前代币的关键和值预测.在生成代币 2,然后再次为代币3,然后再次为代币4.通过代币1000,你已经投影代币1总共999次.

> 在注意计算中,每个代币的查询关注所有前代币的键和值向量――没有缓存时,生成代币 N 需要重新计算所有N-1个前代币的键和值投投放――生成代币 2 时投放 token 1,token 3 时重投放,token 4 时重投放――到代币 1,000 时,代币 1 被投放 999 次――

存储KV缓存存储所有前代币的关键和值预测.在生成代币N时,您只计算代币N的关键和值,然后将它们与从代币1到N-1的缓存K/V连接.

> 缓存存储存储所有前代币的键和值投影──生成代币N时,你只计算代币N的键和值,然后与缓存代币1到N-1的K/V拼拼音──
...
> 拉马370B单个 128K 上下文对话消耗40GB KV 缓存半张 A100内存──100个并发用户每一个4K代币时,只需要128GB的KV 缓存──这就是为什么KV 缓存管理是推理优化的核心挑战──

### 连续批量

静态批量等到N请求的批量到来,处理它们在一起,并在接受新请求之前等到*全部*完成.如果一个请求需要500个代币,另一个需要10,则短请求在完成后490个解码步骤停留无所作为.

> 静态批量处理等待一批 N 请求到达,一起处理,等待*所有*完成后再接受新请求――如果一个请求需要500个代币 另一个需要10个,短请求在完成后置490个解码步骤――

连续批量 (也称为反复级批量) 随着任何请求完成,就会将新请求插入批量中.每次解码步骤都会重新评估批量.在10个代币后完成的请求立即被等待请求取代.

> 连续批处理 (也叫代级批处理) 在任何请求完成后立即将新请求插入批次.
...
> 解码慢是因为它是顺序的生成一个代币,反回去,生成下一个......但是如果你能低估下一个5个代币,然后一次验证所有?
...
> 投机解码使用小而快的**草稿模型**生成 K 个候选标志──大型**目标模型**然后在单次前向传播中处理所有K个候选人 (如果目标模型同意草稿模型的预测,你会在一个目标前向传播的时间内接受所有K个标志.如果它在位置j不同意,你会接受标志1到j-1并丢弃其余.
...
> 加速取决于**接受率**草稿模型的预测多频繁地匹配目标――Llama 3 8B 为 Llama 3 70B 草稿做时,自然语言的接受率通常为70-85%――这转化为解码加速的2-3倍――
...
> 投机解码是*数学上精确的*输出分布与目标模型的分布完全相同. 它不是近似的. 验证步骤确保每个接受的代币的分配概率恰好具有目标模型.

### 预写 缓存

许多请求共享相同的预先语.一个聊天机器人系统提示.一个RAG文本区块.几个拍摄的例子设置.没有预先语缓存,每个请求从零开始重新计算这些共享代币的KV缓存.

> 许多请求共享相同的前──聊天机器人系统提示──RAG 上下文块──少样本示例集──没有前缓存,每个请求从头重新计算这些共享代币的KV缓存──

预先预先存储KV缓存用于常见预先存储器,并在请求中重复使用.当一个新的请求带有已知预先存储器到来时,系统复制 (或引用) 缓存的KV输入,仅计算KV为独特的后音.

> 前缓存存储常见前的KV 缓存并请求间复用──当新请求带有已知前到达时,系统复制或引用)缓存的KV 条目,只计算唯一后的KV──

对于所有请求中共享2000个代币的系统提示,预先缓存消除了每请求的预填量400ms. 在每秒100次请求时,这节省了每秒40秒的GPU计算 - - 超过一个GPU的值.

> 对于所有请求共享的2000代币系统提示,前缓存消除了每一个请求的400ms预充. 在100个请求/秒时,每秒节省40秒的GPU计算超过一个GPU工作量.
...
> 本课产出:
> - `outputs/skill-inference-optimization.md`诊断和优化士学位 推服务技能

## 练习题

1. 修改KV缓存配置文件,以比较FP16与FP8与INT4KV缓存量化.在4K环境下,计算每一个用户的最大共享用户为4xA100-80GB.KV量化到INT4应该大约是4倍的用户容量.
   中文翻译:修改KV 缓存分析器比较FP16 vs FP8 vs INT4 KV 缓存量化。对于4K 上下文的Llama 3 70B,计算4xA100-80GB 上每种格式最大并发用户数量──INT4 KV 量化应大约4倍用户容量──

2. 扩展连续批量模拟器,以追踪GPU利用率 (每步填充的批量插槽的部分).随时间推移,用于静态和连续批量,采用50个请求,其输出长度遵循帕雷托分布 (形状=1.5,规模=20).连续批量应保持80%的利用率.
   中文翻译:扩展连续批处理模拟器跟踪GPU利用率(每步批次槽位填充比例) ・绘制静态和连续批处理随时间的利用率图,50个请求的输出长度遵循帕累托分布(形状=1.5,规模=20) ・连续批处理应维持 >80%利用率。

3. 实现KV缓存的集成查询注意 (GQA) 版本,`num_kv_heads < num_query_heads`计算存储存量与全重多头注意力 (KV缓存大小减少8倍).
   中文翻译:实现分组查询注意力(GQA) 版本的KV 缓存,其中`num_kv_heads < num_query_heads`拉马370B 用64个查询头,但只有8个KV头.

4. 建立一个使用 LRU 驱逐的预写缓存.设置最大_entries为500,生成1000个请求,其中60%共享了5个常见预写中的一个.测量击率并与无限缓存进行比较.如果有良好的驱逐,击率应该保持在 55% 以上.
   中文翻译:构建使用 LRU 淘汰的前缓存――设置最大_输入为500,生成1000个请求,其中60%共享5个常见前之一――测量命中率并与无限缓存比较――良好淘汰下命中率应保持在 55% 以上――

5. 扩展投机解码模拟器实现基于树的投机 (EAGLE-2 式).而不是单个链K草案代币,生成一个候选树 (例如,每个3级的2个分支=8个叶子候选).对验证轮和线性投机均接受的总代币进行比较.
   中文翻译:扩展投机解码模拟器实现树状投机(EAGLE-2风格) ・・・不是 K 个草稿代币的单链,而是生成候选树(例如 3 层每层 2 分支 = 8 个叶候选) ・・・比较每轮验证接受代币 总数与线性投机。
