...
>  前置 轨道B5/20──基于30-33──
> 变压器块 = "现代解码器 LLM 的乐高单元"――LayerNorm+多头注意力+残差+MLP+残差──前LN(不需要加热 稳定)vs后LN(原始论文版)──本课并排实现两个,看哪个能在12层堆叠+常见学习率下存活──
**Time:** ~90 minutes | **时间:** ~90 minutes

## 学习目标

- 在 PyTorch 中构建一个变压器块,从四个移动件中:LayerNorm,多头因果注意力,残留连接,位置智能MLP.
  中文翻译:从四个移动部分构建 PyTorch 中变压器块:LayerNorm,多头因果注意力,残留连接,位置智能MLP.
- 设置LayerNorms在两个配置 (LN前和LN后) 并解释为什么一个火车稳定地没有加热.
  中文翻译:将LayerNorms分为两个配置 (LN前和LN后) 并解释为什么一个人稳定地没有加热.
- 实现因果化掩饰在多头注意力中,所以标志性`i`没有看到代币`j > i`现在,我们要去.
  中文翻译: 实施因果化掩饰在多头注意力中,`i`没有看到代币`j > i`现在,我们要去.
- 追踪梯度流动在12层堆上的两种变体,并读取结果,而不挥手.
  中文翻译: 轨道梯度在12层堆上通过两种变体流动,并没有挥手读取结果.
- 在下一个课程组装12400万参数GPT时,再用块作为一个落入单位.
  中文翻译:当下课组装12400万参数GPT时,再用区块作为一个落入单位.

## 问题问题

> **【中文解读】**变压器是块的重复. 如果块本身有错误,重复12次就会产生一个在第一时代就发散或需要加热的模型.

> **【拓展：Pre-LN vs Post-LN 的工程影响】**GPT-2(2019) 使用后LN,训练需要加热――GPT-3(2020)及之后所有主流LLM(LLaMA、Mistral、Qwen) 使用前LN──Pre-LN 的关键优势:残差路径上的梯度不被 LayerNorm 衰减,12 层甚至 96 层(GPT-3 175B) 的堆叠中梯度传播更稳定──LLaMA 进一步将 LayerNorm 替换为 RMSNorm 参数更少,计算更快),配合 SiLU 激活函数──一次错误地完成块,重复12次,然后你运送一个在第一时代分歧的模型,或者需要加热的模型在其他过程中破解. 在这门课中,你会看到的两个失败模式并不奇怪. 学生第一次起块时,它们就会出现. 一是关注未来的注意力层. 另一种是LayerNorm放置在它不能制在深度的残留信号.

现在,我们可以看到一个小块,它是机械的. 块有两个剩余路径和两个正常化位置.

> 现在,我们可以看到一个单元的位置,然后我们可以看到一个单元的位置.
...
> 每个解码器只有变压器块是一个函数,它采用了形状子.`(batch, sequence, embedding)`在内部,两个子层做了这项工作.
...
> 现在,我们需要一个新的系统,一个新的系统,一个新的系统,一个新的系统,一个新的系统.


后LN变体将LayerNorm移动到剩余添加后.

> 后续增值后将LayerNorm移动到后续增值后.
...
> **【中文解读】**注意力层将输入投影为Q、K、V 三张量,每个从`(B, T, D)`重塑为`(B, H, T, D/H)`△计算`softmax(Q K^T / sqrt(d_k))`并应用因果掩码 ((上三角设为负无穷),然后乘以V──头拼接回`(B, T, D)`后再做一次输出投影――因果掩码是唯一使模型成为解码器的组件忘记掩码等于训练一个作弊模型――
...
> **【中文解读】**差距连接做两件事: 1) 使梯度路径跨深度加法式加加加,保持梯度范数在12层中稳定; 2) 让每个块学习运行表征加法更新而不是完全替代.

它们使梯度路径在深度上变量,从而保持梯度标准在尺度中通过十二层.它们还让每个块学习运行表示的增量更新而不是完全的替代.这两个效果是区块尺度的原因.

> 它们使梯度路径在深度上添加,从而保持梯度标准在尺度中通过十二层.
...
- `class LayerNorm`具有可学习的规模和转移,偏向的eps,按代币向量应用.
  翻译: 中文`class LayerNorm`具有可学习的规模和转移,偏向的eps,按代币向量应用.
- `class MultiHeadAttention`随着`num_heads`现在`head_dim = d_model // num_heads`化QKV投射,注册因果面具,注意力和残留脱节.
  翻译: 中文`class MultiHeadAttention`随着`num_heads`现在`head_dim = d_model // num_heads`化QKV投射,注册因果面具,注意力和残留脱节.
- `class FeedForward`两层线性,GELU激活,脱落.
  翻译: 中文`class FeedForward`两层线性,GELU激活,脱落.
- `class TransformerBlock`具有一个`pre_ln`两种变体之间转换的旗.
  翻译: 中文`class TransformerBlock`具有一个`pre_ln`两种变体之间转换的旗.
- 构建一个6层前LN堆和一个6层后LN堆的演示,具有相同的输入和打印 (a) 出口形状, (b) 后退的传输后嵌入式的梯度标准.
  中文翻译:一个演示,构建一个6层前LN堆和一个6层后LN堆,具有相同的输入和打印 (a) 出口形状, (b) 后退的传输后嵌入的梯度标准.
...
- `torch`对于数数学,自格级,`nn.Module`管道.
  翻译: 中文`torch`对于数数学,自格级,`nn.Module`管道.
- 没有.`transformers`没有预训练的重量. 区块是从原始的实现.
  中文翻译:没有`transformers`没有预训练的重量. 区块是从原始的实现.

## 野生生产模式

> **【拓展：现代 Transformer 块的变体】**除本课的GPT-2风格块,主要变体包括:(1) LLaMA块:RMSNorm + SwiGLU 激活(MLP 中门 分支) + RoPE 旋转位置编码,无偏见;(2) Mistral块:与LLaMA 类似但使用滑窗注意(SWA,窗户大小 4096) 降低长序列成本;(3) Mixtral块:在MLP层使用稀疏混合专家(8个专家中选 2个),参数增加大但计算量不变.
...
- 在第35课中,这个课程的块直接连接到GPT组件中,没有修改.
  中文翻译:本课中的块直接连接到第35课中GPT组件中,没有修改.
- 之前LN变体是每个现代开放权重LLM使用的.后LN变体是原始2017年注意力纸使用的.知道这两种是足够的阅读任何你会遇到的解码架构.
  中文翻译:LN前变体是每个现代开放权重LLM所使用的.LN后变体是原始2017年的注意力论文所使用的.了解这两种都足以阅读你会遇到的任何解码架构.
- 换GELU为SiLU,就会有LLaMA家族激活,换LayerNorm为RMSNorm,就会有LLaMA家族正常化.
  中文翻译:换GELU为SiLU,你会得到LLaMA家族激活.换LayerNorm为RMSNorm,你会得到LLaMA家族正常化.
...
- 阶段7课程02 (自从零开始注意) 为这个区块下面的注意力数学.
  中文翻译:第7阶段课程02 (自从零开始注意) 为这个区块下面的注意力数学.
- 阶段7课05 (全变压器) 对同一骨架的编码解码版本.
  中文翻译:第7阶段课05 (完整变压器) 对于同一骨架的编码解码版本.
- 阶段10课04 (预训练小GPT) 对于该区块所涉及的培训程序.
  中文翻译:第10阶段课时04 (预训练迷你GPT)
- 阶段19课35 (本轨道) 将这两个块堆积成GPT模型.
  中文翻译:第19阶段课程35 (这条轨道) 将这两个块堆积成GPT模型.
