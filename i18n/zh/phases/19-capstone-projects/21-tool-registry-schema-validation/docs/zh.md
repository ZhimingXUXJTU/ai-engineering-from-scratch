...
>  **【前置】**导剂使用 2/10──参考13·07期工具方案
>  工具注册中心 = 工具调用"类检器"――代理 不能验证的工具= 不可调用工具――先建注册表+方案 检查,再建工具――

## 学习目标
- 保持一个输入的工具名称 → schema → 处理器登记,发送者可以一次询问,然后信任.
  中文翻译:保持一个输入工具名称 → schema →处理器的注册表,发送者可以一次询问,然后信任.
- 实现一个JSON Schema 2020-12子集,涵盖了工具调用中90%的关键字.
  中文翻译:实现一个JSON Schema 2020-12子集,涵盖了工具调用中90%的关键字.
- 返回精确的,Json指针形状的错误路径,以便模型在一次回路中自行纠正.
  中文翻译:返回精确的, json指针形状的错误路径,使模型可以在一次回路中自行纠正.
- 拒绝重新注册,没有明确的覆盖,因为无声覆盖是生产工具目录的流动方式.
  中文翻译:拒绝重新注册,没有明确的覆盖,因为默默的覆盖是生产工具目录的漂移方式.
- 保持验证器纯净 (无输入/运输,无时间,无全球)
  中文翻译:保持验证器纯净 (没有I/O,没有时间,没有全球)
...
> **【中文解读】**本节强调注册中心必须先构建工具.2026年编码 代理注册工具数量超过模型单次上下文窗口容量范围200个工具中每轮只暴露10-40个.注册中心是"什么工具存在"",参数是什么形状"",调用什么处理器"的唯一事实来源.避免的错误是:发布处理器没有计划,或发布计划没有验证两者都会让调度器变成猜测游戏.

> **【拓展：MCP 工具注册实践】**模型环境协议 中,每个MCP 服务器通过 `tools/list`方法暴露工具清单,每个工具包含名称,描述,输入Schema(JSON Schema) ――Claude Code等客户端在会话开始时获取工具列表,按需选择暴露模型的子集──JSON Schema 2020-12 子集覆盖90% 实际使用的关键字:类型,属性,要求,数量,项目,最小/最大的模式──验证器必须纯净──无 I/O/时间/全局变量) 支持重放日志──

2026年,编码代理的注册工具超过模型可以在单一的文本窗口中合适的数量. 无关紧要的带将记录200个工具,在任何转折时,表面上出现10到40个. 记录是"有什么工具","他们的论点有什么形状",以及"我叫什么处理器".

> 2026年,一个编码代理的工具比模型可以在单个文本窗口中容纳更多. 无关紧要的带将记录200个工具,在任何转折时,表面上出现10到40个. 记录是"有什么工具","他们的论点有什么形状",以及"我叫什么处理器".


我们避免的错误是没有计划的运输手机,或者没有验证的运输手机.这两者都是共同的.这两者都将下一个层 (第23课中的发送器) 变成了一个猜测游戏,唯一的失败模式是从处理器那里的堆痕迹.

> 我们避免的错误是没有计划的运输处理器,或者没有验证的运输计划.这两者都是共同的.这两者都将下一个层 (第23课中的发送器) 转化为猜测游戏,唯一的失败模式是处理器的堆痕迹.
...
> 验证器触摸的唯一场是 schema. 处理器对它不透明. 我们故意分开它们. 方案是数据. 处理器是代码. 混合它们诱惑你将验证逻辑放进处理器,这是我们正在阻止的错误.
...
> 如果一个方案需要 模型可以更好地读出错误路径,`args.user.email`如果模型通过整数,则错误应该是`/user/email`随着`expected_type: string`模型在下一次电话中没有自然语言的解决方案.
...
> `register(name, schema, handler, **opts)`默认拒绝重新注册.


登记库揭示了三种阅读方法.`get(name)`报价或升级.`validate(name, args)`返回一个`Ok`没有任何错误.`names()`返回工具名称以注册顺序.

> 记录显示了三个读取方法.`get(name)`报价或升级.`validate(name, args)`返回一个`Ok`没有任何错误.`names()`返回工具名称以注册顺序.
