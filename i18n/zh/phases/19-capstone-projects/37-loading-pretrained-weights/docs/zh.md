...
>  前置 轨道B 8/20──基于30-36──
> 载预训权重 = "站在巨人肩膀上"――从零训练 124M 模型=预算决策;载已发布检查点=日常操作――本课从安全感应器 加载GPT-2 风格权重到35课的架构,逐个走参数名映射,生成续写证明加载成功――无网络、无第三方加载者、无黑魔法――
**Time:** ~90 minutes | **时间:** ~90 minutes

## 学习目标

- 阅读一个安全感应器文件`safetensors`检查子名称和形状.
  中文翻译:用 读一个安全感器文件`safetensors`检查子名称和形状.
- 根据教程35GPT模型,每个预训练的参数名称都将被映射到一个参数中.
  中文翻译:将每个预训练参数名称映射到课35GPT模型内的参数中.
- 处理公布的GPT-2重量和该轨道中的模型之间的两个名称公约: `wte/wpe/h.N.attn.c_attn/c_proj`其他`mlp.c_fc/c_proj`根据当地名称`tok_embed/pos_embed/blocks.N.attn.qkv/out_proj`其他`mlp.fc1/fc2`现在,我们要去.
  中文翻译:处理两种名称公约,这些名称不同于GPT-2的发布权重和该轨道中的模型: `wte/wpe/h.N.attn.c_attn/c_proj`其他`mlp.c_fc/c_proj`根据当地名称`tok_embed/pos_embed/blocks.N.attn.qkv/out_proj`其他`mlp.fc1/fc2`现在,我们要去.
- 在任何重量分配发生之前,检测和拒绝与明显错误的形状不匹配.
  中文翻译:在任何权重分配发生之前,检测和拒绝一个形状不匹配的错误.
- 生成加载权重的短续集,并确认代币来自加载分布,而不是随机初始化.
  中文翻译:生成加载权重的短续集,并确认代币来自加载分布,而不是随机初始化.

## 问题问题

> **【中文解读】**发布权重不是为你的架构打包的――它们使用原始实现的命名约定――相同参数出现三个微妙的身份――名称,形状,字节布局),载体必须协调三者――盲目复制会把正确的张量放到错误的位置,得到一个产生垃圾的模型――

> **【拓展：HuggingFace 权重加载生态】**拥抱脸的`transformers`库是权重负载的事实标准,支持200多种模型架构.`from_pretrained()`自动处理名称映射、形状检查、权重转置和d类型 转换──`safetensors`格式(本课使用) 比传统的更安全(不执行任意代码) 和更快(零拷贝加载) ・GGUF 格式(用于 llama.cpp) 支持量化权重,将7B 模型的内存占用从14GB 降至4GB。AutoGPTQ 和 AutoAWQ 提供训练后量化方案。它们携带原始实现使用的名称.预训练文件有`transformer.h.0.attn.c_attn.weight`形状`(2304, 768)`您的模型预计`blocks.0.attn.qkv.weight`形状`(2304, 768)`(在不同的布局会议中相同的矩阵) 或您的模型使用`nn.Linear`输入的矩阵存储.同一个参数显示出三个微妙不同的身份 (名称,形状,字节布局),加载器必须协调所有三个.

随着一个模块的复制,它将正确的子放在错误的地方,你得到了一个模型,它产生了无稽之谈.一个模块拒绝复制,但没有记录任何东西,让你猜测哪个子没有降落.`LoadReport`总结了击中,错失和形状不一致,

> 一个盲目复制的载体将正确的子放在错误的地方,你得到一个产生无稽之谈的模型.一个拒绝复制的载体,但没有记录任何东西,让你猜测哪个子没有降落.本课中的载体是明确的:每个任务都被记录,每个形状都被检查,`LoadReport`总结了击中,错失和形状不一致,
...
> 字符串的函数是从字符串到字符串. 形状检查是一个如果. 任务发生在内`torch.no_grad()`报告中包含了每个名字的结果.


### 基因二级命名公约

> **【中文解读】**发布的GPT-2 权重使用 `wte`现在,我们在这个地方.`wpe`现在,我们在这个地方.`h.N.attn.c_attn`其他类型的子:`c_attn`,我知道.`c_proj`,我知道.`c_fc`等线性层以转换形式存储`nn.Linear.weight`需要转移; 2) LM头 不在文件中模型通过`wte`权值绑定获取头
...
> 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读: 导读:
...
> **【中文解读】**构建权重载器的核心组件:名称映射函数(将预训名称转换为本地模型名称) ‧安全感器 读取器(遍历张量名称、映射、检查形状、转置 conv1d 权重) ‧载荷报告(记录命中、缺失和形状不匹配) ‧演示步骤:随机初始化模型 -> 生成续写 -> 加载权重 -> 再生成 -> 验证两次输出不同──

> **【拓展：权重加载在 LLM 工程中的实际场景】**实际工程中权重加载远比本课复杂:1)LoRA 权重合并(基_重量+lora_A @ lora_B);2)量化权重解量化(GPTQ/AWQ的规模和零_点);3)跨框架转换(PyTorch -> GGUF -> ONNX);4) 分片权重合并并(Megatron-LM的TP 分片需要重新排列) ─HuggingFace 的`transformers`库有200多个模型的加载脚本,每个都是一组名称映射规则.
```figure
cc-weight-remap
```
...
- 经历了这段经验.`GPTModel`所以这门课程是自封的.
  中文翻译:课程35的小副本`GPTModel`所以这门课程是自封的.
- `make_pretrained_to_local(num_layers)`通过此,我们可以扩大每个层次的输入.
  翻译: 中文`make_pretrained_to_local(num_layers)`通过此,我们可以扩大每个层次的输入.
- `load_safetensors(model, path)`它们可以进行代码,绘制它们,检查形状,转换 conv1d式的权重,并分配在`torch.no_grad()`返回一个`LoadReport`现在,我们要去.
  翻译: 中文`load_safetensors(model, path)`它们可以进行代码,绘制它们,检查形状,转换 conv1d式的权重,并分配在`torch.no_grad()`返回一个`LoadReport`现在,我们要去.
- `make_stub_safetensors(path, cfg)`它们可以生成一个具有精确预先训练的命名规则的固定文件.
  翻译: 中文`make_stub_safetensors(path, cfg)`它们可以生成一个具有精确预先训练的命名规则的固定文件.
- 创建一个演示`outputs/gpt2-stub.safetensors`在第一次运行时,构建一个新型模型,从随机 init生成的连续性录取,加载了,捕获了另一个连续性,打印了两者,并验证了两者是否不同 (负载实际上改变了模型).
  中文翻译:一个创建演示`outputs/gpt2-stub.safetensors`在第一次运行时,构建一个新型模型,从随机 init生成的连续性录取,加载了,捕获了另一个连续性,打印了两者,并验证了两者是否不同 (负载实际上改变了模型).
...
- `safetensors`对于磁盘格式和流媒体读器.
  翻译: 中文`safetensors`对于磁盘格式和流媒体读器.
- `torch`对于模型和任务数学.
  翻译: 中文`torch`对于模型和任务数学.
- 没有.`transformers`没有`huggingface_hub`没有网络通话.
  中文翻译:没有`transformers`没有`huggingface_hub`没有网络通话.

## 野生生产模式

> **【拓展：跨架构权重迁移】**加载器的名称映射模式是跨架构迁移的基础.`model.layers.N.self_attn.q_proj.weight`没有偏见,RMSNorm) 与GPT-2完全不同.Conv1D vs 直线的转换差异在GPT-J/GPT-NeoX 中也存在.
...
- 载体可以用于使用预训练命名公约的任何安全传感器文件.真正的GPT-2文件 (小/中/大/xl) 没有代码更改工作;只有模型配置不同.
  中文翻译:Loader 适用于使用预训练命名公约的任何安全传输器文件.真正的GPT-2文件 (小/中/大/xl) 没有代码变化工作;只有模型配置不同.
- 根据该规则,在使用该规则的基础上, 测量量量和测量量量量均保持相同.
  中文翻译:当你更新名字地图后,相同的模式延伸到LLaMA,Mistral,Qwen权重.
- 负载后的智能生成是一个快速的门户:如果负载后的样本看起来像负载前的样本,负载没有改变模型,这意味着地图静默错过了每个子.
  中文翻译:加载后的清洁生成是一个快速的门户:如果加载后的样本看起来像加载前的样本,则负载没有改变模型,这意味着地图将默默错过了每个数.
...
- 阶段19课 35 对于接收重量的建筑.
  中文翻译:第19阶段的课程35:为接收重量的建筑.
- 训练循环的第19阶段课程36:
  中文翻译:第19阶段课程36为训练循环,产生相同形状的检查点.
- 阶段10课时,我们将对记忆紧张时的载重进行量化.
  中文翻译:第十阶段第十一课 (量化) 记忆紧张时,如何处理负载重量.
- 第十阶段课程13 (构建一个完整的LLM管道)
  中文翻译:第十阶段课程13 (构建一个完整的LLM管道) 围绕负载和推断的整个生命周期.
