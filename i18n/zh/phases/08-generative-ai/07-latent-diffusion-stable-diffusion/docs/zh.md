...
> 像素空间扩散在512x512图像上是一种计算战争罪行.Rombach等人 (2022) 注意到,你不需要所有的786k维度来生成图像.你需要足够的 capturing语义结构,和一个独立的解码器.运行扩散在一个VAE的隐藏空间.

> **【中文解读】**在 512x512 像素空间做扩散是计算灾难.罗姆巴赫等发现不需要全部的 78.6万维度只需要捕获语义结构,剩余的使用解码器补充.

> **【拓展：Stable Diffusion 的革命】**稳定扩散将从像素空间转移到潜在空间的扩散过程,计算量减少了数十倍,使消费级GPU能够运行.
...
> 像素空间扩散意味着U-Net在`[B, 3, 512, 512]`张量上运行――每采样步约100GFLOPS――50步就是5 TFLOPS――在十亿图像上训练计算成本荒谬――

罗姆巴赫的想法是:训练一个VAE一次 (第一个阶段*),结它,并在4道64×64潜伏空间 (第二阶段*) 完全运行扩散.同样的U-Net.1/16的像素. ~64x少的FLOPs以获得相似的质量.

> 大部分FLOPs 用于推过感知上不重要的细节.Rombach的想法:训练一次VAE (第一阶段),结结它,完全在4通道64×64 潜在空间运行扩散 (第二阶段) ⋅相同的U-网──1/16 像素──约64倍更少的FLOPs──
...
> 这就是稳定扩散配方.SD 1.x/2.x 用860M U-Net 在 64×64×4 上,SDXL 用 2.6B U-Net 在 128×128×4 上,SD3 用 DiT + 流量匹配 替换 U-Net──Flux.1-dev 用 12B MMDiT──都运行在相同的两个阶段基础上──

> **【中文解读】**稳定扩散的核心架构是两阶段设计: 1) 第一阶段VAE编码器将512x512图像压缩到64x64x4潜在空间; 2) 第二阶段在潜在空间中运行扩散过程――U-Net 在64x64张量上运行,计算量降低约64倍――从SD 1.x到SD3的演变:U-Net → DiT(Diffusion Transformer),DDPM →流量匹配――

> **【拓展：从 U-Net 到 DiT 的架构变迁】**SD 1.x/2.x 使用U-Net 作为去噪网络──SD3(2024) 和FLUX 转向DiT(Diffusion Transformer) 使用Transformer 替代U-Net──DiT的优势在于扩展性更好──Transformer的缩放定律适用) 支持更高分辨率──可以更好地融合文本条件──这一结构与NLP领域的Transformer 统一趋势一致.
...
1. **Stage 1 — VAE.**编码器`E(x) → z`解码器`D(z) → x`目标压缩:每个空间轴的下样本8倍 +调整道,使得潜伏总量为像素数量的1/16分之一.`z`由于我们不需要精确的样本`z`经常被训练,以对抗的损失,所以解码的图像是敏的.
   ...
   **阶段 1 — VAE。**编码器`E(x) → z`解码器`D(z) → x`△目标压缩:每空间轴 8倍下采样──损失 = 重建(L1 + LPIPS) + KL(小权重)──

2. **Stage 2 — diffusion on `z`.**治疗`z = E(x_real)`训练一个U-Net (或DIT) 进行击`z_t`结果: 样本`z_0`通过传播,然后`x = D(z_0)`现在,我们要去.

   **阶段 2 — 在 `z` 上扩散。**将`z = E(x_real)`视为数据. 训练 U-Net.`z_0`然后`x = D(z_0)`,我知道.
...
> **文本条件化。**两个额外组件:结文本编码器和交叉注意注入.`[Q = 图像特征, K = V = 文本 token]`让交叉注意力. 标记是文本影响图像的唯一方式.
...
> 趋势:使用DiT 替代U-Net,扩展文本编码器(T5 在快速遵循上优于CLIP),增加潜在通道(4→16 给更多细节余量) 👇
...
> `code/main.py`在第六课的DDPM上叠加了一个玩具1D"VAE"并添加了类型条件无分类器导向. 它显示了无论在原始值还是编码值上都有有效的扩散损失.
...
> 实际上,这种线性映射足以显示在`z`上操作不关心原始数据空间.

### 步骤2: 扩散在`z`- 空间

网络看到的数据是`z = E(x)`在抽样后`z_0`解码`D(z_0)`现在,我们要去.

> 网络看到的数据是与第06课相同的DDPM`z = E(x)`采样`z_0`后用`D(z_0)`解码.
...
> 用结文本编码器输出替换类别标签.通过交叉注意力将文本嵌入送入U-Net:
...
- **Negative prompts leaking.**空负提示变成零符号;填写的负提示变成了 `ε_uncond`它们不同,有些管道默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默默
  **负向 prompt 泄漏。**空负向即时变为零代币;填充的变为`ε_uncond`两者不同.
...
> 保存`outputs/skill-sd-prompter.md`△ 技能 接收文本提示+目标风格,输出模型+检查点、CFG、采样器、负向提示等──

## 练习题

1. **Easy / 简单.**跑步`code/main.py`带着指导`w ∈ {0, 1, 3, 7, 15}`按类别记录平均样本.`w`类型的意思是否超出了实际数据的意思?
   用`w ∈ {0, 1, 3, 7, 15}`运行.`w`类别平均值与真实数据平均值相比?
2. **Medium / 中等.**换取一个TANH-MLP编码器/解码器对,重建损失.重新训练扩散新潜伏.样品质量有没有改变?
   玩具线性编码器将替换为tanh-MLP编码器/解码器.
3. **Hard / 困难.**设置一个真正的稳定射推理,使用射器:负载 `sdxl-base`现在切换到 ,然后再切换到`sdxl-turbo`描述了什么发生了变化以及为什么.
   用扩散器 搭建真实SD推理,比较SDXL-base 和SDXL-Turbo──
...
## 产品注释:运行Flux-12B在8GB消费GPU上

参考流程集成是"我有一个消费者GPU,我可以运送吗?"的正规配方.

> 参考流动集成是经典的"我只有消费级GPU,能部署吗?"方案.
...
1. **Staggered loading.**流程有三个网络,在VRAM中永远不需要共存:T5-XXL文本编码器 (fp32中约10GB),CLIP-L (小),12BMMDiT和VAE.首先编码提示, *删除*编码器,加载diT,删除, *删除*diT,加载VAE,解码.消费者8GBGPU只适用于一次一个阶段.
   **交错加载。**流有三个不需要同时留在VRAM的网络──编码提示 后删除编码器,加载DiT,去噪声后删除,加载VAE 解码──
