...
> **【中文解读】**3D是2D到3D桥梁最强的模式――2023年突破是3D高斯人分布,2024-2026年的生成趋势是将多视角扩散+3D重建叠加,从单个提示词或照片生成3D物体和场景――

> **【拓展：3D 生成的应用】**3D 生成用于游戏资产创建、VR/AR 内容、建筑设计、电商产品展示──DreamGaussian、TripoSR等模型可以在秒级从文字/图片生成 3D模型──
...
- **Data scarcity.**图像网拥有1400万张图像.最大的清洁3D数据集 (Objaverse-XL, 2023) 拥有约1000万个物体,质量最低.
  **数据稀缺。**图像网有1400万图像. 最清洁的3D数据集约1000个物体,大多数质量低.
- **Memory.**5123 声格格为 128M 声格;一个有用的场景 NeRF 需要 1M 样本/射线.生成比重建更难.
  **内存。**产量比重建更难.
- **Supervision.**在二维图像中,你有像素,在三维图像中,你通常有几种二维图像,
  **监督。**两维图像有像素──三维通常只有少量二维图像──

首先,用扩散模型生成2D多视图,第二,将3D表示 (通常是高斯的) 合到这些图像.

> 2026年方案分为两步:先使用扩散模型生成*2D 多视角图像*,再使用*3D表示*(通常是高射) 拟合──

> **【中文解读】**3D 生成的核心挑战:表示方式多样性(网格、点云、体素、SDF、NeRF、3D 高斯) 、数据稀缺性(远低于2D图像) 、内存需求巨大、监督信号弱 (通常只有2D视图) ⋅2026年的主流方案是两阶段策略:先使用扩散模型生成多视角2D图像,再使用3D 高斯射(Gaussian Splating) 拟合3D表示──

> **【拓展：3D Gaussian Splatting 的革命性影响】**3DGS(2023) 取代NeRF 成为3D重建的主流方法──它使用数百万个3D高球体表示场景,染色速度快100倍以上,质量相当或更优质──结合多视角扩散模型,3DGS使"从文字/图片生成可互动的3D场景"成为可能,在游戏、VR/AR建筑可视化中存在巨大应用前景──
