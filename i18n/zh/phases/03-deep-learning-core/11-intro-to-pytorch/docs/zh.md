...
> **【中文解读】**你从零构建了神经网络的所有组件――现在学真正在使用的框架:PyTorch――本章对应关系:你的价值类 →火.ensor,你的后退() →损失.后退(),你的 SGD/Adam →火.最佳.SGD/AdamW──
...
> **【中文解读】**本章从迷你框架过渡到Pytorch──核心对应关系:模块 → nn.Module、手写向后() →自动化、Python 循环 →GPU 并行──你已经在第十课中理解了底层原理,现在学习工业级实现──

## 问题 问题引入

您有一个工作的迷你框架.线性层,ReLU,脱落,批量标准,亚当,数据加载器,训练循环.它训练一个四层网络在纯Python中循环分类问题.

> 你有一个可用的迷你框架――线性层、ReLU、落地、批量归化、亚当、数据载体、训练循环――它使用纯 Python 在圆形分类问题上训练一个四层网络――

对于同样的问题,它也比PyTorch慢500倍.

> 但它比PyTorch在同样的问题上慢500倍.
...
> 你的迷你框架使用嵌套的Python循环个别处理样本.PyTorch将相同的操作分派到GPU上运行的优化C++/CUDA内核.在单个NVIDIA A100上,PyTorch在ImageNet上训练ResNet-50(2560万参数) 需要大约6小时.你的框架在同一任务上需要大约3000小时.如果它不先使用内存的话.

速度不是唯一的差距.你的框架没有GPU支持.没有自动区分 - - 你手写向后的每个模块.没有序列化.没有分布式训练.没有混合精度.没有方法去调整梯度流程没有打印声明.

> 速度不是唯一的差距――你的框架没有GPU支持――没有自动微分你为每个模块手写回来了()――没有序列化――没有分布式训练――没有混合精度――没有不用打印语句就能调试梯度流的方法――

PyTorch填补了这些空白.它保持了你已经构建的相同的心理模型:模块,前(),参数(),向后),优化.步骤().概念将一个接一个转移.语法几乎是一样的.区别是 PyTorch 包裹了你从零开始设计的相同界面后的十年系统工程.

> 皮托尔奇 填补了所有这些差距.而且它保持与你已经构建的完全相同的心智模型:模块,前,参数,后,优化器.步骤.

> **【中文解读】**你的迷你框架比PyTorch慢500倍因为PyTorch循环个别处理样本,而PyTorch使用C++/CUDA内核并行处理.A100上训练ResNet-50只需要6小时,你的框架需要3000小时.

> **【拓展：PyTorch 为什么赢了 TensorFlow】**2017年PyTorch发布时,TensorFlow占据了80%的市场份额――但PyTorch的热情执行(立即执行)让调试和原型开发远远于TF 1.x的静态图简单――到2022年,PyTorch在ML研究论文中的份额超过75%――Meta、OpenAI、Anthropic、HuggingFace 全部使用PyTorch──教训:开发者体验比绝对性能更重要――

## 概念的核心概念

## 概念的核心概念

### 为什么Pytorch赢了Pytorch为什么赢了

在2015年, TensorFlow要求你在运行任何东西之前定义静态计算图.你构建图,编译它,然后通过它输送数据.调整意味着着图的可视化.改变架构意味着从零开始重建图.

> 2015年,TensorFlow 要求你在运行任何东西之前定义静态计算图――你构建图――编译它――然后通过它输入数据――调试意味着图可视化――改变架构意味着从头重新构建图――

鱼在2017年推出了不同的理念:渴望执行.你写Python.它立即运行.`y = model(x)`这意味着标准的Python调试工具工作.打印() 工作. pdb工作.如果/else 在你的前进通行工作.

> 皮托尔奇在2017年推出,采用了不同的哲学:即时执行――你写Python――它立即运行――`y = model(x)`现在就计算 y,而不是"添加一个稍后计算 y 的节点到图中"――这意味着标准的Python调试工具可用――打印() 可用――pdb可用――前进通过 中的如果/否可用――
...
> 到2020年,市场已经给出了答案.在ML研究论文中PyTorch的份额从7% (在2017年) 增长到75%以上.

开发人员经验组合.一个软件框架每次都会有10%的速度慢,但50%的速度更快.

> 培训:开发者体验会累积――一个慢10%但调试快50%的框架每次都会赢――
...
> **Shape**是维度信息──标量的形状是 (),向量是 (n,),矩阵是 (m,n),一批图像是 (批量,道,高度,宽度)──
...
> 每个操作都需要所有的张量在同一设备上. 这是初学者遇到的第一大PyTorch错误:`RuntimeError: Expected all tensors to be on the same device`△在计算前将所有内容转移到同一设备即可修复.

**Reshaping**它改变了元数据,而不是数据.

> **重塑**是常数时间操作,它改变了元数据,不会改变数据.
...
> 你的迷你框架要求你实现每一个模块向后 (() ・PyTorch 不需要――它将张量上的每个操作记录到有向无环图 ((计算图) 中,然后反向遍历该图自动计算梯度――
...
> 与你的框架的关键区别:PyTorch 使用基于磁带的自动微分――前向传播期间,每个操作都增加到一个"磁带"上――调用`.backward()`转向重放磁带
...
1. 只有叶子子`requires_grad=True`积累梯度
   中文翻译:只有设置了`requires_grad=True`叶张量才会积累梯度
2. 基准默认积累 - 调用`optimizer.zero_grad()`在每次倒退前
   中文翻译:梯度默认累积每次反向传播前调用 `optimizer.zero_grad()`
3. `torch.no_grad()`禁用梯度跟踪 (评估期间使用)
   翻译: 中文`torch.no_grad()`禁用梯度追踪 (查)

> **【拓展：混合精度训练如何加速】**射量是射32的 2-4 倍──PyTorch 的`torch.amp.autocast`自动将矩阵乘法和卷积转为浮动16 ,同时保持软max和损失在浮动32──配合 GradScaler 防止浮动16 梯度下溢──Llama 3 的训练全程使用 bfloat16 混合精度,节省约50%的显存和计算──

### 网络网络模块

`nn.Module`在PyTorch中,每个神经网络组件的基类.你已经在10课程中构建了这个抽象.PyTorch的版本添加了自动参数注册,递归模块发现,设备管理和状态命令序列化.

> `nn.Module`您已经在第10课中构建了这个抽象. PyTorch的版本增加了自动参数注册,递归模块发现,设备管理和状态命令序列化.
...
> 当你在`__init__`中将 `nn.Module`或`nn.Parameter`作为属性时,PyTorch自动注册它.`model.parameters()`返回收集每个注册参数. 这就是为什么你永远不需要像迷你框架中的手动收集权力.
...
> 注意:`CrossEntropyLoss`内部组合了`LogSoftmax`其他`NLLLoss`传入原始逻辑,不要传入软max 输出.
...
> 批量循环内五行代码――训练了GPT-4、稳定分散和LLaMA的五行代码――架构会变化――数据会变化―― 这五行不变――

### 数据集和数据载体

皮托尔奇的`Dataset`是一个具有两个方法的抽象类: `__len__`其他`__getitem__`现在,我们要去.`DataLoader`通过批量,混动和多个进程数据加载.

> 皮托尔奇的`Dataset`是一个抽象类型,有两个方法:`__len__`和 `__getitem__`,我知道.`DataLoader`用批量处理,打乱和多个过程数据加载包装它.
...
> `num_workers=4`产生4个进程并行加载数据,同时GPU在当前批次上训练.
...
> **【中文解读】**下面使用纯PyTorch 训练一个3层MLP做MNIST 手写数字分类(784→256→128→10) 参数只有235K,但训练模式和大模型完全一致:DataLoader →模型.火车() →零_级 →前进 →后退 →步 →模型.eval() △10个时代 达到 ~97.8% 测试准确率──
```figure
dropout-mask
```
...
> 用纯PyTorch 原语训练的3层MLP做MNIST 分类――没有高级封装――没有`torchvision.datasets`我们自己下载和分析原始数据.

### 步骤1:从原始文件中下载MNIST

MNIST 作为4个gzipped文件:训练图像 (60,000 x 28 x 28),训练标签,测试图像 (10,000 x 28 x 28),测试标签.我们下载它们并分析二进制格式.

> MNIST 以4个 gzip 文件提供:训练图像(60,000 x 28 x 28) ✓训练标签、测试图像(10,000 x 28 x 28) ✓测试标签──我们下载它们并解析二进制格式──
...
> 一个3层MLP:784 -> 256 -> 128 -> 10。ReLU 激活──Dropout 正则化──为简单起见不用 BatchNorm──
...
> 输出层产生10个原始的逻辑 (每个数字一个) ⋅不需要软max`CrossEntropyLoss`内部处理

参数数:784*256+256+256+256*128+128+128+128*10+10=235,146. 根据现代标准,GPT-2小小有124M. 这在几秒钟内运行.

> 参数:784*256+256+256+256*128+128+128+128*10+10=235.146──根据现代标准很小──GPT-2小 有124M──这个在几秒内就能训练完毕──
...
> 注意评估时的`torch.no_grad()`没有它,PyTorch会构建你永远不会使用的计算图片.
...
> 后期预期输出:~97.8% 测试准确率──CPU 训练时间:~30秒──GPU:~5秒──用迷你框架相同架构:~45分钟──

> **【拓展：从 MNIST 到大模型】**MNIST MLP 只有235K 参数――现代模型的规模:GPT-2小 124M、BERT-基础110M、Llama 3 8B──参数增长 ~1000x,但训练循环的五步模式完全不变──区别在于:数据并行(多 GPU)、模型并行(单 GPU 放不下)、梯度检查点(节省显存)、混合精度(加速计算)──这些都是 PyTorch 生态系统的一部分──

## 用它实现框架

> **【中文解读】**迷你框架和 PyTorch 的接口几乎一致──关键区别:PyTorch 用自动微分自动分(不需要手写向后((、支持GPU(模型.to("cuda")) 、支持混合精度训练──保存模型使用状态_定位(((可移植的参数典),不要直接化模型对象──
...
> 始终保存`state_dict()`保存模型对象使用,重构代码时会破坏.
...
> 皮托尔奇提供15种调度器:StepLR、ExponentialLR、CosineAnnealingLR、OneCycleLR、ReduceLROnPlateau──所有插入相同的优化器接口──
...
- `outputs/prompt-pytorch-debugger.md`-- 诊断常见的 PyTorch 训练失败的提示
  翻译: 中文`outputs/prompt-pytorch-debugger.md`- 诊断常见 PyTorch 训练故障的提示词
- `outputs/skill-pytorch-patterns.md`-- PyTorch培训模式的技能参考
  翻译: 中文`outputs/skill-pytorch-patterns.md`- 皮托尔奇 训练模式的技能参考
