...
> **【中文解读】**没有非线性激活函数,100 层网络等于一个矩阵乘法. 因为两个线性变换的复合还是线性:W2(W1x+b1)+b2 = (W2W1)x+ (W2b1+b2);;激活函数打破这种线性叠加,让每个层都能为网络增加真正的表达能力.
...
> **【中文解读】**本章目标:实现7种激活函数及其导数,通过实验诊断梯度消失问题,检测RLU中死亡神经元,学会为不同架构选择合适的激活函数.
...
> 这不是理论上的好奇. 这意味着深线性网络实际上无法学习XOR,无法分类螺旋数据集,无法识别人的脸.
...
> 激活函数打破线性――它们通过非线性函数扭曲每个层次的输出,赋予网络曲决策边界、接近任意函数并真正学习的能力――但选择错误激活函数,梯度会消失为零(深层网络中的 sigmoid) ‧爆炸到无穷大 ((没有仔细初始化的无限激活),或神经永久死亡(具有大负偏偏置的 ReLU) ‧ 激活函数的选择直接决定你的网络是否能够学习――
...
> 矩阵乘法是可组合的. 首先使用矩阵A乘向量,再使用矩阵B乘,等于使用AB乘. 这意味着堆积十个线性层在数学上等于一个具有一个大矩阵的线性层. 所有这些参数,所有这些深度都浪费了.
...
> 现在代入被打破了──W2 * g(W1 * x + b1) + b2 无法简化为单一线性变化──网络可以表示非线性函数──每个带激活函数的附加层都增加了表示能力──

> **【中文解读】**数学证明:两个线性变化的复合还是线性.但是插入非线性激活g) 后,W2 *g) W1x + b1) + b2 无法合并为单一矩阵每多个带激活的层,网络的表达能力就真正增加了.
...
> 该导数最大值为0.25,现在x=0时.在反向传播中,梯度在层间相乘.
...
> 网络似乎在学习中后层的损失正在下降,但前层已经结.深度sigmoid网络根本无法训练.

另外一个问题:sigmoid输出总是正 (0到 1),这意味着重量上的梯度总是相同的标志.

> 额外问题:sigmoid 输出总是正数 ((0 到 1),这意味着权重的梯度总是同号――这导致梯度下降呈形路径――

> **【中文解读】**另外,Sigmoid 输出总是正数 ((0到1),导致权重梯度同号,优化路径呈形――

> **【拓展：Sigmoid 在现代 AI 中的位置】**虽然Sigmoid 不再用于隐藏层,但在二分类输出层中仍然常用.
...
> 导数最大为1.0 (在 x = 0 时) 比西格莫ид好4倍.但梯度消失问题仍然存在.对于大正负输入,导数趋近零.

> **【中文解读】**是sigmoid的零中心版本,输出范围 (-1, 1),导数最大值 1.0(比sigmoid 好 4 倍) ⋅但大输入时导数仍然接近零,梯度消失问题仍然存在,只是没有那么严重──

> **【拓展：LSTM 中的 Tanh】**虽然变压器已经基本取代了LSTM,但理解tanh对于理解RNN系列模型很重要.

### 突破:深度学习突破

修改线性单元. 2010 年由纳尔和希顿推广为深度学习 (该功能本身可以追溯到福岛1969年的作品),它改变了一切.

> 修正线性单元──由Nair 和 Hinton在2010年推广用于深度学习.
...
> 正输入没有梯度消失问题.梯度正确是1,直接传递.

但有一个失败模式:死神经元问题.如果神经元的权重输入总是负面 (由于大负偏差或不幸的权重初始化),其输出总是零,其梯度总是零,它永远不会更新.它永久死亡.实际上,ReLU网络中的10-40%的神经元可以在训练中死亡.

> 但有一个失败模式:死亡神经问题――如果某个神经的加权输入始终为负 (由于大负偏置或不幸运输的权重初始化),其输出始终为零,梯度始终为零,永远不会更新――它永久死亡――在实践中,ReLU 网络中10%-40%的神经元可能在训练期间死亡――

> **【中文解读】**对于正确输入梯度恒为1,完全不衰退这是深度网络变得可训练的原因.但它有"死亡神经"问题:如果某个神经元的加权输入始终负,它永远输出0、梯度为0,永远无法恢复――实践中,10-40%的REL神经元可能死亡――

> **【拓展：ReLU 在 CNN 中的统治地位】**网络的结构都使用 ReLU (或其变体) 了.CNN 中的卷积层+ ReLU 是提取视觉特征的标准组合.
...
> 其中,alpha是小常数,通常为0.01──负侧有一个小斜率而不是零,因此死亡神经仍然可以获得梯度信号并可以恢复──

> **【中文解读】**由于漏的RLU在负区间保持一个小的斜率,让死亡神经仍然能收到梯度信号,有可能恢复.

### 现在,我们在这个世界里,

盖斯错误线性单位. 于2016年由亨德里克斯和吉普尔推出. 在BERT,GPT和大多数现代变压器中默认激活.

> 高斯差差线性单元──由亨德里克斯 和吉普尔 于2016年提出──BERT、GPT 和大多数现代变压器的默认激活函数──
...
>  GELU处处平滑,允许小的负值(不像 ReLU 硬截断为零),具有概率解释:按输入在正态分布下为正的概率对每个输入加权.

> **【中文解读】**GELU 是BERT、GPT 和大多数现代变压器的默认激活函数――它处于平滑、允许小的负值(不像 ReLU 硬截分为 0),概率解释是:按输入为正的概率加权──平滑的门控机制优于 ReLU,因为它提供更好的梯度流量并完全避免死亡神经问题──

> **【拓展：GPT/BERT 中的 GELU】**在变压器的FFN中,标准结构是`Linear → GELU → Linear`鱼的`nn.GELU()`和 `F.gelu()`这就是这个函数――GPT-2/3/4、BERT、ROBERTA等模型都使用GELU──
...
> **【中文解读】**通过自动搜索发现 (使用神经网络设计神经网络的一部分) 和 GELU 性能几乎相同,微分是 Swish 用 sigmoid 门控、GELU 用高斯 CDF 门控──Swish 用 EfficientNet 等视觉模型,GELU 统治语言模型──
...
> **【中文解读】**软max 不用于隐藏层,而是输出层把原始分数的) 转化为概率分布.所有输出在0-1之间和总和为1之间.最大分数得到最高概率,但与 argmax 不一样,软max可导并保留可信信息.

> **【拓展：Softmax 在 Transformer 中无处不在】**变压器的自注意力机制使用软max 计算注意力权重:`attention = softmax(Q·K^T / sqrt(d_k))`,每层注意力都不开放,
...
> **【中文解读】**经验法则:变压器/NLP 用GELU,CNN/视觉用ReLU,RNN/LSTM 用tanh──输出层:二分类用sigmoid,可能类用softmax,回归不用激活──
...
> **【中文解读】**PyTorch 提供了所有激活函数的函数式和模块式 API。经验法则:变压器 隐藏层使用 GELU,CNN 隐藏层使用 ReLU,分类输出使用 softmax,回归输出不需要激活──如果 ReLU 神经元死亡,换 GELU(而不是泄漏 ReLU)。
