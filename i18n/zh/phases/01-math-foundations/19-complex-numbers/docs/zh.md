...
- 进行复杂算术 (加,乘,分,结合) 在矩形和极形两种形式
  执行复数运算 (加乘除共),包括直角坐标和极坐标形式
- 应用尤勒公式来转换复杂指数和三角函数
  应用欧拉公式在复指数和三角函数之间转换
- 通过复杂的团结根实现分离福利尔变形
  使用单位根实现离散 里叶变换 (DFT)
- 解释转换器中的复杂旋转是如何构成ROPE和鼻状位置编码的基础
  解释复数旋转如何构建ROPE和变压器 正弦位置编码的基础


> **【中文解读】**
> 虚数 i 是旋转和频率的关键.变压器中的ROPE位置编码.

## 问题 问题引入

你打开一篇关于福利尔转换的论文,`i`它们是的,它们是的.`sin`其他`cos`它们在不同的频率上,它们是复杂指数的真实和虚构部分.

> 你打开一篇关于里叶变化的论文,`i`你看变压器位置编码,看不同频率的`sin`和 `cos`它们是复数指数的实部分和虚部分.

复杂数字似乎是抽象的.一个基于 -1 的平方根构建的数量系统感觉像是一个数学技巧.但它不是一个技巧.它是旋转和振荡的自然语言.每当某种东西旋转,振动或振荡时,复杂数字都是正确的工具.

> 复数看起来很抽象――建立在 -1 的平方根上,数系感觉就像数学戏一样――但它不是戏――它是旋转和振荡的自然语言――每当某物旋转、振动或振荡时,复数是正确的工具――

没有理解复杂数字,你无法理解分离福利尔转换.你无法理解FFT.你无法理解现代语言模型中ROPE (旋转位置嵌入) 如何工作.你无法理解原始转换器纸中的鼻状位置编码为什么使用它们的频率.

> 不理解复数,你就无法理解离散里叶变换 (DFT) ――不能理解 FFT――不能理解 RoPE (旋转位置编码) 在现代语言模型中如何工作――不能理解原始变压器论文中的正弦位置编码为什么使用这些频率――

这一课从零开始构建复杂的算法,将其连接到几何,

> 本课程从零构建复数运算,将其与几何联系起来,并精确展示复数在机器学习中出现的位置.

## 概念的核心概念

> **【中文解读】**
> 复数的核心洞察:i 不是"虚"的,它是一个90度旋转操作.

> **【拓展：复数在 AI 中的实际应用规模】**
> 对于LLaMA-2-70B的80个注意力头,序列长度4096),每步推理执行数百万次复数旋转――音频 AI领域 (如OpenAI Whisper) 依赖于FFT (快速里叶变化),所有计算都在复数域完成――

### 复杂数是什么?

复杂数有两个部分:一个真实部分和一个虚构部分.

> 复数有两个部分:实部 (真实部) 和虚部 (虚部)
...
> 这就是这样. 你把数轴扩展到一个平面. 实数在一个轴上. 虚数在另一个轴上. 每个复数都是这个平面上的一个点.
...
> 复数同时是从原点发出的点和向量. 这种双重解释使复数在几何中非常有用.

### 极形

任何平面的点都可以通过距离原点和距离正实际轴的角度来描述.

> 平面中的任何点都可以用其距离到原点和正轴的角度来描述.
...
> 欧拉公式说`e^(i*theta)`随着太太变化描绘单位圆――太 = 0 时在 (1, 0) ・太 = pi/2 时在 (0, 1) ・太 = pi 时在 (-1, 0) ・太 = 3*pi/2 时在 (0, -1) ・完整旋转是太 = 2*pi。

这意味着复杂的指数是旋转.

> 这意味着复指数就是旋转.旋转在信号处理和ML中无处不在.

> **【中文解读】**
> 欧拉公式e^(i*theta) = cos(theta) + i*sin(theta) 是本课中最重要的公式――它把指数函数和三角函数统一起来,也把复数和旋转统一起来――当theta从0变到2*pi,e^(i*theta) 在复平面上画出一个单位圆――这个圆是理解DFT、RoPE、信号处理的基础――

### 连接到2D旋转

乘以e^(i*theta) 乘以复杂数 (x + yi) 旋转点 (x,y) 通过角度围绕原点.

> 将复数 (x + yi) 乘以 e^(i*theta) 将点 (x, y) 绕原点旋转角的──
...
> 复指数 e^(i*omega*t) 是一个以角频的omega 绕单位圆旋转的点──随着t 增加,点描绘出圆──
...
> 这就是相量 (Phasor) 表示――不需要追踪起伏的正弦波,而是追踪平滑旋转的箭头――相位偏移变成角度偏移――幅度变化变成模的变化――信号相加变成向量相加――

### 团结的根源

单元圈上均等间隔的N点是单元圈的N-th根:

> 单位根 (N 个根) 是单位圆上等间距分布的N 个点:
...
> 单位根是离散里叶变化的基础.DFT将信号分解为这个N个等间距频率的分量.

> **【中文解读】**
> 单位根是N个等间距分布在单位圆上的点.它们的两个奇特性: 1) 每个模都恰好为 1; 2) 全部加起来恰好为零.
...
> 每个X[k] 测量信号与第一个k个单位根的相关性频率为 k的复正弦波──DFT将信号分解为N个旋转相量,告诉你每个的幅度和相位──

### 我为什么不想象

想象的词是历史上的意外.德卡特用它以轻视的方式.但我不比人们第一次拒绝它们时的负数更想象的.负数回答"从3中减去5是什么?"想象的单位回答"你方方数是什么,得到 -1?"

> "虚"这个说法是历史的偶然――德斯卡特用它来低――但我并没有比负数最初被人拒绝时更"虚"――负数回答"3减5得到什么"――虚数单位回答"什么平方得到 -1"――

更多有用的是:i是一个90度旋转运算符.乘以i一个实数,你将90度旋转到想象轴.再乘以i (i^2),你将再旋转90度 - - 现在你指向负实方向.这就是为什么i^2=−1.这不是神秘的.这是一个半旋转,由两个四旋转构成.

> 更有用的理解:i 是一个90度旋转算子――将实数乘以i 一次,你旋转90度到虚轴――再乘以i(i^2),再旋转90度现在向负实方向――这就是为什么i^2 = -1――这不神秘――它是两个四分之一转构成的半转――

因此,复杂数字在工程中到处存在.任何旋转的东西--电磁波,量子状态,信号振荡,定位编码--自然被复杂数字描述.

> 这就是为什么复数在工程中无处不在的原因.任何旋转的东西都自然使用复数描述.

### 复杂指数与三角形函数

在艾勒公式之前,工程师写出信号为A*cos(omega*t + phi) - 幅A,频率omega,相 phi.这有效,但使算术痛苦.添加两个不同相的共数需要三角形识别.

> 在欧拉公式之前,工程师将信号写成A*cos(omega*t + phi) 幅度A、频率 omega、相位phi──这可行,但让算术很痛苦──相加两个不同相位的余弦需要三角恒等式──

复杂指数,同一个信号是A*e^(i*(omega*t + phi)). 添加两个信号只是添加两个复杂数字. 乘法 (调制)只是乘法大小和添加角. 阶段转变成为角度加增. 频率转变成为相数乘法.

> 使用复指数,同样的信号是A*e^(i*(omega*t + phi))。两个信号相加就是两个复数相加――相乘(调制)就是模相乘、角相加――相位偏移变成角加法――频率偏移变成乘相量――

整个信号处理领域都转向复杂的指数符号,因为数学更清洁. "真信号"总是复杂表示的真实部分.想象中的部分作为会计,使所有的代数自然运行.

> 整个信号处理领域转向复指数表示法,因为数学更简洁――"真实信号"总是复数表示的实部――虚部作为账记被保留,使所有代数运算自然成立――

> **【拓展：复数在量子计算中的角色】**
> 量子计算的基础态是复数向量――一个量子比特的状态是alpha 进入0> + 贝塔 进入1>,其中, 亚尔法^2 + 贝塔 进入^2 = 1,alpha 和 贝塔 都是复数――量子门是复数矩阵――谷歌的 Sycamore 量子芯片有53个量子比特,其状态向量有2^53个复数分分――量子机器学习(QML) 直接在复数空间中操作――
...
> 和因对是不同频率复数指数的实部和虚部. 每个频率为位置编码提供不同的"分辨率".

**RoPE (Rotary Position Embedding)**通过复杂的轮换矩阵,它明确乘以查询和关键向量.两个代币之间的相对位置变成了旋转角.注意力是使用这些旋转向量计算的,使模型通过复杂的乘法对相对位置敏感.

> **RoPE（旋转位置编码）**进一步――它显然将查询和关键向量乘以复数旋矩阵――两个符号之间的相对位置变成旋转角――注意使用这些旋转后的向量计算,使模型通过复数乘法对相对位置敏感――

> **【拓展：RoPE 在 LLaMA 和 GPT-NeoX 中的实现】**
> 对于维度d=4096、序列长度L=4096的模型,RoPE对每对 (q,k) 执行L^2/2次复数乘法――与绝对位置编码相比,RoPE的优势是相对位置的旋转角仅取决于位置差异,这自然支持"外推"训练时见过2048 长度,推理可以扩展到更长序列――
...
> 构建复数类,支持算术运算、模、相位以及直角坐标和极坐标之间的转换.
...
> 这是O(N^2) 的 DFT──每个输出 X[k] 是信号样本乘以单位根之和──

### 步骤5:反向DFT

逆 DFT 从其频谱中重建原始信号.从前面 DFT 发生的唯一变化是:将标志翻转在指数中并乘以N.

> 与正向DFT的唯一区别:翻转指数符号并除以N。
...
> 这给你完美重建. 应用DFT,再应用IDFT,你用机器精度恢复原始信号.
...
> 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量 字面量`j`表示虚数单位――
...
> 运行`code/complex_numbers.py`生成 `outputs/skill-complex-arithmetic.md`经验人员:
...
> 术语速查:复杂数量(复数a+bi) 、想象单位(虚数单位 i,旋算子) 、复杂平面(复平面) 、大小/模块(模 √(a2+b2)) 、阶段/论点(相位 atan2(b,a)) 连接(共 a-bi) 、极坐标r·e^iθ、、欧勒公式e^iθ) =θ+i·sinθ) 、Phasor(旋相量) 、 统一单位的根点 N 个均分单位圆点) 、DFT散离里叶变换) 、旋转位置编码,L(RocosPE 等模型使用.
