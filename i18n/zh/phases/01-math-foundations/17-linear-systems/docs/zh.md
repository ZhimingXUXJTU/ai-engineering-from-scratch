...
- 通过部分旋转和后置取代的高斯式消除来解决 Ax = b
  使用带部分主元选取 (部分转换) 的高斯消元法和回代法求解 Ax = b
- 含 LU,QR和Cholesky分解的因子矩阵,并解释每一个是什么时候合适的
  使用 LU、QR 和 Cholesky 分解(分解) 分解矩阵,并解释各方法的适用场景
- 求最小方体的正常方程并将它们连接到线性和脊柱回归
  推导最小二乘法正规方程 (正规方程) 并将其与线性回归和回归联系起来
- 通过条件号码诊断出不良系统,并进行规律化来稳定它们
  使用条件数(条件数) 诊断病态系统,并应用正则化(规范化) 使其稳定


> **【中文解读】**
> 解 Ax=b 是数学中最古老的问题――线性归归的正规方程――归归的正规化系统――高斯过程的矩阵分解都是线性系统――条件数量衡量系统的数值稳定性――
...
> 每次训练线性回归,你都在线性系统解答中. 每次计算最小二乘法,你都在线性系统解答中. 每次神经网络层计算.`y = Wx + b`通过高斯过程 (Gaussian Process) 进行分析,你对矩阵进行分解.为了求马哈拉诺比的距离和反方差矩阵,你也在解对线性系统.

方程Ax=b在任何地方都出现.A是已知系数的矩阵.b是已知输出的向量.x是你想要找到的未知的向量.在线性回归中,A是你的数据矩阵,b是你的目标向量,x是重量向量.整个模型缩小为:找到x以使Ax尽可能接近b.

> 方程 Ax = b 无处不在──A 是已知系数矩阵,b 是已知输出向量,x 是你要求的未知向量──在线性回归中,A 是数据矩阵,b 是目标向量,x 是权重向量──整个模型归结为:找到 x 使 Ax 尽可能接近b──

这一课将从零开始构建解决这个方程的每种主要方法.你会理解为什么有些方法是快速的,有些方法是稳定的,为什么有些方法只适用于方形系统,而有些则适用于过度的,为什么你的矩阵的条件数决定了你的答案是否意味着任何东西.

> 本课程将从零构建到解方程的所有主要方法. 你将了解为什么有些方法是快速的,有些是稳定的,为什么有些方法只适用于方程系统,有些可以处理超定制系统,以及为什么矩阵的条件数决定了你的答案是否有意义.

## 概念的核心概念

> **【中文解读】**
> 对于超定系统 (方程数 > 未知数),没有精确的解,最小二乘法找到让残差最小的"最佳近似"――这是线性归归归的数学本质.

> **【拓展：线性系统在推荐系统中的规模】**
> 在Netflix奖赛中,矩阵分解问题涉及48万用户和17770部电影. 对于这个大约50万×17770矩阵需要高效的矩阵分解算法.

### 几何表示 Ax = b

一个线性方程系统具有几何解释.每个方程定义了一个超平面.解决方案是所有超平面交叉的点 (或点集).

> 线性方程组有几何解释──每个方程定义一个超平面 (超平面) ─解是所有超平面的交点 (交点集) ──
...
> 用矩阵语言说",一个解"意味着可逆.无解"意味着系统不一致.无穷多解"意味着A有零空间.
...
> **行视角。**方程的每个行定义一个方程. 每个方程是一个超平面.

**Column picture.**问题是,A列的线性组合产生了b?

> **列视角。**问题变成:A 的列的什么线性组合 (?? 线性组合) 可以产生b?
...
> 列视角更根本──如果 b 在 A 的列空间中,系统有解──如果 b 不在列空间中,你会找到列空间中最近的点──那最近的点就是最小的二乘解──
...
> 高斯消元法计算量是O (n^3) ⋅对1000x1000的系统,需要大约十亿次浮点运算――很快,但如果需要解决多个系统,还有更好的方法――

> **【中文解读】**
> 高斯消元法的思想很简单:通过行变换矩阵变成上三角形 (对角线下全是零),然后从最后行开始"回代"求解.

### 部分转向:为什么这很重要

如果没有旋转,高斯的消除可以失败或产生垃圾.如果旋转元素是零,你将以零分割.如果它很小,你会放大圆错误.

> 没有主元选取,高斯消元法可能失败或产生垃圾结果. 如果主元为零,你会分为零. 如果主元很小,你会放大差距.
...
> 在有限精度的浮点运算中,无主元选取的版本可能会丢失有效数字.部分主元选取总是选择最大可用主元以最大化差距.
...
> 为什么分解而不是直接消元?因为一旦有L和U,对于任何新的b求解 Ax = b只需要O (n^2):
...
> 由于不一样的 A 且不一样的 b 量求解 1000 个系统,LU 在总工作量上节省 1000/3 倍.

通过部分旋转,你得到PA=LU,其中P是记录行交换的变量矩阵.

> 使用部分主元选取,得到PA = LU,其中P是记录行交换的置换矩阵 (换矩阵) 

> **【拓展：LU 分解在大规模科学计算中的角色】**
> 有限元分析 (FEA) 中,结构力学的刚度矩阵通常是稀疏的百万阶段矩阵――超LU和MUMPS等库通过稀疏 LU 分解求解这些系统――一次分解后的多次求解时间可降低100-1000倍――天气预报模型每6小时更新一次,需要多次求解同一稀疏矩阵的不同右侧的向线性系统――

### 清理器分解

 QR分解因子 A 成直角矩阵 Q 和上方三角矩阵 R: A = QR.

> 解答将分解为正交矩阵 (正方矩阵)  和一个上三角矩阵 R:A = QR。

直角矩阵具有Q^T Q = I的属性.其列是直线向量.乘以Q保存长度和角.

> 正交矩阵具有性质Q^T Q = I.其列是正交量标准.乘以Q 保持长度和角度不变.
...
> 在求解最小二乘数问题时数值上比LU更稳定──Gram-Schmidt 过程逐列构建 Q:
...
> 每一步都移除了前面的方向的分量,只留下了新的正交方向.

### 乔莱斯基的分解

当A是对称 (A = A^T) 和正确的定义 (所有自值都是正),你可以将其作为A = L L^T,L是较低的三角形.这是乔莱斯基分解.

> 当A 是对称的(A = A^T)且正确的(所有特征值为正),你可以将其分解为A = L L^T,其中L 是下三角矩阵──这就是Cholesky 分解──
...
- 曲函数的Hessian最小值是对称正确的.
  凸函数在极小值处的赫西式矩阵是对称正确的.
- 总是对称正半确的.
  总是对称半正确的.

在高斯过程中,你将内核矩阵K与Cholesky进行因数化,然后解决K alpha = y以获得预测平均值.Cholesky因子还给你提供了边际概率的日志决定因素: log det(K) = 2 * sum(log(diag(L))).

> 在高斯过程中,你用Cholesky 分解核矩阵 K,然后求解K alpha = y 得到预测平均值──Cholesky 因子也给出边际似然的行列式对数:log det(K) = 2 * sum(log(diag(L)))。

> **【中文解读】**
> 乔尔斯基 分解是 LU 分解的"特殊优惠版"仅适用于对称正定矩阵,但速度是 LU 的两倍.好消息是 ML 中到处都是对称正定矩阵:协方差矩阵、核矩阵、X^T X、赫西安矩阵──GPTQ等模型量化方法也用于处理赫西安──

### 最小的正方形:当Ax = b没有确切的解决方案时

如果A是m x n,m > n (比未知更多的方程),则系统是过分确定的.没有确切的解决方案.相反,你将最小化二次错误:

> 如果 A 是 m x n 且 m > n 方程数多于未知数),系统是超定 (超定) 了.
...
> 推导:展开A 求梯度并设为零:2 A 求梯度并设为零:2 A 求梯度 A x - 2 A 求梯 b = 0。
...
> 这个联系是精确的. 在线性归归归中,数据矩阵 X 每行一个样本, 每列一个特征.
...
> 这是线性回归的解析解.`sklearn.linear_model.LinearRegression.fit()`通过QR或SVD等价格形式计算这个数字.

添加一个规范化术语 lambda * I 在矩阵,你得到的脊柱回归:

> 在矩阵上添加正则化项 lambda * 我得到回归(Ridge回归):
...
> 正则化使矩阵条件数更好 (更容易精确求逆) 并通过将权重缩小到零以防止过拟合.

> **【拓展：条件数与深度学习训练稳定性】**
> 变压器训练中的梯度爆炸/消失问题与条件数密切相关.残差连接 (ResNet) 和层归化 (LayerNorm) 本质上是改善各层的条件数. 研究表明,当X^T X的条件数量超过10^10时,float32 精度下线性归归的结果可能完全不可靠.混合精度训练 (FP16/BF16) 对条件数量更敏感,这是为什么需要损失扩展的原因.

### 伪逆 (摩尔-罗斯)

伪逆向 A+ 将矩阵逆向将用于非正方形和单一矩阵.

> 伪逆(伪逆) A+ 将矩阵求逆推广到非方阵和奇异矩阵――对于任何矩阵 A:
...
> + 由每个非零奇异值取倒数并转置得到──如果A = U V^T,则A+ = V Sigma+ U^T──
...
> 在 ML 中,当特征近似共线 (Collinear) 时会出现病态条件――正则化 (正则化) 添加 lambda * I) 将条件数从 sigma_max / sigma_min 改善为 (sigma_max + lambda) / (sigma_min + lambda) 。

### 复制方法:结合梯度

对于非常大的稀疏系统 (数百万未知的),像LU或Cholesky这样的直接方法太昂贵.反复方法通过改善许多反复的猜测来接近解决方案.

> 对于非常大的稀疏系统 (数百万未知),LU或Cholesky等直接方法太昂贵――通过多次改进猜测来近似寻求解――

结合梯度 (CG) 在A是对称正确的定义时解决 Ax = b.它在最多的 n 代数中找到准确的解决方案 (在准确的算法中),但如果A的本值集成,通常会更快地收缩.

> 共梯度法 (Conjugate Gradient, CG) 在 A 是对称正定时求解 Ax = b。它最多在 n 次代就能找到精确解 (精确算术中),但如果 A 的特征值聚集在一起,通常收得更快──
...
> **【中文解读】**
> 在图神经网络的消息传递中,邻近矩阵特征矩阵乘法本质上就是稀疏矩阵运算.条件数量越来越小的系统收收收越快,这也是正规化的另一个好处.
...
> **线性回归。**解析解求解正规方程 X^T X w = X^T y──通过Cholesky(n 较小时) 或 QR(需要数值稳定性时) 或 SVD(矩阵可能不满排列时) 完成────
...
> **岭回归。**在 X^T X 上加 lambda * I。正则化系统 (X^T X + lambda * I) w = X^T y 总是可以通过Cholesky 求解,因为当 lambda > 0 时 X^T X + lambda * I 是对称正确的。

**Gaussian processes.**预测平均值需要解决K alpha = y,K是内核矩阵.K的乔莱斯基因数化是标准方法.

> **高斯过程。**预测平均值需要求解 K alpha = y,其中 K 是核矩阵。Cholesky 分解 K 是标准方法。对数边际似然使用 log det(K) = 2 个数量(log(diag(L)))。

**Neural network initialization.**纵向初始化使用QR分解来创建一个重量矩阵,其列是正规的.这可以防止深度网络中信号崩.

> **神经网络初始化。**正交初始化使用QR 分解创建列标准正交的权重矩阵.

**Preconditioning.**大规模优化器使用不完整的乔莱斯基或不完整的LU作为结合梯度溶剂的预先条件.

> **预处理。**大规模优化器使用不完全的Cholesky或不完全的 LU 作为共梯度求解器的预处理条件.

**Feature engineering.**如果卡帕是大,则放下功能或添加规则化.

> **特征工程。**条件数 X^T X告诉你特征是否共线. 如果卡帕很大,删除特征或添加正则化.
