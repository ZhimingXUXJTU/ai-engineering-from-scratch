...
> **【中文解读】**本节介绍了vLLM 推理服务 页面关注 连续批量处理和分块预填充 三大核心优化
**Type:** Learn
...
>  **【前置】**节前请先掌握:阶段17·04(vLLM) 、阶段17·06(Radix注意) ――vLLM生产堆 = K8s 部署参考,LMCache = KV下载到CPU/磁盘――
>  **【类比】**没有必要重算预填) 异步、对用户透明度──即使没有共享前也值──16倍H100基准:KV 超HBM 时大幅升吞吐量;低KV 占用时开销很小──
**Time:** ~60 minutes | **时间:** ~60 minutes

## 学习目标

- 图表vLLM生产堆层:路由器,发动机,KV脱载,可观测性.
  中文翻译:绘制vLLM生产堆层次:路由器、引擎、KV 卸载、可观测性。
- 解释KV脱载连接器API (v0.9.0+) 以及如何隐藏0.11.0异步路径脱载延迟.
  中文翻译:解释 KV卸载连接器API(v0.9.0+) 以及 0.11.0 异步路径如何隐藏卸载延迟──
- 量化LMCache CPU-DRAM帮助 (KV > HBM) 与增加上 (KV足够小以适合HBM) 的时间.
  中文翻译:量化 LMCache CPU-DRAM 在何时有帮助(KV > HBM)vs何时增加开销(KV 足够小可放进 HBM) ⋅
- 根据部署限制,选择原生vLLM CPU脱载和LMCache连接器.
  中文翻译:给定部署约束,在原生 vLLM CPU 卸载和 LMCache 连接器之间选择.

## 问题 问题引入

> **【中文解读】**计算服务在高并发时GPU HBM 占满,发生抢占事件请求被逐出、重新排队、同一个2K标提示一分钟内被重新填充四次。GPU 计算花在冗余预填上,Goodput 远低于原始吞吐量。添加更多的GPU 是线性成本,但CPU DRAM 很便宜一个插座有512GB+,虽然延迟比HBM 差几个数量级,但对于"临时热"的KV缓存 足够。

> **【拓展：vLLM Production Stack 架构】**机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机机
...
> **【中文解读】**两种KV缓存 卸载方案对比:(1) 原生 vLLM CPU 卸载引擎本地,存储KV块到主机 RAM,实现快速,零网络跳转,但不跨引擎共享;(2) LMCache 连接器集群级,存储块到共享LMCache 服务器(CPU DRAM + Ceph/S3 压层),任何引擎都可访问;;选择原生当单引擎有HBM 力量,选择LMCache 当多引擎共享前(RAG 共享系统提示、多租户共享模板) ;;
...
> **【拓展：LMCache 基准测试数据】**通过 16x H100(80GB HBM) 跨 4 个a3-highgpu-4g 的基准测试表现:(1) 低KV 足迹(短提示、低并发) 所有配置匹配基线,LMCache 增加 ~3-5% 开销;(2) 中等足迹LMCache 开始在前复用方面提供帮助;(3) KV 超过HBM原生CPU 卸载显著和LMCache 都有改善吞吐量,LMCache 因引擎共享收益更大.
...
> **【中文解读】**LMCache 在以下场景是决定性的:(1) 多租户服务系统提示跨租户共享;(2) RAG文档块跨查询重复;(3) 微调变体(LoRA) 同一个基础模型的KV重复减少冗余工作;(4) 抢占密集型工作负载从CPU恢复比重复预填 更便宜.

> **【拓展：KV Cache 卸载的集成】**17·17分离式服务 + LMCache 的协同效应:从预填池到解码池的 KV 转移如果不立即使用,可以存入 LMCache;后续查询从 LMCache 拉取而不是重新填充.
...
> 本课产出发 `outputs/skill-vllm-stack-decider.md`鉴于工作负载的形状和vLLM部署,决定原生对LMCache对任何一个.

## 练习题

1. 跑步`code/main.py`什么时候使用HBM开始付钱?
   中文翻译:运行 `code/main.py`,我们在什么 HBM利用率开始划算?
2. 租户共享6K代币系统提示,每小时查询200个.
   中文翻译:一个租户在 200 查询/小时中共享 6K 代币系统提示――计算 LMCache 的预期节省――
3. 设计HA策略 (复制,返回原始).
   中文翻译:LMCache 服务器是单点故障――设计 HA 策略(副本、回退到重算) 』
4. 对于4K标记KV的70BFP8 (500MB),读取时间与重新填充时间是多少?
   中文翻译:LMCache 存储到机械硬盘上的 Ceph──对于70B FP8 上4K代币 KV(500MB),读取延迟是多少?与重算比较──
