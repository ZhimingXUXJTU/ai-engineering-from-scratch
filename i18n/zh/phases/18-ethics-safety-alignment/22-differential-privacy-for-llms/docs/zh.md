...
> **【中文解读】**本节介绍了LLM的差分隐私在训练和推理中保护用户数据隐私的数学方法――DP-SGD是标准方法噪声注入梯度更新提供形式化 (epsilon, delta) 保证――LoRA + DP-SGD是2025年常见配置全量DP-SGD 训练前沿模型成本过高,LoRA 限制梯度更新到小型适配器――

> **【拓展：MIA vs 训练数据提取 → 衡量差距】**2024-2025年两条证据线形成张力:金丝雀 MIA(Duan 等人 2024) 报告语言模型的成功有限;训练数据提取(Carlini 2021,Nasr 等人 2025) 恢复大量逐字记忆.2025年3月的解决方案:两者测量不同的事情MIA 问"示例 e 在 D 中吗?",提取问题"我能恢复 D 的什么?""最可提取的"示例才是隐私的关键.
...
>  **【前置】**学本节前请先掌握:阶段01·09(信息论) 、阶段10·01(大模型训练) ――DP-SGD = 标准DP 训练方法,(ε,δ) 保证。
>  **【类比】**根据"数据隐身衣"的数据,单个样本不会影响整体训练. 形式化数学证明无法从模型中推断是否某条数据在训练集中.
> 🤔 困境:金丝雀式MIA 攻击失败与训练数据提取成功差别在测什么(插入与 最易提取) ⋅2025.3 新金丝雀设计首次对真实数据LLM做非凡DP审计──
...
> 定义 (epsilon,delta) -差分隐私并说明DP-SGD 方法──解释 2024-2025年张力:金丝雀 MIA vs 训练数据提取给出不同图景──描述PMixED 及为什么推理时私有预测是DP 训练的替代──描述通过LLM 反的差分隐私逆转攻击──

## 问题问题

卡利尼等人2021年显示,生产语言模型在需求上复制了文字体训练文本.DP是正式的防御:训练,使输出对任何单一训练例子都不敏感.2024-2025年的证据表明DP-SGD是必要的,但部署的 ε值可能不匹配威胁模型.

> 专业学习会记忆──卡尔尼等2021年展示生产语言模型可按需复现逐字训练文本──DP是形式化防御:训练使输出对任何单个训练样本不可敏感──2024-2025年证据显示DP-SGD是必要的,但部署的 ε 值可能与威胁模型不匹配──

## 概念的概念

> **【中文解读】**(epsilon,delta) -差分隐私定义:随机算法 M 是 (epsilon,delta) -DP 的,如果对于任何两个相差的一个示例的数据集和任何事件 S:P(M(D) 在 S) <= e^epsilon * P(M(D') 在 S) + delta──解释:输出分布足够接近(由 epsilon 参数化),任何单个个体的贡献都不能可靠推断,除概率的 delta──
...
> 随机算法 M 是 (ε, δ) -DP 的,如果对任何两个相差的一个示例的数据集和任何事件 S:P(M(D) 在 S) <= e^ε * P(M(D') 在 S) + δ。

解释:输出分布足够接近 (以 ε 参数化),以免可靠地推断任何单个人的贡献,除非是可能 δ.

> 解释:输出分布足够接近 (由 ε 参数化),任何单个人的贡献都不能可靠推断,除了概率 δ──
...
> 采样小批次――2. 计算逐例梯度――3. 切割每梯度到值C――4. 求和切割后的梯度并增加高噪音――5. 使用噪音和更新参数――
...
> 隐私成本由计量器跟踪. 文献中报告的 ε 值因威胁模型而异;没有普遍的默认"安全" ε ⋅较低 ε 通常需要更多的噪音并可能增加效益损失.

### 洛拉+DP-SGD

边界模型的完整DP-SGD是禁止的.LoRA (Hu et al. 2022) 限制梯度更新到小型适配器,减少每个例子梯度存储.LoRA + DP-SGD是2025年常见的配置.DP保证适配器;基模型保持固定.

> 全量DP-SGD 训练前沿模型成本过高.LoRA 限制梯度更新到小型适配器,减少逐例梯度储存.LoRA +DP-SGD 是2025年常见配置.
...
> 雀 MIA 报告有限成功,暗示 MIA 困难――训练数据提取报告大量记忆,暗示 MIA 在相关意义上很容易――

2025年3月的决议 (arXiv:2503.06808):两种测量不同.MIA问"插入的鱼类上是e的例子吗?"鱼类提取问题"我可以从D中恢复什么?""最可提取的"例子是对隐私的重要;鱼类低于报告,因为它们没有优化以获得提取.

> 据了解,在2025年3月的解决方案中,两者测量不同的事情.

基于损失的MIA,没有影子模型. 基于现实数据的LP证券的LLM的第一次非微观DP审计.

> 新金丝雀设计――无影子模型的损失基于的MIA――第一次对真实数据进行非凡的DP审计――

> **【拓展：PMixED → 推理时隐私】**根据"中国"的数据分析,每一个专家都会看到训练数据的分片,聚合增加噪音以实现DP. 完全避免了DP训练.
...
> **【中文解读】**差分隐私逆转攻击(2025):使用DP 训练模型的置信分数作为预言机重新识别个体――即使输出不泄露,置信分布也可能泄露――防御:不暴露置信度,或在暴露前截断/量化――这是 (epsilon,delta) -DP 训练之外的额外要求――

### 通过LLM反方式的差异性隐私反

现在,我们需要一个新的方法来帮助我们,我们需要一个新的方法来帮助我们.

> 2025年新兴攻击:使用DP 训练模型的置信分数作为预言机重新识别个体――即使输出不泄露,置信分布也可能泄露――

防守:不要暴露秘密,或者在暴露之前切断/量化它们.这是 (ε, δ) -DP训练之外的额外要求.

> 防御:不暴露置信度,或在暴露前截断/量化――这是 (ε, δ) -DP 训练以外的额外要求――

### 在这个阶段的第18阶段

课程20-21是偏见/公平.课程22是隐私.课程23是通过水标的来源.课程27涵盖监管数据来源层.

> 课时20-21是偏见/公平.课时22是隐私.课时23是通过水印的来源.课时27涵盖监管数据来源层.

> **【拓展：DP-SGD 的实际开销 → LoRA 解决方案】**全量DP-SGD 训练前沿模型在计算,内存和效用上代价巨大――LoRA(Hu 等人 2022) 限制梯度更新到小型适配器,减少逐例梯度存储――LoRA +DP-SGD 是2025年常见配置DP保证适用于适配器,基础模型保持固定――这是实用性和隐私之间的工程权衡――
...
> `code/main.py`在玩具二分类数据集上模拟DP-SGD──你可以扫描噪音乘数 σ 和剪裁范数 C,跟踪 (ε, δ) 预算和准确率成本──"金雀攻击"插入唯一的训练样本并测量对数损测试是否可以在DP前后检测到──

## 发射上线

这一课产生了`outputs/skill-dp-audit.md`鉴于对语言模型部署的DP索赔,它审计: (ε, δ) 值,所使用的会计师,MIA评估协议以及是否评估了信任暴露向量.

> 本课产出发 `outputs/skill-dp-audit.md`△给定语言模型部署的DP声明,审计: (ε, δ) 值,使用的计量器,MIA 评估协议以及是否评估了信任暴露量.
