# 度系统 开AI,视角,拉马卫队 拉马卫队视角 审核OpenAI
...
> **【中文解读】**本节介绍内容审核系统OpenAI Perspective、Llama Guard等内容安全工具──OpenAI 适度API(omni-moderation-last, 2024) 基于GPT-4o,在单次调用中分类文本+图像,返回13个类别布尔值──三层模式是2026年默认配置:输入审核(预生成)、输出审核(后生成)、自定义审核(域规则)──

> **【拓展：审核栈 → 生产配置】**根据"暴力犯罪"和"非暴力犯罪"的策略分类,Llama Guard分为"暴力犯罪"和"非暴力犯罪"――部署.
...
>  **【前置】**课本节前请先掌握:阶段18·16(红队工具) ⋅内容审核 = 把12-16 课的安全政策操作化的产品层――
>  **【类比】**审核系统 = "AI 服务台的保安"――OpenAI 适度API(GPT-4o 驱动) = 一次调用分类 13 类(骚扰/仇恨/非法/自残/性/暴力等);Llama Guard 3/4(14 MLCommons 类别,多模态);前景API(Google Jigsaw,毒性打分,LLM 前的旧时代) 』
>  三层默认配置:输入审核(前代) + 输出审核(后代) + 自定义审核(域规则) ――异步并行调用藏延迟──
...
> 描述OpenAI调控API的类别分类法及其与Llama Guard 3 MLCommons 集的区别. 描述三层审核模式和每个层次的一个失败模式. 描述视角API 作为LLM前时代基线的位置.说明Azure 弃用时间线.

## 问题问题

课程12-16描述了攻击和防御工具.课程29涵盖了部署的调节系统,该系统在用户触摸产品的表面上操作防御.三层模式是2026年默认配置.

> 课时12-16 描述攻击和防御工具――第29课涵盖已部署的防御操作审核系统――三层模式是2026年默认配置――

## 概念的概念

> **【中文解读】**开放AI适度API的13个类别:骚扰/骚扰-威胁、仇恨/仇恨-威胁、自残/自残意图/自残指示、性/性-未成年人、暴力/暴力图形、非法/非法暴力──多模态支持适用于暴力、自残和性但不包括性未成年人,其余仅仅仅文本──比上一代审核端点多语言测试集 42%好──
...
> **【中文解读】**三层审核模式的设计逻辑:输入审核必须在生成前完成,输出审核在生成后运行. 三层按顺序设计. 同层内可并行.同时运行多个分类器在同一文本上.
...
> **【拓展：失败模式 → 为什么需要多层】**仅输入审核无法捕获输出幻觉(课时12-14 编码攻击绕输入分类器);仅输出审核允许任何输入到达模型(增加成本,向攻击者暴露内部推理);仅自定义审核不跨类别鲁棒(正则表达式脆弱) 部分层是默认的安全带+吊带――
...
> 第十六课在红队背景下涵盖审核工具――第29课涵盖已部署的审核――第30课以当前双重用途能力证书结束――

> **【拓展：Azure 迁移 → 2024-2027 行业项目】**亚洲内容调节器 2024 年 2 月 废弃,2027 年 2 月退役,取代基于LLM的Azure AI内容安全并与Azure OpenAI 集成.迁移是一个 2024-2027 年的行业级项目.每个使用Azure内容调节器的部署都需要规划迁移路径.这是传统内容审核向LLM 驱动审核的系统转变.
...
> 本课产出发 `outputs/skill-moderation-stack.md`│ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │ │
