...
> **【中文解读】**你已经分别构建了提示,嵌入,RAG,函数调用,缓存和护──本课程将所有组件整合成一个生产级服务能处理真流量,优雅降级,流式输出,追踪成本,承受第一批1万用户──

> **【拓展：生产化→AI工程全栈】**这是一个第11阶段的集成课程.将提示工程,RAG,安全,缓存等能力的整合为端到端应用,是从"使用AI API"到"构建生产系统"的关键步骤.

>  **【前置】**这是第11阶段的石 (顶点课),要求先学完第11阶段·01-12──还需要:(1) 快API或Flask基础本节使用快API 构建服务;(2) 码头基础容器化部署;(3) 至少有一种可观测性工具(长、直升机、开放电气) ⋅

**Type:** Build (Capstone) | **类型:** 构建（顶点课）
**Languages:** Python | **语言:** Python
**Prerequisites:** Phase 11 Lessons 01-15 | **前置知识:** Phase 11 · 01-15
**Time:** ~120 minutes | **时间:** ~120 分钟
**Related:**阶段11·14 (MCP) 为用共享协议取代定制工具方案;阶段11·15 (即时缓存) 为稳定预先文件降低50%90%. 预计这两种产品都将在2026年每一个严重的生产堆中出现.**相关:**11 阶段 · 14 (MCP) 用共享协议替代专用工具方案; 11 阶段 · 15 (提示缓存) 在稳定前上降50%至90% 成本──两者都是2026年严生产的标准配置──

## 学习目标

- 将所有阶段11组件 (提示,RAG,函数调用,缓存,防护) 连接到一个生产准备的服务中
  把第11阶段所有组件 (提示,RAG,函数调用,缓存,护) 串成单个生产级服务
- 实现流媒体代币交付,优雅的错误处理,并请求截止时间管理
  实现流式代币 交付 优雅错误处理和请求超时管理
- 建立可观测性在应用程序中:请求记录,成本跟踪,延迟百分比和错误率仪表板
  构建可观测性应用:请求日志,成本追踪,延迟百分位和误差率仪表盘
- 应用程序的健康检查,利率限制和供应商停产的反弹策略
  部署应用,包括健康检查,限流和供应商机的退回策略

> **【中文解读】**本课目的:将LLM 应用从原型部署到生产环境API 网关 负载平衡 推理引擎 监控警警警报 版本管理 A/B 测试

>  **【类比】**演示与生产 = 学生作业与银行系统――演示:单进程、单用户、不持久化、无监控、API 失败就崩――生产:多进程、并发限流、状态持久化、全链路监控、API 失败自动倒退、灰度发布、版本回滚――差异是工程量,不是AI 能力――

> ️ **【易错点】**产产部署的3个坑:**没做 provider fallback**OpenAI 机整个产品挂掉;用LiteLLM或自己写一层,OpenAI 失败自动转化人类――(2) **没用流式输出**长回答让用户等10人才看到第一个字;用`stream=True`首字延迟降至500ms──(3) **没追踪成本**上线 3 天烧光一个月的预算; 每次API调用记符号 数 + 成本到长/直升机,设日均预算告警.
...
- 用户要求模型生成SQL.模型输出`DROP TABLE users`现在,我们要去.
  用户要求模型生成SQL.模型输出了`DROP TABLE users`,我知道.
- 你的月票达到12,000美元,你不知道什么原因造成了.
  账单达到12,000美元,你不知道是什么功能导致的.
- 平均响应时间为8秒,用户在3秒后离开.
  平均响应时间8秒.用户3秒后就离开了.
...
> 这是一个顶点课程. 你将构建一个完整的生产级LLM服务,集成提示管理,嵌入和向量搜索,调用函数,评估,缓存,护,流式输出,错误处理,可观测性和成本追踪.

## 概念的核心概念

> **【中文解读】**将LLM 应用部署到生产环境需要完整的工程:API 网关到负载平衡到推理引擎到向量数据库到缓存层到监控警警.关键挑战包括:版本管理(模型/快速更新)  A/B测试、故障恢复、成本控制――

> **【拓展：LLM 生产架构】**典型生产架构:FastAPI/Flask API 层到LangChain/LlamaIndex 编排层到VLLM/TGI 推理层到Pinecone/Weaviate 向量存储到Redis 缓存到LangSmith 监控──关键标志:P99 延迟、幻觉率、每日成本、用户满意度──
...
> 请通过API 网关进入,网关处理认证和限流――提示路由器选择合适模板前,输入护检查提示注入和违规内容――语义缓存检查近期是否回答过类似问题――缓存未定时,启动流式调用LLM――输出护验证响应――评估日志机记录质量指标――成本追踪器核算每个代币――响应流式返回客户端――

七个组件,每个组件都是你已经完成的课程.

> 七个组件――每个都是你完成的课――工程在连线中――
...
> 转圈时的用户全程转圈时的用户首个代币在200-500ms到达时的总时间相同.感知延迟下降90%──
...
> 您的服务器从LLMAPI接收块并作为SSE事件转发给客户端.`EventSource`其他地方`httpx`鱼类的消费流.
...
**第一层：API 失败。**解决方案:带动的指数退避――从1秒开始,每次重试翻倍,加随机动防惊群效应――最多3次重试――
...
**第二层：模型失败。**模型返回格式错误的 JSON、幻觉函数名称或产生验证失败的输出――解决方案:使用修复提示重试――把错误包含在重试消息中让模型自我纠正――

**Layer 3: Application failures.**如果下游服务无法实现,向量存储器缓慢,防护子会造成例外.解决方案:优雅的降解.如果RAG文本不可用,请继续使用它.如果缓存下载,请绕过它.永远不要让二级系统撞击主要流.
**第三层：应用失败。**下游服务不可达 量存储慢、护抛异常──解决方案:优雅降级──RAG 上下文不可用时不用上下文继续──缓存机时绕过──绝不让下系统崩主流程──
...
**Perplexity。**用户查询进入. 搜索引擎查询 10-20 网页. 网页分块,嵌入,重排. 首 5 块成为RAG 上下文.
...
**Cursor。**开放文件、周围文件、近期编辑和终端输出构成上下文──提示路由器决定:小模型做自动补充(Cursor-small,大约20ms),大模型做聊天(Claude Sonnet 4.6 / GPT-5,大约3s) ⋅上下文被激进压缩仅相关代码段,非整个文件──代码库嵌入提供长程上下文──投机编辑流式不同,非整个文件──MCP 集成让第三方工具插入而无需按工具修改代码──
...
**ChatGPT。**插件、函数调用和MCP 服务器让模型访问网络、运行代码、生成图像和查询数据库──路由层决定调用哪些能力──记忆跨会话持久化用户偏好──系统提示是1,500+代币的行为规则,通过提示缓存──多模型服务不同功能:GPT-5做聊天、GPT-图像做图像、语、o4-mini做深入推理──
...
> 缓存时同样流量 $11,625/月──35% 缓存命中率省 35% LLM 成本──这就是第11课的存在原因──
...
> 版本化提示模板含A/B测试支持──每个模板名字、版本和模板字符串──路由器根据请求下文和实验分配选择──
...
> 核心 LLM 接口――失败时带动的指数退避――沿模型链回退――支持每代币交付的流式――
...
> 作为真实服务器运行:取消注释并装依赖 `pip install fastapi uvicorn`〔访问〕`http://localhost:8000/docs`看自动生成的API文档.
...
> 四个工人──每一个处理不同步骤 I/O──单机 4个工人 服务 400+ 并发 LLM 请求,因为都在等网络 I/O 而不是CPU──

## 运送它.

这一课产生了`outputs/prompt-architecture-reviewer.md`通过一个可重复使用的提示来对任何LLM申请的结构进行审查,

> 本课产出发 `outputs/prompt-architecture-reviewer.md`对照生产清单审查任何LLM应用架构可复用提示――给它你的系统描述,返回差距分析――

它还产生了`outputs/skill-production-checklist.md`-- 关于将LLM申请运送到生产的决策框架,涵盖本课程的每个组成部分,具有特定的门和通过/失败标准.

> 产出`outputs/skill-production-checklist.md`把LLM应用在线生产的决策框架,覆盖本课程的每个组件,包括具体的价值和通过/失败标准.

## 练习题

1. **Add RAG integration.**建立一个简单的内存向量存储器,包含20份文件.`rag_answer`测量RAG背景和没有RAG背景的响应质量变化. 单独跟踪检索延迟与LLM延迟.
   **添加 RAG 集成。**用20个文件构建简单内存量存储.`rag_answer`时,嵌入查询,找3个最相似的文档,作为上下文注入.
...
2. **Implement real function calling.**添加工具注册表 (从09课程) 加入服务.当用户提出需要外部数据 (天气,计算,搜索) 的问题时,管道应该检测到这一点,执行工具,并将结果列入提示.添加一个`tools_used`答案的场所.
   **实现真实函数调用。**给服务加工具注册表(来自课09) 用户问需要外部数据问题(天气、计算、搜索) 当,流水线应检测、执行工具、把结果加入提示──响应加`tools_used`字段.
...
3. **Build a cost alerting system.**追踪每日用户的成本.$0.50/day, switch them to `gpt-4o-mini`. When total daily cost exceeds $启动紧急模式:只为重复查询提供缓存响应,`gpt-4o-mini`其他所有东西,拒绝超过2000个输入代币的请求.
   **构建成本告警系统。**追踪每用户每天成本.$0.50/天时切换到 `gpt-4o-mini`。每日总成本超 $时激活紧急模式:重复查询仅缓存、其他全用 `gpt-4o-mini`、拒绝 >2,000 输入代币 请求──用模拟流量峰值测试──
...
4. **Implement prompt versioning with rollback.**存储所有提示版本的时间标签.添加一个终端点,显示每个提示版本的质量指标 (延迟,用户评级,错误率).实现自动反弹:如果一个新的提示版本的错误率超过前一个版本的2倍,自动反弹.
   **实现带回滚的提示版本管理。**存所有提示版本配时间──加端点显示每个提示版本的质量指标(延迟、用户评分、错误率)──实现自动回滚:若新提示版本在100 请求上错误率是前一版本 2 倍,自动回归──

5. **Add OpenTelemetry tracing.**工具每一个组件 (缓存查找,防护轨检查,LLM调用,成本计算) 作为一个独立的跨度.每一个跨度记录其持续时间.将痕迹输出到控制台.显示一个单个请求的完整痕迹,每个组件的贡献可见到总延迟.
   **加 OpenTelemetry 追踪。**把每个组件 (缓存查找,护检查,LLM调用,成本计算) 作为独立的跨度插──每个跨度记录时长──导出追踪到控制台──显示单个请求的完整追踪,每个组件对总延迟的贡献可见──
...
- [FastAPI Documentation](https://fastapi.tiangolo.com/)-- 在本课程中使用的非同步Python框架,
  快API 文档本课程所需异步 Python 框架,含原生 SSE 流式和自动开放API 文档
...
- [Hamel Husain -- Optimizing Latency: TGI vs vLLM vs CTranslate2 vs mlc](https://hamel.dev/notes/llm/inference/03_inference.html)测量了主要服务框架的吞吐量和延迟比较.
  哈梅尔·胡塞安延迟优化:TGI VS vLLM VS CTranslate2 vs mlc 跨主要服务框架的吞吐和延迟测量对比
