...
> **【中文解读】**总量微调 7B 模型需要56GB 显存储.LoRA只训练不到1%的参数,6GB 显存储即可完成微调,质量不输全量微调.整个开源微调生态都建立在这个技术上.

> **【拓展：LoRA微调→定制大模型】**洛拉是企业定制大模型的核心技术:使用少量领域的数据微调基础模型,获得专业能力 (如金融分析,法律推理,代码生成等).

>  **【前置】**学本节前请先掌握:(1) 阶段10·06(指示调整SFT) 理解监督微调的基本流程;(2) 线性代数基础矩阵乘法、秩、SVD 分解(看阶段01·11SVD);(3) PyTorch 基础`nn.Linear`,我知道.`backward()`,我知道.`optimizer.step()`着脸`transformers`库的基本使用法.`peft`和 `trl`库.

**Type:** Build | **类型:** 构建
**Languages:** Python | **语言:** Python
**Prerequisites:** Phase 10, Lesson 06 (Instruction Tuning / SFT) | **前置知识:** Phase 10 · 06（指令微调/SFT）
**Time:** ~75 minutes | **时间:** ~75 分钟
**Related:**阶段10将从零开始覆盖SFT/DPO循环. 这一课将这些循环连接到2026年PEFT工具包中 (PEFT,TRL,Unsloth,Axolotl,LLaMA-Factory).**相关:**通过零讲解SFT/DPO循环.本课将将其接入2026年PEFT工具链.

## 学习目标

- 通过注入低级适配器矩阵 (A和B) 进入预训练模型的注意力层来实现LoRA
  通过向预训模型的注意力层注入低级适配矩阵 (A 和 B) 实现LoRA
...
- 使用QLoRA (4位量化基 + LoRA适配器) 调整模型,以适应消费者GPU内存
  用QLoRA(4位量化基础模型 + LoRA 适配器)微调模型以适应消费级GPU内存
- 将LoRA重量重新融入部署的基模型,并将推断速度与无适配器进行比较
  将LoRA权重合并回基础模型用于部署,并比较有/无适配器推理速度

> **【中文解读】**本课目标:使用LoRA/QLoRA 进行参数高效微调――LoRA 仅训练低排序分解矩阵 (百万级参数),而不是全参数 (数十亿),使微调成本降低90%+――
...
> 你有一个基础模型――Llama 3 8B――你希望它能用你的公司语气回答客服工单――SFT是答案――但SFT有成本问题――
...
> 全参数微调更新模型中的每个参数――Llama 3 8B 有80亿参数――fp16 下每个参数占2字节――只需要16GB的重量.
...
> 一张A100 80GB 勉强能装下――两张A100 在云上每小时$3-4。在 50,000 个样本上训练 3 个 epoch 需要 6-10 小时。每次实验 $两到四十个.

现在,我们需要一个更深层次的问题. 完整的细节调整改变了模型的每一个重量. 如果你细节调整了客户支持数据,你可能会降低模型的一般能力. 这被称为灾难性忘记. 模型在你的任务上变得更好,而其他事情上变得更糟.

> 还有一个更深层次的问题――全参数微调修改模型中的每个权重――如果你微调客服数据,可能会降低模型的通用能力――这被称为灾难性遗忘――

你需要一种训练少参数,使用少记忆,

> 你需要一种训练,使用更少的内存,而不会破坏模型现有知识的方法.

>  **【类比】**全量微调像"把全本教科书重写一遍"每个字母(参数) 都改,工作量大也很容易把原来的内容改成错误(灾难性遗忘) ⋅ LORA 像"在教科书页边贴贴贴"原文(W) 结不动,你只在边边贴小纸条(A×B矩阵) 写新注释──最终输出 = 原文 + 便利贴──换任务时,撕裂旧便利贴贴新即可,原文保留──

## 概念的核心概念

> **【中文解读】**结原始权重,仅训练低排分解矩阵 (A*B),将可训练参数从数十亿降至数百万.

> **【拓展：LoRA 的工程实践】**洛拉的排名通常设为8-64,应用于Q/V投影矩阵效果最好──QLoRA(4-bit基础模型 + LoRA)让你在单张RTX 4090上微调 Llama-3-8B──HuggingFace PEFT库让洛拉微调几行代码即可实现──洛拉的成本约为全参数微调的 1/10,效果接近──

>  **【困惑】**问:QLoRA是什么?和LoRA 区别?A:QLoRA =量化LoRA──基础模型使用4位量化(NF4) 存储,LoRA 适配器使用bf16 训练──这样Llama-3-8B的16GB权重缩小到4GB,加上LoRA的训练开销(约100MB),总共6GB 显存就能微调单张RTX 4090 / 3090 即可可.代价:训练速度比纯LoRA 慢约30% ((因为需要边反量化边向前),但成本可控──
...
> 爱德华·胡和微软同事于2021年6月发表的LoRA──论文洞察:微调期间权重更新具有低内在位置──你不需要更新4096x4096 权重矩阵中全部1670万参数──更新中的有用信息可被排列16或32的矩阵捕获──

>  **【困惑】**问:为什么低排矩阵能捕获微调更新的信息?A:经验观察Aghajanyan等(2020) 发现预训练模型的重量更新ΔW 在内在维度 (内在维度) 上很低,通常只有几百到几千维就能学好一个新任务――直觉:基础模型已经"知道"很多,微调只是"轻调调方向",不需要全方位重写――排 r=16的两个矩阵 A(d×16) 和 B(16×d) 只有2*16*4096 = 131K参数,比原 16.7M少 99.2%――
...
> 其中 W 是 d_out x d_in 矩阵.对于 4096x4096 注意力投影,那就是 16,777,216 个参数.

洛拉结W并添加低级分解:

> 洛拉 结 W 并添加低序分解:

> ️ **【易错点】**微调的三个坑:**秩 r 设太大**r=64 起步就太大,参数接近全量微调,省显存优势消失;起点 r=8 或 r=16,效果不够再加倍――(2) **target_modules 选错**只有对`q_proj`加 洛拉 效果有限,标准做法是`["q_proj", "v_proj", "k_proj", "o_proj"]`更多的激励可以增加MLP的`gate_proj`现在,我们要去.`up_proj`现在,我们要去.`down_proj`〔(3) **学习率没调高**LoRA参数是新初始化的,需要比预训重量更大的 lr;典型 lr=1e-4到3e-4(比全量微调的2e-5高 5-10倍)
...
> 其中 B 是 (d_out x r),A 是 (r x d_in) 排列r 比 d 小得多通常是 8、16 或 32、
...
> 一个随机高斯初始化.B初始化为零. 这意味着LoRA 贡献从零开始.

### 规模因素:阿尔法

洛拉引入了扩展因子alpha,它控制了低级更新对输出的影响:

> 引入缩放因子 alfa,控制低排更新对输出的影响程度:
...
> 当alpha = r,缩小为1x──当alpha = 2r(常见默认),缩小为2x──这个超参数独立于基础学习率控制LoRA路径的学习率──
...
- 高级alpha意味着每步的更新更大,这可以加速化或导致不稳定性
  较高的阿尔法意味着每一步都会变得更大,可能会加速收获或导致不稳定
...
> 大多数任务的最佳点:q_proj + v_proj──这针对自注意查询和值投影,控制模型关注什么、提取什么信息──对代码生成等复杂任务,添加MLP层有帮助,但对简单任务回报减少和参数翻倍──
...
>  Hu 等人显示,r=4 已经捕获了简单任务的大部分适应性.

### 定量:4位量化+LORA

蒂姆·德特默斯和华盛顿大学的同事在2023年5月发表了QLoRA. 想法是:将冷基模型量化到4位精度,然后将LoRA适配器附在fp16上方.

> 蒂姆·德特默斯和华盛顿大学同事于2023年5月发表了QLoRA──思想:将结结的基础模型量化到4位精度,然后在上面以fp16 添加LoRA 适配器──
...
> **NF4（Normal Float 4-bit）**专为神经网络权重设计的新数据类型――神经网络权重致重于正态分布――NF4将其16个量化级别放在标准正态分布分数上――这对正态分布数据是信息学中最优的――比率均4位量化(INT4) 或标准浮动4 损失更少的信息――

**Double quantization**量子定位本身需要记忆.每块64重量需要fp32尺度因子 (4字节).对于7B模型,这是额外的0.4GB.双量子定位将这些定位量化为fp8,降低上空成本到0.1GB.小但它加起来.

> **双重量化**对于7B模型,那就是额外的0.4GB──双重量化将这些常量化到fp8,将开销降至0.1GB──小但积累──
...
> **分页优化器**训练期间,长序列上优化器状态(Adam 动量和方差) 可能超出GPU内存.
...
> 在大多数基准上与全量微调差的1%以内.r=16的QLoRA 再损失零点几百分点.r=64的QLoRA实质上匹配全量微调同时内存少于90%──
...
> 单张消费级GPU上QLoRA 成本不到一顿午餐. 这就是2023年开源权微调社区爆发的原因,也是2026年所有训练框架默认搭载QLoRA的原因.
...
> 经验法则:研究或一次性实验 → PEFT──可重复生产管线 → 启用无内核的轴──抛弃式原型 → LLaMA-工厂──

### 融合适配器

训练后,你有两个东西:结的基模型和一个小的LoRA适配器 (通常是10-100MB).

> 训练后你有两样东西:结的基础模型和小的LoRA适配器(通常是10-100MB) 你可以:

1. **Keep them separate**根据本模型的定义,您可以使用一个模型的多个细调变体.

   **保持分离**根据一个基础模型,你可以从一个基础模型中调用多个微调变体的方法.

2. **Merge them permanently**计算W' =W + (alpha/r) *BA,并将结果保存为新的完整模型. 合并模型与原始相同的尺寸. 没有推断费用. 没有适配器可管理.

   **永久合并**计算 W' = W + (alpha/r) * BA 并将结果保存为新完整模型――合并后模型与原始相同的大小――无推理开销――无适配器需要管理――

为了完成多项任务 (客户支持适配器,代码适配器,翻译适配器),将它们分开.

> 服务多个任务 (客服适配器,代码适配器,翻译适配器) 保持分离.
...
- **DARE**和其他2023年:在合并之前随机降低适配器参数,然后重新扩展其余.
  合并前随机丢弃适配器参数并重新缩小其余参数──组合能力效果惊人──
- **Task arithmetic**简单地添加或减去适配器重量.添加"代码"适配器和"数学"适配器通常会产生一个模型在两者都很好.
  简单加减适配器权重.加代码适配器和数学适配器通常都产生了两种优秀的模型.
...
> **第二：RAG。**如果模型需要知道你的特定数据,检索比烤进权重更便宜,更可维护.

**Third: fine-tuning.**需要模型采用特定的风格,格式或推理模式,但不能通过提示实现.需要一致的结构化输出.需要将更大的模型化为更小的模型. 延迟重要,并且您无法承担一些投篮提示的额外代币.

> **第三：微调。**当你需要模型采用特定风格,格式或推理模式时,你需要一个一致的结构输出时,你需要把大模型蒸成小模型时,你需要承担不起少的样本提示的额外代币时.
...
> 我们使用纯PyTorch从零实现LoRA──无库──无魔法──你将构建LoRA层──注入模型──训练它──合并权重回去──
...
> 一 用缩放随机值初始化.B 初始化为零.乘积BA从零开始,所以模型以其原始行为开始.
...
> 首先,结模型中的所有参数――然后遍历模型树,找到匹配的目标名称的线性层,用LoRA 包装版本替换它们――LoRA A 和 B矩阵是整个模型中唯一可训练的参数――
...
> 演示创建小模型、注入LoRA到两层、训练它、合并权重回去──参数计数从全可训练降至LoRA 训练期间约1%可训练,然后合并后返回原始架构──

## 用它实现框架

通过拥抱面孔生态系统, LoRA在一个真正的模型上需要大约20条线:

> 抱脸生态,真实模型上 LoRA只需要20行:
