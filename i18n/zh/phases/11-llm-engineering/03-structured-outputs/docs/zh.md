# 结构化输出:JSON、方案验证与约束解码

> 你的LLM返回一个字符串.你的应用程序需要JSON. 这个差距已经破解了更多的生产系统比任何模型幻觉. 结构化输出是自然语言和打字数据之间的桥梁. 得到正确的,你的LLM成为可靠的API. 错误的,你在3点上通过regex解析自由文本.

> **【中文解读】**结构化输出是自然语言与类型化数据之间的桥梁,是从"聊天机器人"进化为"可靠 API"的关键技术.

> **【拓展：结构化输出→AI应用开发】**结构化输出是 AI 应用的基础.`response_format`‧人类的工具使用‧ 导师库是这个领域的核心工具──

>  **【前置】**学本节前请先掌握:(1) 阶段10·01-05(LLM 基础) 理解代号 生成;(2) JSON 方案 基础(`type`,我知道.`properties`,我知道.`required`);(3) 字符串`pydantic`库或`dataclasses`本节使用Pydantic做验证──如果不懂JSON Schema,先看jonschema.org的5分钟教程──

**Type:** Build | **类型:** 构建
**Languages:** Python | **语言:** Python
**Prerequisites:** Phase 10, Lessons 01-05 (LLMs from Scratch) | **前置知识:** Phase 10 · 01-05 (从零构建 LLM)
**Time:** ~90 minutes | **时间:** ~90 分钟
**Related:**阶段5 · 20 (结构化输出和限制式解码) 涵盖了解码级理论 (FSM/CFG逻辑处理器,概要,XGrammar).本课程重点关注生产SDK表面 (OpenAI `response_format`阅读第5阶段 · 20 首先如果你想了解API下面发生了什么.**相关:**阶段 5 · 20 (结构化输出与约束解码) 讲解码器级理论(FSM/CFG逻辑处理器、概要、XGrammar)`response_format`‧人类工具使用‧ 导师) 想了解API下发生什么 首先阅读第5阶段 · 20──

## 学习目标

- 使用OpenAI和人类API参数实现JSON模式和方案限制输出
  使用OpenAI和人类API 参数实现JSON 模式和方案 约束输出
- 建立一个拒绝错误的LLM输出和反复试验的Pydantic验证层
  构建Pydantic 验证层,拒绝格式错误的LLM 输出并通过错误反重试
- 解释限制解码如何在代币级别上有效的JSON,而不需要后处理
  解释约束解码如何在代币级强制生成有效JSON,无需后处理
- 设计强大的提取提示器,可靠地将未结构化的文本转换为打字数据结构
  设计鲁棒的提取提示,可靠地将非结构化文本转换为类型化数据结构

> **【中文解读】**本课标目标:让LLM 输出结构化数据 ((JSON、XML、表格) ⋅关键技术包括函数调用、JSON模式、约束解码――这是将LLM从聊天工具升级为工程组件的关键步骤――
...
> 你问LLM:"从这段文中提取产品名称、价格和库存状态:"它回复:

您的库存系统需要`{"product": "Sony WH-1000XM5", "price": 348.00, "in_stock": true}`您需要一个具有特定键,特定类型和特定值限制的JSON对象.

> 这是一个完全正确的答案. 但它对你的应用程序也完全不用.`{"product": "Sony WH-1000XM5", "price": 348.00, "in_stock": true}`△你需要一个具有特定键,特定类型和特定值的JSON对象.
...
> 简单的解决方案:在你的提示中加"使用JSON 回复"――这在90%的情况下有效――剩余10%的时间,模型将JSON包装在标记代码块中,或者加"这是JSON:"这样的开头语,或者因为提前关闭括号产生语法不有效的JSON――你的JSON解析器崩了――你的流线断裂了――你添加了尝试/除和重试循环――重试有时会产生不同的数据――现在你在解析问题上又有一致性问题――
...
> 这不是提示工程问题. 这是一个解码问题.模型从左到右生成代币. 在每个位置,它从10万个选项的词表中选出最可能的下一个代币. 其中大部分选项在任何给定的位置都会产生无效的JSON. 如果模型刚刚输出.`{"price":`必须是数字,引号,用于字符串.`null`,我知道.`true`,我知道.`false`或负号――其他任何东西都会产生无效的JSON――没有限制的话,模型可能会选择一个完全合理的英语单词,但在语法上是灾难性的错误――

>  **【类比】**不带约束的 LLM 输出JSON 像让人"边说边造句"说到半可能临时改变主意用别的词,结果语法错乱.约束解码 (约束解码) 像给说话的人"语法监工"每说一个词监工都检查"这能接下来吗",不能接就强制换.技术上:模型计算出逻辑后,把所有非法代币的逻辑设为 -∞,软max后概率变0,模型只能选择合法代币.

> ️ **【易错点】**结构化输出 3 个坑:**Schema 字段过多**超过20个字段模型记不住,会漏字段或填错;修复:拆成嵌套对象,每层不超过5个字段――(2) **要求 LLM 输出"创造性"字段但又强 Schema**如"创意的标题"配合`title: str`模型被设计后变得保守;修复:用`temperature=0.9`方案 中加 `min_length: 10`留余地.**没用 Pydantic 验证**直接`json.loads()`一字符串里有数字 (("348") 已成str而不是浮动;使用Pydantic自动强制类型转换.

## 概念的核心概念

> **【中文解读】**结构化输出是让LLM 生成 JSON、XML等格式可控输出――关键技术:函数调用(函数调用) 让模型输出预定义的 JSON 模式,JSON 强制模型生成合法 JSON,约束解码(限制解码) 在代币级别保证输出格式――

>  **【困惑】**问:开放AI 的`response_format={"type": "json_object"}`和 `response_format={"type": "json_schema", ...}`有什么区别? A: 前者是"JSON模式"保证输出合法JSON,但不保证字段。后者是"结构化输出"你给了JSON方案,模型保证按方案输出 实现约束解码) ⋅前者便宜但不严格,后者每次贵几分钱但100%按规则──生产环境一律使用`json_schema`模式.

> **【拓展：结构化输出的工程实践】**开放AI的结构化输出(2024) 保证模型输出严格匹配给定的JSON方案,可靠性从90%升级到100%──教师库(Python) 将自动转换Pydantic模型为JSON方案并验证输出──这是将LLM集成到生产系统的关键技术──
...
> **基于提示**("用有效的JSON 回复"):没有强制执行.模型通常会遵守,但有时不会.可靠性:约90%──失败模式:标记代码围、前言文本、截断输出、错误结构──

**JSON mode**通过 API 确保输出是有效的 JSON.`response_format: { type: "json_object" }`输出将没有错误进行分析. 但它可能不符合预期的方案 - - 额外的键,错误的类型,缺失的字段.

> **JSON 模式**通过 API 确保输出是有效的 JSON.`response_format: { type: "json_object" }`启用此功能――输出可以无错解──但它可能不符合你期望的方案余键、错误类型、缺失字段──
...
> **Schema 模式**支持:OpenAI的2026年每一个主要供应商都生生`response_format: { type: "json_schema" }`‧人类的带`input_schema`子的工具使用`response_schema`△输出具有你指定的精确键,类型和约束.
...
> **约束解码**: 在生成过程中的每个代币位置,解码器屏蔽所有将产生无效输出代币.如果方案要求数字模型即将输出字母,该代币的概率就设为零.

### 约定语言

 JSON 方案是如何告诉模型 (或验证层) 输出必须有什么形状.

>  JSON Schema 是你告诉模型 (或验证层) 输出必须有什么形状的方式.
...
> 这个方案说明:输出必须是一个对象,包含字符串`product`没有负数`price`值`in_stock`和可选的字符串数组`categories`任何不匹配的输出都会被拒绝.

方案处理硬件:嵌套物体,有打字的物体的阵列,enums (限制一个字符串到特定值),图案匹配 (在字符串上重复),和组合器 (oneOf, anyOf,allOf多形输出).

> 方案处理复杂情况:嵌套对象、带类型项的数组、枚举(将字符串约束为特定值) 模式匹配(字符串上的正则表达式) 和组合器 ((oneOf、任何的、allOf 用于多态输出) ⋅

### 皮达因特模式

在Python中,你不用手写JSON Schema.你定义一个Pydantic模型,它为你生成了该方案.

> 在Python中,你不需要手写JSON Schema──你定义了一个Pydantic模型,它会为你生成一个方案──
...
> 这将产生与上述相同的JSON方案. 导师库 (和OpenAI的SDK) 直接接受Pydantic模型:传入模型类,返回验证的实例.

### 函数调用/工具使用

模特将一个函数调用,并将其输出到一个函数. 模特将其输出一个函数调用,并将其输出到一个函数调用. OpenAI称之为"函数调用".

> 解决相同问题的替代接口――不是要求模型直接产生JSON,而是定义带类型参数的"工具" (函数)――模型输出带有结构化参数的函数调用――OpenAI称之为"函数调用"",人类称之为"工具使用"",结果相同:结构化数据――
...
> 构建最小类到图案转换器──定义Python类,自动生成其JSON图案──
...
> 模拟约束解码──给定部分 JSON字符串和方案,确定当前位置哪些代币 类别有效──
...
> 设置,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构建,构成等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等等
...
> 开放AI的结构化输出模式在内部使用约束解码――模型生成的每个代币都保证产生匹配的Pydantic方案的输出――不需要重试――不需要验证――约束直接嵌入解码过程中――
...
> 通过工具使用实现结构化输出.模型发出一个工具调用,其结构化参数匹配输入_方案.结果相同,API接口不同.
...
- [Dong et al., "XGrammar: Flexible and Efficient Structured Generation Engine for Large Language Models" (MLSys 2025)](https://arxiv.org/abs/2411.15100)现在的最先进的语法引擎; 按下自动组合, 掩盖代币的代币为100ns/代币.
  当前最先进的语法引擎; 下推自动机编译,以约100ns/代币的速度屏蔽代币
