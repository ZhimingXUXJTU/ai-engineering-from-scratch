# Modelos de lenguaje de visión  El patrón de ViT-MLP-LLM 
...
> **【中文解读】**El visual editor transformará las imágenes en tokens, el MLP proyectará los tokens y los proyectará en el espacio de entrada del LLM, el modelo de lenguaje completará el resto de trabajo.

> **【拓展：VLM 的应用】**VLM es el núcleo de la IA:GPT-4V(图像问答) Claude Vision(文档分析) ✓ LLaVA(开源视觉对话) ✓ Google Gemini(多模态推理) ✓ En el ámbito financiero, VLM se puede utilizar para el análisis de datos, el análisis de datos, el procesamiento automático de los datos.
...
> CLIP(4°Class 18 课) le da espacio de intercambio de imágenes y texto, suficiente para hacer un despliegue de clases y de búsqueda. No puede responder "¿Hay algunos vehículos rojos en esta historia?" porque CLIP no produce texto.
...
> 视觉语言模型(VLM) Qwen3-VL、InternVL3.5、LLaVA-Next、GLM-4.6V conectará el código de imágenes de CLIP familia a un modelo de lenguaje completo―model ver imágenes añadidas y generar respuesta―en 2026 años, Open Source VLM en varios modelos基准上匹敌或击败 GPT-5 和 Gemini-2.5-Pro──
...
> Tres componentes (ViT, proyector, LLM) son la configuración estándar. La diferencia entre los modelos es en qué ViT, qué proyector, qué LLM, entrenamiento y datos de preparación. Una vez que comprendes este modelo, reemplazar cualquier componente es una operación mecánica.
...
1. **Vision encoder** un ViT pre-entrenado (CLIP-L/14, SigLIP, DINOv3 o una variante afinada).
   En inglés:**视觉编码器** Pre-entrenamiento de ViT(CLIP-L/14、SigLIP、DINOv3 o micro调变体)  produce el token de rectificación。
2. **Projector** un módulo pequeño (2-4 capas MLP, o un Q-former) que mapea los tokens de visión en la dimensión de incorporación del LLM. Aquí es donde ocurre la mayor parte del ajuste fino.
   En inglés:**投影器**小型模块 (~2 层 MLP o Q-former), será el token de visión 映射到LLM's嵌入维度──la mayor parte de las pequeñas modificaciones ocurren aquí──
3. **LLM** un modelo de lenguaje solo para decodificadores (Qwen3, Llama, Mistral, GLM, InternLM). Lea los tokens de visión + texto en secuencia, genera texto.
   En inglés:**LLM**仅解码器语言模型(Qwen3、Llama、Mistral、GLM、InternLM) ・・・顺序读取视觉 + 文本代币,生成文本。
...
> En la práctica, los programadores de vídeo y el LLM sólo entrenan proyectores para obtener miles de millones de parámetros de señales a bajo costo.
...
> 朴素投影只使用ViT 最后一层――DeepStack(Qwen3-VL) de varios ViT profundidad采样特征并堆叠――更深层携带高级语义;更浅层携带细粒度空间和纹理信息――将两者都送进LLM 弥合了"图像包含什么" (图像包含什么) y"具体在哪里" (空间定位) 之间的差距――
...
1. **Alignment** congelar ViT y LLM. Entrenar sólo el proyector en pares de captura de imagen. enseña al proyector a mapear el espacio visual en el espacio del lenguaje.
   En inglés:**对齐**结 ViT 和 LLM── sólo en imágenes-título sobre el proyector de formación superior──enseñar proyector de espacio visual maquetar a espacio de lenguaje──
2. **Pre-training** deshielo todo. Entrenamiento a gran escala de datos de imagen-texto entrelazados (500M+ pares). Construye el conocimiento visual del modelo.
   En inglés:**预训练**解一切── formación sobre imágenes y textos de gran escala  5 mil millones + 对)                                                                                                                                                                                                                                                    
3. **Instruction tuning** sintonización en triples curados (imagen, pregunta, respuesta). Enseña el comportamiento de conversación y los formatos de tareas. Esto es lo que convierte a un "LM consciente de la visión" en un asistente utilizable.
   En inglés:**指令微调**在精选的 (图像,问题,答案) 三元组上微调──教会对话行为和任务格式──这将" LM de la percepción visual" convertirse en un asistente disponible──
