...
> **【拓展：推理优化→生产部署】**vLLM, TensorRT-LLM, TGI, etc. han logrado el KV-cache management, continuidad de lote de procesamiento, atención pagada, etc.

> ¿ Qué es esto ?**【前置】**学本节前请先掌握:Fase 10·01-08(Transformer 架构、Attention);GPU 计算 vs 显存带宽概念──本节是Fase 17 (Infraestructura y Producción) de la fase 1

> ¿ Qué es esto ?**【类比】**LLM 推理 = 读书 + 写读书笔记。**Prefill**(处理 prompt) = 快速翻完书(并行扫所有代币,计算密集);**Decode**(生成) = 逐字写笔记(一次写一个字,显存带宽密集)**KV cache**= 翻书时做书签(memoró cada página de información clave, escribir notas no necesita volver a escribir);**continuous batching**= 同时给多人读书会服务 (también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido como "también conocido "también conocido como "también conocido "también conocido "también conocido "también conocido "también conocido "también conocido "también conocido "también como "también conocido "también conocido "también conocido "también en el género").
...
- Implementar conceptos de batch continuo y PagedAttention para maximizar la utilización de GPU bajo solicitudes simultáneas
  实现 continuous batch processing y PagedAttention 概念, en并发请求下最大化GPU utilization rate
- Comparar las técnicas de optimización de inferencias (cache de KV, descifrado especulativo, atención flash) y sus compensaciones de rendimiento/latencia
  Conforme el artículo 5 del Tratado de Maastricht, el Consejo Europeo de Seguridad y Seguridad (CEPC) aprobó el artículo 5 del Tratado de Maastricht.

> **【中文解读】**En este contexto, el proceso de procesamiento de los datos de la memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de memoria de
...
> Usted está en 4xA100 GPU en la implementación de Llama 3 70B── un usuario individual obtiene aproximadamente 50 tokens/s── se siente muy fluido── luego 100 usuarios simultáneamente visitan el punto de terminación── la capacidad de desembolso disminuye a 3 tokens/s── usted tiene un GPU de 25.000 dólares al mes que proporciona una respuesta más lenta que el humano打字还慢的响应──
...
> El modelo en sí mismo no cambia entre un usuario y 100 usuarios. El mismo peso, la misma estructura, la misma matemática. La variación es cómo se organiza el trabajo. La lógica simple de perder el 90% + de la GPU disponible en el cálculo.
...
> Esto no es un problema de expansión, sino de regulación.$25k 推理账单降为服务相同流量的 $5k 账单的关键──
...
> vLLM en 4xA100-80GB 上服务 Llama 3 70B, bajo并发时达到约50 tokens/sec/user, mediante continuo lote de procesamiento y PagedAttention en 100并发请求维持15-25 TPS/user── sin estos optimizos, el mismo hardware en este并发下服务 solo 5 TPS/user── la misma GPU、 el mismo modelo、 4 veces la potencia──

## El concepto central.

> **【中文解读】**El proceso de LLM tiene dos etapas muy diferentes: Previo proceso de cálculo limitado a la GPU FLOPS, decodificación de la fase de cálculo limitada a la memoria de cálculo de la CPU.

> **【拓展：vLLM 和 PagedAttention】**Introducción de PagedAttention de vLLM se administrará el caché de KV como en el almacenamiento virtual, con un tamaño fijo de almacenamiento de páginas KV hacia la cantidad, resolviendo el problema de fragmentos de almacenamiento. Esto hace que la throughput de vLLM sea de 2-4 veces mayor que el HuggingFace TGI.
...
> Cada solicitud de LLM tiene dos etapas diferentes.
...
> **预填充**处理整个输入快速――所有代币 已知,所以注意力可以在完整序列上并行计算――这是大型矩阵乘法GPU 核心保持忙碌――瓶是计算:硬件每秒能提供多少FLOPS──A100做 312 TFLOPS(BF16)──70B 模型上 4,096-代币快速的预填充在单张A100 上约需要400ms──
...
> **解码**个别生成输出代币――每个新代币 关注所有前代币,但每次前向传播只产生一个代币――权力矩阵与预填充时的尺寸相同,但你使用单向量而不是矩阵与相乘――GPU 核心在微秒内完成,然后等待下一批权重从内存到内存到内存带宽:你能以多快的速度将模型权重从HBM传输到计算单元――A100 有2TB/s带宽――FP16 的70B模型是140GB――读完整模型需要70ms这是单次解码步骤的下限度.
...
> 核心洞察:* El código de resolución es de tipo de memoria intenso, porque se lee todo el modelo para generar un solo token*── Cada una de las siguientes optimizaciones o reduce la cantidad de lectura, o aumenta cada vez que se procesa la lectura de un token 批次, o evita completamente la lectura──
...
> En el cálculo de la atención, cada token de la consulta se ocupa de todos los tokens anteriores de la clave y el valor de la velocidad. No hay almacenamiento, generar token N  necesita volver a calcular todos los N-1                                                                                                                                                                                                                                                                                                                                                                                                                                                                             
...
> El consumo de KV de Llama 3 70B es de 40 GB 缓存半张 A100 内存──100 发发用户每一个4K token 时,只需要128 GB.
...
> 静态批处理等一批 N 个请求到达,一起处理,等*所有*完成后再接受新请求―― Si una solicitud necesita 500 tokens 另一个需要 10 个,短请求在完成后置 490 个解码步骤――
...
> 连续批处理(也叫代级批处理) Instantáneamente se introducirá una nueva solicitud en cualquier solicitud después de su finalización.
...
>  La elevación de la tonelada depende de cuánta variación de longitud de salida.  Cuando la longitud media, el procesamiento continuo de lote es igual al procesamiento en lote en estado.
...
> Cada solicitud de KV 缓存 es un bloque de memoria continua. Con la llegada y salida de la solicitud, la memoria se fragmenta tal como se hace en el sistema operativo RAM fragmentación. La solicitud de los tokens de 4K requiere 1.28 GB 连续空间. Incluso si en total 2 GB de espacio, puede no tener 1.28 GB *连续* espacio.
...
> PagedAttention (en inglés) (en inglés) se utiliza el tipo de memoria virtual del sistema operativo para KV 缓存. No se distribuye un bloque continuo por solicitud, sino que se distribuye un "página" de tamaño fijo. Normalmente, cada página tiene 16 tokens.
...
> Se trata de un proceso de generación de un token, de regreso a otro, de generación de otro. Pero si puedes adivinar los siguientes 5 tokens, ¿puedes probar todo?
...
> 投机解码使用小而快的**草稿模型**生成 K 个候选标签──**目标模型**Luego, en un solo período de transmisión, se procesan todos los K 个候选人 (en su caso, se ven como preemplores de líneas paralelas ∞ de cálculo intenso ∞ de alta eficiencia) ∞ Si el modelo objetivo acepta la predicción del modelo de proyecto, usted acepta todos los K 个标志在一个目标前向传播的时间内── si está en posición j no acepta, usted acepta el token 1 a j-1 y no pierde el resto.
...
> La velocidad depende de**接受率**Prevé un modelo de proyecto de leyenda Prevé un modelo de proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto de leyenda Prevé un proyecto Prevé un proyecto de leyenda Prevé un proyecto Prevé un proyecto de leyenda Prevé un proyecto Prevé un proyecto Prevé un proyecto Prevé un proyecto Prevé un proyecto de leyenda 
...
> 许多请求共享相同前──聊天机器人系统提示──RAG 上下文块──少样本示例集──没有前缓存, cada solicitud se inicia con el nuevo cálculo de estos tokens de compartir KV 缓存──
...
> Cuando se hace una nueva solicitud con un conocimiento de la fecha de llegada, el sistema copia o cita) el KV de la caja de seguridad, sólo calcula el único KV de la caja de seguridad.
...
>  Para todos los pedidos compartidos de 2,000 tokens system prompt, pre缓存 eliminó el pre-cargo de cada solicitud de aproximadamente 400ms.  En 100 pedidos/segundo, se ahorran 40 segundos por segundo de GPU 计算 超过 una GPU de trabajo
...
> 本课产 出:
> - `outputs/skill-inference-optimization.md` diagnóstico y optimización de la capacidad de la LLM                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
...
1. Modifique el perfilador de caché KV para comparar la cuantización de caché KV FP16 vs FP8 vs INT4 para Llama 3 70B en contexto 4K, compute el máximo de usuarios simultáneos para cada uno en 4xA100-80GB. La cuantización KV a INT4 debe ser aproximadamente 4 veces la capacidad del usuario.
   China Translation: Modify KV 缓存分析器 Comparar FP16 vs FP8 vs INT4 KV 缓存量化──对于4K 上下文的Llama 3 70B,计算 4xA100-80GB 上每种格式最大并发用户数──INT4 KV 量化应应大约4倍用户容量──
...
2. Extenda el simulador de lotes continuos para rastrear la utilización de la GPU (fracción de ranuras de lotes llenas por paso). Utilización de la parcela a lo largo del tiempo tanto para el lotes estático como continuo con 50 solicitudes cuyas longitudes de salida siguen una distribución de Pareto (forma=1.5, escala=20).
   China 翻译:扩展连续批处理模拟器跟踪 GPU利用率(每步批次槽位填充比例) ――绘制静态和连续批处理随时间的利用率图,50 个请求的输出长度遵循帕累托分布(形=1.5,规模=20) ――连续批处理应维持 >80%利用率──
...
3. Implementar una versión de la caché de KV de atención de consulta agrupada (GQA) donde `num_kv_heads < num_query_heads`. Llama 3 70B utiliza 64 cabezas de consulta pero sólo 8 cabezas de KV. Calcule el ahorro de memoria frente a la atención completa de múltiples cabezas (8 veces reducción en el tamaño de la caché de KV).
   Traducción:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           `num_kv_heads < num_query_heads`❖ Llama 3 70B Utiliza 64 cabezas de consulta pero sólo 8 cabezas de KV ⋅ en comparación con el cálculo completo de muchos cabezas de atención ⋅ KV 缓存大小减少8 veces) ⋅
...
4. Construir una caché de prefijos que utilice el desalojo LRU. Establezca 500 entradas máximas y genere 1.000 solicitudes donde el 60% comparte uno de los 5 prefijos comunes. Medir la tasa de hits y comparar con la caché ilimitado. Con un buen desalojo, la tasa de hits debe permanecer por encima del 55%.
   La mayoría de las entradas se han realizado en el campo de la información, pero la mayoría de las entradas se han realizado en el campo de la información.
...
5. Extenda el simulador de descifrado especulativo para implementar la especulación basada en árboles (estilo EAGLE-2). En lugar de una sola cadena de tokens de proyecto K, genere un árbol de candidatos (por ejemplo, 2 ramas en cada uno de los 3 niveles = 8 candidatos de hoja). Compara el total de tokens aceptados por ronda de verificación vs especulación lineal.
   China Translation: expand expand投机解码模拟器实现树状投机(EAGLE-2风格) ・・・ no es una sola cadena de los tokens de K 个草稿, sino que genera árboles candidatos(por ejemplo 3 niveles por nivel 2 分支 = 8 个叶选) ・・・ Compare cada ronda de pruebas aceptadas de los tokens 总数与线性投机──
