# Previsión de múltiples tokens (MTP) 更多 token 预测
...
> **【中文解读】**传统 LLM Cada posición solo predice un token. DeepSeek-V3 en cada posición aumenta una segunda pérdida.

> **【拓展：MTP→DeepSeek-V3创新】**MTP es una de las cuatro grandes innovaciones de DeepSeek-V3 (MLA + MoE + MTP + DualPipe) ⋅ al mismo tiempo mejoró el entrenamiento (MLA + MoE + MTP + DualPipe) ⋅ y la formulación (MTP) ⋅ es el modelo de "entrenamiento en inversiones ⋅ recorrido en tiempo" ⋅

> ¿ Qué es esto ?**【前置】**学本节前请先掌握:Fase 10·04(Pre-Training GPT);Fase 10·15(Dekodación especulativa)MTP 在推理时变成草稿器;因果链(causal chain) concepto──本节是Fase 10·20(DeepSeek-V3 Walkthrough) 的前置──
> ¿ Qué es esto ?**【类比】**传统下语标记 = 学英语时一次记一个单词――MTP = 同时记"这个单词"和"下一个常用搭配"训练时学更多结构(梯度信号丰富),推理时把"搭配预测器"当草稿器免费加速(80% 接受率)
...
- Indique el objetivo de formación MTP y derive la pérdida conjunta en profundidades de predicción.
  Explicar el objetivo de entrenamiento de MTP, la inducción a través de la predicción de la profundidad de pérdida conjunta
- Explica la diferencia entre las cabezas MTP paralelas de Gloeckle et al. (2024) y los módulos MTP secuenciales de DeepSeek-V3 y por qué el diseño secuencial preserva la cadena causal.
   Explicar la diferencia entre la secuencia de MTP 头和 DeepSeek-V3 模块 MTP 模块, así como por qué el orden diseño mantiene la cadena de resultados
- Calcule el parámetro y el gasto de memoria de la adición de módulos MTP a una carrera previa al entrenamiento.
   calcular en el entrenamiento previo añadir MTP  módulos de parámetros y memorias
- Implemente un módulo MTP desde cero: la incorporación compartida, el bloque de transformador por profundidad, la proyección y la cabeza de salida compartida.
  Desde cero implementar un módulo MTP: compartido de emplazamiento, por profundidad de transformador bloques, proyección y salida compartida
...
> Siguiente token 预测是标准 LLM 训练目标――每一个隐藏状态被监督预测恰好一个事:紧接下来的下一个 token――这是一个惊人的弱信号――这是一个惊人的弱信号――这是一个惊叹的弱信号――这是一个惊叹的弱信号――这是一个惊叹的弱信号――这是一个惊叹的弱信号――这是一个预测的预测.
...
> MTP 问: si cada estado oculto es supervisado y simultáneamente predice múltiples tokens futuros 会怎么?Gloeckle 等人(Meta,2024) sugiere que esto es útil. Su realización en la red de base coloca en la parte superior de la red varias salidas independientes, cada predicción tiene diferentes desplazamientos.
...
> **【中文解读】**Más de un token  predicción hacer que el modelo predica simultáneamente varios tokens futuros (en lugar de solo predecir el siguiente), proporcionando un más rico entrenamiento de señales―Los estudios de Meta muestran que más de un token  predicción no solo acelera el entrenamiento, sino que también puede mejorar la capacidad de planificación y la capacidad de raciocinio del modelo―.

> **【拓展：Meta 的多 token 预测研究】**Meta publicó un artículo en 2024 que indica que el 4-token  predicción es significativamente superior a la norma siguiente en la generación y la tarea de cálculo de código  predicción. DeepSeek-V3 también utilizó múltiples tokens auxiliares  predicción para aumentar la capacitación de los signos. Este método tiene un vínculo natural con la resolución de los conjuntos.
...
> **【拓展：多 token 预测与推理加速的联系】**Muchos de los modelos de entrenamiento de pronóstico de tokens se adaptan naturalmente a la teoría de cuentas de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de los sistemas de pronóstico de pronóstico de los sistemas de pronóstico de pronóstico de los sistemas de pronóstico de pronóstico de pronóstico de los sistemas de pronóstico de pronóstico de pronóstico de pronóstico de pronóstico de pronóstico de 5 a 10 a 10 por ciento por ciento.
...
> 本课产 出  `outputs/skill-mtp-planner.md`△给定预训运行规范(模型大小、数据、算力), que vuelve a integrarse en el plan de MTP: profundidad D、`lambda`调度、内存开销和推理时投机解码连线──
...
1. - ¿ Qué ?`code/main.py`. Muestre que la pérdida por profundidad disminuye monótona a medida que la señal sintética se fortalece. Modifique la sintética para usar un patrón fijo y verifique la convergencia de las pérdidas de profundidad 1 y profundidad 2.
   Traducción:运行`code/main.py` mostrar cada pérdida de profundidad con la señal de síntesis de aumento de un solo modo de entrega.
...
2. Compute el coste de un modelo 70B denso (oculto 8192, 80 capas) con módulo D=1 MTP. Compara con el coste de 14B reportado por DeepSeek-V3. Explique por qué el número de DeepSeek es mayor: el bloque de transformador MTP hereda la misma estructura MoE, inflando el recuento de parámetros por módulo.
   En el caso de los módulos de MTP, el número de módulos de MTP es de aproximadamente un millón de unidades.
...
3. Implementar D=2 en el juguete: añadir un segundo módulo MTP que toma h^(1) y predice `t_{i+2}`Verifique la pérdida conjunta y la contabilidad de parámetros coincide con las ecuaciones 19-21 del documento de DeepSeek.
   En el modelo de juguete se realiza D=2: añade un segundo MTP 模块, toma h^(1) 并预测 `t_{i+2}` Prueba de pérdida conjunta y cálculo de parámetros con DeepSeek 论文公式 19-21 匹配──
...
4. Cambiar el juguete a MTP paralelo (estilo de Gloeckle): añadir cabezas de salida D en la parte superior del estado oculto principal, cada uno prediciendo un desvio diferente. Medir cómo las pérdidas por profundidad se comparan con la versión secuencial en la misma señal sintética. La versión secuencial debe producir una menor pérdida de profundidad k para k > 1 porque se condiciona con las predicciones intermedias.
   China Translation:将玩具模型切换为并行 MTP(Gloeckle风格): añadir D 个输出头, en estado de ocultamiento principal, cada predicción difiere de desplazamiento.
...
5. Utilice el módulo de MTP formado como un borrador de estilo EAGLE: llama al módulo k para proponer `t_{i+k}`Si se alcanza el 50% + en el juguete, se ha reproducido la propiedad empírica MTP-as-draft.
   China 风格草稿: 在推理时调用模块 k 提出 `t_{i+k}` Medir estos tokens de rastros en la secuencia de retención para la tasa de aceptación de la predicción del modelo principal. Si el modelo de juguete alcanza el 50%+, ya se ha recuperado la característica experimental de MTP-as-draft.
