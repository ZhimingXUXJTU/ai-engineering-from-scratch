...
> **【中文解读】**文本生成图像创建新东西,图像修复(inpainting) modificar lo existente──70% de la producción de pago de la imagen se realiza en el trabajo de edición 换背景、去 logo、扩展画布、重画手──inpainting es el lugar donde se expande el modelo real钱──

> **【拓展：Photoshop 生成式填充】**Adobe Photoshop (en inglés: Adobe Photoshop) es un software de generación de imágenes de Adobe Photoshop.
...
> 客户发来一张完美产品照片,背景有分散注意的标志――你想抹去标志并保持其他像素完全一致――你不能从头运行文生图结果会有不同的颜色、光照和产品角度――你想只重生*被遮蔽区域*,并让重生的内容尊重周围上下文――

Eso es pintar.

> Éste es el proceso de pintura.

- **Inpainting.**Regenerarse dentro de una máscara, mantener fuera de los píxeles.
  **图像修复。**En el oscuro se vuelve a generar, mantener la imagen externa.
- **Outpainting.**Regenerar fuera de la máscara (o más allá del lienzo), mantenerse dentro.
  **图像扩展。**En el exterior se vuelve a generar, mantenerse dentro.
- **Image editing.**Regenerar toda la imagen pero mantener la fidelidad semántica o estructural al original (SDEdit, InstructPix2Pix).
  **图像编辑。**Re-generar toda la imagen pero mantener la coherencia de la estructura.
...
> 2026  Cada uno de los flujos de agua tiene una pintura 模式──Flux.1-Fill──SD-Inpaint、SDXL-Inpaint、DALL-E 3 Edit──原理相同──

> **【中文解读】**La pintura ([[imagenrepicio]]) es el reto central de la producción: sólo re-generar la zona cubierta, al mismo tiempo que mantener la coherencia con la infraestructura de su entorno. La práctica correcta es entrenar la U-Net modificada, aceptar 9 canales de entrada ([[imagen]] de entrada ([[imagen]] de entrada]]) 4 canales de ruido potencial + 4 canales de código de imagen + 1 canales de cobertura), hacer que el modelo vea la infraestructura de la zona cubierta.

> **【拓展：Photoshop 生成式填充与商业 Inpainting】**Adobe Photoshop generó un generativo de relleno de imágenes en la pintura, que combina modelos de expansión y tecnología de integración de bordes, permitiendo a los usuarios mover/reemplazar cualquier contenido de las imágenes a través de una orden de lenguaje natural.
...
> En cada muestra se sustituye la zona no cubierta, pues el modelo no sabe lo que hay en la zona cubierta.
...
> Damos imágenes y aumentamos el ruido a algún medio `t`, y luego con un nuevo impulso reverso hacia el ruido.`t`La libertad de elección y la libertad de creación:

- `t/T = 0.3`→ casi idéntico a la fuente, pequeños cambios estilísticos / 几乎与源相同,小风格变化
- `t/T = 0.6`→ modificaciones moderadas, conserva la estructura gruesa / 中等编辑,保留粗结构
- `t/T = 0.9`→ generado a partir de ruido cercano, conservación de la fuente mínima / 从近噪声生成,最小源保留
...
> En el`(输入图像, 指令, 输出图像)`Tres grupos de la distribución de la información.
...
> SAM(Meta de Segmento Cualquier cosa) + 扩散 inpaint 是 2026 年的背景移除流水线──SAM 2(2024)支持视频──
