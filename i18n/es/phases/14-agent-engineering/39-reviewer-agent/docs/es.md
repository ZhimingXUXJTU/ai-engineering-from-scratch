# Agente de la revisión: Constructor separado de Marker 审查者
...
> ¿ Qué es esto ?**【前置】**学本节前 請先掌握:Fase 14·38(Verificación Gate) ⋅本节是门的LLM 版用第二代理 审查第一代理的产出──
> ¿ Qué es esto ?**【类比】**El autor escribe el primer libro fácilmente y claramente, el editor es un punto de vista de la lectura.
...
> Usted hace que el agente modifique un error. Edita cuatro documentos, ejecuta un test, realiza un informe.`passed: true`Dos días después descubres que la reparación sólo resolvió la mitad del error.
...
> 验收是必要的,但不充分的──评审者问验收无法问的问题: ¿Ha resuelto correctamente el problema? ¿Ha ampliado su alcance sin marcar? ¿Recordó la hipótesis de que debería ser cuestionado? ¿Será que la plataforma de trabajo permanecerá en el estado de que pueda ser contactada en la próxima reunión?


> **【中文解读】**El inspector agente es un agente independiente que se dedica a revisar el trabajo de otros agentes. El inspector es: 1) el código de calidad, la legibilidad, la capacidad de mantenimiento, la capacidad de funcionamiento, 2) la exactitud, la lógica, la errores, las condiciones de frontera, el tratamiento erróneo, 3) la seguridad, la vulnerabilidad, la vulnerabilidad y la filtración de datos.
...
> 审查者 Agent (revisor agent) especializado en revisar las exportaciones de otros agentes.
