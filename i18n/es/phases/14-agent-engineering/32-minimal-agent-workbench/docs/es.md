# El trabajo de los agentes mínimos está en el escritorio.
...
> ¿ Qué es esto ?**【前置】**学本节前Permanecer el dominio:Fase 14·31(Bens de trabajo ¿qué necesita)。本节实现 mínimo disponible de trabajo:3 个文件(root router、state、taskboard)。

> ¿ Qué es esto ?**【类比】**Último escritorio = 厨房最基础的 3 件物品:菜单板(root router,告诉厨师今天做什么) ✓备菜笔记(state,记录已经切了什么) ✓等办清单(taskboard,还剩什么没做) ✓ Claude Code、Cursor等 Agent 工具底层都是这3 件套的扩展`CLAUDE.md`Es el router,`memory/`Es el estado,`tasks/`Es el tablero de tareas.
...
> La mayoría de los equipos escriben un libro de 3000 páginas.`AGENTS.md`Y lo llama para terminar en busca de trabajo. El modelo lo carga, ignora la parte que no puede resumir, sigue fracasando en la misma superficie.
...
> Necesitas una práctica opuesta. Un pequeño archivo raíz, sólo en el momento de la relación, el agente se dirigirá a un archivo más profundo.


> **【中文解读】**El objetivo es que el estudiante comprenda el papel y la interacción de cada componente.

Tres archivos, cada uno con un trabajo, cada uno lo suficientemente legible para evolucionar a un sistema real más tarde.

> Tres documentos. Cada uno tiene su propio deber. Cada uno es suficiente para que pueda ser desarrollado en un sistema real.

> El Máximo Grupo de Trabajo de Agentes es un entorno de menor grado de acción de los agentes para su experimentación y control. Proporciona un espacio de ejecución separado y un instrumento de análisis de trayectoria detallado.
...
> El contenido más largo se coloca en un archivo más profundo, sólo en el momento de la necesidad de carga.
...
> El estado existe en los archivos, porque el chat history es infactible.
...
>  El curso posterior añadirá el alcance de los acuerdos    operadores  verificación de control  revisores  check check list y enlace                                                                                                                                                                                                                                           
