# Función Llamando y herramienta Uso
...
> **【中文解读】**LLM sólo puede generar texto. Función de rotación de hacer que el modelo salga estructurado JSON.

> **【拓展：Function Calling→MCP与Agent】**Función llamada es el mecanismo central de AI Agent, el protocolo MCP estándarizado en base a la descripción de herramientas y el proceso de uso, es el protocolo básico de Claude 生态.

> ¿ Qué es esto ?**【前置】**学本节前请先掌握:(1) Fase 11·01(Prompt Engineering) 理解LLM 如何处理快速;(2) Fase 11·03(Structured Outputs) 理解JSON Schema,本节重度依赖;(3) Python 字典、JSON 序列化、try/except 异常处理──

**Type:** Build | **类型:** 构建
**Languages:** Python | **语言:** Python
**Prerequisites:** Phase 11 Lesson 03 (Structured Outputs) | **前置知识:** Phase 11 · 03 (结构化输出)
**Time:** ~75 minutes | **时间:** ~75 分钟
**Related:**Fase 11 · 14 (Modelo de Protocolo de Contexto)  cuando una herramienta se comparte entre hosts, graduarse de la llamada de funciones en línea a un servidor MCP. Esta lección cubre el caso en línea; MCP cubre el caso de protocolo.**相关:**Fase 11 · 14 (模型上下文协议)  Cuando las herramientas necesitan ser compartidas en el sistema de intercambio, de la función interna se debe utilizar la actualización para MCP 服务器。本课讲内联场景;MCP 讲协议场景。
...
- Implementar un bucle de llamadas de funciones: definir esquemas de herramientas, analizar el JSON de llamadas de herramientas del modelo, ejecutar funciones y devolver resultados
  实现 función调用循环: define herramienta schema、解析模型的工具调用 JSON、 ejecutar función并返回结果
- Esquemas de herramientas de diseño con descripciones claras y parámetros tipografados que el modelo pueda invocar confiablemente
  Diseño con una descripción clara y un esquema de herramientas de los parámetros de clasificación, para que el modelo pueda ser configurado con confianza
- Construir un bucle de agente de múltiples vueltas que enlace múltiples llamadas de función para responder consultas complejas
  Construir un agente de múltiples rondas  ciclo, cadena  recurrir a múltiples funciones para responder a preguntas complejas
...
> **【中文解读】**Este es el modelo de construcción de la base de un agente de IA mediante la búsqueda de datos, bases de datos, API, etc. para obtener información y ejecutar operaciones.
...
> Ustedes han construido un chatbot.
...
> Un modelo de respuesta con una declaración de responsabilidad.
...
> Es una fantasía de libertad de expresión. El modelo no sabe el tiempo, ni nunca lo sabrá. El tiempo cambia cada hora.
...
> La respuesta correcta es que el modelo no puede utilizar la API OpenWeatherMap, tu código puede, pero falta el protocolo estructurado.
...
> Esto es la función de la configuración. El modelo de salida estructurada JSON describe qué función de la aplicación de la función de la configuración.

Sin la llamada de la función, LLM son enciclopedias.

> No hay función, LLM es un libro.

> ¿ Qué es esto ?**【类比】**LLM 像一位"嘴强王者"能讲清楚任何概念,但不能动手──函调用就是给这个嘴强王者配一个"小弟"系统: se dice "小弟,去查东京天气"→小弟照做→回来报告"18度阴天"→ se转述给用户──模型从不离开王座(生成代币), pero mediante发号施令(JSON) y recibir el ejemplar 战报(tool_resultate), se puede调用 todo el mundo exterior──

## El concepto central.

> **【中文解读】**Función llamada) hacer que LLM se desarrolle una solicitud de consulta de herramientas estructuradas, en lugar de un texto puro.

> **【拓展：函数调用与 Agent 系统】**La función de OpenAI se utiliza en el año 2023 lanzado, ya está apoyado y se utiliza y se utiliza de forma obligatoria.
...
> Cada vez que se utiliza el instrumento, se sigue el mismo ciclo de cinco pasos.
...
> Paso 1: usuario envía mensajes. Paso 2: modelo recibe mensajes y define herramientas. Paso 3: el modelo no regresa al texto, sino que la herramienta de salida se utiliza para una estructura JSON que contiene nombres de funciones y elementos. Paso 4: tu código ejecuta la función y captura el resultado. Paso 5: el resultado vuelve al modelo, el modelo ahora tiene datos reales para generar la respuesta final.

El modelo nunca ejecuta nada, sólo decide qué llamar y con qué argumentos.

> El modelo nunca ejecuta nada. Sólo decide lo que se utiliza.

> ¿ Qué es esto ?**【困惑】**P: ¿Por qué el modelo no ejecuta directamente el código? así no se pierde un paso?**隔离** El modelo en la caja fuera de ejecución tiene riesgos de seguridad                                                                                                                                                                                                                                                        **可观测** ejecutar en su proceso, puede aumentar el diario ∞**可移植**The same model can drive different languages (Python/JS/Go) The same model can drive different languages (Python/JS/Go) The same model can drive different languages (Python/JS/Go) The same model can drive different languages (Python/JS/Go) The same model can drive different languages (Python/JS/Go) The same model can drive different languages) The same model itself is not tied to the operation of the work.
...
> Cada herramienta está definida por JSON Schema, le dice al modelo qué hacer esta función, aceptar qué parámetros, parámetros deben ser de qué tipo.
...
> ️ **【易错点】**工具描述的 3 个坑:(1) **描述太短**"obtenir datos" de esta descripción, modelo分不清该使用 `get_weather`¿ Qué es eso ?`get_stock_price`,会乱选;修复: cada descripción al menos 30 字,写清"做什么 + 输入 + 输出"――(2) **描述互相重叠** dos instrumentos están escribiendo "obtenir información", modelo seleccionar cualquiera con todo su potencial;修复:**隐藏前置条件**比如 `delete_file(path)` necesito primero `confirm()`, pero la descripción no dice, el modelo eliminará directamente;修复:把约束写入描述或拆分两个工具.

### Comparación de proveedores

Cada proveedor principal admite llamadas de funciones, pero la superficie de la API es diferente.

> Cada proveedor principal apoya la función de rotación, pero las API interfaces son diferentes.
...
> Para 2026, tres proveedores de fuente cerrada han tendido a adoptar el mismo formato basado en JSON Schema.`tools`字段匹配 OpenAI's estructura──开源权重微调模型仍然各异Hermes 格式──NousResearch) es el más común de los tres grupos de micro-modules.

### Elegir las herramientas: automáticas, requeridas, específicas

Tú controlas cuando el modelo usa herramientas.

> Puedes controlar el modelo cuándo usar las herramientas.
...
**自动（默认）**El modelo decide por sí mismo si se utiliza la herramienta o el instrumento de la respuesta directa.
...
**必需**El modelo debe utilizar al menos una herramienta. Cuando usted sabe claramente que el usuario tiene que usar la herramienta.
...
**特定函数**: el modelo de fuerza de la función específica.`tool_choice={"type":"function", "function": {"name": "get_weather"}}`Se utilizan herramientas de seguridad climática, independientemente de lo que sea la consulta.
...
> GPT-4o y Claude pueden ser utilizados en una sola rueda de varias funciones.
...
> Su código se ejecuta en dos partes (la situación ideal se ejecuta en dos partes), retorna dos resultados, el modelo se completa en una sola repetida.

> ️ **【易错点】**Y no se ha hecho nada.**顺序依赖未声明** usuario pregunta "previamente a empresa 股价,再查 B 公司", modelo可能并行调用两个 `get_price`, pero no puedes garantizar un regreso anticipado;`get_price`工具的描述 写明"用于独立查询",需要顺序时使用 `compare_stocks(A, B)`单工具封装──(2) **共享状态竞争**并行调用 `increment_counter()`两次,结果只加 1;修复:工具实现里加锁,或让模型串行调用副作用工具──

### Las salidas estructuradas vs. llamadas de función

La lección 03 abarcaba las salidas estructuradas.

> Lección 03 讲了结构化输出―― funciones que usan el mismo mecanismo de JSON Schema, pero con diferentes propósitos―
...
**结构化输出**El modelo de producción es el producto final.`{name, price, in_stock}`¿Qué es eso?
...
**函数调用**El modelo declara ejecutar un movimiento.`get_weather(city="Tokyo")` modelo en la solicitud de movimiento, no produce la respuesta final.
...
Para hacer extracción de datos, utilizar las funciones de salida estructuradas.
...
> 函调用是你赋予LLM 最危险的能力──模型决定执行什么── Si su conjunto de herramientas contiene consultas de base de datos, el modelo construirá consultas语句──
...
**规则 1：永远不要把模型生成的 SQL 直接传给数据库。**模型会(也会) generar DROP TABLE、UNION 注入或返回所有人的查询──始终参数化──始终校验──始终使用操作白名单──
...
**规则 2：函数白名单。**模型只能调用你明确义的函数―― nunca haga uso común de "按名执行任意函数" herramienta―Si hay 50 funciones internas, sólo exponga las 5 que el usuario necesita―
...
**规则 4：净化工具结果。**Si el instrumento devuelve datos sensibles, el modelo se encuentra en el proceso de recuperación de los resultados.
...
> ️ **【易错点】**循环失控的实战案例: modelo调 `get_weather("Tokyo")`→东京返回 "pluvie"→模型"觉得不对"→再调一次→还是雨→继续调... 5 分钟烧了200次调用──修复:(1) 全局 `max_tool_calls=20`计计器,超过即终止;(2) Continuous调调 of the same parameters same tool,3 次后强制跳出;(3) Usando el gobierno de costos de la fase 15·13 monitoring token 消耗,超值杀开──
...
> Retorna el error como resultado de herramientas de estructuración, no abandone lo anormal:
...
> ¿ Qué es esto ?**【困惑】**P: ¿Por qué no directamente lanzar las anomalías para que la capa superior trate/excepto  procesar? A: Porque lanzar las anomalías para que el agente se desplome, el modelo siempre mira no hasta el error  no sabe que el instrumento ha fallado, piensa que el éxito continúa la raciocinio, finalmente saca "ilusión respuesta"― poner el error formateado en JSON  regresar al modelo, el modelo puede ver `"error": true`Y decide el siguiente paso: cambiar los parámetros de nuevo probar, cambiar las herramientas, o decir realmente al usuario "I do not get it"― esta es la base de mi propia corrección del agente―.
...
> MCP es un estándar abierto de Anthropic, utilizado para la interacción de herramientas. MCP proporciona un protocolo general: herramientas de MCP  servidor proporcionadas por MCP  clientela (como Claude Code, Cursor o tu aplicación) consumen, y no cada aplicación define su propia herramienta.
...
> Un MCP  servidor puede dirigirse a cualquier dispositivo de exposición de clientes  Postgres MCP  servidor a cualquier agente de MCP  compatibilidad de datos  Datobase acceso derechos  GitHub MCP  servidor a cualquier agente  almacenamiento de acceso derechos  Instrumento definido una vez, hasta donde se utilice 
...
> MCP 之于函数调用,就像HTTP 之于网络──它标准化传输层,使工具变得可移植── hace que el MCP 之于函数调用, semejante a HTTP 之于网络── sea una normalización de la transmisión de la capa, haciendo que los instrumentos se vuelvan transportables──

> ¿ Qué es esto ?**【前置】**¿Cuándo se debe utilizar la función de actualización de MCP? Tres señales: 1) 工具超过 10 个,快装不下; 2) 同一工具要在多个代理框架中共享; 3) 工具有独立维护团队,需要版本管理;;学到了阶段11·14(MCP) 和阶段13·06-18 后,你就能把工具做独立服务器,Agent 通过协议消费;;
...
> Construir calculador, búsqueda de clima, búsqueda de redes, simulador, lector de documentos y operador de códigos.
...
> Es el motor central. El modelo de simulación decide qué herramienta utilizar.
...
> Construir un equipo de pruebas, en ejecución previo a la ejecución de JSON Schema 检查工具调用参数──
...
> OpenAI poner herramientas en uso como `response.choices[0].message.tool_calls`返回── cada uno está en su lugar.`id`, para devolver el resultado debe incluir. El modelo utiliza este ID, para que el resultado se ajuste al uso.
...
> Antropic 把工具调用作为 `type: "tool_use"`El contenido de los bloques de regreso.`type: "tool_result"`Su nombre es el nombre de la persona que lo ha utilizado.`input_schema`定义工具参数,OpenAI Used `parameters`¿Qué es eso?
...
> MCP 解了工具实现和工具消费──Postgres 服务器懂 SQL──GitHub 服务器懂 API──Tu agente sólo necesita encontrar y utilizar herramientas不需要为每集成写作提供商特定代码──
...
> 本课产 出  `outputs/prompt-tool-designer.md` diseño de herramientas definidas de la muestra de la información.
...
> También producido`outputs/skill-function-calling-patterns.md` marco de decisión de desarrollo de la producción en el entorno de implementación de funciones, abarcando el diseño de herramientas, el manejo de errores, la seguridad y el modo específico del proveedor.
...
1. **Add a 6th tool: database query.**Implemente una herramienta SQL simulada con una tabla en memoria. La herramienta acepta un nombre de tabla y condiciones de filtro (no SQL crudo). Valida que el nombre de tabla está en una lista de permisos y que los operadores de filtro están restringidos a `=`¿ Qué ?`>`¿ Qué ?`<`¿ Qué ?`>=`¿ Qué ?`<=`. Regresa las filas correspondientes como JSON.
   **添加第 6 个工具：数据库查询。**Utilize内存表实现模拟 SQL 工具──工具接受表名和过条件(no es el SQL original)──校验表名在白名单中、过操作符限定为`=`¿Qué es esto?`>`¿Qué es esto?`<`¿Qué es esto?`>=`¿Qué es esto?`<=`                                                                                                                                                                                                                                                              
...
2. **Implement retry with error feedback.**Cuando una llamada de herramienta falla (por ejemplo, la ciudad no se encuentra), envíe el mensaje de error de nuevo a la función de decisión del modelo y déjalo corregir sus argumentos.
   **实现带错误反馈的重试。**Cuando el instrumento de la manipulación fracasa (como encontrar no hasta la ciudad), cuando se corre la información errónea, se corre la función de decisión del modelo para que se corre el parámetro.
...
3. **Build a multi-step agent.**Algunas consultas requieren llamadas de herramientas de cadena: "Lea el archivo de configuración y dime qué modelo está configurado, luego busca en la web el precio de ese modelo". Implemente un bucle que se ejecuta hasta que el modelo decida que no se necesitan más herramientas, pasando los resultados acumulados en cada paso de decisión. Limite a 10 iteraciones para evitar bucles infinitos.
   **构建多步 agent。**Algunas consultas necesitan herramientas de configuración en cadena:"Leer un documento de configuración me dice qué modelo ha configurado, luego buscar en la red el precio del modelo. " Realizar un ciclo, ejecutar hasta que el modelo decide que ya no necesita herramientas, transmitir los resultados acumulados a cada paso de la decisión.
...
4. **Measure tool selection accuracy.**Crea 30 consultas de prueba con nombres de herramientas esperados. ejecuta la función de decisión en las 30 y mide el porcentaje de tiempo que selecciona la herramienta correcta. Identifique qué consultas causan más confusión entre las herramientas.
   **测量工具选择准确率。**Crear 30 带预期工具名的测试查询―― para todas las 30 funciones de ejecución de la decisión, la medición de la selección de los instrumentos %── identificar cuáles son las preguntas más fáciles de confundir instrumentos──
...
5. **Implement tool call caching.**Si se llama la misma herramienta con argumentos idénticos dentro de 60 segundos, devuelva el resultado almacenado en caché en lugar de volver a ejecutarlo.`(tool_name, frozenset(args.items()))`Mide las tasas de caché en una conversación con 20 consultas.
   **实现工具调用缓存。**60 segundos en el que si el mismo instrumento utiliza el mismo parámetro, devuelve el resultado de caché en lugar de volver a ejecutarlo.`(tool_name, frozenset(args.items()))`Por ejemplo, el número de personas que han recibido una solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de la cual cualquiera de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud de solicitud
...
- [OpenAI Function Calling Guide](https://platform.openai.com/docs/guides/function-calling)-- la referencia definitiva para el uso de herramientas con GPT-4o, incluidas las llamadas paralelas, las llamadas forzadas y los argumentos estructurados
  OpenAI  función de regulación   guía  GPT-4o  herramienta de uso de autoridad referencia, incluida la formulación de regulación                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
...
- [Model Context Protocol Specification](https://modelcontextprotocol.io)-- el estándar abierto de interoperabilidad de herramientas en aplicaciones de IA, con arquitectura de servidor/cliente
  模型上下文协议规范AI 应用间工具互操作的开放标准, la adopción de la arquitectura de servidor/client
- [Schick et al., 2023 -- "Toolformer: Language Models Can Teach Themselves to Use Tools"](https://arxiv.org/abs/2302.04761)-- el documento de base sobre la formación de los LLM para decidir cuándo y cómo llamar a las herramientas externas
  Schick 等 2023 "Toolformer" Training LLM decide cuándo cómo utilizar herramientas externas
- [Patil et al., 2023 -- "Gorilla: Large Language Model Connected with Massive APIs"](https://arxiv.org/abs/2305.15334)-- ajuste delgado de los LLM para llamadas precisas de API en 1.645 API con reducción de alucinaciones
  Patil 等 2023 "Gorilla"微调 LLM en 1645  API 上准调并减少幻觉
- [Berkeley Function Calling Leaderboard](https://gorilla.cs.berkeley.edu/leaderboard.html)-- referencia en tiempo real comparando la función de llamada de precisión en GPT-4o, Claude, Gemini y modelos abiertos
  伯克利 funciones调用排行榜比较 GPT-4o、Claude、Gemini 和开源模型函数调用准确率的实时基准
- [Yao et al., "ReAct: Synergizing Reasoning and Acting in Language Models" (ICLR 2023)](https://arxiv.org/abs/2210.03629)-- el bucle de Pensamiento-Acción-Observación que es el bucle de agente externo alrededor de cada llamada de herramienta; donde termina esta lección, la Fase 14 comienza.
  Yao 等 "ReAct" (ICLR 2023) 思考-行动-observer cycle, 是每工具调用外层代理 循环;本课结束处,Phase 14 接力──
- [Anthropic — Building effective agents (Dec 2024)](https://www.anthropic.com/research/building-effective-agents)-- cinco patrones composibles (cadena de la rapidez, enrutamiento, paralelación, orquestación-trabajadores, evaluador-optimizador) construidos a partir de la primitiva de uso de herramientas únicas.
  Antropic构建有效代理(2024年 12月) basado en un solo instrumento utilizando los cinco tipos de combinación de lenguaje original (提示链、路由、并行化、编排者-工人、评估者-优化者) 
