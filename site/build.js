  ...
  for (const phase of phases) {
    for (const lesson of phase.lessons) {
      ...
      if (descriptionSourceLength >= 120 && docSeo.description.length < 120) {
        // FORK-ZH: bilingual titles and embedded Chinese annotations contain
        // long space-free runs, so truncateText's word boundary can land in
        // the 104-118 char window. Upstream throws here; the fork warns and
        // keeps building so annotated lessons still ship.
        console.warn(`SEO lesson ${relPath} has a description shorter than 120 characters (${docSeo.description.length})`);
      }
      courseEntries.push({
        path: relPath,
        ...docSeo,
